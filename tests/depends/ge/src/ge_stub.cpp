...
uint32_t aclStub::InitializePlatformInfo() { return 0; }
...
uint32_t aclStub::InitRuntimePlatformInfos(const std::string& SoCVersion) { return 0; }
...
bool aclStub::GetPlatformResWithLock(const std::string& label, std::map<std::string, std::string>& res) { return true; }

bool aclStub::GetPlatformResWithLock(const string& label, const string& key, string& val) { return true; }
...
int32_t error_message::ReportPredefinedErrMsg(const char* error_code) { return 0; }
...
MockFunctionTest::MockFunctionTest() { ResetToDefaultMock(); }
...
void MockFunctionTest::ResetToDefaultMock()
{
    ...
    ON_CALL(*this, rtDvppFree).WillByDefault([this](void* devPtr) { return aclStub::rtDvppFree(devPtr); });
    ...
    ON_CALL(*this, rtMemAllocManaged)
        .WillByDefault([this](void** ptr, uint64_t size, uint32_t flag, const uint16_t moduleId) {
            return aclStub::rtMemAllocManaged(ptr, size, flag, moduleId);
        });
    ON_CALL(*this, rtMemManagedAdvise)
        .WillByDefault([this](const void* const ptr, uint64_t size, uint16_t advise, rtMemManagedLocation location) {
            return aclStub::rtMemManagedAdvise(ptr, size, advise, location);
        });
    ON_CALL(*this, rtMemManagedGetAttr)
        .WillByDefault(
            [this](rtMemManagedRangeAttribute attribute, const void* ptr, size_t size, void* data, size_t dataSize) {
                return aclStub::rtMemManagedGetAttr(attribute, ptr, size, data, dataSize);
            });
    ON_CALL(*this, rtMemManagedGetAttrs)
        .WillByDefault([this](
                           rtMemManagedRangeAttribute* attributes, size_t numAttributes, const void* ptr, size_t size,
                           void** data, size_t* dataSizes) {
            return aclStub::rtMemManagedGetAttrs(attributes, numAttributes, ptr, size, data, dataSizes);
        });
    ON_CALL(*this, rtFree).WillByDefault([this](void* devPtr) { return aclStub::rtFree(devPtr); });
    ...
    ON_CALL(*this, rtFreeHost).WillByDefault([this](void* devPtr) { return aclStub::rtFreeHost(devPtr); });
