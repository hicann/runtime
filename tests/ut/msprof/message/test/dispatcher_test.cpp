...
TEST_F(MESSAGE_DISPATCHER_TEST, IMsgHandler_OnNewMessage)
{
    ...
    MOCK_METHOD(handler, OnNewMessage).stubs();
...
{
    GlobalMockObject::verify();
    std::shared_ptr<analysis::dvvp::message::MsgDispatcher> disp(new analysis::dvvp::message::MsgDispatcher());
    EXPECT_NE(nullptr, disp);

    // find handler
    std::shared_ptr<analysis::dvvp::message::IMsgHandler> handler(new FakeJobStartHandler());
    disp->RegisterMessageHandler<analysis::dvvp::proto::JobStartReq>(handler);
}
