...
TEST_F(HOST_PROF_MANAGER_TEST, ProcessStreamFileChunk)
{
    ...
    // fileChunkReq empty
    EXPECT_EQ(
        PROFILING_FAILED, ProfManager::ProcessStreamFileChunk(NULL, std::shared_ptr<google::protobuf::Message>()));
    ...
    auto transport =
        std::shared_ptr<analysis::dvvp::transport::IDETransport>(new analysis::dvvp::transport::IDETransport(session));
    ...
    std::vector<std::string> devices = analysis::dvvp::common::utils::Utils::Split(params->devices, false, "", ",");
    std::shared_ptr<analysis::dvvp::host::ProfTask> task(
        new analysis::dvvp::host::ProfTask(transport, devices, params));
    entry->_tasks.insert(std::make_pair(job_id, task));

    MOCKER_CPP(&analysis::dvvp::host::ProfTask::WriteStreamData).stubs().will(returnValue(PROFILING_SUCCESS));
...
TEST_F(HOST_PROF_MANAGER_TEST, ProcessFileChunkFlush)
{
    ...
    std::shared_ptr<analysis::dvvp::proto::FileChunkFlushReq> message(new analysis::dvvp::proto::FileChunkFlushReq());
...
TEST_F(HOST_PROF_MANAGER_TEST, ProcessDataChannelFinish)
{
    ...
    EXPECT_EQ(
        PROFILING_FAILED, ProfManager::ProcessDataChannelFinish(NULL, std::shared_ptr<google::protobuf::Message>()));

    std::shared_ptr<analysis::dvvp::proto::DataChannelFinish> message(new analysis::dvvp::proto::DataChannelFinish());
    // job_ctx empty
    ...
    MOCKER_CPP(&analysis::dvvp::host::ProfManager::GetTask).stubs().will(returnValue(taskFake)).then(returnValue(task));
    ...
    devices = analysis::dvvp::common::utils::Utils::Split(params->devices, false, "", ",");
    std::shared_ptr<analysis::dvvp::host::ProfTask> task2(
        new analysis::dvvp::host::ProfTask(transport, devices, params));
    entry->_tasks.insert(std::make_pair(job_id, task2));

    MOCKER_CPP(&analysis::dvvp::host::Device::PostSyncDataCtrl).stubs();
...
TEST_F(HOST_PROF_MANAGER_TEST, ReceiveStreamData)
{
    ...
    MOCKER(analysis::dvvp::host::ProfManager::ProcessDataChannelFinish).stubs().will(returnValue(PROFILING_SUCCESS));
    // null
    EXPECT_EQ(
        PROFILING_FAILED,
        Analysis::Dvvp::Transport::MsprofDataHandle::ReceiveStreamData(NULL, (void*)NULL, encoded.size()));
    // message error
    std::shared_ptr<analysis::dvvp::proto::Response> fake_message(new analysis::dvvp::proto::Response());
    std::string fake_encoded = analysis::dvvp::message::EncodeMessage(fake_message);
    EXPECT_EQ(
        PROFILING_FAILED, Analysis::Dvvp::Transport::MsprofDataHandle::ReceiveStreamData(
                              NULL, (void*)fake_encoded.c_str(), fake_encoded.size()));
    // success
    EXPECT_EQ(
        PROFILING_SUCCESS,
        Analysis::Dvvp::Transport::MsprofDataHandle::ReceiveStreamData(NULL, (void*)encoded.c_str(), encoded.size()));
}