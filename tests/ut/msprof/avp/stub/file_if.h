...
int32_t file_sys_init(void) { return 0; }
 ...
 * 返回值: 文件句柄, 失败返回NULL
 */
file_t* file_open(const char* filename, const char* mode) { return &g_openFile; }
 ...
 */
long int file_tell(file_t* file) { return 10240; }

/* 功能说明: 测试该文件的结束标识符
 * 参数: 1) file: 入参，文件句柄
 * 返回值: 0, 非end of file; 1, end of file; 如果发生错误，则返回 -1
 */
int file_eof(file_t* file) { return 0; }

/* 功能说明: 测试该文件的操作错误标识符
 * 参数: 1) file: 入参，文件句柄
 * 返回值: 0, 无异常; 非0, 发生错误
 */
int file_error(file_t* file) { return 0; }

int32_t file_dir_get_file_num(char* dir_name, uint32_t* file_num) { return 0; }

int32_t file_dir_get_list(char* dir_name, file_name_t* file_list, uint32_t file_num) { return 0; }
#endif