...
protected:
    virtual void SetUp() { MOCKER(HalGetChipVersion).stubs().will(returnValue(uint32_t(CHIP_NANO_V1))); }
...
TEST_F(ProfileParamUtest, AclJsonParamTest)
{
    ...
    char data[DEFSIZE] =
        "{\"aic_metrics\":\"PipeUtilization\",\"output\":\"./output_dir\",\"switch\":\"on\",\"task_trace\":\"on\"}";
...
TEST_F(ProfileParamUtest, AclProfileParam)
{
    ...
    EXPECT_EQ(
        strcmp(ut_profileParam.config.aicEvents, "0x406,0x305,0x600,0x601,0x602,0x603,0x604,0x605,0x606,0x607"), 0);
    ...
    EXPECT_EQ(
        strcmp(ut_profileParam.config.aivEvents, "0x406,0x305,0x600,0x601,0x602,0x603,0x604,0x605,0x606,0x607"), 0);
...
TEST_F(ProfileParamUtest, CreateMapFailed)
{
    ...
    MOCKER(JsonParse).stubs().will(returnValue((JsonObj*)NULL));
