...
class COMMON_FILE_AGEING_OPEN_TEST: public testing::Test {
protected:
    virtual void SetUp() {
    }
...
TEST_F(COMMON_FILE_AGEING_OPEN_TEST, Init2) {
...
TEST_F(COMMON_FILE_AGEING_OPEN_TEST, GetStorageLimit) {
    std::string storageDir = "/tmp";
...
TEST_F(COMMON_FILE_AGEING_OPEN_TEST, IsNeedAgeingFile) {
    MOCKER_CPP(&Analysis::Dvvp::Common::Config::ConfigManager::GetPlatformType)
        .stubs()
        .will(returnValue(Analysis::Dvvp::Common::Config::PlatformType::DC_TYPE));
    std::string storageDir = "/tmp";
...
TEST_F(COMMON_FILE_AGEING_OPEN_TEST, AppendAgeingFile) {
    GlobalMockObject::verify();
...
TEST_F(COMMON_FILE_AGEING_OPEN_TEST, AppendAgeingFile2) {
    MOCKER_CPP(&Analysis::Dvvp::Common::Config::ConfigManager::GetPlatformType)
        .stubs()
        .will(returnValue(Analysis::Dvvp::Common::Config::PlatformType::DC_TYPE));      
    std::string storageDir = "/tmp";
...
TEST_F(COMMON_FILE_AGEING_OPEN_TEST, PairInsertList) 
{
    GlobalMockObject::verify();
    MOCKER(analysis::dvvp::common::utils::Utils::GetFileSize)
        .stubs()
...
TEST_F(COMMON_FILE_AGEING_OPEN_TEST, IsLastFile) 
{
    std::string storageDir = "/tmp";
    std::string storageLimit = "1000MB";
...
TEST_F(COMMON_FILE_AGEING_OPEN_TEST, RemoveAgeingFile) 
{
    GlobalMockObject::verify();
    MOCKER(analysis::dvvp::common::utils::Utils::GetFileSize)
        .stubs()
    ...
    ageingObj.PrintAgeingFile();
    EXPECT_EQ(8, ageingObj.ageingFileList_.size());
    auto iter = ageingObj.ageingFileList_.begin();
    iter->isValid = false;
    ageingObj.RemoveAgeingFile();
}

TEST_F(COMMON_FILE_AGEING_OPEN_TEST, RemoveAgeingFile2) 
{
    GlobalMockObject::verify();
    MOCKER(analysis::dvvp::common::utils::Utils::GetFileSize)
        .stubs()
