...
TEST_F(JSON_PARSER_UTEST, JsonParserInit) {
    ...
    EXPECT_EQ(false, JsonParser::instance()->GetJsonModuleReporterSwitch(reporterId2));
    EXPECT_EQ(true, JsonParser::instance()->GetJsonModuleProfSwitch(moduleId));
    EXPECT_EQ(16384, JsonParser::instance()->GetJsonModuleReporterBufferLen(reporterId1));
    EXPECT_EQ(true, JsonParser::instance()->GetJsonChannelProfSwitch(channelId));
    EXPECT_EQ(false, JsonParser::instance()->GetJsonChannelReporterSwitch(channelId));
    EXPECT_EQ(0, JsonParser::instance()->GetJsonChannelReportBufferLen(channelId));
    EXPECT_EQ(2048, JsonParser::instance()->GetJsonChannelDriverBufferLen(channelId));
    EXPECT_EQ(100, JsonParser::instance()->GetJsonChannelPeroid(channelId));
    EXPECT_EQ(80, JsonParser::instance()->GetJsonChannelThreshold(channelId));
    JsonParser::instance()->UnInit();
}
