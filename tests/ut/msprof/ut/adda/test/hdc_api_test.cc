...
TEST_F(HDC_API_UTEST, HdcClientInit)
{
...
TEST_F(HDC_API_UTEST, HdcRead)
{
    ...
    EXPECT_EQ(IDE_DAEMON_ERROR, HdcRead(session, &buf, &recv_len, 1));
...
TEST_F(HDC_API_UTEST, HdcStorePackage_memcpy_s_failed)
{
    ...
    packet1->isLast = static_cast<int8_t>(IdeLastPacket::IDE_LAST_PACK);
...
TEST_F(HDC_API_UTEST, IdeGetDevList)
{
    ...
    uint32_t phydevNum = 1;
    MOCKER(drvGetDevNum)
        .stubs()
        .with(outBoundP(&phydevNum, sizeof(phydevNum)))
        .will(returnValue(DRV_ERROR_NO_DEVICE))
        .then(returnValue(DRV_ERROR_NONE));
    EXPECT_EQ(IDE_DAEMON_ERROR, IdeGetDevList(&devNum, devs, DEVICE_NUM_MAX));

    MOCKER(drvGetDeviceLocalIDs)
        .stubs()
        .will(returnValue(DRV_ERROR_NO_DEVICE))
        .then(returnValue(DRV_ERROR_NONE));
    EXPECT_EQ(IDE_DAEMON_ERROR, IdeGetDevList(&devNum, devs, DEVICE_NUM_MAX));
    EXPECT_EQ(IDE_DAEMON_OK, IdeGetDevList(&devNum, devs, DEVICE_NUM_MAX));
    EXPECT_EQ(phydevNum, devNum);
}

TEST_F(HDC_API_UTEST, IdeGetLogIdByPhyId)
{
    uint32_t logId = -1;
