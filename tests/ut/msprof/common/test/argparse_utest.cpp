...
TEST_F(COMMON_ARGPARSE_UTEST, add_option_test)
{
    ...
    Argparser cmd = Argparser("test cmd").AddOption("help", "Shwo this help message", "");
    ...
    cmd.AddOption(
        "option2", "option has check valid function", "", [](const std::string& value) { return ARGPARSE_OK; });
...
Argparser EXPECT_TestCmd(
    std::function<Argparser(void)> createCmd, std::vector<std::string> args, const bool expectedRet,
...
{
    auto createCmd = []() {
        Argparser cmd = Argparser("test cmd").AddOption("option1", "option help msg", "A");
        return cmd;
    };
...
{
    auto createCmd = []() {
        Argparser cmd = Argparser("test cmd").AddOption("option", "option has value range", "A", {"A", "B", "C"});
        return cmd;
    };
    EXPECT_TestCmd(createCmd, {"cmd"}, false, {"--option", "<Optional> option has value range"});
    EXPECT_TestCmd(createCmd, {"cmd", "--option=A"}, true, {});
    EXPECT_TestCmd(
        createCmd, {"cmd", "--option=D"}, false,
        {"Argument --option: invalid option value D. Please input in the range of [A|B|C]"});
}
...
TEST_F(COMMON_ARGPARSE_UTEST, check_subcommand_parse)
{
    auto createCmd = []() {
        Argparser subcmd1 = Argparser("subcmd1 description")
                                ...
                                .BindExecute([](Argparser& cmd) { return 111; });
        Argparser subcmd2 =
            Argparser("subcmd2 description").AddOption("subcmd2-option1", "subcmd2 option2 help msg", "");
        Argparser cmd = Argparser("test cmd")
    ...
    EXPECT_TestCmd(
        createCmd, {"cmd"}, false,
        {"Subcommands:", "subcmd1                           subcmd1 description",
         "subcmd2                           subcmd2 description",
         "Options:", "--option1                         <Optional> option help msg"});
    EXPECT_TestCmd(
        createCmd, {"cmd", "subcmd1"}, false,
        {"Options:", "--subcmd1-option1                 <Optional> subcmd1 option1 help msg",
         "--subcmd1-option2                 <Optional> subcmd1 option2 help msg"});
    EXPECT_TestCmd(
        createCmd, {"cmd", "subcmd2"}, false,
        {
            "Options:",
...
TEST_F(COMMON_ARGPARSE_UTEST, check_rear_app_parse)
{
    auto createCmd = []() {
        ...
        Argparser cmd =
            Argparser("test cmd").AddOption("option1", "option help msg", "").AddSubCommand("subcmd1", subcmd1);
        return cmd;
    };
    EXPECT_TestCmd(createCmd, {"cmd", "subcmd3"}, false, {"unrecognized subcommand subcmd3", "Usage:"});
    EXPECT_TestCmd(
        createCmd, {"cmd", "subcmd1", "--subcmd1-option1"}, false,
        {"[ERROR] Argument --subcmd1-option1: expected one argument"});
    EXPECT_TestCmd(
        createCmd, {"cmd", "subcmd1", "--subcmd1-option1=2"}, false, {"[ERROR] Missing rear app args.", "Usage:"});
    auto cmd =
        EXPECT_TestCmd(createCmd, {"cmd", "subcmd1", "--subcmd1-option1=2", "./user_app.sh", "--app-opt=1"}, true, {});
