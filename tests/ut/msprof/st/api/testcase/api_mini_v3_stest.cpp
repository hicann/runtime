...
TEST_F(ApiMiniV3Test, SubscribeModelTest)
{
...
TEST_F(ApiMiniV3Test, SubscribeOpTest)
{
    std::set<RunnerOpInfo> modelOpInfo;
    MOCKER(mmGetTid).stubs().will(returnValue(16586));
...
TEST_F(ApiMiniV3Test, SubscribeModelWithLoopTest)
{
    std::set<RunnerOpInfo> modelOpInfo;
    DataMgr().SetModelId(1);
    EXPECT_TRUE(RunInfer(modelOpInfo, RunModel));
