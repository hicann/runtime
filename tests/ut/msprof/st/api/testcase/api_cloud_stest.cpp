...
TEST_F(ApiCloudTest, SubscribeModelTest)
{
...
TEST_F(ApiCloudTest, SubscribeOpTest)
{
    std::set<RunnerOpInfo> modelOpInfo;
    MOCKER(mmGetTid).stubs().will(returnValue(324447));
...
TEST_F(ApiCloudTest, SubscribeModelWithLoopTest)
{
    std::set<RunnerOpInfo> modelOpInfo;
    DataMgr().SetModelId(1);
    EXPECT_TRUE(RunInfer(modelOpInfo, RunModel));
...
TEST_F(ApiCloudTest, SubscribeGraphTest)
{
    std::set<RunnerOpInfo> modelOpInfo;
    DataMgr().SetModelId(447);
    EXPECT_TRUE(RunInfer(modelOpInfo, RunModel));
    EXPECT_EQ(DataMgr().CheckSubscribeResult(modelOpInfo), 0);
}