...
class AclApiMiniV3Stest : public testing::Test {
protected:
    ...
    virtual void SetUp()
    {
        ...
        aclProfPath = "api_test_mini_v3_output" + std::to_string(random_number);;
        mkdir(aclProfPath.c_str(), 0750);
        EXPECT_EQ(deviceNum, SimulatorMgr().CreateDeviceSimulator(deviceNum, StPlatformType::MINI_V3_TYPE));
    ...
    virtual void TearDown()
    {
        ...
        EXPECT_EQ(deviceNum, SimulatorMgr().DelDeviceSimulator(deviceNum, StPlatformType::MINI_V3_TYPE));
...
TEST_F(AclApiMiniV3Stest, AclApiDefault)
{
    ...
    std::vector<std::string> deviceDataList = {"stars_soc.data", "ffts_profile.data", "ts_track.data"};
...
TEST_F(AclApiMiniV3Stest, AclApiSetConfig)
{
    uint32_t deviceIdList[1] = {devId};
    aclprofAicoreMetrics aicoreMetrics = ACL_AICORE_ARITHMETIC_UTILIZATION;
    aclprofAicoreEvents *aicoreEvents = nullptr;
    ...
    setConfig = "cpu,mem,network,disk,osrt";
    ret = aclprofSetConfig(configType, setConfig.c_str(), setConfig.size());
    EXPECT_EQ(ACL_SUCCESS, ret);

    EXPECT_EQ(PROFILING_SUCCESS, AclApiStart(config, 0));

    std::vector<std::string> deviceDataList = {"npu_mem.data", "ddr.data", "nic.data", "dvpp.data"};
    std::vector<std::string> hostDataList = {"host_cpu.data", "host_mem.data"/*, "host_disk.data", "host_network.data", "host_pthreadcall.data", "host_syscall.data"*/};
    EXPECT_EQ(0, CheckFiles(aclProfPath, deviceDataList, hostDataList));
}

TEST_F(AclApiMiniV3Stest, AclApiSetConfigHostSysUsage)
{
    uint32_t deviceIdList[1] = {devId};
    aclprofAicoreMetrics aicoreMetrics = ACL_AICORE_ARITHMETIC_UTILIZATION;
    aclprofAicoreEvents *aicoreEvents = nullptr;
...
TEST_F(AclApiMiniV3Stest, AclApiSetConfigError)
{
