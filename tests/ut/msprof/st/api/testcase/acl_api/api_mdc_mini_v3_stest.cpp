...
class AclApiMdcMiniV3Stest : public testing::Test {
protected:
    ...
    virtual void SetUp()
    {
        ...
        aclProfPath = "api_test_mdc_mini_v3_output" + std::to_string(random_number);
        ;
        mkdir(aclProfPath.c_str(), 0750);
        EXPECT_EQ(deviceNum, SimulatorMgr().CreateDeviceSimulator(deviceNum, StPlatformType::CHIP_MDC_MINI_V3));
    ...
    virtual void TearDown()
    {
        ...
        EXPECT_EQ(deviceNum, SimulatorMgr().DelDeviceSimulator(deviceNum, StPlatformType::CHIP_MDC_MINI_V3));
...
TEST_F(AclApiMdcMiniV3Stest, AclApiDefault)
{
    ...
    uint64_t dataTypeConfig = ACL_PROF_ACL_API | ACL_PROF_TASK_TIME | ACL_PROF_AICORE_METRICS | ACL_PROF_L2CACHE |
...
TEST_F(AclApiMdcMiniV3Stest, AclApiSetConfig)
{
    uint32_t deviceIdList[1] = {devId};
    aclprofAicoreMetrics aicoreMetrics = ACL_AICORE_ARITHMETIC_UTILIZATION;
    aclprofAicoreEvents* aicoreEvents = nullptr;
    ...
    std::vector<std::string> deviceDataList = {
        "npu_mem.data", "npu_module_mem.data", "ddr.data", "llc.data", "dvpp.data"};
    std::vector<std::string> hostDataList = {};
    EXPECT_EQ(0, CheckFiles(aclProfPath, deviceDataList, hostDataList));
}

TEST_F(AclApiMdcMiniV3Stest, AclApiSetConfigError)
{
