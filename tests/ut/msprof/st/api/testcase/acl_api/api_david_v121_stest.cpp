...
class AclApiDavidV121Stest : public testing::Test {
protected:
...
TEST_F(AclApiDavidV121Stest, AclApiDefault)
{
...
TEST_F(AclApiDavidV121Stest, AclApiSetConfigError)
{
    ...
    std::string opTypeCritical(256, 't'); // 256 chars is within the allowed limit
    ...
    configType = ACL_PROF_OPTYPE;
    setConfig = "MatMulV3,Index"; // valid op type string
    ret = aclprofSetConfig(configType, setConfig.c_str(), setConfig.size());
    EXPECT_EQ(ACL_SUCCESS, ret);
...
TEST_F(AclApiDavidV121Stest, AclApiSetConfig)
{
    uint32_t deviceIdList[1] = {devId};
    aclprofAicoreMetrics aicoreMetrics = ACL_AICORE_ARITHMETIC_UTILIZATION;
    aclprofAicoreEvents *aicoreEvents = nullptr;
