...
class AclApiDcStest : public testing::Test {
protected:
    ...
    virtual void SetUp()
    {
        ...
        aclProfPath = "api_test_dc_output" + std::to_string(random_number);;
        mkdir(aclProfPath.c_str(), 0750);
        EXPECT_EQ(deviceNum, SimulatorMgr().CreateDeviceSimulator(deviceNum, StPlatformType::DC_TYPE));
    ...
    virtual void TearDown()
    {
        ...
        EXPECT_EQ(deviceNum, SimulatorMgr().DelDeviceSimulator(deviceNum, StPlatformType::DC_TYPE));
...
TEST_F(AclApiDcStest, AclApiDefault)
{
    ...
    std::vector<std::string> deviceDataList = {"aicore.data", "l2_cache.data"};
...
TEST_F(AclApiDcStest, AclApiSetConfig)
{
    uint32_t deviceIdList[1] = {devId};
    aclprofAicoreMetrics aicoreMetrics = ACL_AICORE_ARITHMETIC_UTILIZATION;
    aclprofAicoreEvents *aicoreEvents = nullptr;
    ...
    std::vector<std::string> deviceDataList = {"npu_mem.data", "npu_module_mem.data", "ddr.data", "llc.data", "pcie.data", "dvpp.vpc"};
    std::vector<std::string> hostDataList = {"host_cpu.data", "host_mem.data", "host_network.data"};
    EXPECT_EQ(0, CheckFiles(aclProfPath, deviceDataList, hostDataList));
}

TEST_F(AclApiDcStest, AclApiSetConfigHostSysUsage)
{
    uint32_t deviceIdList[1] = {devId};
    aclprofAicoreMetrics aicoreMetrics = ACL_AICORE_ARITHMETIC_UTILIZATION;
    aclprofAicoreEvents *aicoreEvents = nullptr;
...
TEST_F(AclApiDcStest, AclApiSetConfigError)
{
