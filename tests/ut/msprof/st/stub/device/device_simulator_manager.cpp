    ...
    for (uint32_t i = 0; i < num; i++) {
        if (platformType == StPlatformType::CHIP_V4_1_0) {
            simulator = std::unique_ptr<DeviceSimulator>(new (std::nothrow)
                                                             StarsDeviceSimulator(static_cast<uint32_t>(platformType)));
        ...
        if (platformType == StPlatformType::CHIP_CLOUD_V3) {
            simulator = std::unique_ptr<DeviceSimulator>(new (std::nothrow)
                                                             DavidDeviceSimulator(static_cast<uint32_t>(platformType)));
            if (simulator == nullptr) {
                return 0;
            }
            devices_.emplace_back(std::move(simulator));
        ...
        if (platformType == StPlatformType::CHIP_CLOUD_V4) {
            simulator = std::unique_ptr<DeviceSimulator>(
                new (std::nothrow) DavidV121DeviceSimulator(static_cast<uint32_t>(platformType)));
            if (simulator == nullptr) {
                return 0;
            }
            devices_.emplace_back(std::move(simulator));
        ...
        if (platformType == StPlatformType::MINI_V3_TYPE) {
            simulator = std::unique_ptr<DeviceSimulator>(
                new (std::nothrow) MiniStarsDeviceSimulator(static_cast<uint32_t>(platformType)));
            if (simulator == nullptr) {
                return 0;
            }
            devices_.emplace_back(std::move(simulator));
        ...
        if (platformType == StPlatformType::CHIP_NANO_V1) {
            simulator = std::unique_ptr<DeviceSimulator>(new (std::nothrow)
                                                             NanoDeviceSimulator(static_cast<uint32_t>(platformType)));
            if (simulator == nullptr) {
                return 0;
            }
            devices_.emplace_back(std::move(simulator));
        ...
        if (platformType == StPlatformType::CHIP_5162A) {
            simulator = std::unique_ptr<DeviceSimulator>(
                new (std::nothrow) ModenaDeviceSimulator(static_cast<uint32_t>(platformType)));
            if (simulator == nullptr) {
                return 0;
            }
            devices_.emplace_back(std::move(simulator));
...
int32_t DeviceSimulatorManager::ProfChannelRead(
    uint32_t deviceId, uint32_t channelId, uint8_t* outBuffer, uint32_t bufferSize)
{
...
uint32_t DeviceSimulatorManager::GetPlatformType() { return platformType_; }
...
void DeviceSimulatorManager::GetSocSide(uint32_t* info) { *info = socSide_; }
...
int32_t DeviceSimulatorManager::HalEschedQueryInfo(
...
int32_t DeviceSimulatorManager::HalEschedWaitEvent(
    uint32_t devId, uint32_t grpId, uint32_t threadId, int timeout, struct event_info* event)
{
...
int32_t DeviceSimulatorManager::HalProfSampleDataReport(
