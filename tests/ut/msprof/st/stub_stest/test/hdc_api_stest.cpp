...
TEST_F(HDC_API_STEST, HdcServerCreate)
{
    int32_t logDevId = 1;
    drvHdcServiceType type = (drvHdcServiceType)(0x12345678);
    EXPECT_NE(HdcServerCreate(logDevId, type), nullptr);
...
int32_t g_drvHdcDes = 0;
hdcError_t drvHdcServerDestroyStub(HDC_SERVER server)
{
    g_drvHdcDes++;
    if (g_drvHdcDes == 1) {
...
TEST_F(HDC_API_STEST, HdcServerDestroy)
{
    ...
    MOCKER(drvHdcServerDestroy).stubs().will(invoke(drvHdcServerDestroyStub));

    HdcServerDestroy(nullptr);
    EXPECT_EQ(g_drvHdcDes, 0);
    HdcServerDestroy(server);
    EXPECT_EQ(g_drvHdcDes, 2);
    HdcServerDestroy(server);
    EXPECT_EQ(g_drvHdcDes, 3);
}

hdcError_t drvHdcSessionAcceptStub(HDC_SERVER server, HDC_SESSION* session)
{
    *session = (HDC_SESSION)0x12345678;
    return DRV_ERROR_NONE;
}

TEST_F(HDC_API_STEST, HdcServerAccept)
{
    HDC_SERVER server = (HDC_SERVER)(0x12345678);

    MOCKER(drvHdcSessionAccept).stubs().will(invoke(drvHdcSessionAcceptStub));
    EXPECT_EQ(HdcServerAccept(server), (HDC_SESSION)0x12345678);
}
...
TEST_F(HDC_API_STEST, IdeGetVfIdBySession)
{
    HDC_SESSION session = (HDC_SESSION)0x1234567;
    int vfId = -1;
...
TEST_F(HDC_API_STEST, IdeCreatePacket)
{
    MOCKER(memcpy_s).stubs().will(returnValue(EOK - 1)).then(returnValue(EOK));
...
TEST_F(HDC_API_STEST, IdeXrmalloc)
{
    MOCKER(memcpy_s).stubs().will(returnValue(EOK - 1)).then(returnValue(EOK));
    MOCKER(IdeXmalloc).stubs().will(returnValue((void*)123));
    MOCKER(IdeXfree).stubs().will(ignoreReturnValue());
