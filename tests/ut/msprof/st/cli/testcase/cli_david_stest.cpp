...
TEST_F(CliDavidStest, CliSysHardwareMem)
{
    ...
    const char* argv[] = {
        DAVID_OUTPUT_DIR, "--sys-hardware-mem=on", "--sys-hardware-mem-freq=10000", "--sys-devices=0"};
...
TEST_F(CliDavidStest, CliSysHardwareMemOverflow)
{
    // david: Collect HBM and DDR data
    const char* argv[] = {
        DAVID_OUTPUT_DIR, "--sys-hardware-mem=on", "--sys-hardware-mem-freq=10001", "--sys-devices=0"};
...
TEST_F(CliDavidStest, CliSysInterconnection)
{
    ...
    const char* argv[] = {
        DAVID_OUTPUT_DIR, "--sys-devices=0", "--sys-interconnection-profiling=on", "--sys-interconnection-freq=1"};
...
TEST_F(CliDavidStest, CliDvvpProfiling)
{
    ...
    std::vector<std::string> dataList = {"dvpp.data",  "dvpp.venc", "dvpp.jpege", "dvpp.vdec",
                                         "dvpp.jpegd", "dvpp.vpc",  "dvpp.png",   "dvpp.scd"};
    MsprofMgr().SetDeviceCheckList(dataList);
    EXPECT_EQ(PROFILING_SUCCESS, MsprofMgr().MsprofStartBySysMode(sizeof(argv) / sizeof(char*), argv));
}
