...
TEST_F(PROF_MSG_HANDLER_TEST, CreateResponse_success)
{
    ...
    analysis::dvvp::message::StatusInfo status_info("2", analysis::dvvp::message::SUCCESS, "CreateResponse");

    std::shared_ptr<analysis::dvvp::proto::Response> rsp(std::dynamic_pointer_cast<analysis::dvvp::proto::Response>(
...
TEST_F(PROF_MSG_HANDLER_TEST, CreateResponse_failed)
{
    ...
    analysis::dvvp::message::StatusInfo status_info("2", analysis::dvvp::message::ERR, "CreateResponse");
...
TEST_F(PROF_MSG_HANDLER_TEST, JobStartHandler)
{
    ...
    std::shared_ptr<analysis::dvvp::device::JobStartHandler> handler(
        new analysis::dvvp::device::JobStartHandler(trans));
    ...
    MOCKER_CPP(&analysis::dvvp::device::TaskManager::CreateTask).stubs().will(returnValue(job));
    handler->OnNewMessage(req);
...
TEST_F(PROF_MSG_HANDLER_TEST, JobStopHandler)
{
    ...
    MOCKER_CPP(&analysis::dvvp::device::TaskManager::GetTask).stubs().will(returnValue(job));
    handler->OnNewMessage(req);
...
TEST_F(PROF_MSG_HANDLER_TEST, ReplayStartHandler)
{
    ...
    std::shared_ptr<analysis::dvvp::device::ReplayStartHandler> handler(
        new analysis::dvvp::device::ReplayStartHandler());
    EXPECT_NE(nullptr, handler);
