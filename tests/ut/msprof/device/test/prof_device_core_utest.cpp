...
TEST_F(DEVICE_PROF_DEVICE_CORE_TEST, IdeDeviceProfileProcess_hdc_failed)
{
    ...
    MOCKER_CPP(
        ...
        std::shared_ptr<analysis::dvvp::transport::AdxTransport>(analysis::dvvp::transport::HDCTransportFactory::*)(
            HDC_SESSION session) const)
        .stubs()
...
TEST_F(DEVICE_PROF_DEVICE_CORE_TEST, IdeDeviceProfileProcess)
{
    ...
    std::shared_ptr<analysis::dvvp::transport::AdxTransport> data_tran =
        std::make_shared<analysis::dvvp::transport::HDCTransport>(client);
