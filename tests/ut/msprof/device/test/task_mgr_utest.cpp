...
{
    MOCKER_CPP(&analysis::dvvp::device::TaskManager::ClearTask).stubs();
...
TEST_F(TASK_MGR_TEST, GetTask)
{
    ...
    // task init failed
    MOCKER_CPP(&analysis::dvvp::device::ProfJobHandler::Init).stubs().will(returnValue(PROFILING_SUCCESS));
