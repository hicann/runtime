...
extern "C" {
#endif
extern int32_t ProfAclGetId(
    ProfType type, analysis::dvvp::common::utils::CONST_VOID_PTR opInfo, size_t opInfoLen, uint32_t index);
#ifdef __cplusplus
}
#endif
...
TEST_F(MSPROF_CALLBACK_HANDLER_UTEST, HandleMsprofRequestTest)
{
    ...
    MOCKER_CPP(&MsprofReporter::ReportData).stubs().will(returnValue(PROFILING_SUCCESS));
    ...
    MOCKER_CPP(&MsprofReporter::StartReporter).stubs().will(returnValue(PROFILING_SUCCESS));
    EXPECT_EQ(PROFILING_SUCCESS, handler.HandleReportRequest(MSPROF_REPORTER_INIT, nullptr, 0));

    MOCKER_CPP(&MsprofReporter::StopReporter).stubs().will(returnValue(PROFILING_SUCCESS));
    EXPECT_EQ(PROFILING_SUCCESS, handler.HandleReportRequest(MSPROF_REPORTER_UNINIT, nullptr, 0));

    MOCKER_CPP(&MsprofReporter::GetDataMaxLen).stubs().will(returnValue(PROFILING_SUCCESS));
    EXPECT_EQ(PROFILING_SUCCESS, handler.HandleReportRequest(MSPROF_REPORTER_DATA_MAX_LEN, nullptr, 0));

    MOCKER_CPP(&MsprofReporter::GetHashId).stubs().will(returnValue(PROFILING_SUCCESS));
...
TEST_F(MSPROF_CALLBACK_HANDLER_UTEST, GetHashIdTest)
{
    ...
    MOCKER_CPP(
        &analysis::dvvp::transport::HashData::GenHashId,
        uint64_t(analysis::dvvp::transport::HashData::*)(
            const std::string& module, CONST_CHAR_PTR data, uint32_t dataLen))
        .stubs()
...
TEST_F(MSPROF_CALLBACK_HANDLER_UTEST, StartReporterTest)
{
    ...
    MOCKER_CPP(&Msprofiler::Api::ProfAclMgr::IsInited).stubs().will(returnValue(false)).then(returnValue(true));
    EXPECT_EQ(PROFILING_FAILED, handler.StartReporter());
...
TEST_F(MSPROF_CALLBACK_HANDLER_UTEST, MsprofRegisterCallback)
{
    ...
    EXPECT_EQ(PROFILING_SUCCESS, MsprofRegisterCallback(0, handle));
    MOCKER_CPP(&Msprofiler::Api::ProfAclMgr::IsCmdMode).stubs().will(returnValue(true));
    MOCKER_CPP(&ProfManager::CheckIfDevicesOnline).stubs().will(returnValue(true));
    MOCKER_CPP(&Msprofiler::Api::ProfAclMgr::MsprofHostHandle).stubs().will(ignoreReturnValue());
    MOCKER_CPP(&Msprofiler::Api::ProfAclMgr::MsprofSetDeviceImpl).stubs().will(returnValue(PROFILING_SUCCESS));
...
TEST_F(MSPROF_CALLBACK_HANDLER_UTEST, ProfAclGetId)
{
    ...
    size_t ret = 0;
    MOCKER_CPP(&Msprofiler::AclApi::ProfGetModelId).stubs().will(returnValue(ret));
...
TEST_F(MSPROF_CALLBACK_HANDLER_UTEST, ValidateDataFormat_DependencyMissing)
{
    auto& mgr = Dvvp::Collect::Report::ProfReporterMgr::GetInstance();
    EXPECT_FALSE(
        mgr.ValidateDataFormat("{\"version\":1.0,\"shape\":{\"value\":\"uint64\",\"dependency\":\"missing\"}}"));
}
