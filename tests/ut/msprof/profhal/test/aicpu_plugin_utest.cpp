...
TEST_F(PROF_AICPU_PLUGIN_UTTEST, PROF_AICPU_INIT_FINAL)
{
    uint32_t configSize =
...
TEST_F(PROF_AICPU_PLUGIN_UTTEST, PROF_AICPU_GETVERSION)
{
    uint32_t version = 0;
    EXPECT_EQ(0, ProfAPI::ProfHalPlugin::instance()->ProfHalGetVersion(&version));
}

TEST_F(PROF_AICPU_PLUGIN_UTTEST, PROF_AICPU_GETVERSION_NULLPTR)
{
    uint32_t *version = nullptr;
    EXPECT_EQ(-1, ProfAPI::ProfHalPlugin::instance()->ProfHalGetVersion(version));
}

TEST_F(PROF_AICPU_PLUGIN_UTTEST, PROF_AICPU_INIT_NULLPTR)
{
