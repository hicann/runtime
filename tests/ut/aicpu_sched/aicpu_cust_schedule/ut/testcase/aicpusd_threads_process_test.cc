...
TEST_F(ComputeProcessTest, StartRegisterScheduleTaskFail)
{
    MOCKER_CPP(&ComputeProcess::RegisterScheduleTask).stubs()
        .will(returnValue(static_cast<uint32_t>(AICPU_SCHEDULE_ERROR_INNER_ERROR)));
    const int32_t ret = ComputeProcess::GetInstance().Start(0, 100, PROFILING_OPEN, 100, 0,
                                                            aicpu::AicpuRunMode::PROCESS_SOCKET_MODE);
