...
{
    EXPECT_CALL(GeExecutorStubMock::GetInstance(), GeInitialize())
        .Times(1)
...
TEST_F(AclRtTest, acl_get_version_unnormal)
{
    int32_t minorVersion;
    int32_t patchVersion;
    aclError ret = aclrtGetVersion(nullptr, &minorVersion, &patchVersion);
    EXPECT_NE(ret, ACL_SUCCESS);
}
...
{
    aclrtRunMode runmode;
    EXPECT_CALL(RuntimeStubMock::GetInstance(), rtGetRunMode(_))
        .Times(1)
...
TEST_F(AclRtTest, acl_device_unnormal)
{
    ...
    EXPECT_CALL(GeExecutorStubMock::GetInstance(), GeNofifySetDevice(_, _))
        .Times(1)
        .WillOnce(Return(ACL_ERROR_GE_PARAM_INVALID));
    ret = aclrtSetDevice(0);
    EXPECT_NE(ret, ACL_SUCCESS);
...
TEST_F(AclRtTest, acl_context_unnormal)
{
    ...
    aclrtContext curContext;
    EXPECT_CALL(RuntimeStubMock::GetInstance(), rtCtxGetCurrent(_))
        .Times(1)
        .WillOnce(Return(ACL_ERROR_RT_CONTEXT_NULL));
    ret = aclrtGetCurrentContext(&curContext);
...
TEST_F(AclRtTest, acl_stream_normal_withoutHandle)
{
    ...
    aclrtStream stream;
    EXPECT_CALL(RuntimeStubMock::GetInstance(), rtStreamCreateWithConfig(_, _))
        .Times(1)
...
TEST_F(AclRtTest, aclrtSetStreamConfigOpt_paramNull)
{
    uint32_t priority = 0;
    // handle is Null
    aclError ret = aclrtSetStreamConfigOpt(NULL, ACL_RT_STREAM_PRIORITY, &priority, sizeof(priority));
    EXPECT_NE(ret, ACL_SUCCESS);
}
...
TEST_F(AclRtTest, aclrtSetStreamConfigOpt_attrInvalid)
{
    uint32_t priority = 0;
    // attr set invalid
    aclError ret = aclrtSetStreamConfigOpt(
        (aclrtStreamConfigHandle*)0x1, (aclrtStreamConfigAttr)(ACL_RT_STREAM_PRIORITY + 1), &priority,
        sizeof(priority));
    EXPECT_NE(ret, ACL_SUCCESS);
}
...
TEST_F(AclRtTest, aclrtSetStreamConfigOpt_priority_abnormal)
{
    ...
    aclError ret = aclrtSetStreamConfigOpt(handle, ACL_RT_STREAM_PRIORITY, &priority, 0);
    EXPECT_NE(ret, ACL_SUCCESS);

    priority = priority_nine;
    ret = aclrtSetStreamConfigOpt(handle, ACL_RT_STREAM_PRIORITY, &priority, sizeof(priority));
    EXPECT_NE(ret, ACL_SUCCESS);
...
TEST_F(AclRtTest, aclrtSetStreamConfigOpt_priority_normal)
{
    ...
    EXPECT_NE(handle, nullptr);
    uint32_t priority = 0;
    aclError ret = aclrtSetStreamConfigOpt(handle, ACL_RT_STREAM_PRIORITY, &priority, sizeof(priority));
    EXPECT_EQ(ret, ACL_SUCCESS);
    ret = aclrtDestroyStreamConfigHandle(handle);
    EXPECT_EQ(ret, ACL_SUCCESS);
}
...
TEST_F(AclRtTest, aclrtSetStreamConfigOpt_workPtr)
{
    ...
    aclError ret = aclrtSetStreamConfigOpt(handle, ACL_RT_STREAM_WORK_ADDR_PTR, &workPtr, 0);
    EXPECT_NE(ret, ACL_SUCCESS);
    EXPECT_NE(handle->workptr, workPtr);

    ret = aclrtSetStreamConfigOpt(handle, ACL_RT_STREAM_WORK_ADDR_PTR, &workPtr, sizeof(workPtr));
    EXPECT_EQ(ret, ACL_SUCCESS);
...
TEST_F(AclRtTest, aclrtSetStreamConfigOpt_workSize)
{
    ...
    aclError ret = aclrtSetStreamConfigOpt(handle, ACL_RT_STREAM_WORK_SIZE, &workSize, 0);
    EXPECT_NE(ret, ACL_SUCCESS);

    ret = aclrtSetStreamConfigOpt(handle, ACL_RT_STREAM_WORK_SIZE, &workSize, sizeof(workSize));
    EXPECT_EQ(ret, ACL_SUCCESS);
    ret = aclrtDestroyStreamConfigHandle(handle);
    EXPECT_EQ(ret, ACL_SUCCESS);
}
...
TEST_F(AclRtTest, aclrtSetStreamConfigOpt_flag)
{
    ...
    EXPECT_NE(handle, nullptr);
    size_t flag = 0;
    aclError ret = aclrtSetStreamConfigOpt(handle, ACL_RT_STREAM_FLAG, &flag, 0);
    EXPECT_NE(ret, ACL_SUCCESS);

    ret = aclrtSetStreamConfigOpt(handle, ACL_RT_STREAM_FLAG, &flag, sizeof(flag));
    EXPECT_EQ(ret, ACL_SUCCESS);
    ret = aclrtDestroyStreamConfigHandle(handle);
    EXPECT_EQ(ret, ACL_SUCCESS);
}
...
TEST_F(AclRtTest, acl_stream_unnormal)
{
    ...
    uint32_t priority = 0;
    // handle is Null
    ret = aclrtSetStreamConfigOpt(NULL, ACL_RT_STREAM_PRIORITY, &priority, sizeof(priority));
    EXPECT_NE(ret, ACL_SUCCESS);
    // attr set invalid
    ret = aclrtSetStreamConfigOpt(
        (aclrtStreamConfigHandle*)0x1, (aclrtStreamConfigAttr)(ACL_RT_STREAM_PRIORITY + 1), &priority,
        sizeof(priority));
    EXPECT_NE(ret, ACL_SUCCESS);
...
TEST_F(AclRtTest, aclrtLaunchCallback_paramInvalid)
{
    ...
    EXPECT_CALL(RuntimeStubMock::GetInstance(), rtCallbackLaunch(_, _, _, _)).WillOnce(Return(ACL_ERROR_INVALID_PARAM));
...
{
    REPORT_INPUT_ERROR(
        INVALID_NULL_POINTER_MSG, ARRAY(const_cast<char*>("param")), ARRAY(const_cast<char*>("datatest")));
    const char* message = aclGetRecentErrMsg();
    ASSERT_STREQ(
        message, "EH0002: Argument datatest must not be NULL.\r\n"
...
TEST_F(AclRtTest, aclrtMalloc_policyNotSupported)
{
    ...
    ret = aclrtMalloc(
        &devPtr, size, (aclrtMemMallocPolicy)(ACL_MEM_TYPE_HIGH_BAND_WIDTH | ACL_MEM_MALLOC_NORMAL_ONLY_P2P));
    EXPECT_EQ(ret, ACL_ERROR_FEATURE_UNSUPPORTED);
}
...
TEST_F(AclRtTest, aclrtMalloc_normal_mixedMemoryType)
{
    ...
    aclError ret =
        aclrtMalloc(&devPtr, size, (aclrtMemMallocPolicy)(ACL_MEM_TYPE_HIGH_BAND_WIDTH | ACL_MEM_TYPE_LOW_BAND_WIDTH));
    EXPECT_EQ(ret, ACL_SUCCESS);
    ret = aclrtFree(devPtr);
    EXPECT_EQ(ret, ACL_SUCCESS);
}
