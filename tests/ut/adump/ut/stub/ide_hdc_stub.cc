...
hdcError_t drvHdcAllocMsg(HDC_SESSION session, struct drvHdcMsg** ppMsg, int count) { return DRV_ERROR_NONE; }
...
hdcError_t drvHdcFreeMsg(struct drvHdcMsg* msg) { return DRV_ERROR_NONE; }
...
hdcError_t drvHdcReuseMsg(struct drvHdcMsg* msg) { return DRV_ERROR_NONE; }

hdcError_t drvHdcGetMsgBuffer(struct drvHdcMsg* msg, int index, char** pBuf, int* pLen) { return DRV_ERROR_NONE; }

hdcError_t drvHdcAddMsgBuffer(struct drvHdcMsg* msg, char* pBuf, int len) { return DRV_ERROR_NONE; }

hdcError_t halHdcSend(HDC_SESSION session, struct drvHdcMsg* msg, unsigned long long flag, unsigned int timeout)
{
    return DRV_ERROR_NONE;
}
...
hdcError_t halHdcSessionConnectEx(int peer_node, int peer_devid, int host_pid, HDC_CLIENT client, HDC_SESSION* session)
{
    *session = (HDC_SESSION)0x123245678;
    return DRV_ERROR_NONE;
}
...
drvError_t drvDeviceStartupRegister(drvDeviceStartupNotify startup_callback) { return DRV_ERROR_NONE; }
...
drvError_t drvDeviceStateNotifierRegister(drvDeviceStateNotify state_callback) { return DRV_ERROR_NONE; }
...
drvError_t drvHdcEpollCtl(HDC_EPOLL epoll, int op, void* target, struct drvHdcEvent* event) { return DRV_ERROR_NONE; }

drvError_t drvHdcEpollClose(HDC_EPOLL epoll) { return DRV_ERROR_NONE; }
