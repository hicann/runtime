...
TEST_F(ADX_API_UTEST, AdxGetDeviceFileFileMsgMix)
{
    ...
    MOCKER(HdcReadNb).stubs().will(invoke(HdcReadNbStub));
...
TEST_F(ADX_API_UTEST, AdxGetSpecifiedFileCtrlFail)
{
    ...
    MOCKER(HdcReadTimeout).stubs().will(invoke(HdcReadTimeoutCtrlStub));
...
TEST_F(ADX_API_UTEST, AdxGetSpecifiedFileResponseFailWarn)
{
    ...
    MOCKER(AdxSendMsg).stubs().will(invoke(AdxSendMsgStub));
