...
protected:
    virtual void SetUp() { g_sprintf_s_flag = 0; }
...
TEST_F(IDE_CMD_UTEST, IdeCmdCommonProcess)
{
    ...
    MOCKER(Putpkt).stubs().will(returnValue(IDE_DAEMON_ERROR)).then(returnValue(IDE_DAEMON_OK));

    MOCKER(CommandRes).stubs().will(returnValue(IDE_DAEMON_ERROR)).then(returnValue(IDE_DAEMON_OK));

    MOCKER(IdeCreatePacket).stubs().will(returnValue(IDE_DAEMON_ERROR)).then(returnValue(IDE_DAEMON_OK));
...
TEST_F(IDE_CMD_UTEST, ide_cmd_common_process_succ)
{
    ...
    MOCKER(Putpkt).stubs().will(returnValue(IDE_DAEMON_OK));

    MOCKER(CommandRes).stubs().will(returnValue(IDE_DAEMON_OK));

    MOCKER(IsWhiteListCommand).stubs().with(any(), outBound(flag)).will(returnValue(true));

    MOCKER(IdeCreatePacket).stubs().will(returnValue(IDE_DAEMON_OK));
...
TEST_F(IDE_CMD_UTEST, IdeHostCmdCommandProcess)
{
    ...
    MOCKER(IdeCmdCommonProcess).stubs().will(returnValue(IDE_DAEMON_OK));
...
TEST_F(IDE_CMD_UTEST, IdeCmdLogProcess)
{
    ...
    MOCKER(IdeCmdCommonProcess).stubs().will(returnValue(IDE_DAEMON_OK));
    MOCKER(IdeCmdGetFileCommonProcess).stubs().will(returnValue(IDE_DAEMON_OK));
...
TEST_F(IDE_CMD_UTEST, IdeCmdTimeProcess)
{
    ...
    MOCKER(RemoteOpen).stubs().will(returnValue(invalid_handle)).then(returnValue(valid_handle));

    char* command = (char*)"2019-02-26";
    unsigned int dev_id = 0;

    MOCKER(IdeCmdDetectProcess).stubs().will(returnValue(IDE_DAEMON_OK));
...
TEST_F(IDE_CMD_UTEST, IdeCmdFileProcess)
{
    ...
    MOCKER(IdeCreateSendFilePath).stubs().will(returnValue(IDE_DAEMON_ERROR));
    EXPECT_EQ(IDE_DAEMON_ERROR, IdeCmdFileProcess(handle, dev_id, src_file_path, dest_file_path, cmd_req));
    GlobalMockObject::verify();

    // IdeCreatePacket failed
    MOCKER(IdeCreatePacket).stubs().will(returnValue(IDE_DAEMON_ERROR));
    EXPECT_EQ(IDE_DAEMON_ERROR, IdeCmdFileProcess(handle, dev_id, src_file_path, dest_file_path, cmd_req));
    GlobalMockObject::verify();
    ...
    // mmSocketRecv failed
    MOCKER(Putpkt).stubs().will(returnValue(IDE_DAEMON_OK));
    MOCKER(SockRecv).stubs().will(returnValue(-1));
    EXPECT_EQ(IDE_DAEMON_ERROR, IdeCmdFileProcess(handle, dev_id, src_file_path, dest_file_path, cmd_req));
    GlobalMockObject::verify();
    ...
    MOCKER(Putpkt).stubs().will(returnValue(IDE_DAEMON_OK));
    char ret = '$';
    MOCKER(SockRecv).stubs().with(any(), outBoundP((void*)&ret, sizeof(ret)), eq(1), any()).will(returnValue(1));

    MOCKER(IdeDaemon::Common::Utils::CanonicalizePath).stubs().will(returnValue(std::string("")));
    EXPECT_EQ(IDE_DAEMON_ERROR, IdeCmdFileProcess(handle, dev_id, src_file_path, dest_file_path, cmd_req));
    GlobalMockObject::verify();
    ...
    MOCKER(IdeSockSendFile).stubs().will(returnValue(IDE_DAEMON_ERROR));
    EXPECT_EQ(IDE_DAEMON_ERROR, IdeCmdFileProcess(handle, dev_id, src_file_path, dest_file_path, cmd_req));
    GlobalMockObject::verify();
    ...
    MOCKER(IdeSockSendFile).stubs().will(returnValue(IDE_DAEMON_OK));

    MOCKER(CommandRes).stubs().will(returnValue(IDE_DAEMON_ERROR));
...
TEST_F(IDE_CMD_UTEST, IdeCmdSyncProcess)
{
    ...
    MOCKER(IdeCmdFileProcess).stubs().will(returnValue(IDE_DAEMON_OK));
...
TEST_F(IDE_CMD_UTEST, IdeCmdGetTypeHelp)
{
    ...
    // test parse paras
    MOCKER(getopt_long).stubs().will(returnValue((int)IDE_ARGS_HELP)).then(returnValue(-1));
...
TEST_F(IDE_CMD_UTEST, ide_do_kmc_enc)
{
    ...
    MOCKER(EncWithoutHmacWithKMC).stubs().then(returnValue(0));
...
TEST_F(IDE_CMD_UTEST, ide_cmd_passwd)
{
    ...
    MOCKER(IdeDoKmcEnc).stubs().then(returnValue(IDE_DAEMON_OK));

    MOCKER(memset_s).stubs().will(returnValue(EOK - 1));
...
TEST_F(IDE_CMD_UTEST, IdeCmdGetType_host)
{
    ...
    // test --host
    MOCKER(getopt_long).stubs().will(returnValue((int)IDE_ARGS_HOST)).then(returnValue(-1));
...
TEST_F(IDE_CMD_UTEST, IdeCmdGetType_file)
{
    ...
    // test --file
    MOCKER(getopt_long).stubs().will(returnValue((int)IDE_ARGS_SEND_FILE)).then(returnValue(-1));
    optarg = "src.file";

    MOCKER(IdeGetFilePath).stubs().will(returnValue(IDE_DAEMON_OK));
...
TEST_F(IDE_CMD_UTEST, IdeCmdGetType_device_id)
{
    ...
    // test --device
    MOCKER(getopt_long).stubs().will(returnValue((int)IDE_ARGS_DEVICE_ID)).then(returnValue(-1));
    optarg = "1";
...
TEST_F(IDE_CMD_UTEST, IdeCmdGetType_detect)
{
    ...
    // test --device
    MOCKER(getopt_long).stubs().will(returnValue((int)IDE_ARGS_DETECT)).then(returnValue(-1));
...
TEST_F(IDE_CMD_UTEST, IdeCmdGetType_time)
{
    ...
    // test --device
    MOCKER(getopt_long).stubs().will(returnValue((int)IDE_ARGS_TIME)).then(returnValue(-1));
...
TEST_F(IDE_CMD_UTEST, IdeCmdGetTypeDump)
{
    ...
    // test --device
    MOCKER(getopt_long).stubs().will(returnValue((int)IDE_ARGS_DUMP)).then(returnValue(-1));
...
TEST_F(IDE_CMD_UTEST, IdeCmdGetType_invalid)
{
    ...
    // test --device
    MOCKER(getopt_long).stubs().will(returnValue((int)'?')).then(returnValue(-1));
...
TEST_F(IDE_CMD_UTEST, IdeCmdGetFileProcess)
{
    ...
    MOCKER(mmRealPath).stubs().will(returnValue(EN_ERR));

    MOCKER(Putpkt).stubs().will(returnValue(IDE_DAEMON_OK));
    MOCKER(IdeSockRecvFile).stubs().will(returnValue(IDE_DAEMON_ERROR));
    ...
    // CommandRes failed
    MOCKER(Putpkt).stubs().will(returnValue(IDE_DAEMON_OK));
    MOCKER(IdeSockRecvFile).stubs().will(returnValue(IDE_DAEMON_OK));
    MOCKER(CommandRes).stubs().will(returnValue(IDE_DAEMON_ERROR));
    EXPECT_EQ(IDE_DAEMON_ERROR, IdeCmdGetFileProcess(handle, dev_id, src_file_path, dest_file_path));
    GlobalMockObject::verify();
...
TEST_F(IDE_CMD_UTEST, RemoteHandle)
{
    ...
    // execute log command
    MOCKER(IdeCmdLogProcess).stubs().will(returnValue(IDE_DAEMON_OK));
    ...
    MOCKER(IdeCmdSyncProcess).stubs().will(returnValue(IDE_DAEMON_OK));
    cmd_info.fileType = IDE_ARGS_SYNC_FILE;
    EXPECT_EQ(IDE_DAEMON_OK, RemoteHandle(cmd_info));

    // execute get file command
    MOCKER(IdeCmdGetFileProcess).stubs().will(returnValue(IDE_DAEMON_OK));
    ...
    MOCKER(IdeCmdTimeProcess).stubs().will(returnValue(IDE_DAEMON_OK));
...
TEST_F(IDE_CMD_UTEST, IdeCmdTestMain)
{
    ...
    MOCKER(mmSAStartup).stubs().will(returnValue(EN_ERROR)).then(returnValue(EN_OK));
    ...
    MOCKER(IdeCmdCheckValid).stubs().will(returnValue(IDE_DAEMON_ERROR)).then(returnValue(IDE_DAEMON_OK));

    MOCKER(RemoteHandle).stubs().will(returnValue(IDE_DAEMON_ERROR)).then(returnValue(IDE_DAEMON_OK));

    MOCKER_CPP(&Adx::Manager::Config::AdxConfigManager::Init).stubs().will(returnValue(false)).then(returnValue(true));
