...
TEST_F(IDE_DAEMON_ENC_UTEST, WSECGetEntropy)
{
    ...
    MOCKER(malloc).stubs().will(returnValue((void*)NULL)).then(returnValue((void*)mall));
    MOCKER(SslRandomNumbers).stubs().will(returnValue(0)).then(returnValue(1));
...
TEST_F(IDE_DAEMON_ENC_UTEST, Fopen)
{
    ...
    MOCKER(mmRealPath).stubs().will(returnValue(EN_ERR)).then(returnValue(EN_OK));
    ...
    MOCKER(IdeXmalloc).stubs().will(returnValue(p)).then(returnValue((void*)NULL));
...
TEST_F(IDE_DAEMON_ENC_UTEST, Fclose)
{
    ...
    MOCKER(free).stubs().will(ignoreReturnValue());
...
TEST_F(IDE_DAEMON_ENC_UTEST, Fwrite)
{
    ...
    MOCKER(mmFsync).stubs().will(returnValue(EN_ERR)).then(returnValue(EN_OK));
...
TEST_F(IDE_DAEMON_ENC_UTEST, Fflush)
{
    ...
    MOCKER(mmFsync).stubs().will(returnValue(-1)).then(returnValue(0));
...
TEST_F(IDE_DAEMON_ENC_UTEST, Fremove)
{
    ...
    MOCKER(remove).stubs().will(returnValue(-1)).then(returnValue(0));
...
TEST_F(IDE_DAEMON_ENC_UTEST, Ftell)
{
    ...
    MOCKER(mmLseek).stubs().will(returnValue(-1)).then(returnValue(0));
...
TEST_F(IDE_DAEMON_ENC_UTEST, Fseek)
{
    ...
    MOCKER(mmLseek).stubs().will(returnValue(-1));
...
TEST_F(IDE_DAEMON_ENC_UTEST, Ferrno) { EXPECT_EQ(errno, Ferrno(NULL)); }
...
TEST_F(IDE_DAEMON_ENC_UTEST, UtcTime) { EXPECT_EQ(WSEC_FALSE, UtcTime(NULL, NULL)); }
...
TEST_F(IDE_DAEMON_ENC_UTEST, InitKMC)
{
    ...
    MOCKER(WsecRegFuncEx).stubs().will(returnValue(WSEC_SUCCESS + 1)).then(returnValue(WSEC_SUCCESS));

    MOCKER(WsecInitializeEx).stubs().will(returnValue(WSEC_SUCCESS + 1)).then(returnValue(WSEC_SUCCESS));
...
extern int EncWithKMCWithoutHmac(
    unsigned int encalg, const unsigned char* pln, unsigned int plnlen, unsigned char* cpr, unsigned int* retlen);
TEST_F(IDE_DAEMON_ENC_UTEST, TestEncWithKMCWithoutHmac)
{
    ...
    MOCKER(SdpEncryptEx).stubs().will(returnValue(WSEC_SUCCESS + 1)).then(returnValue(WSEC_SUCCESS));
...
TEST_F(IDE_DAEMON_ENC_UTEST, TestDecWithKMC)
{
    ...
    MOCKER(SdpDecryptEx).stubs().will(returnValue(WSEC_SUCCESS + 1)).then(returnValue(WSEC_SUCCESS));
...
{
    MOCKER(WsecFinalizeEx).stubs().will(returnValue(WSEC_SUCCESS + 1)).then(returnValue(WSEC_SUCCESS));
...
TEST_F(IDE_DAEMON_ENC_UTEST, EncWithoutHmacWithKMC)
{
    ...
    MOCKER(InitKMC).stubs().will(returnValue(-1)).then(returnValue(0));

    MOCKER(EncWithKMCWithoutHmac).stubs().will(returnValue(-1)).then(returnValue(0));
    ...
    MOCKER(mmWrite).stubs().will(returnValue(-1)).then(returnValue(BUF_MAX));
...
TEST_F(IDE_DAEMON_ENC_UTEST, DecryptExWithKMC_mmLseek)
{
    ...
    MOCKER(mmRead).stubs().will(returnValue(-1)).then(returnValue(0));
...
TEST_F(IDE_DAEMON_ENC_UTEST, DecryptExWithKMC_TestDecWithKMC)
{
    ...
    MOCKER(mmRealPath).stubs().will(returnValue(EN_OK));

    MOCKER(InitKMC).stubs().will(returnValue(0));
    ...
    MOCKER(UninitKMC).stubs().will(returnValue(-1)).then(returnValue(-1)).then(returnValue(0));
