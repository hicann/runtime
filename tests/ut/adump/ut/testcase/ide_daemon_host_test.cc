...
TEST_F(IDE_DAEMON_HOST_UTEST, IdeHostSockHostCommandProcess)
{
    ...
    MOCKER(IdeRunCmd).stubs().will(returnValue(IDE_DAEMON_ERROR));
...
TEST_F(IDE_DAEMON_HOST_UTEST, IdeHostSockFileProcess)
{
    ...
    MOCKER(IdeCheckPathIsValid).stubs().will(returnValue(0));
...
TEST_F(IDE_DAEMON_HOST_UTEST, IdeHostSockFileProcessCheckDevicePath)
{
    ...
    MOCKER(IdeCheckPathIsValid).stubs().will(returnValue(-1));
    MOCKER(IdeSockRecvFile).stubs().will(returnValue(0));
...
TEST_F(IDE_DAEMON_HOST_UTEST, IdeHostSockFileProcess_realpath_is_not_valid)
{
    ...
    MOCKER(IdeHostGetResolvedPath).stubs().will(returnValue(IDE_DAEMON_ERROR));
...
TEST_F(IDE_DAEMON_HOST_UTEST, IdeHostApiDeviceStatusProcess)
{
    ...
    MOCKER(IsChipAlive).stubs().will(returnValue(0)).then(returnValue(1));

    MOCKER(IdeHostApiGetStatus).stubs();
...
TEST_F(IDE_DAEMON_HOST_UTEST, IdeHostApiDeviceInfoProcess)
{
    ...
    MOCKER(rtGetDeviceIDs).stubs().with(outBoundP(devid_array, sizeof(devid_array))).will(returnValue(RT_ERROR_NONE));
    ...
    MOCKER(IdeSockDestroy).stubs().will(ignoreReturnValue());
...
TEST_F(IDE_DAEMON_HOST_UTEST, IdeHostApiSysVersionProcess)
{
    ...
    MOCKER(dsmi_get_version).stubs().will(returnValue(-1)).then(returnValue(0));
...
TEST_F(IDE_DAEMON_HOST_UTEST, IdeHostSockApiProcess)
{
    ...
    MOCKER(IdeHostApiDeviceStatusProcess).stubs().will(returnValue(IDE_DAEMON_ERROR));

    MOCKER(IdeHostApiDeviceInfoProcess).stubs().will(returnValue(IDE_DAEMON_OK));

    MOCKER(IdeHostApiBoardIdProcess).stubs().will(returnValue(IDE_DAEMON_OK));

    MOCKER(IdeSockWriteData).stubs().will(returnValue(IDE_DAEMON_OK));

    MOCKER(IdeHostApiSysVersionProcess).stubs().will(returnValue(IDE_DAEMON_OK));

    MOCKER(IdeSockDestroy).stubs().will(returnValue(IDE_DAEMON_OK));
...
TEST_F(IDE_DAEMON_HOST_UTEST, IdeHostSockFileSyncProcess)
{
    ...
    MOCKER(IdeHostSockFileProcess).stubs().will(returnValue(IDE_DAEMON_OK));
    MOCKER(IdeSockWriteData).stubs().will(returnValue(IDE_DAEMON_OK));
    // fopen failed
...
TEST_F(IDE_DAEMON_HOST_UTEST, IdeSendLastData)
{
    ...
    MOCKER(mmCloseFile).stubs().will(returnValue(0));

    MOCKER(IdeTransferFile).stubs().will(returnValue(IDE_DAEMON_ERROR)).then(returnValue(IDE_DAEMON_OK));
