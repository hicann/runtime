...
void UnRegisterRtFunction() { g_stubFuncSet.clear(); }
    ...
    if (coreType == 0) { // aic
        ...
        if (g_stackData.find(coreId + 25) != g_stackData.end()) {
            ...
            *stackSize = g_stackData[coreId + 25].size() + 1;
            ...
            g_stackData[coreId + 25] = std::string("core type ") + std::to_string(coreType + 25) +
                                       std::string(" core id ") + std::to_string(coreId) + " stack data";
            *stack = (void*)g_stackData[coreId + 25].data();
            *stackSize = g_stackData[coreId + 25].size() + 1;
            return RT_ERROR_NONE;
        }
