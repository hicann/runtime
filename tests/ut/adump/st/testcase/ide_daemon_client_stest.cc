...
TEST_F(IDE_CMD_STEST, ide_cmd_main_help)
{
    ...
    MOCKER(RemoteOpen).stubs().will(returnValue(valid_handle));

    MOCKER(setsockopt).stubs().will(returnValue(0));

    MOCKER(mmConnect).stubs().will(returnValue(1));
...
TEST_F(IDE_CMD_STEST, ide_cmd_main_host_cmd)
{
    ...
    const int size = 64;
    int argc = number;
    char tmp[number][size] = {
        "ide_cmd", "--host", "192.168.1.162:22118", "--hostcmd", "cd ~/HIAI_PROJECTS/test/out;./test"};
    ...
    MOCKER(SockSend).stubs().will(invoke(ide_write_ide_cmd_stub));

    MOCKER(SockRecv).stubs().will(invoke(IdeRead_ide_cmd_stub));

    MOCKER(SockConnect).stubs().will(invoke(SockConnect_stub));

    MOCKER(SslInit).stubs().will(returnValue(0));
...
TEST_F(IDE_CMD_STEST, ide_cmd_main_no_host)
{
    ...
    MOCKER(RemoteOpen).stubs().will(returnValue(invalid_handle));
...
TEST_F(IDE_CMD_STEST, ide_cmd_main_sync)
{
    ...
    MOCKER(mmLseek).stubs().will(returnValue(size));

    MOCKER(SockSend).stubs().will(invoke(ide_write_ide_cmd_stub));

    MOCKER(SockRecv).stubs().will(invoke(IdeRead_ide_cmd_stub1));
...
TEST_F(IDE_CMD_STEST, ide_cmd_main_bbox)
{
    ...
    MOCKER(stat).stubs().with(any(), outBoundP(&st, sizeof(st))).will(returnValue(0));
...
TEST_F(IDE_CMD_STEST, ide_cmd_main_get)
{
    ...
    MOCKER(realpath).stubs().will(returnValue(real_path));
    ...
    MOCKER(GetpktLen).stubs().will(invoke(GetpktLen_stub));
...
TEST_F(IDE_CMD_STEST, ide_cmd_main_get_invalid_path)
{
    ...
    const int size = 64;
    int argc = number;
    char tmp[number][size] = {"ide_cmd", "--host",     "192.168.1.162:22118",
                              "--get",   "local_path", "/var/dlog/../remote_path"};
    char* argv[number] = {0};
    int i = 0;
    for (; i < number; i++) {
        argv[i] = (char*)malloc(size);
...
TEST_F(IDE_CMD_STEST, ide_cmd_main_time_for_RemoteOpen)
{
    ...
    MOCKER(setsockopt).stubs().will(returnValue(1)).then(returnValue(0)).then(returnValue(1)).then(returnValue(0));

    MOCKER(mmSocket).stubs().will(returnValue(-1)).then(returnValue(1));

    MOCKER(IsSslClientInited).stubs().will(returnValue(0)).then(returnValue(1));

    MOCKER(mmConnect).stubs().will(returnValue(-1)).then(returnValue(1));

    MOCKER(SockConnect).stubs().will(returnValue(-1)).then(returnValue(0));

    MOCKER(SslInit).stubs().will(returnValue(1));

    MOCKER(inet_pton).stubs().will(returnValue(0)).then(returnValue(1));
...
TEST_F(IDE_CMD_STEST, ide_cmd_main_error)
{
    ...
    MOCKER(mmSACleanup).stubs().will(returnValue(EN_ERROR));
...
TEST_F(IDE_CMD_STEST, ide_cmd_main_key)
{
    ...
    MOCKER(mmSAStartup).stubs().will(returnValue(EN_OK));

    MOCKER(mmSACleanup).stubs().will(returnValue(EN_OK));

    MOCKER(tcgetattr).stubs().then(returnValue(0));

    MOCKER(tcsetattr).stubs().then(returnValue(0));
