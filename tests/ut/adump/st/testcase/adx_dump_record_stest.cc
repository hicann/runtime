...
{
    MOCKER_CPP(&std::string::empty).stubs().will(returnValue(true)).then(returnValue(false));
...
TEST_F(ADX_DUMP_RECORD_STEST, RecordDumpInfoToOptimizedMode)
{
    ...
    TensorStatsResult tensorStatsResult = {
        268435456, toolkit::dump::DT_INT64, toolkit::dump::FORMAT_ND, 3, {0}, 0, 6, {0}, 0, 1};
    ...
    std::shared_ptr<DumpChunk> dumpChunk(
        static_cast<DumpChunk*>(::operator new(dumpChunkSize)), [](DumpChunk* ptr) { ::operator delete(ptr); });
    ...
    std::shared_ptr<MsgProto> msgProto(
        static_cast<MsgProto*>(::operator new(sizeof(MsgProto) + dumpChunkSize)),
        [](MsgProto* ptr) { ::operator delete(ptr); });
    ...
    bool ret = Adx::AdxDumpRecord::Instance().RecordDumpDataToQueue(info);
    EXPECT_EQ(true, ret);
    MOCKER(Adx::FileUtils::IsDiskFull).stubs().will(returnValue(false));
    MOCKER(Adx::FileUtils::FileNameIsReal).stubs().will(returnValue(IDE_DAEMON_OK));
