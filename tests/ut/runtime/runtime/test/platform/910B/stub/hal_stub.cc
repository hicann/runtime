    ...
    if (value) {
        if (moduleType == MODULE_TYPE_SYSTEM && infoType == INFO_TYPE_VERSION) {
            *value = PLATFORMCONFIG_CLOUD_V2_910B1;
            printf(
                "\r\n halGetDeviceInfo:: moduleType = %d, infoType = %d, version value = %d", moduleType, infoType,
...
drvError_t halCqReportGet(uint32_t devId, struct halReportGetInput* in, struct halReportGetOutput* out)
{
    ...
    while ((
        __atomic_load_n(&sendCount[devId], __ATOMIC_ACQUIRE) == __atomic_load_n(&recvCount[devId], __ATOMIC_ACQUIRE))) {
    ...
    // RT_LOG(RT_LOG_INFO, "zyx drvReportGet reportPtr %u,%u,%u,%u",
    // sendCount[devId],recvCount[devId],cqSendCount[devId],sqSendCount[devId]);
...
drvError_t halMemManagedRangeGetAttributes(
...
drvError_t StubHalSqTaskSendNormalType(uint32_t devId, struct halTaskSendInfo* info)
{
    ...
    if (needCq == 0) {
        RT_LOG(
            RT_LOG_INFO, "devId=%d, sqId=%d, head=%d, tail=%d, no cqe.", devId, sqId, g_sqCqInfo[devId][sqId].sqHead,
            g_sqCqInfo[devId][sqId].sqTail);
        return DRV_ERROR_NONE;
    }
...
drvError_t halMemHostGetDevPointer(void* srcPtr, UINT32 devid, void** dstPtr) { return DRV_ERROR_NONE; }
...
unsigned int halGetMaxResMapType(void) { return 0x10; }
...
drvError_t halAsyncDmaDestroy2D(uint32_t devId, struct halAsyncDmaDestroy2DPara* para) { return DRV_ERROR_NONE; }
...
drvError_t halAsyncDmaDestroyBatch(uint32_t devId, struct halAsyncDmaDestroyBatchPara* para) { return DRV_ERROR_NONE; }
...
drvError_t halMemRetainAllocationHandle(drv_mem_handle_t** handle, void* ptr) { return DRV_ERROR_NONE; }
...
drvError_t halMemPoolMalloc(soma_mem_pool_t pool, uint64_t va, uint64_t size, int32_t policy) { return DRV_ERROR_NONE; }

drvError_t halMemPoolFree(soma_mem_pool_t pool, uint64_t va, uint64_t size, int32_t policy) { return DRV_ERROR_NONE; }

drvError_t halMemPoolSetWatermark(uint32_t deviceId, uint64_t poolId, uint64_t size) { return DRV_ERROR_NONE; }
...
DVresult halMemManagedPrefetchBatch(
