...
TEST_F(CloudV2ConfigTest, get_aiCoreSpec_test)
{
    rtError_t error = RT_ERROR_NONE;
    Config *config = new Config();
    rtAiCoreSpec_t *aiCoreSpec;
    rtPlatformType_t platformType_ = PLATFORM_910B1;
    aiCoreSpec = (rtAiCoreSpec_t *)malloc(sizeof(rtAiCoreSpec_t));

    error = config->GetAiCoreSpec(platformType_, aiCoreSpec);
...
TEST_F(CloudV2ConfigTest, get_aiCoreMemoryRates_test)
{
    rtError_t error = RT_ERROR_NONE;
    Config *config = new Config();
    ...
    error = config->GetAiCoreMemoryRates(platformType_, aiCoreMemoryRates);
    EXPECT_EQ(error, RT_ERROR_NONE);
...
TEST_F(CloudV2ConfigTest, get_memoryConfig_test)
{
    rtError_t error = RT_ERROR_NONE;
    Config *config = new Config();
    ...
    error = config->GetMemoryConfig(platformType_, memoryConfig);
    EXPECT_EQ(error, RT_ERROR_NONE);
...
TEST_F(CloudV2ConfigTest, get_aiCoreMemSizes_test)
{
    rtError_t error = RT_ERROR_NONE;
    Config *config = new Config();
    ...
    error = config->GetAiCoreMemorySizes(platformType_, aiCoreMemorySize);
    EXPECT_EQ(error, RT_ERROR_NONE);
...
TEST_F(CloudV2ConfigTest, set_aiCoreMemSizes_test)
{
    rtError_t error = RT_ERROR_NONE;
    Config *config = new Config();
    rtAiCoreMemorySize_t *aiCoreMemorySize;
    ...
    error = config->SetAiCoreMemSizes(platformType_, aiCoreMemorySize);
...
TEST_F(CloudV2ConfigTest, aicore_mem_srize)
{
    rtError_t error;
    void *ptr = NULL;
