...
TEST_F(CloudV2NpuDriverTest, ConvertUvmLocTypeToDrvUvmLocType)
{
    ...
    EXPECT_EQ(
        UvmCallback::ConvertUvmLocTypeToDrvUvmLocType(rtMemLocationTypeHostNuma), DRV_UVM_LOCATION_TYPE_HOST_NUMA);
    EXPECT_EQ(
        UvmCallback::ConvertUvmLocTypeToDrvUvmLocType(rtMemLocationTypeHostNumaCurrent),
...
TEST_F(CloudV2NpuDriverTest, huge_page_managed_memory_failed_03)
{
    ...
    MOCKER(halMemAlloc).stubs().will(returnValue(DRV_ERROR_OUT_OF_MEMORY));
...
{
    rtError_t error;
    MOCKER_CPP(&NpuDriver::CheckIsSupportFeature).stubs().will(returnValue(false));
