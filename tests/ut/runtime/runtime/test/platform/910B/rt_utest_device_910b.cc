...
class DeviceTest910b : public testing::Test
{
protected:
...
uint32_t DeviceTest910b::g_case_num = 0;
uint32_t DeviceTest910b::g_streamId = 0;
uint16_t DeviceTest910b::g_sId = 0;
uint16_t DeviceTest910b::g_tId = 0;
uint32_t DeviceTest910b::g_printType = 0;


TEST_F(DeviceTest910b, device_error_proc1)
{
    ...
    errorProc->fastRingBufferAddr_ = malloc(1);
    error = errorProc->SendTaskToStopUseRingBuffer();
    EXPECT_EQ(error, RT_ERROR_NONE);
...
TEST_F(DeviceTest910b, device_error_proc2)
{
    Device* device = ((Runtime *)Runtime::Instance())->DeviceRetain(0, 0);
    DeviceErrorProc *errorProc = new DeviceErrorProc(device);
    char* hostAddr = (char*)malloc(16);

    rtError_t error = errorProc->ProcRingBufferTask(hostAddr, false, 1);
    EXPECT_EQ(error, RT_ERROR_NONE);

    errorProc->deviceRingBufferAddr_ = hostAddr;
...
TEST_F(DeviceTest910b, device_error_report_ringbuffer_01)
{
    uint16_t streamId;
    Device* device = ((Runtime *)Runtime::Instance())->DeviceRetain(0, 0);
    DeviceErrorProc *errorProc = new DeviceErrorProc(device);
...
TEST_F(DeviceTest910b, device_error_clean_ringbuffer_01)
{
    Device* device = ((Runtime *)Runtime::Instance())->DeviceRetain(0, 0);
    DeviceErrorProc *errorProc = new DeviceErrorProc(device);
    char* hostAddr = (char*)malloc(16);
...
TEST_F(DeviceTest910b, device_error_clean_ringbuffer_02)
{
    Device* device = ((Runtime *)Runtime::Instance())->DeviceRetain(0, 0);
    DeviceErrorProc *errorProc = new DeviceErrorProc(device);
    char* hostAddr = (char*)malloc(16);
...
TEST_F(DeviceTest910b, PrintSnapshotInfo)
{
    Device* device = ((Runtime *)Runtime::Instance())->DeviceRetain(0, 0);
...
TEST_F(DeviceTest910b, PrintStreamTimeoutSnapshotInfo_test)
{
    Device* device = ((Runtime *)Runtime::Instance())->DeviceRetain(0, 0);
    device->SetSnapshotFlag(false);
    DeviceErrorProc *errorProc = new DeviceErrorProc(device);
    ...
    char* hostAddr = (char*)malloc(16);
    error = errorProc->ProcRingBufferTask(hostAddr, false, 1);
    EXPECT_EQ(error, RT_ERROR_NONE);
...
TEST_F(DeviceTest910b, PrintStreamTimeoutSnapshotInfo_add)
{
    Device* device = ((Runtime *)Runtime::Instance())->DeviceRetain(0, 0);
    device->SetSnapshotFlag(false);
    DeviceErrorProc *errorProc = new DeviceErrorProc(device);
...
TEST_F(DeviceTest910b, getDieNum_failed)
{
    rtError_t error;
    int32_t devId = 1;
