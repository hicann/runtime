...
TEST_F(CloudV2ApiKernelTest, TestRtBinaryEnumerateFunctionsInvalidInput)
{
    rtFuncHandle funcHandles[2] = {nullptr, nullptr};
    EXPECT_EQ(rtBinaryEnumerateFunctions(nullptr, funcHandles, 2U), ACL_ERROR_RT_PARAM_INVALID);

    ElfProgram program;
    Program* programBase = &program;
    rtBinHandle binHandle = rt_ut::InitAndExportHandle<rtBinHandle>(programBase);
    EXPECT_EQ(rtBinaryEnumerateFunctions(binHandle, nullptr, 2U), ACL_ERROR_RT_PARAM_INVALID);
}

TEST_F(CloudV2ApiKernelTest, TestRtBinaryEnumerateFunctionsZeroNumFunctions)
{
    ElfProgram program;
    Program* programBase = &program;
    rtBinHandle binHandle = rt_ut::InitAndExportHandle<rtBinHandle>(programBase);

    // numFunctions 为 0：不写入任何句柄，funcHandles 允许为空，直接返回成功
    EXPECT_EQ(rtBinaryEnumerateFunctions(binHandle, nullptr, 0U), RT_ERROR_NONE);

    rtFuncHandle funcHandles[1] = {nullptr};
    EXPECT_EQ(rtBinaryEnumerateFunctions(binHandle, funcHandles, 0U), RT_ERROR_NONE);
    EXPECT_EQ(funcHandles[0], nullptr);
}
