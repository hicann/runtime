...
TEST_F(ModelApiTest, model_api_fail)
{
    ...
    error = rtModelCreate(NULL, 0);
    EXPECT_EQ(error, ACL_ERROR_RT_PARAM_INVALID);

    error = rtModelDestroy(NULL);
    EXPECT_EQ(error, ACL_ERROR_RT_PARAM_INVALID);

    error = rtModelBindStream(NULL, NULL, 0);
    EXPECT_EQ(error, ACL_ERROR_RT_PARAM_INVALID);

    error = rtModelUnbindStream(NULL, NULL);
    EXPECT_EQ(error, ACL_ERROR_RT_PARAM_INVALID);

    error = rtModelExecute(NULL, NULL, 0);
    EXPECT_EQ(error, ACL_ERROR_RT_PARAM_INVALID);

    error = rtModelExecuteSync(NULL, NULL, 0, 1);
    EXPECT_EQ(error, ACL_ERROR_RT_PARAM_INVALID);
}
