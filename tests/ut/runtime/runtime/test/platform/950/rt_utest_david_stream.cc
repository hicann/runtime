...
static rtError_t MemCopySyncCopyStub(
...
rtError_t QueryAbortStatusByTypeTimeoutStub(
    Stream* stm, uint32_t& status, const uint32_t opType, const uint32_t targetId)
{
    (void)stm;
    (void)opType;
    (void)targetId;
...
TEST_F(DavidStreamTest, auto_split_task_clean)
{
    ...
    realStream->Context_()->CreateAutoSplitSlaveStream(realStream, &stream2);
    autoSplitCtx_->slaveStreams.push_back((Stream*)stream2);
    MOCKER_CPP_VIRTUAL(realStream->Context_(), &Context::TearDownStream).stubs().will(returnValue(0));
...
TEST_F(DavidStreamTest, public_queue)
{
    ...
    uint16_t endRecyclePos = 5;
    uint16_t delTaskPos;
    res = davidStream->StarsGetPublicTaskHead(nullptr, false, endRecyclePos, &delTaskPos);
    EXPECT_NE(res, RT_ERROR_NONE);
    endRecyclePos = 15;
...
TEST_F(DavidStreamTest, npu_driver_sw_success)
{
    ...
    MOCKER(halTsdrvCtl)
        .stubs()
        .with(
            mockcpp::any(), mockcpp::any(), mockcpp::any(), mockcpp::any(), mockcpp::any(),
            outBoundP(&ackCount, sizeof(ackCount)))
        .will(returnValue(DRV_ERROR_NONE));
...
rtError_t TaskAbortByTypeByTimes(
    NpuDriver* drv, const uint32_t deviceId, const uint32_t tsId, const uint32_t opType, const uint32_t targetId,
    uint32_t& result)
{
    if (gabort_times == 0U) {
        result = 0x115;
...
rtError_t QueryAbortStatusByTypeByTimes(
    NpuDriver* drv, const uint32_t deviceId, const uint32_t tsId, const uint32_t queryType, const uint32_t targetId,
    uint32_t& status)
{
    if (gquery_times == 0U) {
        status = 1;
...
TEST_F(DavidStreamTest, DavidrtStreamStop_02)
{
    ...
    NpuDriver drv;
    MOCKER_CPP_VIRTUAL(drv, &NpuDriver::TaskAbortByType).stubs().will(invoke(TaskAbortByTypeByTimes));
...
TEST_F(DavidStreamTest, TestCreateStreamAndGet)
{
    ...
    MOCKER_CPP_VIRTUAL((NpuDriver*)device_->Driver_(), &NpuDriver::GetSqRegVirtualAddrBySqid)
        ...
        .will(returnValue(0));
    MOCKER_CPP_VIRTUAL((NpuDriver*)device_->Driver_(), &NpuDriver::MemCopySync).stubs().will(returnValue(0));
    MOCKER_CPP_VIRTUAL(device_, &Device::CheckFeatureSupport).stubs().will(returnValue(true));
    rtStream_t stream = 0;
...
TEST_F(DavidStreamTest, TestSyncDelayTime)
{
    ...
    MOCKER_CPP_VIRTUAL((DavidStream*)stream, &DavidStream::IsTaskExcuted).stubs().will(returnValue(false));
    isFinish = stream->SynchronizeDelayTime(finishedId, taskId);
    EXPECT_EQ(isFinish, false);
}
...
TEST_F(DavidStreamTest, ExternalRecordRefreshEntryUsesDavidWriteValueSqe)
{
    ...
    EXPECT_EQ(GetExternalRecordRefreshEntrySize(), sizeof(rtDavidSqe_t));
    EXPECT_EQ(FillExternalRecordRefreshEntry(nullptr, eventAddr), RT_ERROR_INVALID_VALUE);
    EXPECT_EQ(FillExternalRecordRefreshEntry(&sqe, eventAddr), RT_ERROR_NONE);

    EXPECT_EQ(sqe.writeValueSqe.header.type, RT_DAVID_SQE_TYPE_WRITE_VALUE);
    EXPECT_EQ(sqe.writeValueSqe.header.ptrMode, 0U);
    EXPECT_EQ(sqe.writeValueSqe.awsize, RT_STARS_WRITE_VALUE_SIZE_TYPE_8BIT);
...
TEST_F(DavidStreamTest, ExternalTaskSqeBuildRejectsInvalidTaskAndSkipsNullSqeBuffer)
{
    ...
    MOCKER_CPP_VIRTUAL(device_->Driver_(), &Driver::MemCopySync).stubs().will(invoke(MemCopySyncCopyStub));
    EXPECT_EQ(RebuildExternalTaskSqe(&taskInfo), RT_ERROR_NONE);
    const auto* funcCall = reinterpret_cast<const RtStarsExternalWaitFuncCall*>(funcCallMem);
    EXPECT_EQ(funcCall->loadActual.func3, RT_STARS_COND_ISA_LOAD_FUNC3_LDR);
    EXPECT_EQ(funcCall->loadActual.rs1, RT_STARS_COND_ISA_REGISTER_R2);
    EXPECT_EQ(funcCall->loadActual.rd, RT_STARS_COND_ISA_REGISTER_R1);
    EXPECT_EQ(funcCall->waitSatisfied.func3, RT_STARS_COND_ISA_BRANCH_FUNC3_BGEU);
    MOCKER(memcpy_s).stubs().will(returnValue(EOK)).then(returnValue(EINVAL));
    EXPECT_EQ(RebuildExternalTaskSqe(&taskInfo), RT_ERROR_INVALID_VALUE);
...
TEST_F(DavidStreamTest, Destructor_ArgHandleNonNull)
{
    ...
    MOCKER_CPP_VIRTUAL(device_->ArgLoader_(), &ArgLoader::Release).expects(once()).will(returnValue(RT_ERROR_NONE));
...
TEST_F(DavidStreamTest, Destructor_ArgHandleNull)
{
    ...
    MOCKER_CPP_VIRTUAL(device_->ArgLoader_(), &ArgLoader::Release).expects(never());
