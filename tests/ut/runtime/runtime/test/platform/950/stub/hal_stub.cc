    ...
    if (Runtime::Instance()->GetDisableThread()) {
        ...
        if (Runtime::Instance()->GetChipType() == CHIP_910_B_93) {
            ...
            if (cmd->streamID < 1024) {
                ...
                printf(
                    "halSqMsgSend: illegal stream_id=%u, chip_type=%u", cmd->streamID,
...
drvError_t halSqCqAllocate(uint32_t devId, struct halSqCqInputInfo* in, struct halSqCqOutputInfo* out)
{
    ...
    if (in->type == DRV_SHM_TYPE) {
        if (Runtime::Instance()->GetDisableThread()) {
            if (Runtime::Instance()->GetChipType() != CHIP_ADC && Runtime::Instance()->GetChipType() != CHIP_AS31XM1 &&
                Runtime::Instance()->GetChipType() != CHIP_610LITE &&
                Runtime::Instance()->GetChipType() != CHIP_CLOUD && Runtime::Instance()->GetChipType() != CHIP_DC) {
...
drvError_t halMemFree(void* pp) { return DRV_ERROR_NONE; }
...
drvError_t halDeviceEnableP2PNotify(uint32_t phy_dev, uint32_t peer_phy_dev, uint32_t flag) { return DRV_ERROR_NONE; }
...
DLLEXPORT drvError_t halPutTsegInfo(uint32_t devid, struct halTsegInfo* tsegInfo) { return DRV_ERROR_NONE; }
...
drvError_t halAsyncDmaJettyDestroy(uint32_t devId, struct halAsyncJettyDestroyPara* para) { return DRV_ERROR_NONE; }
