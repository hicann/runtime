...
TEST_F(DavidTaskTest, aic_task_link_error_proc_for_fusion)
{
    ...
    // not hit blklist and hit whitelist
    dev_->SetDeviceFaultType(DeviceFaultType::NO_ERROR);
    faultEventFlag = 8;
    ret = ProcessDavidStarsFusionKernelErrorInfo(&errorInfo, 0, dev_, nullptr);
    faultEventFlag = 0;
...
TEST_F(DavidTaskTest, MapFusionCcuErrorCodeForFastRecovery2)
{
    ...
    faultEventFlag = 7;
    ProcessDavidStarsCcuErrorInfo(&errorInfo, 0, taskInfo.stream->Device_(), nullptr);
    EXPECT_NE(taskInfo.mte_error, TS_ERROR_LOCAL_MEM_ERROR);
...
TEST_F(DavidTaskTest, UpdateUbdmaSqeWithJettyInfo_MultipleTasks)
{
    ...
    context.taskWqeCounts.push_back({&taskInfo0, 3});
    JettyInfo jettyInfo = {};
...
TEST_F(DavidTaskTest, UpdateUbdmaSqeWithJettyInfo_PartialNullTaskInfo)
{
    ...
    context.taskWqeCounts.push_back({nullptr, 2});
    JettyInfo jettyInfo = {};
