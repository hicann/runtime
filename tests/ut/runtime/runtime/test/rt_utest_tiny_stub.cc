...
protected:
    static void SetUpTestCase() { std::cout << "TinyStubTest test start" << std::endl; }

    static void TearDownTestCase() { std::cout << "TinyStubTest test start end" << std::endl; }
...
TEST_F(TinyStubTest, api_c_stub)
{
    ...
    EXPECT_EQ(ret, ACL_ERROR_RT_FEATURE_NOT_SUPPORT);
    ret = rtDeviceGetPCIBusId(0, nullptr, 0);
    EXPECT_EQ(ret, ACL_ERROR_RT_FEATURE_NOT_SUPPORT);
    ret = rtDeviceGetByPCIBusId(nullptr, nullptr);
    EXPECT_EQ(ret, ACL_ERROR_RT_FEATURE_NOT_SUPPORT);
}
...
TEST_F(TinyStubTest, api_impl_stub)
{
    ...
    EXPECT_EQ(ret, RT_ERROR_FEATURE_NOT_SUPPORT);
    ret = impl.GetNotifyPhyInfo(nullptr, nullptr);
    EXPECT_EQ(ret, RT_ERROR_FEATURE_NOT_SUPPORT);
    ret = impl.NotifyGetAddrOffset(nullptr, nullptr);
    EXPECT_EQ(ret, RT_ERROR_FEATURE_NOT_SUPPORT);
    ret = impl.GetNotifyAddress(nullptr, nullptr);
    EXPECT_EQ(ret, RT_ERROR_FEATURE_NOT_SUPPORT);
    ret = impl.NotifyReset(nullptr);
    EXPECT_EQ(ret, RT_ERROR_FEATURE_NOT_SUPPORT);
    ret = impl.IpcSetNotifyName(nullptr, nullptr, 0, 0);
    EXPECT_EQ(ret, RT_ERROR_FEATURE_NOT_SUPPORT);
    ret = impl.IpcOpenNotify(nullptr, nullptr, 0);
    EXPECT_EQ(ret, RT_ERROR_FEATURE_NOT_SUPPORT);
...
TEST_F(TinyStubTest, capture_model_sqcq_stub)
{
    ...
    uint32_t releaseSqNum = 0;
    uint32_t releaseNtyNum = 0;
    EXPECT_EQ(captureModel.ReleaseSqCqAndNotifyId(releaseSqNum, releaseNtyNum), RT_ERROR_FEATURE_NOT_SUPPORT);
