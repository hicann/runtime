...
struct StubDeviceComm : public DeviceComm {
    ...
    TSD_StatusT CommInit(const uint32_t, const bool) override { return TSD_OK; }
    ...
    TSD_StatusT CommRecvData(const uint32_t, const bool, const uint32_t timeout) override
    {
        ++recvCount;
        receivedTimeout = timeout;
        return recvResult;
    }
    TSD_StatusT CommGetConctStatus(int32_t& status) override
    {
        status = 0;
        return TSD_OK;
    }
    TSD_StatusT CommSendMsg(const uint32_t, const HDCMessage& msg) override
    {
        ++sendCount;
        sentType = msg.type();
        return sendResult;
    }
    TSD_StatusT CommGetVersionVerify(const uint32_t, std::shared_ptr<VersionVerify>& inspector) override
    {
        inspector = std::make_shared<VersionVerify>();
        return TSD_OK;
    }
    ...
    HDCMessage::MsgType sentType = HDCMessage::INIT;
};

std::shared_ptr<StubDeviceComm> InjectStubComm(ProcessModeManager& manager, uint32_t devId)
{
...
TEST_F(CapabilityManagerTest, SaveCapabilityResult_DriverVersion_ReturnsLogicError)
{
    ProcessModeManager pm(deviceId, 0);
    uint64_t result = 0;
    uint64_t ptr = reinterpret_cast<uint64_t>(&result);
...
TEST_F(CapabilityManagerTest, UpdateStateFromMsg_LevelResponseError_PreservesCachedLevel)
{
    ProcessModeManager pm(deviceId, 0);
    HDCMessage msg;
    msg.set_type(HDCMessage::TSD_GET_SUPPORT_CAPABILITY_LEVEL_RSP);
...
TEST_F(CapabilityManagerTest, UpdateStateFromMsg_OmResponseOk_MarksSupported)
{
    ProcessModeManager pm(deviceId, 0);
    HDCMessage msg;
...
TEST_F(CapabilityManagerTest, UpdateStateFromMsg_OmResponseError_MarksUnsupported)
{
    ProcessModeManager pm(deviceId, 0);
    HDCMessage msg;
    msg.set_type(HDCMessage::TSD_SUPPORT_OM_INNER_DEC_RSP);
...
TEST_F(CapabilityManagerTest, WaitCapabilityRsp_PidQosResponse_WritesResult)
{
    ProcessModeManager pm(deviceId, 0);
    uint64_t result = 0;
    uint64_t ptr = reinterpret_cast<uint64_t>(&result);
...
TEST_F(CapabilityManagerTest, CapabilityGet_NullPtr_ReturnsOpenFailed)
{
    ProcessModeManager pm(deviceId, 0);
    EXPECT_EQ(pm.capabilityMgr_.CapabilityGet(TSD_CAPABILITY_LEVEL, 0UL), TSD_CLT_OPEN_FAILED);
}

TEST_F(CapabilityManagerTest, CapabilityGet_PidQosClientUninitialized_CurrentContractReturnsOkWithoutCommunication)
{
    ProcessModeManager pm(deviceId, 0);
    uint64_t result = 0xA5A5UL;
    const uint64_t ptr = reinterpret_cast<uint64_t>(&result);
    MOCKER_CPP(&DeviceCommAgent::SendMsg).expects(never());
    MOCKER_CPP(&DeviceCommAgent::RecvData).expects(never());

    EXPECT_EQ(pm.capabilityMgr_.CapabilityGet(TSD_CAPABILITY_PIDQOS, ptr), TSD_OK);
    EXPECT_EQ(result, 0xA5A5UL);
}
...
TEST_F(CapabilityManagerTest, CapabilityGet_PidQosWithoutStartCp_ReturnsOpenFailed)
{
    ProcessModeManager pm(deviceId, 0);
    pm.commAgent_.devCommClient_ = DeviceComm::GetInstance(deviceId, DeviceCommType::HDC);
    pm.capabilityMgr_.SetStartCpStatus(false);
...
TEST_F(CapabilityManagerTest, IsSupportCommonInterface_EmptyCache_ReceivesZeroBitmap)
{
    ProcessModeManager processModeManager(deviceId, 0);
    auto comm = InjectStubComm(processModeManager, deviceId);

    EXPECT_FALSE(processModeManager.capabilityMgr_.IsSupportCommonInterface(TSD_SUPPORT_HS_AISERVER_FEATURE_BIT));
    EXPECT_EQ(comm->sendCount, 1U);
    EXPECT_EQ(comm->sentType, HDCMessage::TSD_GET_SUPPORT_CAPABILITY_LEVEL);
    EXPECT_EQ(comm->recvCount, 1U);
    EXPECT_EQ(comm->receivedTimeout, 10000U);
}

TEST_F(CapabilityManagerTest, IsSupportCommonInterface_CachedBitSet_SkipsCommunication)
{
    ProcessModeManager processModeManager(deviceId, 0);
    auto comm = InjectStubComm(processModeManager, deviceId);
    processModeManager.capabilityMgr_.tsdSupportLevel_ = 1U << TSD_SUPPORT_HS_AISERVER_FEATURE_BIT;

    EXPECT_TRUE(processModeManager.capabilityMgr_.IsSupportCommonInterface(TSD_SUPPORT_HS_AISERVER_FEATURE_BIT));
    EXPECT_EQ(comm->sendCount, 0U);
    EXPECT_EQ(comm->recvCount, 0U);
}

TEST_F(CapabilityManagerTest, IsSupportCommonInterface_ReceiveFails_ReturnsFalse)
{
    ProcessModeManager processModeManager(deviceId, 0);
    auto comm = InjectStubComm(processModeManager, deviceId);
    comm->recvResult = TSD_HDC_RECV_MSG_ERROR;
...
TEST_F(CapabilityManagerTest, IsSupportCommonSink_EmptyHostPathAndInterfaceUnsupported_ReturnsFalse)
{
    ProcessModeManager processModeManager(deviceId, 0);
...
TEST_F(CapabilityManagerTest, IsSupportCommonSink_No_With_hostSoPathInvalid)
{
    ProcessModeManager processModeManager(deviceId, 0);
...
TEST_F(CapabilityManagerTest, IsSupportCommonSink_Yes_With_hostSoPathValid)
{
    ProcessModeManager processModeManager(deviceId, 0);
    std::string hostSoPath = "/usr/local/ascend/driver/";
    MOCKER(tsd::GetHostSoPath).stubs().will(returnValue(hostSoPath));
...
TEST_F(CapabilityManagerTest, IsSupportCommonSink_EmptyHostPathAndInterfaceSupported_ReturnsTrue)
{
    ProcessModeManager processModeManager(deviceId, 0);
    std::string emptyPath = "";
    MOCKER(tsd::GetHostSoPath).stubs().will(returnValue(emptyPath));
