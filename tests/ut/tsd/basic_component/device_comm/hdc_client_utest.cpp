...
namespace {
uint32_t g_typeMismatchReplyCount = 0U;
HDCMessage::MsgType g_typeMismatchRequestType = HDCMessage::INIT;
...
void TypeMismatchReply(struct drvHdcMsg* msg, HdcBufferInfo* buf, const HDCMessage& request)
{
    ++g_typeMismatchReplyCount;
    g_typeMismatchRequestType = request.type();
    StubServerMsgImpl::DefaultStartProcMsgProc(msg, buf, request);
}

hdcError_t DrvHdcSessionConnectWithHandle(int, int, HDC_CLIENT, HDC_SESSION* session)
{
    *session = reinterpret_cast<HDC_SESSION>(0x1234);
    return DRV_ERROR_NONE;
}
...
protected:
    virtual void SetUp()
    {
        ...
        g_typeMismatchReplyCount = 0U;
        g_typeMismatchRequestType = HDCMessage::INIT;
    }
    ...
    static constexpr uint32_t devId_ = 17U;
...
TEST_F(HdcClientTest, GetInstance_ValidDevice_ReturnsClient)
{
...
TEST_F(HdcClientTest, Constructor_NewClient_InitializesClosedStateAndSessionIds)
{
...
TEST_F(HdcClientTest, GetDeviceId_ConstructedClient_ReturnsConfiguredId)
{
    HdcClient client(devId_);
    EXPECT_EQ(client.GetDeviceId(), devId_);
}

TEST_F(HdcClientTest, InitPre_ClosedClient_OpensClient)
{
    HdcClient client(devId_);
...
TEST_F(HdcClientTest, CommInit_ValidPid_StoresHostPid)
{
    HdcClient client(devId_);
...
TEST_F(HdcClientTest, CommCreateSession_ClientClosed_ReturnsClientClosed)
{
    HdcClient client(devId_);
...
TEST_F(HdcClientTest, CommCreateSession_HandshakeSucceeds_ReturnsSessionId)
{
    HdcClient client(devId_);
    client.InitPre();
...
TEST_F(HdcClientTest, GetHdcSession_SessionMissing_ReturnsNotExist)
{
    HdcClient client(devId_);
...
TEST_F(HdcClientTest, GetHdcSession_SessionExists_ReturnsHandle)
{
    HdcClient client(devId_);
    client.InitPre();
    MOCKER(&drvHdcSessionConnect).expects(exactly(1)).will(invoke(DrvHdcSessionConnectWithHandle));
    StubServerReply::GetInstance()->RegisterCallBack(
        HDCMessage::TEST_HDC_SEND, StubServerMsgImpl::DefaultVersionNegotiateMsgProc);
    uint32_t sessionId = 0;
    ...
    EXPECT_EQ(session, reinterpret_cast<HDC_SESSION>(0x1234));
    EXPECT_EQ(session, client.hdcClientSessionMap_.at(sessionId));
}
...
TEST_F(HdcClientTest, CommGetVersionVerify_SessionExists_ReturnsInspector)
{
    HdcClient client(devId_);
    client.InitPre();
    StubServerReply::GetInstance()->RegisterCallBack(
...
TEST_F(HdcClientTest, CommGetConctStatus_OpenSession_ReturnsConnected)
{
...
TEST_F(HdcClientTest, CommSendMsg_SessionMissing_ReturnsSendError)
{
...
TEST_F(HdcClientTest, RecvMsg_SessionMissing_ReturnsRecvError)
{
    HdcClient client(devId_);
    HDCMessage msg;
...
TEST_F(HdcClientTest, CommDestroy_OpenClient_MarksClientClosed)
{
    auto client = std::dynamic_pointer_cast<HdcClient>(DeviceComm::GetInstance(devId_, DeviceCommType::HDC));
    client->InitPre();
    StubServerReply::GetInstance()->RegisterCallBack(
...
TEST_F(HdcClientTest, ClearAllSession_OpenSession_ClearsSessionMap)
{
    HdcClient client(devId_);
    client.InitPre();
    StubServerReply::GetInstance()->RegisterCallBack(
...
TEST_F(HdcClientTest, ClearClientPtr_ClientCached_RemovesCachedInstance)
{
    auto client = std::dynamic_pointer_cast<HdcClient>(DeviceComm::GetInstance(devId_, DeviceCommType::HDC));
    client->InitPre();
...
TEST_F(HdcClientTest, CommDestroy_OpenCachedClient_RemovesCachedInstance)
{
    auto client = std::dynamic_pointer_cast<HdcClient>(DeviceComm::GetInstance(devId_, DeviceCommType::HDC));
    client->InitPre();
...
TEST_F(HdcClientTest, CommRecvData_ServerResponds_ReturnsOk)
{
    HdcClient client(devId_);
    client.InitPre();
    StubServerReply::GetInstance()->RegisterCallBack(
...
TEST_F(HdcClientTest, InitPre_ElevenCreateAttemptsFail_ReturnsInitError)
{
    HdcClient client(devId_);
    MOCKER(&drvHdcClientCreate).expects(exactly(11)).will(returnValue(DRV_ERROR_INNER_ERR));
    MOCKER(&mmSleep).expects(exactly(10)).will(returnValue(0));
    TSD_StatusT ret = client.InitPre();
    EXPECT_EQ(ret, TSD_HDC_CLIENT_INIT_ERROR);
    EXPECT_TRUE(client.isClientClose_);
}

TEST_F(HdcClientTest, InitPre_SecondCreateAttemptSucceeds_OpensClient)
{
    HdcClient client(devId_);
    ...
    MOCKER(&mmSleep).expects(exactly(1)).will(returnValue(0));
    TSD_StatusT ret = client.InitPre();
    EXPECT_EQ(ret, TSD_OK);
    EXPECT_EQ(client.isClientClose_, false);
}

TEST_F(HdcClientTest, CommInit_MessageCapacityQueryFails_ReturnsInitError)
{
    HdcClient client(devId_);
...
TEST_F(HdcClientTest, CommCreateSession_NoAvailableSessionId_ReturnsNotAvailable)
{
    HdcClient client(devId_);
    client.InitPre();
...
TEST_F(HdcClientTest, CommCreateSession_DriverConnectFails_ReturnsCreateError)
{
    HdcClient client(devId_);
    client.InitPre();
...
TEST_F(HdcClientTest, CommCreateSession_SetReferenceFails_ClosesSessionAndReturnsError)
{
    HdcClient client(devId_);
    client.InitPre();
    MOCKER(&drvHdcSetSessionReference).expects(exactly(1)).will(returnValue(DRV_ERROR_INNER_ERR));
    MOCKER(&drvHdcSessionClose).expects(exactly(1)).will(returnValue(DRV_ERROR_NONE));
    uint32_t sessionId = 77U;
    TSD_StatusT ret = client.CommCreateSession(sessionId);
    EXPECT_EQ(ret, TSD_SET_HDCSESSION_REFERENCE_FAILED);
    EXPECT_EQ(sessionId, 77U);
    EXPECT_TRUE(client.hdcClientSessionMap_.empty());
    EXPECT_EQ(client.sessionIdNumVec_.size(), 96U);
}

TEST_F(HdcClientTest, CommCreateSession_SetSessionReferenceAndCloseFail_ReturnsReferenceError)
{
    HdcClient client(devId_);
    client.InitPre();
    MOCKER(&drvHdcSetSessionReference).expects(exactly(1)).will(returnValue(DRV_ERROR_INNER_ERR));
    MOCKER(&drvHdcSessionClose).expects(exactly(1)).will(returnValue(DRV_ERROR_INNER_ERR));
    uint32_t sessionId = 77U;
    TSD_StatusT ret = client.CommCreateSession(sessionId);
    EXPECT_EQ(ret, TSD_SET_HDCSESSION_REFERENCE_FAILED);
    EXPECT_EQ(sessionId, 77U);
...
TEST_F(HdcClientTest, CommCreateSession_HandshakeReceiveFails_ReturnsRecvError)
{
    HdcClient client(devId_);
    ASSERT_EQ(client.InitPre(), TSD_OK);
    MOCKER_CPP(&StubServerReply::ReplyToHost).expects(exactly(1)).will(returnValue(false));
    uint32_t sessionId = 0U;

    EXPECT_EQ(client.CommCreateSession(sessionId), TSD_HDC_RECV_MSG_ERROR);
}

TEST_F(HdcClientTest, CommCreateSession_HandshakeResponseTypeMismatch_ReturnsRecvError)
{
    HdcClient client(devId_);
    ASSERT_EQ(client.InitPre(), TSD_OK);
    StubServerReply::GetInstance()->RegisterCallBack(HDCMessage::TEST_HDC_SEND, TypeMismatchReply);
    MOCKER_CPP(&VersionVerify::PeerVersionCheck).expects(never());
    uint32_t sessionId = 0U;

    EXPECT_EQ(client.CommCreateSession(sessionId), TSD_HDC_RECV_MSG_ERROR);
    EXPECT_EQ(g_typeMismatchReplyCount, 1U);
    EXPECT_EQ(g_typeMismatchRequestType, HDCMessage::TEST_HDC_SEND);
}

TEST_F(HdcClientTest, CommGetVersionVerify_SessionMissing_ReturnsNotExist)
{
    HdcClient client(devId_);
    std::shared_ptr<VersionVerify> inspector = nullptr;
...
TEST_F(HdcClientTest, CommGetConctStatus_ApiVersionQueryFails_ReturnsStatusError)
{
    HdcClient client(devId_);
    MOCKER(&halGetAPIVersion).expects(exactly(1)).will(returnValue(DRV_ERROR_INNER_ERR));
...
TEST_F(HdcClientTest, CommGetConctStatus_SessionStatusQueryFails_ReturnsStatusError)
{
    HdcClient client(devId_);
    client.InitPre();
    StubServerReply::GetInstance()->RegisterCallBack(
...
TEST_F(HdcClientTest, ClearAllSession_CloseFails_ClearsMapWithoutRecyclingId)
{
    HdcClient client(devId_);
    client.InitPre();
    StubServerReply::GetInstance()->RegisterCallBack(
    ...
    client.ClearAllSession();
    EXPECT_TRUE(client.hdcClientSessionMap_.empty());
    EXPECT_EQ(client.sessionIdNumVec_.size(), sessionIdCount);
}

TEST_F(HdcClientTest, ClearClientPtr_DeviceIdNotInMap)
{
    HdcClient client(devId_);
    const uint64_t key = KeyCompose(client.GetDeviceId(), DeviceCommType::HDC);
    (void)DeviceComm::DeviceCommMap()->erase(key);
    ASSERT_EQ(DeviceComm::DeviceCommMap()->count(key), 0U);
    const size_t mapSize = DeviceComm::DeviceCommMap()->size();

    client.ClearClientPtr();

    EXPECT_EQ(DeviceComm::DeviceCommMap()->size(), mapSize);
    EXPECT_EQ(DeviceComm::DeviceCommMap()->count(key), 0U);
    EXPECT_TRUE(client.isClientClose_);
}

TEST_F(HdcClientTest, DestroyClient_DriverDestroyFails_StillMarksClientClosed)
{
    HdcClient client(devId_);
    client.hdcClient_ = reinterpret_cast<HDC_CLIENT>(0x1234);
    client.isClientClose_ = false;
    MOCKER(&drvHdcClientDestroy).expects(exactly(1)).will(returnValue(DRV_ERROR_INNER_ERR));
    client.DestroyClient();
    EXPECT_TRUE(client.isClientClose_);
}

TEST_F(HdcClientTest, CommSendMsg_FeatureCheckFails_ReturnsRecvError)
{
    HdcClient client(devId_);
    client.InitPre();
    StubServerReply::GetInstance()->RegisterCallBack(
...
TEST_F(HdcClientTest, CommSendMsg_CompatibleMessage_ReturnsOk)
{
...
TEST_F(HdcClientTest, CommRecvData_ReceiveFailsWithoutIgnore_ReturnsRecvError)
{
