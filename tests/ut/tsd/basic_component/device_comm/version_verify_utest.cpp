...
TEST_F(VersionVerifyTest, PeerVersionCheck_ZeroVersion_ReturnsFalse)
{
...
TEST_F(VersionVerifyTest, SpecialFeatureCheck_FeatureListMismatch_ReturnsFalse)
{
    std::shared_ptr<VersionVerify> versionVerify = std::make_shared<VersionVerify>();
...
TEST_F(VersionVerifyTest, SetVersionInfo_NewMessage_WritesExactCompatibilityContract)
{
    HDCMessage msg;
    std::shared_ptr<VersionVerify> versionVerify = std::make_shared<VersionVerify>();
    versionVerify->SetVersionInfo(msg);
    const HDCMessage::VersionInfo& versionInfo = msg.version_info();
    EXPECT_EQ(versionInfo.version(), 1230U);
    ASSERT_EQ(versionInfo.feature_list_size(), 3);
    std::map<HDCMessage::MsgType, std::string> features;
    for (const auto& featureList : versionInfo.feature_list()) {
        ASSERT_EQ(featureList.feature_size(), 1);
        features[featureList.msg_type()] = featureList.feature(0);
    }
    EXPECT_EQ(features.at(HDCMessage::TSD_CHECK_PACKAGE), "check before send aicpu package");
    EXPECT_EQ(features.at(HDCMessage::TSD_START_QS_MSG), "check before send open qs message");
    EXPECT_EQ(features.at(HDCMessage::TSD_CHECK_PACKAGE_RETRY), "get check code retry");
}

TEST_F(VersionVerifyTest, PeerVersionCheck_MatchingVersion_ReturnsTrue)
{
...
TEST_F(VersionVerifyTest, SpecialFeatureCheck_CachedUnsupportedType_ReturnsFalse)
{
...
TEST_F(VersionVerifyTest, SpecialFeatureCheck_LegacyTypeAbsentFromBothLists_ReturnsTrue)
{
    const HDCMessage::MsgType msgType = HDCMessage::TSD_OPEN_SUB_PROC_RSP;
    std::shared_ptr<VersionVerify> versionVerify = std::make_shared<VersionVerify>();
...
TEST_F(VersionVerifyTest, SpecialFeatureCheck_ExactAndMismatchedFeatureLists_ReturnsExpectedSupport)
{
    std::shared_ptr<VersionVerify> versionVerify = std::make_shared<VersionVerify>();
...
TEST_F(VersionVerifyTest, PeerVersionCheck_NonzeroMismatchedVersion_ReturnsFalse)
{
    HDCMessage::VersionInfo peerVersionInfo;
    peerVersionInfo.set_version(1229U);
    VersionVerify versionVerify;

    EXPECT_FALSE(versionVerify.PeerVersionCheck(peerVersionInfo));
    EXPECT_EQ(versionVerify.peerVersion_, 1229U);
}
