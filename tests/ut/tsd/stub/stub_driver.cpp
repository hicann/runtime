...
int halVerifyImg(HAL_VERIFY_TYPE verify_type, HAL_IMG_ID image_id, const char* img_path, int mode) { return 0; }

drvError_t drvHdcSendFile(
...
int halGrpAttach(const char* name, int timeout) { return DRV_ERROR_NONE; }

int halBuffInit(BuffCfg* cfg) { return DRV_ERROR_NONE; }

int halGrpAddProc(const char* name, int pid, GroupShareAttr attr) { return DRV_ERROR_NONE; }
...
drvError_t halGetSocVersion(uint32_t devId, char* socVersion, uint32_t len) { return DRV_ERROR_NONE; }