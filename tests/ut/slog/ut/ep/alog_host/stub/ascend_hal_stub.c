...
drvError_t drvHdcClientDestroy(HDC_CLIENT client) { return DRV_ERROR_NONE; }
...
drvError_t drvHdcGetMsgBuffer(struct drvHdcMsg* msg, int index, char** pBuf, int* pLen) { return DRV_ERROR_NONE; }

drvError_t drvHdcSetSessionReference(HDC_SESSION session) { return DRV_ERROR_NONE; }
...
void SetDrvCrlCmd(int cmd) { g_ctlCmd = cmd; }
