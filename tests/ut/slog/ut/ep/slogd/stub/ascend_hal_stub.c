...
int32_t LogHdcServerInit(const struct LogServerInitInfo* info) { return 0; }
// appmon
...
{
    // 固件日志已从废弃的 log_read 切换到 log_read_by_type(LOG_CHANNEL_TYPE_MAX)，
    // 该通道复用原 log_read 的数据以保持固件日志用例的数据来源
    if (channel_type == LOG_CHANNEL_TYPE_MAX) {
        return log_read(device_id, buf, size, timeout);
    }
    (void)buf;
    (void)size;
    usleep(timeout);
