...
extern "C" {
    ...
    extern void DlogChildUnLock(void);
}

class MDC_SLOG_FUNC_UTEST : public testing::Test
{
protected:
    virtual void SetUp()
    {
...
// 日志落盘
TEST_F(MDC_SLOG_FUNC_UTEST, DlogPrint)
{
    ...
    dlog_error(SLOG | STDOUT_LOG_MASK, "[MDC_SLOG_FUNC_STEST][DlogPrint] test for mask_stdout, error_level.");

    // 打印 DLOG_WARN | STDOUT_LOG_MASK
    dlog_warn(SLOG | STDOUT_LOG_MASK, "[MDC_SLOG_FUNC_STEST][DlogPrint] test for mask_stdout, warn_level.");

    // 打印 DLOG_INFO | STDOUT_LOG_MASK
    dlog_info(SLOG | STDOUT_LOG_MASK, "[MDC_SLOG_FUNC_STEST][DlogPrint] test for mask_stdout, info_level.");

    // 打印 DLOG_DEBUG | STDOUT_LOG_MASK
    dlog_debug(SLOG | STDOUT_LOG_MASK, "[MDC_SLOG_FUNC_STEST][DlogPrint] test for mask_stdout, debug_level.");
...
TEST_F(MDC_SLOG_FUNC_UTEST, DlogAllInterface)
{
    MOCKER(SlogIamIoctl).stubs().will(invoke(SlogIamIoctlStub));
    // 初始化
    DlogConstructor();

    DlogInner(RUN_LOG_MASK, DLOG_INFO, "test log interface");
    DlogErrorInner(RUN_LOG_MASK, "test log interface");
    DlogWarnInner(RUN_LOG_MASK, "test log interface");
    DlogInfoInner(RUN_LOG_MASK, "test log interface");
    DlogDebugInner(RUN_LOG_MASK, "test log interface");
    DlogEventInner(RUN_LOG_MASK, "test log interface");

    KeyValue stKeyValue[1];
    stKeyValue[0].kname = "game";
    stKeyValue[0].value = "over";
    DlogWithKVInner(RUN_LOG_MASK, DLOG_INFO, stKeyValue, 1, "test log interface");

    DlogRecord(RUN_LOG_MASK, DLOG_INFO, "test log interface");
    va_list list;
    DlogVaList(RUN_LOG_MASK, DLOG_INFO, "test log interface", list);
...
TEST_F(MDC_SLOG_FUNC_UTEST, DlogPrintOverBufSize)
{
    MOCKER(SlogIamIoctl).stubs().will(invoke(SlogIamIoctlStub));
    // 初始化
    DlogConstructor();
...
TEST_F(MDC_SLOG_FUNC_UTEST, DlogPrintOverBufSizeFlush)
{
    MOCKER(SlogIamIoctl).stubs().will(invoke(SlogIamIoctlStub));
    // 初始化
    DlogConstructor();
    ...
    EXPECT_EQ(0, levelCtrlLoss);
    int32_t coverLoss = DlogGetLogLossNum("covered", 1);
    printf("log coverLoss loss num : %d.\n", coverLoss);

    EXPECT_EQ(10000, coverLoss + DlogCheckLogNum(LOGOUT_IAM_SERVICE_PATH, "ERROR"));
}

TEST_F(MDC_SLOG_FUNC_UTEST, DlogPrintOverMaxLossNum)
{
    MOCKER(SlogIamIoctl).stubs().will(invoke(SlogIamIoctlStub));
    // 初始化
    DlogConstructor();
    ...
    int32_t coverLoss = DlogGetLogLossNum("covered", 1);
    printf("log coverLoss loss num : %d.\n", coverLoss);
    EXPECT_EQ(10002, coverLoss);

    EXPECT_GT(100000, coverLoss + DlogCheckLogNum(LOGOUT_IAM_SERVICE_PATH, "ERROR"));
}

TEST_F(MDC_SLOG_FUNC_UTEST, DlogPrintLevelControl)
{
    SlogSetLevel(0);
    MOCKER(SlogIamIoctl).stubs().will(invoke(SlogIamIoctlStub));
    MOCKER(DlogGetLevelStatus).stubs().will(returnValue(false));
    // 初始化
    DlogConstructor();

    // set log control switch to level=ERROR
    for (int32_t i = 0; i < 10000; i++) {
        dlog_error(SLOG | DEBUG_LOG_MASK, "[MDC_SLOG_FUNC_UTEST][DlogPrint] test for mask_debug, error_level.");
    }
    for (int32_t i = 0; i < 10000; i++) {
        dlog_info(SLOG | DEBUG_LOG_MASK, "[MDC_SLOG_FUNC_UTEST][DlogPrint] test for mask_debug, info_level.");
    }
    // log control down to level=WARNING
    sleep(2);
    for (int32_t i = 0; i < 10; i++) {
        dlog_warn(SLOG | DEBUG_LOG_MASK, "[MDC_SLOG_FUNC_UTEST][DlogPrint] test for mask_debug, warn_level.");
    }
    // log control down to level=INFO
    sleep(2);
    for (int32_t i = 0; i < 10; i++) {
        dlog_info(SLOG | DEBUG_LOG_MASK, "[MDC_SLOG_FUNC_UTEST][DlogPrint] test for mask_debug, info_level.");
    }
    // log control up to level=WARNING
    // log control up to level=ERROR
    for (int32_t i = 0; i < 10000; i++) {
        dlog_error(SLOG | DEBUG_LOG_MASK, "[MDC_SLOG_FUNC_UTEST][DlogPrint] test for mask_debug, error_level.");
    }
    // log control down to level=WARNING
    ...
    // clear log control switch
    sleep(4);
    for (int32_t i = 0; i < 10; i++) {
        dlog_debug(SLOG | DEBUG_LOG_MASK, "[MDC_SLOG_FUNC_UTEST][DlogPrint] test for mask_debug, debug_level.");
    }
    DlogFlush();
    ...
    // 释放
    DlogDestructor();
    int32_t levelCtrlLoss = DlogGetLogLossNum("clear log control", 0) + DlogGetLogLossNum("log covered loss num", 0);
    EXPECT_GE(10050, levelCtrlLoss);
    EXPECT_LT(0, levelCtrlLoss);
    int32_t coverLoss = DlogGetLogLossNum("covered", 1);
    printf("log coverLoss loss num debug : %d.\n", coverLoss);

    EXPECT_EQ(30050, levelCtrlLoss + coverLoss + DlogCheckLogNum(LOGOUT_IAM_SERVICE_PATH, "ERROR") +
        DlogCheckLogNum(LOGOUT_IAM_SERVICE_PATH, "INFO") + DlogCheckLogNum(LOGOUT_IAM_SERVICE_PATH, "WARNING") +
        DlogCheckLogNum(LOGOUT_IAM_SERVICE_PATH, "DEBUG"));
}

TEST_F(MDC_SLOG_FUNC_UTEST, SlogSwitchToApp)
{
    MOCKER(SlogIamIoctl).stubs().will(invoke(SlogIamIoctlStub));
    // 初始化
    DlogConstructor();
    ...
    attr.pid = 123;
    attr.deviceId = 1;
    attr.mode = LOG_SAVE_MODE_SEP;
    EXPECT_EQ(LOG_SUCCESS, DlogSetAttr(attr));

    LogAttr getAttr;
    DlogGetAttr(&getAttr);
    EXPECT_EQ(APPLICATION, getAttr.type);
    EXPECT_EQ(123, getAttr.pid);
    EXPECT_EQ(1, getAttr.deviceId);
    EXPECT_EQ(LOG_SAVE_MODE_SEP, getAttr.mode);
