...
TEST_F(SlogdSlog, TimeDiff00) { EXPECT_EQ(0, DlogTimeDiff(NULL)); }
...
TEST_F(SlogdSlog, ParseLogLevel)
{
    ...
    const char* moduleStrTmp = "SLOG;IDEDD;HCCL;FMK;DVPP;RUNTIME;CCE;HDC;DRV;MLL;DEVMM;KERNEL;LIBMEDIA;CCECPU;ASCENDDK;"
                               "ROS;HCCP;ROCE;TEFUSION;PROFILING;DP;APP;TS;TSDUMP;AICPU;LP;TDT;FE;MD;MB;ME;IMU;IMP;GE;"
                               "CAMERA;ASCENDCL;TEEOS;ISP;SIS;HSM;DSS;PROCMGR;BBOX;AIVECTOR;TBE;FV";
...
static int32_t ToolWrite_stub(int32_t fd, const void* buf, uint32_t len) { return write(fd, buf, (size_t)len); }
...
TEST_F(SlogdSlog, RegisterCallback) { EXPECT_EQ(SUCCESS, RegisterCallback(NULL, LOG_WRITE)); }

TEST_F(SlogdSlog, DlogReportInitialize) { EXPECT_EQ(0, DlogReportInitialize()); }

TEST_F(SlogdSlog, DlogReportFinalize) { EXPECT_EQ(0, DlogReportFinalize()); }
