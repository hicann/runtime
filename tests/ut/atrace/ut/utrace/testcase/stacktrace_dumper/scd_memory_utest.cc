...
void EXPECT_TestMemory(
    size_t (*iter)(void* obj, uintptr_t addr, void* dst, size_t size), void* src, void* dst, size_t size)
{
    *(int8_t*)src = 1;
...
void DstMememoryInit(void** dst, size_t size) { *dst = malloc(size); }
...
void TestMemoryLocalRead(
    void (*srcMemoryInit)(void** src, size_t* size),
    void (*srcMemoryCheckResult)(void* src, void* dst, size_t size, size_t retSize),
    void (*srcMemoryFinalize)(void* src))
{
    void* src;
    size_t size;
...
TEST_F(ScdMemoryUtest, TestMemoryReadArray)
{
    ...
    for (auto iter : readFunc) {
        iter(
            [](void** src, size_t* size) {
                int64_t tmp[2] = {
                    static_cast<int64_t>(0xABCDBCDACDABDABCULL), static_cast<int64_t>(0xDABCABCDBCDACDABULL)};
                *size = sizeof(tmp);
                *src = malloc(*size);
                void* addr = *src;
...
TEST_F(ScdMemoryUtest, TestMemoryReadRemoteMemcpyFailed)
{
    ...
    for (int i = 0; i <= testNum; i++) {
        ...
        MOCKER(memcpy_s).stubs().will(invoke(memcpyStub));
        ...
        for (auto iter : readFunc) {
            iter(
                ...
                [](void* src, void* dst, size_t size, size_t retSize) { EXPECT_EQ(retSize, 0); },
                [](void* src) { free(src - 1); });
        }
