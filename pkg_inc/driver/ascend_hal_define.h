...
typedef int32_t (*report_predefined_err_msg_func)(
    const char* error_code, const char** key, const char** value, unsigned long arg_num);
// report inner error message
typedef int32_t (*report_inner_err_msg_func)(
...
enum drvMemMapCapability { HOST_MEM_MAP_NOT_SUPPORTED = 0, HOST_MEM_MAP_SUPPORTED };
...
struct hal_fault_event_resume {
    ...
    unsigned char
        resv[9]; /* Confirmed with the driver, this field needs to be reserved due to alignment requirements. */
};

typedef enum { HANDLE_ATTR_MEM_MAP_ROUTE = 0, HANDLE_ATTR_TYPE_MAX } HandleAttrType;
