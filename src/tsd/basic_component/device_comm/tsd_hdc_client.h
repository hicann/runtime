...
public:
     ...
     */
    TSD_StatusT CommRecvData(
        const uint32_t sessionId, const bool ignoreRecvErr = false, const uint32_t timeout = 0U) override;
    ...
    TSD_StatusT CommGetVersionVerify(const uint32_t sessionId, std::shared_ptr<VersionVerify>& inspector) override;
