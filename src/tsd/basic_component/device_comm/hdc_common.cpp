 ...
 */
HdcCommon::HdcCommon() : isAdcEnv_(false), msgMaxSize_(0U), msgShortHeadDataMaxSize_(0U), msgLongHeadDataMaxSize_(0U) {}
...
TSD_StatusT HdcCommon::InitMsgSize()
{
    ...
    TSD_INFO(
        "msgMaxSize_ = %u bytes, msgShortHeadDataMaxSize_ = %u bytes, msgLongHeadDataMaxSize_ = %u bytes", msgMaxSize_,
        msgShortHeadDataMaxSize_, msgLongHeadDataMaxSize_);
    return TSD_OK;
}
 ...
 * return Status成功TSD_OK，失败：其他错误码
 */
TSD_StatusT HdcCommon::SendNormalShortMsg(const HDCMessage& msg, const uint32_t size, HDC_SESSION const session)
{
    if (size == 0U) {
    ...
    if (memset_s(serializedMsg, static_cast<size_t>(serialMsgSizeCur), '\0', static_cast<size_t>(serialMsgSizeCur)) !=
        0) {
    ...
    *(PtrToPtr<char_t, uint32_t>(serializedMsg) + HDC_MSG_SIZE_OFFSET) = 0U;
    ...
    if (result != TSD_OK) {
        TSD_CHECK_EQ_RETURN_RUNWARN_LOG(
            result == TSD_HDC_SERVER_CLIENT_SOCKET_CLOSED, TSD_HDC_SERVER_CLIENT_SOCKET_CLOSED,
            "halHdcSend return socket close");

        TSD_CHECK_NO_RETURN(result == TSD_HDC_SERVER_CLIENT_SOCKET_CLOSED, "Send failed ret[%u]", result);
        return TSD_HDC_SEND_MSG_ERROR;
    }
    return TSD_OK;
}
 ...
 * return Status成功TSD_OK，失败：其他错误码
 */
TSD_StatusT HdcCommon::SendHdcDefaultMsg(HDC_SESSION const session, char_t* const hdcMsgBuf, const uint32_t size)
{
    ...
    if (drvRet != DRV_ERROR_NONE) {
        ...
        TSD_CHECK_NO_RETURN(drvRet == DRV_ERROR_NONE, "drvHdcFreeMsg failed ret[%d]", drvRet);
        return TSD_HDC_SEND_ERROR;
    }

    {
        ...
        drvRet =
            halHdcSend(session, drvMsg, static_cast<uint64_t>(HDC_FLAG_WAIT_TIMEOUT), HDC_CLIENT_SEND_WAIT_TIMEOUT_MS);
    }
    if (drvRet != DRV_ERROR_NONE) {
        ...
        TSD_CHECK_EQ_RETURN_RUNWARN_LOG(
            drvRet == DRV_ERROR_SOCKET_CLOSE, TSD_HDC_SERVER_CLIENT_SOCKET_CLOSED, "halHdcSend return socket close");
...
TSD_StatusT HdcCommon::RecvMsg(HDC_SESSION session, HDCMessage& msg, const uint32_t timeout)
{
    ...
    if ((ret != TSD_OK) || (tempBuf == nullptr) || (bufferLengthOut < HDC_MSG_SHORT_HEAD_SIZE)) {
        ...
        (void)msg.ParseFromArray(
            tempBuf + HDC_MSG_SHORT_HEAD_SIZE,
 ...
 * return Status成功TSD_OK，失败：其他错误码
 */
TSD_StatusT HdcCommon::RecvHdcDefaultMsg(
    const HDC_SESSION& session, drvHdcMsg* drvMsg, char_t*& buffer, uint32_t& bufferLengthOut, const uint32_t timeout)
{
    ...
    hdcError_t drvRet = DRV_ERROR_NONE;
    {
        const std::lock_guard<std::mutex> lk(hdcSessionMutex_);
        drvRet = halHdcRecv(
            session, drvMsg, static_cast<int32_t>(GetMsgMaxSize()), static_cast<uint64_t>(HDC_FLAG_WAIT_TIMEOUT),
            &recvBufCounter, timeout);
    }
    ...
    drvRet = drvHdcGetMsgBuffer(drvMsg, static_cast<int32_t>(HDC_DEFAULT_BUFF_INDEX), &buffer, &receivedLenEachTime);
    if (drvRet != DRV_ERROR_NONE) {
