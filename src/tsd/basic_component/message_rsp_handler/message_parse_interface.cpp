...
void MessageParseInterface::ProcessMessage(const uint32_t sessionID, const HDCMessage& msg) const
{
    ParseFuncType func = nullptr;
    {
        ...
        if (pos != parseFuncMap_.end()) {
            ...
            TSD_RUN_INFO(
                "Version is not matched(message_type:%u), you need update the software, "
                "FuncMap size = %zu",
                static_cast<uint32_t>(type), parseFuncMap_.size());
            uint32_t i = 0;
            for (auto iter = parseFuncMap_.begin(); iter != parseFuncMap_.end(); iter++, i++) {
