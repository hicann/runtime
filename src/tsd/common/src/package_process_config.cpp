...
bool PackageProcessConfig::SetConfigDataOnServer(const SinkPackageConfig& hdcConfig)
{
    ...
    if (iter != configMap_.end()) {
        ...
        TSD_RUN_INFO(
            "update package:%s config, dest dir:%u", pkgName.c_str(), static_cast<uint32_t>(iter->second.decDstDir));
    } else {
...
bool PackageProcessConfig::ParseSinglePara(
    std::string& inputLine, PackConfDetail& tempNode, std::unordered_set<std::string>& finishedParseItemSet) const
{
...
bool PackageProcessConfig::SetConfigDataOnHost(
    std::ifstream& inFile, const std::string& fileName, const std::string& pkgTitle)
{
    std::string inputLine;
    uint32_t itCnt = 0U;
...
void PackageProcessConfig::ConstructPkgConfigMsg(HDCMessage& hdcMsg)
{
    ...
    const std::string hashCode =
        CalFileSha256HashValue(GetHostFilePath(COMMON_SINK_PKG_CONFIG_DIR, COMMON_SINK_PKG_CONFIG_NAME));
...
void PackageProcessConfig::RefreshHostPluginVersions()
{
    for (auto& kv : configMap_) {
        ...
        if (!PluginPkgVersionUtil::ParseIniFile(iniPath, info)) {
            TSD_RUN_WARN(
                "parse plugin pkg:%s ini file:%s failed, fallback to checkcode compare", pkgName.c_str(),
                iniPath.c_str());
        } else {
            TSD_RUN_INFO(
                "host plugin pkg:%s ini parsed, version:%s timestamp:%s", pkgName.c_str(), info.version.c_str(),
...
TSD_StatusT PackageProcessConfig::GetPkgHostAndDeviceDstPath(
    const std::string& pkgName, std::string& orgFile, std::string& dstFile, const pid_t hostPid)
{
...
bool PackageProcessConfig::SetPkgHostTruePath(
    PackConfDetail& tempNode, const std::string& pkgName, const std::string& pkgTitle) const
{
