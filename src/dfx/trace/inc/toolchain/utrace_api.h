...
TRACE_EXPORT void UtraceStructItemSet(
    TraceStructEntry* entry, const char* name, uint8_t type, uint8_t mode, uint16_t length) __attribute((weak));
TRACE_EXPORT void UtraceStructEntryExit(TraceStructEntry* entry) __attribute((weak));

#define TRACE_STRUCT_INIT_ENTRY                          \
    {                                                    \
        .name = {0}, .list = UtraceStructEntryListInit() \
    }
