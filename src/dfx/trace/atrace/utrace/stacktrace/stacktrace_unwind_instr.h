...
typedef enum CfaType { VOS_CFA_UNSET, VOS_CFA_REG_OFFSET, VOS_CFA_EXP } CfaType;
...
#define VOS_IS_NO_PUSH_CODE(opcode)                                                                                 \
    ((DW_OP_ROT == (opcode)) || (DW_OP_DROP == (opcode)) || (DW_OP_SWAP == (opcode)) || (DW_OP_SKIP == (opcode)) || \
     (DW_OP_BRA == (opcode)) || (DW_OP_NOP == (opcode)))
...
#define VOS_INS_PC_LOC(opcode) ((DW_CFA_SET_LOC <= (opcode)) && (DW_CFA_ADVANCE_LOC4 >= (opcode)))
#define VOS_INS_REG_DEF(opcode)                                                                            \
    ((DW_CFA_OFFSET_EXTENDED == (opcode)) || (DW_CFA_RESTORE_EXTENDED == (opcode)) ||                      \
     (DW_CFA_SAME_VALUE == (opcode)) || (DW_CFA_UNDEFINED == (opcode)) || (DW_CFA_REGISTER == (opcode)) || \
     (DW_CFA_EXPRESSION == (opcode)) || (DW_CFA_OFF_EXTENDED_SF == (opcode)) || (DW_CFA_GNU_NEG_OFF_EXT == (opcode)))
#define VOS_INS_CFA_DEF(opcode)                                                                                      \
    ((DW_CFA_DEF_CFA == (opcode)) || (DW_CFA_DEF_CFA_REGISTER == (opcode)) || (DW_CFA_DEF_CFA_OFFSET == (opcode)) || \
     (DW_CFA_DEF_CFA_EXPRESSION == (opcode)) || (DW_CFA_DEF_CFA_SF == (opcode)) ||                                   \
     (DW_CFA_DEF_CFA_OFFSET_SF == (opcode)))
#define VOS_INS_REG_VALUE_DEF(opcode) \
    ((DW_CFA_VAL_OFFSET == (opcode)) || (DW_CFA_VAL_OFFSET_SF == (opcode)) || (DW_CFA_VAL_EXPRESSION == (opcode)))
#define VOS_INS_OTHER_DEF(opcode)                                                                             \
    ((DW_CFA_NOP == (opcode)) || (DW_CFA_REMEMBER_STATE == (opcode)) || (DW_CFA_RESTORE_STATE == (opcode)) || \
     (DW_CFA_GNU_ARG_SIZE == (opcode)) || (DW_CFA_GNU_WIN_SAVE == (opcode)))

/* OP指令圈复杂度优化 */
#define VOS_ENC_OP_LIT(opType) ((DW_OP_LIT0 <= (opType)) && (DW_OP_LIT31 >= (opType)))
#define VOS_ENC_OP_REG(opType) ((DW_OP_REG0 <= (opType)) && (DW_OP_REG31 >= (opType)))
#define VOS_ENC_OP_BREG(opType) ((DW_OP_BREG0 <= (opType)) && (DW_OP_BREG31 >= (opType)))
#define VOS_ENC_OP_CONST(opType) ((DW_OP_COUST1U <= (opType)) && (DW_OP_COUSTS >= (opType)))
...
#define VOS_ENC_OP_TWO_DATA(opType)                                                                                \
    ((DW_OP_AND == (opType)) || (DW_OP_DIV == (opType)) || (DW_OP_MINUS == (opType)) || (DW_OP_MOD == (opType)) || \
     (DW_OP_MUL == (opType)) || (DW_OP_OR == (opType)) || (DW_OP_PLUS == (opType)) || (DW_OP_SHL == (opType)) ||   \
     (DW_OP_SHR == (opType)) || (DW_OP_SHRA == (opType)) || (DW_OP_XOR == (opType)) || (DW_OP_LE == (opType)) ||   \
     (DW_OP_GE == (opType)) || (DW_OP_EQ == (opType)) || (DW_OP_LT == (opType)) || (DW_OP_GT == (opType)) ||       \
     (DW_OP_NE == (opType)))
#define VOS_ENC_OP_TWO_DATA1(opType)                                                                               \
    ((DW_OP_AND == (opType)) || (DW_OP_DIV == (opType)) || (DW_OP_MINUS == (opType)) || (DW_OP_MOD == (opType)) || \
     (DW_OP_MUL == (opType)) || (DW_OP_OR == (opType)) || (DW_OP_PLUS == (opType)))
#define VOS_ENC_OP_TWO_DATA2(opType)                                                                              \
    ((DW_OP_SHL == (opType)) || (DW_OP_SHR == (opType)) || (DW_OP_SHRA == (opType)) || (DW_OP_XOR == (opType)) || \
     (DW_OP_LE == (opType)) || (DW_OP_GE == (opType)) || (DW_OP_EQ == (opType)) || (DW_OP_LT == (opType)) ||      \
     (DW_OP_GT == (opType)) || (DW_OP_NE == (opType)))
#define VOS_ENC_OP_OTHER_OPR(opType)                                                            \
    ((DW_OP_REGX == (opType)) || (DW_OP_BREGX == (opType)) || (DW_OP_ADDR == (opType)) ||       \
     (DW_OP_GNU_ENC_ADDR == (opType)) || (DW_OP_SKIP == (opType)) || (DW_OP_BRA == (opType)) || \
     (DW_OP_NOP == (opType)))

#define TRACE_STACK_ADDR_INVALID_CHECK(args, vsp) \
    (((vsp) >= (args)->stackMaxAddr) || ((vsp) < (args)->stackMinAddr) || (((vsp)&3U) != 0))
