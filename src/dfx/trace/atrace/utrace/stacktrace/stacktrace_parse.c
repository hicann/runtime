...
static void TraceWriteProcessReg(int32_t fd, const ScProcessInfo* info)
{
    ...
    ret = snprintf_s(
        tmpBuf, MAX_BUFFER_LENGTH, MAX_BUFFER_LENGTH - 1U,
        "crash registers:\n"
        ...
        info->regs[SCD_REGS_X0], info->regs[SCD_REGS_X1], info->regs[SCD_REGS_X2], info->regs[SCD_REGS_X3],
        info->regs[SCD_REGS_X4], info->regs[SCD_REGS_X5], info->regs[SCD_REGS_X6], info->regs[SCD_REGS_X7],
        info->regs[SCD_REGS_X8], info->regs[SCD_REGS_X9], info->regs[SCD_REGS_X10], info->regs[SCD_REGS_X11],
        info->regs[SCD_REGS_X12], info->regs[SCD_REGS_X13], info->regs[SCD_REGS_X14], info->regs[SCD_REGS_X15],
        info->regs[SCD_REGS_X16], info->regs[SCD_REGS_X17], info->regs[SCD_REGS_X18], info->regs[SCD_REGS_X19],
        info->regs[SCD_REGS_X20], info->regs[SCD_REGS_X21], info->regs[SCD_REGS_X22], info->regs[SCD_REGS_X23],
        info->regs[SCD_REGS_X24], info->regs[SCD_REGS_X25], info->regs[SCD_REGS_X26], info->regs[SCD_REGS_X27],
        info->regs[SCD_REGS_X28], info->regs[SCD_REGS_X29], info->regs[SCD_REGS_SP], info->regs[SCD_REGS_LR],
    ...
    ret = snprintf_s(
        ...
        info->regs[SCD_REGS_RAX], info->regs[SCD_REGS_RBX], info->regs[SCD_REGS_RCX], info->regs[SCD_REGS_RDX],
        info->regs[SCD_REGS_R8], info->regs[SCD_REGS_R9], info->regs[SCD_REGS_R10], info->regs[SCD_REGS_R11],
        info->regs[SCD_REGS_R12], info->regs[SCD_REGS_R13], info->regs[SCD_REGS_R14], info->regs[SCD_REGS_R15],
        info->regs[SCD_REGS_RDI], info->regs[SCD_REGS_RSI], info->regs[SCD_REGS_RBP], info->regs[SCD_REGS_RSP],
...
{
    char tmpBuf[MAX_BUFFER_LENGTH] = {0};
    int32_t ret = snprintf_s(tmpBuf, MAX_BUFFER_LENGTH, MAX_BUFFER_LENGTH - 1U, "kernel version: unknown\n");
    if (ret == -1) {
        LOGE("snprintf_s failed, ret = %d, errno = %d.", ret, errno);
        return;
    }
    ...
    if (fgets(kernelVersion, (int32_t)sizeof(kernelVersion), fp) != NULL) {
        ret = snprintf_s(tmpBuf, MAX_BUFFER_LENGTH, MAX_BUFFER_LENGTH - 1U, "kernel version: %s", kernelVersion);
        if (ret == -1) {
            LOGE("snprintf_s failed, ret = %d, errno = %d.", ret, errno);
        }
    }
...
static void TraceWriteProcess(int32_t fd, const ScProcessInfo* info)
{
    ...
    int32_t ret = snprintf_s(
        tmpBuf, MAX_BUFFER_LENGTH, MAX_BUFFER_LENGTH - 1U,
        PROCESS_SECTION "crash task:%s\n"
                        "crash pid:%d\n"
                        "crash tid:%d\n"
                        "crash stack base:0x%016lx\n"
                        "crash stack top:0x%016lx\n"
                        "crash reason: signal %d\n",
        info->task, info->pid, info->crashTid, info->baseAddr, info->topAddr, info->signo);
    if (ret == -1) {
        LOGE("snprintf_s failed, ret = %d, errno = %d.", ret, errno);
        return;
    }
...
static void TraceWriteStack(int32_t fd, const struct ScThreadInfo* data)
{
    ...
    for (uint32_t i = 0; i < MAX_THREAD_NUM; i++) {
        ...
        int32_t ret = snprintf_s(
            tmpBuf, MAX_BUFFER_LENGTH, MAX_BUFFER_LENGTH - 1U, "Thread %u (%d, %s)\n", i + 1U, data[i].tid,
            data[i].name);
        if (ret == -1) {
            LOGE("snprintf_s failed, ret = %d, errno = %d.", ret, errno);
            return;
