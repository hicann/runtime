...
uintptr_t ScdElfGetLoadBias(const ScdElf* elf) { return elf->loadBias; }

size_t ScdElfGetFdeNum(const ScdElf* elf) { return elf->ehFrameHdrSize / SCD_ELF_FDE_ENTRY_SIZE; }
...
void ScdElfGetFunctionInfo(ScdElf* elf, uintptr_t pc, char* funcName, size_t len, size_t* funcOffset)
{
    ...
    LIST_FOR_EACH(pos, &elf->symbolList.list)
    {
        ...
        for (size_t offset = symbols->symOffset; offset < symbols->symEnd; offset += symbols->symEntrySize) {
            ...
            if (ScdMemoryReadString(
                    elf->memory, strOffset, buf, SCD_MIN(SCD_FUNC_NAME_LENGTH, symbols->strEnd - strOffset)) == 0) {
                continue;
            }
...
{
    ElfW(Shdr)* shdr = ScdMemoryGetAddr(
        elf->memory, ehdr->e_shoff + (uintptr_t)ehdr->e_shstrndx * (uintptr_t)ehdr->e_shentsize, sizeof(ElfW(Shdr)));
    if (shdr == NULL) {
...
STATIC TraStatus ScdElfHandleDynsym(ScdElf* elf, ElfW(Ehdr) * ehdr, ElfW(Shdr) * shdr)
{
    ...
    ElfW(Shdr)* strShdr = (ElfW(Shdr)*)ScdMemoryGetAddr(
        elf->memory, ehdr->e_shoff + (uintptr_t)shdr->sh_link * (uintptr_t)ehdr->e_shentsize, sizeof(ElfW(Shdr)));
