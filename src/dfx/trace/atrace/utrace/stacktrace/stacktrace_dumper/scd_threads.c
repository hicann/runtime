...
TraStatus ScdThreadsRecord(int32_t fd, const ScdThreads* thds)
{
    ...
    LIST_FOR_EACH(pos, &thds->thdList.list)
    {
        ...
        int32_t err = snprintf_s(
            tmpBuf, SCD_FRAME_LENGTH, SCD_FRAME_LENGTH - 1U, "Thread %d (%d, %s)\n", threadIdx, threadInfo->tid,
            threadInfo->tname);
        if (err == -1) {
        ...
        LIST_FOR_EACH(posFrame, &threadInfo->frames.frameList.list)
        {
            ...
            if (strlen(frame->funcName) == 0) {
                err = snprintf_s(
                    ...
                    frame->pc, frame->base, frame->soName);
            } else {
                err = snprintf_s(
...
TraStatus ScdThreadsInit(ScdThreads* thds, int32_t pid, int32_t crashTid, ucontext_t* uc)
{
    ...
    SCD_DLOG_DBG(
        "init all threads successfully, pid=%d, crash tid=%d, pc=0x%lx, sp=0x%lx, fp=0x%lx.", pid, crashTid,
        GET_PCREG(thds->ucRegs.r), GET_SPREG(thds->ucRegs.r), GET_FPREG(thds->ucRegs.r));
    return TRACE_SUCCESS;
}
