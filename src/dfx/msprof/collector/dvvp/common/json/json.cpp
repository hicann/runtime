...
std::unordered_map<uint8_t, ToJsonFunc> g_toJsonFuncMap = {{static_cast<uint8_t>(JsonValueType::OBJECT), ObjToJson},
...
inline void RemoveWhitespaces(std::string& str) { str.erase(0, str.find_first_not_of(" \r\n\t\v\f")); }

inline void RemoveRedundantSym(std::string& str) { str.erase(0, str.find_first_not_of(" \r\n\t\v\f,")); }
...
std::string BoolToJson(const JsonValue& value) { return (value.value.boolValue) ? "true" : "false"; }

std::string IntToJson(const JsonValue& value) { return std::to_string(value.value.intValue); }

std::string UintToJson(const JsonValue& value) { return std::to_string(value.value.uintValue); }

std::string DoubleToJson(const JsonValue& value) { return std::to_string(value.value.doubleValue); }
...
void FreeObj(JsonValue& value)
{
    if (value.value.objectValue != nullptr) {
        for (auto iter = (*value.value.objectValue).begin(); iter != (*value.value.objectValue).end();) {
...
std::string JsonValue::operator()() const { return NanoJson::helper::ToString(*this); }
...
template <>
int64_t JsonValue::GetValue<int64_t>() const
{
    if ((this->type == JsonValueType::BOOLEAN) || (this->type == JsonValueType::UINT) ||
        (this->type == JsonValueType::INT)) {
...
template <>
uint64_t JsonValue::GetValue<uint64_t>() const
{
    if ((this->type == JsonValueType::BOOLEAN) || (this->type == JsonValueType::UINT) ||
        (this->type == JsonValueType::INT)) {
...
template <>
int32_t JsonValue::GetValue<int32_t>() const
{
    if ((this->type == JsonValueType::BOOLEAN) || (this->type == JsonValueType::UINT) ||
        (this->type == JsonValueType::INT)) {
...
template <>
uint32_t JsonValue::GetValue<uint32_t>() const
{
    if ((this->type == JsonValueType::BOOLEAN) || (this->type == JsonValueType::UINT) ||
        (this->type == JsonValueType::INT)) {
...
template <>
JsonArray JsonValue::GetValue<JsonArray>() const
{
...
template <>
JsonObj JsonValue::GetValue<JsonObj>() const
{
...
Json::Json(const std::string& json) { NanoJson::Json::Parse(json); }
...
void Json::Parse(const std::string& json)
{
    std::string str = json;
    try {
        if ((!helper::Parse(str, value_)) || (value_.type != JsonValueType::OBJECT)) {
            helper::MemoryFree(value_);
...
JsonValue& Json::operator[](const std::string& key) { return value_[key]; }
...
std::string Json::ToString() const { return value_(); }

JsonValue Json::GetJsonValue() const { return value_; }

bool Json::Contains(const std::string& key) const { return value_.Contains(key); }
