...
#define MSVP_MAKE_SHARED0(instance, Type, action)                                                                   \
    do {                                                                                                            \
        try {                                                                                                       \
            ...
            MSPROF_ENV_ERROR(                                                                                       \
                "EK0202", std::vector<std::string>({"operation"}), std::vector<std::string>({"std::make_shared"})); \
            action;                                                                                                 \
        }                                                                                                           \
    } while (0)
...
#define FUNRET_CHECK_RET_VAL(EXPR)                    \
    do {                                              \
        if (EXPR) {                                   \
...
#define FUNRET_CHECK_RET_VALUE(EXPR, action)          \
    do {                                              \
        if (EXPR) {                                   \
            MSPROF_LOGE("Function ret check failed"); \
            action;                                   \
...
#define FUNRET_CHECK_FAIL_PRINT(EXPR)                 \
    do {                                              \
        if (EXPR) {                                   \
            MSPROF_LOGE("Function ret check failed"); \
        }                                             \
...
#define FUNRET_CHECK_NULL_PTR(PTR, action)  \
    do {                                    \
        if ((PTR) == nullptr) {             \
...
enum class VolumeSize { AVAIL_SIZE = 0, FREE_SIZE, TOTAL_SIZE };
...
public:
    ...
    static void SetArgEnv(
        CHAR_PTR_CONST argv[], const int32_t argvCount, CHAR_PTR_CONST envp[], const int32_t envCount,
        OsalArgvEnv& argvEnv);
    static int32_t DoCreateCmdProcess(
        const std::string& stdoutRedirectFile, const std::string& fileName, const OsalArgvEnv& argvEnv,
        OsalProcess& tid);
    ...
    static std::vector<std::string> Split(
        const std::string& input_str, bool filter_out_enabled = false, const std::string& filter_out = "",
