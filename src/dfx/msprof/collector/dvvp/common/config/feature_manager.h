...
struct FeatureRecord {
    ...
    FeatureRecord(
        std::string tempFeatureName, std::string tempCompatibility, std::string tempFeatureVersion,
        std::string tempAffectedComponent, std::string tempAffectedComponentVersion, std::string tempInfoLog) noexcept
    {
        ...
        FUNRET_CHECK_EXPR_PRINT(
            ret != EOK, "Failed to copy feature name %s to struct and ret is %d", tempFeatureName.c_str(), ret);
        ret = strcpy_s(info.compatibility, sizeof(info.compatibility), tempCompatibility.c_str());
        FUNRET_CHECK_EXPR_PRINT(
            ret != EOK, "Failed to copy compatibility %s to struct and ret is %d", tempCompatibility.c_str(), ret);
        ret = strcpy_s(info.featureVersion, sizeof(info.featureVersion), tempFeatureVersion.c_str());
        FUNRET_CHECK_EXPR_PRINT(
            ret != EOK, "Failed to copy featureVersion %s to struct and ret is %d", tempFeatureVersion.c_str(), ret);
        ret = strcpy_s(info.affectedComponent, sizeof(info.affectedComponent), tempAffectedComponent.c_str());
        FUNRET_CHECK_EXPR_PRINT(
            ret != EOK, "Failed to copy affectedComponent %s to struct and ret is %d", tempAffectedComponent.c_str(),
            ret);
        ret = strcpy_s(
            info.affectedComponentVersion, sizeof(info.affectedComponentVersion), tempAffectedComponentVersion.c_str());
        FUNRET_CHECK_EXPR_PRINT(
            ret != EOK, "Failed to copy affectedComponentVersion %s to struct and ret is %d",
            tempAffectedComponentVersion.c_str(), ret);
        ret = strcpy_s(info.infoLog, sizeof(info.infoLog), tempInfoLog.c_str());
        FUNRET_CHECK_EXPR_PRINT(
            ret != EOK, "Failed to copy log info %s to struct and ret is %d", tempInfoLog.c_str(), ret);
    }
