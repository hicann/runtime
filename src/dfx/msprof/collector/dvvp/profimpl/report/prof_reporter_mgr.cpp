...
void ProfReporterMgr::FlushAdditonalData() { reporters_[ADDITIONAL].ForceFlush(); }
...
int32_t ProfReporterMgr::RegReportTypeInfo(uint16_t level, uint32_t typeId, const std::string& typeName)
{
    ...
    auto itr = std::find_if(
        reportTypeInfoMapVec_[level].begin(), reportTypeInfoMapVec_[level].end(),
...
int32_t ProfReporterMgr::RegReportDataFormat(uint16_t level, uint32_t typeId, const std::string& dataFormat)
{
    ...
    auto itr = std::find_if(
        reportDataFormatMapVec_[level].begin(), reportDataFormatMapVec_[level].end(),
        [typeId](const std::pair<uint32_t, std::string>& pair) { return pair.first == typeId; });
...
uint64_t ProfReporterMgr::GetHashId(const std::string& info) const { return HashData::instance()->GenHashId(info); }

std::string& ProfReporterMgr::GetHashInfo(uint64_t hashId) const { return HashData::instance()->GetHashInfo(hashId); }

void ProfReporterMgr::FillData(
...
void ProfReporterMgr::SaveDataFormat(bool isLastChunk)
{
    ...
    for (auto& dataFormat : reportDataFormatMapVec_) {
        ...
        for (size_t i = indexMap_[dataFormat.first]; i < currentHashSize; i++) {
            saveHashData.append(
                std::to_string(dataFormat.first) + "_" + std::to_string(dataFormat.second[i].first) +
        ...
        if (ret != PROFILING_SUCCESS) {
            MSPROF_LOGE(
                "Data format upload data failed, level: %u, dataLen:%zu bytes", dataFormat.first, saveHashData.size());
            continue;
        }
        MSPROF_EVENT(
            "total_size_data_format[%u], save data format length: %zu bytes, data format size: %zu", dataFormat.first,
            saveHashData.size(), dataFormat.second.size());
    }
}
...
void ProfReporterMgr::SaveData(bool isLastChunk)
{
    ...
    for (auto& typeInfo : reportTypeInfoMapVec_) {
        ...
        for (size_t i = indexMap_[typeInfo.first]; i < currentHashSize; i++) {
            saveHashData.append(
                std::to_string(typeInfo.first) + "_" + std::to_string(typeInfo.second[i].first) + HASH_DIC_DELIMITER +
                typeInfo.second[i].second + "\n");
        }
        if (saveHashData.empty()) {
            continue;
        }
        ...
        if (ret != PROFILING_SUCCESS) {
            MSPROF_LOGE(
                "Type info upload data failed, level: %u, dataLen:%zu bytes", typeInfo.first, saveHashData.size());
            continue;
        }
        MSPROF_EVENT(
            "total_size_type_info[%u], save type info length: %zu bytes, type info size: %zu", typeInfo.first,
            saveHashData.size(), typeInfo.second.size());
    }
}
...
void ProfReporterMgr::FlushMstxData() { Collector::Dvvp::Mstx::MstxDataHandler::instance()->Stop(); }
