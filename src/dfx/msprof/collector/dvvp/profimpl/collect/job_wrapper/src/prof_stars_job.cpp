...
ProfFftsProfileJob::ProfFftsProfileJob()
    : channelId_(PROF_CHANNEL_UNKNOWN),
      ...
      aicMode_(0),
      aivMode_(0),
      aicPeriod_(DEFAULT_PERIOD_TIME),
      aivPeriod_(DEFAULT_PERIOD_TIME)
{}
ProfFftsProfileJob::~ProfFftsProfileJob() {}
...
int32_t ProfFftsProfileJob::Init(const SHARED_PTR_ALIA<CollectionJobCfg> cfg)
{
    ...
    MSPROF_LOGI(
        "ffts profile init success, channelId_(0x%x), cfgMode(0x%x), aicMode(0x%x), aicPeriod(%u),"
        "aivMode(0x%x), aivPeriod(%u)",
        channelId_, cfgMode_, aicMode_, aicPeriod_, aivMode_, aivPeriod_);
    return PROFILING_SUCCESS;
}
...
int32_t ProfFftsProfileJob::Process()
{
    ...
    FUNRET_CHECK_EXPR_ACTION(configP == nullptr, return PROFILING_FAILED, "StarsAccProfileConfigT ProfMalloc failed.");
    ...
    if (Platform::instance()->GetMaxMonitorNumber() == ACC_PMU_EVENT_MAX_NUM) {
        ret = DrvAccProfileStart(
            drvPeripheralProfileCfg, *collectionJobCfg_->jobParams.cores, *collectionJobCfg_->jobParams.events,
            *collectionJobCfg_->jobParams.aivCores, *collectionJobCfg_->jobParams.aivEvents);
    } else {
        ret = DrvFftsProfileStart(
            drvPeripheralProfileCfg, *collectionJobCfg_->jobParams.cores, *collectionJobCfg_->jobParams.events,
            *collectionJobCfg_->jobParams.aivCores, *collectionJobCfg_->jobParams.aivEvents);
    }
...
ProfStarsSocProfileJob::ProfStarsSocProfileJob() { channelId_ = PROF_CHANNEL_STARS_SOC_PROFILE; }
...
int32_t ProfStarsSocProfileJob::Init(const SHARED_PTR_ALIA<CollectionJobCfg> cfg)
{
    ...
    if (Platform::instance()->GetMaxMonitorNumber() == ACC_PMU_EVENT_MAX_NUM &&
        ...
        cfg->comParams->params->hardware_mem != MSVP_PROF_ON && cfg->comParams->params->sysLp != MSVP_PROF_ON) {
...
void ProfStarsSocProfileJob::SetConfigP(int32_t& period, StarsSocProfileConfigT* configP) const
{
    if (collectionJobCfg_->comParams->params->interconnection_profiling == MSVP_PROF_ON) {
        ...
        configP->interChip.period =
            (!Platform::instance()->CheckIfSupport(PLATFORM_SYS_DEVICE_US)) ? period : (period * US_CONVERT_MS);
        configP->interDie.innerSwitch = TS_PROFILE_COMMAND_TYPE_PROFILING_ENABLE;
        configP->interDie.period =
            (!Platform::instance()->CheckIfSupport(PLATFORM_SYS_DEVICE_US)) ? period : (period * US_CONVERT_MS);
    }
    ...
    MSPROF_LOGI(
        "SocProfileParam: accPmu:[%d, %d], onChip:[%d, %d], interChip:[%d, %d], power:[%d, %d]",
        configP->accPmu.innerSwitch, configP->accPmu.period, configP->onChip.innerSwitch, configP->onChip.period,
        configP->interChip.innerSwitch, configP->interChip.period, configP->power.innerSwitch, configP->power.period);
}
...
int32_t ProfStarsSocProfileJob::SetPeripheralConfig()
{
    ...
    configP = static_cast<StarsSocProfileConfigT*>(Utils::ProfMalloc(static_cast<size_t>(configSize)));
    if (configP == nullptr) {
    ...
    if (Platform::instance()->CheckIfSupport(PLATFORM_STARS_QOS)) {
        ...
        StarsSocQosProfileConfigT* withQosConfigP =
            static_cast<StarsSocQosProfileConfigT*>(Utils::ProfMalloc(static_cast<size_t>(withQosConfigSize)));
