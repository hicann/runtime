...
static const CpuTypes CPU_TYPES[] = {
    {0x41d03, "ARMv8_Cortex_A53"}, {0x41d05, "ARMv8_Cortex_A55"}, {0x41d07, "ARMv8_Cortex_A57"},
    {0x41d08, "ARMv8_Cortex_A72"}, {0x41d09, "ARMv8_Cortex_A73"}, {0x48d01, "TaishanV110"},
};
...
static int32_t GetDevInfo(uint32_t deviceId, DeviceInfo* devInfo)
{
    ...
    PROF_CHK_EXPR_ACTION(
        ret == PROFILING_FAILED, return PROFILING_FAILED, "Failed to HalGetEnvType, deviceId: %d", deviceId);
    devInfo->envType = ret;
    ret = HalGetCtrlCpuId(deviceId);
    PROF_CHK_EXPR_ACTION(
        ret == PROFILING_FAILED, return PROFILING_FAILED, "Failed to HalGetCtrlCpuId, deviceId: %d", deviceId);
    devInfo->ctrlCpuId = ret;
    ret = HalGetCtrlCpuCoreNum(deviceId);
    PROF_CHK_EXPR_ACTION(
        ret == PROFILING_FAILED, return PROFILING_FAILED, "Failed to HalGetCtrlCpuCoreNum, deviceId: %d", deviceId);
    devInfo->ctrlCpuCoreNum = ret;
    ret = HalGetCtrlCpuEndianLittle(deviceId);
    PROF_CHK_EXPR_ACTION(
        ret == PROFILING_FAILED, return PROFILING_FAILED, "Failed to HalGetCtrlCpuEndianLittle, deviceId: %d",
        deviceId);
    devInfo->ctrlCpuEndianLittle = ret;
    ret = HalGetAiCpuCoreNum(deviceId);
    PROF_CHK_EXPR_ACTION(
        ret == PROFILING_FAILED, return PROFILING_FAILED, "Failed to HalGetAiCpuCoreNum, deviceId: %d", deviceId);
    devInfo->aiCpuCoreNum = ret;
    if (devInfo->aiCpuCoreNum != 0) {
        ret = HalGetAiCpuCoreId(deviceId);
        PROF_CHK_EXPR_ACTION(
            ret == PROFILING_FAILED, return PROFILING_FAILED, "Failed to HalGetAiCpuCoreId, deviceId: %d", deviceId);
        devInfo->aiCpuCoreId = ret;
    }
    ret = HalGetAiCpuOccupyBitmap(deviceId);
    PROF_CHK_EXPR_ACTION(
        ret == PROFILING_FAILED, return PROFILING_FAILED, "Failed to HalGetAiCpuOccupyBitmap, deviceId: %d", deviceId);
    devInfo->aiCpuOccupyBitMap = ret;
    ret = HalGetTsCpuCoreNum(deviceId);
    PROF_CHK_EXPR_ACTION(
        ret == PROFILING_FAILED, return PROFILING_FAILED, "Failed to HalGetTsCpuCoreNum, deviceId: %d", deviceId);
    devInfo->tsCpuCoreNum = ret;
    ret = HalGetAiCoreId(deviceId);
    PROF_CHK_EXPR_ACTION(
        ret == PROFILING_FAILED, return PROFILING_FAILED, "Failed to HalGetAiCoreId, deviceId: %d", deviceId);
    devInfo->aiCoreId = ret;
    ret = HalGetAiCoreNum(deviceId);
    PROF_CHK_EXPR_ACTION(
        ret == PROFILING_FAILED, return PROFILING_FAILED, "Failed to HalGetAiCoreNum, deviceId: %d", deviceId);
    devInfo->aiCoreNum = ret;
    ret = HalGetAiVectorCoreNum(deviceId);
    PROF_CHK_EXPR_ACTION(
        ret == PROFILING_FAILED, return PROFILING_FAILED, "Failed to HalGetAiVectorCoreNum, deviceId: %d", deviceId);
    devInfo->aivNum = ret;
    return PROFILING_SUCCESS;
}
 ...
 */
static void CreateNetCardInfo(void) { return; }
...
static int32_t CreateCpuInfo(void)
{
    ...
    PROF_CHK_EXPR_ACTION_TWICE(
        ret != EOK, OsalCpuInfoFree(cpuInfo, cpuNum), return PROFILING_FAILED, "Failed to strcat_s for hwtype.");
    g_infoJson->cpuCores = (uint32_t)cpuNum;
    g_infoJson->infoCpus = (CpuInfo*)OsalCalloc(sizeof(CpuInfo) * cpuNum);
    PROF_CHK_EXPR_ACTION_TWICE(
        g_infoJson->infoCpus == NULL, OsalCpuInfoFree(cpuInfo, cpuNum), return PROFILING_FAILED,
 ...
 */
static void CreateDrvVersion(void) { g_infoJson->drvVersion = HalGetApiVersion(); }
...
static int32_t AddDeviceInfo(uint32_t deviceId)
{
    ...
    // ctrl cpu
    CreateCpuIdString(g_infoJson->deviceInfos.ctrlCpu, CPU_ID_STRING_LEN, 0, devInfo.ctrlCpuCoreNum);
    // ai cpu
    CreateCpuIdString(
        g_infoJson->deviceInfos.aiCpu, CPU_ID_STRING_LEN, devInfo.aiCpuCoreId,
...
static int32_t AddHostInfo()
{
    ...
    PROF_CHK_WARN_NO_ACTION(
        retVal != OSAL_EN_OK, "An exception was detected when obtaining os version, ret is %d.", retVal);
...
{
    obj->SetValueByKey(obj, "id", (JsonObj){{.intValue = g_infoJson->deviceInfos.id}, .type = CJSON_INT})
        ->SetValueByKey(obj, "env_type", (JsonObj){{.intValue = g_infoJson->deviceInfos.envType}, .type = CJSON_INT})
        ->SetValueByKey(
            obj, "ctrl_cpu_core_num",
            (JsonObj){{.intValue = g_infoJson->deviceInfos.ctrlCpuCoreNum}, .type = CJSON_INT})
        ->SetValueByKey(
            obj, "ctrl_cpu_endian_little",
            (JsonObj){{.intValue = g_infoJson->deviceInfos.ctrlCpuEndianLittle}, .type = CJSON_INT})
        ->SetValueByKey(
            obj, "ts_cpu_core_num", (JsonObj){{.intValue = g_infoJson->deviceInfos.tsCpuCoreNum}, .type = CJSON_INT})
        ->SetValueByKey(
            obj, "ai_cpu_core_num", (JsonObj){{.intValue = g_infoJson->deviceInfos.aiCpuCoreNum}, .type = CJSON_INT})
        ->SetValueByKey(
            obj, "ai_core_num", (JsonObj){{.intValue = g_infoJson->deviceInfos.aiCoreNum}, .type = CJSON_INT})
        ->SetValueByKey(
            obj, "ai_cpu_core_id", (JsonObj){{.intValue = g_infoJson->deviceInfos.aiCpuCoreId}, .type = CJSON_INT})
        ->SetValueByKey(
            obj, "aicpu_occupy_bitmap",
            (JsonObj){{.intValue = g_infoJson->deviceInfos.aiCpuOccupyBitMap}, .type = CJSON_INT})
        ->SetValueByKey(obj, "aiv_num", (JsonObj){{.intValue = g_infoJson->deviceInfos.aivNum}, .type = CJSON_INT})
        ->SetValueByKey(
            obj, "ctrl_cpu_id", (JsonObj){{.stringValue = g_infoJson->deviceInfos.ctrlCpuId}, .type = CJSON_STRING})
        ->SetValueByKey(
            obj, "ctrl_cpu", (JsonObj){{.stringValue = g_infoJson->deviceInfos.ctrlCpu}, .type = CJSON_STRING})
        ->SetValueByKey(obj, "ai_cpu", (JsonObj){{.stringValue = g_infoJson->deviceInfos.aiCpu}, .type = CJSON_STRING})
        ->SetValueByKey(
            obj, "hwts_frequency",
...
{
    obj->SetValueByKey(obj, "Id", (JsonObj){{.intValue = g_infoJson->infoCpus[i].id}, .type = CJSON_INT})
        ->SetValueByKey(obj, "Name", (JsonObj){{.stringValue = g_infoJson->infoCpus[i].name}, .type = CJSON_STRING})
        ->SetValueByKey(
            obj, "Frequency", (JsonObj){{.stringValue = g_infoJson->infoCpus[i].frequency}, .type = CJSON_STRING})
        ->SetValueByKey(
            obj, "Logical_CPU_Count",
            (JsonObj){{.stringValue = g_infoJson->infoCpus[i].logicalCpuCount}, .type = CJSON_STRING})
        ->SetValueByKey(obj, "Type", (JsonObj){{.stringValue = g_infoJson->infoCpus[i].type}, .type = CJSON_STRING});
}
...
static void DataFilling(JsonObj* obj, JsonObj* cpuArr)
{
    obj->SetValueByKey(obj, "version", (JsonObj){{.stringValue = g_infoJson->version}, .type = CJSON_STRING})
        ...
        ->SetValueByKey(
            obj, "platform_version", (JsonObj){{.stringValue = g_infoJson->platformVersion}, .type = CJSON_STRING})
...
static int32_t UploadInfoJson(uint32_t deviceId, char* content, size_t contentLen)
{
    ...
    PROF_CHK_EXPR_ACTION_TWICE(
        deviceId > DEFAULT_HOST_ID, OSAL_MEM_FREE(content), return PROFILING_FAILED,
...
{
    PROF_CHK_EXPR_ACTION(
        content == NULL, return PROFILING_FAILED, "UploadCollectionTimeInfo failed with null content.");
    PROF_CHK_EXPR_ACTION_TWICE(
        deviceId > DEFAULT_HOST_ID, OSAL_MEM_FREE(content), return PROFILING_FAILED, "Device id %u is invalid.",
        deviceId);
    ProfFileChunk* chunk = (ProfFileChunk*)OsalMalloc(sizeof(ProfFileChunk));
    if (chunk == NULL) {
        OSAL_MEM_FREE(content);
    ...
    int32_t ret = UploaderUploadData(chunk);
    PROF_CHK_EXPR_ACTION(
        ret != PROFILING_SUCCESS, return PROFILING_FAILED, "Failed to upload CollectionTimeInfo data.");
    return PROFILING_SUCCESS;
}
...
int32_t CreateCollectionTimeInfo(uint32_t deviceId, bool isStartTime)
{
    ...
    if (isStartTime) {
        obj->SetValueByKey(obj, "collectionTimeEnd", (JsonObj){{.stringValue = ""}, .type = CJSON_STRING})
            ...
            ->SetValueByKey(
                obj, "collectionDateBegin", (JsonObj){{.stringValue = hostTimeDateStr}, .type = CJSON_STRING});
    } else {
    ...
    PROF_CHK_EXPR_ACTION(
        ret != PROFILING_SUCCESS, return PROFILING_FAILED, "Failed to upload %s, device: %u.", fileName, deviceId);
    return PROFILING_SUCCESS;
}