...
int32_t CreateSampleJson(ProfileParam* params, uint32_t deviceId)
{
    ...
    g_sampleJson->SetValueByKey(g_sampleJson, "stream_enabled", (JsonObj){{.stringValue = "on"}, .type = CJSON_STRING})
        ->SetValueByKey(
            g_sampleJson, "host_sys_pid", (JsonObj){{.intValue = params->config.hostPid}, .type = CJSON_INT})
        ->SetValueByKey(
            g_sampleJson, "prof_level", (JsonObj){{.stringValue = params->config.profLevel}, .type = CJSON_STRING})
        ->SetValueByKey(
            g_sampleJson, "result_dir", (JsonObj){{.stringValue = params->config.resultDir}, .type = CJSON_STRING})
        ->SetValueByKey(g_sampleJson, "devices", (JsonObj){{.stringValue = devIdStr}, .type = CJSON_STRING})
        ->SetValueByKey(
            g_sampleJson, "aicore_sampling_interval",
            (JsonObj){{.intValue = params->config.aicSamplingInterval}, .type = CJSON_INT})
        ->SetValueByKey(
            g_sampleJson, "aiv_sampling_interval",
            (JsonObj){{.intValue = params->config.aivSamplingInterval}, .type = CJSON_INT})
        ->SetValueByKey(
            g_sampleJson, "ai_core_profiling_mode",
            (JsonObj){{.stringValue = params->config.aiCoreProfilingMode}, .type = CJSON_STRING})
        ->SetValueByKey(
            g_sampleJson, "aiv_profiling_mode",
            (JsonObj){{.stringValue = params->config.aiVectProfilingMode}, .type = CJSON_STRING})
        ->SetValueByKey(
            g_sampleJson, "ai_core_metrics",
            (JsonObj){{.stringValue = params->config.aiCoreMetrics}, .type = CJSON_STRING})
        ->SetValueByKey(
            g_sampleJson, "aiv_metrics", (JsonObj){{.stringValue = params->config.aiVectMetrics}, .type = CJSON_STRING})
        ->SetValueByKey(
            g_sampleJson, "ai_core_profiling_events",
            (JsonObj){{.stringValue = params->config.aicEvents}, .type = CJSON_STRING})
        ->SetValueByKey(
            g_sampleJson, "aiv_profiling_events",
            (JsonObj){{.stringValue = params->config.aivEvents}, .type = CJSON_STRING});
    if (IsEnable(&params->config, PLATFORM_TASK_AIC_METRICS)) {
        g_sampleJson->SetValueByKey(
            g_sampleJson, "ai_core_profiling", (JsonObj){{.stringValue = "on"}, .type = CJSON_STRING});
        g_sampleJson->SetValueByKey(
            g_sampleJson, "aiv_profiling", (JsonObj){{.stringValue = "on"}, .type = CJSON_STRING});
    } else {
        g_sampleJson->SetValueByKey(
            g_sampleJson, "ai_core_profiling", (JsonObj){{.stringValue = "off"}, .type = CJSON_STRING});
        g_sampleJson->SetValueByKey(
            g_sampleJson, "aiv_profiling", (JsonObj){{.stringValue = "off"}, .type = CJSON_STRING});
    }
    OSAL_MEM_FREE(devIdStr);
    char* sampleString = JsonToString(g_sampleJson);
    PROF_CHK_EXPR_ACTION_TWICE(
        sampleString == NULL, JsonFree(g_sampleJson), return PROFILING_FAILED,
