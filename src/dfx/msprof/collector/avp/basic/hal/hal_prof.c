...
int32_t HalProfChannelStop(uint32_t deviceId, uint32_t channelId)
{
    ...
    if (ret != PROF_OK) {
        MSPROF_LOGE("Failed to stop profiling channel[%u] of device[%u], ret=%d.", deviceId, channelId, ret);
...
int32_t HalProfChannelStart(uint32_t deviceId, uint32_t channelId, ChannelStartPara* para)
{
    ...
    if (ret != PROF_OK) {
        MSPROF_LOGE("Failed to start profiling channel[%u] of device[%u], ret=%d.", deviceId, channelId, ret);
        return PROFILING_FAILED;
    }
...
int32_t HalProfChannelRead(uint32_t deviceId, uint32_t channelId, uint8_t* buffer, uint32_t bufferSize)
{
    ...
    if (ret < 0) {
        if (ret == PROF_STOPPED_ALREADY) {
            MSPROF_LOGW(
                "Prof driver channel has been stopped already, device id=%u, channel id=%u, "
                "buffer size=%dbytes",
                deviceId, channelId, bufferSize);
            return 0;
        }
        MSPROF_LOGE(
            "Failed to prof_channel_read, device id=%u, channel id=%u, buffer size=%dbytes, ret=%d", deviceId,
            channelId, bufferSize, ret);
        return PROFILING_FAILED;
    }
