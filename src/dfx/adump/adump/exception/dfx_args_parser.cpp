...
int32_t DfxArgsParser::Init(void* argAddr, uint64_t argSize, const uint8_t* dfxAddr, uint16_t dfxSize)
{
    IDE_CTRL_VALUE_FAILED(
        argAddr != nullptr && argSize != 0, return ADUMP_FAILED, "Invalid arg. argAddr=%p, argSize=%llu.", argAddr,
        argSize);
    IDE_CTRL_VALUE_FAILED(
        dfxAddr != nullptr && dfxSize != 0, return ADUMP_FAILED, "Invalid dfx. dfxAddr=%p, dfxSize=%u.", dfxAddr,
        dfxSize);
    ...
    IDE_CTRL_VALUE_FAILED(
        hostArgsData_ != nullptr, return ADUMP_FAILED, "Copy device args to host failed. argAddr=%p, argSize=%llu.",
        argAddr, argSize);
...
void DfxArgsParser::LogArgsInfo()
{
    ...
    for (uint32_t i = 1; i <= argsLogTimes; ++i) {
        ...
        RecordDumpLog(StrUtils::Format(
            "[AIC_INFO] args(%u to %u) after execute:%s", (i - 1) * ARGS_PER_STRING_MAX_LEN, endIndex,
            ss.str().c_str()));
    }
...
{
    switch (tensorType) {
        case DfxTensorType::INPUT_TENSOR:
            return "INPUT_TENSOR";
        case DfxTensorType::OUTPUT_TENSOR:
            return "OUTPUT_TENSOR";
        case DfxTensorType::WORKSPACE_TENSOR:
            return "WORKSPACE_TENSOR";
        case DfxTensorType::TILING_DATA:
            return "TILING_DATA";
        case DfxTensorType::MC2_CTX:
            return "MC2_CTX";
        case DfxTensorType::SHAPE_TENSOR:
            return "SHAPE_TENSOR";
        case DfxTensorType::GENERAL_TENSOR:
...
const char* DfxArgsParser::GetPointerTypeName(DfxPointerType pointerType)
{
    switch (pointerType) {
        ...
        case DfxPointerType::LEVEL_2_POINTER_WITH_SHAPE:
            return "L2_POINTER_WITH_SHAPE";
        case DfxPointerType::SHAPE_TENSOR_PLACEHOLD:
            return "SHAPE_TENSOR_PLACEHOLD";
        default:
...
int32_t DfxArgsParser::GetShapeData(uint64_t atomicIndex)
{
    ...
    IDE_LOGI(
        "Decode atomicIndex, atomicIndex=0x%llx, bufferId=%u, offset=%u, chunkAddr=%p, chunkSize=%u.", atomicIndex,
        bufferId, offset, chunkAddr, chunkSize);

    IDE_CTRL_VALUE_FAILED(chunkAddr != nullptr, return ADUMP_FAILED, "GetShapeData failed, the chunk memory is null.");
    IDE_CTRL_VALUE_FAILED(
        offset < chunkSize, return ADUMP_FAILED,
    ...
    IDE_CTRL_VALUE_FAILED(
        magicNum == MAGIC_NUM, return ADUMP_FAILED,
        "GetShapeData failed, magic is invalid, atomicIndex=0x%llx, magicAndSpace=0x%llx, magic=0x%llx, "
        "magicMask=0x%llx.",
        atomicIndex, magicAndSpace, magicNum, MAGIC_NUM_MASK);
    IDE_CTRL_VALUE_FAILED(
        space <= DFX_MAX_TENSOR_NUM, return ADUMP_FAILED,
...
int32_t DfxArgsParser::InitTensorModeInfoInner(uint32_t currArgsIndex, const uint8_t*& dfxAddr, uint64_t& currDfxSize)
{
    ...
    if (isTik_) {
        ...
        IDE_CTRL_VALUE_FAILED(
            ret == ADUMP_SUCCESS, return ADUMP_FAILED, "Can not find shape info. tiling data addr=%p, size=%llu",
            argOnHost_[currArgsIndex], tilingDataSize);
    }
...
int32_t DfxArgsParser::InitTensorModeInfo()
{
    ...
    while (currDfxSize < dfxSize_) {
        ...
        IDE_LOGI(
            "current arg index[%u], Tiling current exception dfx raw data:%s ", currArgsIndex,
            exceptionDfxStr.str().c_str());

        if (currArgsIndex >= maxArgNum_) {
            IDE_LOGE("The current arg index[%u] is greater than the max arg number[%llu]", currArgsIndex, maxArgNum_);
            return ADUMP_FAILED;
        }
...
int32_t DfxArgsParser::LoadDfxL1PtrTensor(TensorBuffer& tensor)
{
    ...
    if (dynamicModeFlag_) {
        ...
        IDE_LOGE(
            "[Dump][Exception] tensor type:%u(%s), pointer type:%u(%s)", static_cast<uint16_t>(tensor.tensorType),
            GetTensorTypeName(tensor.tensorType), static_cast<uint16_t>(tensor.pointerType),
            GetPointerTypeName(tensor.pointerType));
        RecordDumpLog(StrUtils::Format(
            "[Dump][Exception] exception info dump args data, addr:%p; size:%llu bytes", tensor.addr, tensor.size));
    } else {
        uint64_t size = 0;
...
int32_t DfxArgsParser::LoadTensorShapeAndSize(
    TensorBuffer& tensor, uint64_t* dynamicTensorAddr, void** tensorAddr, uint64_t shapeInfoCount)
{
    int32_t ret = ADUMP_SUCCESS;
    ...
    while (currShapeCount < shapeInfoCount) {
        ...
        for (uint32_t i = 0; i < tensorCount; ++i) {
            ...
            tensors_.push_back(tensor);
            RecordDumpLog(StrUtils::Format(
                "[Dump][Exception] exception info dump args data, addr:%p; size:%llu bytes", *tensorAddr,
                tensor.GetTotalByteSize()));
            tensorAddr++;
        }
    }
...
int32_t DfxArgsParser::LoadDfxTensor(TensorBuffer& tensor, uint16_t argsInfoNum)
{
    ...
    if (tensor.pointerType == DfxPointerType::LEVEL_2_POINTER_WITH_SHAPE) {
        ...
        tensor.pointerType == DfxPointerType::LEVEL_1_POINTER ||
...
int32_t DfxArgsParser::LoadDfxWorkspace(TensorBuffer& tensor)
{
    ...
    RecordDumpLog(StrUtils::Format(
        "[Dump][Exception] exception info dump workspace data, addr:%p; size:%llu bytes", workspace.addr,
        workspace.bytes));
...
int32_t DfxArgsParser::LoadDfxTilingData(TensorBuffer& tensor)
{
    ...
    tensors_.push_back(tensor);
    RecordDumpLog(StrUtils::Format(
        "[Dump][Exception] exception info dump tiling data, addr:%p; size:%llu bytes", tensor.addr, tilingDataSize));
...
void DfxArgsParser::LoadDfxMc2(const TensorBuffer& tensor)
{
    ...
    shapeDataAddr_++;
    RecordDumpLog(StrUtils::Format(
        "[Dump][Exception] exception info dump mc2 data, addr:%p; size:%llu bytes", workspace.addr, workspace.bytes));
...
int32_t DfxArgsParser::LoadDfxInfo(uint32_t& currArgsIndex)
{
    ...
    IDE_CTRL_VALUE_FAILED(argsInfoNum != 0, return ADUMP_FAILED, "The dfx args info num[%u] is invalid.", argsInfoNum);
    ...
    if (argsInfoType == TYPE_L0_EXCEPTION_DFX_ARGS_INFO) {
        ...
        IDE_LOGI(
            "The arg type info: %llu, tensor type: %u(%s), pointer type: %u(%s)", typeInfo,
            static_cast<uint16_t>(tensorType), GetTensorTypeName(tensorType), static_cast<uint16_t>(pointerType),
            GetPointerTypeName(pointerType));
        ...
        if ((tensorType <= DfxTensorType::OUTPUT_TENSOR && tensorType > DfxTensorType::INVALID_TENSOR) ||
            ...
            IDE_LOGE(
                "[Dump][Exception] args dump dfx info, addr:%p, tensor type:%u, pointer type:%u, index: %u",
