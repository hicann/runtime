...
template <typename T>
int32_t DumpArgs::GetPointerValueByBigEndian(
    const uint8_t** ptr, T& value, uint64_t& currentDfxSize, uint16_t totalDfxSize) const
{
    ...
    for (size_t i = 0; i < sizeof(T); ++i) {
        if (currentDfxSize >= totalDfxSize) {
            IDE_LOGE(
                "The dfx data size[%" PRIu64 "] is larger than the total dfx size[%" PRIu16 "].", currentDfxSize,
                totalDfxSize);
            return ADUMP_FAILED;
        }
...
template <typename T>
int32_t DumpArgs::GetPointerValueByLittleEndian(
    const uint8_t** ptr, T& value, uint64_t& currentDfxSize, uint16_t totalDfxSize) const
{
    uint16_t bitOfByte = 8;
    for (size_t i = 0; i < sizeof(T); ++i) {
...
int32_t DumpArgs::LoadArgsInfoWithDfx(const rtExceptionArgsInfo_t& exceptionArgsInfo)
{
    if (exceptionArgsInfo.argAddr == nullptr || exceptionArgsInfo.argsize == 0 ||
        ...
        IDE_LOGE(
            "In argAddr[%p]|argSize[%u]|dfxAddr[%p]|dfxSize[%u] has invalid attribute.", exceptionArgsInfo.argAddr,
            exceptionArgsInfo.argsize, exceptionArgsInfo.exceptionKernelInfo.dfxAddr,
...
int32_t DumpArgs::LoadArgsInfoWithSizeInfo(
    const rtExceptionArgsInfo_t& exceptionArgsInfo, const rtExceptionExpandInfo_t& exceptionExpandInfo)
{
    ...
    if (static_cast<uint64_t>(skipNum_) + inputNum_ > maxArgNum) {
        IDE_LOGE(
            "[Dump][Exception] skip num[%" PRIu32 "] plus input num[%" PRIu32 "] is over the max arg num[%" PRIu32
            "], load failed.",
    ...
    IDE_LOGE(
        "[Dump][Exception] the begin tensor's index of args is:%" PRIu32 ", args dump count[%" PRIu32 "]", argIndex,
        inputNum_);
    for (uint64_t sizeInfoIdx = sizeBeginIndex_; sizeInfoIdx < inputNum_ + sizeBeginIndex_; ++sizeInfoIdx) {
        ...
        if (size < 0) {
            ...
            if (size > maxArgNum) {
                IDE_LOGE(
                    "[Dump][Exception] size[%" PRId64 "] over arg max[%" PRIu32 "] is invalid, load failed.", size,
                    maxArgNum);
    ...
    const rtExceptionArgsInfo_t& exceptionArgsInfo, const rtExceptionExpandInfo_t& exceptionExpandInfo) const
{
    ...
    if (exceptionArgsInfo.sizeInfo.infoAddr == nullptr || exceptionArgsInfo.sizeInfo.atomicIndex == 0 ||
        ...
        IDE_LOGW(
            "[Dump][Exception] in infoAddr[%p]|atomicIndex[%" PRIu32 "]|argAddr[%p] has invalid attribute.",
            exceptionArgsInfo.sizeInfo.infoAddr, exceptionArgsInfo.sizeInfo.atomicIndex, exceptionArgsInfo.argAddr);
        return ADUMP_FAILED;
    }
    ...
    if (sizeInfo < g_chunk || sizeInfo > (g_chunk + RING_CHUNK_SIZE - 1)) {
        IDE_LOGE(
            "[Dump][Exception] the size info[%p] address may out of the chunk[%p] address range.", sizeInfo, g_chunk);
        return ADUMP_FAILED;
    }
    if (sizeInfo[0] != exceptionArgsInfo.sizeInfo.atomicIndex) {
        IDE_LOGE(
            "[Dump][Exception] args exception atomic index between %" PRIu64 " and %" PRIu32 " is different.",
...
int32_t DumpArgs::InitAttributes(
    const rtExceptionArgsInfo_t& exceptionArgsInfo, const rtExceptionExpandInfo_t& exceptionExpandInfo)
{
    ...
    if (exceptionExpandInfo.type == RT_EXCEPTION_AICORE) {
        ...
        if (sizeInfo_ + inputNum_ + ATOMIC_AND_INPUT_NUM_OFFSET > g_chunk + RING_CHUNK_SIZE + MAX_TENSOR_NUM) {
            IDE_LOGE(
                "[Dump][Exception] the value of size info[%p] plus input num[%" PRIu32
    ...
    if (exceptionExpandInfo.type == RT_EXCEPTION_FFTS_PLUS) {
        ...
        if (sizeInfo_ + totalContextSizeNum + ATOMIC_AND_INPUT_NUM_OFFSET >
            ...
            IDE_LOGE(
                "[Dump][Exception] the value of size info[%p] plus total num[%" PRIu64
                "] may exceed the chunk[%p] address range.",
        ...
        for (uint64_t sizeInfoIdx = contextBeginIndex; sizeInfoIdx < totalContextSizeNum + contextBeginIndex;
            ...
            IDE_LOGI(
                "[Dump][Exception] the context id[%" PRIu64 "] of size info and context id[%" PRIu16
                "] of exception info",
...
int32_t DumpArgs::LoadPointerTensor(const void** argOnHost, const uint32_t argIndex, uint64_t& sizeInfoIdx)
{
    ...
    if (addrBias >= argSize_) {
        IDE_LOGE(
            "[Dump][Exception]address bias[%p - %p] is over args size[%" PRIu64 "], invalid.", argOnHost[argIndex],
            argAddr_, argSize_);
        return ADUMP_FAILED;
    }
    ...
    IDE_LOGE(
        "[Dump][Exception] begin to load %s, index:%" PRIu32 ", addr:%p", pointerType.c_str(), argIndex,
        argOnHost[argIndex]);
    ...
    IDE_LOGE(
        "[Dump][Exception] end to load %s, index:%" PRIu32 ", addr:%p", pointerType.c_str(), argIndex,
        argOnHost[argIndex]);
...
bool DumpArgs::DumpArgsDumpWithDfxFlag() const { return dumpWithDfxFlag_; }

const std::vector<InputBuffer>& DumpArgs::DumpArgsGetInputBuffer() const { return inputBuffer_; }

const std::vector<TensorBuffer>& DumpArgs::DumpArgsGetTensorBuffer() const { return tensorBuffer_; }

const std::vector<DumpWorkspace>& DumpArgs::DumpArgsGetWorkSpace() const { return workspace_; }
