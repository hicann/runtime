...
private:
    ...
    template <typename T>
    static int32_t GetPointerValueByBigEndian(const uint8_t** ptr, T& value, uint64_t& currSize, uint16_t totalSize);
    ...
    static int32_t GetAddressBias(uint64_t& addrBias, const void* argAddr, void* baseAddr, uint64_t argsSize);

    static int32_t CheckAddressOverArgs(const uint64_t* address, const void** argOnHost, uint64_t maxArgNum);
    ...
    int32_t LoadTensorShapeAndSize(
...
template <typename T>
int32_t DfxArgsParser::GetPointerValueByBigEndian(const uint8_t** ptr, T& value, uint64_t& currSize, uint16_t totalSize)
{
