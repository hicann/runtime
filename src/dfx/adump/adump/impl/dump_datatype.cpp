...
int32_t DumpDataType::GetIrDataType(GeDataType dataType)
{
    ...
    int32_t protoDataType = it != GE_DT_TO_PROTO_DT.cend() ? static_cast<int32_t>(it->second) :
                                                             static_cast<int32_t>(ProtoDataType::DT_UNDEFINED);
    IDE_LOGI("Dump dataType conversion: input=%d, output=%d", static_cast<int32_t>(dataType), protoDataType);
