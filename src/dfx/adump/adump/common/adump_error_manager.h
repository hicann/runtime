...
#define ADUMP_INPUT_ERROR(error_code, key, value)                                                                \
    do {                                                                                                         \
        ...
        REPORT_PREDEFINED_ERR_MSG((error_code), Adx::ToCStrVec(adumpErrKeys__), Adx::ToCStrVec(adumpErrVals__)); \
    } while (0)
...
#define REPORT_EP0004_PARSE_ERROR(moduleName, reason, configPath)                                                    \
    do {                                                                                                             \
        ...
        ADUMP_INPUT_ERROR(                                                                                           \
            "EP0004", std::vector<std::string>({"path", "reason"}), std::vector<std::string>({configPath, reason})); \
    } while (0)
...
#define REPORT_EP0001_ITEM_ERROR(moduleName, item, reason, configPath)                                        \
    do {                                                                                                      \
        IDE_LOGE(                                                                                             \
            "[%s] The content of configuration item %s in config file %s is invalid. Reason: %s", moduleName, \
            ADUMP_TO_CSTR(item), configPath, ADUMP_TO_CSTR(reason));                                          \
        ADUMP_INPUT_ERROR(                                                                                    \
            "EP0001", std::vector<std::string>({"item", "path", "reason"}),                                   \
...
#define REPORT_EP0002_ENUM_VALUE_INVALID(moduleName, value, item, expectedValue, configPath)                          \
    do {                                                                                                              \
        IDE_LOGE(                                                                                                     \
            "[%s] Value %s for configuration item %s in file %s is invalid, must be selected from [%s].", moduleName, \
            ADUMP_TO_CSTR(value), ADUMP_TO_CSTR(item), configPath, ADUMP_TO_CSTR(expectedValue));                     \
        ADUMP_INPUT_ERROR(                                                                                            \
            "EP0002", std::vector<std::string>({"value", "item", "path", "expected_value"}),                          \
...
#define REPORT_EP0003_INVALID_VALUE(moduleName, value, item, reason, configPath)                   \
    do {                                                                                           \
        IDE_LOGE(                                                                                  \
            "[%s] Value %s for parameter %s in config file %s is invalid. Reason: %s", moduleName, \
            ADUMP_TO_CSTR(value), ADUMP_TO_CSTR(item), configPath, ADUMP_TO_CSTR(reason));         \
        ADUMP_INPUT_ERROR(                                                                         \
            "EP0003", std::vector<std::string>({"value", "item", "path", "reason"}),               \
...
#define REPORT_EP0005_ITEM_CANNOT_SET_WHEN_ITEM_SET(moduleName, errorItem, conditionItem, configPath)                \
    do {                                                                                                             \
        std::string reason = StrUtils::Format(                                                                       \
            ADUMP_REASON_FMT_ITEM_CANNOT_SET_WHEN_ITEM_SET, ADUMP_TO_CSTR(errorItem), ADUMP_TO_CSTR(conditionItem)); \
        IDE_LOGE("[%s] %s", moduleName, reason.c_str());                                                             \
        ADUMP_INPUT_ERROR(                                                                                           \
            "EP0005", std::vector<std::string>({"path", "reason"}), std::vector<std::string>({configPath, reason})); \
    } while (0)

#define REPORT_EP0005_ITEM_CANNOT_SET_WHEN_ITEM_VALUE(                                                               \
    moduleName, errorItem, conditionItem, conditionValue, configPath)                                                \
    do {                                                                                                             \
        std::string reason = StrUtils::Format(                                                                       \
            ADUMP_REASON_FMT_ITEM_CANNOT_SET_WHEN_ITEM_VALUE, ADUMP_TO_CSTR(conditionItem),                          \
            ADUMP_TO_CSTR(conditionValue), ADUMP_TO_CSTR(errorItem));                                                \
        IDE_LOGE("[%s] %s", moduleName, reason.c_str());                                                             \
        ADUMP_INPUT_ERROR(                                                                                           \
            "EP0005", std::vector<std::string>({"path", "reason"}), std::vector<std::string>({configPath, reason})); \
    } while (0)

#define REPORT_EP0005_ITEM_CANNOT_SET_WHEN_ITEM_NOT_VALUE(                                                           \
    moduleName, errorItem, conditionItem, conditionValue, configPath)                                                \
    do {                                                                                                             \
        std::string reason = StrUtils::Format(                                                                       \
            ADUMP_REASON_FMT_ITEM_CANNOT_SET_WHEN_ITEM_NOT_VALUE, ADUMP_TO_CSTR(conditionItem),                      \
            ADUMP_TO_CSTR(conditionValue), ADUMP_TO_CSTR(errorItem));                                                \
        IDE_LOGE("[%s] %s", moduleName, reason.c_str());                                                             \
        ADUMP_INPUT_ERROR(                                                                                           \
            "EP0005", std::vector<std::string>({"path", "reason"}), std::vector<std::string>({configPath, reason})); \
...
#define REPORT_EP0005_ITEM_VALUE_CANNOT_SET_WHEN_ITEM_VALUE(                                                         \
    moduleName, errorItem, errorValue, conditionItem, conditionValue, configPath)                                    \
    do {                                                                                                             \
        std::string reason = StrUtils::Format(                                                                       \
            ADUMP_REASON_FMT_ITEM_VALUE_CANNOT_SET_WHEN_ITEM_VALUE, ADUMP_TO_CSTR(conditionItem),                    \
            ADUMP_TO_CSTR(conditionValue), ADUMP_TO_CSTR(errorItem), ADUMP_TO_CSTR(errorValue));                     \
        IDE_LOGE("[%s] %s", moduleName, reason.c_str());                                                             \
        ADUMP_INPUT_ERROR(                                                                                           \
            "EP0005", std::vector<std::string>({"path", "reason"}), std::vector<std::string>({configPath, reason})); \
    } while (0)
...
#define REPORT_EP0006_INVALID_ARGUMENT(funcName, value, param, reason)                                   \
    do {                                                                                                 \
        IDE_LOGE(                                                                                        \
            "[%s] %s failed. Value %s for parameter %s is invalid. Reason: %s", ADUMP_TO_CSTR(funcName), \
            ADUMP_TO_CSTR(funcName), ADUMP_TO_CSTR(value), ADUMP_TO_CSTR(param), ADUMP_TO_CSTR(reason)); \
        ADUMP_INPUT_ERROR(                                                                               \
            "EP0006", std::vector<std::string>({"func", "value", "param", "reason"}),                    \
...
#define REPORT_EP0007_NULL_POINTER(funcName, param)                                                                  \
    do {                                                                                                             \
        IDE_LOGE(                                                                                                    \
            "[%s] %s failed because %s cannot be a NULL pointer.", ADUMP_TO_CSTR(funcName), ADUMP_TO_CSTR(funcName), \
            ADUMP_TO_CSTR(param));                                                                                   \
        ADUMP_INPUT_ERROR(                                                                                           \
            "EP0007", std::vector<std::string>({"func", "param"}), std::vector<std::string>({funcName, param}));     \
    } while (0)

#define REPORT_EP0008_API_CALL_SEQUENCE(funcName, reason)                                                            \
    do {                                                                                                             \
        IDE_LOGE(                                                                                                    \
            "[%s] %s failed. Reason: %s.", ADUMP_TO_CSTR(funcName), ADUMP_TO_CSTR(funcName), ADUMP_TO_CSTR(reason)); \
        ADUMP_INPUT_ERROR(                                                                                           \
            "EP0008", std::vector<std::string>({"func", "reason"}),                                                  \
...
constexpr const char* ADUMP_REASON_DUMP_STEP_EXCEED_LIMIT = "The value exceeds the maximum limit of 100 intervals";
constexpr const char* ADUMP_REASON_DUMP_STEP_INVALID_FORMAT = "The value has invalid format, expected format: 5-10";
...
constexpr const char* ADUMP_REASON_BLACKLIST_SIZE_EXCEED_LIMIT = "The value size exceeds the maximum limit of 100";
...
constexpr const char* ADUMP_REASON_PARAM_PATH_NOT_DIRECTORY = "The parameter is not a directory path";
