...
int32_t AdxDumpReceive::Process(const CommHandle& handle, const SharedPtr<MsgProto>& proto)
{
    ...
    if (proto->msgType == MsgType::MSG_CTRL && proto->status == MsgStatus::MSG_STATUS_HAND_SHAKE) {
        IdeErrorT err = AdxMsgProto::SendResponse(
            handle, (CmdClassT)proto->reqType, proto->devId, MsgStatus::MSG_STATUS_NONE_ERROR);
        if (err != IDE_DAEMON_NONE_ERROR) {
            IDE_LOGE(
                "send dump handshake response failed! device(id:%u,session:%zu), err: %d", proto->devId, handle.session,
                err);
            return err;
        }
    } else {
        IDE_LOGW(
            "receive invalid ctrl msg! device(id:%u,session:%zu), type: %d, status: %d", proto->devId, handle.session,
            proto->msgType, proto->status);
        return IDE_DAEMON_OK;
    }
...
int32_t AdxDumpReceive::Receive(const CommHandle& handle, const SharedPtr<MsgProto>& proto)
{
    ...
    while (init_) {
        ...
        IDE_CTRL_VALUE_WARN(
            ret == IDE_DAEMON_OK, return ret, "read dump data error. device(id:%u,session:%zu), err:%d", proto->devId,
            handle.session, ret);
        IDE_CTRL_VALUE_WARN(
            msg != nullptr && length >= 0,
            {
                IdeXfree(msg);
        ...
        if (msgPtr->msgType == MsgType::MSG_CTRL) {
            if (msgPtr->status == MsgStatus::MSG_STATUS_DATA_END) {
                IDE_LOGI(
                    "received dump ctrl msg from device(id:%u,session:%zu), end transfer dump data", proto->devId,
                    handle.session);
                return IDE_DAEMON_OK;
            } else {
                continue;
        ...
        IDE_CTRL_VALUE_WARN(
            msgPtr->sliceLen == protoDataLen, return IDE_DAEMON_ERROR,
            "msg sliceLen(%u) not equal actual msg data buffer size(%u)", msgPtr->sliceLen, protoDataLen);

        IDE_CTRL_VALUE_WARN(
            protoDataLen >= chunkHeaderLen, return IDE_DAEMON_ERROR,
        ...
        IDE_CTRL_VALUE_WARN(
            dumpChunk->bufLen <= chunkDataLen, return IDE_DAEMON_ERROR,
            "chunk bufLen(%u) exceeds actual chunk data buffer size(%u)", dumpChunk->bufLen, chunkDataLen);

        IDE_LOGI(
            "fileName: %s, bufLen: %u, isLastChunk: %u, offset: %ld, flag: %d", dumpChunk->fileName, dumpChunk->bufLen,
            dumpChunk->isLastChunk, dumpChunk->offset, dumpChunk->flag);
        ...
        IDE_CTRL_VALUE_FAILED(
            err == IDE_DAEMON_NONE_ERROR, return IDE_DAEMON_ERROR,
            "send dump handshake response failed! device(id:%u,session:%zu), err: %d", proto->devId, handle.session,
            err);
    }
    return IDE_DAEMON_OK;
