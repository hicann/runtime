...
AdxDumpRecord::AdxDumpRecord() : dumpRecordFlag_(true), dumpInitNum_(0)
{
    int32_t ret =
        pthread_atfork(AdxDumpRecord::PrepareFork, AdxDumpRecord::PostForkParent, AdxDumpRecord::PostForkChild);
    if (ret != 0) {
...
int32_t AdxDumpRecord::Init(const std::string& hostPid)
{
    ...
    if (hostPid.empty()) {
        ...
        if (len < 0 || len > MMPA_MAX_PATH) {
            IDE_LOGE(
                "Can't get app bin directory, strerr: %s, length: %d bytes, path: %s", strerror(errno), len,
                appBin.c_str());
    ...
    if (hostDumpDataInfoQueue_ == nullptr) {
        ...
        IDE_CTRL_VALUE_FAILED(
            hostDumpDataInfoQueue_ != nullptr, return IDE_DAEMON_ERROR, "Failed to new hostDumpDataInfoQueue");
    }
...
void AdxDumpRecord::PrepareFork() { Instance().recordMutex_.lock(); }

void AdxDumpRecord::PostForkParent() { Instance().recordMutex_.unlock(); }
...
bool AdxDumpRecord::RecordDumpDataToDisk(const DumpChunk& dumpChunk) const
{
    ...
    IdeErrorT err = FileUtils::WriteFile(realPath.c_str(), dumpChunk.dataBuf, dumpChunk.bufLen, dumpChunk.offset);
...
void AdxDumpRecord::SetWorkPath(const std::string& path) { workPath_ = path; }
...
{
    IDE_CTRL_VALUE_FAILED(
        shapeSize <= MAX_SHAPE_SIZE, return "NA", "Shape size %d is out of range %d", shapeSize, MAX_SHAPE_SIZE);
...
void AdxDumpRecord::StatisticsData(
    std::stringstream& strStream, std::shared_ptr<OpStatsResult> statsResult, const int64_t& count, const int32_t& idx)
{
    if (statsResult->stat[idx].result != 0) {
        IDE_LOGW(
            "Stats data is unsupported, index is %d, size is %" PRId64 ", result is %u.", statsResult->stat[idx].index,
            statsResult->stat[idx].size, statsResult->stat[idx].result);
        ...
        for (const uint16_t& pos : statsList_) {
            IDE_CTRL_VALUE_FAILED_NODO(
                pos < statsSize, continue, "Index %hu is out of range %" PRIu64 ".", pos, statsSize);
...
bool AdxDumpRecord::GenerateFileData(
    std::stringstream& strStream, const std::string& filename, std::shared_ptr<OpStatsResult> statsResult)
{
    ...
    IDE_CTRL_VALUE_FAILED(
        statsResult->tensorNum <= MAX_STATS_RESULT_NUM, return false, "Dump stats number %d is bigger than %d.",
        statsResult->tensorNum, MAX_STATS_RESULT_NUM);
    ...
    for (int32_t idx = 0; idx < statsResult->tensorNum; ++idx) {
        ...
        strStream << ","
                  << GetStringName(
                         static_cast<toolkit::dump::OutputDataType>(statsResult->stat[idx].dType), DT_STRING_MAP);
        strStream << ","
                  << GetStringName(
                         static_cast<toolkit::dump::OutputFormat>(statsResult->stat[idx].format), FORMAT_STRING_MAP);
        strStream << "," << GetShapeString(statsResult->stat[idx].shape, statsResult->stat[idx].shapeSize, count);
        IDE_LOGD(
            "Process the data with index[%d], io[%d], statsLen[%u], result[%u], shapeSize[%d], size[%" PRId64 "].",
...
bool AdxDumpRecord::DumpDataToCallback(
    const std::string& filename, const std::string& dumpData, int64_t offSet, int32_t flag)
{
    ...
    IDE_CTRL_VALUE_FAILED(
        messageCallback, return false, "Registered messageCallback function is not callable,\
            drop this data packet, filename:%s",
        filename.c_str());

    auto dumpChunkLen = sizeof(DumpChunk) + dumpData.size() + 1;
    std::unique_ptr<DumpChunk, void (*)(DumpChunk*)> dumpChunk(
        static_cast<DumpChunk*>(IdeXmalloc(dumpChunkLen)), [](DumpChunk* p) { IdeXfree(p); });
    ...
    err = strncpy_s(
        reinterpret_cast<AdxStringBuffer>(dumpChunk->dataBuf), dumpChunk->bufLen, dumpData.c_str(), dumpData.size());
...
bool AdxDumpRecord::FileNameCheck(const DumpChunk& dumpChunk) const
{
    ...
    IDE_LOGD(
        "File name [%s] of dump chunk is not end with .bin, or data size %u bytes is different from %" PRIu64 ".",
...
bool AdxDumpRecord::StatsDataParsing(const DumpChunk& dumpChunk)
{
    ...
    std::string realPath;
    IDE_CTRL_VALUE_FAILED(
        FileUtils::FileNameIsReal(filename, realPath) == IDE_DAEMON_OK, return false, "File name is not real: %s",
        filename.c_str());

    IDE_CTRL_VALUE_FAILED(
        FileUtils::WriteFile(filename, strStream.str().c_str(), strStream.str().size(), dumpChunk.offset) ==
            IDE_DAEMON_NONE_ERROR,
        return false, "Failed to dump file %s to path %s", filename.c_str(), dirName.c_str());
...
void AdxDumpRecord::RecordDumpInfo()
{
    ...
    while (hostDumpDataInfoQueue_ != nullptr && (dumpRecordFlag_ || !DumpDataQueueIsEmpty())) {
        ...
        IDE_CTRL_VALUE_FAILED_NODO(
            data.recvLen >= chunkHeaderLen, continue, "recvLen(%u) too small for DumpChunk header(%zu bytes)",
            data.recvLen, chunkHeaderLen);

        DumpChunk* dumpChunk = reinterpret_cast<DumpChunk*>(msgPtr->data);

        IDE_CTRL_VALUE_FAILED_NODO(
            dumpChunk->bufLen <= data.recvLen - chunkHeaderLen, continue,
            "bufLen(%u) exceeds actual data buffer size(%u bytes), fileName: %s", dumpChunk->bufLen,
            data.recvLen - chunkHeaderLen, dumpChunk->fileName);

        IDE_LOGI(
            "Queue pop data success! filename: %s, offset: %" PRId64 ", bufLen: %u bytes, "
            ...
            hostDumpDataInfoQueue_->Size());
#if !defined(ADUMP_SOC_HOST) || ADUMP_SOC_HOST == 1
        if (FileNameCheck(*dumpChunk)) {
            IDE_CTRL_VALUE_FAILED_NODO(
                StatsDataParsing(*dumpChunk), continue, "Failed to parse dump data with file name %s",
                dumpChunk->fileName);
            continue;
        }
#endif
        if (AdxDumpProcess::Instance().IsRegistered()) {
            ...
            if (!messageCallback) {
                IDE_LOGE(
                    "Registered messageCallback function is not callable,\
                    drop this data packet, fileName:%s",
                    dumpChunk->fileName);
                continue;
            }
...
void AdxDumpRecord::SetDumpPath(const std::string& dumpPath) { dumpPath_ = dumpPath; }
} // namespace Adx
