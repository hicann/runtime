...
STATIC LogStatus HiMemWrite(int32_t fd, const char* buffer, size_t len)
{
    ...
    ONE_ACT_ERR_LOG(
        (ret < 0) || (ret != (int32_t)len), return LOG_FAILURE, "write failed. ret = %d, errno :%d", ret, errno);
    return LOG_SUCCESS;
}
...
STATIC int32_t HiMemReadLog(int32_t fd, char* buffer, size_t len, LogHead* head)
{
    ...
    ONE_ACT_ERR_LOG(
        (ret < 0) || (ret < (int32_t)LOGHEAD_LEN), return SYS_ERROR,
...
int32_t HiMemWriteIamLog(int32_t fd, RingBufferStat* logBuf)
{
    ...
    while (true) {
        ...
        if (readRes < 0) {
            NO_ACT_ERR_LOG(
                (readRes != (-(int32_t)BUFFER_READ_FINISH)), "HiMemWriteIamLog read err res = %d check", readRes);
            break;
        }
