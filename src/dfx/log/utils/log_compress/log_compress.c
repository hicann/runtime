...
LogStatus LogCompressAddSuffix(char* file, uint32_t length)
{
    ...
    ONE_ACT_ERR_LOG(
        (length == 0) || (length > TOOL_MAX_PATH), return LOG_FAILURE, "[input] file(%s) length(%u) is invalid.", file,
        length);

    int32_t ret = snprintf_s(file, length, (size_t)length - 1U, "%s%s", file, GZIP_SUFFIX);
    if (ret == -1) {
        SELF_LOG_ERROR("file(%s) add suffix(%s) failed, strerr=%s.", file, GZIP_SUFFIX, strerror(ToolGetErrorCode()));
        return LOG_FAILURE;
    }
    return LOG_SUCCESS;
#else
...
LogStatus LogCompressGetRotatePath(char* file, uint32_t length)
{
    ...
    ONE_ACT_ERR_LOG(
        err != EOK, return LOG_FAILURE, "strcpy_s failed, can not copy active file name, ret = %d.", (int32_t)err);
    ...
    ONE_ACT_ERR_LOG(
        err != EOK, return LOG_FAILURE, "strncpy_s failed, can not rename active file name, ret = %d.", (int32_t)err);
    err = strcpy_s(file + copyLen, (size_t)length - (size_t)copyLen, str + strlen(LOG_ACTIVE_STR));
    ONE_ACT_ERR_LOG(
        err != EOK, return LOG_FAILURE, "strcpy_s failed, can not rename active file name, ret = %d.", (int32_t)err);
    return LOG_SUCCESS;
#else
    (void)file;
    (void)length;
...
LogStatus LogCompressFileRotate(const char* file)
{
    ...
    ONE_ACT_ERR_LOG(
        LogCompressGetRotatePath(newFileName, TOOL_MAX_PATH) != LOG_SUCCESS, return LOG_FAILURE,
...
#elif defined(SOFTWARE_ZIP)
    return SoftwareCompressBuffer(source, sourceLen, dest, destLen);
#else
    (void)source;
    (void)sourceLen;
    (void)dest;
...
bool LogCompressCheckActiveFile(const char* fileName)
{
    ...
    ONE_ACT_WARN_LOG(
        strlen(fileName) <= strlen(LOG_ACTIVE_FILE_GZ_SUFFIX), return false,
