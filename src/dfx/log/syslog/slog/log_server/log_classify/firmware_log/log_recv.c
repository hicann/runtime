...
static void SlogdFirmwareLogWrite(void* handle, uint32_t devId)
{
    ...
    while (retry < MAX_WRITE_WAIT_TIME) {
        ...
        if (ret != LOG_SUCCESS) {
            SELF_LOG_ERROR_N(
                &g_writeDPrintNum, GENERAL_PRINT_NUM,
                "send firmware log to host failed, result=%d, strerr=%s, print once every %u times.", ret,
...
void SlogdFirmwareLogReceive(void* args)
{
    ...
    ONE_ACT_ERR_LOG(
        ret != SUCCESS, return, "get log or parse recvmsg failed, result=%d, strerr=%s.", (int32_t)ret,
...
void SlogdFirmwareLogGet(SessionItem* handle, void* buffer, uint32_t bufferLen, uint32_t devId)
{
    ...
    void* bufHandle = SlogdBufferHandleOpen(FIRM_LOG_TYPE, NULL, LOG_BUFFER_READ_MODE, devId);
    while (true) {
        ...
        ret = snprintf_s(
            fileName, MAX_FILENAME_LEN, MAX_FILENAME_LEN - 1, "debug/device-%u/device-%u_%s.log",
            GetHostDeviceID(devId), GetHostDeviceID(devId), msgData->timeStr);
...
STATIC LogRt FullAddBuffQueue(LogQueue* queue, LogMsgHead* recvMsg, unsigned int* nodeCnt, LogBuff* savedBuff)
{
    ...
    if (SlogdBufferCheckFull(handle, recvMsg->dataLen)) {
        ...
        TWO_ACT_ERR_LOG(
            ret != SUCCESS, SlogdBufferHandleClose(&handle), return ret,
    ...
    SlogdBufferHandleClose(&handle);
    ONE_ACT_ERR_LOG(
        retValue != LOG_SUCCESS, return STR_COPY_FAILED, "write to buffer failed, result=%d, strerr=%s.", retValue,
        strerror(ToolGetErrorCode()));
...
STATIC LogRt
FullAddBuffQueueS(LogQueue* queue, LogMsgHead* recvMsg, unsigned int* nodeCnt, LogBuff* buff, ToolMutex* threadLock)
{
    LogRt ret = SUCCESS;
    do {
...
STATIC bool TimerEnough(struct timespec* lastTv)
{
    ...
    NO_ACT_WARN_LOG(
        LogGetMonotonicTime(&realTv) != LOG_SUCCESS, "get time failed, strerr=%s.", strerror(ToolGetErrorCode()));
...
{
    void* handle = SlogdBufferHandleOpen(FIRM_LOG_TYPE, NULL, LOG_BUFFER_WRITE_MODE, deviceInfo->logBuff->devId);
    if (SlogdBufferCheckEmpty(handle)) {
        SlogdBufferHandleClose(&handle);
        return LOG_SUCCESS;
    }
...
STATIC int32_t SlogdWriteDeviceLog(DeviceRes* deviceInfo, bool flushFlag)
{
    ...
    if (res != OK) {
        SELF_LOG_ERROR_N(
            &g_writeDPrintNum, GENERAL_PRINT_NUM,
            "write device log to file failed, device_id=%u,"
            "result=%u, print once every %u times.",
            deviceId, res, GENERAL_PRINT_NUM);
    }
    return LOG_SUCCESS;
}
...
STATIC void RenameOldFirmwareDirs(const char* path, const char* dir)
{
    ...
    if (result != LOG_SUCCESS) {
        SELF_LOG_ERROR(
            "rename firmware dir failed, source directory=%s, destined directory=%s, result=%d.", srcDir, dstDir,
            result);
    } else {
...
LogStatus SlogdFirmwareLogResInit(void)
{
    ...
    uint32_t devNum = 0;
    ONE_ACT_ERR_LOG(
        DrvGetDeviceId(&devNum, deviceIdArray, MAX_DEV_NUM) != LOG_SUCCESS, return LOG_FAILURE,
