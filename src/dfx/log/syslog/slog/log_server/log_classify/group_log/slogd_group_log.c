...
STATIC LogStatus InitGroupLogList(StSubLogFileList* list, GroupInfo* group, const char* groupLogPath, uint32_t pathLen)
{
    ...
    ONE_ACT_ERR_LOG(
        err == -1, return LOG_FAILURE, "get group name header failed, result=%d, strerr=%s.", err,
 ...
 * @return: void
 */
STATIC void GetGroupFileList(StSubLogFileList* list) { (void)LogAgentGetFileListForModule(list, list->filePath); }

static bool SlogdGroupLogBufCheck(void* srcAttr, void* dstAttr) { return srcAttr == dstAttr; }
...
STATIC LogStatus GroupResourceInit(GroupInfo* group, char* groupLogPath, uint32_t len)
{
    ...
    TWO_ACT_WARN_LOG(
        err != OK, (SlogdBufferExit(GROUP_LOG_TYPE, NULL)), return LOG_FAILURE,
...
STATIC GroupInfo* MallocNewGroup(const GroupInfo* groupInfo, const char* groupLogPath, unsigned int pathLen)
{
    ...
    ONE_ACT_WARN_LOG(
        newGroup == NULL, return NULL, "alloc a new group failed. strerr=%s.", strerror(ToolGetErrorCode()));
    int32_t err = memcpy_s(newGroup, sizeof(GroupInfo), groupInfo, sizeof(GroupInfo));
    TWO_ACT_WARN_LOG(
        err != 0, XFREE(newGroup), return NULL, "Copy group info failed. strerr=%s", strerror(ToolGetErrorCode()));
    newGroup->next = NULL;
...
STATIC LogStatus AddDeviceNewGroup(GroupInfo* groupInfo, char* groupLogPath, unsigned int pathLen)
{
    ...
    for (uint32_t idx = 0; idx < newGroup->deviceNum; idx++) {
        ...
        int32_t err = snprintf_s(
            deviceLogPath, MAX_FILEPATH_LEN + 1U, MAX_FILEPATH_LEN, "%s%s%s%u", groupLogPath, FILE_SEPARATOR,
            DEVICE_HEAD, GetHostDeviceID(idx));
        ONE_ACT_ERR_LOG(
            err == -1, goto fail_init_group_node, "get device log dir path failed, device_id=%u, result=%d, strerr=%s.",
...
STATIC LogStatus LoadGroupInfo(void)
{
    ...
    int32_t err =
        snprintf_s(groupLogPath, MAX_FILEPATH_LEN + 1U, MAX_FILEPATH_LEN, "%s%s", info->agentFileDir, DEBUG_DIR_NAME);
    ONE_ACT_ERR_LOG(
        err == -1, return LOG_FAILURE, "get group log dir path failed, result=%d, strerr=%s.", err,
        strerror(ToolGetErrorCode()));
    GroupInfo group;
    ...
    for (int32_t i = 0; i < GROUP_MAP_SIZE; ++i) {
        if (groupInfo->isInit == 1) {
            ...
            NO_ACT_ERR_LOG(
                ret != LOG_SUCCESS, "Add now group failed. Group Id is %d, Group Name is %s", group.groupId,
                group.groupName);
        }
        groupInfo++;
    }
 ...
 */
GroupInfo* GetGroupListHead(void) { return g_groupInfoListHead; }
...
STATIC void SlogdWriteDeviceGroupLog(void* handle, LogType type, StSubLogFileList* fileList)
{
    ...
    if (ret != OK) {
        SELF_LOG_ERROR_N(
            &g_writeGroupFilePrintNum, GENERAL_PRINT_NUM,
            "write device group log failed, result=%u, strerr=%s, print once every %u times.", ret,
...
void SlogdGroupLogExit(void) { DeleteAllGroup(); }

#else

LogStatus SlogdGroupLogInit(void) { return LOG_SUCCESS; }

void SlogdGroupLogExit(void) { return; }
