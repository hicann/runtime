...
#define SCAN_INTERVAL 15000U // 15s
#define BASE_INTERVAL 1000U  // 1s
#define BASE_DIR_NUM 20U
#define WATCH_DEFAULT_THREAD_ATTR    \
    {                                \
...
#define GETSTATUS_E_PRINT_NUM 10000
STATIC ThreadArg g_args[LOG_TYPE_NUM];
static uint32_t g_scanInterval = SCAN_INTERVAL;
static int32_t g_getStatusFailCnt = 0;
/**
...
STATIC int32_t ScanAndGetDirFile(SortArg* sortArg, const char* path, const ToolDirent** a, const ToolDirent** b)
{
    ...
    if (ret != SYS_OK) {
        SELF_LOG_ERROR_N(
            &g_getStatusFailCnt, GETSTATUS_E_PRINT_NUM, "get status %s failed, print every %d times, strerr=%s",
            sortArg->aDirName, GETSTATUS_E_PRINT_NUM, strerror(ToolGetErrorCode()));
        return SYS_ERROR;
    }
    ...
    if (ret != SYS_OK) {
        SELF_LOG_ERROR_N(
            &g_getStatusFailCnt, GETSTATUS_E_PRINT_NUM, "get status %s failed, print every %d times, strerr=%s",
            sortArg->bDirName, GETSTATUS_E_PRINT_NUM, strerror(ToolGetErrorCode()));
        return SYS_ERROR;
    }
    return SYS_OK;
}
...
STATIC int32_t GetSortResult(SortArg* sortArg, int type, int numA, int numB)
{
    if (type == 0) {
        int ret = snprintf_s(
            sortArg->bFileName, MAX_FILE_NAME_LEN, MAX_FILE_NAME_LEN - 1, "%s/%s", sortArg->bDirName,
            sortArg->listB[numB - 1]->d_name);
        if (ret == -1) {
            SELF_LOG_ERROR("snprintf_s B dir failed, strerr=%s", strerror(ToolGetErrorCode()));
            return SYS_ERROR;
        }
        ONE_ACT_ERR_LOG(
            ToolStatGet(sortArg->bFileName, &sortArg->bStatbuff) != SYS_OK, return SYS_ERROR,
        ...
        int ret = snprintf_s(
            sortArg->aFileName, MAX_FILE_NAME_LEN, MAX_FILE_NAME_LEN - 1, "%s/%s", sortArg->aDirName,
            sortArg->listA[numA - 1]->d_name);
        if (ret == -1) {
            SELF_LOG_ERROR("snprintf_s A dir failed, strerr=%s", strerror(ToolGetErrorCode()));
            return SYS_ERROR;
        }
        ONE_ACT_ERR_LOG(
            ToolStatGet(sortArg->aFileName, &sortArg->aStatbuff) != SYS_OK, return SYS_ERROR,
...
int32_t SlogdApplogSortFileFunc(const char* path, const ToolDirent** a, const ToolDirent** b)
{
    ...
    ONE_ACT_ERR_LOG(
        ToolStatGet(sortArg.aFileName, &sortArg.aStatbuff) != SYS_OK, goto SORTEXIT, "get status %s failed, strerr=%s",
        sortArg.aFileName, strerror(ToolGetErrorCode()));
    ONE_ACT_ERR_LOG(
        ToolStatGet(sortArg.bFileName, &sortArg.bStatbuff) != SYS_OK, goto SORTEXIT, "get status %s failed, strerr=%s",
        sortArg.bFileName, strerror(ToolGetErrorCode()));
    FreeDir(sortArg.listA, numA, sortArg.listB, numB);
...
STATIC void RemoveAppLogDir(int logType, const char* dir)
{
    ...
    if ((logType < (int)LOG_TYPE_NUM) && (logType >= (int)DEBUG_LOG)) {
        ret = snprintf_s(dstDir, MAX_FILE_NAME_LEN, MAX_FILE_NAME_LEN - 1, "%s/%s", g_args[logType].appLogPath, dir);
    } else {
        ret = snprintf_s(dstDir, MAX_FILE_NAME_LEN, MAX_FILE_NAME_LEN - 1, "%s/%s", g_args[DEBUG_LOG].appLogPath, dir);
    }
    ...
    if (ret != SYS_OK) {
        SELF_LOG_ERROR(
            "remove app log dir failed, dir=%s, result=%d, strerr=%s.", dstDir, ret, strerror(ToolGetErrorCode()));
    } else {
...
STATIC LogRt ScanAppLog(const char* path, int logType)
{
    ...
    uint32_t reductionStep =
        ((uint32_t)totalNum > BASE_DIR_NUM) ? (((uint32_t)totalNum - BASE_DIR_NUM) / BASE_DIR_NUM) : 0U;
    g_scanInterval = (reductionStep * BASE_INTERVAL >= g_scanInterval) ?
                         BASE_INTERVAL :
                         (g_scanInterval - reductionStep * BASE_INTERVAL);
...
STATIC void* AppLogWatcher(const ArgPtr arg)
{
    ...
    while (LogGetSigNo() == 0) {
        ...
        (void)ToolSleep(g_scanInterval);
        g_scanInterval = SCAN_INTERVAL;
    }
...
void CreateAppLogWatchThread(void)
{
    ...
    int ret = snprintf_s(
        g_args[DEBUG_LOG].appLogPath, CFG_LOGAGENT_PATH_MAX_LENGTH + 1U, CFG_LOGAGENT_PATH_MAX_LENGTH, "%s/%s",
        rootPath, DEBUG_DIR_NAME);
    ...
    ret = snprintf_s(
        g_args[SECURITY_LOG].appLogPath, CFG_LOGAGENT_PATH_MAX_LENGTH + 1U, CFG_LOGAGENT_PATH_MAX_LENGTH, "%s/%s",
        rootPath, SECURITY_DIR_NAME);
    ...
    ret = snprintf_s(
        g_args[RUN_LOG].appLogPath, CFG_LOGAGENT_PATH_MAX_LENGTH + 1U, CFG_LOGAGENT_PATH_MAX_LENGTH, "%s/%s", rootPath,
        RUN_DIR_NAME);
