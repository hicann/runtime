...
STATIC int32_t DevLogReport(const LogInfo* logInfo, void* handle)
{
    ...
    if (ret != 0) {
        ...
        SELF_LOG_WARN(
            "can not send data to hdc session, ret=%d, pid=%u, devId=%u, res=%u", ret, logInfo->pid, hostDevId, res);
    }
    XFREE(data);
    return ret;
}
    ...
    for (type = (int32_t)DEBUG_LOG; type < (int32_t)LOG_TYPE_NUM; ++type) {
        ...
        if (logNode != NULL) {
            void* handle = SlogdBufferHandleOpen(
                DEBUG_APP_LOG_TYPE + type, (void*)logNode, LOG_BUFFER_WRITE_MODE, logNode->deviceId);
            ...
            if (ret != 0) {
                ...
                uint32_t res = LogAgentWriteDeviceApplicationLog(data, LogStrlen(data), &info, GetGlobalLogFileList());
                SELF_LOG_ERROR(
                    "write log to hdc failed, ret=%d, pid=%u, devId=%u, res=%u, type=%d.", ret, pid, hostDevId, res,
                    type);
            }
            if (timeout <= 0) {
...
STATIC void FlushLogBufToHost(void)
{
    ...
    while (tmp != NULL) {
        ...
        if (!SlogdBufferCheckEmpty(handle)) {
            ...
            if (ret != OK) {
                SELF_LOG_WARN_N(
                    &g_writeAppFilePrintNum, GENERAL_PRINT_NUM,
                    "can not report log to process, result=%d, strerr=%s, print once every %u times.", ret,
...
{
    ONE_ACT_ERR_LOG(
        (msg == NULL) || (info == NULL), return LOG_FAILURE,
        "flush applog to buffer for report failed, input msg is null.")
    SlogdAppLogLock();
    ...
    if (SlogdBufferCheckFull(handle, msgLen)) {
        ...
        if (ret != OK) {
            SELF_LOG_ERROR_N(
                &g_writeAppFilePrintNum, GENERAL_PRINT_NUM,
                "report log to process failed, result=%u, strerr=%s, print once every %u times.", ret,
                strerror(ToolGetErrorCode()), GENERAL_PRINT_NUM);
        }
    }
    if (SlogdBufferCheckEmpty(handle)) {
...
void SlogdAppLogReportExit(void) { FreeSessionList(); }
