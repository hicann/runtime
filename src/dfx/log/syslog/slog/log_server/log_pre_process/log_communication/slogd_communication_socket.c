...
STATIC int32_t
SlogdGetSocketPath(int32_t devId, char socketPath[SLOG_FILE_NUM][WORKSPACE_PATH_MAX_LENGTH + 1U], uint32_t* fileNum)
{
    int32_t ret;
    *fileNum = 0;
    ...
    if ((devId >= MIN_VFID_NUM) && (devId <= MAX_VFID_NUM)) {
        ...
        *fileNum = SLOG_FILE_NUM;
        for (uint32_t i = 0; i < *fileNum; i++) {
            ret = sprintf_s(
                socketPath[i], WORKSPACE_PATH_MAX_LENGTH, "%s%s", defaultPath, g_socketFileInfoTbl[i].filename);
            ONE_ACT_ERR_LOG(ret == -1, return SYS_ERROR, "sprintf_s vfid failed, vfid=%d.", devId);
        }
    }
    return SYS_OK;
...
STATIC toolSockHandle SlogdCreateSocketBySlogFile(char* socketPath, char* groupName, int32_t permission)
{
    ...
    ONE_ACT_ERR_LOG(
        ret != EOK, return SYS_ERROR, "strcpy_s failed, result=%d, strerr=%s.", ret, strerror(ToolGetErrorCode()));

    // Unlink the previous socket file first.
    ret = ToolUnlink(sunx.sun_path);
    NO_ACT_WARN_LOG(ret != EOK, "can not unlink file=%s, strerr=%s.", sunx.sun_path, strerror(ToolGetErrorCode()));

    // Create socket.
    toolSockHandle sockFd = ToolSocket(AF_UNIX, SOCK_DGRAM, 0);
    ONE_ACT_ERR_LOG(sockFd < 0, return SYS_ERROR, "create socket failed, strerr=%s.", strerror(ToolGetErrorCode()));
    ...
    TWO_ACT_ERR_LOG(
        ret != SYS_OK, LOG_CLOSE_SOCK(sockFd), return SYS_ERROR, "bind socket failed, bind path is: %s, strerr=%s.",
        sunx.sun_path, strerror(ToolGetErrorCode()));
    ...
    TWO_ACT_ERR_LOG(
        ret != SYS_OK, LOG_CLOSE_SOCK(sockFd), return SYS_ERROR, "Change the socket file: %s group failed, strerr=%s.",
        sunx.sun_path, strerror(ToolGetErrorCode()));

    // Set the socket files permission.
    ret = ToolChmod(sunx.sun_path, permission);
    TWO_ACT_ERR_LOG(
        ret != SYS_OK, LOG_CLOSE_SOCK(sockFd), return SYS_ERROR, "chmod %s failed , strerr=%s.", sunx.sun_path,
...
STATIC int32_t SlogdCreateSocket(int32_t devId, uint32_t* fileNum)
{
    ...
    for (uint32_t i = 0; i < *fileNum; i++) {
        ...
        uint32_t permission = SyncGroupToOther((uint32_t)g_socketFileInfoTbl[i].permission);
        ONE_ACT_ERR_LOG(
            permission >= INT_MAX, return SYS_ERROR, "create socket failed, check permission(%u) failed.", permission);
        g_sockFd[i] = SlogdCreateSocketBySlogFile(socketPath[i], g_socketFileInfoTbl[i].groupName, (int32_t)permission);
        ONE_ACT_ERR_LOG(
            g_sockFd[i] == SYS_ERROR, return SYS_ERROR, "create socket failed, strerr=%s.",
            strerror(ToolGetErrorCode()));
    }
    return SYS_OK;
}
...
{
    int32_t ret = SlogdCreateSocket(devId, &g_fileNum);
    ONE_ACT_ERR_LOG(ret != SYS_OK, return LOG_FAILURE, "create socket failed, ret=%d.", ret);
    return LOG_SUCCESS;
}
 ...
 */
void SlogdRmtServerExit(void) { return; }
...
int32_t SlogdRmtServerRecv(uint32_t fileNum, char* buf, uint32_t bufLen, int32_t* logType)
{
    ...
    ONE_ACT_ERR_LOG(ret <= 0, return SYS_ERROR, "select error, strerr=%s.", strerror(ToolGetErrorCode()));
...
uint32_t SlogdRmtServerGetFileNum(void) { return g_fileNum; }