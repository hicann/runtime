...
int32_t SysReportInit(void) { return LOG_SUCCESS; }

int32_t SysReportDestroy(void) { return LOG_SUCCESS; }
 ...
 * @param [in]  : handle     handle for communicating with the peer end
 */
STATIC void SysDestroyCommHandle(const CommHandle* handle) { AdxDestroyCommHandle((CommHandle*)handle); }
