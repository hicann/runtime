...
static int32_t g_bufferMap[LOG_TYPE_MAX_NUM] = {
    [DEBUG_SYS_LOG_TYPE] = LOG_STATIC_BUFFER,  [SEC_SYS_LOG_TYPE] = LOG_STATIC_BUFFER,
    [RUN_SYS_LOG_TYPE] = LOG_STATIC_BUFFER,    [EVENT_LOG_TYPE] = LOG_STATIC_BUFFER,
    [FIRM_LOG_TYPE] = LOG_STATIC_BUFFER,       [GROUP_LOG_TYPE] = LOG_DYNAMIC_BUFFER,
    [DEBUG_APP_LOG_TYPE] = LOG_DYNAMIC_BUFFER, [SEC_APP_LOG_TYPE] = LOG_DYNAMIC_BUFFER,
    [RUN_APP_LOG_TYPE] = LOG_DYNAMIC_BUFFER};
#else
static int32_t g_bufferMap[LOG_TYPE_MAX_NUM] = {
    [DEBUG_SYS_LOG_TYPE] = LOG_DYNAMIC_BUFFER, [SEC_SYS_LOG_TYPE] = LOG_DYNAMIC_BUFFER,
    [RUN_SYS_LOG_TYPE] = LOG_DYNAMIC_BUFFER,   [EVENT_LOG_TYPE] = LOG_DYNAMIC_BUFFER,
    [FIRM_LOG_TYPE] = LOG_DYNAMIC_BUFFER,      [GROUP_LOG_TYPE] = LOG_DYNAMIC_BUFFER,
    [DEBUG_APP_LOG_TYPE] = LOG_DYNAMIC_BUFFER, [SEC_APP_LOG_TYPE] = LOG_DYNAMIC_BUFFER,
    [RUN_APP_LOG_TYPE] = LOG_DYNAMIC_BUFFER};
#endif
...
static SlogdBufOperate g_slogdBufOperate[LOG_BUFFER_TYPE_NUM] = {
    {SlogdDynamicBufInit, SlogdDynamicBufExit, SlogdDynamicBufWrite, SlogdDynamicBufRead, SlogdDynamicBufCheckFull,
     SlogdDynamicBufferReset},
    {SlogdStaticBufInit, SlogdStaticBufExit, SlogdStaticBufWrite, SlogdStaticBufRead, SlogdStaticBufCheckFull,
     SlogdStaticBufferReset}};
...
static LogStatus SlogdStaticBufInit(LogBufferMgr* bufMgr, uint32_t bufSize, int32_t logType, uint32_t devId)
{
    ...
    if (size <= BLOCK_HEADER_LEN) {
        SELF_LOG_ERROR(
            "get log buffer[%d] from drv is invalid, size[%u bytes]<minSize[%u bytes].", logType, size,
            BLOCK_HEADER_LEN);
        return LOG_FAILURE;
    }
...
static void SlogdStaticBufExit(LogBufferMgr** bufMgr) { XFREE(*bufMgr); }
...
static LogStatus SlogdDynamicBufWrite(LogBufferMgr* bufMgr, const char* msg, uint32_t msgLen)
{
    ...
    if (curBlkSpace <= msgLen) {
        ...
        ONE_ACT_ERR_LOG(
            err != EOK, return LOG_FAILURE, "memset failed, write log to ring buffer failed, err = %d.", (int32_t)err);
    ...
    ONE_ACT_ERR_LOG(
        ret != EOK, return LOG_FAILURE, "memcpy failed, write log to ring buffer failed, ret = %d.", (int32_t)ret);
...
static LogStatus SlogdStaticBufWrite(LogBufferMgr* bufMgr, const char* msg, uint32_t msgLen)
{
    ...
    ONE_ACT_ERR_LOG(
        err != EOK, return LOG_FAILURE, "memcpy failed, write log to ring buffer failed, ret = %d.", (int32_t)err);
...
static int32_t SlogdStaticBufRead(LogBufferMgr* bufMgr, char* msg, uint32_t msgLen)
{
    ...
    if (bufMgr->operaMode == LOG_BUFFER_READ_MODE) {
        if (msgLen > (uint32_t)sizeof(SlogdMsgData)) {
            ...
            ONE_ACT_ERR_LOG(
                err != EOK, return -1, "memcpy failed, read log buffer time string failed, err = %d.", (int32_t)err);
            err = memcpy_s(
                data->data, (size_t)msgLen - sizeof(SlogdMsgData), BUF_START_ATTR(blockHead) + blockHead->readPtr,
                readLen);
            ONE_ACT_ERR_LOG(err != EOK, return -1, "memcpy failed, read log buffer failed, err = %d.", (int32_t)err);
        } else {
...
uint32_t SlogdBufferGetBufSize(int32_t logType) { return g_slogdBufMgr[logType]->buffer->blockSize - BLOCK_HEADER_LEN; }
...
static void SlogdStaticBufferReset(LogBufferMgr* bufMgr)
{
    ...
    BlockStruct* blockHead = bufMgr->buffer;
    blockHead->readPtr =
        (blockHead->readPtr / blockHead->blockSize + 1U) % bufMgr->blockNum + (uint32_t)sizeof(BlockStruct);
}
...
bool SlogdBufferCheckFull(void* handle, uint32_t msgLen)
{
    ...
    (void)ToolMutexLock(&g_slogdBufMgr[bufMgr->logType]->lock);
    TWO_ACT_ERR_LOG(
        bufMgr->buffer == NULL, (void)ToolMutexUnLock(&g_slogdBufMgr[bufMgr->logType]->lock), return false,
...
{
    ONE_ACT_ERR_LOG(
        (buf == NULL) || (pos == NULL) || (handle == NULL), return LOG_INVALID_PTR,
        "input ptr is null, collect newest log failed.");
    LogBufferMgr* bufMgr = (LogBufferMgr*)handle;
    (void)ToolMutexLock(&g_slogdBufMgr[bufMgr->logType]->lock);
    TWO_ACT_ERR_LOG(
        bufMgr->buffer == NULL, (void)ToolMutexUnLock(&g_slogdBufMgr[bufMgr->logType]->lock), return LOG_INVALID_PTR,
        "check log buffer failed, input buffer is null.");
    BlockStruct* blockHead = bufMgr->buffer;
    ...
    if ((bufMgr->roundFlag == 0) || (blockHead->writePtr - BUF_DATA_OFFSET >= logSize)) {
        ...
        TWO_ACT_ERR_LOG(
            ret != EOK, ((void)ToolMutexUnLock(&g_slogdBufMgr[bufMgr->logType]->lock)), return LOG_FAILURE,
