...
set -e
# parameter list: $1：process name, $2: timestamp, $3: singal, $4: process id, $5: user id, $6: group id
if [ "$#" -ne 6 ]; then
    exit 1
fi
for arg in "$@"; do
...
USER_NAME=$(grep ^.*:x:$USER_ID: /etc/passwd | cut -d : -f 1)
if [ -z "$USER_NAME" ]; then
    exit 5
fi
...
GROUP_NAME=$(grep ^.*:x:$GRUOP_ID: /etc/passwd | cut -d : -f 1)
...
case $USER_NAME in
    ...
    CustAiCpuUser*)
        GROUP_TYPE=2
        COREDUMP_PATH="/var/log/npu/coredump/custproc"
        ;;
    HwDmUser)
        GROUP_TYPE=3
        COREDUMP_PATH="/var/log/npu/coredump"
...
if [ "$GROUP_TYPE" = 0 ]; then
    FILE_NUM=$(/usr/bin/ls $CORETRACE_PATH | grep "coretrace" | /usr/sbin/wc -l)
...
FILE_NUM=$(/usr/bin/ls $COREDUMP_PATH | grep "coretrace" | /usr/sbin/wc -l)
AGING_NUM=0
...
/usr/sbin/chown -RPf $USER_NAME:$GROUP_NAME "$COREDUMP_PATH/coretrace.$1.$4.$3.$2"
