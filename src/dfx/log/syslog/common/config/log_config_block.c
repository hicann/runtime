...
static const LogConfClassMap LOG_CONF_CLASS_MAP[] = {
    {"debug", "system", DEBUG_SYS_LOG_TYPE},  {"debug", "app", DEBUG_APP_LOG_TYPE},
    {"debug", "firmware", FIRM_LOG_TYPE},     {"run", "system", RUN_SYS_LOG_TYPE},
    {"run", "app", RUN_APP_LOG_TYPE},         {"event", "event", EVENT_LOG_TYPE},
    {"security", "system", SEC_SYS_LOG_TYPE}, {"security", "app", SEC_APP_LOG_TYPE}};

static const LogConfRuleMap LOG_CONF_RULE_MAP[] = {{"file", LOG_SAVE_FILE}, {"buffer", LOG_SAVE_BUFFER}};
...
static void LogConfOutputRuleFunc(const char* symbol, LogConfClass* confClass)
{
    ...
    int32_t num = sscanf_s(
        symbol, "%[^;];%[^;];%[^;];%s", modeStr, CONF_VALUE_MAX_LEN, maxSizeStr, CONF_VALUE_MAX_LEN, maxNumStr,
        CONF_VALUE_MAX_LEN, totalSizeStr, CONF_VALUE_MAX_LEN);
    if (num <= 0) {
        SELF_LOG_ERROR("fail to parse output rule, config value = %s", symbol);
        return;
    }
    confClass->outputRule.saveMode = LogConfGetSaveMode(modeStr);
    confClass->outputRule.fileSize = LogConfGetDigit(
        "max_size", maxSizeStr, LOG_OUTPUT_MIN_FILE_SIZE, LOG_OUTPUT_MAX_FILE_SIZE, LOG_OUTPUT_DEFAULT_FILE_SIZE);
    confClass->outputRule.fileNum = LogConfGetDigit(
        "max_num", maxNumStr, LOG_OUTPUT_MIN_FILE_NUM, LOG_OUTPUT_MAX_FILE_NUM, LOG_OUTPUT_DEFAULT_FILE_NUM);
    // totalSize must greater than fileSize
    confClass->outputRule.totalSize = LogConfGetDigit(
        "total_size", totalSizeStr, LOG_OUTPUT_MIN_FILE_SIZE, LOG_OUTPUT_MAX_FILE_SIZE, LOG_OUTPUT_DEFAULT_FILE_SIZE);
...
static void LogConfClassInit(void)
{
    for (uint32_t i = 0; i < LOG_TYPE_MAX_NUM; i++) {
        g_logConfClass[i].logClassify = -1;
    }
}
