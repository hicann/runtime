...
STATIC LogStatus PlogCreateNewFileName(PlogFileList* pstSubInfo)
{
    ...
    if (pstSubInfo->currIndex >= pstSubInfo->maxFileNum) {
        SELF_LOG_WARN(
            "current file number is illegal, file_bumber=%d, upper_limit=%d.", pstSubInfo->currIndex,
            pstSubInfo->maxFileNum);
        return LOG_FAILURE;
    }
    ...
    while (true) {
        ...
        int32_t err = snprintf_s(
            pstSubInfo->aucFileName[pstSubInfo->currIndex], MAX_FILENAME_LEN + 1U, MAX_FILENAME_LEN, "%s%s%s",
...
STATIC LogStatus PlogDeleteCurrentFile(const PlogFileList* pstSubInfo)
{
    ...
    if (pstSubInfo->currIndex >= pstSubInfo->maxFileNum) {
        SELF_LOG_WARN(
            "current file number is illegal, file_number=%d, upper_limit=%d.", pstSubInfo->currIndex,
            pstSubInfo->maxFileNum);
        return LOG_FAILURE;
    }
    ...
    int32_t ret = snprintf_s(
        aucFileName, MAX_FULLPATH_LEN + 1U, MAX_FULLPATH_LEN, "%s%s%s", pstSubInfo->aucFilePath, FILE_SEPARATOR,
...
{
    int32_t ret = snprintf_s(
        pFileName, ucMaxLen + 1U, ucMaxLen, "%s%s%s", pstSubInfo->aucFilePath, FILE_SEPARATOR,
        pstSubInfo->aucFileName[pstSubInfo->currIndex]);
    if (ret == -1) {
 ...
 */
STATIC LogStatus
PlogGetFileOfSize(PlogFileList* pstSubInfo, const StLogDataBlock* pstLogData, const char* pFileName, off_t* filesize)
{
    ToolStat statbuff = {0};
    FILE* fp = NULL;
 ...
 */
STATIC LogStatus
PlogGetFileName(PlogFileList* pstSubInfo, const StLogDataBlock* pstLogData, char* pFileName, size_t ucMaxLen)
{
    off_t filesize = 0;
    ...
    ONE_ACT_ERR_LOG(
        ret != LOG_SUCCESS, return LOG_FAILURE, "get file size failed, file=%s, result=%d.", pFileName, ret);
    ...
    if (((filesize + pstLogData->ulDataLen) > pstSubInfo->maxFileSize) || (pstSubInfo->devWriteFileFlag == 0)) {
        ...
        for (int32_t i = 0; i < pstSubInfo->fileNum; i++) {
            ...
            int32_t err = snprintf_s(
                tmpFileName, MAX_FULLPATH_LEN + 1U, ucMaxLen, "%s/%s", pstSubInfo->aucFilePath,
                pstSubInfo->aucFileName[i]);
            NO_ACT_ERR_LOG(
                err == -1, "snprintf_s filename failed, result=%d, strerr=%s.", err, strerror(ToolGetErrorCode()));

            // logFile may be deleted by user, then selflog will be ignored
            err = ToolChmod((const char*)tmpFileName, LOG_FILE_ARCHIVE_MODE);
            NO_ACT_WARN_LOG(
                (err != 0) && (ToolGetErrorCode() != ENOENT), "can not chmod file, file=%s, strerr=%s.", tmpFileName,
 ...
 */
STATIC LogStatus
PlogWrite(PlogFileList* pstSubInfo, const StLogDataBlock* pstLogData, char* const aucFileName, size_t aucFileNameLen)
{
    ONE_ACT_WARN_LOG(pstSubInfo == NULL, return LOG_FAILURE, "[input] log file list is null.");
    ONE_ACT_WARN_LOG(pstLogData == NULL, return LOG_FAILURE, "[input] log data is null.");
    ONE_ACT_WARN_LOG(aucFileName == NULL, return LOG_FAILURE, "[input] log filename is null.");
    ONE_ACT_WARN_LOG(
        ((aucFileNameLen == 0) || (aucFileNameLen > (MAX_FULLPATH_LEN + 1U))), return LOG_FAILURE,
        "[input] log filename length is invalid, length=%zu", aucFileNameLen);
    ...
    if (fd < 0) {
        SELF_LOG_ERROR("open file failed with mode, file=%s, strerr=%s.", aucFileName, strerror(ToolGetErrorCode()));
        LOG_WARN_FPRINTF_ONCE(
            "can not open file, file: %s, possible reason: %s.", aucFileName, strerror(ToolGetErrorCode()));
        return LOG_FAILURE;
    }
    ...
    if ((ret < 0) || ((unsigned)ret != pstLogData->ulDataLen)) {
        LOG_CLOSE_FD(fd);
        SELF_LOG_ERROR_N(
            &g_rootWritePrintNum, WRITE_PRINT_NUM,
        ...
        LOG_WARN_FPRINTF_ONCE(
            "can not write log to file, file: %s, data_length: %u bytes, "
    ...
    if (ulRet != LOG_SUCCESS) {
        SELF_LOG_ERROR(
            "change file owner failed, file=%s, log_err=%d, strerr=%s.", aucFileName, ulRet,
            strerror(ToolGetErrorCode()));
    }
    LOG_CLOSE_FD(fd);
    return LOG_SUCCESS;
...
STATIC LogStatus PlogMkdir(const char* logPath)
{
    ...
    if (err != SUCCESS) {
        SELF_LOG_ERROR(
            "mkdir %s failed, strerr=%s, log_err=%d.", g_plogFileList->rootPath, strerror(ToolGetErrorCode()),
            (int32_t)err);
        LOG_WARN_FPRINTF_ONCE(
            "can not create directory, directory: %s, possible reason: %s.", g_plogFileList->rootPath,
            strerror(ToolGetErrorCode()));
        return LOG_FAILURE;
    }
    // create log path
    ...
    for (int32_t idx = (int32_t)DEBUG_LOG; idx < (int32_t)LOG_TYPE_NUM; idx++) {
        ...
        ret = snprintf_s(
            sortPath, MAX_FILEDIR_LEN + 1U, MAX_FILEDIR_LEN, "%s/%s", g_plogFileList->rootPath, sortDirName[idx]);
        ...
        if (err != SUCCESS) {
            SELF_LOG_ERROR(
                "mkdir %s failed, strerr=%s, log_err=%d.", sortPath, strerror(ToolGetErrorCode()), (int32_t)err);
            return LOG_FAILURE;
        }
    }
...
LogStatus PlogWriteHostLog(int32_t logType, char* msg, uint32_t len)
{
    ...
    ONE_ACT_WARN_LOG(
        (logType < (int32_t)DEBUG_LOG) || (logType >= (int32_t)LOG_TYPE_NUM), return LOG_FAILURE,
        "[input] wrong log type %d", logType);
...
LogStatus PlogWriteDeviceLog(char* msg, const PlogDeviceLogInfo* info)
{
    ...
    if ((info->deviceId > g_plogFileList->deviceNum) || (info->slogFlag == 1)) {
        SELF_LOG_WARN(
            "[input] wrong device_id=%u, device_number=%u or slogFlag=%d.", info->deviceId, info->deviceId,
            info->slogFlag);
        return LOG_FAILURE;
    }
...
STATIC INLINE LogStatus PlogInitDeviceLogFileHead(char* fileHead, uint32_t length, uint32_t pid)
{
    ...
    if (err == -1) {
        SELF_LOG_ERROR("get device header failed, result=%d, strerr=%s.", err, strerror(ToolGetErrorCode()));
        return LOG_FAILURE;
    }
    return LOG_SUCCESS;
}

STATIC INLINE LogStatus PlogInitDeviceLogFilePath(
    char* filePath, uint32_t length, const char* rootPath, const char* sortName, uint32_t deviceId)
{
    int32_t err = snprintf_s(
        filePath, length, (size_t)length - 1U, "%s%s%s%s%s%u", rootPath, FILE_SEPARATOR, sortName, FILE_SEPARATOR,
        DEVICE_HEAD, deviceId);
    if (err == -1) {
        SELF_LOG_ERROR(
            "get device log dir path failed, device_id=%u, result=%d, strerr=%s.", deviceId, err,
            strerror(ToolGetErrorCode()));
        return LOG_FAILURE;
    }
    return LOG_SUCCESS;
...
STATIC LogStatus PlogInitDeviceMaxFileNum(PlogFileMgrInfo* logList)
{
    ...
    for (uint32_t iType = 0; iType < (uint32_t)LOG_TYPE_NUM; iType++) {
        for (uint32_t idx = 0; idx < logList->deviceNum; idx++) {
            ...
            ONE_ACT_ERR_LOG(
                ret != LOG_SUCCESS, return LOG_FAILURE, "init sort[%u] device[%u] file head failed, result=%d.", iType,
                idx, ret);

            ret = PlogInitDeviceLogFilePath(
                list->aucFilePath, MAX_FILEPATH_LEN + 1U, logList->rootPath, sortDirName[iType], idx);
            ONE_ACT_ERR_LOG(
                ret != LOG_SUCCESS, return LOG_FAILURE, "init sort[%u] device[%u] file path failed, result=%d.", iType,
                idx, ret);

            ret = PlogInitFileNumArray(list);
            ONE_ACT_ERR_LOG(
                ret != LOG_SUCCESS, return LOG_FAILURE, "init max device filename list failed, result=%d.", ret);
        }
    }
    return LOG_SUCCESS;
}
...
STATIC INLINE LogStatus PlogInitHostLogFileHead(char* fileHead, uint32_t length, uint32_t pid)
{
    ...
    if (err == -1) {
        SELF_LOG_ERROR("get proc header failed, result=%d, strerr=%s.", err, strerror(ToolGetErrorCode()));
        return LOG_FAILURE;
    }
    return LOG_SUCCESS;
}

STATIC INLINE LogStatus
PlogInitHostLogFilePath(char* filePath, uint32_t length, const char* rootPath, const char* sortName)
{
    int32_t err = snprintf_s(
        filePath, length, (size_t)length - 1U, "%s%s%s%s%s", rootPath, FILE_SEPARATOR, sortName, FILE_SEPARATOR,
        PROC_DIR_NAME);
    if (err == -1) {
        SELF_LOG_ERROR("get log dir path failed, result=%d, strerr=%s.", err, strerror(ToolGetErrorCode()));
        return LOG_FAILURE;
    }
    return LOG_SUCCESS;
}
...
STATIC LogStatus PlogInitHostLogList(PlogFileMgrInfo* logList)
{
    ...
    for (int32_t i = (int32_t)DEBUG_LOG; i < (int32_t)LOG_TYPE_NUM; i++) {
        ...
        ONE_ACT_ERR_LOG(
            ret != LOG_SUCCESS, return LOG_FAILURE, "init sort[%d] host file head failed, result=%d.", i, ret);

        ret = PlogInitHostLogFilePath(list->aucFilePath, MAX_FILEPATH_LEN + 1U, logList->rootPath, sortDirName[i]);
        ONE_ACT_ERR_LOG(
            ret != LOG_SUCCESS, return LOG_FAILURE, "init sort[%d] host file path failed, result=%d.", i, ret);
...
STATIC LogStatus PlogGetEnvString(char* buf, uint32_t len, const char* env)
{
    ...
    TWO_ACT_ERR_LOG(
        err != EOK, XFREE(path), return LOG_FAILURE, "strcpy env string failed, ret=%d, strerr=%s.", (int32_t)err,
        strerror(ToolGetErrorCode()));
...
STATIC LogStatus PlogInitRootPath(PlogFileMgrInfo* logList)
{
    ...
    if (err != LOG_SUCCESS) {
        ret = snprintf_truncated_s(
            logList->rootPath, MAX_FILEDIR_LEN + 1U, "%s%s%s", homeDir, FILE_SEPARATOR, PROCESS_SUB_LOG_PATH);
        TWO_ACT_ERR_LOG(ret < 0, XFREE(homeDir), return LOG_FAILURE, "get home directory failed, ret=%d.", ret);
...
LogStatus PlogFileMgrInit(void)
{
    ...
    TWO_ACT_ERR_LOG(
        ret != LOG_SUCCESS, XFREE(g_plogFileList), return LOG_FAILURE, "init file root path failed, ret=%d.", ret);
...
void PlogReinitFileHeadsForChild(void)
{
    ...
    for (int32_t i = (int32_t)DEBUG_LOG; i < (int32_t)LOG_TYPE_NUM; i++) {
        ...
        if ((list->aucFileName != NULL) && (list->currIndex < list->maxFileNum)) {
            (void)memset_s(list->aucFileName[list->currIndex], MAX_FILENAME_LEN + 1U, 0, MAX_FILENAME_LEN + 1U);
        }
 ...
 */
PlogFileMgrInfo* PlogGetFileMgrInfo(void) { return g_plogFileList; }
