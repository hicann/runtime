...
STATIC void DlogInitGlobalLogLevelByEnv(void)
{
    ...
    if (globalLevelEnv != NULL) {
        ...
        if ((LogStrToInt(globalLevelEnv, &globalLevel) == LOG_SUCCESS) && (globalLevel >= LOG_MIN_LEVEL) &&
            (globalLevel <= LOG_MAX_LEVEL)) {
    ...
    SELF_LOG_INFO(
        "set default global log level (%d), debug log level (%d), run log level (%d).", DLOG_GLOABLE_DEFAULT_LEVEL,
        DLOG_DEBUG_DEFAULT_LEVEL, DLOG_RUN_DEFAULT_LEVEL);
}
...
STATIC LogStatus DlogCheckItem(const char* token, int32_t* moduleIdPtr, int32_t* logLevelPtr)
{
    ...
    ONE_ACT_ERR_LOG(
        (strchr(itemStr, '=') == NULL) || (strchr(itemStr, '=') != strrchr(itemStr, '=')), return LOG_FAILURE,
        "option string: \"%s\" is not valid", itemStr); // check if only have one "="
    char* nameStr = NULL;
    char* valueStr = NULL;
    char* ptr = itemStr;
    int32_t i;
    ...
    ONE_ACT_ERR_LOG(
        (nameStr == NULL) || (valueStr == NULL) || (strlen(nameStr) == 0) || (strlen(valueStr) == 0),
    ...
    ONE_ACT_ERR_LOG(
        (ret != LOG_SUCCESS) || (value < LOG_MIN_LEVEL) || (value > LOG_MAX_LEVEL), return LOG_FAILURE,
        "Parse the log value failed, valueStr: %s", valueStr); // check if value is valid
    *moduleIdPtr = id;
...
STATIC LogStatus DlogInitModuleLogLevelByEnv(void)
{
    ...
    ONE_ACT_ERR_LOG(
        (size == 0U) || (size > (size_t)MAX_ASCEND_MODULE_LOG_ENV_LEN), return LOG_FAILURE,
    ...
    ONE_ACT_ERR_LOG(
        strcpy_s(moduleEnv, MAX_ASCEND_MODULE_LOG_ENV_LEN + 1, env) != EOK, return LOG_FAILURE, "strcpy_s failed.");
    ...
    while (token != NULL) {
        // INVALID_MODULE_ID
        ONE_ACT_ERR_LOG(
            itemNum >= INVALID_MODULE_ID, return LOG_FAILURE, "env ASCEND_MODULE_LOG_LEVEL number exceed the limit");
        ...
        ONE_ACT_ERR_LOG(
            (res == LOG_FAILURE) || (moduleId == INVALID_NUM) || (logLevel == INVALID_NUM), return LOG_FAILURE,
            "Init log level by ASCEND_MODULE_LOG_LEVEL abort");
