...
#define LOG_MONITOR_TIME_OUT_PRINT(cost, threshold, format, ...)                                           \
    NO_ACT_WARN_LOG(                                                                                       \
        (cost) / LogGetCpuFrequency() * TICK_TO_US >= (threshold), format " timeout, cost time = %llu us", \
        ##__VA_ARGS__, (cost) / LogGetCpuFrequency() * TICK_TO_US)
...
STATIC DlogNsycMgr g_dlogAsyncMgr = {
    false, false, {{NULL, NULL, NULL, NULL}, 0, 0, 0, 0}, PTHREAD_MUTEX_INITIALIZER, {false, DLOG_DEBUG, 0, {0, 0}},
    -1};
...
STATIC void DlogUnLock(void) { UNLOCK_WARN_LOG(&g_dlogAsyncMgr.mutex); }
    ...
    if (timeValue >= LOG_WARN_INTERVAL) {
        if (timeValue < LOG_INFO_INTERVAL) {
            if (g_dlogAsyncMgr.levelCtrl.ctrlLevel != DLOG_WARN) {
                ...
                SELF_LOG_WARN(
                    "log control down to level=WARNING, pid=%d, pid_name=%s,"
            ...
            if (g_dlogAsyncMgr.levelCtrl.ctrlLevel != DLOG_INFO) {
                ...
                SELF_LOG_WARN(
                    "log control down to level=INFO, pid=%d, pid_name=%s, log level control loss num is %lu",
                    DlogGetCurrPid(), DlogGetPidName(), g_dlogAsyncMgr.levelCtrl.lossCount);
            }
        } else {
            ...
            SELF_LOG_WARN(
                "clear log control switch, pid=%d, pid_name=%s, log level control loss num is %lu", DlogGetCurrPid(),
                DlogGetPidName(), g_dlogAsyncMgr.levelCtrl.lossCount);
...
STATIC void LogCtrlIncLogic(void)
{
    if (!g_dlogAsyncMgr.levelCtrl.ctrlSwitch) {
        ...
        SELF_LOG_WARN(
            "set log control switch to level=ERROR, pid=%d, pid_name=%s, log loss num=%lu.", DlogGetCurrPid(),
            DlogGetPidName(), g_dlogAsyncMgr.levelCtrl.lossCount);
    } else {
        if (g_dlogAsyncMgr.levelCtrl.ctrlLevel < DLOG_ERROR) {
            ...
            if (g_dlogAsyncMgr.levelCtrl.ctrlLevel == DLOG_ERROR) {
                SELF_LOG_WARN(
                    "log control up to level=ERROR, pid=%d, pid_name=%s, log loss num=%lu.", DlogGetCurrPid(),
                    DlogGetPidName(), g_dlogAsyncMgr.levelCtrl.lossCount);
            } else {
                SELF_LOG_WARN(
                    "log control up to level=WARNING, pid=%d, pid_name=%s, log loss num=%lu.", DlogGetCurrPid(),
                    DlogGetPidName(), g_dlogAsyncMgr.levelCtrl.lossCount);
            }
        }
    }
 ...
 */
STATIC void DlogBufReInit(RingBufferStat* ringBuffer) { LogBufReInit(ringBuffer); }
...
static void DlogUnifiedTimerCb(void)
{
    ...
    LOG_MONITOR_TIME_OUT(
        DlogSyncAllSendBuf(&g_dlogAsyncMgr.bufMgr.sendBufIndex, writeBufIndex), LOG_SEND_COST_MAX_TIME,
        "%s send all buffer", DLOG_TIMER_NAME);

    bool ret = false;
    LOG_MONITOR_TIME_OUT(
        ret = DlogCheckWriteBufModify(writeBufIndex), LOG_LOCK_COST_MAX_TIME, "%s buffer switch get lock",
        DLOG_TIMER_NAME);
    if (ret) {
        LOG_MONITOR_TIME_OUT(
            DlogModifySendBuf(writeBufIndex), LOG_SEND_COST_MAX_TIME, "%s send newest buffer", DLOG_TIMER_NAME);
...
STATIC void DlogPrintLogLoss(void)
{
    ...
    if (costTime >= periodTime) {
        SELF_LOG_WARN(
            "dlog buffer covered log loss num = %lu, print every %lu seconds.", g_dlogAsyncMgr.bufMgr.lossCount,
            LOG_LOST_PRINT_PERIOD_TIME / (uint64_t)S_TO_MS);
...
STATIC int32_t DlogRingBufInit(RingBufferStat** ringBuffer, uint32_t size)
{
    *ringBuffer = (RingBufferStat*)LogMalloc(sizeof(RingBufferStat));
    ...
    if (err != LOG_SUCCESS) {
        SELF_LOG_ERROR(
            "init log buffer head failed, err = %d, strerr = %s, pid = %d.", err, strerror(ToolGetErrorCode()),
            ToolGetPid());
...
void DlogAsyncExit(void)
{
    ...
    SELF_LOG_INFO(
        "pid=%d, pid_name=%s quit, log covered loss num is %lu, log level control loss num is %lu.", DlogGetCurrPid(),
        DlogGetPidName(), g_dlogAsyncMgr.bufMgr.lossCount, g_dlogAsyncMgr.levelCtrl.lossCount);
    DlogBufExit();
    DlogUnLock();
    if (DlogIsAosCore()) {
