 ...
 */
int32_t IsSocketConnected(void) { return g_connectFlag; }
 ...
 * @return: void
 */
void SetSocketFd(int32_t fd) { g_logFd = fd; }

toolSockHandle GetSocketFd(void) { return g_logFd; }
...
STATIC int32_t DlogSocketConnect(toolSockHandle sockFd, const char* socketName)
{
    ...
    ONE_ACT_ERR_LOG(
        ret < 0, return SYS_ERROR, "set socket option failed, strerr=%s, pid=%d.", strerror(ToolGetErrorCode()), pid);

    ret = memset_s(&sunx, sizeof(sunx), 0, sizeof(sunx));
    ONE_ACT_ERR_LOG(
        ret != EOK, return SYS_ERROR, "memset_s failed, result=%d, strerr=%s, pid=%d.", ret,
        strerror(ToolGetErrorCode()), pid);

    sunx.sun_family = AF_UNIX;
    char socketPath[WORKSPACE_PATH_MAX_LENGTH + 1U] = {0};
    ret = snprintf_s(
        socketPath, WORKSPACE_PATH_MAX_LENGTH + 1U, WORKSPACE_PATH_MAX_LENGTH, "%s/rsyslog_%s", DEFAULT_LOG_WORKSPACE,
        socketName);
    ONE_ACT_ERR_LOG(
        ret == -1, return SYS_ERROR, "snprintf_s failed, result=%d, strerr=%s, pid=%d.", ret,
        strerror(ToolGetErrorCode()), pid)
    ...
    if (ret != SYS_OK) {
        SELF_LOG_WARN_N(
            &g_connPrintNum, CONN_W_PRINT_NUM, "rsyslogd is not ready, will try to connect again later, pid=%d, \
                        print once every %d times.",
            pid, CONN_W_PRINT_NUM);
        return SYS_ERROR;
    }
...
STATIC toolSockHandle GetRsyslogSocket(const char* socketName)
{
    ...
    ONE_ACT_ERR_LOG(
        sockFd == SYS_ERROR, return SYS_ERROR, "create socket failed, strerr=%s, pid=%d", strerror(ToolGetErrorCode()),
        pid);
...
int32_t CloseSocket(void) { return ToolClose(g_logFd); }
...
STATIC int DlogStrcatDir(char* path, const char* filename, const char* dir, unsigned int maxlen)
{
    ...
    do {
        ...
        if (ret != EOK) {
            SYSLOG_WARN(
                "strcpy_s failed, path=%s, dir=%s, result=%d, strerr=%s.\n", path, dir, ret,
        ...
        if (ret != EOK) {
            SYSLOG_WARN(
                "strcat_s failed, path=%s, filename=%s, result=%d, strerr=%s.\n", path, filename, ret,
                strerror(ToolGetErrorCode()));
            break;
        }
        return SYS_OK;
    ...
    if (ret != EOK) {
        SYSLOG_WARN(
            "memset_s failed, path=%s, filename=%s, result=%d, strerr=%s.\n", path, filename, ret,
            strerror(ToolGetErrorCode()));
    }
    return SYS_ERROR;
}
...
STATIC int32_t GetSocketPathByVfid(const uint32_t vfid, char* socketPath, const uint32_t pathLen)
{
    ...
    int32_t ret =
        snprintf_s(fileName, WORKSPACE_FILE_MAX_LENGTH + 1U, WORKSPACE_FILE_MAX_LENGTH, "%s_%u", SOCKET_FILE, vfid);
    if (ret == -1) {
        SELF_LOG_ERROR(
            "snprintf_s failed, strerr=%s, pid=%d, vfid=%u.", strerror(ToolGetErrorCode()), ToolGetPid(), vfid);
        return SYS_ERROR;
    }
...
STATIC int32_t GetSocketPathByPfid(char* socketPath, const uint32_t pathLen)
{
    ...
    ONE_ACT_WARN_LOG(ToolAccess(customSlogPath) != SYS_OK, return SYS_OK, "Path %s cannot access.", customSlogPath);
...
toolSockHandle CreatSocket(uint32_t devId)
{
    ...
    do {
        ...
        ONE_ACT_ERR_LOG(
            ret < 0, break, "set socket option failed, strerr=%s, pid=%d.", strerror(ToolGetErrorCode()), pid);

        ret = memset_s(&sunx, sizeof(sunx), 0, sizeof(sunx));
        ONE_ACT_ERR_LOG(
            ret != EOK, break, "memset_s failed, result=%d, strerr=%s, pid=%d.", ret, strerror(ToolGetErrorCode()),
            pid);
        ...
        if (ret != SYS_OK) {
            SELF_LOG_WARN_N(
                &g_connPrintNum, CONN_W_PRINT_NUM, "slogd is not ready, will try to connect again later, pid=%d, \
                            print once every %d times.",
                pid, CONN_W_PRINT_NUM);
            break;
        }
