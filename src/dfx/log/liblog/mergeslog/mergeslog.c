...
STATIC INLINE bool MergeIsBusy(int32_t err) { return ((err) == ETIMEDOUT) || ((err) == EAGAIN) || ((err) == EBUSY); }
...
STATIC int32_t MergeGetRealPath(char* dir, char* validPath, int32_t length, int32_t mode)
{
    ...
    if ((ToolRealPath(dir, validPath, length) != SYS_OK) && (ToolGetErrorCode() != ENOENT)) {
        SELF_LOG_WARN(
            "can not get realpath, file=%s, pid = %d, strerr=%s.", dir, ToolGetPid(), strerror(ToolGetErrorCode()));
        return SYS_ERROR;
    }
    ONE_ACT_ERR_LOG(
        MergeCheckDir(validPath, mode) != SYS_OK, return SYS_ERROR, "dir[%s] is invalid, pid = %d.", validPath,
        ToolGetPid());
    return SYS_OK;
}
...
int32_t DlogCollectLog(char* dir, uint32_t len)
{
    ...
    ONE_ACT_ERR_LOG(
        MergeGetRealPath(dir, validPath, PATH_MAX, W_OK) != SYS_OK, return MERGE_NO_PERMISSION,
        "get dir real path failed, pid = %d.", ToolGetPid());
    errno_t err = strcat_s(validPath, (size_t)PATH_MAX - 1U, OUTPUT_GZ_LOG_FILE);
    if (err != EOK) {
        SELF_LOG_ERROR(
            "make log path failed, ret = %d, errno = %d, pid = %d.", (int32_t)err, ToolGetErrorCode(), ToolGetPid());
        return MERGE_ERROR;
    }
    ...
    int32_t fd = INVALID;
    ONE_ACT_ERR_LOG(
        MergeOpenIamService(&fd) != SYS_OK, return MERGE_ERROR, "open iam service failed, errno = %d, pid = %d.",
        ToolGetErrorCode(), ToolGetPid());
    ...
    if (ret != SYS_OK) {
        SELF_LOG_ERROR(
            "iam service ioctl failed, ret = %d, errno = %d, pid = %d.", ret, ToolGetErrorCode(), ToolGetPid());
        (void)close(fd);
        return MERGE_ERROR;
    }
...
int32_t DlogCheckCollectStatus(char* dir, uint32_t len)
{
    ...
    char validPath[PATH_MAX] = {0};
    ONE_ACT_ERR_LOG(
        MergeGetRealPath(dir, validPath, PATH_MAX, R_OK) != SYS_OK, return MERGE_NO_PERMISSION,
        "get dir real path failed, pid = %d.", ToolGetPid());
    errno_t err = strcat_s(validPath, (size_t)PATH_MAX - 1U, OUTPUT_GZ_LOG_FILE);
    if (err != EOK) {
        SELF_LOG_ERROR(
...
STATIC void DlogConstructPatterns(struct DlogPattern* pattern, int32_t* num, LogPathPattern* path)
{
    ...
    ret = snprintf_s(pattern[*num].path, PATH_MAX, (size_t)PATH_MAX - 1U, "%s/%s/", path->path, path->dirName);
    retActive =
        snprintf_s(pattern[*num].active, NAME_MAX, (size_t)NAME_MAX - 1U, "%s%s", path->fileName, path->activeSuffix);
    retRotate =
        snprintf_s(pattern[*num].rotate, NAME_MAX, (size_t)NAME_MAX - 1U, "%s%s", path->fileName, path->rotateSuffix);
    if ((ret != -1) && (retActive != -1) && (retRotate != -1)) {
        (*num)++;
    } else {
        SELF_LOG_ERROR(
            "snprintf failed, construct log dir[%s/%s] failed, pid = %d.", path->dirName, path->fileName, ToolGetPid());
 ...
 */
STATIC void DlogGetGroupLogPatterns(
    struct DlogPattern* pattern, int32_t* num, LogPathPattern* pathPattern, const LogConfigInfo* info)
{
    ...
    for (int32_t groupId = 0; groupId < GROUP_MAP_SIZE; groupId++) {
        ...
        if (strncmp(info->groupName[groupId], "device-0", strlen(info->groupName[groupId])) == 0) {
            int32_t ret = snprintf_s(
                pathPattern->dirName, NAME_MAX, (size_t)NAME_MAX - 1U, "%s/%s", DEBUG_DIR_NAME,
                info->groupName[groupId]);
            if (ret == -1) {
                SELF_LOG_ERROR(
                    "snprintf failed, construct dirName[%s/%s] failed, pid = %d.", DEBUG_DIR_NAME,
            ...
            if (ret == -1) {
                SELF_LOG_ERROR(
                    "snprintf failed, construct dirName[%s] failed, pid = %d.", DEBUG_DIR_NAME, ToolGetPid());
                continue;
            }
        }
        ...
        if (err != EOK) {
            SELF_LOG_ERROR(
                "memcpy failed, get group filename failed, filename = %s, pid = %d.", info->groupName[groupId],
                ToolGetPid());
        }
 ...
 * @param [in]      : rootPath     log root path
 */
STATIC void DlogGetGeneralLogPatterns(
    struct DlogPattern* pattern, int32_t* num, LogPathPattern* pathPattern, const LogConfigInfo* info)
{
    (void)memset_s(pathPattern->path, PATH_MAX, 0, PATH_MAX);
    ...
    for (uint32_t type = (uint32_t)DEBUG_LOG; type < (uint32_t)LOG_TYPE_NUM; type++) {
        for (uint32_t i = 0; i < GENERAL_LOG_NUM; i++) {
            (void)memset_s(pathPattern->dirName, NAME_MAX, 0, NAME_MAX);
            int32_t ret = snprintf_s(
                pathPattern->dirName, NAME_MAX, (size_t)NAME_MAX - 1U, "%s/%s*", sortDirName[type], logDir[i]);
            if (ret == -1) {
                SELF_LOG_ERROR(
                    "snprintf failed, construct dirName[%s/%s] failed, pid = %d.", sortDirName[type], logDir[i],
                    ToolGetPid());
                continue;
            }
            (void)memset_s(pathPattern->fileName, NAME_MAX, 0, NAME_MAX);
            ...
            if (err != EOK) {
                SELF_LOG_ERROR(
                    "memcpy failed, get filename failed, filename = %s, pid = %d.", logName[i], ToolGetPid());
            }
            DlogConstructPatterns(pattern, num, pathPattern);
        }
    }
 ...
 * @param [in]      : rootPath     log root path
 */
STATIC void DlogGetEventLogPatterns(
    struct DlogPattern* pattern, int32_t* num, LogPathPattern* pathPattern, const LogConfigInfo* info)
{
    (void)memset_s(pathPattern->path, PATH_MAX, 0, PATH_MAX);
    errno_t err = strncpy_s(pathPattern->path, PATH_MAX, info->rootPath, PATH_MAX);
    ...
    (void)memset_s(pathPattern->dirName, NAME_MAX, 0, NAME_MAX);
    int32_t ret =
        snprintf_s(pathPattern->dirName, NAME_MAX, (size_t)NAME_MAX - 1U, "%s/%s", RUN_DIR_NAME, EVENT_DIR_NAME);
    if (ret == -1) {
        SELF_LOG_ERROR(
            "snprintf failed, construct dirName[%s/%s] failed, pid = %d.", RUN_DIR_NAME, EVENT_DIR_NAME, ToolGetPid());
    }
    (void)memset_s(pathPattern->fileName, NAME_MAX, 0, NAME_MAX);
    err = memcpy_s(pathPattern->fileName, (size_t)NAME_MAX - 1U, EVENT_HEAD, strlen(EVENT_HEAD));
    if (err != EOK) {
        SELF_LOG_ERROR("memcpy failed, get filename failed, filename = %s, pid = %d.", EVENT_HEAD, ToolGetPid());
    }
    DlogConstructPatterns(pattern, num, pathPattern);
}
...
STATIC void DlogGetSelfLogPatterns(struct DlogPattern* pattern, int32_t* num, const char* rootPath, uint32_t pathLen)
{
    (void)pathLen;
    int32_t ret =
        snprintf_s(pattern[*num].path, PATH_MAX, (size_t)PATH_MAX - 1U, "%s%s/", rootPath, LOG_DIR_FOR_SELF_LOG);
...
STATIC int32_t DlogGetAllLogPatterns(struct DlogNamePatterns* logs, const LogConfigInfo* info)
{
    ...
    int32_t retActive =
        snprintf_s(pathPattern.activeSuffix, NAME_MAX, (size_t)NAME_MAX - 1U, ".*%s", LOG_ACTIVE_FILE_GZ_SUFFIX);
    int32_t retRotate = snprintf_s(
        pathPattern.rotateSuffix, NAME_MAX, (size_t)NAME_MAX - 1U, "(?!.*%s).*%s", LOG_ACTIVE_STR, GZIP_SUFFIX);
...
STATIC int32_t DlogGetConfigInfo(LogConfigInfo* info)
{
    ...
    int32_t fd = INVALID;
    ONE_ACT_ERR_LOG(
        MergeOpenIamService(&fd) != SYS_OK, return SYS_ERROR, "open iam service failed, errno = %d, pid = %d.",
        ToolGetErrorCode(), ToolGetPid());
    SELF_LOG_INFO("open iam service succeed, pid = %d.", ToolGetPid());
    struct IAMIoctlArg arg;
...
int32_t DlogGetLogPatterns(struct DlogNamePatterns* logs)
{
    ...
    ONE_ACT_ERR_LOG(
        info == NULL, return MERGE_ERROR, "malloc log config info failed, errno = %d, pid = %d.", ToolGetErrorCode(),
        ToolGetPid());
    int32_t ret = DlogGetConfigInfo(info);
    TWO_ACT_ERR_LOG(
        ret != SYS_OK, LogFree(info), return MERGE_ERROR, "get log config info failed, pid = %d.", ToolGetPid());
    ...
    TWO_ACT_ERR_LOG(
        logs->patterns == NULL, LogFree(info), return MERGE_ERROR, "malloc log patterns failed, errno = %d, pid = %d.",
        ToolGetErrorCode(), ToolGetPid());
