...
private:
     ...
     * Run aicpu profiling
     */
    uint32_t RunAicpuFunc(const void* const kernelBase, AicpuKFCOpFuncPtr& opFuncPtr) const;
    ...
    int32_t GetApiWhenSonameNotEmpty(
        const aicpu::KernelType kernelType, const uint32_t soNameLen, const char_t* kernelSoName,
        const char_t* kernelName, void*& funcAddr);

    int32_t GetApiBySoname(
