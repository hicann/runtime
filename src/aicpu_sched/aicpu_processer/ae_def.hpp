...
#define AE_ERR_LOG(id, fmt, args...)                                                                         \
    do {                                                                                                     \
        if (&DlogRecord != nullptr) {                                                                        \
            dlog_error(                                                                                      \
                static_cast<int32_t>(AE_MODULE_ID), "[%s][tid:%ld][AICPU_PROCESSER] " fmt, &__FUNCTION__[0], \
                cce::GetTid(), ##args);                                                                      \
        }                                                                                                    \
    } while (0)
...
#define AE_RUN_INFO_LOG(id, fmt, args...)                                                                        \
    do {                                                                                                         \
        if ((&CheckLogLevel != nullptr) && (&DlogRecord != nullptr)) {                                           \
            dlog_info(                                                                                           \
                static_cast<int32_t>(static_cast<uint32_t>(AE_MODULE_ID) | static_cast<uint32_t>(RUN_LOG_MASK)), \
                "[%s][tid:%ld][AICPU_PROCESSER] " fmt, &__FUNCTION__[0], cce::GetTid(), ##args);                 \
        }                                                                                                        \
    } while (0)
...
#define AE_SINGLETON_CREAT(CLASS)                       \
    do {                                                \
        mtx_.lock();                                    \
