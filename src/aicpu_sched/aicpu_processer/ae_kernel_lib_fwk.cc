...
aeStatus_t AIKernelsLibFWK::Init() { return tfImpl_.Init(); }
...
{
    const auto fwkKernel = PtrToPtr<const void, const aicpu::HwtsFwkKernel>(kernelBase);
    ...
    const auto fwkOpKernel = PtrToPtr<const void, const STR_FWK_OP_KERNEL>(ValueToPtr(fwkKernel->kernel));
    if (static_cast<bool>(unlikely(fwkOpKernel == nullptr))) {
        AE_ERR_LOG(AE_MODULE_ID, "Input param fwkOpKernel is NULL.");
        return AE_STATUS_BAD_PARAM;
    }
    AE_INFO_LOG(
        AE_MODULE_ID, "Current kernelType:%d, FWK op kernel kernel type:%d.", kernelType, fwkOpKernel->fwkKernelType);
    ...
    switch (static_cast<FwkkernelType_t>(fwkOpKernel->fwkKernelType)) {
        case FMK_KERNEL_TYPE_TF:
            ret = tfImpl_.CallKernelApi(PtrToValue(PtrToPtr<const ::aicpu::FWKAdapter::FWKOperateParam, const void>(
                &fwkOpKernel->fwkKernelBase.fwk_kernel)));
            break;
        default:
            AE_ERR_LOG(
                AE_MODULE_ID,
...
FWKKernelTfImpl::FWKKernelTfImpl()
    : kernelName_("TFOperateAPI"), funcAddr_(nullptr), soHandle_(nullptr), soTensorflowHandle_(nullptr)
{}
...
aeStatus_t FWKKernelTfImpl::Init()
{
    ...
    if (status == aicpu::AICPU_ERROR_NONE) {
        ...
        if (ret != AE_STATUS_SUCCESS) {
            soFile_ = baseSoFile;
            AE_RUN_INFO_LOG(
                AE_MODULE_ID, "So does not exist in path %s, use default soFile %s.", soPath.c_str(),
                baseSoFile.c_str());
        }
    }
...
int32_t FWKKernelTfImpl::CallKernelApi(const uint64_t fwkKernelParam)
{
    ...
    if (static_cast<bool>(unlikely(theFuncAddr == nullptr))) {
        ...
        if (static_cast<bool>(unlikely(funcAddr_ == nullptr))) {
            AE_INFO_LOG(
                AE_MODULE_ID, "Begin to GetApi, soFile=%s, kernelName=%s.", GetSoFile().c_str(),
                GetKernelName().c_str());

            aeStatus_t retGetApi = AE_STATUS_SUCCESS;
            if (static_cast<bool>(unlikely(soHandle_ == nullptr))) {
                retGetApi = SingleSoManager::GetApi(GetSoFile().data(), GetKernelName().data(), &funcAddr_, &soHandle_);
            } else {
...
aeStatus_t FWKKernelTfImpl::TransformKernelErrorCode(const uint32_t errCode, const uint64_t fwkKernelParam)
{
    ...
    AE_ERR_LOG(
        AE_MODULE_ID, "Call tf api return failed:%u, returncode:%u, input param to tf api:0x%lx", errCode, returnCode,
        fwkKernelParam);
    // other error code: transform to inner_error
    return static_cast<aeStatus_t>(returnCode);
}

const std::string& FWKKernelTfImpl::GetSoFile() const { return soFile_; }

const std::string& FWKKernelTfImpl::GetKernelName() const { return kernelName_; }
} // namespace cce
