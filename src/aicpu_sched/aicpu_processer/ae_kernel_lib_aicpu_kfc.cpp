...
int32_t AIKernelsLibAiCpuKFC::GetApiGlobal(AicpuKFCOpFuncPtr& opFuncPtr, const char_t* kernelName) const
{
    ...
    if (static_cast<bool>(unlikely(opFuncPtr == nullptr))) {
        AE_ERR_LOG(
            AE_MODULE_ID, "Get KFC api by global failed, func is nullptr, kernelName[%s], dlerror[%s]", kernelName,
            dlerror());
...
int32_t AIKernelsLibAiCpuKFC::GetApiWhenSonameNotEmpty(
    const aicpu::KernelType kernelType, const uint32_t soNameLen, const char_t* kernelSoName, const char_t* kernelName,
    void*& funcAddr)
{
    ...
    if (static_cast<bool>(unlikely(funcAddr == nullptr))) {
        AE_RUN_WARN_LOG(AE_MODULE_ID, "Get %s api from %s success, but func is nullptr", kernelName, kernelSoName);
        return static_cast<int32_t>(AE_STATUS_INNER_ERROR);
    }
...
int32_t AIKernelsLibAiCpuKFC::GetApiBySoname(
    const aicpu::KernelType kernelType, const char_t* kernelSoName, AicpuKFCOpFuncPtr& opFuncPtr,
    const char_t* kernelName)
{
    const uint32_t soNameLen = strnlen(kernelSoName, AE_MAX_SO_NAME + 1);
    if (soNameLen == 0) {
        if (GetApiGlobal(opFuncPtr, kernelName) != AE_STATUS_SUCCESS) {
            AE_ERR_LOG(
                AE_MODULE_ID, "Get KFC api by global failed, kernelSoName[%s], soNameLen[%u], kernelName[%s].",
    ...
    if (apiIter != apiCacher_.end()) {
        ...
        if (apiIter != apiCacher_.end()) {
            ...
            if (GetApiBySoname(kernelType, kernelSoName, opFuncPtr, kernelName) != AE_STATUS_SUCCESS) {
                AE_RW_LOCK_UN_LOCK(&rwLock_);
                AE_ERR_LOG(
                    AE_MODULE_ID, "Get KFC kernel api failed, kernelSoName[%s], kernelName[%s].", kernelSoName,
                    kernelName);
                return static_cast<int32_t>(AE_STATUS_INNER_ERROR);
            }
...
int32_t AIKernelsLibAiCpuKFC::TransformKernelErrorCode(const uint32_t errCode, const char_t* const kernelName) const
{
