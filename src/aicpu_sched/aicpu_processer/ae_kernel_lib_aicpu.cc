...
aeStatus_t AIKernelsLibAiCpu::GetKernelNameAndKernelSoName(
...
int32_t AIKernelsLibAiCpu::CallKernelApi(const aicpu::KernelType kernelType, const void* const kernelBase)
{
    ...
    if (funcAddr == nullptr) {
        AE_ERR_LOG(AE_MODULE_ID, "Get %s api from %s success, but func is nullptr", &kernelName[0], &kernelSoName[0]);
        return AE_STATUS_INNER_ERROR;
    }
...
uint32_t AIKernelsLibAiCpu::RunAicpuFunc(
    const void* const kernelBase, void* const funcAddr, const char_t* const funcName) const
{
    uint32_t result = 0U;
...
aeStatus_t AIKernelsLibAiCpu::BatchLoadKernelSo(const aicpu::KernelType kernelType, std::vector<std::string>& soVec)
{
    if (soVec.empty()) {
...
aeStatus_t AIKernelsLibAiCpu::TransformKernelErrorCode(
