...
bool ArgsParser::ParseProfilingMode(const std::string& para)
{
    ...
    if ((val != static_cast<uint32_t>(PROFILING_CLOSE)) && (val != static_cast<uint32_t>(PROFILING_OPEN))) {
        return false;
    }
...
std::string ArgsParser::GetParaParsedStr()
{
    ...
    oss << "deviceId=" << deviceId_ << ", hostPid=" << hostPid_ << ", pidSign=" << pidSign_
        ...
        << ", deviceMode=" << deviceMode_ << ", hostProcName=" << hostProcName_ << ", grpNameNum=" << grpNameNum_
        << ", aicpuProcNum=" << aicpuProcNum_ << ", grpNameList=[";
