...
bool TsAicpuSqeAdapter::IsAdapterInvalidParameter() const { return invalidSqe_; }
...
{
    const bool isOutOfRange = (info.proto_info_task_id > INVALID_VALUE16) ||
...
void TsAicpuSqeAdapter::GetAicpuDataDumpInfoLoad(AicpuDataDumpInfoLoad& info)
{
    ...
    aicpusd_info(
        "Dump load request: address=%p length=%u task=%u stream=%u.", tmpinfo.dumpinfoPtr, tmpinfo.length,
        tmpinfo.task_id, tmpinfo.stream_id);
...
int32_t TsAicpuSqeAdapter::AicpuDumpResponseToTs(const int32_t result)
{
    ...
    return ResponseToTs(
        aicpuSqe, handleId, AicpuDrvManager::GetInstance().GetDeviceId(), static_cast<uint32_t>(aicpuSqe.ts_id));
}
...
int32_t TsAicpuSqeAdapter::AicpuDataDumpLoadResponseToTs(const int32_t result)
{
    ...
    aicpusd_info(
        ...
        aicpuSqe.u.aicpu_dump_resp.stream_id, aicpuSqe.u.aicpu_dump_resp.task_id);
    return ResponseToTs(
        aicpuSqe, 0U, AicpuDrvManager::GetInstance().GetDeviceId(), static_cast<uint32_t>(aicpuSqe.ts_id));
}
...
void TsAicpuSqeAdapter::GetAicpuModelOperateInfo(AicpuModelOperateInfo& info)
{
    ...
    aicpusd_info(
        "Model operation request: argument=%p operation=%u model=%u task=%u.", tmpInfo.arg_ptr, tmpInfo.cmd_type,
        tmpInfo.model_id, tmpInfo.task_id);
...
void TsAicpuSqeAdapter::AicpuActiveStreamSetMsg(ActiveStreamInfo& info)
{
    ...
    aicpusd_info(
        "Active stream update: message_type=%u stream=%u stamp=%u.", aicpuSqe.cmd_type,
...
int32_t TsAicpuSqeAdapter::AicpuMsgVersionResponseToTs(const int32_t result)
{
    ...
    aicpusd_info(
        "Version response: message_type=%u result=%u stream=%u task=%u.", msgInfo.cmd_type,
        msgInfo.u.aicpu_resp.result_code, msgInfo.u.aicpu_resp.stream_id, msgInfo.u.aicpu_resp.task_id);
    return ResponseToTs(
        msgInfo, 0U, AicpuDrvManager::GetInstance().GetDeviceId(), static_cast<uint32_t>(msgInfo.ts_id));
}
...
void TsAicpuSqeAdapter::GetAicpuTaskReportInfo(AicpuTaskReportInfo& info)
{
    ...
    aicpusd_info(
        "Task report request: task=%u model=%u result=%u stream=%u", tmpInfo.task_id, tmpInfo.model_id,
        tmpInfo.result_code, tmpInfo.stream_id);
    info.task_id = tmpInfo.task_id;
    info.model_id = tmpInfo.model_id;
    info.stream_id = tmpInfo.stream_id;
...
int32_t TsAicpuSqeAdapter::ErrorMsgResponseToTs(ErrMsgRspInfo& rspInfo)
{
    ...
    aicpusd_info(
        "Error response: message_type=%u result=%u response_stream=%u response_task=%u", aicpuSqe.cmd_type,
        rspInfo.err_code, rspInfo.stream_id, rspInfo.task_id);
    return ResponseToTs(
        aicpuSqe, rspInfo.model_id, AicpuDrvManager::GetInstance().GetDeviceId(),
        static_cast<uint32_t>(aicpuSqe.ts_id));
}
...
void TsAicpuSqeAdapter::GetAicpuInfoLoad(AicpuInfoLoad& info)
{
    ...
    aicpusd_info(
        "Info load request: address=%p length=%u stream=%u task=%u", tmpinfo.aicpuInfoPtr, tmpinfo.length,
...
int32_t TsAicpuSqeAdapter::AicpuInfoLoadResponseToTs(const int32_t result)
{
    ...
    aicpusd_info(
        "Info load response: message_type=%u result=%u stream=%u task=%u.", aicpuSqe.cmd_type, result,
        tmpInfo.stream_id, tmpInfo.task_id);
    return ResponseToTs(
        aicpuSqe, 0U, AicpuDrvManager::GetInstance().GetDeviceId(), static_cast<uint32_t>(aicpuSqe.ts_id));
}
...
int32_t TsAicpuSqeAdapter::AicpuRecordResponseToTs(AicpuRecordInfo& info)
{
    ...
    aicpusd_info(
        "Record response: pid=%u message_type=%u vf=%u tid=%u ts=%u record_type=%u "
    ...
    int32_t ret = halTsDevRecord(
        info.dev_id, static_cast<uint32_t>(info.ts_id), static_cast<uint32_t>(info.record_type), info.record_id);
