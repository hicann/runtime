...
bool TsAicpuMsgInfoAdapter::IsAdapterInvalidParameter() const { return invalidMsgInfo_; }
...
int32_t TsAicpuMsgInfoAdapter::AicpuDumpResponseToTs(const int32_t result)
{
    ...
    aicpusd_info(
        "Dump response: message_type=%u result=%u task=%u stream=%u", cmdType_, msgInfo.u.aicpu_resp.result_code,
        msgInfo.u.aicpu_resp.task_id, msgInfo.u.aicpu_resp.stream_id);
    return ResponseToTs(
...
int32_t TsAicpuMsgInfoAdapter::AicpuDataDumpLoadResponseToTs(const int32_t result)
{
    ...
    aicpusd_info(
        "Dump load response: message_type=%u result=%u stream=%u task=%u.", msgInfo.cmd_type, result,
...
void TsAicpuMsgInfoAdapter::GetAicpuModelOperateInfo(AicpuModelOperateInfo& info)
{
    ...
    aicpusd_info(
        "Model operation request: argument=%p operation=%u model=%u.", tmpInfo.arg_ptr, tmpInfo.cmd_type,
        tmpInfo.model_id);
...
int32_t TsAicpuMsgInfoAdapter::AicpuModelOperateResponseToTs(const int32_t result, const uint32_t subEvent)
{
    ...
    aicpusd_info(
        "Model operation response: message_type=%u result=%u stream=%u task=%u", cmdType_, result,
...
void TsAicpuMsgInfoAdapter::AicpuActiveStreamSetMsg(ActiveStreamInfo& info)
{
    ...
    aicpusd_info(
        "Active stream update: message_type=%u stream=%u stamp=%u.", msgInfo.cmd_type,
...
int32_t TsAicpuMsgInfoAdapter::ErrorMsgResponseToTs(ErrMsgRspInfo& rspInfo)
{
    ...
    aicpusd_info(
        "Error response: message_type=%u result=%u response_stream=%u response_task=%u", msgInfo.cmd_type,
        rspInfo.err_code, rspInfo.stream_id, rspInfo.task_id);
    return ResponseToTs(
        msgInfo, rspInfo.model_id, AicpuDrvManager::GetInstance().GetDeviceId(), static_cast<uint32_t>(msgInfo.ts_id));
}
...
int32_t TsAicpuMsgInfoAdapter::AicpuTimeOutConfigResponseToTs(const int32_t result)
{
    ...
    aicpusd_info(
        "Timeout config response: message_type=%u result=%u stream=%u task=%u.", msgInfo.cmd_type,
...
void TsAicpuMsgInfoAdapter::GetAicpuInfoLoad(AicpuInfoLoad& info)
{
    ...
    aicpusd_info(
        "Info load request: address=%p length=%u stream=%u task=%u", tmpinfo.aicpu_info_ptr, tmpinfo.length,
...
int32_t TsAicpuMsgInfoAdapter::AicpuInfoLoadResponseToTs(const int32_t result)
{
    ...
    aicpusd_info(
        "Info load response: message_type=%u result=%u stream=%u task=%u.", msgInfo.cmd_type, result, tmpInfo.stream_id,
        tmpInfo.task_id);
    return ResponseToTs(
        msgInfo, 0U, AicpuDrvManager::GetInstance().GetDeviceId(), static_cast<uint32_t>(msgInfo.ts_id));
}
