...
{
    const AicpuPlat mode = (runMode_ == aicpu::AicpuRunMode::PROCESS_PCIE_MODE) ? AicpuPlat::AICPU_ONLINE_PLAT :
                                                                                  AicpuPlat::AICPU_OFFLINE_PLAT;
    int32_t ret = 0;
    ret = AicpuDrvManager::GetInstance().BindHostPid(pidSign, static_cast<int32_t>(mode), vfId, DEVDRV_PROCESS_CP1);
    if (runMode_ == aicpu::AicpuRunMode::THREAD_MODE) {
        int32_t getRet = 0;
        getRet =
            AicpuDrvManager::GetInstance().BindHostPid(pidSign, static_cast<int32_t>(mode), vfId, DEVDRV_PROCESS_CP2);
