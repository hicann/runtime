...
int32_t AicpuMc2MaintenanceThread::InitMc2MaintenanceProcess(
...
int32_t StartMC2MaintenanceThread(
    ...
    void* paramStopFun)
{
    int32_t ret = AicpuSchedule::AicpuMc2MaintenanceThread::GetInstance(THREAD_TYPE_HCOM)
                      .InitMc2MaintenanceProcess(loopFun, paramLoopFun, stopNotifyFun, paramStopFun);
    if (ret != AicpuSchedule::AICPU_SCHEDULE_OK) {
        return ret;
    }
    return AicpuSchedule::AICPU_SCHEDULE_OK;
    ...
    void* paramStopFun)
{
    ...
    int32_t ret = AicpuSchedule::AicpuMc2MaintenanceThread::GetInstance(type).InitMc2MaintenanceProcess(
