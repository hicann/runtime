...
// permissions to 640
constexpr mode_t MODE_640 =
    static_cast<mode_t>(READ_OWNER) | static_cast<mode_t>(WRITE_OWNER) | static_cast<mode_t>(READ_GROUP);
// permissions to 750
constexpr mode_t MODE_750 = static_cast<mode_t>(READ_OWNER) | static_cast<mode_t>(WRITE_OWNER) |
                            static_cast<mode_t>(EXEC_OWNER) | static_cast<mode_t>(READ_GROUP) |
...
int32_t AicpuCustSoManager::InitAicpuCustSoManager(const aicpu::AicpuRunMode mode, const AicpuSchedMode schedMode)
{
    runMode_ = mode;
    ...
    aicpusd_run_info(
        "Cust so manager init successfully, dir=%s, runMode=%u, schedMode=%u.", custSoDirName_.c_str(),
        static_cast<uint32_t>(runMode_), static_cast<uint32_t>(schedMode));
...
int32_t AicpuCustSoManager::CheckAndCreateSoFile(const LoadOpFromBufArgs* const args, std::string& soName)
{
    ...
    if ((soNameLen == 0U) || (soNameLen > MAX_CUST_SO_NAME_LEN)) {
        aicpusd_err(
            "BatchLoadOpFromBuf kernel input param soNameLen=%u not in (0, %u].", soNameLen, MAX_CUST_SO_NAME_LEN);
        return AICPU_SCHEDULE_ERROR_PARAMETER_NOT_VALID;
    }
...
int32_t AicpuCustSoManager::CheckAndDeleteSoFile(const LoadOpFromBufArgs* const args)
{
    ...
    const uint32_t soNameLen = static_cast<uint32_t>(args->kernelSoNameLen);
    if ((soNameLen == 0U) || (soNameLen > MAX_CUST_SO_NAME_LEN)) {
        aicpusd_err("LoadOpFromBuf kernel input param soNameLen=%u not in (0, %u].", soNameLen, MAX_CUST_SO_NAME_LEN);
        return AICPU_SCHEDULE_ERROR_PARAMETER_NOT_VALID;
    }
    // check so name
    ...
    if (access(fileName.c_str(), F_OK) != 0) {
        aicpusd_run_warn("Check deleteCustOp's so full path:%s failed, error:%s", fileName.c_str(), strerror(errno));
        return AICPU_SCHEDULE_OK;
    }
    // check so file
...
int32_t AicpuCustSoManager::WriteBufToSoFile(const FileInfo& fileInfo, const std::string& dirPath) const
{
    ...
    try {
        ...
        if (!soFile.good()) {
            aicpusd_err(
                "Fail to write file:%s, length:%u, eof=%d, bad=%d, fail=%d, reason=%s. please check host so size",
        ...
        if (!soFile.good()) {
            aicpusd_err(
                "Fail to flush file:%s, length:%u, eof=%d, bad=%d, fail=%d, reason=%s. please check host so size",
                soName.c_str(), fileInfo.size, soFile.eof(), soFile.bad(), soFile.fail(), strerror(errno));
            return AICPU_SCHEDULE_ERROR_INNER_ERROR;
        }
    ...
    aicpusd_run_info(
        "Write buffer to so success, path=%s, hash=%llu, fileSize=%zu", soName.c_str(), hashValue, fileInfo.size);
    return AICPU_SCHEDULE_OK;
}
...
int32_t AicpuCustSoManager::CreateSoftLinkToSoFile(const std::string& softLinkPath, const std::string& existedSoPath)
{
    ...
    if (access(softLinkPath.c_str(), F_OK) == 0) {
        aicpusd_info(
            "The soft link has existed in device, cannot create again. linkPath=%s, targetPath=%s",
    ...
    if (ret != AICPU_SCHEDULE_OK) {
        aicpusd_err("Invalid soft link path, linkPath=%s, targetPath=%s", softLinkPath.c_str(), existedSoPath.c_str());
        return ret;
    }
    ...
    if (ret != 0) {
        aicpusd_err(
            "Create soft links failed, ret=%d, linkPath=%s, targetPath=%s, reason=%s", ret, softLinkPath.c_str(),
            existedSoPath.c_str(), strerror(errno));
        return AICPU_SCHEDULE_ERROR_INNER_ERROR;
    }

    aicpusd_run_info(
        "Create soft link so success, linkPath=%s, targetPath=%s", softLinkPath.c_str(), existedSoPath.c_str());
    return AICPU_SCHEDULE_OK;
}
...
std::string AicpuCustSoManager::GetPathForCustAicpuRealSo() const { return custSoRootPath_ + CP_CUSTOM_SO_LIB_NAME; }
...
std::string AicpuCustSoManager::GetPathForCustAicpuSoftLink() const
{
    ...
    (void)dirName.append(CP_CUSTOM_SO_DIR_NAME_PREFIX)
        ...
        .append("_")
        .append(std::to_string(static_cast<int32_t>(hostPid)))
        .append("/");
    return dirName;
}
...
int32_t AicpuCustSoManager::DeleteCustSoDir() const
{
    ...
    if (access(custSoDirName_.c_str(), F_OK) != 0) {
        aicpusd_run_info(
            "Accessing cust so dir %s was not successful, reason is %s.", custSoDirName_.c_str(), strerror(errno));
        return AICPU_SCHEDULE_OK;
    }
...
std::string AicpuCustSoManager::GenerateSoFullPath(const std::string& soName) const
{
    ...
    const uint32_t uniqueVfId = AicpuDrvManager::GetInstance().GetUniqueVfId();
    const uint32_t deviceId = AicpuDrvManager::GetInstance().GetDeviceId();
    return GetPathForCustAicpuRealSo() + soName + "." + std::to_string(deviceId) + "." + std::to_string(uniqueVfId) +
...
int32_t HashCalculator::GenerateFileHashInfo(const std::string& filePath, FileHashInfo& hashInfo) const
{
    if (!IsFileExist(filePath) || !IsRegularFile(filePath)) {
        aicpusd_err(
            "The file is not a regular file or not exist, filePath=%s, exist=%d", filePath.c_str(),
            static_cast<int32_t>(IsFileExist(filePath)));
        return AICPU_SCHEDULE_ERROR_INNER_ERROR;
    }
...
void HashCalculator::AddHashInfoToCache(const FileHashInfo& hashInfo)
{
    ...
    aicpusd_info(
        "Add file %s to cache, size=%lu, hash=%lu", hashInfo.filePath.c_str(), hashInfo.fileSize, hashInfo.hashValue);
}
