...
const std::string OPERATE_VALUE[static_cast<int32_t>(AicpuSchedule::AicpuModelOperate::MODEL_OPERATE_MAX)] = {
    "LOAD", "EXECUTE", "ABORT", "TASK_REPORT", "END_GRAPH", "RUN_TASK", "DESTROY", "STOP", "RESTART", "CLEAR_INPUT"};

// model status values
const std::string STATUS_VALUE[static_cast<int32_t>(AicpuSchedule::AicpuModelStatus::MODEL_STATUS_MAX)] = {
    "UNINIT", "IDLE", "LOADING", "RUNNING", "ERROR", "ABORT", "STOPPED"};
...
const bool AicpuModel::modelOperatePermission[static_cast<int32_t>(
    AicpuModelStatus::MODEL_STATUS_MAX)][static_cast<int32_t>(AicpuModelOperate::MODEL_OPERATE_MAX)] = {
...
void AicpuStream::InitAicpuStream(const uint32_t streamId, const std::vector<const AicpuTaskInfo*>& tasks)
{
    ...
    nextTaskIndex_ = 0LU;
    (void)std::transform(
        tasks.begin(), tasks.end(), std::back_inserter(tasks_),
...
int32_t AicpuStream::ExecuteNextTask(const RunContext& runContext, bool& streamEnd)
{
    ...
    aicpusd_info(
        "Model[%u] stream[%u] begin to execute %zuth task, taskId[%u], kernelType[%u].", runContext.modelId, streamId_,
        nextTaskIndex_, taskInfo.taskID, taskInfo.kernelType);
    *(const_cast<int32_t*>(&runContext.gotoTaskIndex)) = INVALID_TASK_INDEX;
    ...
    if (ret != AICPU_SCHEDULE_OK) {
        ...
        aicpusd_err(
            "Model[%u] stream[%u] execute %zuth task failed, taskId[%u], ret[%d], transId[%lu].", runContext.modelId,
            streamId_, nextTaskIndex_, taskInfo.taskID, ret, transId);
        nextTaskIndex_++;
        return ret;
    }
    if (runContext.pending) {
        aicpusd_info(
            "Model[%u] stream[%u] pending on %zuth task, taskId[%u].", runContext.modelId, streamId_, nextTaskIndex_,
            taskInfo.taskID);
    } else if (runContext.gotoTaskIndex != INVALID_TASK_INDEX) {
        nextTaskIndex_ = static_cast<size_t>(runContext.gotoTaskIndex);
        aicpusd_info(
            "Model[%u] stream[%u] goto %zuth task, taskId[%u].", runContext.modelId, streamId_, nextTaskIndex_,
            taskInfo.taskID);
    } else {
        aicpusd_info(
            "Model[%u] stream[%u] execute %zuth task success, taskId[%u].", runContext.modelId, streamId_,
            nextTaskIndex_, taskInfo.taskID);
        nextTaskIndex_++;
    }
...
int32_t AicpuStream::ExecuteTask(const AicpuTaskInfo& kernelTaskInfo, const RunContext& taskContext)
{
    ...
    if ((kernelTaskInfo.taskFlag & AICPU_STREAM_TASK_DUMP_MASK) != 0U) {
        ...
        if (opDumpTaskMgr.DumpOpInfo(dumpTaskInfo, dumpFileName) != AICPU_SCHEDULE_OK) {
            aicpusd_warn(
                "dump op info failed, modelId[%u], streamId[%u], taskID[%u].", taskContext.modelId,
...
int32_t AicpuStream::ConvertToTsKernel(const AicpuTaskInfo& kernelTaskInfo, aicpu::HwtsTsKernel& aicpufwKernelInfo)
{
...
int32_t AicpuStream::AttachReportStatusQueue()
{
    ...
    for (auto& taskInfo : tasks_) {
        ...
        if (std::string(kernelName) == REPORT_STATUS_TASK_NAME) {
            ...
            if (bufInfo == nullptr) {
                aicpusd_err(
                    "ModelReportStatus kernelTaskInfo paramBase is null, streamId[%u], taskId[%u]", taskInfo.streamID,
                    taskInfo.taskID);
                return AICPU_SCHEDULE_ERROR_PARAMETER_NOT_VALID;
            }
            ...
            if (drvRet != DRV_ERROR_NONE) {
                aicpusd_err("Aicpusd attach report status queue[%u] failed, ret[%d]", queueId, drvRet);
...
int32_t AicpuModel::ModelLoad(const AicpuModelInfo* const modelInfo, const ModelCfgInfo* const cfgInfo)
{
    ...
    if (ret != AICPU_SCHEDULE_OK) {
        ...
        if (destroyRet != AICPU_SCHEDULE_OK) {
            aicpusd_err(
                "Model[%u] load stream and task failed and rollback failed, rollback ret[%d].", modelId, destroyRet);
        }
        return ret;
    }
    ...
    if (ret != AICPU_SCHEDULE_OK) {
        ...
        const int32_t destroyRet = ModelDestroy();
        if (destroyRet != AICPU_SCHEDULE_OK) {
            aicpusd_err("Model[%u] load queue info failed and rollback failed, rollback ret[%d].", modelId, destroyRet);
        }
    ...
    if (ret != AICPU_SCHEDULE_OK) {
        ...
        if (destroyRet != AICPU_SCHEDULE_OK) {
            aicpusd_err(
                "Model[%u] attach report status queue failed and rollback failed, rollback ret[%d].", modelId,
                destroyRet);
        }
        aicpusd_info("Model[%u] rollback end.", modelId);
        return ret;
    ...
    aicpusd_info(
        "Model[%u] load success, abnormal break[%d], abnormal enqueue[%d], abnormal enabled[%d].", modelId,
        static_cast<int32_t>(abnormalBreak_), static_cast<int32_t>(abnormalEnqueue_),
...
void AicpuModel::ActiveOtherAicpuStreams()
{
    ...
    for (const auto aicpustreamId : otherAicpuStreams_) {
        ...
        const int32_t ret = AicpuMsgSend::SendAICPUSubEvent(
            ...
            AICPU_SUB_EVENT_ACTIVE_STREAM, CP_DEFAULT_GROUP_ID, syncSendFlag);
        if (ret != AICPU_SCHEDULE_OK) {
...
int32_t AicpuModel::ModelRestart()
{
    aicpusd_info("Model[%u] begin to restart", modelId_);
    {
        ...
        if (updateRet != AICPU_SCHEDULE_OK) {
            aicpusd_err(
                "Model[%u] restart failed, as CheckOperateAndUpdateStatus failed, ret[%d].", modelId_, updateRet);
            return updateRet;
        }
    ...
    for (const auto queueId : queueIds) {
        do {
            ...
            if ((ret != DRV_ERROR_NONE) || (mbuf == nullptr)) {
                aicpusd_err(
                    "Dequeue from queueId[%u], deviceId[%u] fail, ret is %d", queueId, deviceId,
...
int32_t AicpuModel::ExecuteStream(const uint32_t streamId, const bool executeInline)
{
    ...
    while (true) {
        ...
        if (executeInline) {
            ...
            if (switchStreamId != INVALID_NUMBER) {
                ...
                if (stream == nullptr) {
                    aicpusd_err(
                        "Model[%u] execute stream[%u] failed as stream[%u] not found.", modelId_, streamId,
                        currentStreamId);
    ...
    if (ret != AICPU_SCHEDULE_OK) {
        aicpusd_err(
            "Model[%u] execute stream[%u] failed currentStreamId[%u], ret[%d].", modelId_, streamId, currentStreamId,
            ret);
...
int32_t AicpuModel::CheckOperateAndUpdateStatus(const AicpuModelOperate operate)
{
    ...
    if ((dstStatus < AicpuModelStatus::MODEL_STATUS_MAX) && (dstStatus != modelStatus_)) {
        aicpusd_info(
            "Model[%u] status change from [%s] to [%s] as operate[%s].", modelId_,
            STATUS_VALUE[static_cast<int32_t>(modelStatus_)].c_str(),
            STATUS_VALUE[static_cast<int32_t>(dstStatus)].c_str(), OPERATE_VALUE[operateIndex].c_str());
...
int32_t AicpuModel::LoadStreamAndTask(const AicpuModelInfo* const modelInfo)
{
    ...
    for (uint32_t i = 0U; i < static_cast<uint32_t>(modelInfo->streamInfoNum); i++) {
        ...
        if ((streamInfo.streamFlag & AICPU_STREAM_INDEX) != 0U) {
            ...
            if (modelS0Stream_ != INVALID_NUMBER) {
                (void)pthread_rwlock_unlock(&rwlockForStream_);
                aicpusd_err(
                    "load model[%u] failed as stream[%u] and stream[%u] are both s0 stream.", modelId_, modelS0Stream_,
                    streamInfo.streamID);
                return AICPU_SCHEDULE_ERROR_PARAMETER_NOT_VALID;
            }
    ...
    for (size_t i = 0U; i < aicpuTaskNum; i++) {
        ...
        aicpusd_info(
            "Task: taskID[%u], streamId[%u], kernelType[%u], "
            "kernelName[%lu], kernelSo[%lu], taskFlag[%u].",
            taskPtr[i].taskID, taskPtr[i].streamID, taskPtr[i].kernelType, taskPtr[i].kernelName, taskPtr[i].kernelSo,
            taskPtr[i].taskFlag);
    }

    for (auto& streamTask : streamTasks) {
        aicpuStreams_[streamTask.first].InitAicpuStream(streamTask.first, streamTask.second);
        aicpusd_info("Model[%u] aicpu stream[%u] has %zu tasks.", modelId_, streamTask.first, streamTask.second.size());
    }
    (void)pthread_rwlock_unlock(&rwlockForStream_);
    return AICPU_SCHEDULE_OK;
}

void AicpuModel::LoadWaitNotifyId(const AicpuTaskInfo& aicpuTaskInfo, std::unordered_set<size_t>& waitNotifyIdSet) const
{
    ...
    if (notifyId == nullptr) {
        aicpusd_warn(
            "Stream[%u] taskId[%u] is waitNotify, but paraBase is null.", aicpuTaskInfo.streamID, aicpuTaskInfo.taskID);
        return;
    }
    (void)waitNotifyIdSet.emplace(*notifyId);
    aicpusd_info(
        "Model[%u] stream[%u] taskId[%u] use notifyId[%u]", modelId_, aicpuTaskInfo.streamID, aicpuTaskInfo.taskID,
        *notifyId);
}
...
int32_t AicpuModel::LoadQueueInfo(const AicpuModelInfo* const modelInfo)
{
    ...
    if (infoPtr == nullptr) {
        aicpusd_info(
            "Model[%u] load was not successful, as queueSize[%u] but queueInfoPtr is null.", modelId_,
            modelInfo->queueSize);
        return AICPU_SCHEDULE_ERROR_PARAMETER_NOT_VALID;
    }
    ...
    for (int32_t i = 0; i < static_cast<int32_t>(modelInfo->queueSize); i++) {
        ...
        if (infoPtr[i].flag == static_cast<uint32_t>(QueueDirectionFlag::QUEUE_INPUT_FLAG)) {
            ...
            if (drvRet != DRV_ERROR_NONE) {
                aicpusd_err(
                    "Aicpusd set work mode for queue[%u] failed ret[%d]", infoPtr[i].queueID,
...
StoreResult AicpuModel::StoreDequedMbuf(
    const uint64_t transId, const uint32_t routeLabel, const size_t qIndex, Mbuf* const mbuf, const uint32_t queueCount)
{
    ...
    if (gatheredMbuf_[transId][routeLabel].Init(queueCount) &&
        ...
        aicpusd_info(
            "Model[%u] Store mbuf[%u:%u] into [%u]th queue, queuesize[%llu].", modelId_, transId, routeLabel, qIndex,
            gatheredMbufCntList_[qIndex]);
...
void AicpuModel::ClearExceptionStore()
{
    ...
    for (const auto transId : transIdsToClear) {
        ...
        for (auto& routeLabelIter : transIdIter->second) {
            ...
            aicpusd_info("clear exception for transid[%lu], routelabel[%u]", transIdIter->first, routeLabelIter.first);
...
GatherResult AicpuModel::SelectGatheredMbuf(Mbuf*** const mbufPptr, const int32_t timeOut, const uint32_t cacheNum)
{
    ...
    if (ret != GatherResult::UN_SELECTED) {
        aicpusd_info(
            "[%u:%u] is selected in Model[%u], ret is %d", mbufKey.first, mbufKey.second, modelId_,
            static_cast<int32_t>(ret));
...
bool AicpuModel::IsTableLocked(const uint32_t tableId) { return tableLocked_.find(tableId) != tableLocked_.end(); }
...
uint32_t& AicpuModel::GetInputConsumeNumRef() { return inputConsumeNum_; }
...
size_t AicpuModel::GetCurDequeIndex(const size_t qCnt)
{
    ...
    for (size_t index = 0UL; index < qCnt; index++) {
        ...
        aicpusd_info(
            "update modelId:%u, qIndex:%zu, cnt:%llu, index:%zu, curCnt:%zu", modelId_, qIndex, minCnt, index, curCnt);
    }
    return qIndex;
}
...
int32_t AicpuModel::ProcessDataException(const uint64_t transId, const uint32_t type)
{
    ...
    if (type == static_cast<uint32_t>(ExceptionAction::ADD)) {
        ...
        const auto ret = AicpuMsgSend::SendAICPUSubEvent(
            PtrToPtr<AICPUSubEventInfo, const char_t>(&subEventInfo), static_cast<uint32_t>(sizeof(AICPUSubEventInfo)),
            AICPU_SUB_EVENT_SUPPLY_ENQUEUE, CP_DEFAULT_GROUP_ID, true);
        if (ret != AICPU_SCHEDULE_OK) {
    ...
    if (type == static_cast<uint32_t>(ExceptionAction::EXPIRE)) {
        if (exceptionTranses_.find(transId) == exceptionTranses_.end()) {
            aicpusd_warn("Transid[%lu] is not in exception list in model[%u], no need to expire.", transId, modelId_);
        } else {
    ...
    aicpusd_err(
        "Model[%u] failed to process data exception of transid[%lu] for type[%u] is invalid.", modelId_, transId, type);
    return AICPU_SCHEDULE_ERROR_PARAMETER_NOT_VALID;
}
...
bool QueueMbufStore::Store(const size_t qIndex, Mbuf* const mbuf, std::map<size_t, uint64_t>& gCntList)
{
    ...
    gCntList[qIndex]++;
    aicpusd_info(
        "store mbuf index:%zu, queue list size:%zu, buf cnt map cnt:%llu", qIndex, queuesLists_[qIndex].size(),
        gCntList[qIndex]);
    return true;
}
    ...
    for (size_t arrIndex = 0U; arrIndex < queuesLists_.size(); ++arrIndex) {
        if (!queuesLists_[arrIndex].empty()) {
            *mbufPptr[arrIndex] = queuesLists_[arrIndex].front();
            ...
            aicpusd_info(
                "consume mbuf index:%zu, queue list size:%zu, buf cnt map cnt:%llu", arrIndex,
...
QueueMbufStore::~QueueMbufStore() { Free(); }
