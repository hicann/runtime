...
int32_t OperatorKernelModelBatchDequeue::CheckAndParseBatchDequeueParams(
    const AicpuTaskInfo& kernelTaskInfo, const RunContext& taskContext, BatchDequeueInfo& batchDeqInfo) const
{
    const BatchDequeueDesc* const batchDeqDesc = PtrToPtr<void, BatchDequeueDesc>(ValueToPtr(kernelTaskInfo.paraBase));
    ...
    if (batchDeqDesc->inputNums != inputsIsDequeue.size()) {
        aicpusd_err(
            "KernelTaskInfo inputNums[%u] is not equal model input queue size[%zu],"
            "modelId[%u], streamId[%u], taskId[%u]",
            batchDeqDesc->inputNums, inputsIsDequeue.size(), taskContext.modelId, taskContext.streamId,
            kernelTaskInfo.taskID);
        return AICPU_SCHEDULE_ERROR_PARAMETER_NOT_VALID;
    }
