...
int32_t OperatorKernelModelBatchDequeueBuff::Compute(const AicpuTaskInfo& kernelTaskInfo, const RunContext& taskContext)
{
    ...
    if (batchDeqBufDesc == nullptr) {
        aicpusd_err(
            "KernelTaskInfo paramBase is null, modelId[%u], streamId[%u], taskId[%u].", taskContext.modelId,
    ...
    const RunContext& taskContext, BatchDequeueBuffInfo& batchDeqBufInfo) const
{
    ...
    if (batchDeqBufInfo.queueIds == nullptr) {
        aicpusd_err(
            "KernelTaskInfo queueIds is null, modelId[%u], streamId[%u], taskId[%u]", taskContext.modelId,
            taskContext.streamId, kernelTaskInfo.taskID);
        return AICPU_SCHEDULE_ERROR_PARAMETER_NOT_VALID;
    }
    ...
    if (batchDeqBufInfo.mbufAddrs == nullptr) {
        aicpusd_err(
            "KernelTaskInfo mbufAddrs is null, modelId[%u], streamId[%u], taskId[%u]", taskContext.modelId,
            taskContext.streamId, kernelTaskInfo.taskID);
        return AICPU_SCHEDULE_ERROR_PARAMETER_NOT_VALID;
    }
    ...
    if (batchDeqBufInfo.deviceIds == nullptr) {
        aicpusd_err(
            "KernelTaskInfo deviceIds is null, modelId[%u], streamId[%u], taskId[%u]", taskContext.modelId,
            taskContext.streamId, kernelTaskInfo.taskID);
        return AICPU_SCHEDULE_ERROR_PARAMETER_NOT_VALID;
    }
    return AICPU_SCHEDULE_OK;
...
int32_t OperatorKernelModelBatchDequeueBuff::ModelAttachAndDequeueBuff(
    BufEnQueueBuffInfo& queueInfo, const RunContext& taskContext, bool tryOnce) const
{
...
int32_t OperatorKernelModelBatchDequeueBuff::ModelDequeueBuffTaskKernel(
    BufEnQueueBuffInfo& bufInfo, const RunContext& taskContext, bool tryOnce) const
{
    ...
    if (model == nullptr) {
        aicpusd_err("Cannot get model by modelId:[%u], streamId[%u].", taskContext.modelId, taskContext.streamId);
        return AICPU_SCHEDULE_ERROR_PARAMETER_NOT_VALID;
    }
    ...
    if (deqRet != AICPU_SCHEDULE_OK) {
        aicpusd_warn(
            "DequeueBuff failed from queue[%u] in device[%u] failed, deqRet[%d], respLen[%u]", queueId, deviceId,
            static_cast<int32_t>(deqRet), respLen);
    }
    return deqRet;
}

int32_t OperatorKernelModelBatchDequeueBuff::DequeueBuff(
    ...
    const int32_t deviceId) const
{
    ...
    if (drvRet != DRV_ERROR_NONE) {
        aicpusd_err(
            "halQueueDeQueueBuff to queue[%u] in device[%d] failed, error[%d]", queueId, deviceId,
...
int32_t OperatorKernelModelBatchDequeueBuff::AlignBatchDequeueBuff(
    BatchDequeueBuffInfo& batchDeqBufInfo, const RunContext& taskContext) const
{
    BatchDequeueInfo batchDeqInfo = {
        batchDeqBufInfo.inputNums, batchDeqBufInfo.alignInterval, batchDeqBufInfo.alignOffsets,
        batchDeqBufInfo.queueIds, batchDeqBufInfo.mbufAddrs};
    while (true) {
        ...
        BufEnQueueBuffInfo queBuffInfo = {
            batchDeqBufInfo.queueIds[minTimestampIndex], batchDeqBufInfo.deviceIds[minTimestampIndex],
            batchDeqBufInfo.mbufAddrs[minTimestampIndex]};
