...
int32_t OperatorKernelModelPrepare::Compute(const AicpuTaskInfo& kernelTaskInfo, const RunContext& taskContext)
{
    ...
    if (prepareInfo == nullptr) {
        aicpusd_err(
            "ModelPrepare kernelTaskInfo paramBase is null, modelId[%u], streamId[%u], taskId[%u].",
    ...
    if (prepareInfo->aicpuPareInfoSize != sizeof(AicpuPrepareInfo)) {
        aicpusd_err(
            "Failed check AicpuPrepareInfo size. msgInfo.aicpuPareInfoSize is [%u], "
    ...
    if (!CheckPointListNullptr(
            PtrToPtr<void, uint64_t>(ValueToPtr(static_cast<uintptr_t>(prepareInfo->inputAddrList))),
            prepareInfo->inputAddrNum)) {
        aicpusd_err("inputAddrList has null pointers!");
        return AICPU_SCHEDULE_ERROR_PARAMETER_NOT_VALID;
    }
    if (!CheckPointListNullptr(
            PtrToPtr<void, uint64_t>(ValueToPtr(static_cast<uintptr_t>(prepareInfo->outputAddrList))),
    ...
    if ((prepareInfo->outQueueNum != ONLY_ONE_QUEUE) && (prepareInfo->outQueueNum != prepareInfo->outputMbufNum)) {
        aicpusd_err(
            "Failed check AicpuPrepareInfo, outQueueNum[%u] is not 1 or equal with outputMbufNum[%u].",
...
int32_t OperatorKernelModelPrepare::DequeueMbufList(
    const AicpuPrepareInfo& msgInfo, ModelPrepareData& prepareData, std::vector<void*>& inputsData,
...
int32_t OperatorKernelModelPrepare::CopyDequeueDataPtrToInputAddr(
    AicpuPrepareInfo& msgInfo, const std::vector<void*>& inputsData) const
{
    int32_t ret = AICPU_SCHEDULE_OK;
    const uint32_t* inputIndexList =
        PtrToPtr<void, uint32_t>(ValueToPtr(static_cast<uintptr_t>(msgInfo.inputIndexList)));
...
int32_t OperatorKernelModelPrepare::AllocOutputMbufList(
    AicpuPrepareInfo& msgInfo, Mbuf** lastInputMbuflistPptr, Mbuf* (&mbufPtrStore)[MAX_SIZE_NUM],
    const RunContext& taskContext) const
{
    int32_t ret = AICPU_SCHEDULE_OK;
    uint32_t* const outDataSizeList =
        PtrToPtr<void, uint32_t>(ValueToPtr(static_cast<uintptr_t>(msgInfo.outDataSizeList)));

    g_aicpuProfiler.SetPrepareOutStart();
    if (msgInfo.outQueueNum == ONLY_ONE_QUEUE) {
        ret = BufManager::GetInstance().MallocAndGuardBufList(
            outDataSizeList, msgInfo.outputMbufNum, taskContext.modelId, true, &mbufPtrStore[0]);
        if (ret != AICPU_SCHEDULE_OK) {
            return ret;
        }
    } else if (msgInfo.outputMbufNum == msgInfo.outQueueNum) {
        ret = BufManager::GetInstance().MallocAndGuardBufList(
            outDataSizeList, msgInfo.outputMbufNum, taskContext.modelId, false, &mbufPtrStore[0]);
        if (ret != AICPU_SCHEDULE_OK) {
            return ret;
        }
    } else {
        aicpusd_err(
            "error outputMbufNum. outputMbufNum:%u, outQueueNum:%u.", msgInfo.outputMbufNum, msgInfo.outQueueNum);
        return AICPU_SCHEDULE_ERROR_PARAMETER_NOT_VALID;
    }
...
int32_t OperatorKernelModelPrepare::GetDataPtrsFromMbufs(
    const AicpuPrepareInfo& msgInfo, Mbuf* (&mbufPtrStore)[MAX_SIZE_NUM], void* (&dataPtrStore)[MAX_SIZE_NUM]) const
{
    int32_t ret = AICPU_SCHEDULE_OK;
    void* dataPtr = nullptr;
    if (msgInfo.outQueueNum == ONLY_ONE_QUEUE) {
        ...
        aicpusd_err(
            "error GetDataPtrsFromMbufs. outputMbufNum:%u, outQueueNum:%u.", msgInfo.outputMbufNum,
            msgInfo.outQueueNum);
        return AICPU_SCHEDULE_ERROR_PARAMETER_NOT_VALID;
    }
    return AICPU_SCHEDULE_OK;
...
int32_t OperatorKernelModelPrepare::CopyOutputDataPtrToOutputAddr(
    AicpuPrepareInfo& msgInfo, void* const (&dataPtrStore)[MAX_SIZE_NUM]) const
{
    uint64_t* const outputAddrList =
        PtrToPtr<void, uint64_t>(ValueToPtr(static_cast<uintptr_t>(msgInfo.outputAddrList)));
    ...
    for (uint32_t addrIndex = 0U; addrIndex < msgInfo.outputAddrNum; addrIndex++) {
        if (outputIndexList[addrIndex] < msgInfo.outputMbufNum) {
            ...
            *(outputAddrPtr) =
            ...
            aicpusd_err(
                "Prepare output datas index out of range, index:[%u], outputIndexList[addrIndex]:[%u], "
...
int32_t OperatorKernelModelPrepare::BuildEnqueueMbufPtrList(
    AicpuPrepareInfo& msgInfo, Mbuf* (&mbufPtrStore)[MAX_SIZE_NUM]) const
{
    ...
    aicpusd_err(
        "BuildEnqueueMbufPtrList:error outputMbufNum. outputMbufNum:%u, outQueueNum:%u.", msgInfo.outputMbufNum,
        msgInfo.outQueueNum);
    return AICPU_SCHEDULE_ERROR_PARAMETER_NOT_VALID;
}

int32_t OperatorKernelModelPrepare::GetMbufListDataPtr(
    void* mbufPtr, void** dataAddrPtr, const uint32_t mbufIndex) const
{
