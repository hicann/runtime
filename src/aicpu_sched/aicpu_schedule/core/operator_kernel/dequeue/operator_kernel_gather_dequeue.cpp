...
{
    aicpusd_info(
        "Start ModelGatherDeque. modelId=%u, streamId=%u, taskId=%u", taskContext.modelId, kernelTaskInfo.streamID,
    ...
    while (!DequeAndCheckIfReady(batchDeqInfo, gatherRet, model, tempContext, blockOnClientQ)) {
        bool needWait = false;
        aicpusd_info(
            "Batch queue is not gathered, modelId[%u], streamId[%u], taskId[%u].", taskContext.modelId,
            taskContext.streamId, kernelTaskInfo.taskID);
        EventWaitManager::AnyQueNotEmptyWaitManager().WaitEvent(
        ...
        if (needWait) {
            ...
            *pending = true;
            aicpusd_info(
                "ModelGatherDeque pending, modelId[%u], streamId[%u], taskId[%u].", taskContext.modelId,
                taskContext.streamId, kernelTaskInfo.taskID);
...
bool OperatorKernelGatherDequeue::SelectMbuf(
...
{
    Mbuf* stubMbuf =
        BufManager::GetInstance().MallocAndGuardBufU64(static_cast<uint64_t>(sizeof(RuntimeTensorDesc)), modelId);
    if (stubMbuf == nullptr) {
...
bool OperatorKernelGatherDequeue::DequeAndCheckIfReady(
    const GatherDequeParam* const batchDeqInfo, int32_t& gatherRet, void* const modelPtr, const RunContext& taskContext,
...
bool OperatorKernelGatherDequeue::StoreMbufIntoModel(
    Mbuf* const mbuf, const size_t index, const uint32_t capacity, void* const modelPtr) const
{
    AicpuModel* const model = PtrToPtr<void, AicpuModel>(modelPtr);
