...
int32_t PrepareDynamicInputOutputBase::PrepareDynamicInputOutput(
    const AicpuTaskInfo& kernelTaskInfo, const RunContext& taskContext, const bool hostAllocDynamicOutput) const
{
    aicpusd_info(
        "Start ModelPrepareDynamicInputOutput. modelId=%u, streamId=%u, taskId=%u.", taskContext.modelId,
    ...
    if (((param->inputsNum != 0U) && ((param->inputDynamicFlagsAddr == 0U) || (param->inputMbufAddrsAddr == 0U))) ||
        ...
        aicpusd_err(
            "input or output invalid, input: {inputNums[%u], inputDynamicFlagsAddr[%u], inputMbufAddr[%u]}, "
            "output: {outputNums[%u], outputTensorSizeAddr[%u], outputMbufAddr[%u]}, reqMsgMbufAddr[%u].",
            param->inputsNum, param->inputDynamicFlagsAddr, param->inputMbufAddrsAddr, param->outputsNum,
            param->outputTensorSizesAddr, param->outputMbufAddrsAddr, param->reqMsgMbufAddr);
        return AICPU_SCHEDULE_ERROR_PARAMETER_NOT_VALID;
    }
...
int32_t PrepareDynamicInputOutputBase::AllocateAndInitOutput(
    ...
    std::vector<Mbuf*>& mbufsToFree, const bool hostAllocDynamicOutput) const
{
    ...
    for (size_t outputIndex = 0U; outputIndex < static_cast<size_t>(param->outputsNum); ++outputIndex) {
        if ((outputTensorSizes[outputIndex] < 0) || (outputPptrs[outputIndex] == 0U)) {
            aicpusd_err(
                "Invalid outputTensorSizes[%zu]:%ld, or invalid pptr.", outputIndex, outputTensorSizes[outputIndex]);
            return AICPU_SCHEDULE_ERROR_PARAMETER_NOT_VALID;
        }
...
int32_t PrepareDynamicInputOutputBase::PrepareReqMsg(
    const PrepareDynamicInputOutputKernelArgs* const param, const RunContext& taskContext,
    std::vector<Mbuf*>& mbufsToFree, const bool hostAllocDynamicOutput) const
{
    // CalculateReqMsgSize
    ...
    for (size_t i = 0U; i < static_cast<size_t>(param->inputsNum); ++i) {
        ...
        if ((srcFusionOffsets != nullptr) && (srcFusionOffsets[i] > 0)) {
            ...
            const auto ret = OperatorKernelCommon::UpdateDataPtr(
                PtrToValue(&inputMbuf), static_cast<int32_t>(srcFusionOffsets[i]), dataPtr, totalOffset);
            if (ret != AICPU_SCHEDULE_OK) {
...
int32_t PrepareDynamicInputOutputBase::UpdateReqMsgHead(
    Mbuf* const reqMbuf, const uint64_t* const inputPptrs, const uint32_t inputNum, const RunContext& taskContext) const
{
    ...
    if ((ret != DRV_ERROR_NONE) || (headBuf == nullptr) || (static_cast<size_t>(headSize) < sizeof(MbufHeadMsg))) {
        aicpusd_err(
            "Skip %s. modelId=%u, headSize=%u, baseSize=%lu", __func__, taskContext.modelId, headSize,
            sizeof(MbufHeadMsg));
    ...
    if (isEndofSequence) {
        ...
        uint8_t* const res = PtrAdd<uint8_t>(
...
void PrepareDynamicInputOutputBase::ExtractHeadInfo(
    Mbuf* const mbuf, int32_t& retCode, bool& nullDataFlag, bool& isEndofSequence) const
{
    void* customBuf = nullptr;
    uint32_t customBufSize = 0U;
    ...
    MbufHeadMsg* const inputMsg = PtrToPtr<uint8_t, MbufHeadMsg>(PtrAdd<uint8_t>(
        ...
        static_cast<size_t>(customBufSize) - sizeof(MbufHeadMsg)));
    ...
    if (!isEndofSequence) {
        const uint8_t* const endOfSequence = PtrAdd<uint8_t>(
            PtrToPtr<void, uint8_t>(customBuf), MBUF_HEAD_MAX_SIZE, static_cast<size_t>(MBUF_HEAD_END_OF_SEQUENCE_POS));
...
int32_t OperatorKernelPrepareDynamicInputOutput::Compute(
...
int32_t OperatorKernelPrepareDynamicInputOutputV2::Compute(
    const AicpuTaskInfo& kernelTaskInfo, const RunContext& taskContext)
{
