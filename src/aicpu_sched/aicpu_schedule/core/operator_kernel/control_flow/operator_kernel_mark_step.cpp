...
int32_t OperatorKernelMarkStep::Compute(const AicpuTaskInfo& kernelTaskInfo, const RunContext& taskContext)
{
    ...
    if (CheckMarkStepPara(bufInfo) != AICPU_SCHEDULE_OK) {
        aicpusd_err(
            "MarkStep para check failed, modelId[%u], streamId[%u], taskId[%u]", taskContext.modelId,
    ...
    const uint64_t stepId =
        (bufInfo->groupIndex == 0U) ? tempCnt + static_cast<uint64_t>(bufInfo->groupTotalCount - 1U) : tempCnt - 1UL;
    aicpusd_info(
        "[MarkStep] headFlag[true], iteratorCount[%lu], totalCnt[%u], groupIndex[%u], stepId:[%lu].", iteratorCount,
        bufInfo->groupTotalCount, bufInfo->groupIndex, stepId);
    *stepIdAddr = stepId;
