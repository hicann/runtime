...
enum class OutputType { STATIC_OUTPUT = 0, DYNAMIC_OUTPUT_WITH_MAXSIZE = 1, DYNAMIC_OUTPUT_WITHOUT_MAXSIZE = 2 };
} // namespace

int32_t OperatorKernelPostProcessDynamicOutput::Compute(
    const AicpuTaskInfo& kernelTaskInfo, const RunContext& taskContext)
{
    aicpusd_info(
        "Start ModelPostprocessDynamicOutput. modelId=%u, streamId=%u, taskId=%u.", taskContext.modelId,
    ...
    if ((param->outputsNum > 0U) && ((param->outputDynamicFlagsAddr == 0U) || (param->outputMbufAddrsAddr == 0U))) {
        aicpusd_err(
            "Parameter invalid: outputsNum[%u], outputDynamicFlagsAddr[%u], outputMbufAddrsAddr[%u]", param->outputsNum,
            param->outputDynamicFlagsAddr, param->outputMbufAddrsAddr);
        return AICPU_SCHEDULE_ERROR_PARAMETER_NOT_VALID;
    }
...
int32_t OperatorKernelPostProcessDynamicOutput::CopyTensorDesc(
    const PostprocessDynamicOutputKernelArgs* const param, const RunContext& taskContext) const
{
    ...
    for (size_t index = 0U; index < param->outputsNum; ++index) {
        ...
        if ((dynamicFlags[index] == static_cast<uint32_t>(OutputType::STATIC_OUTPUT)) ||
            ...
            const auto ret =
                AllocatedOutput(param, outputMbuf, dynamicFlags[index], index, &dynamicSrcDesc, &staticSrcDesc);
        ...
        if (dynamicFlags[index] == static_cast<uint32_t>(OutputType::DYNAMIC_OUTPUT_WITHOUT_MAXSIZE)) {
            const auto getHeadRet = GetMbufHeadFromResp(
                reinterpret_cast<Mbuf**>(param->respMsgMbufAddr), taskContext, &customBuf, &customBufSize);
            if (getHeadRet != AICPU_SCHEDULE_OK) {
                return getHeadRet;
            }
            const auto ret = PostProcessForOutputToAllocate(
                param, reinterpret_cast<Mbuf**>(outputPptrs[index]), index, &dynamicSrcDesc, customBuf, customBufSize,
                taskContext);
            if (ret != AICPU_SCHEDULE_OK) {
                return ret;
            }
...
int32_t OperatorKernelPostProcessDynamicOutput::AllocatedOutput(
    const PostprocessDynamicOutputKernelArgs* const param, Mbuf* const outputMbuf, const uint32_t dynamicFlag,
    const size_t index, RuntimeTensorDesc** dynamicSrcDescPptr, RuntimeTensorDesc** staticSrcDescPptr) const
{
...
int32_t OperatorKernelPostProcessDynamicOutput::GetRuntimeTensor(
    const PostprocessDynamicOutputKernelArgs* const param, RuntimeTensorDesc** dynamicSrcDescPtr) const
{
...
int32_t OperatorKernelPostProcessDynamicOutput::PostprocessSetDataLen(
    const RuntimeTensorDesc* const runtimeTensorDesc, Mbuf* const outputMbuf, const size_t index) const
{
    ...
    if (setLenRet != static_cast<int32_t>(DRV_ERROR_NONE)) {
        ...
        if ((getSizeRet != static_cast<int32_t>(DRV_ERROR_NONE)) || (outputMbufLen < sizeof(RuntimeTensorDesc))) {
            aicpusd_err(
                "Fail to get buff size for [%zu]th mbuf, ret=[%d], outputMbufLen[%lu]", index, getSizeRet,
                outputMbufLen);
        }
        ...
        aicpusd_err(
            "set [%zu]th mbuf's datalen to %lu fail, mbuf's len is %lu, dataSize is %lu, ret is %d", index, mbufLen,
            outputMbufLen, runtimeTensorDesc->dataSize, setLenRet);
        return AICPU_SCHEDULE_ERROR_FROM_DRV;
    }
...
int32_t OperatorKernelPostProcessDynamicOutput::GetMbufHeadFromResp(
    Mbuf** const respMbufPtr, const RunContext& taskContext, void** const customBufPtr,
...
int32_t OperatorKernelPostProcessDynamicOutput::MakeupDynamicOutputMbuf(
    Mbuf** const outputMbufPtr, const size_t index, const RuntimeTensorDesc* const runtimeTensorDesc,
    void* const customBuf, const uint32_t customBufSize, const RunContext& taskContext) const
{
    const uint64_t allocSize = runtimeTensorDesc->dataSize + static_cast<uint64_t>(sizeof(RuntimeTensorDesc));
    Mbuf* mbuf = BufManager::GetInstance().MallocAndGuardBufU64(static_cast<uint64_t>(allocSize), taskContext.modelId);
    if (mbuf == nullptr) {
    ...
    cpyRet = OptimizedMemCopy(
        dataPtr, static_cast<size_t>(runtimeTensorDesc->dataSize), ValueToPtr(runtimeTensorDesc->dataAddr),
        static_cast<size_t>(runtimeTensorDesc->dataSize));
    if (!cpyRet) {
...
bool OperatorKernelPostProcessDynamicOutput::IsSupportSdmaCopy() const { return (&halSdmaCopy != nullptr); }

bool OperatorKernelPostProcessDynamicOutput::OptimizedMemCopy(
    void* const dst_data, const size_t dst_size, const void* const src_data, const size_t src_size) const
{
    if (IsSupportSdmaCopy()) {
        const auto ret = halSdmaCopy(
            reinterpret_cast<DVdeviceptr>(dst_data), dst_size, reinterpret_cast<DVdeviceptr>(src_data), src_size);
        if (ret != DRV_ERROR_NONE) {
            aicpusd_err(
                "Failed to call halSdmaCopy, driver api ret:%d, dst_size:%zu, src_size:%zu.", static_cast<int32_t>(ret),
                dst_size, src_size);
        }
...
int32_t OperatorKernelPostProcessDynamicOutput::FreeMbuf(
    const PostprocessDynamicOutputKernelArgs* const param, const RunContext& taskContext) const
{
    ...
    if ((param->inputsNum > 0U) && (param->inputMbufAddrsAddr == 0U)) {
        aicpusd_err("Invalid inputsNum[%u], inputMbufAddrsAddr[%u]", param->inputsNum, param->inputMbufAddrsAddr);
        return AICPU_SCHEDULE_ERROR_PARAMETER_NOT_VALID;
    }
