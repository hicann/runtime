...
public:
    ...
    inline uint64_t BirthDay() const { return birthTimeStamp_; }
...
enum class GatherResult : int32_t { UN_SELECTED = 0, SELECTED, FAKE_SELECTED };

enum class StoreResult : int32_t { SUCCESS_STORE = 0, FAIL_STORE, ABORT_STORE };

enum class ExceptionAction : uint32_t { ADD = 0, EXPIRE = 1 };
...
public:
    ...
    ~AicpuModel() { (void)pthread_rwlock_destroy(&rwlockForStream_); }
    ...
    uint32_t GetModelTsId() const { return modelTsId_; }

    void SetModelTransId(const uint64_t transId) { modelTransId_ = transId; }

    uint64_t GetModelTransId() const { return modelTransId_; }

    int32_t UnGardModelBuf(Mbuf* const mbuf);

    uint32_t GetReportStmId() const { return reportStream_; }

    bool IsValid() const { return isValid; }

    AicpuModelStatus GetModelStatus() const { return modelStatus_; }

    ModelPrepareData& GetModelPrepareData() { return prepareData_; }

    ModelPostpareData& GetModelPostpareData() { return postpareData_; }

    std::vector<void*>& GetInputDataPtrs() { return inputDataPtrs_; }

    void SetModelEndOfSequence() { endOfSequence_ = true; }

    void ReSetModelEndOfSequence() { endOfSequence_ = false; }

    bool IsEndOfSequence() const { return endOfSequence_.load(); }

    void SetExtModelId(const uint32_t extId) { extModelId_ = extId; }

    uint64_t GetIteratorId() const { return iteratorCount_; }

    inline StepIdInfo GetStepIdInfo() const { return stepIdInfo_; }
    ...
    std::vector<bool>& MutableInputsIsDequeue() { return inputsIsDequeue_; }

    void SetModelRetCode(const int32_t retVal) { retCode_ = retVal; }

    int32_t GetModelRetCode() const { return retCode_.load(); }

    bool AbnormalNeedBreak() const { return abnormalBreak_.load(); }

    bool AbnormalNeedEnqueue() const { return abnormalEnqueue_.load(); }

    bool AbnormalEnabled() const { return abnormalEnabled_.load(); }

    inline uint32_t GetId() const { return modelId_; }

    void WaitReleaseThreadsFinish();

    inline void SetNullDataFlag(const bool val) { nullDataFlag_ = val; }

    inline bool GetNullDataFlag() const { return nullDataFlag_.load(); }

    inline uint32_t GetInputQueueCount() const { return static_cast<uint32_t>(inputQueueIds_.size()); }

    inline bool HasQueue(uint32_t queueId) const { return inputQueueIds_.count(queueId) > 0U; }

    StoreResult StoreDequedMbuf(
        const uint64_t transId, const uint32_t routeLabel, const size_t qIndex, Mbuf* const mbuf,
    ...
    inline void SetTableTryLock(const int64_t tableId) { tableTryLock_ = tableId; }

    inline int64_t GetTableTryLock() const { return tableTryLock_; }

    uint32_t& GetInputConsumeNumRef();

    inline uint32_t GetActiveStreamNum() const { return activeStreamNum_; }

    inline void IncreaseActiveStreamNum() { activeStreamNum_++; }

    bool GetModelDestroyStatus() const { return isDestroyModel_; }
    ...
    inline void ResetStaticNNModelOutputIndex() { staticNNCurOutIndex_ = 0U; }

    inline void IncreaseStaticNNModelOutputIndex() { staticNNCurOutIndex_++; }

    inline uint32_t GetCurStaticNNModelOutputIndex() const { return staticNNCurOutIndex_; }
...
private:
    ...
    __attribute__((visibility("hidden"))) void LoadWaitNotifyId(
        const AicpuTaskInfo& aicpuTaskInfo, std::unordered_set<size_t>& waitNotifyIdSet) const;

    GatherResult GatherDequedMbuf(
