 ...
 */
AicpuEventProcess::AicpuEventProcess() : exitQueueFlag_(false), platformFuncPtr_(nullptr)
{
...
void AicpuEventProcess::InitQueueFlag() { exitQueueFlag_ = true; }
...
int32_t AicpuEventProcess::AICPUEventUpdateProfilingMode(const AICPUSubEventInfo& subEventInfo) const
{
    ...
    aicpusd_info(
        "Begin to update aicpu event profiling mode, flag[%lu], isStart[%d], mode[%d],"
        " isModelModeOn[%d]",
        flag, isStart, mode, isModelModeOn);
    ...
    if (SendUpdateProfilingRspToTsd(
...
int32_t AicpuEventProcess::AICPUEventPrepareMem(const AICPUSubEventInfo& subEventInfo) const
{
    ...
    if (ret == AICPU_SCHEDULE_OK) {
        ...
        aicpusd_err(
            "PrepareMem event of model[%u] recover execute stream[%u] failed, ret[%d].", modelId, waitStreamId, ret);
    }
    return ret;
}
...
int32_t AicpuEventProcess::AICPUEventTableUnlock(const AICPUSubEventInfo& subEventInfo) const
{
    ...
    for (const auto model : AicpuModelManager::GetInstance().GetModelsByTableId(tableId)) {
        bool hasWait = false;
        uint32_t waitStreamId = 0U;
        EventWaitManager::TableUnlockWaitManager().Event(static_cast<size_t>(model->GetId()), hasWait, waitStreamId);
        if (!hasWait) {
            continue;
        }
        const auto ret = model->RecoverStream(waitStreamId);
        if (ret != AICPU_SCHEDULE_OK) {
            aicpusd_err(
                "TableUnlock event to recover model[%u], stream[%u] failed, ret is [%d]", model->GetId(), waitStreamId,
                ret);
            return ret;
        }
    }
...
int32_t AicpuEventProcess::AICPUEventSupplyEnque(const AICPUSubEventInfo& subEventInfo) const
{
    ...
    if (ret == AICPU_SCHEDULE_OK) {
        ...
        aicpusd_err("AnyQueNotEmpty[%u] event recover execute stream[%u] failed, ret[%d].", modelId, waitStreamId, ret);
    }
    return ret;
}
...
int32_t AicpuEventProcess::ProcessSetTimeoutEvent(AicpuSqeAdapter& aicpuSqeAdapter) const
{
    ...
    aicpusd_run_info(
        "Get opTimeoutEnable[%u] value[%u] waitTimeoutEnable[%u] value[%u] from ts success.", excTimeoutEnable,
    ...
    if (excTimeoutEnable == 1U) {
        if (AicpuUtil::IsValidTimeoutVal(excTimeoutVal)) {
            ...
            aicpusd_info(
                "Config OP execute timeout success, enable[%u] timeout[%us].", excTimeoutEnable, excTimeoutVal);
        } else {
    ...
    if (waitTimeoutEnable == 1U) {
        if ((waitTimeoutVal != 0U) && (waitTimeoutVal < curTimeoutVal)) {
            ...
            aicpusd_run_info("Close aicpu op timer, waitTimeVal[%us], curTimeVal[%us].", waitTimeoutVal, curTimeoutVal);
    ...
    const int32_t tmpRet = aicpuSqeAdapter.AicpuTimeOutConfigResponseToTs(static_cast<uint32_t>(ret));
...
int32_t AicpuEventProcess::ProcessDumpDataEvent(AicpuSqeAdapter& aicpuSqeAdapter) const
{
    ...
    TaskInfoExt dumpTaskInfo(
        static_cast<volatile uint32_t>(info.dump_stream_id), static_cast<volatile uint32_t>(info.dump_task_id));
    ...
    if (info.is_model && info.is_debug) {
        TaskInfoExt debugdumpTaskInfo(
            static_cast<volatile uint32_t>(info.debug_dump_stream_id),
...
int32_t AicpuEventProcess::ProcessDumpFFTSPlusDataEvent(AicpuSqeAdapter& aicpuSqeAdapter) const
{
    ...
    aicpusd_info(
        "Begin to dump FFTSPlus data, stream id[%u], task id[%u], stream id1[%u], task id1[%u], "
        "model id[%u], context id[%u], thread id[%u], reserved[%u].",
        info.i.stream_id, info.i.task_id, info.i.stream_id1, info.i.task_id1, static_cast<uint32_t>(info.i.model_id),
        info.i.context_id, info.i.thread_id, info.i.reserved[0]);
    ...
    if ((info.i.task_id != INVALID_VAL) && (info.i.stream_id != INVALID_VAL)) {
        ...
        TaskInfoExt dumpTaskInfo(
            static_cast<volatile uint32_t>(info.i.stream_id), static_cast<volatile uint32_t>(info.i.task_id), contextId,
            threadId);
    ...
    if ((info.i.task_id1 != INVALID_VAL) && (info.i.stream_id1 != INVALID_VAL)) {
        validDumpParam = true;
        TaskInfoExt dumpTaskInfo(
            static_cast<volatile uint32_t>(info.i.stream_id1), static_cast<volatile uint32_t>(info.i.task_id1),
            static_cast<volatile uint32_t>(info.i.context_id), static_cast<volatile uint32_t>(info.i.thread_id));
        DumpFileName dumpFileName(info.i.stream_id1, info.i.task_id1, info.i.context_id, info.i.thread_id);
    ...
    if (!validDumpParam) {
        aicpusd_err(
            "Invalid dump FFTSPlus param, stream id[%hu], task id[%hu], stream id1[%hu], task id1[%hu], "
            "context id[%hu], thread id[%u].",
            info.i.stream_id, info.i.task_id, info.i.stream_id1, info.i.task_id1, info.i.context_id, info.i.thread_id);
...
int32_t AicpuEventProcess::ProcessLoadOpMappingEvent(AicpuSqeAdapter& aicpuSqeAdapter) const
{
    ...
    const char_t* const opMappingInfo = PtrToPtr<void, char_t>(ValueToPtr(static_cast<uintptr_t>(info.dumpinfoPtr)));
    ...
    AicpuSchedule::AicpuSdCustDumpProcess::GetInstance().InitCustDumpProcess(
        deviceId, static_cast<aicpu::AicpuRunMode>(runMode));
    // send response information to ts.
...
int32_t AicpuEventProcess::ProcessAICPUEvent(const event_info& drvEventInfo)
{
    ...
    aicpusd_info(
        "Begin to receive the aicpu event, eventId[%d], drvEventId[%d]", drvEventInfo.comm.subevent_id, drvEventId);
...
int32_t AicpuEventProcess::ProcessQueueNotEmptyEvent(const uint32_t queueId) const
{
    ...
    if (model == nullptr) {
        aicpusd_err(
            "Queue[%u] non-empty event recover execute stream[%u] failed, as find modelId failed.", queueId,
            waitStreamId);
        return AICPU_SCHEDULE_ERROR_STREAM_NOT_FOUND;
    }
    aicpusd_info("Begin to recover execute stream[%u].", waitStreamId);
    ...
    const auto ret = model->RecoverStream(waitStreamId);
    if (ret == AICPU_SCHEDULE_OK) {
        aicpusd_info("Queue[%u] non-empty event recover execute stream[%u] success.", queueId, waitStreamId);
    } else {
        aicpusd_err(
            "Queue[%u] non-empty event recover execute stream[%u] failed, ret[%d].", queueId, waitStreamId, ret);
    }
    return ret;
}
...
int32_t AicpuEventProcess::ProcessEnqueueEvent(const event_info& drvEventInfo) const
{
    ...
    aicpusd_info(
        "Finish to call ProcessEnqueue by eventId:[%u] subeventId:[%u] end, ret:[%d]", drvEventId, drvSubeventId, ret);
    return ret;
}
...
int32_t AicpuEventProcess::ProcessQueueNotFullEvent(const uint32_t queueId) const
{
    ...
    if (model == nullptr) {
        aicpusd_err(
            "Queue[%u] not full event recover execute stream[%u] failed, as find modelId failed.", queueId,
            waitStreamId);
        return AICPU_SCHEDULE_ERROR_STREAM_NOT_FOUND;
    }
    ...
    const auto ret = model->RecoverStream(waitStreamId);
    if (ret == AICPU_SCHEDULE_OK) {
        aicpusd_info("Queue[%u] not full event recover execute stream[%u] success.", queueId, waitStreamId);
    } else {
        aicpusd_err("Queue[%u] not full event recover execute stream[%u] failed, ret[%d].", queueId, waitStreamId, ret);
    }
    return ret;
}
 ...
 * @return AICPU_SCHEDULE_OK: success, other: error code
 */
int32_t AicpuEventProcess::ProcessResponseInfo(
    AicpuSqeAdapter& adapter, const uint16_t resultCode, const uint32_t subEvent, const bool noThreadFlag) const
{
...
void AicpuEventProcess::PostProcessTsKernelTask(
    const int32_t errorCode, const uint64_t streamId, const uint64_t taskId) const
{
    ...
    aicpusd_err(
        "Aicpu engine process failed, result[%d], opName[%s], streamId[%llu], taskId[%llu].", errorCode, opname.c_str(),
        streamId, taskId);
    (void)aicpu::SetOpname("null");
...
int32_t AicpuEventProcess::ExecuteLogicKernel(aicpu::HwtsTsKernel& tsKernelInfo) const
{
    ...
    const size_t cpySize = strnlen(
        static_cast<const char_t*>(ValueToPtr(tsKernelInfo.kernelBase.cceKernel.kernelName)), MAX_KERNEL_NAME_LEN);
    char_t kernelName[MAX_KERNEL_NAME_LEN + 1UL] = {};
    const errno_t memRet = memcpy_s(
        &kernelName[0], MAX_KERNEL_NAME_LEN + 1UL,
...
int32_t AicpuEventProcess::SendLoadPlatformFromBufMsgRsp(
    const int32_t resultCode, AicpuSqeAdapter& aicpuSqeAdapter) const
{
    // send response information to ts.
