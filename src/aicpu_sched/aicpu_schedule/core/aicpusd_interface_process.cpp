...
int32_t AicpuScheduleInterface::LoadModelWithQueue(const void* const ptr) const
{
    ...
    const auto ret = AicpuModelManager::GetInstance().TransModelInfo(
        ptr, curAicpuModelInfo, aicpuTaskInfos, streamInfos, queInfos, nullptr);
...
int32_t AicpuScheduleInterface::LoadModelWithEvent(const void* const ptr) const
{
    ...
    const auto ret = AicpuModelManager::GetInstance().TransModelInfo(
        ptr, curAicpuModelInfo, aicpuTaskInfos, streamInfos, queInfos, &modelCfgs);
    if (ret != AICPU_SCHEDULE_OK) {
        return ret;
    }
...
int32_t AicpuScheduleInterface::ExecuteModelAsync(const uint32_t modelId) const
{
    ...
    return AicpuMsgSend::SendAICPUSubEvent(
        ...
        AICPU_SUB_EVENT_EXECUTE_MODEL, CP_DEFAULT_GROUP_ID, syncSendFlag);
}
...
int32_t AicpuScheduleInterface::BindHostPid(const std::string& pidSign, const uint32_t vfId) const
{
    ...
    if (runMode_ == aicpu::AicpuRunMode::THREAD_MODE) {
        ...
        ret = AicpuDrvManager::GetInstance().BindHostPid(pidSign, mode, vfId, DEVDRV_PROCESS_CP1);
        int32_t getRet = 0;
        getRet = AicpuDrvManager::GetInstance().BindHostPid(pidSign, mode, vfId, DEVDRV_PROCESS_CP2);
 ...
 */
int32_t AicpuScheduleInterface::InitAICPUScheduler(
    const std::vector<uint32_t>& deviceVec, const pid_t hostPid, const std::string& pidSign,
    const uint32_t profilingMode, const uint32_t vfId, const bool isOnline, const std::string& hostProcName)
{
    if (deviceVec.empty()) {
    ...
    if (AicpuDrvManager::GetInstance().InitDrvMgr(deviceVec, hostPid, vfId, true, runMode_, hostProcName) !=
        AICPU_SCHEDULE_OK) {
    ...
    const auto retCp = ComputeProcess::GetInstance().Start(deviceVec, hostPid, pidSign, profilingMode, vfId, runMode_);
    ...
    aicpusd_info(
        "Successfully started compute process, deviceId[%u], hostPid[%d], runMode[%u]", deviceVec[FIRST_INDEX], hostPid,
        runMode_);
...
int32_t AicpuScheduleInterface::Destroy(const uint32_t modelId) const
{
    ...
    if (tsId < static_cast<uint32_t>(TSIDFlag::AICPU_SCHEDULE_TS_ID)) {
        ...
        const int32_t drvRet = tsDevSendMsgAsync(
            AicpuDrvManager::GetInstance().GetDeviceId(), tsId, PtrToPtr<TsAicpuSqe, char_t>(&tsAicpuSqe),
    ...
    const std::vector<uint32_t>& deviceVec, const pid_t hostPid, const bool waitThreadStop)
{
    ...
    if ((runMode_ == aicpu::AicpuRunMode::PROCESS_PCIE_MODE) ||
        (runMode_ == aicpu::AicpuRunMode::PROCESS_SOCKET_MODE) || waitThreadStop) {
...
int32_t AicpuScheduleInterface::GetAicpuCustSdProcId() const { return aicpuCustSdPid_.load(); }
...
int32_t AicpuScheduleInterface::ProcessException(const DataFlowExceptionNotify* const exceptionInfo) const
{
    ...
    for (uint32_t i = 0; i < modelNum; ++i) {
        ...
        aicpusd_run_info(
            "Successfully processed kernel data exception for transid[%lu] of model[%u].", exceptionInfo->transId,
            modelId);
    }
...
void AicpuScheduleInterface::SetBatchLoadMode(const uint32_t aicpuProcNum)
{
    if (aicpuProcNum >= MAX_AICPU_PROC_NUM) {
        ...
        aicpusd_run_info("Not need batch load so, aicpuProcNum[%u], max num[%u]", aicpuProcNum, MAX_AICPU_PROC_NUM);
    }
}

bool AicpuScheduleInterface::NeedLoadKernelSo() const { return isNeedBatchLoadSo_; }
} // namespace AicpuSchedule
