...
ThreadPool::ThreadPool() : semInitedNum_(0UL) { aicpusd_run_info("ThreadPool"); }
...
int32_t ThreadPool::CreateOneWorker(const size_t threadIndex, const uint32_t deviceId)
{
    try {
...
{
    const int32_t ret =
        halEschedSubscribeEvent(deviceId, CP_DEFAULT_GROUP_ID, static_cast<uint32_t>(threadIndex), CP_EVENT_MASK);
    if (ret != DRV_ERROR_NONE) {
        aicpusd_err(
            "Subscribe event failed, deviceId[%u], groupId[%u], threadIndex[%zu] "
            "eventBitmap[%llu].",
    ...
    aicpusd_info(
        "halEschedSubscribeEvent success, deviceId[%u], groupId[%u], threadIndex[%zu] eventBitmap[%llu].", deviceId,
        CP_DEFAULT_GROUP_ID, threadIndex, CP_EVENT_MASK);
...
int32_t ThreadPool::WriteTidForAffinity(const size_t threadIndex)
{
    if (threadIndex >= threadStatusList_.size()) {
        aicpusd_err("threadIndex[%zu], out of rank[0, %zu]", threadIndex, threadStatusList_.size());
        return AICPU_SCHEDULE_ERROR_INIT_FAILED;
    }
    ...
    if (ret != 0) {
        ...
        aicpusd_err(
            "write tid[%llu] to /sys/fs/cgroup/cpuset/AICPU/tasks failed, ret[%d], strerror[%s]", GetTid(), ret,
            strerror(errno));
        return AICPU_SCHEDULE_ERROR_INIT_FAILED;
    }
...
int32_t ThreadPool::SetAffinityBySelf(const size_t threadIndex, const uint32_t deviceId)
{
    ...
    cpu_set_t mask;
    CPU_ZERO(&mask);
    const uint32_t aicpuLogIndex =
        hasAicpu_ ? static_cast<uint32_t>(threadIndex) % AicpuDrvManager::GetInstance().GetAicpuNumPerDevice() : 0U;
    ...
    if (ret != 0) {
        threadStatusList_[threadIndex] = ThreadStatus::THREAD_EXIT;
        aicpusd_err(
            "set affinity failed ret[%d], aicpu logical index[%zu], aicpu physical index[%u], "
            "device id[%u]",
            ret, threadIndex, physIndex, deviceId);
        return AICPU_SCHEDULE_ERROR_INIT_FAILED;
    }
    threadStatusList_[threadIndex] = ThreadStatus::THREAD_RUNNING;
    aicpusd_info(
        "set affinity success, aicpu logical index[%zu], aicpu physical index[%u], device id[%u]", threadIndex,
        physIndex, deviceId);
    return AICPU_SCHEDULE_OK;
}
...
int32_t ThreadPool::SetAffinityByPm(const size_t threadIndex, const uint32_t deviceId)
{
    ...
    uint32_t physIndex;
    if (FeatureCtrl::IsVfModeCheckedByDeviceId(deviceId)) {
        physIndex = AicpuDrvManager::GetInstance().GetAicpuPhysIndex(aicpuLogIndex, (deviceId - VDEVICE_MIN_CPU_NUM));
    } else {
        physIndex = hasAicpu_ ? AicpuDrvManager::GetInstance().GetAicpuPhysIndex(aicpuLogIndex, deviceId) :
    ...
    if (ret != 0U) {
        threadStatusList_[threadIndex] = ThreadStatus::THREAD_EXIT;
        aicpusd_err(
            "set affinity failed ret[%d], aicpu logical index[%zu], "
    ...
    aicpusd_info(
        "set affinity success, aicpu logical index[%zu], aicpu physical index[%u], device id[%u]", threadIndex,
        physIndex, AicpuDrvManager::GetInstance().GetDeviceId());
    return AICPU_SCHEDULE_OK;
}
...
void ThreadPool::PostSem(const size_t threadIndex) { (void)sem_post(&(sems_[threadIndex])); }

void ThreadPool::SetThreadIdRelation(const size_t threadIndex, const pid_t threadId)
{
    threadIdLists_[threadIndex] = threadId;
    aicpusd_info(
        "set thread index:%zu and tid:%d relation to List", threadIndex,
