...
AicpuEventManager::AicpuEventManager()
    : noThreadFlag_(true), runningFlag_(true), groupId_(0U), eventGetFunc_(nullptr), eventRspFunc_(nullptr)
{
 ...
 */
void AicpuEventManager::InitEventMgr(const bool noThreadFlag, const bool runningFlag, const uint32_t grpId)
{
...
{
    eventGetFunc_ = (aicpuSchedMode == AicpuSchedMode::SCHED_MODE_MSGQ) ? &AicpuEventManager::DoOnceMsq :
                                                                          &AicpuEventManager::DoOnceEsched;
    eventRspFunc_ = (aicpuSchedMode == AicpuSchedMode::SCHED_MODE_MSGQ) ? &AicpuEventManager::ResponseMsq :
 ...
 */
int32_t AicpuEventManager::ExecuteProcessSyc(const event_info* const drvEventInfo, event_ack* const drvEventAck)
{
...
int32_t AicpuEventManager::ProcessModelEvent(AicpuSqeAdapter& aicpuSqeAdapter, const uint32_t subEvent)
{
    ...
    ret = AicpuEventProcess::GetInstance().ProcessResponseInfo(
        aicpuSqeAdapter, static_cast<uint16_t>(ret), subEvent, noThreadFlag_);
    return ret;
}
...
int32_t AicpuEventManager::ProcessHWTSControlEventV0(const event_info& drvEventInfo)
{
    ...
    switch (cmdType) {
        ...
        case AIC_ERROR_REPORT: {
            ...
            aicpu::AsyncEventManager::GetInstance().ProcessOpEvent(drvEventInfo.comm.event_id, cmdType, &param);
            break;
        }
        default:
...
void AicpuEventManager::PrintAicErrReportInfo(const AicpuSqeAdapter::AicErrReportInfo& reportInfo)
{
    ...
    if (bitMapTotalNum > bitMapCapacity) {
        aicpusd_err(
            "The total number of bit maps of AIC[%hu] and AIV[%hu] exceeds bitmap capacity[%zu].", aicBitmapNum,
            aivBitmapNum, bitMapCapacity);
        return;
    }
    std::ostringstream oss;
    for (size_t i = 0; i < aicBitmapNum; i++) {
        oss << "aicmap"
            << "[" << i << "] : " << std::bitset<BIT_8>(aicAivErrMsg.bitmap[i]).to_string() << ",";
    }
 ...
 * @brief it use to init control event version function map
 */
void AicpuEventManager::GetStreamIdAndTaskIdFromEvent(
    const hwts_ts_kernel& tsKernel, uint32_t& streamId, uint64_t& taskId, uint32_t subevent_id) const
{
    ...
    aicpusd_info("Get stream id[%u] and task id[%u] in version[%hu].", streamId, taskId, version);
    return;
}
/**
 * @ingroup AicpuEventManager
 ...
 * @return AICPU_SCHEDULE_OK: success, other: error code
 */
int32_t AicpuEventManager::HWTSKernelEventMessageParse(
    const event_info& drvEventInfo, uint32_t& mailboxId, aicpu::HwtsTsKernel& info, uint64_t& serialNo,
    uint32_t& streamId, uint64_t& taskId, uint16_t& dataDumpEnableMode) const
{
    // for the msg is address of array, it is not a nullptr.
...
int32_t AicpuEventManager::ExecuteHWTSKFCEventTask(
    const event_info& drvEventInfo, const aicpu::HwtsTsKernel& aicpufwKernelInfo, const uint32_t threadIndex,
    const uint32_t streamId, const uint64_t taskId) const
{
 ...
 * @return AICPU_SCHEDULE_OK: success, other: error code
 */
int32_t AicpuEventManager::ProcessHWTSKernelEvent(const event_info& drvEventInfo, const uint32_t threadIndex) const
{
    ...
    int32_t ret = HWTSKernelEventMessageParse(
        drvEventInfo, mailboxId, aicpufwKernelInfo, serialNo, streamId, taskId, dataDumpEnableMode);
    if (ret != AICPU_SCHEDULE_OK) {
        return ret;
    }
    ...
    if (kernelType == aicpu::KERNEL_TYPE_AICPU_KFC) {
        ...
        hwtsResp.status =
            (ret == AICPU_SCHEDULE_OK) ? static_cast<uint32_t>(TASK_SUCC) : static_cast<uint32_t>(TASK_FAIL);
    } else {
        const TaskInfoForMonitor monitorTaskInfo = {
            .serialNo = serialNo, .taskId = taskId, .streamId = streamId, .isHwts = true};
        AicpuMonitor::GetInstance().SetTaskInfo(threadIndex, monitorTaskInfo);
        ret = ExecuteHWTSEventTask(
    ...
    const auto kernelName =
        PtrToPtr<const void, const char_t>(ValueToPtr(aicpufwKernelInfo.kernelBase.cceKernel.kernelName));
    ...
    if (drvRet != DRV_ERROR_NONE) {
        aicpusd_err(
            "Failed to send ack to Ts, kernelType[%u] deviceId[%u], subevent_id[%u], mailboxId[%u], "
            "serialNo[%lu], streamId[%u], taskId[%lu], result[%d], originResult[%d], status[%d], error[%d].",
            kernelType, deviceId, drvSubeventId, mailboxId, serialNo, streamId, taskId, hwtsResp.result, ret,
            hwtsResp.status, drvRet);
        return AICPU_SCHEDULE_ERROR_INNER_ERROR;
    }
    aicpusd_info(
        "End send ack to Ts, kernelType[%u], deviceId[%u], subevent_id[%u], mailboxId[%u], "
        "serialNo[%lu], streamId[%u], taskId[%lu], result[%d], originResult[%d], status[%d].",
        kernelType, deviceId, drvSubeventId, mailboxId, serialNo, streamId, taskId, hwtsResp.result, ret,
        hwtsResp.status);
    return AICPU_SCHEDULE_OK;
}
int32_t AicpuEventManager::ExecuteHWTSEventTask(
    const uint32_t& threadIndex, const uint32_t& streamId, const uint64_t taskId, const uint64_t& serialNo,
    aicpu::HwtsTsKernel& aicpufwKernelInfo, const event_info& drvEventInfo, const uint32_t& mailboxId,
    hwts_response_t& hwtsResp, uint16_t& dataDumpEnableMode) const
{
    (void)aicpu::SetThreadLocalCtx(CONTEXT_KEY_STREAM_ID, std::to_string(streamId));
    (void)aicpu::SetTaskAndStreamId(taskId, streamId);

    aicpusd_info(
        "Begin to ExecuteTsKernelTask, mailboxId[%u], serialNo[%lu], streamId[%u], taskId[%lu], "
        "dataDumpEnableMode[%u], kernelType[%u], subeventId[%u].",
        mailboxId, serialNo, streamId, taskId, dataDumpEnableMode, aicpufwKernelInfo.kernelType,
        drvEventInfo.comm.subevent_id);
    int32_t ret = AicpuEventProcess::GetInstance().ExecuteTsKernelTask(
        aicpufwKernelInfo, threadIndex, static_cast<uint64_t>(drvEventInfo.comm.submit_timestamp),
        static_cast<uint64_t>(drvEventInfo.comm.sched_timestamp), static_cast<uint64_t>(streamId), taskId);
    ...
    if ((dataDumpEnableMode == 1U) && (ret == AICPU_SCHEDULE_OK)) {
        ...
        const int32_t dumpRet = AicpuSchedule::OpDumpTaskManager::GetInstance().DumpOpInfo(dumpTaskInfo, fileName);
...
int32_t AicpuEventManager::ProcessDvppEvent(const event_info& drvEventInfo, const uint32_t threadIndex) const
{
    ...
    *attr = PtrToValue(PtrToPtr<const event_info, const void>(&drvEventInfo));
    ...
    const auto taskRet = AicpuEventProcess::GetInstance().ExecuteTsKernelTask(
        dvppKernel, threadIndex, static_cast<uint64_t>(drvEventInfo.comm.submit_timestamp),
        static_cast<uint64_t>(drvEventInfo.comm.sched_timestamp));
    aicpusd_info(
        "Find and execute dvpp task end, threadIndex[%u], taskRet[%d], argsSize[%zu].", threadIndex, taskRet, argsSize);
    return taskRet;
}
...
int32_t AicpuEventManager::ProcessMpiDvppEvent(const event_info& drvEventInfo, const uint32_t threadIndex) const
{
    ...
    const auto taskRet = AicpuEventProcess::GetInstance().ExecuteTsKernelTask(
        mpiKernel, threadIndex, static_cast<uint64_t>(drvEventInfo.comm.submit_timestamp),
        static_cast<uint64_t>(drvEventInfo.comm.sched_timestamp));
...
int32_t AicpuEventManager::ProcessRetrEvent(const event_info& drvEventInfo, const uint32_t threadIndex) const
{
    ...
    *attr = PtrToValue(PtrToPtr<const event_info, const void>(&drvEventInfo));
    ...
    const auto taskRet = AicpuEventProcess::GetInstance().ExecuteTsKernelTask(
        retrKernel, threadIndex, static_cast<uint64_t>(drvEventInfo.comm.submit_timestamp),
        static_cast<uint64_t>(drvEventInfo.comm.sched_timestamp));
    aicpusd_info(
        "Find and execute retr task end, threadIndex[%u], taskRet[%d], argsSize[%zu].", threadIndex, taskRet, argsSize);
    return taskRet;
}
...
{
    (void)threadIndex;
    aicpusd_err("Receive event but no process handle, eventID=%d", static_cast<int32_t>(drvEventInfo.comm.event_id));
    return AICPU_SCHEDULE_ERROR_NOT_FOUND_EVENT;
}
...
int32_t AicpuEventManager::ProcSplitKernelEvent(const event_info& drvEventInfo, const uint32_t threadIndex)
{
    ...
    aicpusd_info(
        "Begin to process split kernel event. parallelId=%u, type=wait", eventInfo->para.sharderTaskInfo.parallelId);

    const bool ret = ComputeProcess::GetInstance().DoSplitKernelTask(eventInfo->para.sharderTaskInfo);
    if (!ret) {
        aicpusd_err(
            "Process split kernel event failed. parallelId=%u, type=wait", eventInfo->para.sharderTaskInfo.parallelId);
...
int32_t AicpuEventManager::ProcTsCtrlEvent(const event_info& drvEventInfo, const uint32_t threadIndex)
{
    ...
    aicpusd_info(
        "Finish to process CTRL event, eventid[%u], threadIndex[%u], ret[%d].", drvEventInfo.comm.event_id, threadIndex,
        ret);
    return ret;
}

int32_t AicpuEventManager::ProcQueueEmptyToNotEmptyEvent(const event_info& drvEventInfo, const uint32_t threadIndex)
{
    (void)threadIndex;
    ...
    aicpusd_info(
        "Finish to process non-empty event, queueId[%u], threadIndex[%u], ret[%d].", queueId, threadIndex, ret);
    return ret;
}

int32_t AicpuEventManager::ProcQueueFullToNotFullEvent(const event_info& drvEventInfo, const uint32_t threadIndex)
{
    (void)threadIndex;
    const uint32_t queueId = drvEventInfo.comm.subevent_id;
    const int32_t ret = AicpuEventProcess::GetInstance().ProcessQueueNotFullEvent(queueId);
    aicpusd_info("Finish to process not full event, queueId[%u], threadIndex[%u], ret[%d].", queueId, threadIndex, ret);
    return ret;
}
...
int32_t AicpuEventManager::ProcOnPreprocessEvent(const event_info& drvEventInfo, const uint32_t threadIndex)
{
    ...
    DataPreprocess::TaskQueueMgr::GetInstance().OnPreprocessEvent(static_cast<uint32_t>(drvEventInfo.comm.event_id));
    aicpusd_info(
        "Finish to process queue enqueue event, event id[%d], threadIndex[%u]", drvEventInfo.comm.event_id,
        threadIndex);
    return AICPU_SCHEDULE_OK;
}
...
int32_t AicpuEventManager::ProcQsEvent(const event_info& drvEventInfo, const uint32_t threadIndex)
{
    ...
    aicpusd_info(
        "Finish to process queue event qs, event id[%d], threadIndex[%u], ret[%d]", drvEventInfo.comm.event_id,
        threadIndex, ret);
    return ret;
}
...
int32_t AicpuEventManager::DoOnceEsched(const uint32_t threadIndex, const uint32_t deviceId, const int32_t timeout)
{
    ...
    if (retVal == DRV_ERROR_NONE) {
        ...
        aicpusd_warn(
            "Failed to get event, error code=%d, deviceId[%u], groupId[%u], threadIndex[%u]", retVal, deviceId,
            groupId_, threadIndex);
    } else if (retVal == DRV_ERROR_SCHED_RUN_IN_ILLEGAL_CPU) {
        runningFlag_ = false;
        aicpusd_err(
            "Cpu Illegal get event, error code[%d], deviceId[%u], groupId[%u], threadIndex[%u]", retVal, deviceId,
            groupId_, threadIndex);
...
int32_t AicpuEventManager::DoOnceMsq(const uint32_t threadIndex, const uint32_t deviceId, const int32_t timeout)
{
    ...
    if (MessageQueue::WaitMsqInfoOnce(datas)) {
        ...
        resp.status = (ret == AICPU_SCHEDULE_OK) ? static_cast<uint32_t>(TASK_SUCC) : static_cast<uint32_t>(TASK_FAIL);
        (void)ResponseMsq(0U, 0U, resp);
    }
...
int32_t AicpuEventManager::ResponseMsq(const uint32_t deviceId, const uint32_t subEventId, hwts_response_t& resp) const
{
    (void)deviceId;
    (void)subEventId;
...
int32_t AicpuEventManager::ResponseEsched(
    const uint32_t deviceId, const uint32_t subEventId, hwts_response_t& resp) const
{
    return halEschedAckEvent(
        deviceId, EVENT_TS_HWTS_KERNEL, subEventId, PtrToPtr<hwts_response_t, char_t>(&resp),
...
int32_t AicpuEventManager::FillEventPrivCommInfo(const AicpuTopicMailbox& mb, event_info& eventInfo) const
{
    ...
    const int32_t ret =
        memcpy_s(eventInfo.priv.msg, EVENT_MAX_MSG_LEN, &mb.userData[0], static_cast<uint64_t>(userDataLen));
    if (ret != AICPU_SCHEDULE_OK) {
        aicpusd_err(
            "Failed to copy event msg, eventId=%u, subEventId=%u, max=%d, userDataLen=%u", mb.topicId, mb.subtopicId,
            EVENT_MAX_MSG_LEN, userDataLen);
