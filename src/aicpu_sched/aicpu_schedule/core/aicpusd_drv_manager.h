...
public:
    ...
    int32_t InitDrvMgr(
        ...
        const aicpu::AicpuRunMode runMode = aicpu::AicpuRunMode::INVALID_MODE, const std::string& hostProcName = "");
     ...
     * @return AICPU_SCHEDULE_OK: success, other: error code
     */
    int32_t InitDrvSchedModule(const uint32_t grpId, const std::map<EVENT_ID, SCHEDULE_PRIORITY>& priority);
    ...
    uint32_t GetDeviceId() const { return (deviceVec_.size() == 0LU) ? 0U : deviceVec_[0]; }

    uint32_t GetUniqueVfId() const { return uniqueVfId_; }
    ...
    bool HasThread() const { return hasThread_; }
    ...
    int32_t BindHostPid(
        const std::string& signStr, const int32_t mode, const uint32_t vfId,
