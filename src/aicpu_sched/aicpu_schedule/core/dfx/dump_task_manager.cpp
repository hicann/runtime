    ...
    const aicpu::dump::OpMappingInfo& opMappingInfo, MappingInfoOptionalParam& optionalParam) const
{
    ...
    if (opMappingInfo.dump_switch_case() == aicpu::dump::OpMappingInfo::kDumpSwitchAddr) {
        optionalParam.dumpSwitchAddr = PtrToPtr<void, uint64_t>(ValueToPtr(opMappingInfo.dump_switch_addr()));
        optionalParam.hasDumpSwitch = true;
    }
}
int32_t OpDumpTaskManager::CreateOpDumpTask(
    std::shared_ptr<OpDumpTask>& opDumpTaskPtr, const int32_t hostPid, const uint32_t deviceId) const
{
    DATADUMP_MAKE_SHARED(
        opDumpTaskPtr = std::make_shared<OpDumpTask>(hostPid, deviceId), return AICPU_SCHEDULE_ERROR_DUMP_FAILED);
...
int32_t OpDumpTaskManager::Load(const aicpu::dump::OpMappingInfo& opMappingInfo, AicpuSqeAdapter& aicpuSqeAdapter)
{
    ...
    const uint32_t deviceId = AicpuSchedule::AicpuDrvManager::GetInstance().GetDeviceId();
    {
        ...
        for (int32_t i = 0; i < opMappingInfo.task_size(); ++i) {
            ...
            AicpuSqeAdapter::AicpuOpMappingDumpTaskInfo opmappingInfo(
                task.task_id(), task.stream_id(), taskId, streamId);
            const bool isOpMappingDumpTaskInfoVaild = aicpuSqeAdapter.IsOpMappingDumpTaskInfoVaild(opmappingInfo);
            if (!isOpMappingDumpTaskInfoVaild) {
                aicpusd_err(
                    "streamId or taskId is INVALID_VAL, task.task_id()[%u], task.stream_id()[%u], "
                    "streamId[%u], taskId[%u]",
                    opmappingInfo.proto_info_task_id, opmappingInfo.proto_info_stream_id, opmappingInfo.stream_id,
            ...
            if (task.tasktype() == aicpu::dump::Task::FFTSPLUS) {
                ...
                for (uint32_t j = 0U; j < contextFromMapInfoSize; ++j) {
                    ...
                    ret = opDumpTaskPtr->PreProcessOpMappingInfo(task, dumpPath, optionalParam, dumpStep, dumpMode);
                    if (ret != AICPU_SCHEDULE_OK) {
                    ...
                    aicpusd_info(
                        "[stream id:%u, task id:%u, context id:%u, thread id:%u]", dumpTaskInfo.stream_id,
                        dumpTaskInfo.task_id, contextIdTmp, threadIdTmp);
...
int32_t OpDumpTaskManager::Unload(const aicpu::dump::OpMappingInfo& opMappingInfo, AicpuSqeAdapter& aicpuSqeAdapter)
{
    const std::lock_guard<std::mutex> mapLock(dumpTaskMapMtx_);
    uint32_t streamId = INVALID_VAL;
    uint32_t taskId = INVALID_VAL;
    ...
    for (int32_t i = 0; i < opMappingInfo.task_size(); ++i) {
        ...
        for (uint32_t j = 0U; j < contextFromMapInfoSize; ++j) {
            ...
            aicpusd_info(
                "Unload stream id[%u], task id[%u], streamIdTmp[%u], taskIdTmp[%u],"
                " context id[%u], threadIdTmp[%u].",
                streamId, taskId, taskInfo.stream_id, taskInfo.task_id, contextIdTmp, threadIdTmp);
        ...
        if (isFftsPlusUnload == false) {
            ...
            UnloadClearTaskInfo(dumpTaskInfo);
            aicpusd_info(
                "Unload op mapping info, stream id[%u], task id[%u], streamIdTmp[%u], taskIdTmp[%u],"
                " context id[%u], threadIdTmp[%u].",
                streamId, taskId, taskInfo.stream_id, taskInfo.task_id, contextIdTmp, threadIdTmp);
        }
    }
    ...
    if (optionalParam.hasModelId) {
        ...
        for (const auto taskInfo : iter->second) {
            ...
            aicpusd_info(
                "Check and clean data dump task resource, stream id[%u], task id[%u]"
...
{
    TaskInfo curTaskInfo(dumpTaskInfo.streamId_, dumpTaskInfo.taskId_, dumpTaskInfo.contextId_, dumpTaskInfo.threadId_);
    dumpTaskMapMtx_.lock();
    const auto range = dumpTaskMap_.equal_range(curTaskInfo);
    if (range.first == range.second) {
        dumpTaskMapMtx_.unlock();
        aicpusd_warn(
            "task required to dump does not exist, stream id[%u], task id[%u], "
            "context id[%u], thread id[%u].",
            dumpTaskInfo.streamId_, dumpTaskInfo.taskId_, dumpTaskInfo.contextId_, dumpTaskInfo.threadId_);
        return AICPU_SCHEDULE_OK;
    } else {
        ...
        dumpTaskMapMtx_.unlock();
        aicpusd_run_info(
            "require to dump op info, stream id=%u, task id=%u, context id=%u, "
            "thread id=%u, task number=%zu",
            dumpTaskInfo.streamId_, dumpTaskInfo.taskId_, dumpTaskInfo.contextId_, dumpTaskInfo.threadId_,
            opDumptasks.size());
        uint32_t indexId = 0;
        for (auto item : opDumptasks) {
            ...
            aicpusd_run_info(
                "start to dump op info, stream id=%u, task id=%u, context id=%u, "
                "thread id=%u, op name=%s",
                dumpTaskInfo.streamId_, dumpTaskInfo.taskId_, dumpTaskInfo.contextId_, dumpTaskInfo.threadId_,
            ...
            aicpusd_run_info(
                "end of dump op info, result=%d, stream id=%u, task id=%u, "
                "context id=%u, thread id=%u, op name=%s, cost time is [%.2lf]us",
                ret, dumpTaskInfo.streamId_, dumpTaskInfo.taskId_, dumpTaskInfo.contextId_, dumpTaskInfo.threadId_,
...
int32_t OpDumpTaskManager::DoDump(
    const aicpu::dump::OpMappingInfo& opMappingInfo, const MappingInfoOptionalParam& optionalParam) const
{
    ...
    DumpMode dumpMode = opMappingInfo.dump_data();
    aicpusd_info(
        "skipAddressConversion[true], hasStepId[%u], task_id[%u], stream_id[%u] dumpMode[%d]",
        static_cast<uint32_t>(optionalParam.hasStepId), task.task_id(), task.stream_id(),
        static_cast<int32_t>(dumpMode));
    ret = opDumpTaskPtr->PreProcessOpMappingInfo(task, dumpPath, optionalParam, dumpStep, dumpMode, true);
    if (ret != AICPU_SCHEDULE_OK) {
    ...
    AicpuSqeAdapter::AicpuOpMappingDumpTaskInfo opmappingInfo(
        task.task_id(), task.stream_id(), INVALID_VAL, INVALID_VAL);
    AicpuSqeAdapter::AicpuDumpTaskInfo taskInfo{};
...
int32_t OpDumpTaskManager::GetAndClearOverflowStatus(
    const uint32_t deviceId, const uint32_t streamId, const uint32_t opType, uint32_t* status) const
{
    ...
    if (!EnsureDeviceOpened(deviceId)) {
        aicpusd_err(
            "The open device interface return failed, device id is %u, stream id is %u, op type is %u.", deviceId,
            streamId, opType);
        return AICPU_SCHEDULE_ERROR_FROM_DRV;
    }
    const drvError_t drvRet = halTsdrvCtl(
        deviceId, TSDRV_CTL_CMD_CTRL_MSG, static_cast<void*>(&para), sizeof(TsDrvCtrlMsg),
        static_cast<void*>(&queryResult), &queryResultCount);
    if (drvRet != DRV_ERROR_NONE) {
        aicpusd_err(
            "The device id is %u, stream id is %u, op type is %u, return result is %d.", deviceId, streamId, opType,
            static_cast<int32_t>(drvRet));
        return static_cast<int32_t>(drvRet);
    }
    *status = queryResult.u.query_stream_overflow_status.status;
    aicpusd_run_info(
        "The device id is %u, stream id is %u, op type is %u, overflow status result is %u.", deviceId, streamId,
        opType, queryResult.u.query_stream_overflow_status.status);
    return AICPU_SCHEDULE_OK;
}
...
int32_t OpDumpTaskManager::DoDumpBySwitchBitmap(
    const aicpu::dump::OpMappingInfo& opMappingInfo, const MappingInfoOptionalParam& optionalParam,
    const uint64_t switchBitMap) const
{
    // 全0表示关闭dump开关，或者溢出检测开关开启但是halTsdrvCtl接口或者drvDeviceOpen不存在，不做dump处理
    if ((switchBitMap == 0UL) ||
        ((switchBitMap & OVERFLOW_DUMP_BIT) != 0UL && ((&halTsdrvCtl == nullptr) || (&drvDeviceOpen == nullptr)))) {
        aicpusd_run_info(
            "Get op mapping info dump bitmap is [%llx], halTsdrvCtl available is [%d], drvDeviceOpen available is "
            "[%d].",
            switchBitMap, &halTsdrvCtl == nullptr, &drvDeviceOpen == nullptr);
        return AICPU_SCHEDULE_OK;
    }
    ...
    if (taskSize > MAX_TASK_SIZE) {
        aicpusd_run_info(
            "Task number[%d] exceeds 2, op mapping info: %s", taskSize, opMappingInfo.DebugString().c_str());
    }
    int32_t ret = AICPU_SCHEDULE_OK;
    for (int32_t i = 0; i < taskSize; i++) {
        ...
        if ((switchBitMap & OVERFLOW_DUMP_BIT) != 0UL) {
            ...
            const int32_t getRet =
                GetAndClearOverflowStatus(deviceId, task.stream_id(), OP_QUERY_OVERFLOW, &overflowStatus);
            if (getRet != AICPU_SCHEDULE_OK) {
                aicpusd_err(
                    "Query overflow status interface returned: ret is %d, device id is %u, stream id is %u.", getRet,
                    deviceId, task.stream_id());
                return AICPU_SCHEDULE_ERROR_FROM_DRV;
            }
            if (overflowStatus == 0U) { // result 为0表示不溢出，不做dump处理直接返回
                aicpusd_run_info(
                    "The query overflow status is %u, the device id is %u, and the stream id is %u.", overflowStatus,
                    deviceId, task.stream_id());
                return AICPU_SCHEDULE_OK;
            }
        }
        aicpusd_info(
            "Dump the parameter information : single or unknown shape flag[false], has step id[%u], task id[%u], "
            "stream id[%u], dump mode[%d].",
            static_cast<uint32_t>(optionalParam.hasStepId), task.task_id(), task.stream_id(),
            static_cast<int32_t>(dumpMode));
        ret = opDumpTaskPtr->PreProcessOpMappingInfo(task, dumpPath, optionalParam, dumpStep, dumpMode, false);
        if (ret != AICPU_SCHEDULE_OK) {
            aicpusd_err(
                "Preprocess op mapping info failed. Op mapping info is %s", opMappingInfo.DebugString().c_str());
            return ret;
        }
        ...
        aicpusd_run_info(
            "End of dump operation information: result is %d, op name is %s, cost time is [%.2lf]us.", ret,
            opDumpTaskPtr->GetOpName().c_str(), drUs);
        UNUSED(drUs);
        opDumpTaskPtr->ClearBaseDumpData();
        const_cast<OpDumpTaskManager*>(this)->ClearKfcDumpTaskInfo(kfcDumpTaskinfo);
    }
...
void OpDumpTaskManager::ClearKfcDumpTaskInfo(const KfcDumpTask& kfcTaskinfo)
{
    ...
    if (kfcDumpTaskMap_.find(kfcTaskinfo) != kfcDumpTaskMap_.end()) {
        ...
        aicpusd_info(
            "erase key of dump task map : streamId[%u], taskId[%u], index[%u]", kfcTaskinfo.streamId_,
            kfcTaskinfo.taskId_, kfcTaskinfo.index_);
    }
    if (kfcDumpInfoMap_.find(kfcTaskinfo) != kfcDumpInfoMap_.end()) {
        (void)kfcDumpInfoMap_.erase(kfcTaskinfo);
        aicpusd_info(
            "erase key of dump info map : streamId[%u], taskId[%u], index[%u]", kfcTaskinfo.streamId_,
            kfcTaskinfo.taskId_, kfcTaskinfo.index_);
    }
    ...
    if (custDumpTaskMap_.find(taskInfo) != custDumpTaskMap_.end()) {
        ...
        aicpusd_info(
            "erase key of cust dump task map : streamId[%u], taskId[%u], index[%u]", kfcTaskinfo.streamId_,
            kfcTaskinfo.taskId_, kfcTaskinfo.index_);
    }
    return;
}
...
void OpDumpTaskManager::MakeDumpOpInfoforKfc(const KfcDumpTask& taskinfo, std::shared_ptr<OpDumpTask> dumpTask)
{
    ...
    if (!(dumpTask->IsSupportKfcDump())) {
        aicpusd_info(
            "task does not support kfc statistical dump, streamId[%u], taskId[%u].", taskinfo.streamId_,
            taskinfo.taskId_);
        return;
    }
    kfcDumpTaskMap_.insert(std::make_pair(taskinfo, dumpTask));
    aicpusd_info(
        "Make kfc dump task map, streamId[%u], taskId[%u], index[%u]", taskinfo.streamId_, taskinfo.taskId_,
        taskinfo.index_);
    return;
}
int32_t OpDumpTaskManager::CreateKfcDumpInfo(std::shared_ptr<KfcDumpInfo>& kfcDumpInfoPtr) const
{
    DATADUMP_MAKE_SHARED(kfcDumpInfoPtr = std::make_shared<KfcDumpInfo>(), return AICPU_SCHEDULE_ERROR_DUMP_FAILED);
    return AICPU_SCHEDULE_OK;
}
/*
 * 向kfc提供dump查询信息接口
...
int32_t OpDumpTaskManager::GetDumpOpTaskDataforKfc(const KfcDumpTask& taskKey, KfcDumpInfo** dumpInfo)
{
    const std::lock_guard<std::mutex> mapLock(kfcDumpTaskMapMtx_);
    aicpusd_info(
        "Get dump info for kfc, streamId[%u], taskId[%u], index[%u].", taskKey.streamId_, taskKey.taskId_,
        taskKey.index_);
    // dumpInfo 是否为空在上层调用处保证
    const auto iter = kfcDumpTaskMap_.find(taskKey);
    if (iter == kfcDumpTaskMap_.end()) {
        aicpusd_err(
            "Cannot find dump task info key for kfc. streamId[%u], taskId[%u], index[%u].", taskKey.streamId_,
            taskKey.taskId_, taskKey.index_);
        return AICPU_SCHEDULE_ERROR_DUMP_FAILED;
    }
    if (iter->second == nullptr) {
        aicpusd_err(
            "Get dump task info failed. streamId[%u], taskId[%u], index[%u]", taskKey.streamId_, taskKey.taskId_,
            taskKey.index_);
        return AICPU_SCHEDULE_ERROR_DUMP_FAILED;
    }
    ...
    if (ret != AICPU_SCHEDULE_OK) {
        aicpusd_err(
            "Create opDumpTask failed streamId[%u], taskId[%u], index[%u]", taskKey.streamId_, taskKey.taskId_,
            taskKey.index_);
        return ret;
    }
...
{
    aicpusd_info(
        "dump op task data for kfc, length [%u], streamId[%u], taskId[%u], index[%u]", length, taskKey.streamId_,
        taskKey.taskId_, taskKey.index_);
    ...
    if (iter->second == nullptr) {
        aicpusd_err(
            "Get dump task failed. length [%u], streamId[%u], taskId[%u], index[%u]", length, taskKey.streamId_,
            taskKey.taskId_, taskKey.index_);
        return AICPU_SCHEDULE_ERROR_DUMP_FAILED;
    }
    ...
    if (ideSession == nullptr) {
        aicpusd_err(
            "op name[%s], call IdeDumpStart failed, path[%s]. length [%u], streamId[%u], taskId[%u], index[%u]",
            opName.c_str(), dumpFilePath.c_str(), length, taskKey.streamId_, taskKey.taskId_, taskKey.index_);
        return AICPU_SCHEDULE_ERROR_DUMP_FAILED;
    }

    return iter->second->Dump(
        dumpFilePath, PtrToPtr<void, char>(dumpData), static_cast<uint64_t>(length), ideSession, true);
}
} // namespace AicpuSchedule