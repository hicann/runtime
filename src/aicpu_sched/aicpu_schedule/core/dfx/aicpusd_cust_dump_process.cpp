...
AicpuSdCustDumpProcess::AicpuSdCustDumpProcess() : deviceId_(0U), runningFlag_(false), initFlag_(false) {}
...
int32_t AicpuSdCustDumpProcess::SetDataDumpThreadAffinity() const
{
    ...
    if (AicpuUtil::IsEnvValEqual(ENV_NAME_PROCMGR_AICPU_CPUSET, "1")) {
        ...
        if (ret != 0) {
            aicpusd_run_warn(
                "Binding datadump thread to control CPU was not successful, will run in aicpu. ret=%d, reason=%s", ret,
                strerror(errno));
        } else {
...
void AicpuSdCustDumpProcess::StartProcessEvent()
{
    ...
    if (ret != DRV_ERROR_NONE) {
        aicpusd_err(
            "halEschedCreateGrp dev[%u] group[%u] type[%d] failed, result[%d].", deviceId_, DATA_DUMP_GRUOP_ID,
            GRP_TYPE_BIND_CP_CPU, ret);
        sem_post(&workerSme_);
        return;
    }
    ...
    if (ret != DRV_ERROR_NONE) {
        aicpusd_err(
            "halEschedSubscribeEvent failed, deviceId[%u], groupId[%u], threadIndex[%u]", deviceId_, DATA_DUMP_GRUOP_ID,
            DATA_DUMP_THREAD_INDEX);
        sem_post(&workerSme_);
        return;
    }
    ProcessMessage(0U);
...
int32_t AicpuSdCustDumpProcess::ProcessMessage(const int32_t timeout)
{
    ...
    const int32_t retVal =
        halEschedWaitEvent(deviceId_, DATA_DUMP_GRUOP_ID, DATA_DUMP_THREAD_INDEX, timeout, &drvEventInfo);
    if (retVal == DRV_ERROR_NONE) {
        ...
        aicpusd_warn(
            "Failed to get event, error code=%d, deviceId[%u], groupId[%u], threadIndex[%u]", retVal, deviceId_,
            DATA_DUMP_GRUOP_ID, DATA_DUMP_THREAD_INDEX);
    } else if (retVal == DRV_ERROR_SCHED_RUN_IN_ILLEGAL_CPU) {
        runningFlag_ = false;
        aicpusd_err(
            "Cpu Illegal get event, error code[%d], deviceId[%u], groupId[%u], threadIndex[%u]", retVal, deviceId_,
            DATA_DUMP_GRUOP_ID, DATA_DUMP_THREAD_INDEX);
    } else {
        // record a error code
        aicpusd_err(
...
int32_t AicpuSdCustDumpProcess::DoCustDatadumpTask(const event_info& drvEventInfo) const
{
    ...
    aicpusd_info(
        "begin cust data dump streamid:%u, taskid:%u, threadindex:%u, custpid:%d, ackStreamId:%u", streamId, taskId,
        threadIndex, custPid, ackStreamId);
    ...
    if (dumpRet != AICPU_SCHEDULE_OK) {
        aicpusd_err(
            "cust data dump error streamid:%u, taskid:%u, threadindex:%u, custpid:%d", streamId, taskId, threadIndex,
            custPid);
    }
    ...
    aicpusd_info(
        "sendback dump event to streamid[%u], taskid[%u], custpid[%d], devId[%u]", streamId, taskId, custPid, devId);
    return AICPU_SCHEDULE_OK;
}
    ...
    const char_t* const msg, const uint32_t len, struct event_proc_result& rsp) const
{
    ...
    aicpusd_info(
        "backEvent dst[%d], pid[%d], grpId[%d], eventId[%d], subeventId[%d], msgLen[%d], deviceId[%u]",
        backEvent.dst_engine, backEvent.pid, backEvent.grp_id, backEvent.event_id, backEvent.subevent_id,
        backEvent.msg_len, deviceId);
    if (ret != 0) {
        aicpusd_err(
            "halEschedSubmitEvent, ret=%d. dst[%d],pid[%d],"
            " grpId[%d], eventId[%d], subeventId[%d], msgLen[%d], deviceId[%u]",
            ret, backEvent.dst_engine, backEvent.pid, backEvent.grp_id, backEvent.event_id, backEvent.subevent_id,
            backEvent.msg_len, deviceId);
        return ret;
    }
    return AICPU_SCHEDULE_OK;
...
int32_t AicpuSdCustDumpProcess::DoUdfDatadumpTask(const event_info& drvEventInfo) const
{
    uint32_t headLen = sizeof(struct event_sync_msg);
    aicpusd_info(
        "event id[%u], headLen[%u], msg len[%u] udfInfo len[%u]", drvEventInfo.comm.subevent_id, headLen,
    ...
    if (static_cast<size_t>(drvEventInfo.priv.msg_len) != (sizeof(AICPUDumpUdfInfo) + headLen)) {
        aicpusd_err(
            "The len[%u] is not correct[%u + %u].", drvEventInfo.priv.msg_len, sizeof(AICPUDumpUdfInfo), headLen);
        return AICPU_SCHEDULE_ERROR_PARAMETER_NOT_VALID;
    }
    ...
    UNUSED(udfPid);
    aicpusd_info(
        "udf data dump begin dumpInfo[%p], length[%llu], info->udfInfo[%llu], udfPid[%d]", dumpInfo, length,
        info->udfInfo, udfPid);
    ...
    aicpusd_info(
        "udf data dump finish dumpInfo[%p], length[%lld], udfPid[%d], deviceId[%u]", dumpInfo, length, udfPid,
        deviceId);
    return AICPU_SCHEDULE_OK;
}
...
int32_t CreateDatadumpThread(const struct TsdSubEventInfo* const msg)
{
    ...
    return AicpuSchedule::AicpuSdCustDumpProcess::GetInstance().CreateUdfDatadumpThread(message, EVENT_MAX_MSG_LEN);
}
