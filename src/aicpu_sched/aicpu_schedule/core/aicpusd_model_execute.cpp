...
StatusCode AicpuModelManager::TransModelInfo(
    const void* const ptr, AicpuModelInfo& aicpuModelInfo, std::vector<AicpuTaskInfo>& aicpuTaskInfos,
    std::vector<StreamInfo>& streamInfos, std::vector<QueInfo>& queInfos, std::vector<ModelCfgInfo>* const modelcfgs)
{
...
StatusCode AicpuModelManager::ProcessExtInfoCfgMsg(const aicpu::AicpuExtendInfo& cfgInfo)
{
    ...
    aicpusd_info(
        "Begin to ProcessExtInfoCfgMsg, msgType[%u], version[%u], model_id[%u], extend_model_id[%u].", msgType,
        cfgInfo.version, cfgInfo.modelIdMap.modelId, cfgInfo.modelIdMap.extendModelId);
...
{
    aicpusd_info(
        "Begin to process model config, version[%d], model_id[%u], extend_model_id[%u], "
        "abnormal_break[%d], abnormal_enqueue[%d], inputMsgQueue[%d], outputMsgQueue[%d].",
        cfg.version, cfg.runtimeModelId, cfg.geModelId, cfg.abnormalBreak, cfg.abnormalEnqueue, cfg.inputMsgQueue,
        cfg.outputMsgQueue);
    ...
    if (cfg.inputMsgQueue >= 0) {
        ...
        if (ret != DRV_ERROR_NONE) {
            aicpusd_err(
                "Fail to set work mode for queue[%d], ret is %d.", cfg.inputMsgQueue, static_cast<int32_t>(ret));
...
StatusCode AicpuModelManager::SetPidPriority(const AicpuPriInfo& cfg, const std::vector<uint32_t>& deviceVec)
{
    if ((cfg.pidPriority == INVALID_ESCAPE_PRI_VALUE) ||
        ...
        aicpusd_info(
            "[AicpuModelEschedPriority] the new pid priority[%d] is lower than the current[%d]", cfg.pidPriority,
            curPidPri_);
        return AICPU_SCHEDULE_OK;
    }
    ...
    aicpusd_info(
        "[AicpuModelEschedPriority] Begin to set pid priority vect size[%u], pidPriority[%d], input[%d]",
    ...
    for (size_t i = 0U; i < deviceVec.size(); i++) {
        if (cfg.pidPriority < curPidPri_ || curPidPri_ == INVALID_ESCAPE_PRI_VALUE) {
            ...
            if (ret != DRV_ERROR_NONE) {
                aicpusd_err(
                    "[AicpuModelEschedPriority]Failed to set priority [%d], deviceid[%u], result[%d].", cfg.pidPriority,
                    deviceVec[i], ret);
                return AICPU_SCHEDULE_ERROR_DRV_ERR;
            }
            aicpusd_info(
                "[AicpuModelEschedPriority] set pid priority success Index[%u], deviceid[%u]", i, deviceVec[i]);
...
StatusCode AicpuModelManager::SetEventPriority(const AicpuPriInfo& cfg, const std::vector<uint32_t>& deviceVec)
{
    if ((cfg.eventPriority == INVALID_ESCAPE_PRI_VALUE) ||
        ...
        aicpusd_info(
            "[AicpuModelEschedPriority] the new event priority[%d] is lower than the current[%d]", cfg.eventPriority,
            curEventPri_);
        return AICPU_SCHEDULE_OK;
    }
    ...
    aicpusd_info(
        "[AicpuModelEschedPriority] Begin to set event priority vecSize[%u], eventPri[%d], input[%d]", deviceVec.size(),
...
bool AicpuModelManager::IsUsed() { return AicpuModelManager::isUsed_; }
} // namespace AicpuSchedule
