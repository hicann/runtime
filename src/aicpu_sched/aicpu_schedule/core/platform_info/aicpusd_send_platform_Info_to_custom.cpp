...
void AicpuSdLoadPlatformInfoProcess::LoadPlatformInfoSemPost() { sem_post(&loadPlatformInfoProcessSem_); }
    ...
    const uint8_t* const msg, const uint32_t len) const
{
    ...
    eRet =
        memcpy_s(eventMsg + sizeof(struct event_sync_msg), EVENT_MAX_MSG_LEN - sizeof(struct event_sync_msg), msg, len);
    if (eRet != EOK) {
    ...
    if (ret != DRV_ERROR_NONE) {
        aicpusd_err(
            "halEschedSubmitEventSync failed, ret:%d, devid:%u, aicpuCustomPid:%u", ret, deviceId, aicpuCustomPid);
...
int32_t SendLoadPlatformInfoToCust(const struct TsdSubEventInfo* const msg)
{
    ...
    return AicpuSchedule::AicpuSdLoadPlatformInfoProcess::GetInstance().SendLoadPlatformInfoMessageToCustSync(
        message, len);
}
