...
{
    aicpusd_debug(
        "Begin to process drv event. eventId=%u, subEventId=%u, hostPid=%d, grpId=%u", event.comm.event_id,
        event.comm.subevent_id, event.comm.host_pid, event.comm.grp_id);
    bool needRes = false;
    ...
    aicpusd_debug(
        "End to process drv event. eventId=%u, subEventId=%u, hostPid=%d, grpId=%u", event.comm.event_id,
        event.comm.subevent_id, event.comm.host_pid, event.comm.grp_id);
    return ret;
}

int32_t AicpuQueueEventProcess::DoProcessQsMsg(
    const event_info& event, std::shared_ptr<CallbackMsg>& callback, const bqs::QsProcMsgRspDstAicpu*& qsProcMsgRsp,
    bool& isRes)
{
    int32_t ret = AICPU_SCHEDULE_OK;
    switch (event.comm.subevent_id) {
...
int32_t AicpuQueueEventProcess::ProcessQsMsg(const event_info& event)
{
    ...
    if (callback != nullptr) {
        return ResponseEvent(
            callback->event, PtrToPtr<const bqs::QsProcMsgRspDstAicpu, const char_t>(qsProcMsgRsp),
    ...
    if ((ret != AICPU_SCHEDULE_OK) && (!isRes)) {
        ...
        msg.retCode = ret;
        (void)ResponseEvent(
            event, PtrToPtr<bqs::QsProcMsgRspDstAicpu, const char_t>(&msg), sizeof(bqs::QsProcMsgRspDstAicpu));
        return ret;
    }
    return ret;
}
...
int32_t AicpuQueueEventProcess::AddCallback(uint64_t userData, std::shared_ptr<CallbackMsg>& callback)
{
    ...
    if (!ret) {
        aicpusd_err(
            "Save event callback failed, subevent_id[%u], userData[%llu]", callback->event.comm.subevent_id, userData);
        return AICPU_SCHEDULE_ERROR_ADD_CALLBACK_FAILED;
    }
    aicpusd_info(
        "Successfully added callback, subevent_id[%d], userData[%llu].", callback->event.comm.subevent_id, userData);
    return AICPU_SCHEDULE_OK;
}

int32_t AicpuQueueEventProcess::GetAndDeleteCallback(const uint64_t userData, std::shared_ptr<CallbackMsg>& callback)
{
    const std::lock_guard<std::mutex> guard(lockCallback_);
    ...
    aicpusd_info(
        "Successfully got and deleted callback, subevent_id[%u], userData[%llu].", callback->event.comm.subevent_id,
        userData);
    return AICPU_SCHEDULE_OK;
}

int32_t AicpuQueueEventProcess::CreateAndAddCallbackMsg(
    const event_info& event, Mbuf* const buff, const uint64_t userData, std::shared_ptr<CallbackMsg>& callback)
{
    try {
...
int32_t AicpuQueueEventProcess::ProcessBindQueueInit(const event_info& event)
{
    ...
    ret = SendEventToQs(PtrToPtr<bqs::QsBindInit, char_t>(&msg), sizeof(bqs::QsBindInit), bqs::AICPU_BIND_QUEUE_INIT);
    if (ret != AICPU_SCHEDULE_OK) {
...
int32_t AicpuQueueEventProcess::ProcessBindQueueInitRet(
    const event_info& event, std::shared_ptr<CallbackMsg>& callback,
    const bqs::QsProcMsgRspDstAicpu** const qsProcMsgRsp)
{
    ...
    if ((drvRet != DRV_ERROR_NONE) && (drvRet != DRV_ERROR_REPEATED_INIT)) {
        aicpusd_err("halQueueInit error, deviceId[%u], ret[%d]", AicpuDrvManager::GetInstance().GetDeviceId(), drvRet);
        return AICPU_SCHEDULE_ERROR_DRV_ERR;
    }
    drvRet = halQueueAttach(AicpuDrvManager::GetInstance().GetDeviceId(), pipelineQueueId_, 0);
    if (drvRet != DRV_ERROR_NONE) {
        aicpusd_err("Cp attach Qs queue failed, queueId[%u] ret[%d]", pipelineQueueId_, static_cast<int32_t>(drvRet));
        return AICPU_SCHEDULE_ERROR_DRV_ERR;
    }
...
int32_t AicpuQueueEventProcess::ProcessQueryQueueNum(const event_info& event)
{
    ...
    ret = SendEventToQs(
        PtrToPtr<bqs::QueueRouteQuery, char_t>(&sendMsg), sizeof(bqs::QueueRouteQuery), bqs::AICPU_QUERY_QUEUE_NUM);
    if (ret != AICPU_SCHEDULE_OK) {
        (void)GetAndDeleteCallback(userData, callback);
        return ret;
    }
...
int32_t AicpuQueueEventProcess::ProcessQsRet(
    const event_info& event, std::shared_ptr<CallbackMsg>& callback,
    const bqs::QsProcMsgRspDstAicpu** const qsProcMsgRsp)
{
    const char_t* msg = nullptr;
...
int32_t AicpuQueueEventProcess::CheckAndInitParamWithMbuf(
    const event_info& event, const bqs::QueueRouteList*& msg, bqs::QsRouteHead*& routeHead) const
{
    if (initPipeline_ != BindQueueInitStatus::INITED) {
    ...
    const uint32_t routeSize =
        static_cast<uint32_t>(sizeof(bqs::QsRouteHead) + (routeHead->routeNum * sizeof(bqs::QueueRoute)));
...
int32_t AicpuQueueEventProcess::ProcessQueueEventWithMbuf(
    const event_info& event, const bqs::QueueSubEventType drvSubeventId)
{
    ...
    if (drvSubeventId == bqs::AICPU_BIND_QUEUE) {
        const bqs::QueueRoute* const queueRoute =
            PtrToPtr<void, bqs::QueueRoute>(ValueToPtr(msg->routeListMsgAddr + sizeof(bqs::QsRouteHead)));
    ...
    ret = SendEventToQs(
        PtrToPtr<bqs::QueueRouteList, char_t>(&msgToQs), sizeof(bqs::QueueRouteList),
        bqs::AICPU_QUEUE_RELATION_PROCESS);
    if (ret != AICPU_SCHEDULE_OK) {
        AicpuMonitor::GetInstance().SendKillMsgToTsd();
        return ret;
    }
...
int32_t AicpuQueueEventProcess::SendEventToQs(
    char_t* const msg, const size_t msgLen, const bqs::QueueSubEventType drvSubeventId) const
{
    event_summary sched = {};
    sched.pid = qsPid_;
...
int32_t AicpuQueueEventProcess::ParseQueueEventMessage(
    const event_info& event, const char_t*& msg, const size_t msgSize, const bool isSyncEvent) const
{
    size_t offset = 0U;
    if (isSyncEvent) {
    ...
    if (event.priv.msg_len != (msgSize + offset)) {
        aicpusd_err("The len[%u] is not equal to sizeof(msg)[%zu] + [%zu].", event.priv.msg_len, msgSize, offset);
        return AICPU_SCHEDULE_ERROR_PARAMETER_NOT_VALID;
    }
...
int32_t AicpuQueueEventProcess::ResponseEvent(const event_info& event, const char_t* const msg, const size_t len) const
{
    aicpusd_info("Begin to response event event_id[%u], subevent_id[%u].", event.comm.event_id, event.comm.subevent_id);
    ...
    if (drvRet != DRV_ERROR_NONE) {
        aicpusd_err(
            "Failed to response event to acl event_id[%u], subevent_id[%u].", event.comm.event_id,
            event.comm.subevent_id);
        return AICPU_SCHEDULE_ERROR_DRV_ERR;
    }
    aicpusd_info(
        "Successfully responded to event, event_id[%u], subevent_id[%u].", event.comm.event_id, event.comm.subevent_id);
    return AICPU_SCHEDULE_OK;
}

int32_t AicpuQueueEventProcess::ProcessQsRetWithMbuf(
    const event_info& event, std::shared_ptr<CallbackMsg>& callback,
    const bqs::QsProcMsgRspDstAicpu** const qsProcMsgRsp)
{
    ...
    const auto drvRet = halQueueDeQueue(
        AicpuDrvManager::GetInstance().GetDeviceId(), pipelineQueueId_, PtrToPtr<Mbuf*, void*>(&resultMbuf));
...
int32_t AicpuQueueEventProcess::CopyResult(const std::shared_ptr<CallbackMsg>& callback) const
{
    ...
    const errno_t memRet =
        memcpy_s(routeHead, static_cast<size_t>(routeHead->length), mbufData, static_cast<size_t>(mbufSize));
    if (memRet != EOK) {
...
int32_t AicpuQueueEventProcess::AddQueueAuthToQs(const bqs::QueueRoute* const queueRoute, const uint32_t routeNum)
{
    ...
    for (auto iter = srcQueuesId.begin(); iter != srcQueuesId.end(); ++iter) {
        ...
        const int32_t drvRet =
            halQueueGrant(AicpuDrvManager::GetInstance().GetDeviceId(), static_cast<int32_t>(*iter), qsPid_, attr);
        if ((drvRet != DRV_ERROR_NONE) && (drvRet != DRV_ERROR_REPEATED_INIT)) {
...
int32_t AicpuQueueEventProcess::AllocMbufAndEnqueue(
    const bqs::QsRouteHead* const data, const size_t size, Mbuf** const buff)
{
...
int32_t AicpuQueueEventProcess::CreateGroupForMaster(
    std::string& outGroupName, const char_t* const inGroupName, const uint64_t size, const uint32_t allocFlag)
{
    ...
    if (drvRet != DRV_ERROR_NONE) {
        aicpusd_err("Add group[%s] for master aicpusd[%d] failed, ret[%d]", groupName.c_str(), curPid_, drvRet);
        return drvRet;
    }
    drvRet = halGrpAttach(groupName.c_str(), 0);
    if (drvRet != DRV_ERROR_NONE) {
        aicpusd_err("Attach group[%s] for master aicpusd[%d] failed, ret[%d]", groupName.c_str(), curPid_, drvRet);
        return drvRet;
    }
...
int32_t AicpuQueueEventProcess::AttachGroupForSlave(
    const std::map<std::string, GroupShareAttr>& grpInfos, std::string& outGroupName)
{
    // only one group
    const uint32_t groupNum = grpInfos.size();
    if (groupNum != 1U) {
        aicpusd_err("Slave aicpusd[%d] should own only one group rather than [%u]", curPid_, groupNum);
...
int32_t AicpuQueueEventProcess::GrantQueue(const event_info& event)
{
    ...
    const int32_t drvRet =
        halQueueGrant(grantPara->devid, static_cast<int32_t>(grantPara->qid), grantPara->pid, grantPara->attr);
    if (drvRet != DRV_ERROR_NONE) {
        aicpusd_err(
            "Fail to add queue[%d] authority for aicpusd[%d], result[%d].", grantPara->qid, grantPara->pid, drvRet);
        return drvRet;
    }
...
int32_t AicpuQueueEventProcess::ProcessProxyMsg(const event_info& event)
{
    ...
    const int32_t resRet = ResponseEvent(event, PtrToPtr<ProxyMsgRsp, const char_t>(&rsp), sizeof(ProxyMsgRsp));
    return resRet;
}
...
int32_t AicpuQueueEventProcess::ProxyCreateGroup(const event_info& event)
{
    ...
    if (createGroupMsg->allocSize >= 0) {
        ...
        allocPar.memSize =
            (createGroupMsg->allocSize == 0) ? createGroupMsg->size : static_cast<uint64_t>(createGroupMsg->allocSize);
...
int32_t AicpuQueueEventProcess::ProxyCopyQMbuf(const event_info& event) const
{
    ...
    Mbuf* mbuf = nullptr;
    const auto drvRet =
        halQueueDeQueue(AicpuDrvManager::GetInstance().GetDeviceId(), queueId, PtrToPtr<Mbuf*, void*>(&mbuf));
    ...
    if (eRet != EOK) {
        aicpusd_err("Data copy failed. dstAddrLen[%lu], dataSize[%lu], ret[%d]", destLen, dataLen, eRet);
...
int32_t AicpuQueueEventProcess::ProxyAddGroup(const event_info& event) const
{
    ...
    const GroupShareAttr attrForGroup = {
        addGroupMsg->admin, addGroupMsg->read, addGroupMsg->write, addGroupMsg->alloc, 0U};
    ...
    if ((ret != static_cast<int32_t>(DRV_ERROR_NONE)) && (ret != static_cast<int32_t>(DRV_ERROR_REPEATED_INIT))) {
        aicpusd_err("Add group[%s] for proxy process [%d] failed, ret[%d]", groupName, pid, ret);
        return AICPU_SCHEDULE_ERROR_DRV_ERR;
    }
...
int32_t AicpuQueueEventProcess::ProxyAllocCache(const event_info& event) const
{
    ...
    aicpusd_info(
        "Proxy alloc cache [%llu:%u] in group[%s] success", allocCacheMsg->memSize, allocCacheMsg->allocMaxSize,
        grpName_.c_str());
    return AICPU_SCHEDULE_OK;
}
...
int32_t AicpuQueueEventProcess::DoAllocCache(const char_t* const groupName, GrpCacheAllocPara* const allocPar) const
{
    ...
    if (allocRet != DRV_ERROR_NONE) {
        aicpusd_err(
            "Failed to allocate group [%s], size[%llu]KB, allocMaxSize[%u], ret is %d.", groupName, allocPar->memSize,
            allocPar->allocMaxSize, static_cast<int32_t>(allocRet));
        return AICPU_SCHEDULE_ERROR_DRV_ERR;
    }
    return AICPU_SCHEDULE_OK;
}
