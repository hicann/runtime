...
int32_t AicpuDrvManager::GetNormalAicpuDCpuInfo(const std::vector<uint32_t>& deviceVec)
{
    ...
    uint32_t retDrv =
        static_cast<uint32_t>(AicpuGetDeviceInfo(deviceIdTmp, MODULE_TYPE_AICPU, INFO_TYPE_CORE_NUM, &aicpuNum));
    retDrv |=
        static_cast<uint32_t>(AicpuGetDeviceInfo(deviceIdTmp, MODULE_TYPE_AICPU, INFO_TYPE_OS_SCHED, &aicpuOsSched));
    retDrv |= static_cast<uint32_t>(AicpuGetDeviceInfo(deviceIdTmp, MODULE_TYPE_CCPU, INFO_TYPE_CORE_NUM, &ccpuNum));
    retDrv |=
        static_cast<uint32_t>(AicpuGetDeviceInfo(deviceIdTmp, MODULE_TYPE_CCPU, INFO_TYPE_OS_SCHED, &ccpuOsSched));
    retDrv |= static_cast<uint32_t>(AicpuGetDeviceInfo(deviceIdTmp, MODULE_TYPE_DCPU, INFO_TYPE_CORE_NUM, &dcpuNum));
    retDrv |=
        static_cast<uint32_t>(AicpuGetDeviceInfo(deviceIdTmp, MODULE_TYPE_DCPU, INFO_TYPE_OS_SCHED, &dcpuOsSched));
    retDrv |= static_cast<uint32_t>(AicpuGetDeviceInfo(deviceIdTmp, MODULE_TYPE_TSCPU, INFO_TYPE_CORE_NUM, &tsCpuNum));
    retDrv |=
        static_cast<uint32_t>(AicpuGetDeviceInfo(deviceIdTmp, MODULE_TYPE_TSCPU, INFO_TYPE_OS_SCHED, &tsCpuOsSched));
    if (retDrv != 0U) {
    ...
    // connot overflow
    coreNumPerDev_ =
        (ccpuNum * ccpuOsSched) + (dcpuNum * dcpuOsSched) + (aicpuNum * aicpuOsSched) + (tsCpuNum * tsCpuOsSched);
    aicpusd_run_info(
        "GetNormalAicpuDCpuInfo, deviceId[%u], aicpu_num[%u], aicpu_os_sched[%u], ccpu_num[%u], "
    ...
    const uint32_t deviceId, const DEV_MODULE_TYPE moduleType, const DEV_INFO_TYPE infoType, int64_t* value) const
{
    if (value == nullptr) {
        aicpusd_err(
            "halGetDeviceInfo value is null, deviceId[%u], DEV_MODULE_TYPE[%d], DEV_INFO_TYPE[%d]", deviceId,
            static_cast<int32_t>(moduleType), static_cast<int32_t>(infoType));
        return -1;
    }
    const auto ret =
        halGetDeviceInfo(deviceId, static_cast<int32_t>(moduleType), static_cast<int32_t>(infoType), value);
    if (ret != DRV_ERROR_NONE) {
        aicpusd_err(
            "halGetDeviceInfo failed, ret[%d], deviceId[%u] DEV_MODULE_TYPE[%d], DEV_INFO_TYPE[%d], "
            "value[%lld]",
            ret, deviceId, static_cast<int32_t>(moduleType), static_cast<int32_t>(infoType), *value);
    }
...
int32_t AicpuDrvManager::GetThreadVfAicpuDCpuInfo(const std::vector<uint32_t>& deviceVec)
{
    ...
    int64_t aicpuNum = 0L;
    uint32_t ret =
        static_cast<uint32_t>(AicpuGetDeviceInfo(deviceId, MODULE_TYPE_AICPU, INFO_TYPE_CORE_NUM, &aicpuNum));
    int64_t aicpuOsSched = 0;
    ret |= static_cast<uint32_t>(AicpuGetDeviceInfo(deviceId, MODULE_TYPE_AICPU, INFO_TYPE_OS_SCHED, &aicpuOsSched));
    int64_t ccpuNum = 0;
    ret |= static_cast<uint32_t>(AicpuGetDeviceInfo(deviceId, MODULE_TYPE_CCPU, INFO_TYPE_CORE_NUM, &ccpuNum));
    int64_t ccpuOsSched = 0;
    ret |= static_cast<uint32_t>(AicpuGetDeviceInfo(deviceId, MODULE_TYPE_CCPU, INFO_TYPE_OS_SCHED, &ccpuOsSched));
    int64_t dcpuNum = 0;
    ret |= static_cast<uint32_t>(AicpuGetDeviceInfo(deviceId, MODULE_TYPE_DCPU, INFO_TYPE_CORE_NUM, &dcpuNum));
    int64_t dcpuOsSched = 0;
    ret |= static_cast<uint32_t>(AicpuGetDeviceInfo(deviceId, MODULE_TYPE_DCPU, INFO_TYPE_OS_SCHED, &dcpuOsSched));
    int64_t tsCpuNum = 0;
    ret |= static_cast<uint32_t>(AicpuGetDeviceInfo(deviceId, MODULE_TYPE_TSCPU, INFO_TYPE_CORE_NUM, &tsCpuNum));
    int64_t tsCpuOsSched = 0;
    ret |= static_cast<uint32_t>(AicpuGetDeviceInfo(deviceId, MODULE_TYPE_TSCPU, INFO_TYPE_OS_SCHED, &tsCpuOsSched));
    if (ret != 0U) {
    ...
    aicpusd_info(
        "GetThreadVfAicpuDCpuInfo, deviceId[%u], aicpuNum[%llu], aicpuOsSched[%llu], ccpuNum[%llu], "
...
int32_t AicpuDrvManager::InitDrvMgr(
    const std::vector<uint32_t>& deviceVec, const pid_t hostPid, const uint32_t vfId, const bool hasThread,
    const aicpu::AicpuRunMode runMode, const std::string& hostProcName)
{
    ...
    aicpusd_run_info(
        "host pid[%d], host proc name[%s], vf id[%u], first aicpu index[%u], aicpu num[%u],"
...
int32_t AicpuDrvManager::InitDrvSchedModule(const uint32_t grpId, const std::map<EVENT_ID, SCHEDULE_PRIORITY>& priority)
{
    ...
    for (size_t i = 0LU; i < deviceNum_; i++) {
        ...
        if (ret != DRV_ERROR_NONE) {
            ...
            aicpusd_err(
                "Failed to create group in drv, result[%d], group[%u], device[%d], type[%d].", ret, grpId,
                deviceVec_[i], cpuGrpType);
            return AICPU_SCHEDULE_ERROR_INIT_FAILED;
        }
...
int32_t AicpuDrvManager::SubscribeQueueNotEmptyEvent(const uint32_t queueId) const
{
    ...
    aicpusd_info(
        "Begin to subscribe non-empty event, deviceId[%u], queueId[%u], groupId[%u].", deviceId, queueId, groupId_);
    ...
    aicpusd_info(
        "End to subscribe non-empty event, deviceId[%u], queueId[%u], groupId[%u].", deviceId, queueId, groupId_);
    return AICPU_SCHEDULE_OK;
}
...
int32_t AicpuDrvManager::SubscribeQueueNotFullEvent(const uint32_t queueId) const
{
    const uint32_t deviceId = GetDeviceId();
    aicpusd_info(
        "Begin to subscribe not full event, deviceId[%u], queueId[%u], groupId[%u].", deviceId, queueId, groupId_);
    ...
    aicpusd_info(
        "End to subscribe not full event, deviceId[%u], queueId[%u], groupId[%u].", deviceId, queueId, groupId_);
    return AICPU_SCHEDULE_OK;
}
...
int32_t AicpuDrvManager::BindHostPid(
    const std::string& signStr, const int32_t mode, const uint32_t vfId, const enum devdrv_process_type cpType) const
{
    aicpusd_info(
    ...
    for (size_t i = 0LU; i < deviceNum_; i++) {
        ...
        if (memRet != EOK) {
            aicpusd_err(
                "memcpy ret[%d], pid[%d], device id[%u], cp type[%d], mode[%d]", memRet, hostPid_, deviceVec_[i],
                cpType, mode);
            return AICPU_SCHEDULE_ERROR_INNER_ERROR;
        }
        ...
        if (drvRet != DRV_ERROR_NONE) {
            aicpusd_err(
                "Call drvBindHostPid failed, ret[%d] device id[%u], host pid[%d], mode[%d], cp type[%d],"
                " vf id[%u]",
                drvRet, deviceVec_[i], hostPid_, mode, cpType, vfId);
...
int32_t AicpuDrvManager::CheckBindHostPid() const
{
    ...
    for (uint32_t i = 0; i < QUERY_BIND_HOST_PID_TIME / QUERY_BIND_HOST_PID_INTERVBALE; i++) {
        ...
        if ((i % QUERY_BIND_HOST_PID_LOG_INTERVAL) == 0U) {
            aicpusd_run_info(
                "Query process host pid end, ret=%d, hostpid=%u, expect=%u, cpType=%u", static_cast<int32_t>(ret),
                hostpid, hostPid_, cpType);
        }
        ...
        if (hostPid_ == static_cast<pid_t>(hostpid)) {
            ...
            aicpusd_err(
                "CheckBindHostPid failed, hostpid not right. ret[%d], pid[%d], hostpid[%d], cpType[%d]",
                static_cast<int32_t>(ret), pid, hostpid, cpType);
            return AICPU_SCHEDULE_ERROR_DRV_ERR;
        }
    }
    aicpusd_err(
        "CheckBindHostPid failed, try timeout. ret[%d], pid[%d], hostpid[%d], cpType[%d]", static_cast<int32_t>(ret),
        pid, hostpid, cpType);
    return AICPU_SCHEDULE_ERROR_DRV_ERR;
}
...
int32_t AicpuDrvManager::QueryProcBuffInfo(uint32_t procId, std::map<std::string, GroupShareAttr>& buffGrpInfo) const
{
    ...
    const auto drvRet = halGrpQuery(
        GRP_QUERY_GROUPS_OF_PROCESS, &procId, static_cast<uint32_t>(sizeof(procId)),
