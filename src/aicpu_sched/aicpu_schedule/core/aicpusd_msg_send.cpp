...
void AicpuMsgSend::SetTsDevSendMsgAsync(
...
void AicpuMsgSend::SendEvent()
{
    ...
    if (g_sendType == AICPUSendType::TS_DEV_SEND) {
        ...
        for (uint32_t index = 0; index < TSDEV_SEND_MSG_ASYNC_RETRY_NUM; index++) {
            ...
            if (ret == DRV_ERROR_NONE) {
                aicpusd_info(
                    "Send async msg to ts success, deviceId=%u, tsId=%u", g_tsDevSendMsgAsync.deviceId,
                    g_tsDevSendMsgAsync.tsId);
                g_sendType = AICPUSendType::NO_NEED_SEND;
                return;
            } else {
                aicpusd_run_info(
                    "Sending async msg to ts was not successful. ret=%d, deviceId=%u, tsId=%u, retry times=%d", ret,
                    g_tsDevSendMsgAsync.deviceId, g_tsDevSendMsgAsync.tsId, index);
            }
            (void)usleep(TSDEV_SEND_MSG_ASYNC_RETRY_INTERVAL);
        }
        if (ret != DRV_ERROR_NONE) {
            aicpusd_err(
                "After retry, Send async msg to ts failed. ret=%d, deviceId=%u, tsId=%u", ret,
                g_tsDevSendMsgAsync.deviceId, g_tsDevSendMsgAsync.tsId);
        }
        return;
    }
...
int32_t AicpuMsgSend::SendAICPUSubEvent(
    ...
    const bool syncSendFlag)
{
    ...
    DeployContext deployCtx = DeployContext::DEVICE;
    (void)GetAicpuDeployContext(deployCtx);
    if ((deployCtx == DeployContext::HOST) || (subEventId == static_cast<uint32_t>(AICPU_SUB_EVENT_ACTIVE_MODEL))) {
...
{
    aicpusd_err(
        "Send aicpu notify info to ts, streamId=%u, taskId=%lu, retcode=%u.", notifyInfo.streamId, notifyInfo.taskId,
        notifyInfo.retCode);

    AicpuSqeAdapter aicpuSqeAdapter(FeatureCtrl::GetTsMsgVersion());
    AicpuSqeAdapter::AicpuRecordInfo recordInfo(
        notifyInfo.waitId, notifyInfo.waitType, notifyInfo.retCode, static_cast<uint8_t>(notifyInfo.ctx.tsId),
        notifyInfo.taskId, notifyInfo.streamId, notifyInfo.ctx.deviceId);
    const int32_t ret = aicpuSqeAdapter.AicpuRecordResponseToTs(recordInfo);
    if (ret != AICPU_SCHEDULE_OK) {
        aicpusd_err(
            "AicpuRecordResponseToTs failed. ret=%d, streamId=%u, taskId=%lu.", ret, notifyInfo.streamId,
            notifyInfo.taskId);
    }
}
...
void AicpuMsgSend::SendAicpuRecordMsg(const void* const notifyParam, const uint32_t paramLen)
{
    if ((notifyParam == nullptr) || (paramLen != sizeof(aicpu::AsyncNotifyInfo))) {
        aicpusd_err(
            "NotifyEventWait param is null or paramLen not matched. paramLen = %u, AsyncNotifyInfo size = %zu",
    ...
    if ((notifyInfo->retCode != 0U) && (notifyInfo->retCode != AICPU_KERNEL_END_OF_SEQUENCE_FLAG)) {
        ...
        aicpusd_info(
            "NotifyEventWait, wait_type[%u], wait_id[%u], task_id[%lu], stream_id[%u], ret_code[%u].",
            notifyInfo->waitType, notifyInfo->waitId, notifyInfo->taskId, notifyInfo->streamId, notifyInfo->retCode);
    }
    ...
    retCode = retSucc ? (notifyInfo->retCode) :
                        (isRetEndSequence ? static_cast<uint16_t>(TS_ERROR_END_OF_SEQUENCE) :
                                            static_cast<uint16_t>(TS_ERROR_AICPU_EXCEPTION));
    AicpuSqeAdapter aicpuSqeAdapter(FeatureCtrl::GetTsMsgVersion());
    AicpuSqeAdapter::AicpuRecordInfo recordInfo(
        notifyInfo->waitId, notifyInfo->waitType, retCode, static_cast<uint8_t>(notifyInfo->ctx.tsId),
        notifyInfo->taskId, notifyInfo->streamId, notifyInfo->ctx.deviceId);
    sendRet = aicpuSqeAdapter.AicpuRecordResponseToTs(recordInfo);
    if (sendRet != static_cast<uint32_t>(AICPU_SCHEDULE_OK)) {
        aicpusd_err(
            "Send event notify to ts failed, sendRet=%d, notifyId=%u, retVal=%d.", sendRet, notifyInfo->waitId,
            notifyInfo->retCode);
        return;
    }
    aicpusd_info(
        "send event notify to ts success, notifyId=%u, hostpid=%u, retCode=%d.", notifyInfo->waitId,
