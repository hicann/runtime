...
Mbuf* BufManager::MallocBuf(const uint32_t allocSize) { return MallocBufU64(static_cast<uint64_t>(allocSize)); }
...
Mbuf* BufManager::MallocBufU64(const uint64_t allocSize)
{
    ...
    drvRet =
        halMbufAllocEx(static_cast<uint64_t>(allocSize), MBUF_ALLOC_ALIGN_SIZE, flag, MBUF_ALLOC_DEFAULT_GRP_ID, &mbuf);
...
int32_t BufManager::MallocAndAppend(
...
int32_t BufManager::MallocAndGuardBufList(
...
bool EventWaitManager::CheckEvent(const bool eventStateNeedCheck, const bool waitStreamNeedCheck, const size_t length)
{
    const std::unique_lock<std::mutex> lk(waitMutex_);
...
void ModelStreamManager::Reg(const uint32_t modelId, const std::vector<StreamInfo>& streams)
{
    ...
    for (const auto& stream : streams) {
        const auto& ret =
            streamInfos_.emplace(stream.streamID, std::pair<uint32_t, uint32_t>(modelId, stream.streamFlag));
        if (!ret.second) {
            aicpusd_err(
                "Reg stream failed, streamId=%u, modelId=%u, streamFlag=%u", stream.streamID, modelId,
                stream.streamFlag);
        } else {
            aicpusd_info(
                "Reg stream success, streamId=%u, modelId=%u, streamFlag=%u, size=%lu", stream.streamID, modelId,
                stream.streamFlag, streamInfos_.size());
        }
    }
    ...
    for (const auto& stream : streams) {
        ...
        if (iter != streamInfos_.end()) {
            if (iter->second.first == modelId) {
                aicpusd_info(
                    "UnReg stream success, streamId=%u, modelId=%u, size=%lu", stream.streamID, modelId,
                    streamInfos_.size());
                (void)streamInfos_.erase(stream.streamID);
            } else {
                aicpusd_warn(
                    "UnReg stream[%u] failed, as param modelId[%u] but stream modelId[%u].", stream.streamID, modelId,
                    iter->second.first);
            }
        }
    }
