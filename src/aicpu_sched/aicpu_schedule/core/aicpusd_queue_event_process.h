...
private:
    AicpuQueueEventProcess()
        : initPipeline_(BindQueueInitStatus::UNINIT),
          qsPid_(0),
          pipelineQueueId_(0U),
          type_(CpType::MASTER),
          curPid_(drvDeviceGetBareTgid())
    {}
     ...
     * @return AICPU_SCHEDULE_OK: success
     */
    int32_t ProcessBindQueueInitRet(
     ...
     * @return AICPU_SCHEDULE_OK: success
     */
    int32_t SendEventToQs(char_t* const msg, const size_t msgLen, const bqs::QueueSubEventType drvSubeventId) const;
     ...
     * @return AICPU_SCHEDULE_OK: success
     */
    int32_t CreateGroupForMaster(
        std::string& outGroupName, const char_t* const inGroupName = nullptr, const uint64_t size = 0U,
        const uint32_t allocFlag = 0U);
     ...
     * @return AICPU_SCHEDULE_OK: success
     */
    int32_t AttachGroupForSlave(const std::map<std::string, GroupShareAttr>& grpInfos, std::string& outGroupName);
     ...
     * @return AICPU_SCHEDULE_OK: success
     */
    int32_t CreateAndAddCallbackMsg(
     ...
     * @return AICPU_SCHEDULE_OK: success
     */
    int32_t CheckAndInitParamWithMbuf(
     ...
     * @return AICPU_SCHEDULE_OK: success
     */
    int32_t ParseQueueEventMessage(
        const event_info& event, const char_t*& msg, const size_t msgSize, const bool isSyncEvent = false) const;
