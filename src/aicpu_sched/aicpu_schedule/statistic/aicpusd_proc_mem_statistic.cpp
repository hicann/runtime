...
bool AicpuSdProcMemStatistic::GetSvmInfoFromFile(uint64_t& svmValue)
{
    ...
    if (!inFile) {
        aicpusd_info(
            "Opening svmMem file was not successful, pid=%d, errno=%d, reason:%s", static_cast<int32_t>(curPid_), errno,
            strerror(errno));
        return false;
    }
...
bool AicpuSdProcMemStatistic::GetXsMemInfoFromFile(uint64_t& xsMemValue)
{
    ...
    if (!inFile) {
        aicpusd_info(
            "Opening xsMem file was not successful, pid=%d, errno=%d, reason:%s", static_cast<int32_t>(curPid_), errno,
            strerror(errno));
        return false;
    }
    const ScopeGuard fileGuard([&inFile]() { inFile.close(); });
    ...
    if (ret < XSMEM_VALUE_CNT) {
        aicpusd_warn(
            "read summaryStr:%s failed, ret:%d, allocsize:%llu, real size:%llu", summaryStr.c_str(), ret, allocSize,
            realSize);
        return false;
    }
...
bool AicpuSdProcMemStatistic::GetOsMemInfoFromFile(uint64_t& rssValue, uint64_t& hwmValue)
{
    ...
    if (!ifFile) {
        aicpusd_info(
            "Opening rss file was not successful, pid=%d, errno=%d, reason:%s", static_cast<int32_t>(curPid_), errno,
            strerror(errno));
        return false;
    }
