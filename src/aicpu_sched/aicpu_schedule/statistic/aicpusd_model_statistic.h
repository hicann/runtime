...
public:
    ...
    void StatNNModelInput(
        const uint32_t modelId, const std::vector<uint64_t>& totalSizeList,
    ...
    void StatNNModelOutput(const uint32_t modelId, const RuntimeTensorDesc* const tensorDesc, const int32_t nextIndex);

private:
    void DumpOutModelMetrics(const uint32_t modelId);
    void AddNewDataToStatVec(
        std::vector<InAndOutParamStatInfo>& delVec, const uint64_t totalSize, const int64_t* shapes) const;
    void UpdateStatVecInfo(InAndOutParamStatInfo& dstStat, const uint64_t totalSize, const int64_t* shapes) const;
