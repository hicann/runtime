...
std::string AicpuSdModelStatistic::GetRangeString(
    const std::vector<InAndOutParamStatInfo>& statInfos, size_t startIdx, size_t endIdx) const
{
    ...
    strStream << "min_size=[" << minSizeStream.str() << "], max_size=[" << maxSizeStream.str() << "], min_shape=["
              << minShapeStream.str() << "], max_shape=[" << maxShapeStream.str() << "]";
...
void AicpuSdModelStatistic::StatNNModelExecTime(const uint32_t modelId)
{
    ...
    AicpuUtil::UpdateMaxAndSubMaxData(
...
void AicpuSdModelStatistic::UpdateStatVecInfo(
    InAndOutParamStatInfo& dstStat, const uint64_t totalSize, const int64_t* shapes) const
{
...
void AicpuSdModelStatistic::AddNewDataToStatVec(
    std::vector<InAndOutParamStatInfo>& delVec, const uint64_t totalSize, const int64_t* shapes) const
{
    ...
    aicpusd_info(
        "max size:%llu, min size:%llu, min shape size:%zu, max shape size:%zu", tempNode.maxSize, tempNode.minSize,
        tempNode.minShape.size(), tempNode.maxShape.size());
    delVec.emplace_back(tempNode);
}

void AicpuSdModelStatistic::StatNNModelInput(
    const uint32_t modelId, const std::vector<uint64_t>& totalSizeList, std::vector<ModelConfigTensorDesc>& curStatVec)
{
    if ((modelId >= MAX_MODEL_COUNT) || (totalSizeList.size() != curStatVec.size())) {
        aicpusd_err(
            "invalid modelId:%u, max:%u, size list:%zu, stat list:%zu", modelId, MAX_MODEL_COUNT, totalSizeList.size(),
            curStatVec.size());
        return;
    }
    ...
    for (size_t index = 0; index < curMinSize; index++) {
        ...
        aicpusd_info(
            "index:%zu max size:%llu, min size:%llu, min shape size:%zu, max shape size:%zu", index,
            delVec[index].maxSize, delVec[index].minSize, delVec[index].minShape.size(), delVec[index].maxShape.size());
    }
...
void AicpuSdModelStatistic::StatNNModelOutput(
    const uint32_t modelId, const RuntimeTensorDesc* const tensorDesc, const int32_t nextIndex)
{
    ...
    const int32_t ret =
        AicpuUtil::CalcDataSizeByShape(&(tensorDesc->shape[1]), tensorDesc->shape[0], tensorDesc->dtype, curTotalSize);
    ...
    if (curIndex < curDelSize) {
        ...
        aicpusd_info(
            "max size:%llu, min size:%llu, min shape size:%zu, max shape size:%zu", delVec[curIndex].maxSize,
            delVec[curIndex].minSize, delVec[curIndex].minShape.size(), delVec[curIndex].maxShape.size());
...
void AicpuSdModelStatistic::DumpOutModelMetrics(const uint32_t modelId)
{
    ...
    aicpusd_run_info(
        "model_metrics:name=%s, min_exec_time=%llu us, max_exec_time=%llu us, "
        "sub_max_exec_time=%llu us, total_exec_time=%llu us, total_exec_num=%llu.",
        std::to_string(modelId).c_str(), curModeInfo.minExecTime, curModeInfo.maxExecTime, curModeInfo.subMaxExecTime,
        curModeInfo.totalTime, curModeInfo.totalCnt);
    ...
    for (size_t inputIdx = 0; inputIdx < inputSize; inputIdx += maxCntPrintPreLine) {
        ...
        const size_t endIdx =
            (inputIdx + maxCntPrintPreLine) > inputSize ? (inputSize - 1) : (inputIdx + maxCntPrintPreLine - 1);
        aicpusd_run_info(
            "model_metrics:name=%s, input[%zu-%zu], %s.", std::to_string(modelId).c_str(), startIdx, endIdx,
            GetRangeString(curModeInfo.inputStatVec, startIdx, endIdx).c_str());
    }

    for (size_t outputIdx = 0; outputIdx < outputSize; outputIdx += maxCntPrintPreLine) {
        const size_t startIdx = outputIdx;
        const size_t endIdx =
            (outputIdx + maxCntPrintPreLine) > outputSize ? (outputSize - 1) : (outputIdx + maxCntPrintPreLine - 1);
        aicpusd_run_info(
            "model_metrics:name=%s, output[%zu-%zu], %s.", std::to_string(modelId).c_str(), startIdx, endIdx,
            GetRangeString(curModeInfo.outputStatVec, startIdx, endIdx).c_str());
    }
}
} // namespace AicpuSchedule