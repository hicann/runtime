...
public:
    ...
    void AicpuMesCountInc() { aicpuMesCount++; }

    void DpMesCountInc() { dpMesCount++; }

    void ModelMesCountInc() { modelMesCount++; }

    void MiMesCountInc() { miMesCount++; }
...
private:
    ...
    {
        AICPU_LOG_INFO(
            "The total data is: aicpu[%llu], dp[%llu], model[%llu], aicpumi[%llu]", aicpuMesCount, dpMesCount,
            modelMesCount, miMesCount);
    }
};
...
public:
    ...
    bool IsProfApiSo() const { return isProfApiSo_; }
