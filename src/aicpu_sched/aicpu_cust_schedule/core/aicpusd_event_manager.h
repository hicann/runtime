...
private:
    ...
    int32_t HWTSKernelEventMessageParse(
        const event_info& eventInfo, uint32_t& mailboxId, aicpu::HwtsTsKernel& info, uint64_t& serialNo,
     ...
     * @return AICPU_SCHEDULE_OK: success, other: error code
     */
    int32_t HWTSConverMsgFromDriver2Aicpu(
    ...
    int32_t ExecuteHWTSEventTask(
        const EventInfoForExecute& executeEventInfo, aicpu::HwtsTsKernel& aicpufwKernelInfo,
        const event_info& drvEventInfo, hwts_response_t& hwtsResponse) const;
    /**
     * @ingroup AicpuEventManager
