    ...
    const uint64_t drvSchedTick, const uint64_t streamId, const uint64_t taskId)
{
    ...
    if (retAicpu != AICPU_SCHEDULE_OK) {
        ...
        aicpusd_err(
            "Execute aicpu custom kernel failed, result[%d], opName[%s], streamId[%llu], taskId[%llu].", retAicpu,
            opName.c_str(), streamId, taskId);
    }
...
int32_t AicpuEventProcess::AICPUEventBindSdPid(const event_info_priv& privEventInfo) const
{
    if (privEventInfo.msg_len != sizeof(AICPUBindSdPidEventMsg)) {
        aicpusd_err(
            "The event msg len[%u] is not correct[%zu].", privEventInfo.msg_len, sizeof(AICPUBindSdPidEventMsg));
        return AICPU_SCHEDULE_ERROR_PARAMETER_NOT_VALID;
    }
    const AICPUBindSdPidEventMsg* const info = PtrToPtr<const char_t, const AICPUBindSdPidEventMsg>(privEventInfo.msg);
    ...
    if (AicpuSchedule::AicpuDrvManager::GetInstance().GetSafeVerifyFlag()) {
        ...
        // new
        drvRet =
            halMemBindSibling(hostPid, info->pid, vfId, deviceId, SVM_MEM_BIND_SVM_GRP | SVM_MEM_BIND_SP_GRP_NO_ALLOC);
    }
    if (drvRet != DRV_ERROR_NONE) {
        aicpusd_err(
            "Failed to halMemBindSibling, hostpid=%d, aicpuSdPid=%d, deviceId=%u, ret=%d.\n", hostPid, info->pid,
            deviceId, drvRet);
        return drvRet;
    }
    aicpusd_info("Bind Sibling pid success, hostpid=%d, aicpuSdPid=%d, deviceId=%u.", hostPid, info->pid, deviceId);
    return AICPU_SCHEDULE_OK;
}
...
int32_t AicpuEventProcess::AICPUEventCustUpdateProfilingMode(const event_info_priv& privEventInfo) const
{
    ...
    aicpusd_info("Update aicpu profiling mode success, profilingMode[%u], kernelFlag[%d]", profilingMode, kernelFlag);
...
int32_t AicpuEventProcess::SendCtrlCpuMsg(
