...
namespace {
constexpr uint32_t EVENT_MASK = (static_cast<uint32_t>(1U) << static_cast<uint32_t>(EVENT_RANDOM_KERNEL)) |
...
ThreadPool::ThreadPool() : semInitedNum_(0U) {}
...
int32_t ThreadPool::SecureCompute(const uint32_t threadIndex)
{
    ...
    for (auto filterSystemCall : filterSystemCalls) {
        ...
        if (ret != 0) {
            ...
            aicpusd_err(
                "Add the system call failed, thread threadIndex[%u], ret[%d],"
                " syscall number[%d].",
...
void ThreadPool::Work(const uint32_t threadIndex)
{
    ...
    if (ret != static_cast<int32_t>(DRV_ERROR_NONE)) {
        aicpusd_err(
            "halEschedSubscribeEvent failed, deviceId[%u], groupId[%u], threadIndex[%u] eventBitmap[%llu].", deviceId,
            DEFAULT_GROUP_ID, threadIndex, EVENT_MASK);
        AicpuSchedule::ThreadPool::Instance().PostSem(threadIndex);
        return;
    }
    aicpusd_info(
        "halEschedSubscribeEvent success, deviceId[%u], groupId[%u], threadIndex[%u] eventBitmap[%llu].", deviceId,
        DEFAULT_GROUP_ID, threadIndex, EVENT_MASK);
    ...
    if (AicpuSchedule::ThreadPool::Instance().SecureCompute(threadIndex) != AICPU_SCHEDULE_OK) {
        AicpuSchedule::ThreadPool::Instance().PostSem(threadIndex);
        return;
    }
    AicpuSchedule::ThreadPool::Instance().PostSem(threadIndex);
...
int32_t ThreadPool::WriteTidForAffinity(const size_t threadIndex)
{
    if (threadIndex >= threadStatus_.size()) {
        aicpusd_err("threadIndex[%zu], out of rank[0, %zu]", threadIndex, threadStatus_.size());
        return AICPU_SCHEDULE_ERROR_INIT_FAILED;
    }
    ...
    if (ret != 0) {
        ...
        aicpusd_err(
            "write tid[%lu] to /sys/fs/cgroup/cpuset/AICPU/tasks failed, ret[%d], strerror[%s]", GetTid(), ret,
            strerror(errno));
        return AICPU_SCHEDULE_ERROR_INIT_FAILED;
    }
    return AICPU_SCHEDULE_OK;
...
int32_t ThreadPool::SetAffinityByPm(const size_t threadIndex)
{
    ...
    if (ret != 0U) {
        threadStatus_[threadIndex] = ThreadStatus::THREAD_EXIT;
        aicpusd_err(
            "set affinity failed ret[%d], aicpu logical threadIndex[%zu], "
...
int32_t ThreadPool::SetAffinityBySelf(const size_t threadIndex)
{
    ...
    if (ret != 0) {
        threadStatus_[threadIndex] = ThreadStatus::THREAD_EXIT;
        aicpusd_err(
            "set affinity failed ret[%d], aicpu logical threadIndex[%u], aicpu physical threadIndex[%u], "
            "device id[%u]",
...
int32_t ThreadPool::SetAffinity(const size_t threadIndex, const uint32_t deviceId)
{
    ...
    if (cpuSetFlag == "1") {
        ...
        aicpusd_run_info(
            "aicpu bind tid by pm, cpuSetFlag:[%s], threadIndex[%zu], deviceId[%u], res[%d].", cpuSetFlag.c_str(),
            threadIndex, AicpuDrvManager::GetInstance().GetDeviceId(), res);
    } else {
        res = SetAffinityBySelf(threadIndex);
        aicpusd_run_info(
            "aicpu bind tid by self, cpuSetFlag:[%s], threadIndex[%zu], deviceId[%u], res[%d].", cpuSetFlag.c_str(),
            threadIndex, AicpuDrvManager::GetInstance().GetDeviceId(), res);
    }
    return res;
}

void ThreadPool::PostSem(const uint32_t threadIndex) { (void)sem_post(&(sems_[static_cast<size_t>(threadIndex)])); }
} // namespace AicpuSchedule
