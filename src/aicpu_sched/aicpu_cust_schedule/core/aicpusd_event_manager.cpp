 ...
 */
AicpuEventManager::AicpuEventManager() : noThreadFlag_(true), runningFlag_(true) {}
...
int32_t AicpuEventManager::HWTSKernelEventMessageParse(
    const event_info& eventInfo, uint32_t& mailboxId, aicpu::HwtsTsKernel& info, uint64_t& serialNo, uint32_t& streamId,
    uint64_t& taskId, uint16_t& dataDumpEnableMode) const
{
 ...
 * @return AICPU_SCHEDULE_OK: success, other: error code
 */
int32_t AicpuEventManager::HWTSConverMsgFromDriver2Aicpu(
    const event_info& eventInfo, aicpu::HwtsTsKernel& info, uint32_t& streamId, uint64_t& taskId,
    uint16_t& dataDumpEnableMode) const
{
    ...
    aicpusd_info("Kernel type of current task is [%u], dataDumpEnableMode[%u].", eventKernelType, dataDumpEnableMode);
...
int32_t AicpuEventManager::ProcessHWTSKernelEvent(const event_info& eventInfo, const uint32_t threadIndex) const
{
    ...
    aicpusd_info(
        "Begin send ack to ts, subevent_id[%u], mailboxId[%u], serialNo[%lu], "
        "streamId[%u], taskId[%lu], result[%d], originResult[%d], status[%d].",
        subeventId, mailboxId, serialNo, streamId, taskId, hwtsResponse.result, ret, hwtsResponse.status);
    const auto drvRet = halEschedAckEvent(
        AicpuDrvManager::GetInstance().GetDeviceId(), EVENT_TS_HWTS_KERNEL, subeventId,
    ...
    if (hwtsResponse.result != 0) {
        aicpusd_err(
            "Failed to execute the task, subevent id[%u], mailbox id[%u], serial number[%lu], "
            "stream id[%u], task id[%lu], result[%d], originResult[%d], status[%d], error[%d].",
            subeventId, mailboxId, serialNo, streamId, taskId, hwtsResponse.result, ret, hwtsResponse.status, drvRet);
    }
    aicpusd_info(
        "End send ack to Ts, subevent_id[%u], mailboxId[%u], serialNo[%lu], result[%d], "
...
int32_t AicpuEventManager::ExecuteHWTSEventTask(
    const EventInfoForExecute& executeEventInfo, aicpu::HwtsTsKernel& aicpufwKernelInfo, const event_info& drvEventInfo,
    hwts_response_t& hwtsResponse) const
{
    ...
    const uint32_t debugDumpEnableMode = static_cast<uint32_t>((tsKernel.l2Ctrl >> DEBUGDUMP_ENABLE_MODE_OFFSET) & 1U);
    aicpusd_info(
        "Begin to ExecuteTsKernelTask, mailboxId[%u], serialNo[%lu], streamId[%u], taskId[%lu], "
        "dataDumpEnableMode[%u], debugDumpEnableMode[%u].",
        executeEventInfo.mailboxId, executeEventInfo.serialNo, executeEventInfo.streamId, executeEventInfo.taskId,
        static_cast<uint32_t>(executeEventInfo.dataDumpEnableMode), debugDumpEnableMode);
    int32_t ret = AicpuEventProcess::GetInstance().ExecuteTsKernelTask(
        aicpufwKernelInfo, executeEventInfo.threadIndex, drvEventInfo.comm.submit_timestamp,
        drvEventInfo.comm.sched_timestamp, executeEventInfo.streamId, executeEventInfo.taskId);
    hwtsResponse.status =
        (ret == AICPU_SCHEDULE_OK) ? static_cast<uint32_t>(TASK_SUCC) : static_cast<uint32_t>(TASK_FAIL);
    ...
    if ((ret == AICPU_SCHEDULE_OK) && (dataDumpEnableMode == 1U)) {
        ...
        const int32_t dumpRet = AicpuSchedule::AicpuCustDumpProcess::GetInstance().BeginDatadumpTask(
            aicpu::GetAicpuThreadIndex(), executeEventInfo.streamId, static_cast<uint32_t>(executeEventInfo.taskId));
...
int32_t AicpuEventManager::ProcessEvent(const event_info& eventInfo, const uint32_t threadIndex)
{
    ...
    if (ret != AICPU_SCHEDULE_OK) {
        aicpusd_err(
            "Process event failed. eventId=%d, threadIdx=%u, ret=%d", static_cast<int32_t>(eventInfo.comm.event_id),
            threadIndex, ret);
    }
...
void AicpuEventManager::DoOnce(const uint32_t threadIndex)
{
    ...
    const auto retVal =
        halEschedWaitEvent(deviceId, groupId, threadIndex, static_cast<int32_t>(TIMEOUT_INTERVAL), &eventInfo);
...
int32_t AicpuEventManager::SendCtrlCpuMsg(
