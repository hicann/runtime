...
AicpuPlatformFuncPtr AicpuCustomSdLoadPlatformInfoProcess::GetAicpuPlatformFuncPtr()
{
    ...
    platformFuncPtr =
        reinterpret_cast<AicpuPlatformFuncPtr>(dlsym(RTLD_DEFAULT, CUSTOM_EXTEND_SO_PLATFORM_FUNC_NAME.c_str()));
    if (platformFuncPtr == nullptr) {
...
int32_t AicpuCustomSdLoadPlatformInfoProcess::ProcessLoadPlatform(const uint8_t* const msgInfo, const uint32_t infoLen)
{
    ...
    aicpusd_info(
        "func:%s process, info[%llu], length[%llu], pid[%u]", CUSTOM_EXTEND_SO_PLATFORM_FUNC_NAME.c_str(),
        info->platformInfo, info->length, info->aicpuPid);
    ret = funcPtr(info->platformInfo, info->length);
    if (ret != AICPU_SCHEDULE_OK) {
        aicpusd_err(
            "func:%s process failed, ret:%u, info[%llu], length[%llu], pid[%u]",
...
int32_t AicpuCustomSdLoadPlatformInfoProcess::DoSubmitEventSync(
    const uint8_t* const msg, const uint32_t len, struct event_proc_result& rsp) const
{
    uint32_t headLen = sizeof(struct event_sync_msg);
    if (len < headLen) {
...
int32_t CustProcessLoadPlatform(const struct event_info* const msg)
{
    ...
    return AicpuSchedule::AicpuCustomSdLoadPlatformInfoProcess::GetInstance().ProcessLoadPlatform(
        message, EVENT_MAX_MSG_LEN);
}
