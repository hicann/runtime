...
public:
    ArgsParser()
        : deviceId_(0U),
          ...
          withHostPid_(false),
          withPidSign_(false),
          withCustSoPath_(false),
          withAicpuPid_(false),
          withGrpNameNum_(false),
          withGrpNameList_(false),
          withControlCpuList_(false),
          withTsdPid_(false),
          argsParseFuncMap_(
    ...
    inline uint32_t GetProfilingMode() const { return profilingMode_; }
    ...
    inline int32_t GetCcecpuLogLevel() const { return ccecpulogLevel_; }

    inline int32_t GetAicpuLogLevel() const { return aicpulogLevel_; }

    inline std::string GetCustSoPath() const { return custSoPath_; }

    inline uint32_t GetAicpuPid() const { return aicpuPid_; }

    inline uint32_t GetGrpNameNum() const { return grpNameNum_; }

    inline std::vector<std::string> GetGrpNameList() const { return grpNameList_; }

    inline std::vector<uint32_t> GetControlCpuList() const { return controlCpuList_; }
    inline bool HasControlCpuList() const { return withControlCpuList_; }
    inline bool HasTsdPid() const { return withTsdPid_; }
    inline uint32_t GetTsdPid() const { return tsdPid_; }
