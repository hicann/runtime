...
int32_t AicpuCustMc2MaintenanceThread::InitCustMc2MaintenanceProcess(
...
int32_t AicpuCustMc2MaintenanceThread::SetCustMc2MaintenanceThreadAffinity()
{
    ...
    if (AicpuUtil::IsEnvValEqual(ENV_NAME_PROCMGR_AICPU_CPUSET, "1")) {
        ...
        for (const auto cpuId : ccpuIds) {
            ...
            aicpusd_info(
                "prepare bind threadId=%lu to cpuId=%u, type=%u", threadId, static_cast<uint32_t>(cpuId), type_);
        }
...
extern "C" {
#endif // __cplusplus
int32_t StartMC2MaintenanceThread(
    AicpuMC2MaintenanceFuncPtr loopFun, void* paramLoopFun, AicpuMC2MaintenanceFuncPtr stopNotifyFun,
    void* paramStopFun)
{
    int32_t ret = AicpuSchedule::AicpuCustMc2MaintenanceThread::GetInstance(THREAD_TYPE_HCOM)
                      .InitCustMc2MaintenanceProcess(loopFun, paramLoopFun, stopNotifyFun, paramStopFun);
    if (ret != AicpuSchedule::AICPU_SCHEDULE_OK) {
        return ret;
    }
    return AicpuSchedule::AICPU_SCHEDULE_OK;
}
int32_t AicpuCreateCtrlThread(
    uint32_t type, AicpuMC2MaintenanceFuncPtr loopFun, void* paramLoopFun, AicpuMC2MaintenanceFuncPtr stopNotifyFun,
    void* paramStopFun)
{
    ...
    int32_t ret = AicpuSchedule::AicpuCustMc2MaintenanceThread::GetInstance(type).InitCustMc2MaintenanceProcess(
        loopFun, paramLoopFun, stopNotifyFun, paramStopFun);
    if (ret != AicpuSchedule::AICPU_SCHEDULE_OK) {
        return ret;
    }
    return AicpuSchedule::AICPU_SCHEDULE_OK;
