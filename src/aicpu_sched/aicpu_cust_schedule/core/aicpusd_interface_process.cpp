 ...
 */
AicpuScheduleInterface::AicpuScheduleInterface() : noThreadFlag_(true), initFlag_(false), aicpuSdPid_(-1), tsdPid_(-1)
{}
...
int32_t AicpuScheduleInterface::InitAICPUScheduler(
    const uint32_t deviceId, const pid_t hostPid, const std::string& pidSign, const uint32_t profilingMode,
    const pid_t aicpuPid, const uint32_t vfId, const bool isOnline)
{
    (void)pidSign;
    ...
    aicpusd_info(
        "Successfully started compute process, deviceId[%u], hostPid[%d], mode[%d].", deviceId, hostPid,
        static_cast<int32_t>(isOnline));
    initFlag_ = true;
    return AICPU_SCHEDULE_OK;
}
...
pid_t AicpuScheduleInterface::GetAicpuSdProcId() const { return aicpuSdPid_; }
...
pid_t AicpuScheduleInterface::GetTsdProcId() const { return tsdPid_; }
...
bool AicpuScheduleInterface::AttachHostGroup(
    const std::vector<std::string>& groupNameVec, const uint32_t grpNameNum) const
{
    if (grpNameNum == 0U) {
...
int32_t AicpuScheduleInterface::CustAicpuMainProcess(int32_t argc, char_t* argv[]) const
{
    try {
        ...
        int32_t ret = AicpuSchedule::AicpuScheduleInterface::GetInstance().InitAICPUScheduler(
            deviceId, hostPid, startParams.GetPidSign(), mode, aicpuPid, vfId, true);
