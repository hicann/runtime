...
public:
    ...
    inline uint64_t GetStartTick() const { return startTick_.load(); }

    inline void SetStartTick(const uint64_t tick) { startTick_ = tick; }

    inline bool GetRunFlag() const { return runFlag_.load(); }

    inline void SetRunFlag(const bool flag) { runFlag_ = flag; }
...
struct TaskInfoForMonitor {
    ...
    std::string DebugString() const
    {
        ...
        oss << "serialNo=" << serialNo << ", stream_id=" << streamId << ", task_id=" << taskId;
        return oss.str();
    }
};
