...
AicpuCustDumpProcess::~AicpuCustDumpProcess() { UnitDataDumpProcess(); }

AicpuCustDumpProcess::AicpuCustDumpProcess() : deviceId_(0U), runningFlag_(false), initFlag_(false) {}
...
int32_t AicpuCustDumpProcess::GetDataDumpRetCode(const uint32_t threadIndex)
{
    ...
    aicpusd_info(
        "dump task finish retcode:%d, streamid:%u, taskId:%u, threadIndex:%u", retCode,
        waitCondVec_[threadIndex].streamId, waitCondVec_[threadIndex].taskId, threadIndex);
    return retCode;
}
...
int32_t AicpuCustDumpProcess::BeginDatadumpTask(
    const uint32_t threadIndex, const uint32_t streamId, const uint32_t taskId)
{
    ...
    aicpusd_info(
        "dump task finish streamid:%u, taskId:%u, threadIndex:%u, dumpRet:%d", streamId, taskId, threadIndex, dumpRet);
    return dumpRet;
}
} // namespace AicpuSchedule
