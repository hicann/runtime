...
int32_t CustomOpExecutor::OpenKernelSo(const event_info_priv& privEventInfo) const
{
    ...
    if ((soNameLen == 0U) || (soNameLen > MAX_CUST_SO_NAME_LEN)) {
        aicpusd_err(
            "Input param custom so name length is zero or max reached(%u), soNameBufLen=%u.", MAX_CUST_SO_NAME_LEN,
            soNameLen);
    ...
    const aeStatus_t ret =
        aeBatchLoadKernelSo(static_cast<uint32_t>(aicpu::KERNEL_TYPE_AICPU_CUSTOM), loadSoCnt, &(soNames[0]));
...
int32_t CustomOpExecutor::ExecuteKernel(aicpu::HwtsTsKernel& tsKernelInfo) const
{
    ...
    if ((ret == AE_STATUS_SUCCESS) || (ret == AE_STATUS_END_OF_SEQUENCE) || (ret == AE_STATUS_TASK_WAIT)) {
