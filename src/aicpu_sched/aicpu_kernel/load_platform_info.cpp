...
__attribute__((visibility("default"))) int32_t AicpuExtendKernelsLoadPlatformInfo(
    const uint64_t input_info, const uint32_t info_len)
{
...
int32_t PlatformRebuild::ProcessPlatformInfoMsg(const uint64_t info_args, const uint32_t info_len)
{
    if (info_len != static_cast<uint32_t>(sizeof(PlatformInfoArgs))) {
        KERNEL_LOG_ERROR("The length of args is invalid, input:%u, base:%zu", info_len, sizeof(PlatformInfoArgs));
    ...
    KERNEL_LOG_INFO(
        "input param: input_data_info:0x%x, input_data_len:%llu, platform_instance:0x%x",
        platform_args->input_data_info, platform_args->input_data_len, platform_args->platform_instance);
