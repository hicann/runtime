    ...
    const void* ptr, size_t size, rtMemManagedLocation location, uint32_t flags, Stream* const stream)
{
    ...
    COND_PROC_RETURN_ERROR_MSG_INNER(
        (ret != RT_ERROR_NONE), ret, DELETE_O(params), "ConvertUvmLocationStruct failed, retCode=%#x",
    ...
    ERROR_PROC_RETURN_MSG_INNER(
        ret, DELETE_O(params), "LaunchHostFunc fails with error code %#x", static_cast<uint32_t>(ret));
    return ret;
}
...
{
    COND_RETURN_WARN(
        &halMemManagedPrefetchBatch == nullptr, RT_ERROR_DRV_NOT_SUPPORT,
    ...
    COND_PROC_RETURN_ERROR_MSG_INNER(
        (ret != RT_ERROR_NONE), ret, DELETE_A(memBuffer), "Fill para numPrefetchLocs failed");
    ...
    COND_PROC_RETURN_ERROR_MSG_INNER(
        (err != EOK), RT_ERROR_SEC_HANDLE, DELETE_A(memBuffer), "memcpy_s arr failed, retCode=%#x",
    ...
    for (size_t index = 0; index < numPrefetchLocs; index++) {
        ...
        COND_RETURN_WITH_NOLOG((ret == RT_ERROR_DRV_NOT_SUPPORT), ret);
        COND_PROC_RETURN_ERROR_MSG_INNER(
            (ret != RT_ERROR_NONE), ret, DELETE_A(memBuffer), "ConvertUvmLocationStruct failed, retCode=%#x",
            static_cast<uint32_t>(ret));
    }
    ...
    ERROR_PROC_RETURN_MSG_INNER(
        ret, DELETE_A(memBuffer), "LaunchHostFunc fails with error code %#x", static_cast<uint32_t>(ret));
    return ret;
}
