...
public:
    ...
    rtError_t LaunchKernelV2(
        ...
        const rtKernelLaunchCfg_t* const cfg = nullptr) override;
    ...
    rtError_t DavidGetGroupAccNum(const int32_t moduleType, const int32_t infoType, int64_t* const val);
...
protected:
    ...
    rtError_t LoopMemcpyAsync(
        void** const dsts, const size_t* const destMaxs, void** const srcs, const size_t* const sizes,
        const size_t count, const rtMemcpyBatchAttr* const attrs, const size_t* const attrsIdxs, const size_t numAttrs,
        size_t* const failIdx, Stream* const stm);
    rtError_t ValidateMemCpyParamsAndAttributes(
...
private:
    static void SetHostFuncStreamError(const Device* const dev, const uint16_t streamId, const int32_t callbackRet);
    ...
    virtual rtError_t GetCaptureEvent(
        const Stream* const stm, Event* const evt, Event** const captureEvt, const bool isNewEvt = false);
