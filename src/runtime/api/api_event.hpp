...
public:
    ...
    virtual rtError_t EventElapsedTime(
        float32_t* const timeInterval, Event* const startEvent, Event* const endEvent) = 0;
    virtual rtError_t EventGetTimeStamp(uint64_t* const timeStamp, Event* const evt) = 0;
    virtual rtError_t IpcOpenEventHandle(rtIpcEventHandle_t* handle, IpcEvent** const event) = 0;
    virtual rtError_t IpcGetEventHandle(IpcEvent* const event, rtIpcEventHandle_t* handle) = 0;
};
