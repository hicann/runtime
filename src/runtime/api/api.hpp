...
static inline uint64_t CalculateMemcpyAsyncSingleMaxSize(
    const rtMemcpyKind_t kind, const uint32_t transType = UINT32_MAX)
{
...
public:
    ...
    virtual rtError_t BinaryGetFunction(
        const Program* const prog, const uint64_t tilingKey, Kernel** const funcHandle) = 0;
    virtual rtError_t BinaryLoadWithoutTilingKey(const void* data, const uint64_t length, Program** const prog) = 0;
    virtual rtError_t BinaryGetFunctionByName(
    ...
    virtual rtError_t BinaryGetMetaInfo(
        Program* const binHandle, const rtBinaryMetaType type, const size_t numOfMeta, void** data,
        const size_t* dataSize) = 0;
    virtual rtError_t FunctionGetMetaInfo(
        const Kernel* const funcHandle, const rtFunctionMetaType type, void* data, const uint32_t length) = 0;
    virtual rtError_t FunctionGetMetaInfoSize(
        const Kernel* const funcHandle, const rtFunctionMetaType type, size_t* size) = 0;
    virtual rtError_t GetFunctionBySymbol(const void* symbol, Kernel** const funcHandle) = 0;
    virtual rtError_t RegisterFuncSymbol(
        void* const binHandle, const void* const symbol, const char_t* const kernelName) = 0;
    virtual rtError_t RegisterCpuFunc(
        rtBinHandle binHandle, const char_t* const funcName, const char_t* const kernelName,
        rtFuncHandle* funcHandle) = 0;
    virtual rtError_t LaunchKernel(
    ...
    virtual rtError_t LaunchKernelV2(
        Kernel* const kernel, uint32_t blockDim, const RtArgsWithType* const argsWithType, Stream* const stm,
        const rtKernelLaunchCfg_t* const cfg = nullptr) = 0;
    virtual rtError_t BinaryUnLoad(Program* const binHandle) = 0;
    virtual rtError_t BinaryLoadFromFile(
        const char* const binPath, const rtLoadBinaryConfig_t* const optionalCfg, Program** handle) = 0;
    virtual rtError_t BinaryLoadFromData(
    ...
    virtual rtError_t GetAddrAndPrefCntWithHandle(
        void* const hdl, const void* const kernelInfoExt, void** const addr, uint32_t* const prefetchCnt) = 0;
    virtual rtError_t KernelGetAddrAndPrefCnt(
        void* const hdl, const uint64_t tilingKey, const void* const stubFunc, const uint32_t flag, void** const addr,
        uint32_t* const prefetchCnt) = 0;
    virtual rtError_t KernelGetAddrAndPrefCntV2(
        void* const hdl, const uint64_t tilingKey, const void* const stubFunc, const uint32_t flag,
        rtKernelDetailInfo_t* const kernelInfo) = 0;
    virtual rtError_t QueryFunctionRegistered(const char_t* const stubName) = 0;
    virtual rtError_t KernelLaunch(
        const void* const stubFunc, uint32_t coreDim, const rtArgsEx_t* const argsInfo, Stream* const stm,
        const rtTaskCfgInfo_t* const cfgInfo = nullptr, const bool isLaunchVec = false) = 0;
    virtual rtError_t KernelLaunchWithHandle(
        void* const hdl, const uint64_t tilingKey, const uint32_t coreDim, const rtArgsEx_t* const argsInfo,
        Stream* const stm, const rtTaskCfgInfo_t* const cfgInfo = nullptr, const bool isLaunchVec = false) = 0;
    virtual rtError_t KernelLaunchEx(
        const char_t* const opName, const void* const args, const uint32_t argsSize, const uint32_t flags,
        Stream* const stm) = 0;
    virtual rtError_t CpuKernelLaunch(
        const rtKernelLaunchNames_t* const launchNames, const uint32_t coreDim, const rtArgsEx_t* const argsInfo,
        Stream* const stm, const uint32_t flag) = 0;
    virtual rtError_t CpuKernelLaunchExWithArgs(
        const char_t* const opName, const uint32_t coreDim, const rtAicpuArgsEx_t* const argsInfo, Stream* const stm,
        const uint32_t flag, const uint32_t kernelType) = 0;
    virtual rtError_t MultipleTaskInfoLaunch(
        const rtMultipleTaskInfo_t* const taskInfo, Stream* const stm, const uint32_t flag) = 0;
    ...
    virtual rtError_t AppendLaunchHostInfo(
        rtLaunchArgs_t* const hdl, size_t const hostInfoSize, void** const hostInfo) = 0;
    ...
    virtual rtError_t CalcLaunchArgsSize(
        size_t const argsSize, size_t const hostInfoTotalSize, size_t hostInfoNum, size_t* const launchArgsSize) = 0;
    virtual rtError_t CreateLaunchArgs(
        size_t const argsSize, size_t const hostInfoTotalSize, size_t hostInfoNum, void* const argsData,
        rtLaunchArgs_t** const argsHandle) = 0;
    ...
    virtual rtError_t GetBinBuffer(
        rtBinHandle const binHandle, const rtBinBufferType_t type, void** bin, uint32_t* binSize) = 0;
    virtual rtError_t GetStackBuffer(
        rtBinHandle const binHandle, uint32_t deviceId, const uint32_t stackType, const uint32_t coreType,
        const uint32_t coreId, const void** stack, uint32_t* stackSize) = 0;
    ...
    virtual rtError_t StreamBeginCapture(
        Stream* const stm, const rtStreamCaptureMode mode, Model* const mdl = nullptr) = 0;
    virtual rtError_t StreamEndCapture(Stream* const stm, Model** const captureMdl) = 0;
    virtual rtError_t StreamGetCaptureInfo(
        const Stream* const stm, rtStreamCaptureStatus* const status, Model** const captureMdl) = 0;
    ...
    virtual rtError_t MemManagedAdvise(
        const void* const ptr, uint64_t size, uint16_t advise, rtMemManagedLocation location) = 0;
    virtual rtError_t DevMallocCached(
        void** const devPtr, const uint64_t size, const rtMemType_t type,
        const uint16_t moduleId = MODULEID_RUNTIME) = 0;
    ...
    virtual rtError_t MemCopySyncEx(
        void* const dst, uint64_t destMax, const void* const src, const uint64_t cnt, const rtMemcpyKind_t kind) = 0;
    virtual rtError_t MemcpyAsync(
        void* const dst, uint64_t destMax, const void* const src, const uint64_t cnt, const rtMemcpyKind_t kind,
        ...
        const rtMemcpyConfig_t* const memcpyConfig = nullptr) = 0;
    virtual rtError_t LaunchSqeUpdateTask(
        uint32_t streamId, uint32_t taskId, void* src, uint64_t cnt, Stream* const stm, bool needCpuTask = false) = 0;
    virtual rtError_t MemcpyAsyncPtr(
        void* const memcpyAddrInfo, const uint64_t destMax, const uint64_t count, Stream* stm,
        const rtTaskCfgInfo_t* const cfgInfo = nullptr, const bool isMemcpyDesc = false) = 0;
    virtual rtError_t ReduceAsync(
    ...
    virtual rtError_t ReduceAsyncV2(
        void* const dst, const void* const src, const uint64_t cnt, const rtRecudeKind_t kind, const rtDataType_t type,
        Stream* const stm, void* const overflowAddr) = 0;
    virtual rtError_t MemSetSync(
        const void* const devPtr, const uint64_t destMax, const uint32_t val, const uint64_t cnt) = 0;
    virtual rtError_t MemsetAsync(
        void* const ptr, const uint64_t destMax, const uint32_t val, const uint64_t cnt, Stream* const stm) = 0;
    virtual rtError_t MemsetD32(
        void* const dst, const uint64_t destMax, const uint32_t value, const uint64_t count) = 0;
    virtual rtError_t MemsetD32Async(
        void* const dst, const uint64_t destMax, const uint32_t value, const uint64_t count, Stream* const stm) = 0;
    virtual rtError_t MemGetInfoByDeviceId(
        uint32_t deviceId, bool isHugeOnly, size_t* const freeSize, size_t* const totalSize) = 0;
    ...
    virtual rtError_t CheckMemType(
        void** addrs, uint32_t size, uint32_t memType, uint32_t* checkResult, uint32_t reserve) = 0;
    virtual rtError_t GetMemUsageInfo(
        const uint32_t deviceId, rtMemUsageInfo_t* const memUsageInfo, const size_t inputNum,
        size_t* const outputNum) = 0;
    virtual rtError_t MemGetInfoEx(
        const rtMemInfoType_t memInfoType, size_t* const freeSize, size_t* const totalSize) = 0;
    ...
    virtual rtError_t MemManagedGetAttr(
        rtMemManagedRangeAttribute attribute, const void* ptr, size_t size, void* data, size_t dataSize) = 0;
    virtual rtError_t MemManagedGetAttrs(
    ...
    virtual rtError_t MemCopy2DSync(
        ...
        const rtMemcpyKind newKind = RT_MEMCPY_KIND_MAX) = 0;
    virtual rtError_t MemCopy2DAsync(
        void* const dst, const uint64_t dstPitch, const void* const src, const uint64_t srcPitch, const uint64_t width,
        const uint64_t height, Stream* const stm, const rtMemcpyKind_t kind = RT_MEMCPY_RESERVED,
        const rtMemcpyKind newKind = RT_MEMCPY_KIND_MAX) = 0;
    virtual rtError_t MemcpyHostTask(
    ...
    virtual rtError_t RtsMemcpy(
        void* const dst, const uint64_t destMax, const void* const src, const uint64_t cnt, const rtMemcpyKind kind,
        rtMemcpyConfig_t* const config) = 0;
    virtual rtError_t SetMemcpyDesc(
    ...
    virtual rtError_t DevMalloc(
        ...
        const rtMallocConfig_t* const cfg) = 0;
    virtual rtError_t MemWriteValue(
        const void* const devAddr, const uint64_t value, const uint32_t flag, Stream* const stm) = 0;
    virtual rtError_t MemWaitValue(
        const void* const devAddr, const uint64_t value, const uint32_t flag, Stream* const stm) = 0;
    virtual rtError_t MemcpyBatch(
        void** dsts, void** srcs, size_t* sizes, size_t count, rtMemcpyBatchAttr* attrs, size_t* attrsIdxs,
        size_t numAttrs, size_t* failIdx) = 0;
    virtual rtError_t MemcpyBatchAsync(
        void** const dsts, const size_t* const destMaxs, void** const srcs, const size_t* const sizes,
        const size_t count, const rtMemcpyBatchAttr* const attrs, const size_t* const attrsIdxs, const size_t numAttrs,
        size_t* const failIdx, Stream* const stm) = 0;
    virtual rtError_t MemManagedPrefetchAsync(
        const void* ptr, size_t size, rtMemManagedLocation location, uint32_t flags, Stream* const stream) = 0;
    ...
    virtual rtError_t HdcSessionConnect(
        const int32_t peerNode, const int32_t peerDevId, rtHdcClient_t const client, rtHdcSession_t* const session) = 0;
    ...
    virtual rtError_t GetDeviceInfo(
        const uint32_t deviceId, const int32_t moduleType, const int32_t infoType, int64_t* const val) = 0;
    virtual rtError_t GetDeviceInfoFromPlatformInfo(
        const uint32_t deviceId, const std::string& label, const std::string& key, int64_t* const value) = 0;
    virtual rtError_t GetDeviceInfoByAttr(uint32_t deviceId, rtDevAttr attr, int64_t* val) = 0;
    virtual rtError_t GetPhyDeviceInfo(
        const uint32_t phyId, const int32_t moduleType, const int32_t infoType, int64_t* const val) = 0;
    ...
    virtual rtError_t GetPairPhyDevicesInfo(
        const uint32_t devId, const uint32_t otherDevId, const int32_t infoType, int64_t* const val) = 0;
    virtual rtError_t GetRtCapability(
        const rtFeatureType_t featureType, const int32_t featureInfo, int64_t* const val) = 0;
    virtual rtError_t GetDeviceCapability(
        const int32_t deviceId, const int32_t moduleType, const int32_t featureType, int32_t* const val) = 0;
    ...
    virtual rtError_t GetFaultEvent(
        const int32_t deviceId, rtDmsEventFilter* filter, rtDmsFaultEvent* dmsEvent, uint32_t len,
    ...
    virtual rtError_t GetDevicePCIBusId(const int32_t devId, char* pciBusId, const int32_t len) = 0;
    virtual rtError_t GetDeviceByPCIBusId(const char* pciBusId, int32_t* devId) = 0;
    ...
    virtual rtError_t ProfilerTrace(
        const uint64_t id, const bool notifyFlag, const uint32_t flags, Stream* const stm) = 0;
    virtual rtError_t ProfilerTraceEx(
        const uint64_t id, const uint64_t modelId, const uint16_t tagId, Stream* const stm) = 0;
    ...
    virtual rtError_t GetOnlineProfData(
        Stream* const stm, rtProfDataInfo_t* const pProfData, const uint32_t profDataNum) = 0;
    ...
    // inquire information
    virtual rtError_t GetMaxStreamAndTask(
        const uint32_t streamType, uint32_t* const maxStrCount, uint32_t* const maxTaskCount) = 0;
    ...
    virtual rtError_t ModelExecute(Model* const mdl, Stream* const stm, const uint32_t flag, int32_t timeout = -1) = 0;
    ...
    virtual rtError_t DebugRegister(
        Model* const mdl, const uint32_t flag, const void* const addr, uint32_t* const streamId,
        uint32_t* const taskId) = 0;
    virtual rtError_t DebugUnRegister(Model* const mdl) = 0;
    virtual rtError_t DebugRegisterForStream(
        Stream* const stm, const uint32_t flag, const void* const addr, uint32_t* const streamId,
        uint32_t* const taskId) = 0;
    ...
    virtual rtError_t ModelTaskUpdate(
        Stream* desStm, uint32_t desTaskId, Stream* sinkStm, rtMdlTaskUpdateInfo_t* para) = 0;
    ...
    virtual rtError_t ModelCondHandleCreate(
        Model* const mdl, uint32_t defaultValue, rtCondHandleFlag_t flag, CondHandle** const handle) = 0;
    ...
    virtual rtError_t GetDeviceSatModeForStream(Stream* const stm, rtFloatOverflowMode_t* const floatOverflowMode) = 0;
    ...
    virtual rtError_t RegDeviceStateCallback(
        const char_t* regName, void* callback, void* args, DeviceStateCallback type,
        rtDevCallBackDir_t notifyPos = DEV_CB_POS_BACK) = 0;
    virtual rtError_t RegProfCtrlCallback(const uint32_t moduleId, const rtProfCtrlHandle callback) = 0;
    virtual rtError_t RegTaskFailCallbackByModule(
        const char_t* regName, void* callback, void* args, TaskFailCallbackType type) = 0;
    virtual rtError_t RegStreamStateCallback(
        const char_t* regName, void* callback, void* args, StreamStateCallback type) = 0;
    ...
    virtual rtError_t CntNotifyWaitWithTimeout(
        CountNotify* const inCntNotify, Stream* const stm, const rtCntNtyWaitInfo_t* const info) = 0;
    virtual rtError_t CntNotifyReset(CountNotify* const inCntNotify, Stream* const stm) = 0;
    virtual rtError_t GetCntNotifyAddress(
        CountNotify* const inCntNotify, uint64_t* const cntNotifyAddress, rtNotifyType_t const regType) = 0;
    ...
    // switch api
    virtual rtError_t StreamSwitchEx(
        void* const ptr, const rtCondition_t condition, void* const valuePtr, Stream* const trueStream,
        Stream* const stm, const rtSwitchDataType_t dataType) = 0;
    virtual rtError_t StreamActive(Stream* const activeStream, Stream* const stm) = 0;

    virtual rtError_t StreamSwitchN(
        void* const ptr, const uint32_t size, void* const valuePtr, Stream** const trueStreamPtr,
        const uint32_t elementSize, Stream* const stm, const rtSwitchDataType_t dataType) = 0;
    ...
    virtual rtError_t CallbackLaunch(
        const rtCallback_t callBackFunc, void* const fnData, Stream* const stm, const bool isBlock) = 0;
    ...
    // stream label func
    virtual ::rtError_t LabelSwitchByIndex(
        void* const ptr, const uint32_t maxVal, void* const labelInfoPtr, Stream* const stm) = 0;
    virtual ::rtError_t LabelGotoEx(Label* const lbl, Stream* const stm) = 0;
    virtual ::rtError_t LabelListCpy(
        Label** const lbl, const uint32_t labelNumber, void* const dst, const uint32_t dstMax) = 0;
    ...
    virtual rtError_t CheckArchCompatibility(
        const char_t* socVersion, const char_t* omSocVersion, int32_t* canCompatible) = 0;
    ...
    virtual rtError_t StarsTaskLaunch(
        const void* const sqe, const uint32_t sqeLen, Stream* const stm, const uint32_t flag) = 0;
    virtual rtError_t FftsPlusTaskLaunch(
        const rtFftsPlusTaskInfo_t* const fftsPlusTaskInfo, Stream* const stm, const uint32_t flag) = 0;
    virtual rtError_t NpuGetFloatStatus(
        void* const outputAddrPtr, const uint64_t outputSize, const uint32_t checkMode, Stream* const stm) = 0;
    virtual rtError_t NpuClearFloatStatus(const uint32_t checkMode, Stream* const stm) = 0;
    virtual rtError_t NpuGetFloatDebugStatus(
        void* const outputAddrPtr, const uint64_t outputSize, const uint32_t checkMode, Stream* const stm) = 0;
    ...
    virtual rtError_t MemQueueCreate(
        const int32_t devId, const rtMemQueueAttr_t* const queAttr, uint32_t* const qid) = 0;

    virtual rtError_t MemQueueExport(
        const int32_t devId, const uint32_t qid, const int32_t peerDevId, const char* const shareName) = 0;
    virtual rtError_t MemQueueUnExport(
        const int32_t devId, const uint32_t qid, const int32_t peerDevId, const char* const shareName) = 0;
    virtual rtError_t MemQueueImport(
        const int32_t devId, const int32_t peerDevId, const char* const shareName, uint32_t* const qid) = 0;
    virtual rtError_t MemQueueUnImport(
        const int32_t devId, const uint32_t qid, const int32_t peerDevId, const char* const shareName) = 0;

    virtual rtError_t MemQueueSet(
        const int32_t devId, rtMemQueueSetCmdType cmd, const rtMemQueueSetInputPara* const input) = 0;
    ...
    virtual rtError_t MemQueuePeek(
        const int32_t devId, const uint32_t qid, size_t* const bufLen, const int32_t timeout) = 0;
    virtual rtError_t MemQueueEnQueueBuff(
        const int32_t devId, const uint32_t qid, rtMemQueueBuff_t* const inBuf, const int32_t timeout) = 0;
    virtual rtError_t MemQueueDeQueueBuff(
        const int32_t devId, const uint32_t qid, rtMemQueueBuff_t* const outBuf, const int32_t timeout) = 0;
    virtual rtError_t MemQueueQuery(
    ...
    virtual rtError_t MemQueueQueryInfo(
        const int32_t devId, const uint32_t qid, rtMemQueueInfo_t* const queryQueueInfo) = 0;
    ...
    virtual rtError_t MemGrpCacheAlloc(
        const char_t* const name, const int32_t devId, const rtMemGrpCacheAllocPara* const para) = 0;
    virtual rtError_t BuffGetInfo(
        const rtBuffGetCmdType type, const void* const inBuff, const uint32_t inLen, void* const outBuff,
        uint32_t* const outLen) = 0;
    virtual rtError_t MemGrpAddProc(
        const char_t* const name, const int32_t pid, const rtMemGrpShareAttr_t* const attr) = 0;
    ...
    virtual rtError_t BarrierTaskLaunch(
        const rtBarrierTaskInfo_t* const taskInfo, Stream* const stm, const uint32_t flag) = 0;

    virtual rtError_t DvppGroupCreate(DvppGrp** grp, const uint32_t flags) = 0;
    virtual rtError_t DvppGroupDestory(DvppGrp* grp) = 0;
    virtual rtError_t DvppWaitGroupReport(
        DvppGrp* const grp, const rtDvppGrpCallback callBackFunc, const int32_t timeout) = 0;
    ...
    virtual rtError_t GetExceptionRegInfo(
        const rtExceptionInfo_t* const exceptionInfo, rtExceptionErrRegInfo_t** exceptionErrRegInfo, uint32_t* num) = 0;
    ...
    virtual rtError_t LaunchHostFuncV2(Stream* const stm, rtHostCpuFunc const callBackFunc, void* const fnData) = 0;
    ...
    virtual rtError_t KernelTaskGetAttribute(
