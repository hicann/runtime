...
void ConstructDavidSqeForCcuLaunchTask(TaskInfo* taskInfo, void* const sqe, const TaskSqeInfo& sqeInfo)
{
    ...
    const errno_t ret = memcpy_s(
        sqeAddr, sizeof(rtDavidStarsCommonSqe_t), (args + CCU_1ST_SQE_ARGS_LEN),
        (sizeof(uint32_t) * CCU_2ND_SQE_LEFT_LEN));
    if (ret != EOK) {
    ...
    RT_LOG(
        RT_LOG_INFO,
        "device_id=%u, stream_id=%hu, task_id=%hu, missionId=%hu, dieId=%hu, instStartId=%hu, "
        "instCnt=%hu",
...
template <typename T>
static void FusionCcuSubSqeCommonInit(
    const rtCcuTaskInfo_t* const subInfo, const TaskInfo* const taskInfo, uint8_t taskCnt, T const sqe)
{
...
static rtError_t ConstructCcuSubSqeFor128B(
    const rtCcuTaskInfo_t* const subInfo, rtDavidSqe_t* const davidSqe, const TaskInfo* const taskInfo,
    uint32_t& sqeIndex, uint8_t taskCnt, uint64_t sqBaseAddr)
{
    ...
    errno_t ret = memcpy_s(
        RtPtrToPtr<uint8_t*>(sqe) + sizeof(rtDavidSqe_t) - firstCpySize, firstCpySize,
    ...
    COND_RETURN_ERROR_MSG_INNER(
        ...
        RtPtrToPtr<uint8_t*>(sqe) + sizeof(rtDavidSqe_t) - firstCpySize, firstCpySize,
        RtPtrToPtr<const uint8_t*>(subInfo->args), firstCpySize, ret);
    ...
    ret = memcpy_s(
        sqeAddr1, sizeof(rtDavidSqe_t), (RtPtrToPtr<const uint8_t*>(&(subInfo->args[firstCpySize / 8U]))),
        secondCpySize);
    COND_RETURN_ERROR_MSG_INNER(
        (ret != EOK), RT_ERROR_SEC_HANDLE,
        "Failed to call memcpy_s to copy subInfo->args[%zu], dest=%p, dest_max=%zu, src=%p, count=%zu, retCode=%d.",
        firstCpySize / 8U, sqeAddr1, sizeof(rtDavidSqe_t),
        (RtPtrToPtr<const uint8_t*>(&(subInfo->args[firstCpySize / 8U]))), secondCpySize, ret);

    RT_LOG(
        RT_LOG_INFO,
        "taskIdx=%d, sqeIndex=%u, missionId=%hhu, dieId=%hhu, instStartId=%hd, instCnt=%hu,"
        "instAddrKeyValue=%u.",
        taskInfo->id, sqeIndex, subInfo->missionId, subInfo->dieId, subInfo->instStartId, subInfo->instCnt,
        subInfo->key);
...
static rtError_t ConstructCcuSubSqeFor32B(
    const rtCcuTaskInfo_t* const subInfo, rtDavidSqe_t* const davidSqe, const TaskInfo* const taskInfo,
    uint32_t sqeIndex, uint32_t idx, uint8_t taskCnt, uint64_t sqBaseAddr)
{
    rtDavidSqe_t* sqeAddr = &davidSqe[sqeIndex];
    if (sqBaseAddr != 0ULL) {
    ...
    RT_LOG(
        RT_LOG_INFO,
        "taskIdx=%u, sqeIndex=%u, missionId=%hhu, dieId=%hhu, instStartId=%hu, instCnt=%hu,"
        "instAddrKeyValue=%u.",
        taskInfo->id, sqeIndex, subInfo->missionId, subInfo->dieId, subInfo->instStartId, subInfo->instCnt,
        subInfo->key);
...
rtError_t ConstructCcuSubSqe(
    const TaskInfo* const taskInfo, rtDavidSqe_t* const davidSqe, uint32_t& sqeIndex, const uint32_t taskIdx,
    const uint64_t sqBaseAddr)
{
    ...
    if (ccuInfo->ccuTaskInfo[0].argSize == RT_CCU_SQE32B_ARGS_SIZE) {
        ...
        for (uint32_t i = 0U; i < taskNum32B; i += TASK_SQE_NUM_TWO) {
            error = ConstructCcuSubSqeFor32B(
                &(ccuInfo->ccuTaskInfo[i]), davidSqe, taskInfo, sqeIndex, 0, taskCnt, sqBaseAddr);
            ERROR_RETURN(error, "ConstructCcuSubSqeFor32B failed.");
            error = ConstructCcuSubSqeFor32B(
                &(ccuInfo->ccuTaskInfo[i + 1U]), davidSqe, taskInfo, sqeIndex, 1, taskCnt, sqBaseAddr);
            ERROR_RETURN(error, "ConstructCcuSubSqeFor32B failed.");
            sqeIndex += 1U;
        }
        // 32B sqe 是单数
        if (isOddNum) {
            error = ConstructCcuSubSqeFor32B(
                &(ccuInfo->ccuTaskInfo[taskNum32B]), davidSqe, taskInfo, sqeIndex, 0, taskCnt, sqBaseAddr);
            ERROR_RETURN(error, "ConstructCcuSubSqeFor32B failed.");
        }
    } else {
        for (uint32_t i = 0; i < ccuInfo->taskNum; i++) {
            error = ConstructCcuSubSqeFor128B(
                &(ccuInfo->ccuTaskInfo[i]), davidSqe, taskInfo, sqeIndex, taskCnt, sqBaseAddr);
