...
namespace {
static const char_t* g_snapShotStageStr[] = {"RT_SNAPSHOT_LOCK_PRE",     "RT_SNAPSHOT_BACKUP_PRE",
                                             "RT_SNAPSHOT_BACKUP_POST",  "RT_SNAPSHOT_RESTORE_PRE",
                                             "RT_SNAPSHOT_RESTORE_POST", "RT_SNAPSHOT_UNLOCK_POST"};
}
...
rtError_t SnapshotCallbackManager::RegisterCallback(rtSnapShotStage stage, rtSnapShotCallBack callback, void* args)
{
    ...
    const auto& callBackIter = std::find_if(
        callBackList.begin(), callBackList.end(),
        [callback](const SnapShotCallBackInfo& info) { return info.callback == callback; });
    COND_RETURN_AND_MSG_OUTER(
        callBackIter != callBackList.end(), RT_ERROR_SNAPSHOT_REGISTER_CALLBACK_FAILED, ErrorCode::EE1017,
        "Registering the callback function related to snapshot operations", "callback",
        "The callback function " + std::to_string(RtPtrToValue(callback)) + " has been registered for stage " +
            std::string(GetStageString(stage)) + " and cannot be registered again");
...
rtError_t SnapshotCallbackManager::UnregisterCallback(rtSnapShotStage stage, rtSnapShotCallBack callback)
{
    ...
    if (callbackMap_.find(stage) == callbackMap_.end()) {
        RT_LOG_OUTER_MSG_WITH_FUNC_DESC(
            ErrorCode::EE1017, "Deregistering the callback function related to snapshot operations", "callback",
    ...
    const auto& callBackIter = std::find_if(
        ...
        [callback](const SnapShotCallBackInfo& info) { return info.callback == callback; });
    COND_RETURN_AND_MSG_OUTER(
        callBackIter == callBackList.end(), RT_ERROR_INVALID_VALUE, ErrorCode::EE1017,
        "Deregistering the callback function related to snapshot operations", "callback",
        "The callback function " + std::to_string(RtPtrToValue(callback)) + " has not been registered for stage " +
            std::string(GetStageString(stage)));
    (void)callBackList.erase(callBackIter);
    RT_LOG(
        RT_LOG_EVENT, "unregister snapshot callback successfully, stage:%s, callback=%p.", GetStageString(stage),
        callback);
    return RT_ERROR_NONE;
}
...
rtError_t SnapshotCallbackManager::InvokeCallbacks(rtSnapShotStage stage)
{
    ...
    for (const auto& callBackInfo : iter->second) {
        ...
        const rtError_t error = Runtime::Instance()->ProcessForAllOpenDevice(
            [&callBackInfo, stage](Device* dev) -> rtError_t {
                const int32_t devId = static_cast<int32_t>(dev->Id_());
                uint32_t userDeviceId = 0U;
                rtError_t err =
                    Runtime::Instance()->GetUserDevIdByDeviceId(static_cast<uint32_t>(devId), &userDeviceId);
            ...
            true);
        ERROR_RETURN(
            error, "Process callback failed for stage %s, error=%#x", GetStageString(stage),
