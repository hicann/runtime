...
static void ConstructLaunchGqmInstr(
    const rtStarsCondIsaRegister_t dstReg, const rtStarsCondIsaRegister_t srcReg1,
...
static void InitMbufOpDot(const rtStarsCondIsaRegister_t dstReg, uint64_t cntAddr, MbufOpDotInitFc& fc)
{
    constexpr rtStarsCondIsaRegister_t r0 = RT_STARS_COND_ISA_REGISTER_R0;

    ConstructLLWI(dstReg, cntAddr, fc.llwiDotAddr);
    ConstructLHWI(dstReg, cntAddr, fc.lhwiDotAddr);
    ConstructStore(dstReg, r0, 0U, RT_STARS_COND_ISA_STORE_FUNC3_SB, fc.initDotVal);
...
{
    ConstructLLWI(avail_r1, mbufOpCntAddr, fc.llwiCntAddr);
    ConstructLHWI(avail_r1, mbufOpCntAddr, fc.lhwiCntAddr);
    ConstructLoad(avail_r1, 0U, avail_r2, RT_STARS_COND_ISA_LOAD_FUNC3_LDR, fc.ldrCntAddr);
    ConstructOpImmAndi(avail_r2, avail_r2, 1U, RT_STARS_COND_ISA_OP_IMM_FUNC3_ADDI, fc.addiCntAddr);
    ConstructStore(avail_r1, avail_r2, 0U, RT_STARS_COND_ISA_STORE_FUNC3_SB, fc.incCnt);

    return;
}

static void ConstructMbufTrace(
    CondMbufTraceFc& fc, const CondMbufTraceParam& funcCallPara, MbufTraceRegParam& param, const uint32_t traceFcNop)
{
    ...
    ConstructOpImmSlli(
        param.avail_reg0, param.avail_reg0, dqsBlockIdOffset, RT_STARS_COND_ISA_OP_IMM_FUNC3_SRLI,
    ...
    ConstructBranch(
        param.avail_reg2, r0, RT_STARS_COND_ISA_BRANCH_FUNC3_BEQ, static_cast<uint8_t>(traceFcNop), fc.beqForBranch);
    ...
    // cfg use PA
    ConstructSystemCsr(
        param.avail_reg0, r0, RT_STARS_COND_CSR_AXI_USER_REG, RT_STARS_COND_ISA_SYSTEM_FUNC3_CSRRC,
        fc.csrrcForLpSysCntAddr);
    // 读取sys cnt
    ConstructLoad(param.avail_reg3, 0U, param.avail_reg3, RT_STARS_COND_ISA_LOAD_FUNC3_LDR, fc.ldrLpSysCnt);
    // restore to use VA
    ConstructSystemCsr(
        param.avail_reg0, r0, RT_STARS_COND_CSR_AXI_USER_REG, RT_STARS_COND_ISA_SYSTEM_FUNC3_CSRRS,
        fc.csrrsForLpSysCntAddr);
...
{
    // free pre/post dot init
    InitMbufOpDot(r1, funcCallPara.freePreDotAddr, fc.freePreDotInitFc);
    InitMbufOpDot(r1, funcCallPara.freePostDotAddr, fc.freePostDotInitFc);
    ...
    // 根据mbufHandleAddr的地址 r2 读mbuf handle 到 r4 寄存器中, LOAD 64bit数据，而mbuf handle是32bit的,
    // 存在读越界风险(数组多开4Byte)
    ...
    // increment free pre dot cnt
    AddMbufOpDot(funcCallPara.freePreDotAddr, fc.freepreCntFc, r10, r9);
    ...
    // increment freePostDotAddr
    AddMbufOpDot(funcCallPara.freePostDotAddr, fc.freePostCntFc, r10, r9);
...
{
    // enquePostDot / enquePreDot
    InitMbufOpDot(r1, funcCallPara.enquePreDotAddr, fc.enqueuePreDotInitFc);
    InitMbufOpDot(r1, funcCallPara.enquePostDotAddr, fc.enqueuePostDotInitFc);
    ...
    ConstructOpImmSlli(
        r6, r6, dqsErrCodeRightShift, RT_STARS_COND_ISA_OP_IMM_FUNC3_SRLI, RT_STARS_COND_ISA_OP_IMM_FUNC7_SRLI,
        fc.srli1);
    ...
    ConstructOpImmSlli(r5, r5, 32U, RT_STARS_COND_ISA_OP_IMM_FUNC3_SRLI, RT_STARS_COND_ISA_OP_IMM_FUNC7_SRLI, fc.srli2);
    ...
    MbufTraceRegParam enqueRegInfo = {
        ...
        .avail_reg3 = r8};
    mbufTraceNop =
        static_cast<uint32_t>((RtPtrToValue(&(fc.enqueMbufTraceFc.nop)) - RtPtrToValue(&fc)) / sizeof(uint32_t));
    ...
    // increment enquePreDotAddr
    AddMbufOpDot(funcCallPara.enquePreDotAddr, fc.enqueOpPreCntFc, r4, r6);

    // Qmanager enqueue
    ConstructStore(r7, r5, 0U, RT_STARS_COND_ISA_STORE_FUNC3_SW, fc.enquei);

    // increment enquePostDotAddr
    AddMbufOpDot(funcCallPara.enquePostDotAddr, fc.enqueOpPostCntFc, r5, r7);
    ...
    ConstructSystemCsr(r7, r0, RT_STARS_COND_CSR_CSQ_STATUS_REG, RT_STARS_COND_ISA_SYSTEM_FUNC3_CSRRW, fc.dfxFsm1);
    ...
    ConstructSystemCsr(r8, r0, RT_STARS_COND_CSR_CSQ_STATUS_REG, RT_STARS_COND_ISA_SYSTEM_FUNC3_CSRRW, fc.dfxFsm2);
...
void ConstructDqsDequeueFc(RtStarsDqsDequeueFc& fc, const RtStarsDqsFcPara& funcCallPara)
{
    ...
    // dequeuePostDot = 0
    InitMbufOpDot(r4, funcCallPara.dequeuePostDotAddr, fc.dequeuePostDotInitFc);
    ...
    uint64_t offset = static_cast<uint64_t>(RtPtrToPtr<const uint32_t*>(&fc.llwi5) - RtPtrToPtr<const uint32_t*>(&fc));
    ...
    AddMbufOpDot(funcCallPara.dequeuePostDotAddr, fc.dequePostDotFc, r4, r5);
    ...
    ConstructSystemCsr(r4, r1, RT_STARS_COND_CSR_CSQ_STATUS_REG, RT_STARS_COND_ISA_SYSTEM_FUNC3_CSRRW, fc.dfxFsm1);
    ConstructErrorInstr(fc.err1);

    ConstructLLWI(r4, mbufInvalidErrCode, fc.llwi6);
    ConstructSystemCsr(r4, r1, RT_STARS_COND_CSR_CSQ_STATUS_REG, RT_STARS_COND_ISA_SYSTEM_FUNC3_CSRRW, fc.dfxFsm2);
    ConstructErrorInstr(fc.err2);
...
void ConstructDqsBatchDequeueFc(RtStarsDqsBatchDequeueFc& fc, const RtStarsDqsBatchDeqFcPara& funcCallPara)
{
    ...
    ConstructBranch(
        r10, r0, RT_STARS_COND_ISA_BRANCH_FUNC3_BEQ, static_cast<uint8_t>(offset),
        fc.eqNoData); // 如果当前input没数据，则继续下一个循环
    ...
    // // pop status [63:23] reserve, [22:8] remain counter, [7:0] return code
    ConstructOpImmSlli(
        r5, r5, 56U, RT_STARS_COND_ISA_OP_IMM_FUNC3_SLLI, RT_STARS_COND_ISA_OP_IMM_FUNC7_SLLI, fc.slliGqmErrcode);
    ...
    // r5 = r10 + r5
    ConstructOpOp(r5, r10, r5, RT_STARS_COND_ISA_OP_FUNC3_ADD, RT_STARS_COND_ISA_OP_FUNC7_ADD, fc.AddQueueStatus);
    ...
    /* r1 = newValue;最新的元素的下标恒为0 */
    ConstructLoad(r10, 0U, r1, RT_STARS_COND_ISA_LOAD_FUNC3_LDR, fc.ldrNewValue);
    ...
    ConstructOpImmAndi(
        r3, r3, funcCallPara.sizeofHandleCache, RT_STARS_COND_ISA_OP_IMM_FUNC3_ADDI, fc.addi1UpdateInputMbufHandleAddr);
...
void ConstructDqsFrameAlignForDssFc(RtStarsDqsFrameAlignForDssFc& fc, const RtStarsDqsFrameAlignFcPara& fcPara)
{
    ...
    ConstructOpImmSlli(
        r6, r6, 48U, RT_STARS_COND_ISA_OP_IMM_FUNC3_SLLI, RT_STARS_COND_ISA_OP_IMM_FUNC7_SLLI, fc.slliCnt);
    ConstructOpImmSlli(
        r6, r6, 48U, RT_STARS_COND_ISA_OP_IMM_FUNC3_SRLI, RT_STARS_COND_ISA_OP_IMM_FUNC7_SRLI, fc.srliCnt);
    ...
    ConstructOpImmAndi(
        r1, r1, static_cast<uint32_t>(sizeof(uint32_t)), RT_STARS_COND_ISA_OP_IMM_FUNC3_ADDI, fc.addiInputMbufListAddr);
    ConstructOpImmAndi(
        r2, r2, fcPara.sizeofHandleCache, RT_STARS_COND_ISA_OP_IMM_FUNC3_ADDI, fc.addiInputMbufCacheListAddr);
...
static void ConstructDqsFrameAlignCheckPhase(RtStarsDqsFrameAlignFc& fc, const RtStarsDqsFrameAlignFcPara& fcPara)
{
    ...
    ConstructOpImmSlli(
        r6, r6, 48U, RT_STARS_COND_ISA_OP_IMM_FUNC3_SLLI, RT_STARS_COND_ISA_OP_IMM_FUNC7_SLLI, fc.slliCheckCnt);
    ConstructOpImmSlli(
        r6, r6, 48U, RT_STARS_COND_ISA_OP_IMM_FUNC3_SRLI, RT_STARS_COND_ISA_OP_IMM_FUNC7_SRLI, fc.srliCheckCnt);
    ...
    ConstructOpImmAndi(
        r2, r2, fcPara.sizeofHandleCache, RT_STARS_COND_ISA_OP_IMM_FUNC3_ADDI, fc.addiCheckCacheListAddr);
...
static void ConstructDqsFrameAlignCommitPhase(RtStarsDqsFrameAlignFc& fc, const RtStarsDqsFrameAlignFcPara& fcPara)
{
    ...
    ConstructOpImmSlli(
        r6, r6, 48U, RT_STARS_COND_ISA_OP_IMM_FUNC3_SLLI, RT_STARS_COND_ISA_OP_IMM_FUNC7_SLLI, fc.slliCommitCnt);
    ConstructOpImmSlli(
        r6, r6, 48U, RT_STARS_COND_ISA_OP_IMM_FUNC3_SRLI, RT_STARS_COND_ISA_OP_IMM_FUNC7_SRLI, fc.srliCommitCnt);
    ...
    ConstructBranch(
        r6, r7, RT_STARS_COND_ISA_BRANCH_FUNC3_BNE, static_cast<uint8_t>(offset), fc.bneLoadHandle1IfCntNotOne);
    ...
    ConstructBranch(
        r0, r0, RT_STARS_COND_ISA_BRANCH_FUNC3_BEQ, static_cast<uint8_t>(offset), fc.beqStoreSelectedHandle);

    // cnt!=1时加载handle_array[1]，与cnt==1分支共用后续写input_mbuf_list逻辑。
    ConstructLoad(
        r2, static_cast<uint16_t>(sizeof(uint32_t)), r9, RT_STARS_COND_ISA_LOAD_FUNC3_LDR, fc.ldrCacheHandle1);
    ...
    ConstructOpImmAndi(
        r1, r1, static_cast<uint32_t>(sizeof(uint32_t)), RT_STARS_COND_ISA_OP_IMM_FUNC3_ADDI,
        fc.addiCommitMbufListAddr);
    ConstructOpImmAndi(
        r2, r2, fcPara.sizeofHandleCache, RT_STARS_COND_ISA_OP_IMM_FUNC3_ADDI, fc.addiCommitCacheListAddr);
    ...
    ConstructBranch(r4, r3, RT_STARS_COND_ISA_BRANCH_FUNC3_BLTU, static_cast<uint8_t>(offset), fc.bltuCommitStartLoop);
}
...
void ConstructDqsPrepareFc(RtStarsDqsPrepareOutFc& fc, const RtStarsDqsPrepareFcPara& fcPara)
{
    ...
    // r2 = block_id
    ConstructOpImmSlli(
        r2, r2, dqsBlockIdOffset, RT_STARS_COND_ISA_OP_IMM_FUNC3_SRLI, RT_STARS_COND_ISA_OP_IMM_FUNC7_SRLI, fc.srli11);
    ...
    AddMbufOpDot(fcPara.allocPostDotAddr, fc.allocPostCntFc, r9, r10);
    ...
    MbufTraceRegParam param = {
        ...
        .avail_reg3 = r8};
    const uint32_t mbufTraceNop =
        static_cast<uint32_t>((RtPtrToValue(&(fc.allocMbufTracefc.nop)) - RtPtrToValue(&fc)) / sizeof(uint32_t));
    ...
    // r4 = block_id
    ConstructOpImmSlli(
        r4, r4, dqsBlockIdOffset, RT_STARS_COND_ISA_OP_IMM_FUNC3_SRLI, RT_STARS_COND_ISA_OP_IMM_FUNC7_SRLI, fc.srli14);
...
void ConstructDqsZeroCopyFc(RtStarsDqsZeroCopyFc& fc, const RtStarsDqsZeroCopyPara& funcCallPara)
{
    ...
    ConstructOpImmSlli(
        r4, r4, dqsBlockIdOffset, RT_STARS_COND_ISA_OP_IMM_FUNC3_SRLI, RT_STARS_COND_ISA_OP_IMM_FUNC7_SRLI, fc.srli3);
    ...
    // r8: index << 3 == index * 8
    ConstructOpImmSlli(r6, r8, 3U, RT_STARS_COND_ISA_OP_IMM_FUNC3_SLLI, RT_STARS_COND_ISA_OP_IMM_FUNC7_SLLI, fc.slli3);
    ...
    if (funcCallPara.isCpyAddrLow32First) {
        ...
        ConstructOpImmSlli(
            r4, r4, 32U, RT_STARS_COND_ISA_OP_IMM_FUNC3_SRLI, RT_STARS_COND_ISA_OP_IMM_FUNC7_SRLI, fc.srli4);
...
void ConstructConditionCopyFc(RtStarsDqsConditionCopyFc& fc, const RtStarsDqsConditionCopyPara& funcCallPara)
{
    ...
    ConstructSystemCsr(r7, r1, RT_STARS_COND_CSR_CSQ_STATUS_REG, RT_STARS_COND_ISA_SYSTEM_FUNC3_CSRRW, fc.dfxFsm1);
    ConstructErrorInstr(fc.err);
    ...
    RtStarsDqsInterChipPostProcFc& fc, const RtStarsDqsInterChipPostProcPara& funcCallPara)
{
    ...
    ConstructOpImmSlli(
        r1, r1, dqsErrCodeRightShift, RT_STARS_COND_ISA_OP_IMM_FUNC3_SRLI, RT_STARS_COND_ISA_OP_IMM_FUNC7_SRLI,
        fc.srli1);
    ...
    MbufTraceRegParam dstProdFreePara = {
        ...
        .avail_reg3 = r9};
    uint32_t mbufTraceNop =
        static_cast<uint32_t>((RtPtrToValue(&(fc.dstProdFreeMbufTracefc.nop)) - RtPtrToValue(&fc)) / sizeof(uint32_t));
    ...
    MbufTraceRegParam dstProdEnqueRegInfo = {
        ...
        .avail_reg3 = r10};
    mbufTraceNop =
        static_cast<uint32_t>((RtPtrToValue(&(fc.dstProdEnqueMbufTraceFc.nop)) - RtPtrToValue(&fc)) / sizeof(uint32_t));
    ConstructMbufTrace(
        fc.dstProdEnqueMbufTraceFc, funcCallPara.dstProdEnqueMbufTracePara, dstProdEnqueRegInfo, mbufTraceNop);
    ...
    MbufTraceRegParam srcConsFreePara = {
        ...
        .avail_reg3 = r7};
    mbufTraceNop =
        static_cast<uint32_t>((RtPtrToValue(&(fc.srcConsFreembufTraceFc.nop)) - RtPtrToValue(&fc)) / sizeof(uint32_t));
    ...
    ConstructSystemCsr(r2, r0, RT_STARS_COND_CSR_CSQ_STATUS_REG, RT_STARS_COND_ISA_SYSTEM_FUNC3_CSRRW, fc.dfxFsm1);
    ConstructErrorInstr(fc.err1);

    // error2
    ConstructLLWI(r3, OW_NOT_ENABLE_ERR_CODE, fc.llwi12);
    ConstructLHWI(r3, OW_NOT_ENABLE_ERR_CODE, fc.lhwi12);
    ConstructSystemCsr(r3, r0, RT_STARS_COND_CSR_CSQ_STATUS_REG, RT_STARS_COND_ISA_SYSTEM_FUNC3_CSRRW, fc.dfxFsm2);
    ConstructErrorInstr(fc.err2);
...
void ConstructDqsAdspcFc(RtStarsDqsAdspcFc& fc, const RtStarsDqsAdspcFcPara& fcPara)
{
    ...
    ConstructOpImmAndi(
        r2, r2, static_cast<uint32_t>(fcPara.cqeHeadTailMask), RT_STARS_COND_ISA_OP_IMM_FUNC3_ANDI, fc.andi1);
    ...
    ConstructOpImmAndi(
        r3, r3, static_cast<uint32_t>(fcPara.cqeHeadTailMask), RT_STARS_COND_ISA_OP_IMM_FUNC3_ANDI, fc.andi2);
    ...
    ConstructOpImmAndi(
        r2, r2, static_cast<uint32_t>(fcPara.cqeHeadTailMask), RT_STARS_COND_ISA_OP_IMM_FUNC3_ANDI, fc.andi3);
    // store cqHead
    ...
    ConstructOpImmSlli(r1, r2, 32U, RT_STARS_COND_ISA_OP_IMM_FUNC3_SLLI, RT_STARS_COND_ISA_OP_IMM_FUNC7_SLLI, fc.slli1);
    ConstructOpImmSlli(
        r2, r2, dqsErrCodeRightShift, RT_STARS_COND_ISA_OP_IMM_FUNC3_SRLI, RT_STARS_COND_ISA_OP_IMM_FUNC7_SRLI,
        fc.srli1);
