...
static std::map<DqsInterChipTaskType, DqsInterChipTaskInitInfo> DQS_INTER_CHIP_TASK_INIT_FUNC = {
    {DqsInterChipTaskType::DQS_INTER_CHIP_TASK_PREPROC, {"dqs inter-chip pre-proc task", &DqsInterChipPreProcTaskInit}},
    ...
    {DqsInterChipTaskType::DQS_INTER_CHIP_TASK_NOP, {"dqs inter-chip nop task", &DqsInterChipNopTaskInit}},
};
...
static inline rtError_t GetDqsInterChipTaskInitInfo(
    const DqsInterChipTaskType type, DqsInterChipTaskInitInfo** taskInitInfo)
{
...
static rtError_t SendAccSubscribeInfo(
    StreamWithDqs* const streamWithDqs, const uint8_t queNum, uint32_t notifyId, const uint16_t* queIdArray,
    bool isCntNotify)
{
    ...
    COND_RETURN_ERROR(
        (error != RT_ERROR_NONE), error, "ioctl failed, cmd=%#x, retCode=%#x", STARS_IOCTL_CMD_ACC_SUBSCRIBE_QUEUE,
...
static rtError_t DqsCountNotifyWaitCreate(StreamWithDqs* const streamWithDqs, bool& createdFlag)
{
    ...
    if (cntNotify == nullptr) {
        cntNotify =
            new (std::nothrow) CountNotify(streamWithDqs->Device_()->Id_(), streamWithDqs->Device_()->DevGetTsId());
        COND_RETURN_ERROR_MSG_CALL(
            ERR_MODULE_SYSTEM, cntNotify == nullptr, RT_ERROR_NOTIFY_NEW,
...
static rtError_t LaunchDqsTaskByType(
    const rtDqsTaskType type, Stream* const stm, const DqsTaskConfig* const cfg = nullptr)
{
    DqsTaskInitInfo* taskInitInfo = nullptr;
    rtError_t error = GetDqsTaskInitInfo(type, &taskInitInfo);
    COND_RETURN_ERROR(
        (error != RT_ERROR_NONE) || (taskInitInfo == nullptr), error, "get dqs task init info failed, retCode=%#x",
        static_cast<uint32_t>(error));
...
static rtError_t LaunchDqsInterChipTaskByType(
    Stream* const stm, const uint32_t blockIdx, const DqsInterChipTaskType type)
{
...
static rtError_t InterChipDqsTaskInit(Stream* const stream)
{
    ...
    for (uint32_t groupIdx = 0U; groupIdx < grpCnt; groupIdx++) {
        ...
        ERROR_RETURN(
            result, "Init pre-proc task failed for groupIdx=%u, ret=%#x", groupIdx, static_cast<uint32_t>(result));

        result =
            LaunchDqsInterChipTaskByType(stream, groupIdx, DqsInterChipTaskType::DQS_INTER_CHIP_TASK_MEMCPY_MBUF_HEAD);
        ERROR_RETURN(
            result, "Init memcpy head task failed for groupIdx=%u, ret=%#x", groupIdx, static_cast<uint32_t>(result));

        result =
            LaunchDqsInterChipTaskByType(stream, groupIdx, DqsInterChipTaskType::DQS_INTER_CHIP_TASK_MEMCPY_MBUF_DATA);
        ERROR_RETURN(
            result, "Init memcpy data task failed for groupIdx=%u, ret=%#x", groupIdx, static_cast<uint32_t>(result));
...
static rtError_t DqsZeroCopyTaskCfgCheck(const rtDqsZeroCopyCfg_t* const cfg)
{
    ...
    NULL_PTR_RETURN_MSG_OUTER_WITH_FUNC_DESC(
        cfg->offset, RT_ERROR_INVALID_VALUE, "Checking the configuration validity of the DQS zero-copy task");
...
{
    NULL_PTR_RETURN_MSG_OUTER_WITH_FUNC_DESC(
        cfg, RT_ERROR_INVALID_VALUE, "Checking the configuration validity of the DQS conditional copy task");
    NULL_PTR_RETURN_MSG_OUTER_WITH_FUNC_DESC(
        cfg->condition, RT_ERROR_INVALID_VALUE, "Checking the configuration validity of the DQS conditional copy task");
    NULL_PTR_RETURN_MSG_OUTER_WITH_FUNC_DESC(
        cfg->dst, RT_ERROR_INVALID_VALUE, "Checking the configuration validity of the DQS conditional copy task");
    NULL_PTR_RETURN_MSG_OUTER_WITH_FUNC_DESC(
        cfg->src, RT_ERROR_INVALID_VALUE, "Checking the configuration validity of the DQS conditional copy task");
    ZERO_RETURN_MSG(cfg->cnt);

    COND_RETURN_ERROR_MSG_INNER(
        (RtPtrToValue(cfg->dst) % 8ULL) != 0ULL, RT_ERROR_INVALID_VALUE,
        "dstAddr must be 8-byte aligned, actual dstAddr=%p", cfg->dst);
    COND_RETURN_ERROR_MSG_INNER(
        (RtPtrToValue(cfg->src) % 8ULL) != 0ULL, RT_ERROR_INVALID_VALUE,
        "srcAddr must be 8-byte aligned, actual srcAddr=%p", cfg->src);
    COND_RETURN_ERROR_MSG_INNER(
        (cfg->cnt % 8ULL) != 0ULL, RT_ERROR_INVALID_VALUE, "cnt must be 8-byte aligned, actual cnt=%" PRIu64 " bytes",
        cfg->cnt);
    COND_RETURN_ERROR_MSG_INNER(
        (cfg->cnt > cfg->destMax), RT_ERROR_INVALID_VALUE,
        "cnt (%" PRIu64 " bytes) exceeds destination buffer capacity (%" PRIu64 " bytes)", cfg->cnt, cfg->destMax);
    COND_RETURN_ERROR_MSG_INNER(
        (cfg->cnt > DQS_CONDITION_COPY_SIZE_MAX), RT_ERROR_INVALID_VALUE,
        "cnt (%" PRIu64 " bytes) exceeds maximum limit (%" PRIu64 " bytes)", cfg->cnt, DQS_CONDITION_COPY_SIZE_MAX);
...
{
    NULL_PTR_RETURN_MSG_OUTER_WITH_FUNC_DESC(
        taskCfg->cfg, RT_ERROR_INVALID_VALUE, "Starting a DQS scheduling configuration task");
...
{
    NULL_PTR_RETURN_MSG_OUTER_WITH_FUNC_DESC(
        taskCfg->cfg, RT_ERROR_INVALID_VALUE, "Starting a DQS cross-chip solver (ADSPC) task");
...
{
    NULL_PTR_RETURN_MSG_OUTER_WITH_FUNC_DESC(
        taskCfg->cfg, RT_ERROR_INVALID_VALUE, "Starting a DQS conditional copy task");
