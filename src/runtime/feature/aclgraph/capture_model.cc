...
rtError_t CaptureModel::ExecuteCommon(Stream * const stm, int32_t timeout, const uint8_t executeMode)
{
    ...
    if (IsCapturing()) {
        RT_LOG_OUTER_MSG_IMPL(ErrorCode::EE1017, "Checking whether the capture model is ready", "modelRI", RtFmtMsg("ModelRI (model_id=%u) is in capturing state, status=CAPTURING", Id_()));
        return RT_ERROR_MODEL_CAPTURED;
    }

    if (captureModelStatus_ != RtCaptureModelStatus::READY) {
        RT_LOG_OUTER_MSG_IMPL(ErrorCode::EE1017, "Checking whether the capture model is ready", "modelRI",
