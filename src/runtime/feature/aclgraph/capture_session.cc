...
void CaptureSession::UpdateEndGraphTaskSqeType(Stream* const captureStream) const
{
...
rtError_t CaptureSession::AllocCascadeCaptureStream(
    const Stream* const stm, Model* const captureModel, Stream** newCaptureStream)
{
    ...
    rtError_t error = ctx_->StreamCreate(
        0U, RT_STREAM_PERSISTENT, &newCaptureStreamTmp, nullptr, captureModelTmp->IsSoftwareSqEnable());
    if (error != RT_ERROR_NONE) {
        RT_LOG(
            RT_LOG_ERROR, "Capture stream create failed, device_id=%u, original stream_id=%d, retCode=%#x.",
            ctx_->Device_()->Id_(), stm->Id_(), error);
        return error;
    }
    ...
    if (captureModelTmp->IsSoftwareSqEnable()) {
        ...
        error = ctx_->ModelBindStream(captureModel, newCaptureStreamTmp, static_cast<uint32_t>(RT_INVALID_FLAG));
    }
    ...
    if (error != RT_ERROR_NONE) {
        RT_LOG(
            ...
            ctx_->Device_()->Id_(), captureModel->Id_(), newCaptureStreamTmp->Id_(), stm->Id_(), error);
...
void CaptureSession::FreeCascadeCaptureStream(Stream* const cascadeCaptureStm)
{
    ...
    if (cascadeCaptureStm->Model_() != nullptr) {
        ...
        if (captureModelTmp->IsSoftwareSqEnable()) {
            ...
            (void)ctx_->ModelDelStream(cascadeCaptureStm->Model_(), cascadeCaptureStm);
        } else {
            /* steam is bind to model, only need unbind from model */
            (void)ctx_->ModelUnbindStream(cascadeCaptureStm->Model_(), cascadeCaptureStm);
        }
    }

    (void)ctx_->StreamDestroy(cascadeCaptureStm, true);
...
rtError_t CaptureSession::StreamBeginTaskGrp(Stream* const stm)
{
    ...
    for (uint8_t idx = 0; idx < prefetchCnt; idx++) {
        const rtError_t ret = SendNopTask(ctx_, stm);
...
rtError_t CaptureSession::CreateSubCaptureModels(CondHandle* condHandle, rtCondTaskParams params, Stream* const stm)
{
    for (uint32_t loop = 0; loop < params.size; loop++) {
        Model* subModel = nullptr;
        const rtError_t ret = ctx_->ModelCreate(&subModel, RT_MODEL_CAPTURE_MODEL);
        ...
        RT_LOG(
            ...
            ctx_->Device_()->Id_(), condHandle->GetParentModel()->Id_(), subModel->Id_(), stm->Id_(),
...
rtError_t CaptureSession::SubmitCaptureConditionTask(CondHandle* condHandle, Stream* const stm)
{
...
rtError_t CaptureSession::StreamAddCondTask(
    CondHandle* condHandle, rtCondTaskParams params, Stream* const stm, uint32_t flags)
{
    UNUSED(flags);
    ...
    if (notify == nullptr) {
        notify = new (std::nothrow) Notify(ctx_->Device_()->Id_(), ctx_->Device_()->DevGetTsId());
...
rtError_t CreateSubCaptureModels(
    Context* const ctx, CondHandle* const condHandle, rtCondTaskParams params, Stream* const stm)
{
    CaptureSession* const captureSession = GetCaptureSession(ctx);
