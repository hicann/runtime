...
rtError_t Context::UpdateSuModelExeStreamNotifyWaitSqe(TaskInfo* taskInfo, Stream* const exeStream) const
{
...
rtError_t Context::StreamAddToCaptureModelProc(Stream* const stm, Model* const captureMdl, const bool isOriginal)
{
    ...
    if (captureMdl->GetModelType() != RT_MODEL_CAPTURE_MODEL) {
        RT_LOG(
            RT_LOG_ERROR, "model type not match, device_id=%u, original stream_id=%d, model_id=%u.", device_->Id_(),
            streamId, captureMdl->Id_());
    ...
    if (status != RT_STREAM_CAPTURE_STATUS_NONE) {
        RT_LOG(
            RT_LOG_ERROR, "stream is already in capture status, device_id=%u, stream_id=%d, status=%s.", device_->Id_(),
...
rtError_t Context::StreamBeginCapture(Stream* const stm, const rtStreamCaptureMode mode, Model* const mdl)
{
    ...
    COND_RETURN_AND_MSG_OUTER(
        ...
        RtFmtMsg(
            "Stream is already in capture status, device_id=%u, stream_id=%d, status=%s", device_->Id_(), streamId,
    ...
    if ((stm->Device_()->IsSupportFeature(RtOptionalFeatureType::RT_FEATURE_MODEL_ACL_GRAPH_SOFTWARE_ENABLE)) &&
        ...
        RT_LOG(
            RT_LOG_DEBUG, "Capture model set software sq enable, device_id=%u, model_id=%u", device_->Id_(),
            captureModel->Id_());
    }
...
rtError_t Context::CheckCaptureModelValidity(Model* const captureMdl) const
{
    ...
    for (auto it = streams.begin(); it != streams.end(); it++) {
        ...
        COND_RETURN_ERROR(
            (event->GetEventFlag() == RT_EVENT_EXTERNAL), RT_ERROR_STREAM_UNJOINED,
            "The event flag is external, stream_id=%d, task_id=%u, event_id=%d.", streamId, taskId, event->EventId_());
        COND_RETURN_ERROR(
            (event->IsCaptureStreamWaited() == false), RT_ERROR_STREAM_UNJOINED,
            "A free-state event record task was discovered, stream_id=%d, task_id=%u, event_id=%d.", streamId, taskId,
            event->EventId_());
    ...
    COND_RETURN_ERROR(
        ((isOnlyOrigStream == false) && (hasRecordOrigStream == false)), RT_ERROR_STREAM_UNJOINED,
...
rtError_t Context::AddNotifyToAddedCaptureStream(Stream* const oriSingleStm, CaptureModel* const captureMdl)
{
    ...
    for (auto& streamObj : streams) {
        ...
        ERROR_RETURN(
            ...
            device_->Id_(), oriSingleStm->Id_(), captureMdl->Id_(), streamObj.second.back()->Id_(),
...
rtError_t Context::StreamEndCapture(Stream* const stm, Model** const captureMdl)
{
    ...
    *captureMdl = nullptr;
    ...
    if (!(captureStream->IsOrigCaptureStream())) {
        RT_LOG(
            RT_LOG_ERROR, "The capture was not initiated in this stream. device_id=%u, stream_id=%d.", device_->Id_(),
            stm->Id_());
        return RT_ERROR_STREAM_CAPTURE_UNMATCHED;
    }

    rtError_t error = CheckCaptureStreamThreadIsMatch(stm);
    COND_PROC_RETURN_ERROR(error != RT_ERROR_NONE, error, CaptureModeExit(stm); ClearCaptureModel(this, stm);
    ...
    if (captureModel == nullptr) {
        RT_LOG(
            RT_LOG_ERROR, "captureModel is null, device_id=%u, origin stream_id=%d, status=%s.", device_->Id_(),
    ...
    const Stream* const stm, rtStreamCaptureStatus* const status, Model** const captureMdl) const
{
    ...
    if ((statusTmp != RT_STREAM_CAPTURE_STATUS_NONE) && (captureStream != nullptr)) {
        ...
        if (mdlTmp == nullptr) {
            RT_LOG(
                RT_LOG_WARNING, "stream is not in capture status, device_id=%u, stream_id=%d.", device_->Id_(),
                stm->Id_());
        } else {
            modelId = mdlTmp->Id_();
        }
