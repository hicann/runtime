...
rtError_t XpuStream::Setup(void)
{
    ...
    COND_RETURN_AND_MSG_INNER(
        streamId_ == -1, RT_ERROR_DRV_NO_STREAM_RESOURCES,
    ...
    COND_RETURN_AND_MSG_OUTER(
        taskPublicBuff_ == nullptr, RT_ERROR_STREAM_NEW, ErrorCode::EE1013,
    ...
    COND_RETURN_AND_MSG_INNER(error != RT_ERROR_NONE, error, "Failed to create arg resource, stream_id=%d.", streamId_);
    ...
    COND_RETURN_AND_MSG_INNER(
        error != RT_ERROR_NONE, error, "Failed to allocate SQ/CQ resources, stream_id=%d, retCode=%#x.", streamId_,
...
{
    COND_RETURN_AND_MSG_INNER(tsk == nullptr, RT_ERROR_TASK_NULL, "Task info is null, stream_id=%d.", streamId_);
    ...
    RT_LOG(
        ...
        streamId_, tsk->id, tsk->taskSn, static_cast<int32_t>(tsk->type), tsk->typeName, sendSqeNum,
    ...
    TaskInfo* workTask, const bool isTaskBind, const uint16_t tailTaskId, uint16_t* const delTaskId)
{
    (void)workTask;
    (void)isTaskBind;
    COND_RETURN_AND_MSG_INNER(delTaskId == nullptr, RT_ERROR_TASK_NULL, "delTaskId is null, stream_id=%d.", streamId_);
...
rtError_t XpuStream::CreateStreamTaskRes()
{
    ...
    COND_RETURN_AND_MSG_OUTER(
        taskResMang_ == nullptr, RT_ERROR_MEMORY_ALLOCATION, ErrorCode::EE1013, sizeof(TaskResManageDavid), "new");
...
rtError_t XpuStream::SynchronizeExecutedTask(const uint32_t taskId, const mmTimespec& beginTime, int32_t timeout)
{
    ...
    while (true) {
        ...
        if (IsProcessTimeout(beginTime, reportTime)) {
            ...
            RT_LOG(
                RT_LOG_EVENT, "report three minutes timeout! stream_id=%u, task_id=%u, pendingNum=%u.", Id_(), taskId,
                GetPendingNum());
        }
...
rtError_t XpuStream::Synchronize(const bool isNeedWaitSyncCq, int32_t timeout)
{
    ...
    COND_RETURN_ERROR(
        (error != RT_ERROR_NONE), error, "Stream synchronize failed, stream_id=%d, retCode=%#x.", Id_(),
        static_cast<uint32_t>(error));
