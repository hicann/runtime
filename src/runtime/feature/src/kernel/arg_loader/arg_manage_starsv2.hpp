 ...
 */
#ifndef __CCE_RUNTIME_ARG_MANAGE_STARSV2_HPP__
#define __CCE_RUNTIME_ARG_MANAGE_STARSV2_HPP__
#include <mutex>
...
constexpr uint8_t STARSV2_ARG_STREAM_NUM_MAX = 8U;
constexpr uint32_t STARSV2_ARG_POOL_SQ_SIZE = 1024U;
...
class StarsV2ArgManage {
public:
    explicit StarsV2ArgManage(Stream * const stm) : stream_(stm) {}

    virtual ~StarsV2ArgManage()
    {
        stream_ = nullptr;
    }
    ...
    template<typename T>
    rtError_t LoadArgs(const T *argsInfo, const bool useArgPool, StarsV2ArgLoaderResult * const result)
    {
    ...
    virtual bool AllocStmPool(const uint32_t size, StarsV2ArgLoaderResult * const result) = 0;
    ...
    virtual rtError_t AllocCopyPtr(const uint32_t size, const bool useArgPool,
                                   StarsV2ArgLoaderResult * const result) = 0;
    ...
    virtual rtError_t H2DArgCopy(const StarsV2ArgLoaderResult * const result, void * const args, const uint32_t size) = 0;
...
private:
    void FreeFail(StarsV2ArgLoaderResult * const result);

    rtError_t LoadInputOutputArgs(const StarsV2ArgLoaderResult * const result, const rtArgsEx_t * const argsInfo);
    rtError_t LoadInputOutputArgs(const StarsV2ArgLoaderResult * const result, const rtAicpuArgsEx_t * const argsInfo);
    rtError_t LoadInputOutputArgs(const StarsV2ArgLoaderResult * const result, const rtFusionArgsEx_t * const argsInfo);
...
class UbArgManage : public StarsV2ArgManage {
public: 
    using StarsV2ArgManage::StarsV2ArgManage;
    ~UbArgManage() override = default;
    rtError_t MallocArgMem(void *&devAddr, void *&hostAddr) override;
    void FreeArgMem() override;
    bool AllocStmPool(const uint32_t size, StarsV2ArgLoaderResult * const result) override;
    rtError_t AllocCopyPtr(const uint32_t size, const bool useArgPool, StarsV2ArgLoaderResult * const result) override;
    rtError_t H2DArgCopy(const StarsV2ArgLoaderResult * const result, void * const args, const uint32_t size) override;
    void RecycleDevLoader(void * const handle) override;
private:
    rtError_t ParseArgsCpyWqe(const void *hostPtr, const void *devPtr, const uint32_t size) const;
};

class PcieArgManage : public StarsV2ArgManage {
public:
    using StarsV2ArgManage::StarsV2ArgManage;
...
#endif // __CCE_RUNTIME_ARG_MANAGE_STARSV2_HPP__
