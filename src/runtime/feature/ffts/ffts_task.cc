...
rtError_t CopyFftsPlusContextToHost(TaskInfo* taskInfo, const uint32_t contextId, void* ctxInfo)
{
    ...
    COND_RETURN_ERROR_MSG_INNER(
        (ctxInfo == nullptr) || (fftsPlusTask->descAlignBuf == nullptr) ||
            ((offset + CONTEXT_LEN) > fftsPlusTask->descBufLen),
        RT_ERROR_INVALID_VALUE, "invalid ffts context copy request, context_id=%u, descBufLen=%" PRIu64 ".", contextId,
    ...
    TaskInfo* taskInfo, const uint32_t devId, const uint32_t contextId, const uint8_t* ctxInfo)
{
    ...
    RT_LOG(
        RT_LOG_ERROR,
        "fftsplus context detail, dev_id=%u, stream_id=%d, task_id=%u, context_id=%u, "
        "context_type=%u[%s], successor_num=%u, thread_id=%u, thread_dim=%u.",
        devId, taskInfo->stream->Id_(), taskInfo->id, contextId, commonCtx->contextType, contextTypeName.c_str(),
        commonCtx->successorNum, commonCtx->threadId, commonCtx->threadDim);
    for (uint32_t j = 0U; j < (CONTEXT_LEN / sizeof(uint32_t)); ++j) {
        RT_LOG(
            RT_LOG_ERROR, "FftsPlusTask-%s The %u context-buf[%u]=%#010x.", contextTypeName.c_str(), contextId, j,
            buf[j]);
    }
}
...
rtError_t CheckFftsPlusDsaContext(const void* const descBuf, const uint64_t descBufLen)
{
    ...
    COND_RETURN_ERROR_MSG_INNER(
        descBuf == nullptr, RT_ERROR_INVALID_VALUE, "fftsplus desc buffer is null, desc_buf_len=%" PRIu64 ".",
        descBufLen);
    COND_RETURN_ERROR_MSG_INNER(
        (descBufLen % CONTEXT_LEN) != 0U, RT_ERROR_INVALID_VALUE,
...
rtError_t CheckFftsPlusDsaContextFromDevice(TaskInfo* taskInfo, const rtFftsPlusTaskInfo_t* const fftsPlusTaskInfo)
{
    ...
    COND_RETURN_ERROR_MSG_INNER(
        fftsPlusTaskInfo->descBuf == nullptr, RT_ERROR_INVALID_VALUE,
    ...
    const rtError_t error = taskInfo->stream->Device_()->Driver_()->MemCopySync(
        hostDesc.data(), fftsPlusTask->descBufLen, fftsPlusTaskInfo->descBuf, fftsPlusTask->descBufLen,
        RT_MEMCPY_DEVICE_TO_HOST);
    COND_RETURN_ERROR_MSG_INNER(
        error != RT_ERROR_NONE, error,
        "copy fftsplus device desc to host failed, retCode=%#x, desc_buf_len=%" PRIu64 ".", error,
        fftsPlusTask->descBufLen);
...
rtError_t FillFftsPlusSqe(TaskInfo* taskInfo, const void* const devMem)
{
    ...
    if (supportFlag &&
        ...
        RT_LOG(
            RT_LOG_INFO, "Begin to Create Ffts Plus Task timeoutConfig=%u, timeout=%us, subType=%u.", timeoutConfig,
            fftsPlusTask->fftsSqe.timeout, fftsPlusTask->fftsSqe.subType);
    }
    ...
    if (fftsPlusTask->errInfo == nullptr) {
        ...
        COND_RETURN_AND_MSG_OUTER(
            fftsPlusTask->errInfo == nullptr, RT_ERROR_MEMORY_ALLOCATION, ErrorCode::EE1013,
            std::to_string(sizeof(std::vector<rtFftsPlusTaskErrInfo_t>)), "new");
    }

    return RT_ERROR_NONE;
}

static void ShowFftsPlusTaskDebug(
    TaskInfo* taskInfo, const rtFftsPlusTaskInfo_t* const fftsPlusTaskInfo, const FftsPlusTaskInfo* fftsPlusTask)
{
    ...
    for (uint32_t i = 0U; i < (fftsPlusTask->descBufLen / 128U); i++) {
        RT_LOG(
            RT_LOG_INFO, "stream_id=%d,task_id=%u,FftsPlusTask-The %u context:", taskInfo->stream->Id_(), taskInfo->id,
            i);
...
static rtError_t FftsPlusTmpAllocH2D(
    TaskInfo* taskInfo, const rtFftsPlusTaskInfo_t* const fftsPlusTaskInfo, FftsPlusTaskInfo* fftsPlusTask)
{
    ...
    COND_RETURN_ERROR(
        (ret != RT_ERROR_NONE) || (fftsPlusTask->descBuf == nullptr), ret,
        "alloc fftsPlusDescDev failed, retCode=%#x, descBufLen=%" PRIu64 "(bytes), dev_id=%u.", ret,
    ...
    ret = dev->Driver_()->MemCopySync(
        fftsPlusTask->descAlignBuf, fftsPlusTask->descBufLen, fftsPlusTaskInfo->descBuf, fftsPlusTask->descBufLen,
...
static rtError_t FftsPlusPoolH2D(
    TaskInfo* taskInfo, const rtFftsPlusTaskInfo_t* const fftsPlusTaskInfo, FftsPlusTaskInfo* fftsPlusTask)
{
    ...
    COND_RETURN_ERROR_MSG_INNER(
        error != RT_ERROR_NONE, error,
        "FftsPlusPoolH2D Failed, stream_id=%d,"
        " retCode=%#x, size=%" PRIu64,
        taskInfo->stream->Id_(), static_cast<uint32_t>(error), fftsPlusTask->descBufLen);
...
static rtError_t FftsPlusTaskFillArgsAddr(TaskInfo* taskInfo, const rtFftsPlusTaskInfo_t* const fftsPlusTaskInfo)
{
    ...
    COND_RETURN_ERROR_MSG_INNER(
        dev == nullptr, RT_ERROR_INVALID_VALUE, "device is null, stream_id=%d,", taskInfo->stream->Id_());
    ...
    if ((argsHandleInfoNum != 0) && (fftsPlusTaskInfo->argsHandleInfoPtr != nullptr)) {
        ...
        COND_RETURN_AND_MSG_OUTER(
            taskInfo->u.fftsPlusTask.argsHandleInfoPtr == nullptr, RT_ERROR_MEMORY_ALLOCATION, ErrorCode::EE1013,
            std::to_string(sizeof(std::vector<void*>)), "new");
...
rtError_t FftsPlusTaskInit(TaskInfo* taskInfo, const rtFftsPlusTaskInfo_t* const fftsPlusTaskInfo, const uint32_t flag)
{
    ...
    RT_LOG(
        RT_LOG_INFO, "totalContextNum=%hu, readyContextNum=%hu, flag=%u, descAddrType=%u, argsHandleInfoNum=%u.",
        fftsPlusTaskInfo->fftsPlusSqe->totalContextNum, fftsPlusTaskInfo->fftsPlusSqe->readyContextNum, flag,
    ...
    if (argsSize.infoAddr != nullptr) {
        ...
        RT_LOG(
            RT_LOG_INFO, "infoAddr=%p, atomicIndex=%u.", fftsPlusTask->inputArgsSize.infoAddr,
            fftsPlusTask->inputArgsSize.atomicIndex);
    }
    ...
    if (descAddrType == RT_FFTS_PLUS_CTX_DESC_ADDR_TYPE_HOST) {
        ...
        COND_PROC(error != RT_ERROR_NONE, return error);
        if ((taskInfo->stream->Device_() != nullptr) && (taskInfo->stream->Device_()->ArgLoader_() != nullptr) &&
    ...
    if (fftsPlusTask->argsHandleInfoPtr != nullptr) {
        for (auto iter : *(fftsPlusTask->argsHandleInfoPtr)) {
            if (iter != nullptr) {
                ...
                if (isModel) {
                    static_cast<Model*>(stm->Model_())
                        ->PushbackArgHandle(static_cast<uint16_t>(stm->Id_()), taskInfo->id, argHdl);
                } else {
...
static void ConstructFftsPlusDumpInfoSqe(
    const TaskInfo* const taskInfo, rtStarsSqe_t* const command, const uint64_t dumpInfo, const uint32_t len,
    const uint16_t flag)
{
    ...
    RT_LOG(
        RT_LOG_INFO, "FftsPlus add dataDumpInfoTask stream_id=%d task_id=%hu", taskInfo->stream->Id_(), taskInfo->id);
}
...
static void ConstructFftsPlusSqe(TaskInfo* taskInfo, rtStarsSqe_t* const command)
{
    ...
    if (error != EOK) {
        ...
        RT_LOG_INNER_MSG(
            RT_LOG_ERROR,
            "Failed to call memcpy_s to copy fftPlusTask->fftsSqe, src=%p, dest=%p,"
            " dest_max=%zu, count=%zu, retCode=%#x.",
            &fftPlusTask->fftsSqe, command, sizeof(rtStarsSqe_t), sizeof(fftPlusTask->fftsSqe), error);
    }

    PrintSqe(command, "FftsPlus");
    RT_LOG(
        RT_LOG_INFO, "FftsPlusTask stream_id=%d, task_id=%hu, dsa_sq_id=%u", taskInfo->stream->Id_(), taskInfo->id,
        fftPlusTask->fftsSqe.dsaSqId);
}
static void SqeTaskUpdateForAllFftsPlus(rtStarsSqe_t* const fftsplusSqe) // plan B
{
    ...
    fftsplusSqe->fftsPlusSqe.sqeHeader.rtStreamId =
        (realTaskId & RT_MASK_BIT0_BIT11) |
    ...
    fftsplusSqe->fftsPlusSqe.sqeHeader.taskId =
        (realStreamId & RT_MASK_BIT0_BIT11) | (realTaskId & (RT_MASK_BIT12 | RT_MASK_BIT13_BIT15));

    RT_LOG(
        RT_LOG_DEBUG,
        "Update fftsplus sqe for all, real_stream_id=%hu, real_task_id=%hu, "
        "new_stream_id=%hu, new_task_id=%hu",
        realStreamId, realTaskId, fftsplusSqe->fftsPlusSqe.sqeHeader.rtStreamId,
        fftsplusSqe->fftsPlusSqe.sqeHeader.taskId);
}
...
static void SqeTaskUpdateForExpandSpec(rtStarsSqe_t* const fftsplusSqe) // plan C
{
    ...
    fftsplusSqe->fftsPlusSqe.sqeHeader.rtStreamId =
        (realTaskId & RT_MASK_BIT0_BIT14) | (RT_UPDATE_FOR_STREAM_EXTEND << RT_UPDATE_FOR_STREAM_EXTEND_FLAG_BIT);
    ...
    RT_LOG(
        RT_LOG_DEBUG,
        "Update fftsplus sqe for expand spec, real_stream_id=%hu, real_task_id=%hu, "
        "new_stream_id=%hu, new_task_id=%hu",
        realStreamId, realTaskId, fftsplusSqe->fftsPlusSqe.sqeHeader.rtStreamId,
        fftsplusSqe->fftsPlusSqe.sqeHeader.taskId);
}
...
void SqeTaskUpdateForFftsPlus(TaskInfo* taskInfo, rtStarsSqe_t* const fftsplusSqe)
{
    ...
    fftsplusSqe->fftsPlusSqe.sqeHeader.rtStreamId =
        (realStreamId & RT_MASK_BIT0_BIT11) | (0x1U << RT_TASK_UPDATE_FLAG_BIT12) | (realTaskId & RT_MASK_BIT13_BIT15);
    // taskid[0:12] + (index) The hccl index occupies the highest three bits of taskid.
    fftsplusSqe->fftsPlusSqe.sqeHeader.taskId =
        ((realTaskId & (RT_MASK_BIT0_BIT11 | RT_MASK_BIT12)) |
         ((taskInfo->stream->hcclIndex_) << RT_BEGIN_INDEX_OF_HCCL_INDEX));
    RT_LOG(
        RT_LOG_DEBUG,
        "Update fftsplus sqe, real_stream_id=%hu, real_task_id=%hu, "
        "new_stream_id=%hu, new_task_id=%hu",
        realStreamId, realTaskId, fftsplusSqe->fftsPlusSqe.sqeHeader.rtStreamId,
        fftsplusSqe->fftsPlusSqe.sqeHeader.taskId);
}
...
static const std::string GetErrMsgStrForFftsPlusTask(const uint32_t errType)
{
    const std::map<uint32_t, std::string> fftsPlusErrMsgMap = {
        {FFTS_PLUS_AICORE_ERROR, "fftsplus aicore error"}, {FFTS_PLUS_AIVECTOR_ERROR, "fftsplus aivector error"},
        {FFTS_PLUS_SDMA_ERROR, "fftsplus sdma error"},     {FFTS_PLUS_AICPU_ERROR, "fftsplus aicpu error"},
        {FFTS_PLUS_DSA_ERROR, "fftsplus dsa error"},       {HCCL_FFTSPLUS_TIMEOUT_ERROR, "hccl fftsplus timeout"},
    };
...
static bool ReportFftsPlusCurrentContextDetail(
    TaskInfo* taskInfo, const uint32_t devId, const rtFftsPlusTaskErrInfo_t& info)
{
    ...
    if (ret != RT_ERROR_NONE) {
        ...
        STREAM_REPORT_ERR_MSG(
            ...
            "thread_id=%u, err_type=%u[%s], error_code=%u, retCode=%#x.",
...
void ConstructSqeForFftsPlusTask(TaskInfo* taskInfo, rtStarsSqe_t* const command)
{
    ...
    if ((fftsPlusTask->kernelFlag & RT_KERNEL_FFTSPLUS_DYNAMIC_SHAPE_DUMPFLAG) != 0U) {
        ...
        ConstructFftsPlusDumpInfoSqe(
            taskInfo, command + 2, fftsPlusTask->unloadDumpInfo, fftsPlusTask->unloadDumpInfoLen,
            STARS_DATADUMP_LOADINFO_END_BITMAP);
    } else {
...
void PrintAicAivErrorInfoForFftsPlusTask(TaskInfo* taskInfo, const rtFftsPlusTaskErrInfo_t& info, uint32_t devId)
{
    ...
    if (ret != RT_ERROR_NONE) {
        STREAM_REPORT_ERR_MSG(
            ...
            "context_id=%u, thread_id=%u, err_type=%u[%s], error_code=%u.",
            devId, taskInfo->stream->Id_(), taskInfo->id, info.contextId, info.threadId, info.errType,
    ...
    if (!kernelName.empty()) {
        STREAM_REPORT_ERR_MSG(
            ...
            "fault kernel_name=%s, blockDim=%u.",
            devId, taskInfo->stream->Id_(), taskInfo->id, info.contextId, info.threadId, info.errType,
            GetErrMsgStrForFftsPlusTask(info.errType).c_str(), taskInfo->errorCode, info.pcStart, kernelName.c_str(),
            blockDim);
    } else {
        STREAM_REPORT_ERR_MSG(
            reportStream, ERR_MODULE_RTS,
...
static void PrintSdmaErrorInfoForFftsPlusTask(
    TaskInfo* taskInfo, const rtFftsPlusTaskErrInfo_t& info, const uint32_t devId)
{
    Stream* const reportStream = GetReportStream(taskInfo->stream);
    STREAM_REPORT_ERR_MSG(
        reportStream, ERR_MODULE_RTS,
    ...
    rtExceptionArgsInfo_t* argsInfo)
{
    ...
    FftsPlusTaskInfo* fftsPlusTask = &taskInfo->u.fftsPlusTask;
    if ((fftsPlusTask->errInfo->empty()) ||
        ((errType != FFTS_PLUS_AICORE_ERROR) && (errType != FFTS_PLUS_AIVECTOR_ERROR))) {
    ...
    if ((fftsPlusTask->inputArgsSize.infoAddr == nullptr) || (fftsPlusTask->descAlignBuf == nullptr) ||
        ((offset + CONTEXT_LEN) > fftsPlusTask->descBufLen)) {
        RT_LOG(
            RT_LOG_WARNING, "FftsPlus get args failed, maxctx_num:%u, offset=%llu.",
    ...
    RT_LOG(
        RT_LOG_INFO, "FftsPlus argAddr=%p, infoAddr=%p, atomicIndex=%u.", argsInfo->argAddr,
        argsInfo->sizeInfo.infoAddr, argsInfo->sizeInfo.atomicIndex);
}

static void TaskFailCallBackForFftsPlusTask(
    TaskInfo* taskInfo, const uint32_t deviceId, const rtExceptionExpandInfo_t* const expandInfo,
    const rtFftsPlusTaskErrInfo_t& info)
{
    ...
    COND_RETURN_NORMAL(
        retCode == static_cast<uint32_t>(RT_ERROR_NONE), "task ok, stream_id=%u, task_id=%u, retCode=%#x.", streamId,
        taskId, retCode);
    COND_RETURN_NORMAL(
    ...
    GetExceptionArgsForFftsPlus(
        taskInfo, expandInfo, info.errType, &(exceptionInfo.expandInfo.u.fftsPlusInfo.exceptionArgs));
...
static void DumpContext(TaskInfo* taskInfo)
{
    ...
    RT_LOG(
        RT_LOG_ERROR,
        "dump contexts begin, stream_id=%d, task_id=%u, "
        "total_ctx_num=%u, print_num=%u, descAlignBuf=%p, descBufLen=%llu",
        taskInfo->stream->Id_(), taskInfo->id, totalCtxNum, printNum, fftsPlusTask->descAlignBuf,
        fftsPlusTask->descBufLen);
    ...
    for (uint32_t ctxId = 0U; ctxId < printNum; ctxId++) {
        ...
        for (uint32_t row = 0U; row < 4U; row++) {
            ...
            RT_LOG(
                RT_LOG_ERROR,
                "context[%u] buf[%02u-%02u]="
                "%08x %08x %08x %08x %08x %08x %08x %08x",
                ctxId, start, start + 7U, buf[start], buf[start + 1U], buf[start + 2U], buf[start + 3U],
                buf[start + 4U], buf[start + 5U], buf[start + 6U], buf[start + 7U]);
        }
    }

    RT_LOG(RT_LOG_ERROR, "dump contexts end, stream_id=%d, task_id=%u", taskInfo->stream->Id_(), taskInfo->id);
}
    ...
    for (auto loop : *fftsPlusTask->errInfo) {
        if ((loop.errType == FFTS_PLUS_AICORE_ERROR) || (loop.errType == FFTS_PLUS_AIVECTOR_ERROR)) {
            ...
            STREAM_REPORT_ERR_MSG(
                ...
                "FftsPlus task execution failed, dev_id=%u, stream_id=%d, task_id=%u, "
                "context_id=%u, thread_id=%u, err_type=%u[%s], error_code=%u.",
                devId, streamId, taskId, loop.contextId, loop.threadId, loop.errType,
                GetErrMsgStrForFftsPlusTask(loop.errType).c_str(), taskInfo->errorCode);
...
void DoCompleteSuccForFftsPlusTask(TaskInfo* taskInfo, const uint32_t devId)
{
    if (unlikely(taskInfo->errorCode != static_cast<uint32_t>(RT_ERROR_NONE))) {
        ...
        RT_LOG(
            RT_LOG_ERROR, "fftsplus report error, retCode=%#x, [%s].", taskInfo->errorCode,
...
void PushBackErrInfoForFftsPlusTask(TaskInfo* taskInfo, const void* errInfo, uint32_t len)
{
    ...
    if (memcpy_s(&taskErrInfo, sizeof(rtFftsPlusTaskErrInfo_t), errInfo, len) != EOK) {
        RT_LOG_INNER_MSG(
            RT_LOG_ERROR,
            "Failed to call memcpy_s to copy errInfo, src=%p, dest=%p,"
            " dest_max=%zu, count=%u.",
            errInfo, &taskErrInfo, sizeof(rtFftsPlusTaskErrInfo_t), len);
        return;
    }
...
void SetStarsResultForFftsPlusTask(TaskInfo* taskInfo, const rtCqReport_t& logicCq)
{
    if ((logicCq.errorType & RT_STARS_EXIST_ERROR) != 0U) {
        uint32_t errMap[] = {TS_ERROR_FFTSPLUS_TASK_EXCEPTION, TS_ERROR_FFTSPLUS_TASK_TRAP,
                             TS_ERROR_FFTSPLUS_TASK_TIMEOUT,   TS_ERROR_TASK_SQE_ERROR,
                             TS_ERROR_FFTSPLUS_TASK_EXCEPTION, logicCq.errorCode};
        ...
        RT_LOG(
            RT_LOG_ERROR,
            "FftsPlusTask errorCode=%u, logicCq:err=%u, errCode=%u, "
