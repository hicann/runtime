    ...
    const LaunchTaskCfgInfo_t* const launchTaskCfg)
{
    ...
    if (aicPart->schemMode == static_cast<uint8_t>(RT_SCHEM_MODE_END)) {
        aicPart->schemMode =
            (aicPart->kernel == nullptr) ? static_cast<uint8_t>(RT_SCHEM_MODE_NORMAL) : aicPart->kernel->GetSchedMode();
    }
    ...
    if (argsSize.infoAddr != nullptr) {
        ...
        RT_LOG(
            RT_LOG_INFO, "infoAddr=%p, atomicIndex=%u.", aicPart->inputArgsSize.infoAddr,
            aicPart->inputArgsSize.atomicIndex);
    } else {
    ...
    RT_LOG(
        RT_LOG_INFO,
        "aicAivType=%hhu, taskRation=%u, kernelFlag=0x%x, qos=%u,"
        " partId=%u, schemMode=%u.",
        fusionTaskInfo->aicAivType, taskRation, aicPart->kernelFlag, aicPart->qos, aicPart->partId, aicPart->schemMode);
}
...
static rtError_t PreProcAicAivTaskForFusion(
    rtFusionSubTaskInfo_t* const fusionTask, const Stream* const stm, Program*& prog, rtAicAivFusionInfo_t* aicAivInfo,
    LaunchTaskCfgInfo_t* launchTaskCfg)
{
    ...
    COND_RETURN_AND_MSG_OUTER(
        (stubFunc == nullptr && progHandle == nullptr) || (stubFunc != nullptr && progHandle != nullptr),
        RT_ERROR_INVALID_VALUE, ErrorCode::EE1017, "AIC/AIV subtask preprocessing", "stubFunc/progHandle",
    ...
    COND_RETURN_ERROR_MSG_INNER(
        error != RT_ERROR_NONE, error, "Kernel launch GetLaunchConfigInfo failed, stream_id=%d.", stm->Id_());

    uint64_t tilingKey = 0ULL;
    if (progHandle != nullptr) {
        const rtError_t handleRet = GetValidatedObject<Program>(progHandle, realProgram);
        ERROR_RETURN(
            handleRet, "Failed to validate fusion program handle, retCode=%#x.", static_cast<uint32_t>(handleRet));
    ...
    error = StreamLaunchKernelPrepare(
        stm, registeredKernel, prog, kernelAttrType, launchMdl, stubFunc, addr1, addr2, realProgram, tilingKey);
    ERROR_RETURN(
        error, "Failed to prepare kernel launch, stream_id=%d, retCode=%#x.", stm->Id_(), static_cast<uint32_t>(error));
...
static rtError_t PreProcCcuTaskForFusion(
    rtFusionSubTaskInfo_t* const fusionTask, uint8_t& sqeSubType, uint32_t& sqeLen, uint8_t& ccuArgSize,
    Stream* const stm)
{
    ...
    COND_RETURN_AND_MSG_OUTER(
        (isHas32B == 1U) && (isHas128B == 1U), RT_ERROR_INVALID_VALUE, ErrorCode::EE1017, "CCU subtask preprocessing",
        "argSize", "all CCU subtasks must use the same argSize, either 32B or 128B");
    ...
    if (isDoubleDie) {
        COND_RETURN_AND_MSG_OUTER(
            ((taskNum & 1U) == 1U), RT_ERROR_INVALID_VALUE, ErrorCode::EE1011, "CCU subtask preprocessing",
            static_cast<uint32_t>(taskNum), "taskNum", "taskNum must be an even number when double die is used");
        COND_RETURN_AND_MSG_OUTER(
            (dieIdArrange != checkArrange), RT_ERROR_INVALID_VALUE, ErrorCode::EE1017, "CCU subtask preprocessing",
            "dieIdArrange", "die IDs must alternate between die 0 and die 1 when double die is used");
        taskNum = taskNum / 2U;
    }
    ...
    // mission id的递增
    COND_RETURN_AND_MSG_OUTER(
        !CheckCcuMissionId(ccuInfo->ccuTaskInfo, ccuInfo->taskNum, isDoubleDie), RT_ERROR_INVALID_VALUE,
        ErrorCode::EE1017, "CCU subtask preprocessing", "missionId",
    ...
    RT_LOG(RT_LOG_INFO, "ccu task info: sqeSubType=%hhu, taskNum=%u.", sqeSubType, ccuInfo->taskNum);
    return RT_ERROR_NONE;
}

static void FusionKernelTaskProc(
    const rtFunsionTaskInfo_t* const fusionKernel, TaskInfo* const taskInfo,
    ...
    for (uint32_t idx = 0; idx < subKernelNum; idx++) {
        ...
        switch (subKernelType) {
            ...
            case RT_FUSION_CCU:
                ...
                break;
            default:
                RT_LOG(RT_LOG_INFO, "sub task [%u]'s type=%u.", idx, static_cast<uint32_t>(subKernelType));
                break;
        }
    }
}

static rtError_t FusionKernelTaskPreProc(
    rtFunsionTaskInfo_t* const fusionKernel, Stream* const stm, Program*& prog, uint32_t& sqeLen, uint8_t& sqeSubType,
    uint8_t& ccuArgSize, rtAicAivFusionInfo_t* aicAivInfo, LaunchTaskCfgInfo_t* launchTaskCfg)
{
    rtError_t error = RT_ERROR_NONE;
    const uint32_t subKernelNum = fusionKernel->subTaskNum;

    std::array<uint32_t, RT_FUSION_CCU> fusionSubTaskMove = {
        RT_FUSION_HCOMCPU_BIT_MOVE, RT_FUSION_AICPU_BIT_MOVE, RT_FUSION_AIC_BIT_MOVE};
    for (uint32_t idx = 0; idx < subKernelNum; idx++) {
        ...
        switch (subKernelType) {
            case RT_FUSION_AICORE:
                ...
                ERROR_RETURN(
                    error, "stream_id=%d failed to proc aic/aiv sub task, retCode=%#x.", stm->Id_(),
            ...
            case RT_FUSION_CCU:
                ...
                ERROR_RETURN(
                    error, "stream_id=%d failed to proc ccu sub task, retCode=%#x", stm->Id_(),
                    static_cast<uint32_t>(error));
                /* 0x18U means die0 ccu + die1 ccu */
                COND_RETURN_AND_MSG_OUTER(
                    (subKernelNum == 1U) && ((sqeSubType != 0x18U) || (ccuArgSize != RT_CCU_SQE128B_ARGS_SIZE)),
                    RT_ERROR_INVALID_VALUE, ErrorCode::EE1017, "Fusion kernel task preprocessing", "ccuArgSize",
    ...
    const rtFusionArgsEx_t* const argsInfo, TaskInfo* const taskInfo, StarsArgLoaderResult* const result)
{
    ...
    for (uint8_t i = 0U; i < aicpuNum; ++i) {
        fusionKernelTask->aicpuArgsDesc[i].soName =
            static_cast<void*>(RtPtrToPtr<char_t*>(fusionKernelTask->args) + argsInfo->aicpuArgs[i].soNameAddrOffset);
        fusionKernelTask->aicpuArgsDesc[i].funcName = static_cast<void*>(
            RtPtrToPtr<char_t*>(fusionKernelTask->args) + argsInfo->aicpuArgs[i].kernelNameAddrOffset);
    }
...
static rtError_t CheckUpdateFusionTaskInfo(
    const TaskInfo* const updateTask, uint32_t sqeLen, uint8_t sqeSubType, const Kernel* const kernel,
    uint8_t kernelType, const Stream* const stm)
{
    COND_RETURN_AND_MSG_OUTER(
        updateTask->stream->Model_() == nullptr, RT_ERROR_MODEL_NULL, ErrorCode::EE1017,
        "Checking and updating fusion task information", "task", "The update task must be a sinked task");

    COND_RETURN_AND_MSG_OUTER(
        stm->Model_() != nullptr, RT_ERROR_STREAM_MODEL, ErrorCode::EE1017,
        "Checking and updating fusion task information", "stream",
        RtFmtMsg(
            "The update stream (stream_id=%d) must be a single operator stream, not bound to a model (model_id=%u)",
            stm->Id_(), stm->Model_()->Id_()));

    COND_RETURN_AND_MSG_OUTER(
        updateTask->u.fusionKernelTask.sqeLen != sqeLen, RT_ERROR_INVALID_VALUE, ErrorCode::EE1011,
        "Checking and updating fusion task information", static_cast<uint32_t>(sqeLen), "sqeLen",
        "sqeLen must match the original fusion task's sqeLen (" +
            std::to_string(updateTask->u.fusionKernelTask.sqeLen) + ")");

    COND_RETURN_AND_MSG_OUTER(
        (sqeSubType & (1U << RT_FUSION_AIC_BIT_MOVE)) == 0U, RT_ERROR_INVALID_VALUE, ErrorCode::EE1017,
        "Checking and updating fusion task information", "sqeSubType",
...
rtError_t LaunchFusionKernel(Stream* stm, void* const fusionKernelInfo, rtFusionArgsEx_t* argsInfo)
{
    ...
    rtError_t error =
        FusionKernelTaskPreProc(fusionKernel, stm, prog, sqeLen, sqeSubType, ccuArgSize, &aicAivInfo, &launchTaskCfg);
    ...
    ERROR_PROC_RETURN_MSG_INNER(error, rt->PutProgram(prog);
                                , "stream_id=%d check failed, retCode=%#x.", stm->Id_(), static_cast<uint32_t>(error));
    ...
    ERROR_RETURN_MSG_INNER(
        error, "Failed to check SimtSmSize, stream_id=%d, retCode=%#x.", stm->Id_(), static_cast<uint32_t>(error));
    if (argsInfo != nullptr) {
        error = static_cast<DavidStream*>(dstStm)->LoadArgsInfo(argsInfo, useArgPool, &result);
        ERROR_RETURN_MSG_INNER(
            error, "Failed to load args with stm pool, stream_id=%d, useArgPool=%u, retCode=%#x.", stm->Id_(),
    ...
    ERROR_RETURN_MSG_INNER(
        error, "Submit task failed, stream_id=%d, retCode=%#x.", stm->Id_(), static_cast<uint32_t>(error));
