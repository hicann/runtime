...
halMemManagedAdvise(DVdeviceptr ptr, size_t size, uint32_t advise, struct drv_uvm_location location);
drvError_t __attribute__((weak)) halMemManagedRangeGetAttributes(
    void** data, size_t* data_sizes, uint32_t* attributes, size_t attribute_num, DVdeviceptr ptr, size_t size);
...
drvError_t __attribute__((weak)) halCheckProcessStatusEx(
...
halMbufAllocEx(uint64_t size, unsigned int align, unsigned long flag, int grp_id, Mbuf** mbuf);
drvError_t __attribute__((weak))
halGetPhyDeviceInfo(uint32_t phyId, int32_t moduleType, int32_t infoType, int64_t* value);
drvError_t __attribute__((weak))
halGetPairPhyDevicesInfo(uint32_t devId, uint32_t otherDevId, int32_t infoType, int64_t* value);
...
drvError_t __attribute__((weak))
halEschedCreateGrpEx(uint32_t devId, struct esched_grp_para* grpPara, unsigned int* grpId);
drvError_t __attribute__((weak)) halEschedThreadSwapout(unsigned int devId, unsigned int grpId, unsigned int threadId);
drvError_t __attribute__((weak)) drvDeviceStatus(uint32_t devId, drvStatus_t* status);
drvError_t __attribute__((weak)) halEschedQueryInfo(
...
DVresult __attribute__((weak))
halShmemOpenHandleV2(DVdevice dev_id, const char* name, DVdeviceptr* vptr, uint64_t flag);
DVresult __attribute__((weak)) halShmemMapRouteCheck(const char* name, DVdevice dst_devid, uint32_t map_route);
drvError_t __attribute__((weak))
halMemAddressReserve(void** ptr, size_t size, size_t alignment, void* addr, uint64_t flag);
...
drvError_t __attribute__((weak))
halMemMap(void* ptr, size_t size, size_t offset, drv_mem_handle_t* handle, uint64_t flag);
drvError_t __attribute__((weak))
halMemMapNoAccess(void* ptr, size_t size, size_t offset, drv_mem_handle_t* handle, uint64_t flag);
drvError_t __attribute__((weak))
...
drvError_t __attribute__((weak))
halMemSetAccess(void* ptr, size_t size, struct drv_mem_access_desc* desc, size_t count);
drvError_t __attribute__((weak)) halMemExportToShareableHandle(
...
drvError_t __attribute__((weak)) halMemSetPidToShareableHandle(uint64_t shareable_handle, int pid[], uint32_t pid_num);
drvError_t __attribute__((weak)) halMemGetAccess(void* ptr, struct drv_mem_location* location, uint64_t* flags);
drvError_t __attribute__((weak)) halMemTransShareableHandle(
...
drvError_t __attribute__((weak))
halMemHandleGetAttribute(drv_mem_handle_t* handle, HandleAttrType type, HandleAttr* attr);
drvError_t __attribute__((weak)) halGetHostID(uint32_t* host_id);
drvError_t __attribute__((weak)) halMemGetAllocationGranularity(
...
drvError_t __attribute__((weak))
halMemcpy(void* dst, size_t dst_size, void* src, size_t count, struct memcpy_info* info);
...
drvError_t __attribute__((weak)) halGetFaultEvent(
    uint32_t devId, struct halEventFilter* filter, struct halFaultEventInfo* eventInfo, uint32_t len,
    uint32_t* eventCount);
drvError_t __attribute__((weak)) halGetNotifyEvent(
    uint32_t devId, struct halEventFilter* filter, struct halFaultEventInfo* eventInfo, uint32_t len,
    uint32_t* eventCount);
drvError_t __attribute__((weak))
...
drvError_t __attribute__((weak))
halAsyncDmaCreate2D(uint32_t devId, struct halAsyncDmaInput2DPara* in, struct halAsyncDmaOutputPara* out);
drvError_t __attribute__((weak)) halAsyncDmaDestroy2D(uint32_t devId, struct halAsyncDmaDestroy2DPara* para);
drvError_t __attribute__((weak))
halAsyncDmaCreateBatch(uint32_t devId, struct halAsyncDmaInputBatchPara* in, struct halAsyncDmaOutputPara* out);
drvError_t __attribute__((weak)) halAsyncDmaDestroyBatch(uint32_t devId, struct halAsyncDmaDestroyBatchPara* para);

drvError_t __attribute__((weak))
halAsyncDmaJettyCreate(uint32_t devId, struct halAsyncDmaJettyCreateIn* in, struct halAsyncDmaJettyCreateOut* out);
drvError_t __attribute__((weak)) halAsyncDmaJettyDestroy(uint32_t devId, struct halAsyncJettyDestroyPara* para);
drvError_t __attribute__((weak))
halAsyncDmaJettyQuery(uint32_t devId, struct halAsyncDmaJettyQueryIn* in, struct halAsyncDmaJettyQueryOut* out);
drvError_t __attribute__((weak))
halAsyncDmaWqeConvert(uint32_t devId, struct halAsyncDmaWqeInputPara* in, struct halAsyncDmaWqeOutputPara* out);
...
drvError_t __attribute__((weak))
drvHdcSessionConnect(int peer_node, int peer_devid, HDC_CLIENT client, HDC_SESSION* session);
...
DVresult __attribute__((weak))
halMemManagedPrefetch(DVdeviceptr devPtr, size_t size, struct drv_uvm_location location, unsigned int flags);
DVresult __attribute__((weak)) halMemManagedPrefetchBatch(
    DVdeviceptr* devPtrs, size_t* sizes, size_t count, struct drv_uvm_location* prefetchLocs, size_t* prefetchLocIdxs,
...
drvError_t __attribute__((weak))
halCdqCreate(uint32_t devId, uint32_t tsId, struct halCdqPara* cdqPara, uint32_t* queId);
drvError_t __attribute__((weak)) halCdqDestroy(unsigned int devId, unsigned int tsId, unsigned int queId);
drvError_t __attribute__((weak)) halCdqAllocBatch(
    unsigned int devId, unsigned int tsId, unsigned int queId, unsigned int timeout, unsigned int* batchId);
drvError_t __attribute__((weak))
halStreamTaskFill(uint32_t dev_id, uint32_t stream_id, void* stream_mem, void* task_info, uint32_t task_cnt);
drvError_t __attribute__((weak))
halSqSwitchStreamBatch(uint32_t dev_id, struct sq_switch_stream_info* info, uint32_t num);
...
DVresult __attribute__((weak))
drvLoadProgram(DVdevice device_id, void* program, unsigned int offset, size_t byte_count, void** v_ptr);
...
drv_log_report_err_msg_handle_register(struct err_msg_report_handle* handle, size_t input_size);
...
#define RUNTIME_WHEN_NO_VIRTUAL_MODEL_RETURN                 \
    if (sysMode_ != RUN_MACHINE_VIRTUAL || addrMode_ == 0) { \
...
#define DRV_MALLOC_ERROR_PROCESS(drvErrorCode, moduleId, format, ...)                                                \
    do {                                                                                                             \
        if (&halMapErrorCode != nullptr) {                                                                           \
            const int32_t errCodeAfterTrans = halMapErrorCode(drvErrorCode);                                         \
            RT_LOG(                                                                                                  \
                RT_LOG_INFO, "halMapErrorCode ret:%d, drvErrorCode:%d, moduleId: %d", errCodeAfterTrans,             \
