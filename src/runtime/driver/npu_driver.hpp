...
public:
    ...
    rtError_t HostMemAlloc(
        void** const dptr, const uint64_t size, const uint32_t deviceId, const uint16_t moduleId = MODULEID_RUNTIME,
        const uint32_t vaFlag = 0) override;
    rtError_t AllocFastRingBufferAndDispatch(
        void** const dptr, const uint64_t size, const uint32_t deviceId,
    ...
    rtError_t MallocHostSharedMemory(
        rtMallocHostSharedMemoryIn* const in, rtMallocHostSharedMemoryOut* const out, const uint32_t deviceId) override;
    ...
    rtError_t HostGetDevPointer(void* srcPtr, uint32_t deviceId, void** dstPtr, const bool isLogError = true) override;
    ...
    rtError_t MemManagedAdvise(
        const void* const ptr, uint64_t size, uint16_t advise, rtMemManagedLocation location) override;

    rtError_t MemGetInfo(
        const uint32_t deviceId, bool isHugeOnly, size_t* const freeSize, size_t* const totalSize) override;

    rtError_t MemGetInfoByType(const uint32_t deviceId, const rtMemType_t type, rtMemInfo_t* const info) override;

    rtError_t CheckMemType(
        void** addrs, uint32_t size, uint32_t memType, uint32_t* checkResult, uint32_t deviceId) override;

    rtError_t GetMemUsageInfo(
        const uint32_t deviceId, rtMemUsageInfo_t* const memUsageInfo, const size_t inputNum,
        size_t* const outputNum) override;

    rtError_t MemGetInfoEx(
        const uint32_t deviceId, const rtMemInfoType_t memInfoType, size_t* const freeSize,
    ...
    rtError_t MemManagedGetAttr(
        rtMemManagedRangeAttribute attribute, const void* ptr, size_t size, void* data, size_t dataSize) override;

    rtError_t MemManagedGetAttrs(
        rtMemManagedRangeAttribute* attributes, size_t numAttributes, const void* ptr, size_t size, void** data,
        size_t* dataSizes) override;

    rtError_t PtrGetAttributes(const void* const ptr, rtPtrAttributes_t* const attributes) override;

    rtError_t PtrGetRealLocation(
        const void* const ptr, rtMemLocationType& location, rtMemLocationType& realLocation) override;

    rtError_t MemPrefetchToDevice(const void* const devPtr, const uint64_t len, const int32_t deviceId) override;

    // Create IPC Memory
    rtError_t CreateIpcMem(
        const void* const vptr, const uint64_t byteCount, char_t* const name, const uint32_t len) override;
    ...
    rtError_t MemCopySync(
        ...
        bool errShow = true, uint32_t devId = INVALID_COPY_MODULEID) override;

    // Asynchronous memcpy.
    rtError_t MemCopyAsync(
        void* const dst, const uint64_t destMax, const void* const src, const uint64_t size, const rtMemcpyKind_t kind,
        volatile uint64_t& copyFd) override;
    ...
    rtError_t MemConvertAddr(
        const uint64_t src, const uint64_t dst, const uint64_t len, struct DMA_ADDR* const dmaAddress) override;
    ...
    rtError_t StreamIdAlloc(
        int32_t* const id, const uint32_t deviceId, const uint32_t tsId, const uint32_t priority,
        uint32_t streamFlag = 0U) override;

    // Free stream ID.
    rtError_t StreamIdFree(
        const int32_t id, const uint32_t deviceId, const uint32_t tsId, const uint32_t streamFlag = 0) override;

    rtError_t StreamIdReservedFree(const int32_t id, const uint32_t deviceId, const uint32_t tsId) override;

    // Alloc event ID.
    rtError_t EventIdAlloc(
        int32_t* const id, const uint32_t deviceId, const uint32_t tsId, const uint32_t eventFlag = 0,
        const bool createFlag = false) override;

    // Free event ID.
    rtError_t EventIdFree(
        const int32_t id, const uint32_t deviceId, const uint32_t tsId, const uint32_t eventFlag = 0) override;
    ...
    rtError_t DebugSqTaskSend(
        const uint32_t sqId, uint8_t* const sqe, const uint32_t deviceId, const uint32_t tsId) override;

    // Wait report from driver. Blocked if no report.
    rtError_t ReportWait(
        void** const report, int32_t* const cnt, const uint32_t deviceId, const uint32_t tsId,
        const uint32_t cqId) override;

    // Release report to driver.
    rtError_t ReportRelease(
        const uint32_t deviceId, const uint32_t tsId, const uint32_t cqId, const drvSqCqType_t type) override;

    rtError_t NormalSqCqAllocate(
        const uint32_t deviceId, const uint32_t tsId, const uint32_t drvFlag, uint32_t* const sqId,
    ...
    rtError_t NormalSqCqFree(
        const uint32_t deviceId, const uint32_t tsId, const uint32_t drvFlag, const uint32_t sqId,
        const uint32_t cqId) override;

    rtError_t SqSwitchStreamBatch(
        const uint32_t deviceId, struct sq_switch_stream_info* switchInfo, const uint32_t num) override;
    rtError_t LogicCqAllocate(
        const uint32_t devId, const uint32_t tsId, const uint32_t streamId, const bool isNeedFast, uint32_t& cqId,
        bool& isFastCq, const bool isCtrlSq) override;

    rtError_t LogicCqAllocateV2(
        const uint32_t devId, const uint32_t tsId, const uint32_t streamId, uint32_t& cqId,
        const bool isDvppGrp = false, const uint32_t drvFlag = 0U) override; // for stars

    rtError_t LogicCqFree(
        const uint32_t devId, const uint32_t tsId, const uint32_t cqId, const uint32_t drvFlag = 0U) override;
    ...
    rtError_t CreateAsyncDmaWqe(
        uint32_t devId, const AsyncDmaWqeInputInfo& input, AsyncDmaWqeOutputInfo* output, bool isUbMode,
        bool isSqeUpdate) override;
    rtError_t DestroyAsyncDmaWqe(
        uint32_t devId, struct AsyncDmaWqeDestroyInfo* destroyPara, bool isUbMode = true) override;

    rtError_t CreateAsyncDmaWqe2D(
        uint32_t devId, const AsyncDmaWqeInputInfo2D& input, AsyncDmaWqeOutputInfo* output) override;
    rtError_t DestroyAsyncDmaWqe2D(uint32_t devId, struct AsyncDmaWqeDestroyInfo2D* destroyPara) override;

    rtError_t CreateAsyncDmaWqeBatch(
        uint32_t devId, const AsyncDmaWqeInputInfoBatch& input, AsyncDmaWqeOutputInfo* output) override;
    rtError_t DestroyAsyncDmaWqeBatch(uint32_t devId, struct AsyncDmaWqeDestroyInfoBatch* destroyPara) override;

    rtError_t DebugCqReport(
        const uint32_t devId, const uint32_t tsId, const uint32_t cqId, uint8_t* const report,
    ...
    rtError_t DevMemAllocConPhy(
        void** const dptr, const uint64_t size, const rtMemType_t type, const uint32_t deviceId) override;
    ...
    rtError_t DevDvppMemAlloc(
        void** const dptr, const uint64_t size, const uint32_t deviceId, const uint32_t flag,
        const uint16_t moduleId = MODULEID_RUNTIME) override;
    ...
    // get device info
    rtError_t GetDevInfo(
        const uint32_t deviceId, const int32_t moduleType, const int32_t infoType, int64_t* const val) override;
    ...
    // get phy device info
    rtError_t GetPhyDevInfo(
        const uint32_t phyId, const int32_t moduleType, const int32_t infoType, int64_t* const val) override;

    // alloc notify id
    rtError_t NotifyIdAlloc(
        const int32_t deviceId, uint32_t* const id, const uint32_t tsId, const uint32_t notifyFlag = 0,
        const bool isCountNotify = false, const bool isEvent = false) override;
    // free notify id
    rtError_t NotifyIdFree(
        const int32_t deviceId, const uint32_t id, const uint32_t tsId, const uint32_t notifyFlag = 0,
        const bool isCountNotify = false) override;
    // Create IPC Notify
    rtError_t CreateIpcNotify(
    ...
    // Destroy IPC Notify
    rtError_t DestroyIpcNotify(
        const char_t* const name, const int32_t devId, const uint32_t notifyId, const uint32_t tsId) override;

    // Open IPC Notify
    rtError_t OpenIpcNotify(
        const IpcNotifyOpenPara& openPara, uint32_t* const phyId, uint32_t* const notifyId, uint32_t* const tsId,
        uint32_t* const isPod, uint32_t* const adcDieId) override;

    // Close IPC Notify
    rtError_t CloseIpcNotify(
        const char_t* const name, const int32_t devId, const uint32_t notifyId, const uint32_t tsId) override;
    ...
    // get addr notify
    rtError_t NotifyGetAddrOffset(
        const int32_t deviceId, const uint32_t notifyId, uint64_t* const devAddrOffset, const uint32_t tsId) override;
    ...
    rtError_t LoadProgram(
        const int32_t devId, void* const prog, const uint32_t offset, const uint64_t size, void** const vPtr) override;
    ...
    rtError_t SqCqAllocate(
        const uint32_t deviceId, const uint32_t tsId, const uint32_t groupId, uint32_t* const sqId,
        uint32_t* const cqId) override;

    rtError_t DebugSqCqAllocate(const uint32_t deviceId, const uint32_t tsId, uint32_t& sqId, uint32_t& cqId) override;

    rtError_t SqCqFree(const uint32_t sqId, const uint32_t cqId, const uint32_t deviceId, const uint32_t tsId) override;

    rtError_t CtrlSqCqAllocate(
        const uint32_t deviceId, const uint32_t tsId, uint32_t* const sqId, uint32_t* const cqId,
        const uint32_t logicCqId) override;

    rtError_t CtrlSqCqFree(
        const uint32_t deviceId, const uint32_t tsId, const uint32_t sqId, const uint32_t cqId) override;

    rtError_t VirtualCqAllocate(
        const uint32_t devId, const uint32_t tsId, uint32_t& sqId, uint32_t& cqId, uint8_t*& addr,
        bool& shmSqReadonly) override;

    rtError_t VirtualCqFree(
        const uint32_t devId, const uint32_t tsId, const uint32_t sqId, const uint32_t cqId) override;

    rtError_t CqReportIrqWait(
        const uint32_t deviceId, const uint32_t tsId, const uint32_t groupId, const int32_t timeout,
        uint64_t* const getCqidList, const uint32_t cqidListNum) override;

    rtError_t CqReportGet(
        const uint32_t deviceId, const uint32_t tsId, const uint32_t cqId, rtHostFuncCqReport_t** const report,
        uint32_t* const cnt) override;
    rtError_t CqReportRelease(
        rtHostFuncCqReport_t* const report, const uint32_t deviceId, const uint32_t cqId, const uint32_t tsId,
        const bool noLog = false) override;

    rtError_t SqCommandOccupy(
        const uint32_t deviceId, const uint32_t tsId, const uint32_t sqId, rtHostFuncCommand_t** const command,
        const uint32_t cnt) override;

    rtError_t SqCommandSend(
        const uint32_t deviceId, const uint32_t tsId, const uint32_t sqId, rtHostFuncCommand_t* const command,
        const uint32_t cnt) override;
    ...
    rtError_t SetSqHead(
        const uint32_t deviceId, const uint32_t tsId, const uint32_t sqId, const uint32_t head) override;

    rtError_t SetSqTail(
        const uint32_t deviceId, const uint32_t tsId, const uint32_t sqId, const uint32_t tail) override;

    rtError_t CleanSq(
        const uint32_t deviceId, const uint32_t tsId, const uint32_t sqId, const uint32_t streamFlag) override;

    rtError_t TaskKill(
        const uint32_t deviceId, const uint32_t tsId, const uint32_t sqId, const uint32_t operationType) override;

    rtError_t QueryOpExecTimeoutInterval(
        const uint32_t deviceId, const uint32_t tsId, uint64_t& timeoutInterval) override;

    rtError_t TaskAbortByType(
        const uint32_t deviceId, const uint32_t tsId, const uint32_t opType, const uint32_t targetId,
        uint32_t& result) override;

    rtError_t QueryAbortStatusByType(
        const uint32_t deviceId, const uint32_t tsId, const uint32_t queryType, const uint32_t targetId,
        uint32_t& status) override;

    rtError_t RecoverAbortByType(
        const uint32_t deviceId, const uint32_t tsId, const uint32_t opType, const uint32_t targetId,
        uint32_t& result) override;

    rtError_t QueryRecoverStatusByType(
        const uint32_t deviceId, const uint32_t tsId, const uint32_t queryType, const uint32_t targetId,
        uint32_t& status) override;

    rtError_t QuerySq(
        const uint32_t deviceId, const uint32_t tsId, const uint32_t sqId, const uint32_t queryType,
        uint32_t& status) override;

    rtError_t GetSqHead(
        const uint32_t deviceId, const uint32_t tsId, const uint32_t sqId, uint16_t& head,
        bool needLog = true) override;

    rtError_t GetCtrlSqHead(const uint32_t deviceId, const uint32_t tsId, const uint32_t sqId, uint16_t& head) override;
    ...
    rtError_t StreamUnBindLogicCq(
        const uint32_t deviceId, const uint32_t tsId, const uint32_t streamId, const uint32_t logicCqId,
        const uint32_t drvFlag = 0U) override;

    rtError_t GetChipCapability(const uint32_t deviceId, struct halCapabilityInfo* const info) override;

    rtError_t GetCapabilityGroupInfo(
        const int32_t deviceId, const int32_t ownerId, const int32_t groupId,
        struct capability_group_info* const groupInfo, const int32_t groupCount) override;
    ...
    rtError_t GetPairDevicesInfo(
        ...
        const bool deviceFlag = false) override;
    ...
    rtError_t GetSqRegVirtualAddrBySqid(
        const int32_t deviceId, const uint32_t tsId, const uint32_t sqId, uint64_t* const addr,
        uint32_t* const len) override;
    ...
    static rtError_t MemQueueQuery(
    ...
    static rtError_t MemQueueGrant(
    ...
    static rtError_t EschedSubmitEventSync(
    ...
    static rtError_t MbufChainGetMbuf(
        const rtMbufPtr_t memBufChainHead, const uint32_t index, rtMbufPtr_t* const memBuf);
    static rtError_t MemGrpCreate(const char_t* const name, const rtMemGrpConfig_t* const cfg);
    static rtError_t MemGrpCacheAlloc(
        const char_t* const name, const int32_t devId, const rtMemGrpCacheAllocPara* const para);
    static rtError_t BuffGetInfo(
    ...
    static rtError_t QueueSubscribe(
    ...
    static rtError_t EschedWaitEvent(
    ...
    static rtError_t StreamEnableStmSyncEsched(
        const uint32_t deviceId, const uint32_t tsId, const uint32_t streamId, const uint32_t grpId,
        const uint32_t eventId);
    static rtError_t EschedCreateGrpEx(const uint32_t devId, const uint32_t maxThreadNum, uint32_t* const grpId);
    static drvError_t DrvEschedManage(
    ...
    static rtError_t HdcSessionConnect(
    ...
    static rtError_t EschedQueryInfo(
    ...
    static rtError_t MemMapNoAccess(void* virPtr, size_t size, size_t offset, rtDrvMemHandle handle, uint64_t flags);
    ...
    static rtError_t GetServerIdAndshareableHandle(
    ...
    static rtError_t ExportToShareableHandle(
    ...
    static rtError_t GetAllocationGranularity(
    ...
    static rtError_t QueryProcessHostPid(
    ...
    static rtError_t GetFaultEvent(
        const int32_t deviceId, const rtDmsEventFilter* const filter, rtDmsFaultEvent* dmsEvent, uint32_t len,
        uint32_t* eventCount);
    static rtError_t GetAllFaultEvent(
        const uint32_t deviceId, rtDmsFaultEvent* const dmsEvent, uint32_t* eventCount, bool needLog = true);
    ...
    static rtError_t GetDevResAddress(
    ...
    static rtError_t GetDeviceInfoByBuff(
        ...
        int32_t* const size);
    static rtError_t SetDeviceInfoByBuff(
    ...
    drvError_t MemCopySyncAdapter(
        void* const dst, const uint64_t destMax, const void* const src, const uint64_t size, const rtMemcpyKind_t kind,
        const uint32_t devId) override;
    ...
    rtError_t ResetSqCq(
        const uint32_t deviceId, const uint32_t tsId, const uint32_t sqId, const uint32_t streamFlag) override;
    rtError_t ResetLogicCq(
        const uint32_t deviceId, const uint32_t tsId, const uint32_t logicCqId, const uint32_t streamFlag) override;
    ...
    static rtError_t ClearPageFaultInfo(const uint32_t deviceId, const bool isLogError = true);
    ...
    rtError_t StreamMemPoolTrim(
        const uint32_t deviceId, const uint64_t poolId, uint64_t* size, uint64_t poolUsedSize,
        uint64_t poolFreeSize) override;
    rtError_t StreamMemPoolSetAttr(
        const uint32_t deviceId, const uint64_t poolId, const rtMemPoolAttr attr, void* value) override;
    rtError_t StreamMemPoolGetAttr(
        const uint32_t deviceId, const uint64_t poolId, const rtMemPoolAttr attr, void* value) override;
    ...
    rtError_t GetChipIdDieId(
        ...
        int64_t& dieId) override;
    rtError_t GetTopologyType(
        const uint32_t devId, const uint32_t remoteDevId, const uint32_t remotePhyId, int64_t* const val) override;

    // jetty
    rtError_t AsyncDmaJettyCreate(
        const uint32_t devId, const uint32_t piType, const uint32_t depth, const uint32_t dir,
        uint64_t* const handle) override;
    rtError_t AsyncDmaJettyDestroy(const uint32_t devId, const uint64_t handle) override;
    rtError_t AsyncDmaJettyQuery(
        const uint32_t devId, const uint64_t handle, uint32_t& dieId, uint32_t& functionId, uint32_t& jettyId) override;
    rtError_t AsyncDmaWqeConvert(
        const uint32_t devId, AsyncWqeInputPara* inParam, AsyncWqeOutputPara* outParam) override;
    ...
    rtError_t GetSwapBufferInfo(
        const uint32_t deviceId, const uint32_t tsId, uint64_t* const swapBufferBaseAddr) override;

private:
    rtError_t ManagedMemAllocInner(
        void** const dptr, const uint64_t size, const ManagedMemFlag flag, const uint32_t deviceId,
    ...
    rtError_t CreateIpcNotifyWithFlag(
        char_t* const name, const uint32_t len, const int32_t devId, uint32_t* const notifyId, const uint32_t tsId,
        const uint32_t notifyFlag) const;

    rtError_t GetSqRegVirtualAddrBySqidForDavid(
