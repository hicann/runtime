...
public:
    ...
    virtual rtError_t RecycleSeparatedStmByFinishedId(
        Stream* const stm, const uint16_t endTaskId, bool isCqeProcess = false);
    virtual rtError_t SyncTask(
        Stream* const stm, const uint32_t taskId, const bool isStreamSync, int32_t timeout = -1, bool isForce = false);
    ...
    // stars feature
    virtual rtError_t DvppWaitGroup(
        DvppGrp* grp, rtDvppGrpCallback callBackFunc, int32_t timeout = RT_REPORT_TIMEOUT_TIME)
    {
        (void)grp;
        (void)callBackFunc;
        (void)timeout;
    ...
    uint32_t GetPendingNum() const { return pendingNum_.Value(); }

    void AddPendingNum() { pendingNum_.Add(1U); }
    ...
    virtual uint32_t GetShareSqId() const { return MAX_UINT32_NUM; }
...
protected:
    ...
    void ReportTimeoutProc(
        const rtError_t error, int32_t& timeoutCnt, const uint32_t streamId = MAX_UINT16_NUM,
    ...
    uint32_t GetRecycleDavinciTaskNum(
        const uint16_t lastRecycleEndTaskId, const uint16_t recycleEndTaskId,
        const uint16_t recyclePublicTaskCount) const;
    void TrigerAsyncRecycle(
    ...
    void SetWaitTimeout(const int32_t timeout) { waitTimeout_ = timeout; }
    int32_t GetWaitTimeout(void) const { return waitTimeout_; }

    bool TaskIdIsGEQ(const uint32_t lsh, const uint32_t rsh) const { return TASK_ID_GEQ(lsh, rsh); }
    bool TaskIdIsLEQ(const uint32_t lsh, const uint32_t rsh) const { return TASK_ID_LEQ(lsh, rsh); }
    bool TaskIdIsGT(const uint32_t lsh, const uint32_t rsh) const { return TASK_ID_GT(lsh, rsh); }
    bool TaskIdIsLT(const uint32_t lsh, const uint32_t rsh) const { return TASK_ID_LT(lsh, rsh); }
