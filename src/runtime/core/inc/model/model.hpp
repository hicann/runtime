...
struct MdlDestroyCallbackInfo {
    ...
    {
        return RtPtrToValue(callback) < RtPtrToValue(other.callback);
    }
};
...
public:
    ...
    enum ModelExecuteApiType : uint8_t { RT_MODEL_SYNC, RT_MODEL_ASYNC, RT_MODEL_RESERVED };
    ...
    uint32_t Id_() const { return static_cast<uint32_t>(id_); }

    std::string GetName() const { return name_; }

    uint32_t LastStreamId_() const { return lastStreamId_; }

    uint32_t LastTaskId_() const { return kernelTaskId_; }
    ...
    void PushbackDmaAddr(struct DMA_ADDR& dmaAddr) { dmaAddrRecord_.push_back(dmaAddr); }
    ...
    void ModelRemoveStream(Stream* const stream) { streams_.remove(stream); }

    void ModelPushFrontStream(Stream* const stream) { streams_.push_front(stream); }
    ...
    void SetModelExecutorType(uint32_t executorFlag) { executorFlag_ = executorFlag; }

    void SetModelName(std::string& name) { name_ = name; }

    uint32_t GetModelExecutorType() const { return (uint32_t)executorFlag_; }

    void* GetAicpuModelInfo() const { return aicpuModelInfo_; }

    uint32_t GetFirstTaskId() const { return firstTaskId_; }
    ...
    const void* GetDevModelID() const { return modeID_; }
    ...
    void PushbackArgActiveStream(void* argHandle) { argActiveStreamRecord_.push_back(argHandle); }
    void IncEndGraphNum(void) { endGraphNum_++; }
    uint32_t EndGraphNum_(void) const { return endGraphNum_; }
    void IncModelExitNum(void) { modelExitNum_++; }
    uint32_t ModelExitNum_(void) const { return modelExitNum_; }

    void SetEndGraphNotify(Notify* notify) { endGraphNotify_ = notify; }
    Notify* GetEndGraphNotify() { return endGraphNotify_; }
    Stream* GetExeStream() const { return exeStream_; }
    void SetExeStream(Stream* exeStream) { exeStream_ = exeStream; }
    int16_t GetSchGroupId() const { return schGroupId_; }
    void SetSchGroupId(int16_t schGrpId) { schGroupId_ = schGrpId; }
    ...
    uint64_t GetLabelCountdevPtr() const { return RtPtrToValue(labelCountPtr_); }
    void LabelCountInc() { labelCount_++; }
    ...
    const std::list<Stream*> GetHeadStreamList_() const { return headStreams_; }

    std::mutex& GetFirstExecuteMutex() { return firstExecuteMutex_; }

    bool GetFirstExecute() const { return firstExecuteFlag_.load(); }

    void SetFirstExecute(bool firstExecuteFlag) { firstExecuteFlag_.store(firstExecuteFlag); }

    bool GetNeedUpdateUBPi() const { return needUpdateUbPi_.load(); }

    void SetNeedUpdateUBPi(bool needUpdateUbPi) { needUpdateUbPi_.store(needUpdateUbPi); }

    void* GetFuncCallHostMem() const { return funcCallHostMem_; }

    void SetFuncCallHostMem(void* mem) { funcCallHostMem_ = mem; }

    uint64_t GetFunCallMemSize() const { return funCallMemSize_; }

    void SetFunCallMemSize(uint64_t funCallMemSize) { funCallMemSize_ = funCallMemSize; }

    void SetFuncCallSvmMem(uint64_t funcCallSvmMem) { funcCallSvmMem_ = funcCallSvmMem; }

    uint64_t GetFuncCallSvmMem() const { return funcCallSvmMem_; }

    void SetBaseFuncCallSvmMem(void* baseFuncCallSvmMem) { baseFuncCallSvmMem_ = baseFuncCallSvmMem; }

    void* GetBaseFuncCallSvmMem() const { return baseFuncCallSvmMem_; }

    void SetFuncCallDfxBaseSvmMem(void* funcCallDfxBaseSvmMem) { funcCallDfxBaseSvmMem_ = funcCallDfxBaseSvmMem; }

    void* GetFuncCallDfxBaseSvmMem() const { return funcCallDfxBaseSvmMem_; }

    void SetDfxPtr(void* dfxPtr) { dfxPtr_ = dfxPtr; }
    ...
    void SetIsSendSqe(bool isSendSqe) { isSqeSendFinish_ = isSendSqe; }
