...
enum { TASK_ERROR_SUBMIT_FAIL = 256 };
...
typedef enum tagTsTaskType {
    ...
    TS_TASK_TYPE_MEMSET = 32,                       /* memset task */
...
enum class ProfTaskType : uint16_t {
    ...
    PROF_TASK_TYPE_NOTIFY_RESET,
    PROF_TASK_TYPE_COUNT_NOTIFY_RECORD,
    PROF_TASK_TYPE_COUNT_NOTIFY_WAIT,
};
...
typedef enum tagMtType { MT_STREAM_DESTROY = 0, MT_EVENT_DESTROY, MT_STREAM_RECYCLE_TASK } MtType;

typedef enum tagFusionFlag { FUSION_START = 0, FUSION_END = 1 } FusionFlag;
/**
 * @ingroup
 ...
 */
enum rtMemcpyAddrConvertType { RT_NOT_NEED_CONVERT, RT_DMA_ADDR_CONVERT, RT_NO_DMA_ADDR_CONVERT };
