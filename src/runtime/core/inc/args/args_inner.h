...
constexpr uint32_t SIMT_SYNC_COUNTER_SIZE = static_cast<uint32_t>(sizeof(uint64_t));
constexpr uint32_t SIMT_DIM_PARAM_SIZE = SIMT_IMPLICIT_PARAM_COUNT * static_cast<uint32_t>(sizeof(uint32_t));
