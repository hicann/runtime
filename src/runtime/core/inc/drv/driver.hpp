...
public:
    ...
    virtual rtError_t ParseSDID(
        const uint32_t sdid, uint32_t* srvId, uint32_t* chipId, uint32_t* dieId, uint32_t* pyhId) = 0;

    virtual rtError_t GetDeviceIDs(uint32_t* const deviceIds, const uint32_t len) = 0;

    // Alloc page-locked host memory.
    virtual rtError_t HostMemAlloc(
        void** const dptr, const uint64_t size, const uint32_t deviceId, const uint16_t moduleId = MODULEID_RUNTIME,
        const uint32_t vaFlag = 0) = 0;
    virtual rtError_t AllocFastRingBufferAndDispatch(
    ...
    virtual rtError_t MallocHostSharedMemory(
        rtMallocHostSharedMemoryIn* const in, rtMallocHostSharedMemoryOut* const out, const uint32_t deviceId) = 0;
    ...
    virtual rtError_t HostGetDevPointer(
        void* srcPtr, uint32_t deviceId, void** dstPtr, const bool isLogError = true) = 0;
    ...
    virtual rtError_t MemGetInfo(
        const uint32_t deviceId, bool isHugeOnly, size_t* const freeSize, size_t* const totalSize) = 0;
    ...
    virtual rtError_t CheckMemType(
        void** addrs, uint32_t size, uint32_t memType, uint32_t* checkResult, uint32_t deviceId) = 0;
    ...
    virtual rtError_t PtrGetRealLocation(
        const void* const ptr, rtMemLocationType& location, rtMemLocationType& realLocation) = 0;

    virtual rtError_t MemPrefetchToDevice(const void* const devPtr, const uint64_t len, const int32_t deviceId) = 0;

    // Create IPC Memory
    virtual rtError_t CreateIpcMem(
        const void* const vptr, const uint64_t byteCount, char_t* const name, const uint32_t len) = 0;
    ...
    virtual rtError_t MemCopySync(
        void* dst, uint64_t destMax, const void* src, uint64_t size, rtMemcpyKind_t kind, bool errShow = true,
        uint32_t devId = INVALID_COPY_MODULEID) = 0;
    // Asynchronous memcpy.
    virtual rtError_t MemCopyAsync(
    ...
    virtual rtError_t MemConvertAddr(
        const uint64_t src, const uint64_t dst, const uint64_t len, struct DMA_ADDR* const dmaAddress) = 0;
    ...
    virtual rtError_t StreamIdAlloc(
        ...
        uint32_t streamFlag = 0U) = 0;

    // Free stream ID.
    virtual rtError_t StreamIdFree(
        const int32_t id, const uint32_t deviceId, const uint32_t tsId, const uint32_t streamFlag = 0) = 0;

    virtual rtError_t StreamIdReservedFree(const int32_t id, const uint32_t deviceId, const uint32_t tsId) = 0;

    // Alloc event ID.
    virtual rtError_t EventIdAlloc(
        int32_t* const id, const uint32_t deviceId, const uint32_t tsId, const uint32_t eventFlag = 0,
        const bool createFlag = false) = 0;

    // Free event ID.
    virtual rtError_t EventIdFree(
        const int32_t id, const uint32_t deviceId, const uint32_t tsId, const uint32_t eventFlag = 0) = 0;
    ...
    virtual rtError_t CommandOccupy(
    ...
    virtual rtError_t ReportWait(
        void** const report, int32_t* const cnt, const uint32_t deviceId, const uint32_t tsId, const uint32_t cqId) = 0;

    // Release report to driver.
    virtual rtError_t ReportRelease(
        const uint32_t deviceId, const uint32_t tsId, const uint32_t cqId, const drvSqCqType_t type) = 0;

    virtual rtError_t NormalSqCqAllocate(
    ...
    virtual rtError_t NormalSqCqFree(
        const uint32_t deviceId, const uint32_t tsId, const uint32_t drvFlag, const uint32_t sqId,
        const uint32_t cqId) = 0;

    virtual rtError_t SqSwitchStreamBatch(
        const uint32_t deviceId, struct sq_switch_stream_info* switchInfo, const uint32_t num) = 0;
    virtual rtError_t LogicCqAllocate(
        const uint32_t devId, const uint32_t tsId, const uint32_t streamId, const bool isNeedFast, uint32_t& cqId,
        bool& isFastCq, const bool isCtrlSq) = 0;

    virtual rtError_t LogicCqAllocateV2(
        const uint32_t devId, const uint32_t tsId, const uint32_t streamId, uint32_t& cqId,
        const bool isDvppGrp = false, const uint32_t drvFlag = 0U) = 0; // for stars

    virtual rtError_t LogicCqFree(
        const uint32_t devId, const uint32_t tsId, const uint32_t cqId, const uint32_t drvFlag = 0U) = 0;
    ...
    virtual rtError_t LogicCqReportV2(
        const LogicCqWaitInfo& waitInfo, uint8_t* report, uint32_t reportCnt, uint32_t& realCnt) = 0;
    virtual rtError_t CreateAsyncDmaWqe(
        uint32_t devId, const AsyncDmaWqeInputInfo& input, AsyncDmaWqeOutputInfo* output, bool isUbMode,
        bool isSqeUpdate) = 0;
    virtual rtError_t DestroyAsyncDmaWqe(
        uint32_t devId, struct AsyncDmaWqeDestroyInfo* destroyPara, bool isUbMode = true) = 0;

    virtual rtError_t CreateAsyncDmaWqe2D(
        uint32_t devId, const AsyncDmaWqeInputInfo2D& input, AsyncDmaWqeOutputInfo* output) = 0;
    virtual rtError_t DestroyAsyncDmaWqe2D(uint32_t devId, struct AsyncDmaWqeDestroyInfo2D* destroyPara) = 0;

    virtual rtError_t CreateAsyncDmaWqeBatch(
        uint32_t devId, const AsyncDmaWqeInputInfoBatch& input, AsyncDmaWqeOutputInfo* output) = 0;
    ...
    virtual rtError_t DebugCqReport(
        const uint32_t devId, const uint32_t tsId, const uint32_t cqId, uint8_t* const report, uint32_t& realCnt) = 0;
    ...
    virtual rtError_t DevMemAllocConPhy(
        void** const dptr, const uint64_t size, const rtMemType_t type, const uint32_t deviceId) = 0;

    // Free contiguous memory
    virtual rtError_t DevMemConPhyFree(void* const dptr, const uint32_t deviceId) = 0;

    // Alloc device global memory for dvpp.
    virtual rtError_t DevDvppMemAlloc(
    ...
    // Destroy IPC Notify
    virtual rtError_t DestroyIpcNotify(
        const char_t* const name, const int32_t devId, const uint32_t notifyId, const uint32_t tsId) = 0;

    // Open IPC Notify
    virtual rtError_t OpenIpcNotify(
        const IpcNotifyOpenPara& openPara, uint32_t* const phyId, uint32_t* const notifyId, uint32_t* const tsId,
        uint32_t* const isPod, uint32_t* const adcDieId) = 0;

    // Close IPC Notify
    virtual rtError_t CloseIpcNotify(
        const char_t* const name, const int32_t devId, const uint32_t notifyId, const uint32_t tsId) = 0;

    virtual rtError_t NotifyGetAddrOffset(
        const int32_t deviceId, const uint32_t notifyId, uint64_t* const devAddrOffset, const uint32_t tsId) = 0;
    ...
    virtual rtError_t LoadProgram(
        const int32_t devId, void* const prog, const uint32_t offset, const uint64_t size, void** const vPtr) = 0;

    virtual rtError_t DeviceGetBareTgid(uint32_t* const pid) const = 0;

    virtual rtError_t SqCqAllocate(
        const uint32_t deviceId, const uint32_t tsId, const uint32_t groupId, uint32_t* const sqId,
        uint32_t* const cqId) = 0;

    virtual rtError_t DebugSqCqAllocate(
        const uint32_t deviceId, const uint32_t tsId, uint32_t& sqId, uint32_t& cqId) = 0;

    virtual rtError_t SqCqFree(
        const uint32_t sqId, const uint32_t cqId, const uint32_t deviceId, const uint32_t tsId) = 0;

    virtual rtError_t CtrlSqCqAllocate(
        const uint32_t deviceId, const uint32_t tsId, uint32_t* const sqId, uint32_t* const cqId,
        const uint32_t logicCqId) = 0;

    virtual rtError_t CtrlSqCqFree(
        const uint32_t deviceId, const uint32_t tsId, const uint32_t sqId, const uint32_t cqId) = 0;

    virtual rtError_t VirtualCqAllocate(
        const uint32_t devId, const uint32_t tsId, uint32_t& sqId, uint32_t& cqId, uint8_t*& addr,
        bool& shmSqReadonly) = 0;

    virtual rtError_t VirtualCqFree(
        const uint32_t devId, const uint32_t tsId, const uint32_t sqId, const uint32_t cqId) = 0;

    virtual rtError_t CqReportIrqWait(
        const uint32_t deviceId, const uint32_t tsId, const uint32_t groupId, const int32_t timeout,
        uint64_t* const getCqidList, const uint32_t cqidListNum) = 0;

    virtual rtError_t CqReportGet(
        const uint32_t deviceId, const uint32_t tsId, const uint32_t cqId, rtHostFuncCqReport_t** const report,
        uint32_t* const cnt) = 0;

    virtual rtError_t CqReportRelease(
        rtHostFuncCqReport_t* const report, const uint32_t deviceId, const uint32_t cqId, const uint32_t tsId,
        const bool noLog = false) = 0;

    virtual rtError_t SqCommandOccupy(
        const uint32_t deviceId, const uint32_t tsId, const uint32_t sqId, rtHostFuncCommand_t** const command,
        const uint32_t cnt) = 0;

    virtual rtError_t SqCommandSend(
        const uint32_t deviceId, const uint32_t tsId, const uint32_t sqId, rtHostFuncCommand_t* const command,
        const uint32_t cnt) = 0;
    ...
    virtual rtError_t SetSqHead(
        const uint32_t deviceId, const uint32_t tsId, const uint32_t sqId, const uint32_t head) = 0;

    virtual rtError_t SetSqTail(
        const uint32_t deviceId, const uint32_t tsId, const uint32_t sqId, const uint32_t tail) = 0;

    virtual rtError_t GetSqHead(
        const uint32_t deviceId, const uint32_t tsId, const uint32_t sqId, uint16_t& head, bool needLog = true) = 0;

    virtual rtError_t CleanSq(
        const uint32_t deviceId, const uint32_t tsId, const uint32_t sqId, const uint32_t streamFlag) = 0;

    virtual rtError_t TaskKill(
        const uint32_t deviceId, const uint32_t tsId, const uint32_t sqId, const uint32_t operationType) = 0;

    virtual rtError_t QueryOpExecTimeoutInterval(
        const uint32_t deviceId, const uint32_t tsId, uint64_t& timeoutInterval) = 0;

    virtual rtError_t TaskAbortByType(
        const uint32_t deviceId, const uint32_t tsId, const uint32_t opType, const uint32_t targetId,
        uint32_t& result) = 0;

    virtual rtError_t QueryAbortStatusByType(
        const uint32_t deviceId, const uint32_t tsId, const uint32_t queryType, const uint32_t targetId,
        uint32_t& status) = 0;

    virtual rtError_t RecoverAbortByType(
        const uint32_t deviceId, const uint32_t tsId, const uint32_t opType, const uint32_t targetId,
        uint32_t& result) = 0;

    virtual rtError_t QueryRecoverStatusByType(
        const uint32_t deviceId, const uint32_t tsId, const uint32_t queryType, const uint32_t targetId,
        uint32_t& status) = 0;

    virtual rtError_t QuerySq(
        const uint32_t deviceId, const uint32_t tsId, const uint32_t sqId, const uint32_t queryType,
        uint32_t& status) = 0;

    virtual rtError_t GetCtrlSqHead(
        const uint32_t deviceId, const uint32_t tsId, const uint32_t sqId, uint16_t& head) = 0;

    virtual rtError_t GetSqTail(const uint32_t deviceId, const uint32_t tsId, const uint32_t sqId, uint16_t& tail) = 0;

    virtual rtError_t GetSqAddrInfo(
        const uint32_t deviceId, const uint32_t tsId, const uint32_t sqId, uint64_t& sqAddr) = 0;
    virtual rtError_t GetSqEnable(const uint32_t deviceId, const uint32_t tsId, const uint32_t sqId, bool& enable) = 0;

    virtual rtError_t StreamBindLogicCq(
        const uint32_t deviceId, const uint32_t tsId, const uint32_t streamId, uint32_t logicCqId,
        const uint32_t drvFlag = 0U) = 0;

    virtual rtError_t StreamUnBindLogicCq(
        const uint32_t deviceId, const uint32_t tsId, const uint32_t streamId, uint32_t logicCqId,
        const uint32_t drvFlag = 0U) = 0;

    virtual rtError_t GetChipCapability(const uint32_t deviceId, struct halCapabilityInfo* const info) = 0;

    virtual rtError_t GetCapabilityGroupInfo(
        const int32_t deviceId, const int32_t ownerId, const int32_t groupId,
        struct capability_group_info* const groupInfo, const int32_t groupCount) = 0;
    ...
    virtual drvError_t MemCopySyncAdapter(
        void* const dst, const uint64_t destMax, const void* const src, const uint64_t size, const rtMemcpyKind_t kind,
    ...
    virtual rtError_t ResetSqCq(
        const uint32_t deviceId, const uint32_t tsId, const uint32_t sqId, const uint32_t streamFlag) = 0;
    virtual rtError_t ResetLogicCq(
        const uint32_t deviceId, const uint32_t tsId, const uint32_t logicCqId, const uint32_t streamFlag) = 0;
    ...
    virtual rtError_t StreamTaskFill(
        uint32_t devId, uint32_t streamId, void* streamMem, void* taskInfo, uint32_t taskCnt) = 0;
    // dqs
    ...
    virtual rtError_t GetTsegInfoByVa(
        uint32_t devid, uint64_t va, uint64_t size, uint32_t flag, struct halTsegInfo* tsegInfo) = 0;
    virtual rtError_t PutTsegInfo(uint32_t devid, struct halTsegInfo* tsegInfo) = 0;
    virtual rtError_t GetChipIdDieId(
        const uint32_t devId, const uint32_t remoteDevId, const uint32_t remotePhyId, int64_t& chipId,
        int64_t& dieId) = 0;
    virtual rtError_t GetTopologyType(
        const uint32_t devId, const uint32_t remoteDevId, const uint32_t remotePhyId, int64_t* const val) = 0;
    // soma
    virtual rtError_t StreamMemPoolCreate(
        const uint32_t deviceId, const uint64_t poolId, const uint64_t size, bool isGraphPool, uint64_t& outVa) = 0;
    virtual rtError_t StreamMemPoolDestroy(const uint32_t deviceId, const uint64_t poolId) = 0;
    virtual rtError_t StreamMemPoolTrim(
        const uint32_t deviceId, const uint64_t poolId, uint64_t* size, uint64_t poolUsedSize,
        uint64_t poolFreeSize) = 0;
    virtual rtError_t AsyncDmaJettyCreate(
        const uint32_t devId, const uint32_t piType, const uint32_t depth, const uint32_t dir,
        uint64_t* const handle) = 0;
    virtual rtError_t AsyncDmaJettyDestroy(const uint32_t devId, const uint64_t handle) = 0;
    virtual rtError_t AsyncDmaJettyQuery(
        const uint32_t devId, const uint64_t handle, uint32_t& dieId, uint32_t& functionId, uint32_t& jettyId) = 0;
    virtual rtError_t AsyncDmaWqeConvert(
        const uint32_t devId, AsyncWqeInputPara* inParam, AsyncWqeOutputPara* outParam) = 0;
    virtual rtError_t AsyncDmaWqeFill(const uint32_t devId, AsyncWqeFillInfo* fillInfo) = 0;
    virtual rtError_t StreamMemPoolSetAttr(
    ...
    virtual rtError_t GetSwapBufferInfo(
        const uint32_t deviceId, const uint32_t tsId, uint64_t* const swapBufferBaseAddr) = 0;
