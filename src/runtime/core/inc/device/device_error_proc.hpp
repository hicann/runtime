...
rtError_t GetDeviceFaultEvents(
    const uint32_t deviceId, rtDmsFaultEvent* const faultEventInfo, uint32_t& eventCount, bool needLog = true);
...
bool HasBlacklistEventOnDevice(
    const uint32_t deviceId, const std::map<uint32_t, std::string>& eventIdBlkList = g_mulBitEccEventId);
...
public:
    ...
    using StarsErrorInfoProc = rtError_t (*)(
    ...
    using ExtProcessorFn = std::function<void(
        const DevRingBufferCtlInfo* ctlInfo, size_t elementSize, uint32_t& head, uint32_t tail, uint32_t errorType,
        const StarsDeviceErrorInfoRingBuffer* rbErrorInfo, StarsDeviceErrorInfo& mergedOut)>;
    ...
    rtError_t ProcessReportRingBuffer(
    ...
    void SetRealFaultTaskPtr(TaskInfo* taskPtr) { realFaultTaskPtr_ = taskPtr; }

    TaskInfo* GetRealFaultTaskPtr() const { return realFaultTaskPtr_; }
    ...
    uint32_t GetSnapshotLen() const { return snapshotLen_; }
    ...
    static rtError_t ProcessStarsCoreErrorInfo(
        const StarsDeviceErrorInfo* const info, const uint64_t errorNumber, const Device* const dev,
        const DeviceErrorProc* const insPtr);
    static rtError_t ProcessStarsWaitTimeoutErrorInfo(
        const StarsDeviceErrorInfo* const info, const uint64_t errorNumber, const Device* const dev,
        const DeviceErrorProc* const insPtr);
    // process sdma error information and record info to host log
    static rtError_t ProcessStarsSdmaErrorInfo(
        const StarsDeviceErrorInfo* const info, const uint64_t errorNumber, const Device* const dev,
        const DeviceErrorProc* const insPtr);
    static rtError_t ProcessStarsDvppErrorInfo(
        const StarsDeviceErrorInfo* const info, const uint64_t errorNumber, const Device* const dev,
        const DeviceErrorProc* const insPtr);
    static rtError_t ProcessStarsDsaErrorInfo(
        const StarsDeviceErrorInfo* const info, const uint64_t errorNumber, const Device* const dev,
        const DeviceErrorProc* const insPtr);
    static rtError_t ProcessStarsSqeErrorInfo(
        const StarsDeviceErrorInfo* const info, const uint64_t errorNumber, const Device* const dev,
        const DeviceErrorProc* const insPtr);
    static rtError_t ProcessStarsHcclFftsPlusTimeoutErrorInfo(
        const StarsDeviceErrorInfo* const info, const uint64_t errorNumber, const Device* const dev,
        const DeviceErrorProc* const insPtr);
    static rtError_t ProcessStarsCoreTimeoutDfxInfo(
        const StarsDeviceErrorInfo* const info, const uint64_t errorNumber, const Device* const dev,
        const DeviceErrorProc* const insPtr);
...
private:
    ...
    // process sdma error information and record info to host log
    static rtError_t ProcessSdmaErrorInfo(
    ...
    static rtError_t ProcessAicpuErrorInfo(
        const DeviceErrorInfo* const info, const uint64_t errorNumber, const Device* const dev);

    // process one element in ringbuffer.
    rtError_t ProcessOneElementInRingBuffer(
        const DevRingBufferCtlInfo* const ctlInfo, uint32_t head, const uint32_t tail) const;
    rtError_t ProcessStarsOneElementInRingBuffer(
    ...
    static void ProcessStarsCoreErrorOneMapInfo(
...
private:
    using ErrorInfoProc =
        rtError_t (*)(const DeviceErrorInfo* const info, const uint64_t errorNumber, const Device* const dev);
    void ProcessStarsRingBufferErrorInfo(
