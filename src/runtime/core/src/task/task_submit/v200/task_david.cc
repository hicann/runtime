...
namespace cce {
namespace runtime {
constexpr uint16_t TASK_QUERY_INTERVAL_NUM = 256U; // task recycle interval.
...
{
    RT_LOG(
        RT_LOG_DEBUG, "Enter ExpandHostSqeBufferLocked, master_stream_id=%d, cur_stream_id=%d",
        stm->GetExposedStreamId(), stm->Id_());
    AutoSplitSqContext* ctx = stm->GetAutoSplitCtx();
    if (ctx == nullptr) {
        RT_LOG_INNER_MSG(
            RT_LOG_ERROR,
            "ExpandHostSqeBufferLocked failed because ctx cannot be a NULL pointer, master_stream_id=%d, "
            "cur_stream_id=%d.",
            stm->GetExposedStreamId(), stm->Id_());
    ...
    COND_RETURN_AND_MSG_OUTER(
        newBuffer == nullptr, RT_ERROR_MEMORY_ALLOCATION, ErrorCode::EE1013, sizeof(uint8_t) * newSize, "new");

    errno_t ret = memset_s(newBuffer, newSize, 0U, newSize);
    COND_PROC_RETURN_ERROR_MSG_INNER(
        ret != EOK, RT_ERROR_MEMORY_ALLOCATION, delete[] newBuffer,
    ...
    RT_LOG(
        RT_LOG_INFO, "Expanded host SQ buffer, master_stream_id=%d, cur_stream_id=%d, newSize=%u",
...
static rtError_t TryCreateAutoSplitSlaveStream(Stream* const masterStm, uint32_t sqeNum, Stream*& curStream)
{
    RT_LOG(
        RT_LOG_DEBUG, "Enter TryCreateAutoSplitSlaveStream, master_stream_id=%d, cur_stream_id=%d, sqeNum=%u",
        masterStm->GetExposedStreamId(), curStream->Id_(), sqeNum);
    AutoSplitSqContext* splitCtx = curStream->GetAutoSplitCtx();
    COND_RETURN_ERROR_MSG_INNER(
        splitCtx == nullptr, RT_ERROR_INVALID_VALUE,
        "TryCreateAutoSplitSlaveStream failed because splitCtx cannot be a NULL pointer, stream_id=%d.",
        curStream->Id_());

    if ((splitCtx->curStreamSqeCount + sqeNum + CAPTURE_TASK_RESERVED_NUM +
         masterStm->Device_()->GetDevProperties().expandStreamRsvTaskNum) < HOST_SQ_MAX_COUNT) {
    ...
    if (error != RT_ERROR_NONE) {
        RT_LOG(
            RT_LOG_ERROR, "Create slave stream failed, master_stream_id=%d, cur_stream_id=%d, retCode=%#x.",
    ...
    if (error != RT_ERROR_NONE) {
        RT_LOG(
            RT_LOG_ERROR, "CondStreamActive failed, master_stream_id=%d, slave_stream_id=%d, retCode=%#x.",
...
{
    RT_LOG(
        RT_LOG_DEBUG, "Enter AllocTaskInfoOnAutoSplitStream, master_stream_id=%d, cur_stream_id=%d, sqeNum=%u",
    ...
    *taskInfo = curStream->Device_()->GetTaskFactory()->Alloc(curStream, TS_TASK_TYPE_RESERVED, error);
    if (*taskInfo == nullptr) {
        ...
        RT_LOG_INNER_MSG(
            RT_LOG_ERROR, "Failed to allocate taskInfo, master_stream_id=%d, cur_stream_id=%d, error=%#x.",
    ...
    TaskInfo** taskInfo, Stream* const stm, uint32_t& pos, Stream*& dstStm, uint32_t sqeNum)
{
    ...
    COND_RETURN_ERROR_MSG_INNER(
        masterCtx == nullptr, RT_ERROR_INVALID_VALUE,
    ...
    if (slaveStream != nullptr) {
        ...
        RT_LOG(
            RT_LOG_DEBUG, "Auto-split using slave stream, master_stream_id=%d, cur_stream_id=%d",
...
rtError_t AllocTaskInfo(TaskInfo** taskInfo, Stream* const stm, uint32_t& pos, uint32_t sqeNum)
{
    ...
    RT_LOG(
        RT_LOG_DEBUG, "alloc taskinfo success, stream_id=%d, task_id=%hu, task_sn=%u.", stmId, (*taskInfo)->id,
        (*taskInfo)->taskSn);
...
rtError_t ProcAicpuTask(TaskInfo* submitTask)
{
    ...
    if (submitTask->stream->NeedSaveTask(submitTask)) {
        ...
        COND_RETURN_ERROR_MSG_INNER(
            (submitTask->stream->Model_() == nullptr), RT_ERROR_MODEL_NULL,
...
static rtError_t WriteAutoSplitSqeToHostBuffer(const TaskInfo* taskInfo, const Stream* stm, void* sqeAddr)
{
    ...
    if (ctx == nullptr || sqeBuffer == nullptr) {
        RT_LOG(RT_LOG_ERROR, "auto-split ctx or sqeBuffer is null, stream_id=%d, task_id=%u", stm->Id_(), taskInfo->id);
        return RT_ERROR_INVALID_VALUE;
    }
    ...
    if ((hostPos + taskInfo->sqeNum) > hostSqeCapacity) {
        RT_LOG(
            RT_LOG_ERROR,
            "hostSqeBuffer boundary check failed, stream_id=%d, task_id=%u, "
    ...
    COND_RETURN_AND_MSG_INNER(
        ...
        "Failed to call memcpy_s to copy sqeAddr, src=%p, dest=%p, dest_max=%lu, sqe_num=%u, retCode=%#x.", sqeAddr,
        hostSqPos, sqeSize, taskInfo->sqeNum, ret);
    RT_LOG(
        RT_LOG_DEBUG, "Auto-split SQE written to host buffer, stream_id=%d, task_id=%u, sqeNum=%u, hostPos=%u",
...
rtError_t DavidSendTask(TaskInfo* taskInfo, Stream* const stm)
{
    ...
    if (stm->IsSoftwareSqEnable()) {
        ...
        RT_LOG(
            RT_LOG_INFO,
            "device_id=%u, ts_id=%u, sq_id=%u, stream_id=%d, task_id=%hu, flip_num=%u, task_sn=%u, task_type=%u(%s)",
...
rtError_t SubmitTaskPostProc(Stream* const stm, uint32_t pos, bool isNeedStreamSync, int32_t timeout)
{
    ...
    if (stm->IsSeparateSendAndRecycle()) {
        COND_RETURN_INFO(
            (stm->taskResMang_ == nullptr), RT_ERROR_NONE, "taskResMang_ is null device_id=%u, stream_id=%d",
            stm->Device_()->Id_(), stm->Id_());
...
const char_t* TaskIdDesc() { return "task_pos"; }
...
uint32_t GetProfTaskId(const TaskInfo* const taskInfo) { return taskInfo->taskSn; }
