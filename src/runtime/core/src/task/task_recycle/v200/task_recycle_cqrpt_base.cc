    ...
    std::string& errMsg)
{
    const uint32_t reportTaskSn =
        (static_cast<uint32_t>(logicCq.taskId) << UINT16_BIT_NUM) | static_cast<uint32_t>(logicCq.streamId);
    if ((IsDvppTask(static_cast<uint16_t>(logicCq.sqeType)) || (logicCq.sqeType == RT_DAVID_SQE_TYPE_PLACE_HOLDER) ||
         ...
         (logicCq.sqeType == RT_DAVID_SQE_TYPE_CCU)) &&
        (logicCq.errorCode == TS_ERROR_TASK_TIMEOUT) &&
        ((logicCq.errorType & static_cast<uint8_t>(RT_STARS_CQE_ERR_TYPE_SW_STATUS)) != 0U) &&
        (reportTask != nullptr)) {
        ...
        RT_LOG(
            RT_LOG_ERROR,
            "Task run timeout, stream_id=%hu, pos=%hu, task_sn=%u, task_type=%u(%s), "
            "errType=%#x(%s), sqSwStatus=%#x.",
            streamId, pos, reportTaskSn, static_cast<uint32_t>(reportTask->type), GetTaskDescByType(reportTask->type),
        ...
        RT_LOG(
            ...
            "errType=%#x(%s), sqSwStatus=%#x.",
            streamId, pos, reportTaskSn, static_cast<uint32_t>(logicCq.sqeType), GetDavidSqeDescByType(logicCq.sqeType),
            logicCq.errorType, errMsg.c_str(), logicCq.errorCode);
    }
}
...
{
    return (type == TS_TASK_TYPE_DIRECT_SEND) || (type == TS_TASK_TYPE_MEMCPY) || (type == TS_TASK_TYPE_CCU_LAUNCH) ||
...
static rtError_t PollingSqDisable(const rtLogicCqReport_t* logicCq, Stream* const failStm)
{
    ...
    const int32_t getSqTimeout =
        (failStm->GetSyncRemainTime() == -1) ? RT_GET_SQ_STATUS_TIMEOUT_TIME : (failStm->GetSyncRemainTime() * 1000);
    uint64_t cnt = 0U;
    ...
    while (true) {
        if ((cnt++ % pollingCycleCnt) == 0U) {
            ...
            COND_PROC_RETURN_ERROR(
                error != RT_ERROR_NONE, error, failStm->SetStreamStatus(StreamStatus::ABNORMAL),
                "Failed to get sq enable, device_id=%u, stream_id=%d, retCode=%#x.", dev->Id_(), failStm->Id_(),
                static_cast<uint32_t>(error));
            if ((cnt % RT_QUERY_CNT_NUM) == 0U) {
                RT_LOG(
                    RT_LOG_EVENT, "dev_id=%u, ts_id=%u, stream_id=%d, enable=%u", devId, tsId, logicCq->sqId, enable);
            }
            if (!enable) {
            ...
            COND_RETURN_ERROR(
                ...
                "stream is in device task abort status, device_id=%u, stream_id=%d", devId, failStm->Id_());
            COND_RETURN_ERROR(
...
rtError_t StarsResumeRtsq(const rtLogicCqReport_t* logicCq, const TaskInfo* const taskInfo)
{
    ...
    COND_PROC_RETURN_ERROR(
        error != RT_ERROR_NONE, error, failStm->SetStreamStatus(StreamStatus::ABNORMAL),
        "Failed to enable sq, stream_id=%d, sq_id=%hu, device_id=%u, retCode=%#x.", failStm->Id_(), logicCq->sqId,
        devId, static_cast<uint32_t>(error));

    RT_LOG(
        RT_LOG_WARNING, "Resume stream_id=%d, sq_id=%hu, sq_head=%hu, task_id=%hu, taskType=%hu, head=%u.",
    ...
    Device* const dev, rtLogicCqReport_t& logicCq, TaskInfo* reportTask, uint16_t streamId, TaskInfo** outFaultTaskPtr)
{
    ...
    if (ProcReportIsException(logicCq)) {
        ...
        RT_LOG(
            ...
            dev->Id_(), streamId, pos, static_cast<uint32_t>(logicCq.sqeType), GetSqeDescByType(logicCq.sqeType),
            errType, errMsg.c_str(), logicCq.errorCode);
    } else {
        // no operation
    }
}
    ...
    rtLogicCqReport_t* const logicReport, bool& isFinished, bool& hasCqeReportErr)
{
    ...
    for (uint32_t idx = 0U; idx < cnt; ++idx) {
        ...
        const uint32_t reportTaskSn = (static_cast<uint32_t>(report.taskId) << UINT16_BIT_NUM) | report.streamId;
        RT_LOG(
            RT_LOG_INFO,
            "Get logic report: cnt=%u, idx=%u, stream_id=%hu, report_pos=%u, sync_pos=%u,"
        ...
        if (unlikely(reportTask == nullptr)) {
            RT_LOG(
                RT_LOG_WARNING, "GetTask error, device_id=%u, stream_id=%u, pos=%hu.", dev->Id_(), streamId,
                report.sqHead);
...
bool CompleteProcMultipleTaskReport(TaskInfo* const workTask, const rtLogicCqReport_t& report)
{
    ...
    RT_LOG(
        RT_LOG_DEBUG, "Get stars defined retCode=%hhu, sqe_type=%hhu, stream_id=%d, task_id=%u.", ret, report.sqeType,
        workTask->stream->Id_(), report.sqHead);
