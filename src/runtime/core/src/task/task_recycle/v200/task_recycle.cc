...
void ProcLogicCqUntilEmpty(const Stream* const stm)
{
    ...
    while (true) {
        ...
        COND_PROC_RETURN_ERROR_MSG_CALL(
            ERR_MODULE_DRV, error == RT_ERROR_LOST_HEARTBEAT, ,
...
rtError_t TryRecycleTask(Stream* const stm)
{
    ...
    if (stm->StreamSyncTryLock(3U)) { // 3ms
        ...
        COND_PROC_RETURN_ERROR(
            error != RT_ERROR_NONE, error, stm->StreamSyncUnLock(), "GetDrvSqHead failed, retCode=%#x.",
            static_cast<uint32_t>(error));
...
rtError_t TaskReclaimByStream(const Stream* const stm, const bool limited, const bool needLog)
{
    ...
    COND_RETURN_ERROR(error != RT_ERROR_NONE, error, "GetDrvSqHead failed, retCode=%#x.", static_cast<uint32_t>(error));
