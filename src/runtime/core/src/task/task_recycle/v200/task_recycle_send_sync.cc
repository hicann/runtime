...
// =================================================== static 函数区 ======================================== //
static void ReportTimeoutProc(
    const rtError_t error, int32_t& timeoutCnt, Stream* const stm, const uint32_t taskResPos, bool& isFinished)
{
...
static rtError_t SyncTaskEarlyBreak(const Stream* const stm, bool& breakFlag)
{
    ...
    COND_RETURN_ERROR(
        (stm->GetAbortStatus() == RT_ERROR_STREAM_ABORT), RT_ERROR_STREAM_ABORT_SYNC_TASK_FAIL,
        "stream is in stream abort status, sync task fail, device_id=%u, stream_id=%d", dev->Id_(), streamId);

    breakFlag = false;
    return RT_ERROR_NONE;
}

static rtError_t SyncTaskProcCqReport(
    Stream* const stm, const uint32_t taskResPos, int32_t timeout, uint64_t beginCnt, int32_t& remainTime,
    int32_t& rptTimeoutCnt, bool& isFinished)
{
    Device* dev = stm->Device_();
    const uint32_t streamId = static_cast<uint32_t>(stm->Id_());
    ...
    COND_RETURN_WARN(
        (timeout > 0) && (remainTime == 0) && (cnt == 0U) && (!isFinished), RT_ERROR_STREAM_SYNC_TIMEOUT,
    ...
    if (((error != RT_ERROR_NONE) && (error != RT_ERROR_SOCKET_CLOSE)) || (cnt == 0U)) {
        ...
        RT_LOG(
            RT_LOG_INFO, "No logic report: stream_id=%d, pos=%u, logicCqId=%u, ret=%#x.", streamId, taskResPos,
            stm->GetLogicalCqId(), static_cast<uint32_t>(error));
    }
    COND_RETURN_ERROR(
...
rtError_t SyncTaskForSeparateSendAndRecycle(Stream* const stm, const uint32_t taskResPos, int32_t timeout)
{
    ...
    Driver* const devDrv = dev->Driver_();
    COND_RETURN_ERROR(
        (devDrv == nullptr), RT_ERROR_NONE, "dev drv is null, device_id=%u, stream_id=%u.", dev->Id_(), streamId);
...
rtError_t SyncTask(Stream* const stm, const uint32_t taskResPos, int32_t timeout)
{
    ...
    COND_RETURN_ERROR_MSG_INNER(
        (dev->GetDeviceStatus() == RT_ERROR_DEVICE_TASK_ABORT), RT_ERROR_DEVICE_TASK_ABORT,
        "The stream is in device task abort state, SyncTask failed, device_id=%u, stream_id=%d.", dev->Id_(), streamId);
    COND_RETURN_ERROR_MSG_INNER(
        (stm->GetAbortStatus() == RT_ERROR_STREAM_ABORT), RT_ERROR_STREAM_ABORT_SYNC_TASK_FAIL,
        "The stream is in stream abort state, SyncTask failed, device_id=%u, stream_id=%d.", dev->Id_(), streamId);
    ...
    RT_LOG(
        RT_LOG_INFO, "Task Wait: device_id=%u, stream_id=%u, taskResPos=%u, logicCqId=%u.", dev->Id_(), streamId,
        taskResPos, logicCqId);
