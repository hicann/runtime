...
rtError_t MemcpyAsyncTaskInitV2(
    ...
    const uint64_t srcPitch, const uint64_t width, const uint64_t height, const uint32_t kind,
...
rtError_t MemWaitValueTaskInit(
    TaskInfo* taskInfo, const void* const devAddr, const uint64_t value, const uint32_t flag);
