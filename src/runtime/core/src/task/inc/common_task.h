...
rtError_t StarsCommonTaskInit(TaskInfo* taskInfo, const T& sqe, const uint32_t flag)
{
    ...
    const errno_t error = memcpy_s(
        &starsCommTask->commonStarsSqe.commonSqe, sizeof(starsCommTask->commonStarsSqe.commonSqe), &sqe, sizeof(sqe));
    if (error != EOK) {
        RT_LOG(
            RT_LOG_ERROR, "copy to starsSqe failed,ret=%d,src size=%zu,dst size=%zu", error, sizeof(sqe),
    ...
    starsCommTask->cmdList = RtValueToPtr<void*>(
    ...
    RT_LOG(
        RT_LOG_INFO, "dvpp type=%hu,need to write value=%u, needPostProc=%u.", sqeType, sqe.sqeHeader.reserved,
        taskInfo->needPostProc);
    return RT_ERROR_NONE;
}
