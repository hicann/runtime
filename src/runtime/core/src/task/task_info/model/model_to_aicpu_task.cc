...
void DoCompleteSuccForModelToAicpuTask(TaskInfo* taskInfo, const uint32_t devId)
{
    if (unlikely(taskInfo->errorCode != static_cast<uint32_t>(RT_ERROR_NONE))) {
        RT_LOG(
            RT_LOG_ERROR, "Model to aicpu task process error,retCode=%#x, [%s].", taskInfo->errorCode,
...
void SetStarsResultForModelToAicpuTask(TaskInfo* taskInfo, const rtCqReport_t& logicCq)
{
    if ((logicCq.errorType & RT_STARS_EXIST_ERROR) != 0U) {
        uint32_t aicpuErrMap[] = {TS_ERROR_AICPU_MODEL_RSP_ERR, TS_ERROR_AICPU_MODEL_RSP_ERR, TS_ERROR_AICPU_TIMEOUT,
                                  TS_ERROR_AICPU_MODEL_RSP_ERR, TS_ERROR_AICPU_MODEL_RSP_ERR, logicCq.errorCode};
