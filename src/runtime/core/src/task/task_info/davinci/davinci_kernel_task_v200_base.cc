...
static void SetStarsResultByErrorType(TaskInfo *taskInfo, const rtLogicCqReport_t &logicCq)
{
    ...
    uint32_t aicpuErrMap[TS_STARS_ERROR_MAX_INDEX] = {
        ...
        TS_ERROR_AICPU_EXCEPTION,
        logicCq.errorCode};
    uint32_t aicoreErrMap[TS_STARS_ERROR_MAX_INDEX] = {
...
void StarsV2SetStarsResultForDavinciTask(TaskInfo* taskInfo, const rtLogicCqReport_t &logicCq)
{
    if (taskInfo->type == TS_TASK_TYPE_KERNEL_AICPU) {
        ...
        if (aicpuErrorCode == AICPU_HCCL_OP_UB_DDRC_FAILED || aicpuErrorCode == AICPU_HCCL_OP_UB_POISON_FAILED || aicpuErrorCode == AICPU_HCCL_OP_UB_LINK_FAILED) {
