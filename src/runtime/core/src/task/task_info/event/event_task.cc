...
void TaskTriggerEvent(TaskInfo* const taskInfo)
{
    ...
    if (notifier != nullptr) {
        ...
        RT_LOG(
            RT_LOG_INFO, "notifier trigger, device_id=%u, stream_id=%d, task_id=%hu.",
...
void ToCommandBodyForEventRecordTask(TaskInfo* const taskInfo, rtCommand_t* const command)
{
    ...
    RT_LOG(
        RT_LOG_INFO,
        "device_id=%u, stream_id=%d, task_id=%hu, event_id=%d, flag=%#" PRIx64 ", Task_flag=%#x, "
        ...
        static_cast<uint32_t>(eventRecordTaskInfo->waitCqflag), static_cast<uint32_t>(eventRecordTaskInfo->waitCqId));
}
...
{
    RT_LOG(
        RT_LOG_INFO, "event destroy synchronize event_id=%d, isDestroySync_=%u.", evt->EventId_(),
        evt->GetDestroySync());
    const rtError_t error = evt->WaitForBusy();
    COND_PROC_RETURN_ERROR(
        (error != RT_ERROR_NONE), error, , "destroy synchronize failed, event_id=%d", evt->EventId_());
...
void DoCompleteSuccessForEventRecordTask(TaskInfo* const taskInfo, const uint32_t devId)
{
    ...
    eventRecordTaskInfo->timelineBase = MAX_UINT64_NUM;
    eventRecordTaskInfo->timelineOffset = MAX_UINT32_NUM;
    eventRecordTaskInfo->event->RecordComplete(taskInfo, eventRecordTaskInfo->timestamp, timeline);
    // model or simple must try to free id and destroy event.
    RT_LOG(
        RT_LOG_INFO,
        "event_record: device_id=%u, stream_id=%d, task_id=%hu, sq_id=%u, event_id=%d, "
        "timeline=%#" PRIx64 ", timestamp=%#" PRIx64,
        devId, stream->Id_(), taskInfo->id, stream->GetSqId(), eventRecordTaskInfo->eventid, timeline,
...
rtError_t EventResetTaskInit(
    TaskInfo* const taskInfo, Event* const eventPtr, const bool isNotifyFlag, const int32_t eventIndex)
{
    ...
    if (eventPtr != nullptr) {
        taskInfo->taskOwner = (eventPtr->EventOwner_() == EventOwner::EVENT_INNER) ?
                                  static_cast<uint8_t>(TaskOwner::RT_TASK_INNER) :
                                  taskInfo->taskOwner;
    }
    return RT_ERROR_NONE;
}
...
void ToCommandBodyForEventResetTask(TaskInfo* const taskInfo, rtCommand_t* const command)
{
    ...
    RT_LOG(
        RT_LOG_INFO, "event_reset: device_id=%u, stream_id=%d, task_id=%hu, event_id=%d.", stream->Device_()->Id_(),
        stream->Id_(), taskInfo->id, eventResetTaskInfo->eventid);
}
...
void DoCompleteSuccessForEventResetTask(TaskInfo* const taskInfo, const uint32_t devId)
{
    ...
    RT_LOG(
        RT_LOG_INFO, "event_reset: device_id=%u, stream_id=%d, task_id=%hu, event_id=%d, sq_id=%u,", devId,
        stream->Id_(), taskInfo->id, eventResetTaskInfo->eventid, stream->GetSqId());
...
rtError_t RemoteEventWaitTaskInit(
    TaskInfo* const taskInfo, Event* const eventRec, const int32_t srcDeviceId, const int32_t eventIndex)
{
...
rtError_t EventWaitTaskInit(
    TaskInfo* const taskInfo, Event* const eventRec, const int32_t eventIndex, const uint32_t timeout,
    const uint8_t waitFlag)
{
    ...
    if (eventRec != nullptr) {
        taskInfo->taskOwner = (eventRec->EventOwner_() == EventOwner::EVENT_INNER) ?
                                  static_cast<uint8_t>(TaskOwner::RT_TASK_INNER) :
                                  taskInfo->taskOwner;
    }
...
void ToCommandBodyForEventWaitTask(TaskInfo* const taskInfo, rtCommand_t* const command)
{
    ...
    RT_LOG(
        RT_LOG_INFO, "device_id=%u, stream_id=%d, task_id=%hu, event_id=%d", stream->Device_()->Id_(), stream->Id_(),
        taskInfo->id, eventWaitTaskInfo->eventId);
}
...
void DoCompleteSuccessForEventWaitTask(TaskInfo* const taskInfo, const uint32_t devId)
{
    ...
    RT_LOG(
        RT_LOG_INFO, "event_wait: device_id=%u, stream_id=%d, task_id=%hu, event_id=%u, sq_id=%u", devId, stream->Id_(),
        taskInfo->id, eventWaitTaskInfo->eventId, stream->GetSqId());
...
void PrintErrorInfoForEventWaitTask(TaskInfo* const taskInfo, const uint32_t devId)
{
    ...
    if (taskInfo->errorCode == TS_ERROR_AICPU_TIMEOUT) {
        ...
        STREAM_REPORT_ERR_MSG(
            ...
            taskInfo->typeName, flag);
    } else {
        STREAM_REPORT_ERR_MSG(
            reportStream, ERR_MODULE_HCCL,
            "EventWait task execution failed, "
            "device_id=%u, stream_id=%d, %s=%hu, flip_num=%hu, task_type=%d(%s), event_flag=%#" PRIx64 ". ",
            devId, streamId, TaskIdDesc(), taskInfo->id, taskInfo->flipNum, static_cast<int32_t>(taskInfo->type),
            taskInfo->typeName, flag);
    }
...
{
    const uint64_t eventTableId = static_cast<uint64_t>(eventId) / STARS_EVENT_NUM_OF_SINGLE_TABLE;
    const uint64_t eventNum = static_cast<uint64_t>(eventId) & 0xFFFUL;
    uint64_t base = static_cast<uint64_t>(srcDevice->GetDevProperties().eventBase);
    ...
    RT_LOG(
        RT_LOG_INFO, "Cross device event addr calculated: src_device=%u, event_id=%d, addr=0x%llx.", srcDevice->Id_(),
        eventId, addr);
    return addr;
}
#endif
