...
void DoCompleteSuccessForNotifyRecordTask(TaskInfo* taskInfo, const uint32_t devId)
{
    ...
    if (Runtime::Instance()->ChipIsHaveStars() && (taskInfo->bindFlag == 0U)) {
        ...
        RT_LOG(
            RT_LOG_INFO,
            "[DFX_SYNC] notify record finish. notify_id=%u, stream_id=%d, task_id=%hu, sq_id=%u,"
...
static rtError_t GetIpcNotifyBaseAddrForNotifyRecordTask(TaskInfo* taskInfo, uint64_t& baseAddr)
{
    ...
    localServerId =
        (((error != 0) || (localServerId > RT_NOTIFY_MAX_SRV_ID)) ? RT_NOTIFY_INVALID_SRV_ID : localServerId);
    ...
    RT_LOG(
        RT_LOG_INFO, "Get localServerId, retCode=%#x, local_ServerId=%lld, remote=%lld", static_cast<uint32_t>(error),
        localServerId, remoteServerId);
    const bool isPod =
        (localServerId != remoteServerId && localServerId != RT_NOTIFY_INVALID_SRV_ID &&
    ...
    if (isPod) {
        ...
        baseAddr = RT_CROSS_NODE_BASE_ADDR_48T + static_cast<uint64_t>(remoteServerId) * RT_SERVER_ADDR_OFFSET_1T +
                   static_cast<uint64_t>(chipId) * RT_CHIP_ADDR_OFFSET_128G +
                   static_cast<uint64_t>(dieId) * RT_DIE_ADDR_OFFSET_64G + RT_NOTIFY_ADDR_OFFSET_PER_DIE_1100000;
        RT_LOG(
            RT_LOG_INFO, "DevId=%u, remoteSrvId=%lld, rDevId=%u, chipId=%lld, dieId=%lld, baseAddr=0x%llx", localDevId,
            remoteServerId, remoteDevId, chipId, dieId, baseAddr);
        return RT_ERROR_NONE;
    }
    error = curDrv->GetTopologyType(localDevId, remoteDevId, remotePhyId, &topologyType);
    COND_RETURN_ERROR(
        (error != RT_ERROR_NONE), error, "Failed to get topology type, retCode=%#x, remoteDevId=%u, remotePhyId=%u.",
        error, remoteDevId, remotePhyId);
    if ((topologyType == TOPOLOGY_HCCS) || (topologyType == TOPOLOGY_HCCS_SW) || (topologyType == TOPOLOGY_SIO)) {
        notifyRecord->uInfo.singleBitNtfyInfo.isPcie = false;
        const rtError_t err = curDrv->GetChipIdDieId(localDevId, remoteDevId, remotePhyId, chipId, dieId);
        ERROR_RETURN_MSG_INNER(
            err, "Failed to get chipId and dieId, retCode=%#x, deviceId=%u, phyId=%u.", err, remoteDevId, remotePhyId);
        ...
        RT_LOG(
            RT_LOG_INFO,
            "localDevId=%u, remoteDevId=%u, remotePhyId=%u, chipId=%lld, dieId=%lld, hccs addr=0x%llx, "
            "topologyType=%lld",
            localDevId, remoteDevId, remotePhyId, chipId, dieId, baseAddr, topologyType);
        return RT_ERROR_NONE;
    } else if (
        (topologyType == TOPOLOGY_PIX) || (topologyType == TOPOLOGY_PIB) || (topologyType == TOPOLOGY_PHB) ||
        (topologyType == TOPOLOGY_SYS)) {
        ...
        COND_RETURN_ERROR(
            (error != RT_ERROR_NONE), error, "Failed to get localDevId phyId, retCode=%#x, deviceId=%u.", error,
            localDevId);
        baseAddr = static_cast<uint64_t>(RT_STARS_PCIE_BASE_ADDR) +
                   ...
                   (static_cast<uint64_t>(localPhyId) << static_cast<uint64_t>(RT_PCIE_LOCAL_DEV_OFFSET)) +
                   STARS_NOTIFY_BASE_ADDR;
        RT_LOG(
            RT_LOG_DEBUG, "localDevId=%u, localPhyId=%u, remoteDevId=%u, remotePhyId=%u, pcie addr=%#" PRIx64,
...
static rtError_t GetIpcSqeWriteAddrByTsId(const TaskInfo* const taskInfo, uint64_t& addr)
{
    ...
    addr = baseAddr + (notifyTableId * STARS_MC62CM12A_NOTIFY_TABLE_OFFSET) +
           (notifyPos * STARS_MC62CM12A_NOTIFY_OFFSET) + (notifySeparateId * STARS_MC62CM12A_NOTIFY_ID_4K_SEPARATE);
    RT_LOG(
        RT_LOG_INFO, "ipcNotifyId=%u, serverId=%u, dieId=%u, tsId=%u, baseAddr=%#llx", notifyId, serverId, dieId, tsId,
        addr);
...
rtError_t GetIpcSqeWriteAddrForNotifyRecordTask(TaskInfo* taskInfo, uint64_t& addr)
{
    if (IS_SUPPORT_CHIP_FEATURE(
            Runtime::Instance()->GetChipType(), RtOptionalFeatureType::RT_FEATURE_TASK_IPC_NOTIFY_ADDR_DOT_BY_TSID)) {
    ...
    const uint64_t notifyTableId =
        static_cast<uint64_t>(notifyId / taskInfo->stream->Device_()->GetDevProperties().starsNotifyTableSize);
    const uint64_t notifyNum =
        static_cast<uint64_t>(notifyId & taskInfo->stream->Device_()->GetDevProperties().ipcNotifyNumMask);
    ...
    if (taskInfo->stream->Device_()->GetDevProperties().starsResourceAddrCalculateMethod ==
        ...
        ERROR_RETURN_MSG_INNER(
            error, "GetIpcNotifyBaseAddrForNotifyRecordTask failed, retCode=%#x.", static_cast<uint32_t>(error));
    }
...
static void MapNotifyErrorCodeForFastRecovery(TaskInfo* taskInfo, const uint32_t devId)
{
    ...
    if (taskInfo->errorCode == AICPU_HCCL_OP_RETRY_FAILED) {
        ...
        if (hasMteErr && !HasMemUceErr(stream->Device_(), g_aicOrSdmaOrHcclLocalMulBitEccEventIdBlkList)) {
            ...
            RT_LOG(
                RT_LOG_ERROR,
                "hccl aicpu task error is local mem error, device_id=%u, stream_id=%d, task_id=%hu, "
                "taskInfo->errorCode=%u",
        ...
        if (!hasMteErr && !HasMemUceErr(stream->Device_(), g_hcclRemoteMulBitEccEventIdBlkList)) {
            ...
            RT_LOG(
                RT_LOG_ERROR,
                "hccl aicpu task error is remote mem error, device_id=%u, stream_id=%d, task_id=%hu, "
                "taskInfo->errorCode=%u",
                devId, stream->Id_(), taskInfo->id, taskInfo->errorCode);
        }
...
void DoCompleteSuccessForNotifyWaitTask(TaskInfo* taskInfo, const uint32_t devId)
{
    ...
    if (Runtime::Instance()->ChipIsHaveStars() && (taskInfo->bindFlag == 0U)) {
        ...
        RT_LOG(
            RT_LOG_INFO,
            "[DFX_SYNC] notify wait finish. notify_id=%u, stream_id=%d, task_id=%hu, sq_id=%u,"
            " device_id=%u",
...
void PrintErrorInfoForNotifyWaitTask(TaskInfo* const taskInfo, const uint32_t devId)
{
    ...
    STREAM_REPORT_ERR_MSG(
        ...
        "notify_id=%u, isCountNotify=%u.",
        devId, streamId, TaskIdDesc(), taskId, taskInfo->flipNum, taskInfo->u.notifywaitTask.notifyId,
        taskInfo->u.notifywaitTask.isCountNotify);
}
