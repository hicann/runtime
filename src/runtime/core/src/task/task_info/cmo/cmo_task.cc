    ...
    TaskInfo* taskInfo, const rtCmoTaskInfo_t* const cmoTaskInfo, const Stream* const stm, const uint32_t flag)
{
    ...
    if (stm->Device_()->IsSupportFeature(RtOptionalFeatureType::RT_FEATURE_TASK_CMO)) {
        if (cmoModel == nullptr) {
            ...
            // sqe info copy
            const errno_t error =
                memcpy_s(&cmoTsk->cmoSqeInfo, sizeof(rtCmoTaskInfo_t), cmoTaskInfo, sizeof(rtCmoTaskInfo_t));
            if (error != EOK) {
                std::stringstream ss;
                ss << std::hex << "dest=0x" << RtPtrToValue(&cmoTsk->cmoSqeInfo) << ", src=0x"
            ...
            RT_LOG(
                RT_LOG_DEBUG, "CmoTask Init, opCode=%u, length=%u", cmoTsk->cmoSqeInfo.opCode,
        ...
        RT_LOG_INNER_MSG(
            RT_LOG_ERROR,
            "The stream that delivers the CMO task is not in the model,"
            " device_id=%u, stream_id=%d.",
            taskInfo->stream->Device_()->Id_(), taskInfo->stream->Id_());
    ...
    if (error != EOK) {
        ...
        RT_LOG_OUTER_MSG_IMPL(
            ErrorCode::EE1020, "Initializing CMO task", "memcpy_s", std::to_string(error).c_str(), strerror(error),
            ss.str().c_str());
        return RT_ERROR_SEC_HANDLE;
    }
    ...
    RT_LOG(
        RT_LOG_DEBUG, "CmoTaskInfo: cmoType=%u, logicId=%u, cmoId=%u", cmoTsk->cmoSqeInfo.cmoType,
        cmoTsk->cmoSqeInfo.logicId, cmoTsk->cmoid);
    return RT_ERROR_NONE;
}
...
rtError_t CmoAddrTaskInit(TaskInfo* taskInfo, void* cmoAddrInfo, const rtCmoOpCode_t cmoOpCode)
{
    ...
    COND_RETURN_ERROR_MSG_INNER(
        (cmoModel == nullptr), RT_ERROR_MODEL_NULL,
    ...
    RT_LOG(
        RT_LOG_DEBUG, "CmoAddrTask Init, device_id=%d, stream_id=%d, task_id=%hu.",
