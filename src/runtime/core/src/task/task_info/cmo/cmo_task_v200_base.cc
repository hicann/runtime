...
void ConstructDavidCmoAddrSqe(TaskInfo* const taskInfo, rtDavidSqe_t* const davidSqe, uint64_t sqBaseAddr)
{
    ...
    RT_LOG(
        RT_LOG_INFO, "ConstructCmoAddrSqe, device_id=%u, stream_id=%d, task_id=%hu, task_sn=%u.",
...
void PrintErrorInfoForDavidCmoTask(TaskInfo* taskInfo, const uint32_t devId)
{
    ...
    const rtError_t error = dev->Driver_()->MemCopySync(
        RtPtrToPtr<void*>(cmoInfo.data()), sizeof(rtDavidCmoAddrInfo), cmoAddrTaskInfo->cmoAddrInfo,
    ...
    RT_LOG(
        RT_LOG_ERROR,
        "Sdma for CmoAddrTask in model stream execute failed, device_id=%u, stream_id=%d, task_id=%u,"
        "cmoAddrInfo=0x%llx.",
        devId, streamId, taskId, RtPtrToValue(cmoAddrTaskInfo->cmoAddrInfo));
    for (size_t i = 0UL; i < cmoInfoSize; i += 8U) {
        RT_LOG(
            RT_LOG_ERROR, "%s: %08x %08x %08x %08x %08x %08x %08x %08x", "rtCmoAddrInfo", cmoInfo[i], cmoInfo[i + 1U],
