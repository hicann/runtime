...
rtError_t StreamSqCqManage::UpdateStreamSqCq(Stream* newStm)
{
    ...
    const std::lock_guard<std::mutex> stmLock(streamMapLock_);
    const uint32_t remoteFlag = ((newStm->Flags() & RT_STREAM_CP_PROCESS_USE) != 0U) ? TSDRV_FLAG_REMOTE_ID : 0U;
    uint32_t drvFlag = (TSDRV_FLAG_RSV_CQ_ID | remoteFlag);
...
rtError_t StreamSqCqManage::ReAllocSqCqId(const Stream* const newStm)
{
    ...
    drvFlag = remoteFlag;
    error = device_->Driver_()->StreamBindLogicCq(device_->Id_(), device_->DevGetTsId(), streamId, logicCqId, drvFlag);
    COND_RETURN_ERROR(
