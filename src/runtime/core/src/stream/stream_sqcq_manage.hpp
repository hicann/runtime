...
public:
    ...
    rtError_t AllocLogicCq(
        const uint32_t streamId, const bool isNeedFast, uint32_t& logicCqId, bool& isFastCq,
        const bool isDefaultCq = false, const uint32_t drvFlag = 0U);
