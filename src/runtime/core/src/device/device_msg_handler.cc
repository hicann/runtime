...
DeviceMsgHandler::DeviceMsgHandler(Device* const devInfo, const rtGetMsgCallback msgCallback)
    : dev_(devInfo), callback_(msgCallback), devMemSize_(GET_DEVICE_MSG_BUF_SIZE)
{}

DeviceMsgHandler::~DeviceMsgHandler() noexcept { FreeDevMem(); }
...
void DeviceMsgHandler::MsgNotify(const char_t* const msg, const uint32_t len) const { callback_(msg, len); }
...
rtError_t DeviceMsgHandler::AllocDevMem()
{
    ...
    COND_RETURN_ERROR(
        devMem == nullptr, RT_ERROR_DRV_MEMORY,
        "DevMemAlloc failed, size=%u (bytes), "
        "memType=%u, deviceId=%u",
        devMemSize_, memType, dev_->Id_());
    devMemAddr_ = devMem;
    return RT_ERROR_NONE;
...
rtError_t DeviceMsgHandler::CheckGetDevMsgCtrlValid(const rtGetDevMsgCtrlInfo_t* const ctrlInfo) const
{
    if (ctrlInfo->magic != DEVICE_GET_MSG_MAGIC) {
        RT_LOG_INNER_MSG(
            RT_LOG_ERROR, "The magic %#x of the device message is invalid. Expected value: %#x.", ctrlInfo->magic,
            DEVICE_GET_MSG_MAGIC);
    ...
    COND_RETURN_ERROR_MSG_INNER(
        ctrlInfo->pid != pid, RT_ERROR_INVALID_VALUE,
        "The PID of the error message does not match, device pid=%u, host pid=%u.", ctrlInfo->pid, pid);

    COND_RETURN_ERROR_MSG_INNER(
        ((ctrlInfo->bufferLen < sizeof(rtGetDevMsgCtrlInfo_t)) || (ctrlInfo->bufferLen > devMemSize_)),
        RT_ERROR_INVALID_VALUE,
        "CheckGetDevMsgCtrlValid failed because value %u for parameter ctrlInfo->bufferLen is invalid, pid=%u. "
        "Expected value: [%zu, %u].",
...
rtError_t DeviceMsgHandler::HandleMsg()
{
    ...
    COND_RETURN_AND_MSG_OUTER(
        hostBuf == nullptr, RT_ERROR_MEMORY_ALLOCATION, ErrorCode::EE1013, devMemSize_ * sizeof(char_t), "new");
...
rtError_t DeviceErrMsgHandler::HandleMsgInHostBuf(const char_t* const msgBuff, const uint32_t msgBuffSize)
{
    ...
    while (offset < msgSize) {
        ...
        COND_RETURN_ERROR_MSG_INNER(
            ...
            "Failed to call strncpy_s to copy reportMsg,"
            " destMax=%zu, strSrc=%s, count=%zu, retCode=%d.",
            sizeof(logBuffer), reportMsg.c_str() + offset, copySize, err);
        offset += copySize;
...
rtError_t DeviceStreamSnapshotHandler::HandleMsgInHostBuf(const char_t* const msgBuff, const uint32_t msgBuffSize)
{
    if ((msgBuffSize % sizeof(rtStreamSnapshot_t)) != 0U) {
        RT_LOG(
            RT_LOG_ERROR,
            "msgBuffSize=%u (bytes) is not an integer multiple of "
            "sizeof(rtStreamSnapshot_t)=%zu (bytes).",
            msgBuffSize, sizeof(rtStreamSnapshot_t));
        return RT_ERROR_INVALID_VALUE;
    }
...
std::string DeviceStreamSnapshotHandler::GetActiveStreamSnapshot(
    const rtStreamSnapshot_t* const streamSnapshots, const uint32_t cnt) const
{
