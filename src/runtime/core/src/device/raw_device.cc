...
rtError_t RawDevice::SetCurGroupInfo(void)
{
    ...
    COND_RETURN_ERROR_MSG_INNER(
        (GetDevRunningState() == static_cast<uint32_t>(DEV_RUNNING_DOWN)), RT_ERROR_DRV_ERR,
    ...
    if (deviceErrorProc_ != nullptr) {
        ...
        COND_PROC_RETURN_ERROR_MSG_INNER(
            (error != RT_ERROR_NONE) && (error != RT_ERROR_DRV_MEMORY), error, DeleteStream(primaryStream_);
...
rtError_t RawDevice::InitRawDriver()
{
    ...
    ERROR_RETURN(error, "Failed to open device, retCode=%#x, device_id=%u.", static_cast<uint32_t>(error), deviceId_);
...
rtError_t RawDevice::RegisterAndLaunchDcacheLockOp(Context* ctx)
{
    ...
    if (dCacheLockFlag_ || !stackAddrIsDcache_) {
        RT_LOG(
            ...
            dCacheLockFlag_, stackAddrIsDcache_);
        return RT_ERROR_NONE;
    }
...
rtError_t RawDevice::Init()
{
    ...
    if (GetDevProperties().starsBaseMethod == StarsBaseMethod::STARS_BASE_CALCULATE_BY_DRIVER) {
        ...
        ERROR_RETURN(
            error, "Failed to get stars info, retCode=%#x, device_id=%u.", static_cast<uint32_t>(error), deviceId_);
        int64_t dieNum = 0;
    ...
    ERROR_GOTO(
        error, L2_FREE, "Failed to init dev capability, retCode=%#x, device_id=%u.", static_cast<uint32_t>(error),
        deviceId_);
    ...
    if (IsStarsPlatform()) {
        ...
        ERROR_GOTO(
            error, SQ_VIRTUAL_ADDR_FREE, "AllocMemForSqVirtualArr failed, retCode=%#x.", static_cast<uint32_t>(error));
        error = Driver_()->GetDevInfo(
            deviceId_, static_cast<int32_t>(MODULE_TYPE_SYSTEM), static_cast<int32_t>(INFO_TYPE_PHY_CHIP_ID),
            &phyChipId_);
        if (error != RT_ERROR_NONE) {
        ...
        error = Driver_()->GetDevInfo(
            deviceId_, static_cast<int32_t>(MODULE_TYPE_SYSTEM), static_cast<int32_t>(INFO_TYPE_PHY_DIE_ID),
            &phyDieId_);
        if (error != RT_ERROR_NONE) {
    ...
    COND_GOTO_ERROR(
        engine_ == nullptr, SQ_VIRTUAL_ADDR_FREE, error, RT_ERROR_MEMORY_ALLOCATION, "CreateEngine failed.");

    if (IsSupportFeature(RtOptionalFeatureType::RT_FEATURE_DEVICE_SPM_POOL)) {
        spmPool_ = new (std::nothrow) SpmPool(this);
        COND_GOTO_MSG_OUTER(
            spmPool_ == nullptr, ENGINE_FREE, error, RT_ERROR_MEMORY_ALLOCATION, ErrorCode::EE1013, sizeof(SpmPool),
            "new");
    ...
    if (IsSupportFeature(RtOptionalFeatureType::RT_FEATURE_DEVICE_EVENT_POOL) && (runMode == RT_RUN_MODE_ONLINE)) {
        eventPool_ = new (std::nothrow) EventPool(this, tsId_);
        COND_GOTO_MSG_OUTER(
            eventPool_ == nullptr, SPM_FREE, error, RT_ERROR_MEMORY_ALLOCATION, ErrorCode::EE1013, sizeof(EventPool),
            "new");
    }
    ...
    if (IsSupportFeature(RtOptionalFeatureType::RT_FEATURE_TASK_VALUE_WAIT)) {
        ...
        COND_GOTO_MSG_OUTER(
            eventExpandingPool_ == nullptr, SNAPSHOT_FREE, error, RT_ERROR_MEMORY_ALLOCATION, ErrorCode::EE1013,
            sizeof(EventExpandingPool), "new");
    }

    argLoader_ = new (std::nothrow) UmaArgLoader(this);
    COND_GOTO_MSG_OUTER(
        argLoader_ == nullptr, EVENT_EXE_FREE, error, RT_ERROR_MEMORY_ALLOCATION, ErrorCode::EE1013,
        sizeof(UmaArgLoader), "new");
    ...
    ERROR_GOTO(error, LOADER_FREE, "Failed to init ubArgLoader, retCode=%#x.", static_cast<uint32_t>(error));

    streamSqCqManage_ = new (std::nothrow) StreamSqCqManage(this);
    COND_GOTO_MSG_OUTER(
        streamSqCqManage_ == nullptr, LOADER_FREE, error, RT_ERROR_MEMORY_ALLOCATION, ErrorCode::EE1013,
    ...
    ERROR_GOTO(error, TASK_FACTORY_FREE, "Failed to init task factory, retCode=%#x.", static_cast<uint32_t>(error));

    kernelMemPoolMng_ = new (std::nothrow) MemoryPoolManager(this);
    COND_GOTO_MSG_OUTER(
        kernelMemPoolMng_ == nullptr, TASK_FACTORY_FREE, error, RT_ERROR_MEMORY_ALLOCATION, ErrorCode::EE1013,
        sizeof(MemoryPoolManager), "new");
    if (IsSupportFeature(RtOptionalFeatureType::RT_FEATURE_KERNEL_MEMORY_POOL)) {
        error = kernelMemPoolMng_->Init();
        ERROR_GOTO(
            error, KERNEL_POOL_FREE, "Failed to init MemoryPoolManager, retCode=%#x.", static_cast<uint32_t>(error));
    }

    modulesAllocator_ =
        new (std::nothrow) ObjAllocator<RefObject<Module*>>(DEFAULT_PROGRAM_NUMBER, Runtime::maxProgramNum_);
    COND_GOTO_MSG_OUTER(
    ...
    COND_GOTO_MSG_OUTER(
        deviceSqCqPool_ == nullptr, MODULES_ALLOC_FREE, error, RT_ERROR_MEMORY_ALLOCATION, ErrorCode::EE1013,
        sizeof(DeviceSqCqPool), "new");
    RT_LOG(RT_LOG_INFO, "new DeviceSqCqPool ok, Runtime_alloc_size %zu(bytes)", sizeof(DeviceSqCqPool));
    error = deviceSqCqPool_->Init();
    ERROR_GOTO(
        error, DEVICE_SQ_CQ_FREE, "Init raw device failed, init deviceSqCqPool_ failed, retCode=%#x.",
        static_cast<uint32_t>(error));

    sqAddrMemoryOrder_ = new (std::nothrow) SqAddrMemoryOrder(this);
    COND_GOTO_MSG_OUTER(
        sqAddrMemoryOrder_ == nullptr, DEVICE_SQ_CQ_FREE, error, RT_ERROR_MEMORY_ALLOCATION, ErrorCode::EE1013,
        sizeof(SqAddrMemoryOrder), "new");
    ...
    ERROR_GOTO(
        error, SQ_ADDR_MEMORY_FREE, "Init raw device failed, init sqAddrMemoryOrder_ failed, retCode=%#x.",
        static_cast<uint32_t>(error));

    RT_LOG(RT_LOG_INFO, "Runtime_alloc_size=%zu(bytes)", sizeof(RefObject<Module*>) * DEFAULT_PROGRAM_NUMBER);
...
rtError_t RawDevice::AllocStackPhyAddrForDcache()
{
    ...
    if ((error != RT_ERROR_NONE) || (stackPhyBase16k_ == nullptr)) {
        ...
        RT_LOG(
            RT_LOG_ERROR, "Alloc stack phy base failed, mem alloc failed, retCode=%#x.", static_cast<uint32_t>(error));
        return error;
    }
...
rtError_t RawDevice::AllocCustomerStackPhyBase()
{
    ...
    COND_RETURN_AND_MSG_OUTER(
        customerStackSize > props.maxCustomerStackSize, RT_ERROR_INVALID_VALUE, ErrorCode::EE1011,
        "Allocating physical memory with a customized stack size", std::to_string(customerStackSize),
        "customerStackSize",
        "The AI Core stack size " + std::to_string(customerStackSize) +
            " set by calling rtDeviceSetLimit must be less than or equal to " +
            std::to_string(props.maxCustomerStackSize));
    ...
    COND_RETURN_ERROR(
        ...
        ", customerAiCoreStackSize=%u(bytes), "
        "stackPhySize=%#" PRIx64 ".",
...
void RawDevice::FreeStackPhyBase()
{
    RT_LOG(
        RT_LOG_DEBUG,
        "FreeStackPhyBase drv devId=%u, stackPhyBase32k_=%p, drvMemCtrlHandle_=%p, stackAddrIsDcache_=%u.", deviceId_,
        stackPhyBase32k_, drvMemCtrlHandle_, stackAddrIsDcache_);
    if (stackPhyBase32k_ != nullptr) {
        if (stackAddrIsDcache_) {
            FreeDcacheAddr(deviceId_, stackPhyBase32k_, drvMemCtrlHandle_);
...
static rtError_t GetOneAicoreQosCfg(const uint32_t deviceId, QosMasterConfigType& aicoreQosCfg)
{
    ...
    RT_LOG(
        RT_LOG_INFO,
        "Begin get QOS info from halGetDeviceInfoByBuff, deviceId=%u, type=%u, moduleType=%d, infoType=%d.", deviceId,
        aicoreQosCfg.type, MODULE_TYPE_QOS, INFO_TYPE_QOS_MASTER_CONFIG);
    const rtError_t error = NpuDriver::GetDeviceInfoByBuff(
        deviceId, MODULE_TYPE_QOS, INFO_TYPE_QOS_MASTER_CONFIG, static_cast<void*>(&aicoreQosCfg), &bufSize);
    COND_RETURN_WARN(
    ...
    if ((error != RT_ERROR_NONE) || (bufSize != static_cast<int32_t>(sizeof(QosMasterConfigType)))) {
        RT_LOG(
            RT_LOG_ERROR,
            "Calling drv api halGetDeviceInfoByBuff failed, bufSize is %d, expect bufSize is %d, error=%#x.", bufSize,
            sizeof(QosMasterConfigType), static_cast<uint32_t>(error));
        return RT_ERROR_DRV_ERR;
    }
    RT_LOG(
        RT_LOG_INFO, "The QOS info from halGetDeviceInfoByBuff is: type=%u, mpamId=%u, qos=%u, pmg=%u, mode=%u.",
...
rtError_t RawDevice::GetQosInfoByIpc()
{
    ...
    aicoreQosCfg
        [static_cast<int32_t>(QosMasterType::MASTER_AIC_DAT) - static_cast<int32_t>(QosMasterType::MASTER_AIC_DAT)]
            .type = QosMasterType::MASTER_AIC_DAT;
    aicoreQosCfg
        [static_cast<int32_t>(QosMasterType::MASTER_AIC_INS) - static_cast<int32_t>(QosMasterType::MASTER_AIC_DAT)]
            .type = QosMasterType::MASTER_AIC_INS;
    aicoreQosCfg
        [static_cast<int32_t>(QosMasterType::MASTER_AIV_DAT) - static_cast<int32_t>(QosMasterType::MASTER_AIC_DAT)]
            .type = QosMasterType::MASTER_AIV_DAT;
    aicoreQosCfg
        [static_cast<int32_t>(QosMasterType::MASTER_AIV_INS) - static_cast<int32_t>(QosMasterType::MASTER_AIC_DAT)]
            .type = QosMasterType::MASTER_AIV_INS;
...
rtError_t RawDevice::Start()
{
    ...
    if (IsStarsPlatform()) {
        ...
        ERROR_GOTO_MSG_INNER(
            error, ERROR_FREE, "Failed to allocate stack phy, retCode=%#x.", static_cast<uint32_t>(error));
    ...
    if (GetDevProperties().ringbufSize != 0) {
        ...
        COND_GOTO_MSG_OUTER(
            deviceErrorProc_ == nullptr, ERROR_STOP, error, RT_ERROR_MEMORY_ALLOCATION, ErrorCode::EE1013,
        ...
        if (NpuDriver::CheckIsSupportFeature(deviceId_, FEATURE_DMS_GET_QOS_MASTER_CONFIG)) {
            error = InitQosCfg();
            ERROR_GOTO_MSG_INNER(
                error, ERROR_STOP, "Failed to get acc qos config, retCode=%#x, drv device_id=%u.",
                static_cast<uint32_t>(error), deviceId_);
        } else {
    ...
    if (!IsSupportFeature(RtOptionalFeatureType::RT_FEATURE_DEVICE_AICPUSD_LATER_PROCEDURE)) {
        ...
        ERROR_GOTO_MSG_INNER(
            error, ERROR_STOP, "Failed to send topic message version to AI CPU, retCode=%#x.",
            static_cast<uint32_t>(error));
    }
    ...
    ERROR_GOTO(
        error, ERROR_STOP, "Failed to allocate the prof switch address, retCode=%#x.", static_cast<uint32_t>(error));
...
uint64_t RawDevice::AllocSqIdMemAddr(void)
{
    if (sqIdMemAddrPool_ == nullptr) {
        ...
        COND_RETURN_AND_MSG_OUTER(
            sqIdMemAddrPool_ == nullptr, RT_ERROR_MEMORY_ALLOCATION, ErrorCode::EE1013, sizeof(BufferAllocator), "new");
    }
...
rtError_t RawDevice::AllocProfSwitchAddr(void)
{
    ...
    (void)driver_->MemCopySync(
        RtValueToPtr<void*>(profSwitchAddr_), sizeof(uint64_t), static_cast<const void*>(&profSwitch), sizeof(uint64_t),
...
void RawDevice::SetSupportFlipVersionSwitch()
{
    ...
    RT_LOG(
        RT_LOG_DEBUG, "chipType=%u, tschVersion=%u, supportFlipTask_=%u", chipType, tschVersion_,
        static_cast<uint32_t>(supportFlipTask_));
}
...
rtError_t RawDevice::UpdateTimeoutConfig()
{
    ...
    if (props.timeoutUpdateMethod == TimeoutUpdateMethod::TIMEOUT_SET_WITH_AICPU) {
        ...
        ERROR_RETURN_MSG_INNER(
            Runtime::Instance()->StartAicpuSd(this),
    ...
    ERROR_GOTO_MSG_INNER(
        error, ERROR_RECYCLE, "Failed to submit TaskTimeoutSetTask, retCode=%#x.", static_cast<uint32_t>(error));

    error = stm->Synchronize();
    ERROR_RETURN_MSG_INNER(
        error, "Failed to synchronize stream, stream_id=%d, retCode=%#x.", stm->Id_(), static_cast<uint32_t>(error));
...
rtError_t RawDevice::AicpuModelLoad(void* const modelInfo) { return aicpuSdAgent_.AicpuModelLoad(modelInfo); }

rtError_t RawDevice::AicpuModelDestroy(const uint32_t modelId) { return aicpuSdAgent_.AicpuModelDestroy(modelId); }

rtError_t RawDevice::AicpuModelExecute(const uint32_t modelId) { return aicpuSdAgent_.AicpuModelExecute(modelId); }
...
rtError_t RawDevice::AllocMemForSqVirtualArr()
{
    ...
    ERROR_RETURN(
        ...
        "size = %#" PRIx64 "(bytes).",
        static_cast<uint32_t>(error), deviceId_, allocSize);
    error = driver_->MemSetSync(sqVirtualArrBaseAddr_, allocSize, 0U, allocSize);
    ERROR_RETURN(
        error, "Failed to memset value, size=%#" PRIx64 "(bytes), retCode=%#x.", allocSize,
        static_cast<uint32_t>(error));
    return RT_ERROR_NONE;
}
...
void RawDevice::CreateMessageQueue()
{
    if ((Runtime::Instance()->GetDisableThread()) &&
        ...
        COND_RETURN_VOID_AND_MSG_OUTER(
            messageQueue_ == nullptr, ErrorCode::EE1013, arraySize * sizeof(uint64_t), "new");
...
void RawDevice::CreateFreeEventQueue()
{
    ...
    COND_RETURN_VOID_AND_MSG_OUTER(freeEvent_ == nullptr, ErrorCode::EE1013, arraySize * sizeof(int32_t), "new");
...
Stream* RawDevice::GetCtrlStream(Stream* const stream) const
{
    ...
    if ((Runtime::Instance()->GetDisableThread()) /* virtual situation, not enough sq for ctrlsq */
        ...
        (GetTschVersion() >= static_cast<uint32_t>(TS_VERSION_CTRL_SQ)) && (CtrlStream_() != nullptr)) {
        return CtrlStream_();
...
bool RawDevice::IsNeedFreeEventId() { return (freeEventQueueHead_ != freeEventQueueTail_); }
...
rtError_t RawDevice::FreeEventIdFromDrv(const int32_t eventId, const uint32_t eventFlag, bool freeSyncFlag)
{
    ...
    if (IsSupportFeature(RtOptionalFeatureType::RT_FEATURE_DEVICE_NOTIFY_ONLY)) {
        ...
        error =
            Driver_()->NotifyIdFree(static_cast<int32_t>(Id_()), static_cast<uint32_t>(eventId), DevGetTsId(), flag);
        ERROR_RETURN(
            error,
    ...
    ERROR_GOTO(
        error, ERROR_RECYCLE, "Failed to init the event destruction task, device_id=%u, event_id=%d, retCode=%#x.",
        Id_(), eventId, static_cast<uint32_t>(error));
    ...
    ERROR_GOTO_MSG_INNER(
        error, ERROR_RECYCLE,
        "Failed to submit the event destruction task, device_id=%u, event_id=%d, "
        "retCode=%#x.",
        Id_(), eventId, static_cast<uint32_t>(error));
    if (freeSyncFlag == true) {
        bool needWait = true;
...
void RawDevice::CtrlStreamSetup()
{
    ...
    COND_PROC_RETURN_VOID_AND_MSG_OUTER(
        ctrlStream_ == nullptr, ErrorCode::EE1013, DELETE_O(ctrlRes_), sizeof(CtrlStream), "new");

    const rtError_t error = ctrlStream_->Setup();
    COND_PROC((error != RT_ERROR_NONE), DeleteStream(ctrlStream_); DELETE_O(ctrlRes_); return;)
    RT_LOG(
        RT_LOG_INFO, "[ctrlsq]New ctrlStream ok, Runtime_alloc_size %zu(bytes), stream_id=%d.", sizeof(Stream),
        ctrlStream_->Id_());
    return;
}
...
{
    if ((tschVersion >= static_cast<uint32_t>(TS_VERSION_CTRL_SQ)) && (Runtime::Instance()->GetDisableThread()) &&
        (IsSupportFeature(RtOptionalFeatureType::RT_FEATURE_DEVICE_STREAM_CTRL))) {
...
rtError_t RawDevice::WriteDevString(void* const dest, const size_t max, const char_t* const str)
{
    ...
    COND_RETURN_ERROR(
        devStrLen == 0UL, RT_ERROR_INVALID_VALUE,
    ...
    ERROR_RETURN(error, "Failed to memset string, size=%zu, retCode=%#x.", devStrBufLen, static_cast<uint32_t>(error));
...
{
    COND_RETURN_ERROR(
        size == 0U, RT_ERROR_INVALID_VALUE, "Failed to write device value, data size(%zu) must be greater than 0.",
        size);

    rtError_t error = driver_->MemSetSync(dest, static_cast<uint64_t>(size), 0U, static_cast<uint64_t>(size));
    ERROR_RETURN(error, "Failed to memset string, size=%zu, retCode=%#x.", size, static_cast<uint32_t>(error));
    error = driver_->MemCopySync(
...
rtError_t RawDevice::InitPrintInfo()
{
    ...
    RT_LOG(
        RT_LOG_DEBUG, "InitPrintInfo succ, device_id=%u, printfAddr_=%p, printblockLen_=%u.", deviceId_, printfAddr_,
        printblockLen_);
    return RT_ERROR_NONE;
}
...
rtError_t RawDevice::InitSimtPrintInfo()
{
    ...
    RT_LOG(
        RT_LOG_DEBUG, "InitSimtPrintf succ, device_id=%u, simtPrintfAddr_=%p, simtPrintLen_=%u.", deviceId_,
        simtPrintfAddr_, simtPrintLen_);
    return RT_ERROR_NONE;
}
...
void RawDevice::WakeUpPrintf() { printfCv_.notify_one(); }
...
rtError_t RawDevice::GetPrintFifoAddrAndCreateThread(uint64_t* const addr, const uint32_t model)
{
    ...
    if (model == PRINT_SIMD) {
        if (printfAddr_ == nullptr) {
            ret = InitPrintInfo();
            COND_RETURN_ERROR(
                (ret != RT_ERROR_NONE), ret, "InitPrintInfo failed, device_id=%u, ret=%u.", deviceId_, ret);
        }
        // 内部会判空保证不重复创建线程
...
rtError_t RawDevice::StoreEndGraphNotifyInfo(const uint32_t streamId, Model* captureModel, uint32_t endGraphNotifyPos)
{
    ...
    RT_LOG(
        RT_LOG_INFO,
        "Store exeStreamId=%u captureModelId=%u endGraphNotifyPos=%u in captureModelExeInfoMap_, numOfPos=%zu.",
    ...
    const uint32_t streamId, Model* captureModel, uint32_t endGraphNotifyPos, const uint32_t errCode)
{
    ...
    RT_LOG(
        RT_LOG_INFO,
        "Delete exeStreamId=%u captureModelId=%u endGraphNotifyPos=%u in captureModelExeInfoMap_, numOfPos=%u.",
        streamId, captureModel->Id_(), endGraphNotifyPos, numOfPos);
    return RT_ERROR_NONE;
}
...
bool JudgeIsExecuted(const uint16_t sqHead, const uint16_t sqTail, const uint32_t pos)
{
    ...
    RT_LOG(RT_LOG_INFO, "JudgeIsExecuted: isExecuted=%d, head=%u, tail=%u, pos=%u", isExecuted, sqHead, sqTail, pos);

    return isExecuted;
}

bool RawDevice::JudgeIsEndGraphNotifyWaitExecuted(
    const Stream* const exeStream, Model* captureModel, std::list<uint32_t>& sqePosList) const
{
    ...
    for (auto it = sqePosList.begin(); it != sqePosList.end();) {
        ...
        COND_RETURN_ERROR(
            error != RT_ERROR_NONE, false, "Query sq head failed in JudgeIsEndGraphNotifyWaitExecuted, retCode=%#x.",
            static_cast<uint32_t>(error));
...
void RawDevice::PollEndGraphNotifyInfo()
{
    ...
    for (auto it = captureModelExeInfoMap_.begin(); it != captureModelExeInfoMap_.end();) {
        ...
        COND_PROC(((ret != RT_ERROR_NONE) || (exeStream == nullptr)), it = captureModelExeInfoMap_.erase(it);
                  continue;);
...
void RawDevice::PollEndGraphNotifyInfoByModelId(const uint32_t modelId)
{
    ...
    for (auto it = captureModelExeInfoMap_.begin(); it != captureModelExeInfoMap_.end();) {
        Model* model;
        std::tie(streamId, model) = it->first;
        COND_PROC((model->Id_() != modelId), ++it; continue;);
        const rtError_t ret = GetStreamSqCqManage()->GetStreamSharedPtrById(streamId, exeStream);
        COND_PROC(((ret != RT_ERROR_NONE) || (exeStream == nullptr)), it = captureModelExeInfoMap_.erase(it); break;);
...
void RawDevice::UnRegisterProgram(Program* prog) { (void)programSet_.erase(prog); }
...
rtError_t RawDevice::SetSupportHcomcpuFlag()
{
    ...
    ERROR_RETURN(
        error, "get hcom cpu num failed, deviceId=%d, retCode=%#x, hcomCpuNum=%lld", deviceId_, error, hcomCpuNum);
...
rtError_t RawDevice::RestoreSqCqPool()
{
    ...
    COND_RETURN_ERROR(
        err != RT_ERROR_NONE, err, "Failed to realloc sqcq in deviceSqCqFreeList_, device_id=%u, retCode=%#x",
...
void RawDevice::SetBaseTime()
{
    ...
    COND_RETURN_VOID(
        (error != RT_ERROR_NONE) && (error != RT_ERROR_FEATURE_NOT_SUPPORT) && (error != RT_ERROR_DRV_INPUT),
