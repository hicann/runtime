...
public:
    ...
    uint32_t DevGetTsId() const override { return tsId_; }

    bool DevGetOnlineProfStart() override { return onProfEnable_.Value(); }

    void SetDevFailureMode(const uint64_t failureMode) override { devFailureMode_.Set(failureMode); }
    ...
    uint32_t GetPendingNum() const override { return engine_->GetPendingNum(); }
    ...
    void AddObserver(EngineObserver* observer) override { engine_->AddObserver(observer); }
    void WaitCompletion() override { engine_->WaitCompletion(); }
    Driver* Driver_() const override { return driver_; }

    ArgLoader* ArgLoader_() const override { return argLoader_; }

    UbArgLoader* UbArgLoaderPtr() const override { return ubArgLoader_; }
    Stream* PrimaryStream_() const override { return primaryStream_; }

    Stream* CtrlStream_() const override { return ctrlStream_; }

    uint32_t Id_() const override { return deviceId_; }

    uint16_t GetTsdQos() const override { return tsdQos_; }

    void SetDeviceStatus(rtError_t status) override { deviceStatus_.Set(status); }
    ...
    DeviceFaultType GetDeviceFaultType() const override { return deviceFaultType_.Value(); }

    uint64_t GetTTBR_() const override { return TTBR_; }

    uint32_t GetSSID_() const override { return SSID_; }
    uint64_t GetTCR_() const override { return TCR_; }

    void* GetL2Buffer_() const override { return l2buffer_; }

    const void* GetStackPhyBase32k() const override { return stackPhyBase32kAlign_; }

    const void* GetStackPhyBase16k() const override { return stackPhyBase16k_; }

    uint32_t GetDeviceAllocStackSize() const override { return deviceAllocStackSize_; }

    uint32_t GetDeviceCustomerStackLevel() const override { return deviceCustomerStackLevel_; }
    ...
    {
        COND_RETURN_ERROR(
            !IsSupportFeature(RtOptionalFeatureType::RT_FEATURE_DEVICE_SPM_POOL), RT_ERROR_FEATURE_NOT_SUPPORT,
    ...
    rtError_t AllocPoolEvent(int32_t* eventId) override { return eventPool_->AllocEventId(eventId); }
    rtError_t FreePoolEvent(const int32_t eventId) override { return eventPool_->FreeEventId(eventId); }
    rtError_t FreeEventIdFromDrv(
        const int32_t eventId, const uint32_t eventFlag = 0, bool freeSyncFlag = false) override;
    ...
    void FreeExpandingPoolEvent(const int32_t eventId) override { return eventExpandingPool_->FreeEventId(eventId); }
    ...
    rtChipType_t GetChipType() const override { return chipType_; }

    void SetChipType(rtChipType_t chipType) override { chipType_ = chipType; }

    std::string GetSocVersion() const override { return socVersion_; }
    ...
    uint64_t GetDevProfStatus() const override { return devProfStatus_; }
    ...
    MemoryPoolManager* GetKernelMemoryPool() const override { return kernelMemPoolMng_; }

    JettyManager* GetJettyManager() const override { return jettyManager_.get(); }

    uint32_t GetTschVersion() const override { return tschVersion_; }

    uint32_t IsSupportHcomcpu() const override { return isSupportHcomcpu_; }
    ...
    rtError_t InitStreamRes(const uint32_t streamId) override { return engine_->InitStreamRes(streamId); }
    ...
    uint32_t GetShareLogicCqId() const override { return engine_->GetShareLogicCqId(); }

    uint32_t GetShareSqId() const override { return engine_->GetShareSqId(); }

    bool GetFastCqFlag() const override { return fastCqFlag_; }

    void SetFastCqFlag(bool flag) override { fastCqFlag_ = flag; }

    void DmaCpyIncrease() override { dmaCpyUsedNum_++; }

    void DmaCpyDecrease() override { --dmaCpyUsedNum_; }

    bool IsDmaCpyNumLimit() const override { return (dmaCpyUsedNum_.load() >= DMA_CPY_NUM_DEVICE_MAX) ? true : false; }

    int64_t GetPhyChipId() const override { return phyChipId_; }
    ...
    void CtrlTaskReclaim(CtrlStream* const stm) override { return engine_->CtrlTaskReclaim(stm); }
    void* GetSqVirtualArrBaseAddr_() const override { return sqVirtualArrBaseAddr_; }
    ...
    bool GetSnapshotFlag() override { return isPrintSnapshotFlag_; }

    void SetSnapshotFlag(const bool flag) override { isPrintSnapshotFlag_ = flag; }

    uint32_t GetTsLogToHostFlag() override { return isSupportTslogTohost; }

    void SetTsLogToHostFlag(const uint32_t flag) override { isSupportTslogTohost |= flag; }

    Engine* Engine_() const { return engine_; }
    ...
    uint32_t GetDevRunningState() override { return engine_->GetDevRunningState(); }

    void SetDevStatus(const rtError_t status) override { devStatus_ = status; }

    rtError_t GetDevStatus(void) override { return devStatus_; }

    SpinLock* GetAiCpuSdLock() override { return &aicpuSchSdLock_; }

    void SetIsAiCpuSdStarted(const bool value) override { isAicpuSdStarted_ = value; }

    bool IsAiCpuSdStarted() const override { return isAicpuSdStarted_; }

    void SetSupportFlipVersionSwitch() override;

    bool IsSupportTsVersionForFlipTask() const override { return supportFlipTask_; }
    uint64_t GetChipAddr() const override { return chipBaseAddr; }

    uint64_t GetChipOffset() const override { return chipOffset; }

    uint64_t GetStarsRegBaseAddr() const override { return starsRegBaseAddr_; }
    uint64_t GetDieOffset() const override { return chipDieOffset; }

    bool AllocHcclIndex(uint16_t& hcclIndex, uint16_t stmid) override;
    void FreeHcclIndex(uint16_t stmid) override;
    uint16_t GetHcclStreamIdWithIndex(uint16_t index) override { return hcclStream[index]; }

    void SetIsChipSupportRecycleThread(bool flag) override { isChipSupportRecycleThread_ = flag; }

    bool GetIsChipSupportRecycleThread() const override { return isChipSupportRecycleThread_; }

    void SetIsChipSupportEventThread(bool flag) override { isChipSupportEventThread_ = flag; }

    bool GetIsChipSupportEventThread() const override { return isChipSupportEventThread_; }

    void TryFreeAllEventPoolId() override;
    void SetLastUsagePoolTimeStamp() override { lastUsagePoolTimeStamp_ = mmGetTickCount(); }
    ...
    const void* GetSimtStackPhyBase() const override { return simtStackPhyBaseAlign_; }

    void SetSimtStackPhyBase(void* simtStackPhyBaseAlign) override { simtStackPhyBaseAlign_ = simtStackPhyBaseAlign; }

    uint8_t GetDavidDieNum() const override { return davidDieNum_; }
    ...
    void SetIsRingbufferGetErr(bool flag) override { isRingbufferGetErr_ = flag; }

    bool GetIsRingbufferGetErr() const override { return isRingbufferGetErr_; }
    ...
    void SetDebugSqId(const uint32_t debugSqId) override { debugSqId_ = debugSqId; }
    uint32_t GetDebugSqId() const override { return debugSqId_; }
    void SetDebugCqId(const uint32_t debugCqId) override { debugCqId_ = debugCqId; }
    uint32_t GetDebugCqId() const override { return debugCqId_; }
    void SetCoredumpEnable() override { isCoredumpEnable_ = true; }
    bool IsCoredumpEnable() const override { return isCoredumpEnable_; }

    uint64_t GetInterCoreSyncAddr() const override { return interCoreSyncAddr_; }
    ...
    const void* GetCustomerStackPhyBase() const override { return customerStackPhyBase_; }

    uint32_t GetSimtDvgWarpStkSize() const override { return simtDvgWarpStkSize_; }

    uint32_t GetSimtWarpStkSize() const override { return simtWarpStkSize_; }
    ...
    void SetPageFaultBaseCnt(const uint32_t value) override { pageFaultBaseCnt_ = value; }

    void SetPageFaultBaseTime(const uint64_t time) override { pageFaultBaseTime_ = time; }

    uint32_t GetPageFaultBaseCnt() const override { return pageFaultBaseCnt_; }

    uint64_t GetPageFaultBaseTime() const override { return pageFaultBaseTime_; }

    void SetAixErrRecoverCnt() override { aixErrRecoverCnt_.Add(1U); }

    uint32_t GetAixErrRecoverCnt() const override { return aixErrRecoverCnt_.Value(); }

    void AddSimtPrintTlvCnt(uint64_t val) const override { simtPrintTlvCnt_.Add(val); }

    uint64_t GetSimtPrintTlvCnt() const override { return simtPrintTlvCnt_.Value(); }

    bool GetPrintSimtEnable() const override { return simtEnable_; }

    uint32_t GetSimtPrintLen() const override { return simtPrintLen_; }

    void* GetSimtPrintfAddr() const override { return simtPrintfAddr_; }
     ...
     */
    void SetRunMode(uint32_t runMode) override { runMode_ = runMode; }
    /*
     * get run mode.
     * @return uint32_t: current run mode
     */
    uint32_t GetRunMode() const override { return runMode_; }
    ...
    rtError_t DeleteEndGraphNotifyInfo(
        const uint32_t streamId, Model* captureModel, uint32_t endGraphNotifyPos, const uint32_t errCode) override;
    ...
    bool IsSupportStopOnStreamError(void) override { return isSupportStopOnStreamError_; }

    uint64_t GetC2cCtrlAddr(void) override { return c2cCtrlAddr_.Value(); }

    uint32_t GetC2cCtrlAddrLen(void) override { return c2cCtrlAddrLen_.Value(); }
    ...
    DeviceSqCqPool* GetDeviceSqCqManage() const override { return deviceSqCqPool_; }

    SqAddrMemoryOrder* GetSqAddrMemoryManage() const override { return sqAddrMemoryOrder_; }

    const DevProperties& GetDevProperties(void) const override { return properties_; }

    void RefreshDevProperties(const DevProperties& props) override { properties_ = props; }
    ...
    std::mutex& GetExceptionRegMutex() override { return exceptionRegMutex_; }
    ...
    uint64_t GetProfSwitchAddr(void) override { return profSwitchAddr_; }

    CtrlSQ& GetCtrlSQ(void) const override { return *(ctrlSQ_.get()); }
    ...
    const qos_master_config_t& GetQosCfg() const { return aicoreQosCfgs_; }
...
private:
     ...
     */
    std::map<std::tuple<uint32_t, Model*>, std::list<uint32_t>>
        captureModelExeInfoMap_; // key: tuple<exeStreamId, captureModel>, value: endGraphNotifyPosList
