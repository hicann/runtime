...
rtError_t CheckReduceCapability(Device* const dev, const rtRecudeKind_t kind, const rtDataType_t type)
{
    ...
    ERROR_RETURN_MSG_INNER(error, "Get chip capability failed, device_id=%u, retCode=%#x.", dev->Id_(), error);
    ...
    if (starsFlag) {
        ...
        if (((sdmaReduceKind >> shift) & 0x1U) == 0U) {
            RT_LOG_OUTER_MSG_WITH_FUNC_DESC(
                ...
                "Parameter kind value " + ReduceKindToString(kind),
    ...
    if (((kind == RT_MEMCPY_SDMA_AUTOMATIC_EQUAL) && (!starsFlag))) {
        RT_LOG_OUTER_MSG_WITH_FUNC_DESC(
            ErrorCode::EE1006, "Checking the availability of Reduce on the current device",
            "Parameter kind value RT_MEMCPY_SDMA_AUTOMATIC_EQUAL(13)",
...
rtError_t InitReduceTask(
    TaskInfo* const task, const void* const src, void* const dst, const uint64_t cpySize, const rtRecudeKind_t kind,
    const rtDataType_t type, const rtTaskCfgInfo_t* const cfgInfo)
{
...
rtError_t MemWaitValue(const void* const devAddr, const uint64_t value, const uint32_t flag, Stream* const stm)
{
    ...
    TaskInfo taskSubmit = {};
    TaskInfo* rtMemWaitValueTask =
        stm->AllocTask(&taskSubmit, TS_TASK_TYPE_MEM_WAIT_VALUE, errorReason, MEM_WAIT_SQE_NUM);
