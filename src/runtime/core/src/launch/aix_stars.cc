...
namespace {
rtError_t InternalLaunchKernelPrepare(
    Context* const ctx, Kernel*& registeredKernel, Program*& prog, rtKernelAttrType& kernelAttrType, Module*& mdl,
    ...
    uint32_t& prefetchCnt1, uint32_t& prefetchCnt2)
{
    ...
    if (tilingKey == DEFAULT_TILING_KEY) {
        ...
        if (device->GetDevProperties().enabledTSNum == ENABLED_TS_NUM_2) {
            ...
            COND_RETURN_ERROR_MSG_INNER(
                (((kernelAttrType == RT_KERNEL_ATTR_TYPE_AICORE) || (kernelAttrType == RT_KERNEL_ATTR_TYPE_CUBE)) &&
                 (tsId == 1U)) ||
    ...
    COND_RETURN_ERROR_MSG_CALL(
        ...
        "Can not find kernel by function[0x%" PRIx64 "], tilingKey=%" PRIu64 ".", RtPtrToValue<const void*>(stubFunc),
        tilingKey);
...
rtError_t InternalUpdateTaskPrepare(
    const Context* const ctx, TaskInfo* const updateTask, const Kernel* const kernel, const Stream* const stm)
{
    Device* const device = ctx->Device_();
    COND_RETURN_ERROR(
        stm->IsCapturing(), RT_ERROR_STREAM_CAPTURED,
        "The stream used for updating task cannot be in capture status, device_id=%u, stream_id=%d", device->Id_(),
        stm->Id_());

    if (updateTask->stream->Model_() == nullptr) {
        RT_LOG(
            RT_LOG_ERROR, "The update task must be a sinked task, device_id=%u, stream_id=%d, task_id=%hu.",
    ...
    if (stm->Model_() != nullptr) {
        RT_LOG(
            RT_LOG_ERROR,
            "The update stream must be a single operator stream, "
...
rtError_t InternalUpdateMixKernelTask(
    const Context* const ctx, TaskInfo* const updateTask, Stream* const stm, void* const updateArgHandle)
{
    ...
    if (error != RT_ERROR_NONE) {
        RT_LOG(
            RT_LOG_ERROR, "device_id=%u, stream_id=%d, allocSize=%llu, retCode=%#x.", device->Id_(), stm->Id_(),
            copySize, error);
        goto ERROR_RECYCLE;
    }
...
rtError_t InternalUpdateNormalKernelTaskH2DSubmitComm(
    const Context* const ctx, TaskInfo* const updateTask, Stream* const stm, void* const targetAddrOfUpdatedSqe,
    void* const updateArgHandle)
{
    ...
    if (error != RT_ERROR_NONE) {
        RT_LOG(
            RT_LOG_ERROR, "prepare failed, device_id=%u, stream_id=%d, allocSize=%llu, retCode=%#x.", device->Id_(),
            stm->Id_(), allocSize, error);
        goto ERROR_RECYCLE;
    }

    error = SqeUpdateH2DTaskInit(
        rtMemcpyAsyncTask, hostAddr, targetAddrOfUpdatedSqe, allocSize, oldArgHandle, updateArgHandle);
    if (error != RT_ERROR_NONE) {
        RT_LOG(
            RT_LOG_ERROR, "device_id=%u, stream_id=%d, allocSize=%llu, retCode=%#x.", device->Id_(), stm->Id_(),
            allocSize, error);
...
rtError_t InternalUpdateNormalKernelTaskH2DSubmit(
    const Context* const ctx, TaskInfo* const updateTask, Stream* const stm, void* const updateArgHandle)
{
    AicTaskInfo* aicTaskInfo = &(updateTask->u.aicTaskInfo);
    return InternalUpdateNormalKernelTaskH2DSubmitComm(ctx, updateTask, stm, aicTaskInfo->sqeDevBuf, updateArgHandle);
}

rtError_t InternalUpdateNormalKernelTaskD2HSubmit(
    const Context* const ctx, TaskInfo* const updateTask, Stream* const stm)
{
    TaskInfo submitTask = {};
    rtError_t errorReason;
    ...
    rtError_t error =
        UpdateD2HTaskInit(rtMemcpyAsyncTask, sqeDevBuf, copySize, sqId, pos, updateTask->u.aicTaskInfo.updateSqeOffset);
    if (error != RT_ERROR_NONE) {
        RT_LOG(
            RT_LOG_ERROR, "device_id=%u, stream_id=%d, allocSize=%u, retCode=%#x.", device->Id_(), stm->Id_(),
            allocSize, error);
...
rtError_t InternalUpdateNormalKernelTaskByTS(
    const Context* const ctx, TaskInfo* const updateTask, Stream* const stm, void* const updateArgHandle)
{
    rtError_t errorReason;
    TaskInfo submitTask = {};
    TaskInfo* kernTask = nullptr;
    kernTask = stm->AllocTask(
        &submitTask, TS_TASK_TYPE_TASK_SQE_UPDATE, errorReason, 1U, UpdateTaskFlag::NOT_SUPPORT_AND_SKIP);
    COND_RETURN_ERROR_MSG_INNER(
...
rtError_t InternalUpdateNormalKernelTask(
    const Context* const ctx, TaskInfo* const updateTask, Stream* const stm, void* const updateArgHandle)
{
    Device* const device = ctx->Device_();
    rtError_t error = RT_ERROR_NONE;
    if (!Runtime::Instance()->ChipIsHaveStars()) {
        ...
        if (error != RT_ERROR_NONE) {
            RT_LOG(
                RT_LOG_ERROR, "submit SqeUpdate task failed, device_id=%u, stream_id=%d, retCode=%#x.", device->Id_(),
                stm->Id_(), error);
        }
        return error;
    }
    if (updateTask->stream->IsSoftwareSqEnable()) {
        ...
        if (error != RT_ERROR_NONE) {
            RT_LOG(
                RT_LOG_ERROR, "submit D2H task failed, device_id=%u, stream_id=%d, retCode=%#x.", device->Id_(),
                stm->Id_(), error);
        }
        return error;
    }
    ...
    if (error != RT_ERROR_NONE) {
        RT_LOG(
            RT_LOG_ERROR, "submit H2D task failed, device_id=%u, stream_id=%d, retCode=%#x.", device->Id_(), stm->Id_(),
            error);
        return error;
    }
    ...
    if (error != RT_ERROR_NONE) {
        RT_LOG(
            RT_LOG_ERROR, "submit D2H task failed, device_id=%u, stream_id=%d, retCode=%#x.", device->Id_(), stm->Id_(),
            error);
        return error;
    }
...
rtError_t InternalLaunchUpdateKernelSubmit(
    const Context* const ctx, TaskInfo* const updateTask, Stream* const stm, const rtArgsEx_t* const argsInfo,
    StarsArgLoaderResult& result)
{
    ...
    ERROR_RETURN_MSG_INNER(
        error,
        "Failed to submit kernel update task, device_id=%u, stream_id=%d, task_id=%hu, "
        "retCode=%#x.",
        ctx->Device_()->Id_(), updateTask->stream->Id_(), updateTask->id, error);
...
rtError_t InternalLaunchKernelSubmit(
    const Context* const ctx, TaskInfo*& submitTask, Stream*& stm, const rtArgsEx_t*& argsInfo,
    StarsArgLoaderResult& result, const Program* const programPtr)
{
...
void InternalLaunchKernelRecycle(
    const Context* const ctx, StarsArgLoaderResult& result, TaskInfo*& recycleTask, const Program* const prog)
{
    rtError_t error = RT_ERROR_NONE;
    Runtime* const rt = Runtime::Instance();
    ...
    const bool isLaunchVec)
{
    ...
    if ((mixType != NO_MIX) && (device->ArgLoader_()->CheckPcieBar()) &&
        ...
        ERROR_GOTO_MSG_INNER(error, ERROR_FREE, "Failed to load args, stream_id=%d, retCode=%#x.", stm->Id_(), error);
        copyFlag = false;
    } else {
        // do nothing
    }
    ...
    const TaskCfg* const taskCfg, const bool isLaunchVec)
{
    ...
    error = InternalLaunchKernelPrepare(
        ctx, registeredKernel, prog, kernelAttrType, launchMdl, stubFunc, addr1, addr2, nullptr, 0ULL, prefetchCnt1,
        prefetchCnt2);
    COND_PROC_RETURN_ERROR_MSG_INNER((error == RT_ERROR_MEMORY_ADDRESS_UNALIGNED) || (error == RT_ERROR_KERNEL_NULL),
                                     error, InternalLaunchKernelRecycle(ctx, result, kernTask, nullptr);
    ...
    if ((mixType != static_cast<uint8_t>(NO_MIX)) && (device->ArgLoader_()->CheckPcieBar()) &&
        ...
        ERROR_GOTO_MSG_INNER(
            error, ERROR_RECYCLE, "Failed to load args, stream_id=%d, retCode=%#x.", stm->Id_(), error);
        copyFlag = false;
    } else {
        // do nothing
    }
    ...
    COND_PROC(
        ...
        isNeedAllocSqeDevBuf = true);
    AicTaskInit(
        kernTask, registeredKernel, kernelAttrType, static_cast<uint16_t>(coreDim), taskCfg, isNeedAllocSqeDevBuf);
    ...
    const TaskCfg* const taskCfg, const bool isLaunchVec)
{
    ...
    NULL_PTR_RETURN_MSG_OUTER_WITH_FUNC_DESC(
        progHandle, RT_ERROR_PROGRAM_NULL, "Delivering a kernel task based on BinHandle");
    ...
    error = InternalLaunchKernelPrepare(
        ctx, registeredKernel, prog, kernelAttrType, launchMdl, nullptr, addr1, addr2, progHandle, tilingKey,
        prefetchCnt1, prefetchCnt2);
    COND_PROC_RETURN_ERROR_MSG_INNER((error == RT_ERROR_MEMORY_ADDRESS_UNALIGNED) || (error == RT_ERROR_KERNEL_NULL),
                                     error, InternalLaunchKernelRecycle(ctx, result, kernTask, nullptr);
                                     , "kernel launch kernel prepare failed.");
    COND_PROC_RETURN_ERROR_MSG_INNER((error == RT_ERROR_PROGRAM_BASE) || (error == RT_ERROR_PROGRAM_NULL), error,
                                     InternalLaunchKernelRecycle(ctx, result, kernTask, nullptr);
                                     , "kernel launch prog prepare failed.");
