...
static rtError_t CondStreamActiveForAicpuStream(const Stream* const activeStream, Stream* const stm)
{
    ...
    ERROR_RETURN_MSG_INNER(
        error, "Failed to allocate device value for active task, retCode=%#x.", static_cast<uint32_t>(error));
    ...
    const Stream* const activeStream, Stream* const stm, Context* const ctx, const bool alreadyCascaded)
{
    ...
    if ((activeStrFlag & RT_STREAM_AICPU) != 0U) {
        ...
        NULL_PTR_RETURN(mdl, RT_ERROR_MODEL_NULL);
        RT_LOG(
            RT_LOG_INFO, "no process flags=%u, active_stream_id=%u, stream flags=%u, stream_id=%d.", activeStrFlag,
            activeStreamId, strFlag, streamId);
        return RT_ERROR_NONE;
    }
    ...
    ERROR_RETURN_MSG_INNER(
        error, "Failed to submit active task, stream_id=%d, pos=%u, retCode=%#x.", streamId, pos,
        static_cast<uint32_t>(error));
    ...
    Stream* const stm, const rtSwitchDataType_t dataType, Context* const ctx)
{
    ...
    ERROR_RETURN_MSG_INNER(
        error, "Failed to initialize stream switch task, stream_id=%d, retCode=%#x.", streamId,
        static_cast<uint32_t>(error));
    error = DavidSendTask(rtStreamSwitchTask, dstStm);
    ERROR_RETURN_MSG_INNER(
        error, "Failed to submit stream switch task, stream_id=%d, pos=%u, retCode=%#x.", streamId, pos,
        static_cast<uint32_t>(error));
    tskErrRecycle.ReleaseGuard();
    stm->StreamUnLock();
...
rtError_t SubmitCaptureConditionTask(CondHandle* condHandle, Stream* const stm)
{
    ...
    rtError_t error = CheckTaskCanSend(stm);
    ERROR_RETURN_MSG_INNER(
        error, "Failed to check whether task can be sent on stream (stream_id=%d), retCode=%#x.", streamId,
        static_cast<uint32_t>(error));
    ...
    ERROR_RETURN_MSG_INNER(
        error, "Capture model condition task submit failed, stream_id=%d, pos=%u, retCode=%#x.", streamId, pos,
        static_cast<uint32_t>(error));
    tskErrRecycle.ReleaseGuard();
    stm->StreamUnLock();
    ...
    ERROR_RETURN_MSG_INNER(
        ...
        condTask->id, static_cast<uint32_t>(error));
