      ...
      policy_(policy)
{
    ...
    if (policy_ == COPY_POLICY_PCIE_BAR) {
        devAllocator_ = new (std::nothrow)
            BufferAllocator(size, initCnt, maxCnt, stg, &MallocPcieBarBuffer, &FreePcieBarBuffer, device_);
        ...
        devAllocator_ =
            new (std::nothrow) BufferAllocator(size, initCnt, maxCnt, stg, &MallocBuffer, &FreeBuffer, &cpyInfoDmaMap_);
        ...
        devAllocator_ = new (std::nothrow)
            BufferAllocator(size, initCnt, maxCnt, stg, &MallocUbBuffer, &FreeUbBuffer, &cpyInfoUbMap_);
...
void H2DCopyMgr::FreeDevMem(void* item)
{
    ...
    if (policy_ == COPY_POLICY_ASYNC_PCIE_DMA) {
        ...
        if (handle->copyStatus != ASYNC_COPY_STATU_INIT) {
            ...
            if (error != RT_ERROR_NONE) {
                RT_LOG(
                    RT_LOG_ERROR, "MemCopyAsyncWaitFinish for cpy result failed, retCode=%#x, isDmaPool=%d.", error,
                    handle->isDmaPool);
            }
            handle->copyStatus = ASYNC_COPY_STATU_INIT;
        }
...
rtError_t H2DCopyMgr::H2DMemCopy(void* dst, const void* const src, const uint64_t size)
{
    ...
    if (policy_ == COPY_POLICY_PCIE_BAR) {
        ...
        if (!(iter->second.isDma)) {
            ...
            error = drv_->MemCopyAsync(handle->devAddr, size, src, size, RT_MEMCPY_HOST_TO_DEVICE, handle->copyStatus);
            if (error != RT_ERROR_NONE) {
...
rtError_t H2DCopyMgr::ArgsPoolConvertAddr(void)
{
    ...
    for (auto iter = cpyInfoDmaMap_.cpyDmaMap.begin(); iter != cpyInfoDmaMap_.cpyDmaMap.end(); ++iter) {
        ...
        const rtError_t error = dev->Driver_()->MemConvertAddr(
            cpyAddr.hostAddr, devAddr, static_cast<uint64_t>(cpyInfoDmaMap_.cpyItemSize), &cpyAddr.dmaAddr);
        if (error != RT_ERROR_NONE) {
            RT_LOG(RT_LOG_ERROR, "Failed to convert virtual address to DMA physical address, retCode=%#x.", error);
            return error;
        }
        RT_LOG(
            RT_LOG_DEBUG, "devAddr=%#llx, iter->second=%p, dmaAddr.phyAddr.priv=%p.", devAddr,
            iter->second.dmaAddr.phyAddr.priv, cpyAddr.dmaAddr.phyAddr.priv);
    }
    return RT_ERROR_NONE;
}
...
void* H2DCopyMgr::MallocBuffer(const size_t size, void* const para)
{
    ...
    if ((cpyInfoDmaInfo->cpyCount != 0U) && (addr != nullptr)) {
        ...
        for (uint32_t item = 0U; item < cpyInfoDmaInfo->cpyCount; item++) {
            ...
            const rtError_t error = dev->Driver_()->MemConvertAddr(
                cpyAddr.hostAddr, devAddr, static_cast<uint64_t>(cpyInfoDmaInfo->cpyItemSize), &dmaAddr);
...
void H2DCopyMgr::FreeBuffer(void* const addr, void* const para)
{
    ...
    if ((iter != cpyInfoDmaInfo->cpyDmaMap.end()) && (iter->second.devBaseAddr == RtPtrToValue(addr))) {
        for (uint32_t item = 0U; item < cpyInfoDmaInfo->cpyCount; item++) {
            const uint64_t devAddr =
                iter->second.devBaseAddr + item * static_cast<uint64_t>(cpyInfoDmaInfo->cpyItemSize);
...
void* H2DCopyMgr::MallocPcieBarBuffer(size_t size, void* para)
{
    ...
    if (ret != RT_ERROR_NONE) {
        (void)dev->Driver_()->DevMemFree(addr, dev->Id_());
        RT_LOG(
            RT_LOG_ERROR, "Pcie Host Register failed, retCode=%#x, size=%u(bytes), dev_id=%u.", ret, size, dev->Id_());
        return nullptr;
    }
...
{
    rtError_t ret = dev->Driver_()->GetTsegInfoByVa(dev->Id_(), RtPtrToValue(hostAddr), size, 1U, hostTsegInfo);
    if (ret != RT_ERROR_NONE) {
...
void* H2DCopyMgr::MallocUbBuffer(const size_t size, void* const para)
{
    ...
    if (ret != RT_ERROR_NONE) {
        (void)dev->Driver_()->DevMemFree(devAddr, dev->Id_());
        RT_LOG(
            RT_LOG_ERROR, "alloc host mem failed, retCode=%#x, size=%u(bytes), device_id=%u.", ret, size, dev->Id_());
        return nullptr;
    }
    ...
    if (ret != RT_ERROR_NONE) {
        ...
        (void)dev->Driver_()->DevMemFree(devAddr, dev->Id_());
        RT_LOG(
            RT_LOG_ERROR, "alloc mem for tseg info failed, retCode=%#x, size=%u(bytes), device_id=%u.", ret,
            sizeof(struct memTsegInfo), dev->Id_());
        return nullptr;
    }
    struct memTsegInfo* memInfo = (struct memTsegInfo*)tsegInfoAddr;
    ret = GetMemTsegInfo(
        dev, devAddr, hostAddr, static_cast<uint64_t>(size), &(memInfo->devTsegInfo), &(memInfo->hostTsegInfo));
    if (ret != RT_ERROR_NONE) {
...
void H2DCopyMgr::FreeUbBuffer(void* const addr, void* const para)
{
    ...
    if ((iter == cpyInfoUbInfo->cpyUbMap.end()) || (iter->second.devBaseAddr != RtPtrToValue(addr))) {
    ...
    (void)dev->Driver_()->PutTsegInfo(dev->Id_(), &(cpyInfoUbInfo->memTsegInfoMap[poolIndex]->hostTsegInfo));
    (void)dev->Driver_()->PutTsegInfo(dev->Id_(), &(cpyInfoUbInfo->memTsegInfoMap[poolIndex]->devTsegInfo));
...
rtError_t H2DCopyMgr::UbArgsPoolConvertAddr(void)
{
    ...
    for (uint32_t poolIndex = 0U; poolIndex < cpyInfoUbMap_.poolIndex; poolIndex++) {
        ...
        if (tsegInfo == cpyInfoUbMap_.memTsegInfoMap.end()) {
            RT_LOG(
                RT_LOG_WARNING, "Can't find tseg info by pool index=%u, devAddr=%#lx.", poolIndex,
                cpyAddrPtr->devBaseAddr);
            continue;
        }
        ...
        if (memInfo == nullptr) {
            RT_LOG(
                RT_LOG_WARNING, "MemTsegInfo is null, poolIndex=%u, devAddr=%#lx.", poolIndex, cpyAddrPtr->devBaseAddr);
            continue;
        }
        const rtError_t error = GetMemTsegInfo(
            dev, RtValueToPtr<void*>(cpyAddrPtr->devBaseAddr), RtValueToPtr<void*>(cpyAddrPtr->hostBaseAddr),
            static_cast<uint64_t>(cpyInfoUbMap_.cpyItemSize), &(memInfo->devTsegInfo), &(memInfo->hostTsegInfo));
        ERROR_RETURN(
            error, "Refresh ub segment info failed, poolIndex=%u, devAddr=%#lx, retCode=%#x.", poolIndex,
            cpyAddrPtr->devBaseAddr, error);
        RT_LOG(
            RT_LOG_DEBUG, "Refresh ub segment success, poolIndex=%u, devAddr=%#lx.", poolIndex,
            cpyAddrPtr->devBaseAddr);
    }
