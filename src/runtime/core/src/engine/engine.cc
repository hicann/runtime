...
void EngineObserver::DeviceIdle(Device* const dev) { UNUSED(dev); }
...
rtError_t Engine::SubmitTask(TaskInfo* const workTask, uint32_t* const flipTaskId, int32_t timeout)
{
    ...
    if (ret != RT_ERROR_NONE) {
        ...
        RT_LOG(
            RT_LOG_WARNING, "SubmitTask stream_id=%d, task_id=%u, task_type=%u, result=%d", workTask->stream->Id_(),
            workTask->id, workTask->type, ret);
    }
...
uint32_t Engine::GetRecycleDavinciTaskNum(
    const uint16_t lastRecycleEndTaskId, const uint16_t recycleEndTaskId, const uint16_t recyclePublicTaskCount) const
{
    ...
    if (totalTaskNum < recyclePublicTaskCount) {
        RT_LOG(
            RT_LOG_ERROR,
            "Failed to get task num, lastRecycleEndTaskId=%hu, recycleEndTaskId=%hu, recyclePublicTaskCount=%hu.",
...
bool Engine::ProcessTask(TaskInfo* workTask, const uint32_t deviceId)
{
    ...
    if (recycleStm->isHasPcieBar_) {
        uint32_t davinciTaskNum =
            GetRecycleDavinciTaskNum(lastRecycleEndTaskId, recycleEndTaskId, recyclePublicTaskCount);
...
void Engine::TrigerAsyncRecycle(
...
bool Engine::ProcessPublicTask(TaskInfo* workTask, const uint32_t deviceId, uint16_t* recycleTaskCount)
{
    ...
    do {
        ...
        if (error != RT_ERROR_NONE) {
            RT_LOG(
                RT_LOG_WARNING, "Get delete task failed,stream_id=%d,tail_task_id=%hu,del_task_id=%hu,retCode=%#x.",
        ...
        if (workTask == nullptr) {
            ...
            RT_LOG(
                RT_LOG_EVENT,
                "workTask has been recycled,stream_id=%d,isHasPcieBar_=%d "
        ...
        if (error != RT_ERROR_NONE) {
            RT_LOG(
                RT_LOG_WARNING, "Delete task failed,stream_id=%d,tail_task_id=%hu,del_task_id=%hu,retCode=%#x.",
                streamId, endTaskId, delTaskId, static_cast<uint32_t>(error));
            break;
        }
        ...
        if (isDisableThread && TaskIdIsGT(static_cast<uint32_t>(delTaskId), static_cast<uint32_t>(endTaskId))) {
            RT_LOG(
                RT_LOG_DEBUG, "DelTaskId is greater than endTaskId, delTaskId=%hu endTaskId=%hu.", delTaskId,
                endTaskId);
            break;
        }
    ...
    *recycleTaskCount = count;
    ...
    RT_LOG(
        RT_LOG_INFO, "device_id=%u, stream_id=%d, delTaskId=%hu, bindFlag=%d, streamFreeFlag=%d, resultFlag=%d.",
...
bool Engine::ProcessTaskDavinciList(Stream* const stm, const uint16_t endTaskId, const uint32_t deviceId)
{
    ...
    do {
        ...
        if (error != RT_ERROR_NONE) {
            RT_LOG(
                RT_LOG_DEBUG, "Delete task failed, stream_id=%d, tail_task_id=%hu, del_task_id=%hu, retCode=%#x.",
                streamId, endTaskId, delTaskId, static_cast<uint32_t>(error));
            break;
        }
        ...
        if (recycleTsk == nullptr) {
            ...
            RT_LOG_INNER_MSG(
                RT_LOG_ERROR,
                "Failed to find davinci task from task factory, device_id=%u, stream_id=%d, "
                "isHasPcieBar_=%d, hasTaskResMang_=%u, task_id=%u, endTaskId=%hu, pendingNum=%u, lastTaskId=%u.",
                deviceId, streamId, stm->isHasPcieBar_, hasTaskResMang, static_cast<uint32_t>(delTaskId), endTaskId,
                stm->pendingNum_.Value(), stm->GetLastTaskId());
            break;
        }
...
rtError_t Engine::DelPublicRecycleTask(
    const TaskInfo* const workTask, Stream* const stm, const bool isTaskBind, const uint16_t endTaskId) const
{
...
rtError_t Engine::GetPublicRecycleTask(
    TaskInfo* const workTask, Stream* const stm, const bool isTaskBind, const uint16_t endTaskId,
...
rtError_t Engine::GetDavinciDelRecordedTask(
    Stream* const stm, const uint16_t endTaskId, uint16_t* const delTaskId) const
{
...
rtError_t Engine::ProcessTaskWait(TaskInfo* const task) const
{
    ...
    if (unlikely(error != RT_ERROR_NONE)) {
        RT_LOG(
            RT_LOG_ERROR, "WaitAsyncCopyComplete Failed. task_id=%hu, task_type=%d (%s), retCode=%d.", task->id,
            static_cast<int32_t>(task->type), task->typeName, error);
...
void Engine::ReportStatusFailProc(const rtError_t error, const uint32_t deviceId) const
{
    if ((error == RT_ERROR_SOCKET_CLOSE) || (error == RT_ERROR_LOST_HEARTBEAT)) {
        ...
        (void)memset_s(
            &(exceptionInfo.expandInfo), sizeof(exceptionInfo.expandInfo), 0, sizeof(exceptionInfo.expandInfo));
...
void Engine::ReportSocketCloseProcV2()
{
    ...
    if (hdcStatus == RT_ERROR_NONE) {
        const rtError_t hdcErr = static_cast<rtError_t>(
            (hdcStat == static_cast<int32_t>(HDC_SESSION_STATUS_CLOSE)) ? RT_ERROR_SOCKET_CLOSE : RT_ERROR_NONE);
...
void Engine::ReportStatusOomProc(const rtError_t error, const uint32_t deviceId) const
{
    ...
    COND_RETURN_VOID_AND_MSG_OUTER(exceptionInfo == nullptr, ErrorCode::EE1013, sizeof(rtExceptionInfo_t), "new");
    ...
    (void)memset_s(
        &(exceptionInfo.get()->expandInfo), sizeof(rtExceptionExpandInfo_t), 0, sizeof(rtExceptionExpandInfo_t));
...
void Engine::ReportTimeoutProc(
    const rtError_t error, int32_t& timeoutCnt, const uint32_t streamId, const uint32_t taskId, const uint32_t execId,
    const uint64_t msec)
{
...
void Engine::RecycleTask(const uint32_t streamId, const uint32_t taskId)
{
    ...
    RT_LOG(
        RT_LOG_DEBUG, "Report receive, stream_id=%u, task_id=%u, task_type=%u(%s).", streamId, workTask->id,
    ...
    if (unlikely(reportCount_ != parseTaskCount_)) {
        RT_LOG(
            RT_LOG_WARNING,
            "stream_id=%u taskId =%hu task_type=%u(%s) report_count=%" PRIu64 " parse_task_count=%" PRIu64 " not equal",
            streamId, workTask->id, static_cast<uint32_t>(taskType), workTask->typeName, reportCount_, parseTaskCount_);
    }
...
void Engine::RecycleCtrlTask(CtrlStream* const stm, const uint32_t sqPos)
{
    ...
    do {
        ...
        TIMESTAMP_END(ObserverFinished);
        RT_LOG(
            RT_LOG_INFO, "[ctrlsq]process task recycle,stream_id=%d, task_id=%u, task_type=%d (%s).", stm->Id_(),
            taskId, static_cast<int32_t>(workTask->type), workTask->typeName);
...
rtError_t Engine::SendFlipTask(uint16_t preTaskId, Stream* stm)
{
    ...
    ERROR_GOTO_MSG_INNER(
        error, ERROR_RECYCLE,
        "SendFlipTask failed, dev_id=%u, stream_id=%d, task_id=%hu, flipNumReport=%hu, retCode=%#x.", device_->Id_(),
        stm->Id_(), fliptask->id, fliptask->u.flipTask.flipNumReport, error);
    RT_LOG(
        RT_LOG_INFO, "SendFlipTask dev_id=%u, stream_id=%d, task_id=%hu, flipNum=%hu.", device_->Id_(), stm->Id_(),
    ...
    TaskInfo* const workTask, rtTsCommand_t& cmdLocal, rtTsCmdSqBuf_t* const command, const uint32_t sendSqeNum)
{
    ...
    if (unlikely(error != RT_ERROR_NONE)) {
        RT_LOG(
            RT_LOG_ERROR,
            "copy error device_id=%u, ts_id=%u, sq_id=%u, cq_id=%u, stream_id=%d,"
            " task_id=%hu, task_type=%d(%s), retCode=%d.",
            deviceId, tsId, sqId, cqId, stm->Id_(), workTask->id, static_cast<int32_t>(workTask->type),
    ...
    error = devDrv->CommandSend(
        sqId, command, GetReportCount(workTask->pkgStat, profileEnable), deviceId, tsId, sendSqeNum);
...
rtError_t Engine::SendTask(TaskInfo* const workTask, uint16_t& taskId, uint32_t* const flipTaskId)
{
    ...
    while (command == nullptr) {
        ...
        stm->SetFlowCtrlFlag();
        COND_RETURN_ERROR(
            (stm->PrintStmDfxAndCheckDevice(beginCnt, endCnt, checkCount, tryCount) != RT_ERROR_NONE), error,
            "device status down when command occupy, device_id=%u, stream_id=%d", devId, stm->Id_());
    }
    ...
    RT_LOG(
        ...
        stm->GetIsSupportASyncRecycle(), stm->IsNeedPostProc(workTask), stm->GetDavinciTaskHead(),
        stm->GetDavinciTaskTail(), stm->GetTaskHead(), stm->GetTaskTail(), stm->GetBindFlag(), posHead, newPosTail,
        posTail, stm->GetDelayRecycleTaskSize());
...
bool Engine::isEnablePrintfThread(void) { return (printfThread_ != nullptr); }
...
void Engine::PrintfRun()
{
    ...
    const ScopeGuard ctxGuard([]() { InnerThreadLocalContainer::SetCurCtx(nullptr); });
