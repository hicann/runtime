...
uint32_t DirectHwtsEngine::GetShareSqId() const { return shmCq_.GetShareSqId(); }
...
rtError_t DirectHwtsEngine::Init()
{
    ...
    rtError_t error = devDrv->LogicCqAllocate(
        device_->Id_(), device_->DevGetTsId(), MAX_UINT32_NUM, false, logicCqId_, isFastCq, false);
    ...
    ERROR_RETURN_MSG_INNER(
        error, "Failed to initialize shmCq_, device_id=%u, retCode=%#x.", device_->Id_(), static_cast<uint32_t>(error));
...
rtError_t DirectHwtsEngine::Start()
{
    ...
    threadName = (device_->DevGetTsId() == static_cast<uint32_t>(RT_TSC_ID)) ? "MONITOR_0" : "MONITOR_1";
    monitorThread_ = OsalFactory::CreateThread(threadName, this, monitor);
    COND_GOTO_MSG_OUTER(
        monitorThread_ == nullptr, ERROR_RETURN, error, RT_ERROR_MEMORY_ALLOCATION, ErrorCode::EE1013,
...
void DirectHwtsEngine::SyncTaskQueryShm(const uint32_t streamId, const uint32_t taskId, const uint32_t cqId)
{
    ...
    RT_LOG(RT_LOG_INFO, "Task Wait: stream_id=%u, task_id=%u, cq_id=%u, exec_id=%u.", streamId, taskId, cqId, execId);
}
...
void DirectHwtsEngine::IsSyncFinish(const uint32_t taskId, uint32_t cnt, const rtLogicReport_t* logicReport) const
{
    ...
    for (uint32_t idx = 0U; idx < cnt; ++idx) {
        ...
        RT_LOG(
            RT_LOG_DEBUG, "Notify: count=%u, idx=%u, stream_id=%hu, task_id=%u, type=%u.", cnt, idx, report.streamId,
            taskIdVar, static_cast<uint32_t>(logicCqTypeVar));
...
rtError_t DirectHwtsEngine::SyncTask(
    Stream* const stm, const uint32_t taskId, const bool isStreamSync, int32_t timeout, bool isForce)
{
    ...
    COND_RETURN_INFO(
        (isForce && (stm->GetPendingNum() == 0)), RT_ERROR_NONE, "finish: stream_id=%d, task_id=%u, cq_id=%u.",
    ...
    while (!isNotified) {
        ...
        if (isFastCq && (error == RT_ERROR_NONE)) {
            RT_LOG(
                RT_LOG_INFO, "Task Wait: get fast cq interrupt, stream_id=%d, task_id=%u, cq_id=%u, retCode=%#x",
        ...
        COND_PROC_RETURN_ERROR_MSG_CALL(
            ERR_MODULE_DRV, error == RT_ERROR_DRV_IOCTRL, RT_ERROR_DRV_IOCTRL,
            RT_LOG_INNER_DETAIL_MSG(RT_DRV_INNER_ERROR, {"device_id"}, {std::to_string(device_->Id_())});
            , "Device[%u] Ioctrl fail.", device_->Id_());

        COND_PROC_RETURN_ERROR_MSG_CALL(
            ERR_MODULE_DRV, error == RT_ERROR_LOST_HEARTBEAT, RT_ERROR_LOST_HEARTBEAT,
            RT_LOG_INNER_DETAIL_MSG(RT_DRV_INNER_ERROR, {"device_id"}, {std::to_string(device_->Id_())});
            , "Device[%u] lost heartbeat.", device_->Id_());

        if (unlikely(
                (pendingNum_.Value() == 0U) && (error == RT_ERROR_REPORT_TIMEOUT) && (stm->GetPendingNum() == 0))) {
            RT_LOG(
                RT_LOG_EVENT, "May recycle by other thread: stream_id=%u, task_id=%u, cqId=%u", streamId, taskId, cqId);
            return RT_ERROR_NONE;
        }
        if (timeout > 0) {
        ...
        if (stm->isDeviceSyncFlag && (stm->GetFailureMode() != ABORT_ON_FAILURE)) {
            ...
            COND_RETURN_ERROR(
                (timeout > 0) && (remainTime == 0) && (cnt == 0U), RT_ERROR_STREAM_SYNC_TIMEOUT,
                "Failed to synchronize task. Task wait timeout, timeout=%dms, remainTime=%dms, cnt=%u.", timeout,
                remainTime, cnt);
        }
        ...
        if (unlikely(
                ...
                (cnt == 0U))) {
            RT_LOG(
                RT_LOG_INFO, "Task Wait: stream_id=%d, task_id=%u, cqId=%u, retCode=%#x", streamId, taskId, cqId,
                error);
        ...
        for (uint32_t idx = 0U; idx < cnt; ++idx) {
            ...
            RT_LOG(
                RT_LOG_INFO,
                "Notify: count=%u, idx=%u, stream_id=%hu, task_id=%u, type=%u, retCode=%#x,"
                " payLoad=%#x, drvRetCode=%#x.",
                cnt, idx, report.streamId, taskIdVar, static_cast<uint32_t>(logicCqTypeVar), report.errorCode,
            ...
            COND_PROC((report.streamId != streamId),
                      RT_LOG(RT_LOG_ERROR, "reportStmId=%hu waitStmId=%u, cqId=%u.", report.streamId, streamId, cqId);
                      continue);
...
rtError_t DirectHwtsEngine::TryRecycleTask(Stream* const stm)
{
    ...
    if ((lastTaskId % TASK_QUERY_INTERVAL_NUM) == 0U) {
        ...
        COND_RETURN_ERROR_MSG_INNER(
            error != RT_ERROR_NONE, error, "Failed to query task status, retCode=%#x.", static_cast<uint32_t>(error));
...
bool DirectHwtsEngine::ProcLogicReport(const rtLogicReport_t& logic, const uint32_t drvErr, const bool isStreamSync)
{
    ...
    if (logicCqTypeVal == static_cast<uint8_t>(LOGIC_RPT_OVERFLOW_ERROR)) {
        RT_LOG(
            RT_LOG_INFO, "Overflow message received, cq_id=%hu, device_id=%u, ts_id=%u", logic.logicCqId, devId, tsId);
    ...
    if (logicCqTypeVal == static_cast<uint8_t>(LOGIC_RPT_TASK_ERROR)) { // Task error
        ...
        (logicCqTypeVal == static_cast<uint8_t>(LOGIC_RPT_MODEL_ERROR)) ||
...
void DirectHwtsEngine::MonitoringRun()
{
    ...
    while ((!terminal) && (GetDevRunningState() == static_cast<uint32_t>(DEV_RUNNING_NORMAL)) &&
        ...
        LOST_HEARTBEAT_PROC_RETURN(
            ret, std::to_string(device_->Id_()), Runtime::Instance()->MonitorNumSub(1U),
        ...
        for (uint32_t idx = 0U; idx < cnt; ++idx) {
            ...
            RT_LOG(
                RT_LOG_INFO,
                "Notify: count=%u, idx=%u, stream_id=%hu, task_id=%hu, type=%u, retCode=%#x, "
                "payLoad=%#x, ts_id=%u, retCode=%#x.",
                cnt, idx, report.streamId, report.taskId, static_cast<uint32_t>(logicCqType), report.errorCode,
                report.payLoad, tsId, static_cast<uint32_t>(error));
...
rtError_t DirectHwtsEngine::SubmitSend(TaskInfo* const workTask, uint32_t* const flipTaskId)
{
    ...
    COND_RETURN_ERROR(
        error != RT_ERROR_NONE, error, "Failed to send task, stream_id=%d, task_id=%hu, retCode=%#x.", stm->Id_(),
        workTask->id, error);
...
void DirectHwtsEngine::TaskReclaimEx(
    const uint32_t streamId, const bool limited, uint32_t& taskId, rtShmQuery_t& shareMemInfo)
{
    if (shareMemInfo.valid != SQ_SHARE_MEMORY_VALID) {
        taskId = UINT32_MAX;
        return;
...
bool DirectHwtsEngine::HandleShmTask(const uint32_t streamId, const bool limited, rtShmQuery_t& shareMemInfo)
{
    ...
    if (lastInfo != nullptr) {
        ...
        if (TaskIdIsGEQ(lastTaskId, taskIdInMem)) { // no new task.
            RT_LOG(
                RT_LOG_DEBUG, "handle share memory task, no new task. lastInfo: task_id=%u, shmInfo: task_id=%u.",
                lastInfo->taskId, shareMemInfo.taskId);
            return false;
        }

        const bool limitedInner =
            (taskSuccess && limited &&
...
bool DirectHwtsEngine::UpdateTaskIdForTaskStatus(const uint32_t streamId, const uint16_t taskId)
{
    ...
    if (reportTask == nullptr) {
        RT_LOG(
            RT_LOG_WARNING, "Get null task, is already recycle. device_id=%u, stream_id=%u, task_id=%hu",
    ...
    if (shmInfo != nullptr) {
        RT_LOG(
            RT_LOG_DEBUG, "UpdateTaskIdForTaskStatus, stream_id=%u, task_id=%hu, last_task_id=%u.", streamId, taskId,
            shmInfo->taskId);
        const uint32_t lastTaskId = shmInfo->taskId;
...
void DirectHwtsEngine::ProcAicpuErrMsgReport(const rtLogicReport_t& logic) const
{
    ...
    (void)memset_s(&(exceptionInfo.expandInfo), sizeof(exceptionInfo.expandInfo), 0, sizeof(exceptionInfo.expandInfo));
    ...
    RT_LOG_INNER_MSG(
        RT_LOG_ERROR, "aicpu model retCode=%#x,[%s], task_id=%u, stream_id=%u, kernel_name=%s.",
        static_cast<uint32_t>(rtErrCode), retDes, exceptionInfo.taskid, exceptionInfo.streamid, kernelNameStr.c_str());
...
void DirectHwtsEngine::ReportLastError(
...
bool DirectHwtsEngine::ProcessReport(const rtTaskReport_t* const report, const uint32_t drvErr, const bool isStreamSync)
{
    const uint16_t streamId = static_cast<uint16_t>(
        static_cast<uint32_t>(report->streamID) |
    ...
    if (likely(reportTask != nullptr)) {
        RT_LOG(
            RT_LOG_DEBUG,
            "RTS_DRIVER: report receive, device_id=%u, ts_id=%u, stream_id=%hu, task_id=%hu, "
            "task_type=%d(%s)",
            device_->Id_(), device_->DevGetTsId(), streamId, report->taskID, static_cast<int32_t>(reportTask->type),
            reportTask->typeName);
        parseTaskCount_++;
        if (unlikely(reportCount_ != parseTaskCount_)) {
            RT_LOG(
                RT_LOG_WARNING,
                "parse task from report stream_id=%hu task_id=%hu, "
                "task_type=%d(%s), report_count=%" PRIu64 " parse_task_count=%" PRIu64 " failed",
                streamId, reportTask->id, static_cast<int32_t>(reportTask->type), reportTask->typeName, reportCount_,
                parseTaskCount_);
        }
        ...
        RT_LOG(
            RT_LOG_ERROR, "Failed to get task from report stream, stream_id=%hu, task_id=%hu.", streamId,
            report->taskID);
    }
...
bool DirectHwtsEngine::PreProcessTask(TaskInfo* preTask, const uint32_t deviceId)
{
    ...
    do {
        ...
        if (error != RT_ERROR_NONE) {
            RT_LOG(
                RT_LOG_ERROR,
                "Failed to delete task, stream_id=%d, tail_task_id=%hu, "
                "del_task_id=%hu, retCode=%#x.",
                streamId, endTaskId, preRecycleTaskId, error);
            return false;
        }
        ...
        if (preTask == nullptr) {
            RT_LOG(
                RT_LOG_ERROR, "Failed to find task from task factory, stream_id=%d, task_id=%hu.", streamId,
                preRecycleTaskId);
            return false;
        }
