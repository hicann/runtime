    ...
    void* binHandle, const void* hostVar, const char* deviceVarName, size_t size, uint32_t flags)
{
    ...
    if (it != symbolMap_.end()) {
        ...
        RT_LOG(
            RT_LOG_WARNING, "Symbol hostVar=%p, binHandle=%p already registered, skip duplicate registration.", hostVar,
            binHandle);
    ...
    RT_LOG(
        RT_LOG_DEBUG, "Register symbol success, hostVar=%p, deviceVarName=%s, offset=%llu, size=%llu", hostVar,
        deviceVarName, entry.deviceSymbolOffset, entry.deviceSymbolSize);
    return RT_ERROR_NONE;
}
...
rtError_t SymbolTable::Lookup(const void* hostVar, SymbolEntry& entry)
{
    ...
    if (it == symbolMap_.end()) {
        ...
        RT_LOG_OUTER_MSG_IMPL(
            ErrorCode::EE1017, "Looking up symbol table", "symbol",
...
rtError_t SymbolTable::GetDeviceAddress(const void* hostVar, uint32_t deviceId, void** devPtr)
{
    ...
    COND_RETURN_ERROR_MSG_INNER(
        baseAddr == nullptr, RT_ERROR_PROGRAM_DATA, "binary not loaded to device, deviceId=%u", deviceId);

    *devPtr = RtValueToPtr<void*>(RtPtrToValue(baseAddr) + entry.deviceSymbolOffset);
    RT_LOG(RT_LOG_DEBUG, "Get symbol device address, hostVar=%p, deviceId=%u, devPtr=%p", hostVar, deviceId, *devPtr);
    return RT_ERROR_NONE;
}
