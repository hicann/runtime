...
rtError_t IpcEvent::IpcVaAndPaOperation(size_t granularity, rtDrvMemProp_t* prop, uint64_t* deviceMemHandle)
{
    ...
    COND_RETURN_ERROR(
        error != RT_ERROR_NONE, error, "Failed to setup ipc event. Reason: ReserveMemAddress failed, error=%#x.",
    ...
    COND_RETURN_ERROR(
        error != RT_ERROR_NONE, error, "Failed to setup ipc event. Reason: MallocPhysical failed, error=%#x.",
        static_cast<uint32_t>(error));
    ...
    COND_RETURN_ERROR(
        error != RT_ERROR_NONE, error, "Failed to setup ipc event. Reason: MapMem failed, error=%#x.",
        static_cast<uint32_t>(error));
    mapFlag_ |= DEV_MEM_MAP_FLAG;
    error = device_->Driver_()->MemSetSync(deviceMemVa, p2pSize, 0U, p2pSize);
    COND_RETURN_ERROR(
        error != RT_ERROR_NONE, error, "Failed to setup ipc event. Reason: MemSetSync failed, error=%#x.",
        static_cast<uint32_t>(error));
    // 1.5 halMemExportToShareableHandle pa for handle, SetMemShareHandleDisablePidVerify close whitelist
    error = NpuDriver::ExportToShareableHandle(
        deviceMemPa, RT_MEM_HANDLE_TYPE_NONE, IPC_EVENT_P2P_NO_CHECK_FLAG, deviceMemHandle);
    COND_RETURN_ERROR(
        error != RT_ERROR_NONE, error, "Failed to setup ipc event. Reason: ExportToShareableHandle failed, error=%#x.",
        static_cast<uint32_t>(error));
    error = NpuDriver::SetMemShareHandleDisablePidVerify(*deviceMemHandle);
    COND_RETURN_ERROR(
        error != RT_ERROR_NONE, error,
        "Failed to setup ipc event. Reason: SetMemShareHandleDisablePidVerify failed, error=%#x.",
        static_cast<uint32_t>(error));
    ...
    COND_RETURN_ERROR(
        error != RT_ERROR_NONE, error, "Failed to setup ipc event. Reason: MemSetAccess failed, error=%#x.",
        static_cast<uint32_t>(error));
...
rtError_t IpcEvent::IpcHandleAllocAndExport(size_t granularity, rtDrvMemProp_t* prop, uint64_t* ipcHandle)
{
    ...
    COND_RETURN_ERROR(
        error != RT_ERROR_NONE, error, "Failed to allocate ipc handle. Reason: ReserveMemAddress failed, error=%#x.",
        static_cast<uint32_t>(error));
    ...
    COND_RETURN_ERROR(
        error != RT_ERROR_NONE, error, "Failed to allocate ipc handle. Reason: MallocPhysical failed, error=%#x.",
        static_cast<uint32_t>(error));
    ...
    COND_RETURN_ERROR(
        error != RT_ERROR_NONE, error, "Failed to allocate ipc handle. Reason: MapMem failed, error=%#x.",
        static_cast<uint32_t>(error));
    mapFlag_ |= IPC_HANDLE_MAP_FLAG;
    error = device_->Driver_()->MemSetSync(ipcHandleVa, ipcHandleSize, 0U, ipcHandleSize);
    COND_RETURN_ERROR(
        error != RT_ERROR_NONE, error, "Failed to allocate ipc handle. Reason: MemSetSync failed, error=%#x.",
        static_cast<uint32_t>(error));
    ...
    error = NpuDriver::ExportToShareableHandle(
        ipcHandlePa, RT_MEM_HANDLE_TYPE_NONE, IPC_EVENT_P2P_NO_CHECK_FLAG, ipcHandle);

    COND_RETURN_ERROR(
        error != RT_ERROR_NONE, error,
        "Failed to allocate ipc handle. Reason: ExportToShareableHandle failed, error=%#x.",
        static_cast<uint32_t>(error));
    error = NpuDriver::SetMemShareHandleDisablePidVerify(*ipcHandle);
    COND_RETURN_ERROR(
        error != RT_ERROR_NONE, error,
        "Failed to allocate ipc handle. Reason: SetMemShareHandleDisablePidVerify failed, error=%#x.",
        static_cast<uint32_t>(error));
    return error;
}
...
rtError_t IpcEvent::Setup()
{
    ...
    COND_RETURN_ERROR(
        error != RT_ERROR_NONE, error, "Failed to setup ipc event. Reason: GetAllocationGranularity failed, error=%#x.",
        static_cast<uint32_t>(error));
    RT_LOG(RT_LOG_INFO, "GetAllocationGranularity success.");
    std::function<void()> const recycleFunc = [this]() { ReleaseDrvResource(); };
    ScopeGuard errRecycle(recycleFunc);

    // p2p proc
    error = IpcVaAndPaOperation(granularity, &prop, &deviceMemHandle);
    COND_RETURN_ERROR(
        error != RT_ERROR_NONE, error, "Failed to setup ipc event. Reason: IpcVaAndPaOperation failed, error=%#x.",
        static_cast<uint32_t>(error));

    // share handle proc
    error = IpcHandleAllocAndExport(granularity, &prop, &ipcHandle);
    COND_RETURN_ERROR(
        error != RT_ERROR_NONE, error, "Failed to setup ipc event. Reason: IpcHandleAllocAndExport failed, error=%#x.",
        static_cast<uint32_t>(error));
...
rtError_t IpcEvent::IpcHandleAllocAndImport(size_t granularity, rtIpcEventHandle_t* ipcEventHandle)
{
    ...
    rtError_t error = NpuDriver::ReserveMemAddress(&ipcHandleVa, ipcHandleSize, 0, nullptr, 0);
    COND_RETURN_ERROR(
        error != RT_ERROR_NONE, error, "Failed to open ipc event. Reason: ReserveMemAddress failed, error=%#x.",
        static_cast<uint32_t>(error));
    ipcHandleVa_ = RtPtrToPtr<IpcHandleVa*>(ipcHandleVa);
    ...
    COND_RETURN_ERROR(
        error != RT_ERROR_NONE, error, "Failed to open ipc event. Reason: ImportFromShareableHandle failed, error=%#x.",
        static_cast<uint32_t>(error));
    ipcHandlePa_ = ipcHandlePa;
    ...
    error = NpuDriver::MapMem(ipcHandleVa, ipcHandleSize, 0, ipcHandlePa, 0);
    COND_RETURN_ERROR(
        error != RT_ERROR_NONE, error, "Failed to open ipc event. Reason: MapMem failed, error=%#x.",
        static_cast<uint32_t>(error));
    mapFlag_ |= IPC_HANDLE_MAP_FLAG;
...
rtError_t IpcEvent::EnableP2PForIpc(uint64_t deviceMemHandle) const
{
    ...
    COND_RETURN_ERROR(
        error != RT_ERROR_NONE, error,
        "Failed to enable p2p for ipc. Reason: GetPhyDevIdByMemShareHandle failed, error=%#x.",
        static_cast<uint32_t>(error));
    // enable p2p
...
rtError_t IpcEvent::IpcMemHandleImport(size_t granularity, bool hostFlag, uint64_t deviceMemHandle)
{
    ...
    COND_RETURN_ERROR(
        error != RT_ERROR_NONE, error,
        "Failed to import ipc memory handle. Reason: ReserveMemAddress failed, error=%#x.",
        static_cast<uint32_t>(error));
    if (hostFlag) {
    ...
    COND_RETURN_ERROR(
        error != RT_ERROR_NONE, error,
        "Failed to import ipc memory handle. Reason: ImportFromShareableHandle failed, error=%#x.",
        static_cast<uint32_t>(error));
    if (hostFlag) {
        hostMemPa_ = MemPa;
    } else {
    ...
    COND_RETURN_ERROR(
        error != RT_ERROR_NONE, error, "Failed to import ipc memory handle. Reason: MapMem failed, error=%#x.",
        static_cast<uint32_t>(error));
    if (hostFlag) {
...
rtError_t IpcEvent::IpcOpenEventHandle(rtIpcEventHandle_t* ipcEventHandle)
{
    ...
    error = NpuDriver::GetAllocationGranularity(&prop, RT_MEM_ALLOC_GRANULARITY_MINIMUM, &granularity);
    COND_RETURN_ERROR(
        error != RT_ERROR_NONE, error, "Failed to open ipc event. Reason: GetAllocationGranularity failed, error=%#x.",
        static_cast<uint32_t>(error));
    ...
    COND_RETURN_ERROR(
        error != RT_ERROR_NONE, error, "Failed to open ipc event. Reason: IpcHandleAllocAndImport failed, error=%#x.",
        static_cast<uint32_t>(error));
    ...
    COND_RETURN_ERROR(
        error != RT_ERROR_NONE, error, "Failed to open ipc event. Reason: EnableP2PForIpc failed, error=%#x.",
        static_cast<uint32_t>(error));

    // host import
    error = IpcMemHandleImport(granularity, true, deviceMemHandle);
    COND_RETURN_ERROR(
        error != RT_ERROR_NONE, error, "Failed to open ipc event. Reason: IpcMemHandleImport failed, error=%#x.",
        static_cast<uint32_t>(error));
...
rtError_t IpcEvent::IpcEventRecord(Stream* const stm)
{
    ...
    COND_RETURN_ERROR_MSG_INNER(
        (tsk == nullptr), errorReason, "Failed to allocate task for ipc record, stream_id=%d.", stm->Id_());
    uint16_t curIndex = 0U;
    error = GetIpcRecordIndex(&curIndex);
    COND_PROC_RETURN_ERROR(
        error != RT_ERROR_NONE, error, (void)dev->GetTaskFactory()->Recycle(tsk),
    ...
    ERROR_RETURN_MSG_INNER(
        error, "Failed to initialize mem write value task, stream_id=%d, task_id=%hu, retCode=%#x.", stm->Id_(),
    ...
    eventStatus_ = INIT;
    RT_LOG(
        RT_LOG_INFO, "ipc event task submit success, device_id=%u, stream_id=%d, task_id=%hu, event_id=%u", dev->Id_(),
        stm->Id_(), tsk->id, curIndex);
    return error;
}
...
rtError_t IpcEvent::IpcEventWait(Stream* const stm)
{
    ...
    COND_PROC_RETURN_ERROR_MSG_CALL(ERR_MODULE_SYSTEM, tsk == nullptr, errorReason, IpcVaLock();
                                    ipcHandleVa_->deviceMemRef[ipcHandleVa_->currentIndex]--;
                                    IpcVaUnLock(), "Failed to allocate task for ipc wait, retCode=%#x.",
    ...
    ERROR_RETURN_MSG_INNER(
        error, "Failed to initialize mem wait value task, stream_id=%d, task_id=%hu, retCode=%#x.", stm->Id_(), tsk->id,
        static_cast<uint32_t>(error));
    ...
    GET_THREAD_TASKID_AND_STREAMID(tsk, stm->AllocTaskStreamId());
    RT_LOG(
        RT_LOG_INFO, "ipc wait task submit success, device_id=%u, stream_id=%d, task_id=%hu, event_id=%u",
...
rtError_t IpcEvent::IpcEventQuery(rtEventStatus_t* const status)
{
    ...
    ERROR_RETURN(
        error, "Failed to query ipc event status. Reason: context is abort, status=%#x.", static_cast<uint32_t>(error));
    IpcVaLock();
...
rtError_t IpcEvent::IpcEventSync(int32_t timeout)
{
    rtError_t error = context_->CheckStatus();
    ERROR_RETURN(
        error, "Failed to synchronize ipc event. Reason: context is abort, status=%#x.", static_cast<uint32_t>(error));
    IpcVaLock();
    ...
    while ((*hostaddr == 0U) && !IsIpcFinished()) {
        error = context_->CheckStatus();
        ERROR_RETURN(
            error, "Failed to synchronize ipc event. Reason: context is abort, status=%#x.",
            static_cast<uint32_t>(error));
        queryTimes++;
        if (queryTimes == 1000U) {
            (void)mmSleep(100U);
        ...
        COND_RETURN_ERROR(
            (IsProcessTimeout(beginTimeSpec, timeout)), RT_ERROR_EVENT_SYNC_TIMEOUT,
            "event synchronize timeout, device_id=%u, timeout=%dms.", device_->Id_(), timeout);
    }
