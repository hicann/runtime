...
public:
    ...
    rtError_t BinaryGetFunction(
        const Program* const prog, const uint64_t tilingKey, Kernel** const funcHandle) override;
    rtError_t BinaryLoadWithoutTilingKey(const void* data, const uint64_t length, Program** const prog) override;
    rtError_t BinaryGetFunctionByName(
    ...
    rtError_t FunctionGetMetaInfo(
        const Kernel* const funcHandle, const rtFunctionMetaType type, void* data, const uint32_t length) override;
    rtError_t FunctionGetMetaInfoSize(
        const Kernel* const funcHandle, const rtFunctionMetaType type, size_t* size) override;
    rtError_t GetFunctionBySymbol(const void* symbol, Kernel** const funcHandle) override;
    rtError_t RegisterFuncSymbol(
        void* const binHandle, const void* const symbol, const char_t* const kernelName) override;
    rtError_t RegisterCpuFunc(
    ...
    rtError_t BinaryLoadFromFile(
        const char_t* const binPath, const rtLoadBinaryConfig_t* const optionalCfg, Program** handle) override;
    ...
    rtError_t GetAddrAndPrefCntWithHandle(
        void* const hdl, const void* const kernelInfoExt, void** const addr, uint32_t* const prefetchCnt) override;
    rtError_t KernelGetAddrAndPrefCnt(
        void* const hdl, const uint64_t tilingKey, const void* const stubFunc, const uint32_t flag, void** const addr,
        uint32_t* const prefetchCnt) override;
    rtError_t KernelGetAddrAndPrefCntV2(
        void* const hdl, const uint64_t tilingKey, const void* const stubFunc, const uint32_t flag,
        rtKernelDetailInfo_t* const kernelInfo) override;
    rtError_t QueryFunctionRegistered(const char_t* const stubName) override;
    rtError_t KernelLaunch(
    ...
    rtError_t CpuKernelLaunchExWithArgs(
    ...
    rtError_t MultipleTaskInfoLaunch(
    ...
    rtError_t AppendLaunchHostInfo(
        rtLaunchArgs_t* const hdl, size_t const hostInfoSize, void** const hostInfo) override;
    rtError_t CalcLaunchArgsSize(
        size_t const argsSize, size_t const hostInfoTotalSize, size_t hostInfoNum,
        size_t* const launchArgsSize) override;
    rtError_t CreateLaunchArgs(
        size_t const argsSize, size_t const hostInfoTotalSize, size_t hostInfoNum, void* const argsData,
        rtLaunchArgs_t** const argsHandle) override;
    ...
    rtError_t GetBinBuffer(
        const rtBinHandle binHandle, const rtBinBufferType_t type, void** bin, uint32_t* binSize) override;
    rtError_t GetStackBuffer(
        const rtBinHandle binHandle, uint32_t deviceId, const uint32_t stackType, const uint32_t coreType,
        const uint32_t coreId, const void** stack, uint32_t* stackSize) override;
    ...
    rtError_t StreamBeginCapture(
        Stream* const stm, const rtStreamCaptureMode mode, Model* const mdl = nullptr) override;
    rtError_t StreamEndCapture(Stream* const stm, Model** const captureMdl) override;
    rtError_t StreamGetCaptureInfo(
        const Stream* const stm, rtStreamCaptureStatus* const status, Model** const captureMdl) override;
    ...
    rtError_t MemCopySyncEx(
        void* const dst, const uint64_t destMax, const void* const src, const uint64_t cnt,
    ...
    rtError_t LaunchSqeUpdateTask(
        uint32_t streamId, uint32_t taskId, void* src, uint64_t cnt, Stream* const stm,
        bool needCpuTask = false) override;
    ...
    rtError_t MemGetInfoByDeviceId(
        uint32_t deviceId, bool isHugeOnly, size_t* const freeSize, size_t* const totalSize) override;
    ...
    rtError_t CheckMemType(
        void** addrs, uint32_t size, uint32_t memType, uint32_t* checkResult, uint32_t reserve) override;
    ...
    rtError_t MemGetInfoEx(const rtMemInfoType_t memInfoType, size_t* const freeSize, size_t* const totalSize) override;
    ...
    rtError_t MemCopy2DSync(
        ...
        const rtMemcpyKind newKind = RT_MEMCPY_KIND_MAX) override;
    ...
    rtError_t RtsMemcpy(
        void* const dst, const uint64_t destMax, const void* const src, const uint64_t cnt, const rtMemcpyKind kind,
        rtMemcpyConfig_t* const config) override;
    ...
    rtError_t MemManagedPrefetchAsync(
        const void* ptr, size_t size, rtMemManagedLocation location, uint32_t flags, Stream* const stream) override;
    ...
    rtError_t GetDeviceInfo(
        const uint32_t deviceId, const int32_t moduleType, const int32_t infoType, int64_t* const val) override;
    rtError_t GetDeviceInfoFromPlatformInfo(
        const uint32_t deviceId, const std::string& label, const std::string& key, int64_t* const value) override;
    ...
    rtError_t GetDeviceCapability(
        const int32_t deviceId, const int32_t moduleType, const int32_t featureType, int32_t* const val) override;
    rtError_t ModelCheckArchVersion(const char_t* omsocVersion) override;
    rtError_t CheckArchCompatibility(
        const char_t* socVersion, const char_t* omSocVersion, int32_t* canCompatible) override;
    rtError_t DeviceStatusQuery(const uint32_t devId, rtDeviceStatus* deviceStatus) override;
    rtError_t GetFaultEvent(
    ...
    rtError_t HdcSessionConnect(
        const int32_t peerNode, const int32_t peerDevId, rtHdcClient_t const client,
    ...
    rtError_t ProfilerTrace(const uint64_t id, const bool notifyFlag, const uint32_t flags, Stream* const stm) override;
    rtError_t ProfilerTraceEx(
        const uint64_t id, const uint64_t modelId, const uint16_t tagId, Stream* const stm) override;
    ...
    rtError_t GetOnlineProfData(
        Stream* const stm, rtProfDataInfo_t* const pProfData, const uint32_t profDataNum) override;
    ...
    // inquire information
    rtError_t GetMaxStreamAndTask(
        const uint32_t streamType, uint32_t* const maxStrCount, uint32_t* const maxTaskCount) override;
    ...
    rtError_t ModelExecute(Model* const mdl, Stream* const stm, const uint32_t flag, int32_t timeout = -1) override;
    ...
    rtError_t ModelCondHandleCreate(
        Model* const mdl, uint32_t defaultValue, rtCondHandleFlag_t flag, CondHandle** const handle) override;
    ...
    rtError_t CntNotifyWaitWithTimeout(
        CountNotify* const inCntNotify, Stream* const stm, const rtCntNtyWaitInfo_t* const info) override;
    rtError_t CntNotifyReset(CountNotify* const inCntNotify, Stream* const stm) override;
    rtError_t GetCntNotifyAddress(
    ...
    rtError_t RegDeviceStateCallback(
        ...
        rtDevCallBackDir_t notifyPos = DEV_CB_POS_BACK) override;
    rtError_t RegTaskFailCallbackByModule(
        const char_t* regName, void* callback, void* args, TaskFailCallbackType type) override;
    rtError_t RegProfCtrlCallback(const uint32_t moduleId, const rtProfCtrlHandle callback) override;
    rtError_t RegStreamStateCallback(
    ...
    rtError_t CallbackLaunch(
        const rtCallback_t callBackFunc, void* const fnData, Stream* const stm, const bool isBlock) override;
    ...
    // stream label func
    rtError_t LabelSwitchByIndex(
        void* const ptr, const uint32_t maxVal, void* const labelInfoPtr, Stream* const stm) override;
    rtError_t LabelGotoEx(Label* const lbl, Stream* const stm) override;
    rtError_t LabelListCpy(
        Label** const lbl, const uint32_t labelNumber, void* const dst, const uint32_t dstMax) override;
    ...
    void SetImpl(Api* const impl) { ApiDecorator::impl_ = impl; }
    ...
    rtError_t FftsPlusTaskLaunch(
        const rtFftsPlusTaskInfo_t* const fftsPlusTaskInfo, Stream* const stm, const uint32_t flag) override;
    ...
    rtError_t NpuGetFloatDebugStatus(
        void* const outputAddrPtr, const uint64_t outputSize, const uint32_t checkMode, Stream* const stm) override;
    ...
    rtError_t MemQueueCreate(const int32_t devId, const rtMemQueueAttr_t* const queAttr, uint32_t* const qid) override;
    rtError_t MemQueueExport(
        const int32_t devId, const uint32_t qid, const int32_t peerDevId, const char* const shareName) override;
    rtError_t MemQueueUnExport(
        const int32_t devId, const uint32_t qid, const int32_t peerDevId, const char* const shareName) override;
    rtError_t MemQueueImport(
        const int32_t devId, const int32_t peerDevId, const char* const shareName, uint32_t* const qid) override;
    rtError_t MemQueueUnImport(
        const int32_t devId, const uint32_t qid, const int32_t peerDevId, const char* const shareName) override;

    rtError_t MemQueueSet(
        const int32_t devId, const rtMemQueueSetCmdType cmd, const rtMemQueueSetInputPara* const input) override;
    ...
    rtError_t MemQueuePeek(
        const int32_t devId, const uint32_t qid, size_t* const bufLen, const int32_t timeout) override;
    rtError_t MemQueueEnQueueBuff(
        const int32_t devId, const uint32_t qid, rtMemQueueBuff_t* const inBuf, const int32_t timeout) override;
    rtError_t MemQueueDeQueueBuff(
        const int32_t devId, const uint32_t qid, rtMemQueueBuff_t* const outBuf, const int32_t timeout) override;
    rtError_t MemQueueQueryInfo(
        const int32_t devId, const uint32_t qid, rtMemQueueInfo_t* const queryQueueInfo) override;
    rtError_t MemQueueQuery(
    ...
    rtError_t MemGrpCacheAlloc(
        const char_t* const name, const int32_t devId, const rtMemGrpCacheAllocPara* const para) override;
    rtError_t BuffGetInfo(
        const rtBuffGetCmdType type, const void* const inBuff, const uint32_t inLen, void* const outBuff,
        uint32_t* const outLen) override;
    rtError_t MemGrpAddProc(
        const char_t* const name, const int32_t pid, const rtMemGrpShareAttr_t* const attr) override;
    ...
    rtError_t DvppWaitGroupReport(
        DvppGrp* const grp, const rtDvppGrpCallback callBackFunc, const int32_t timeout) override;
...
private:
    ...
    rtError_t GetLocationType(
        ...
        rtMemLocationType& srcRealLocation, rtMemLocationType& dstLocationType,
        rtMemLocationType& dstRealLocation) const;
    rtError_t MemcpyAsyncCheckExLocation(
        bool checkKind, const rtMemcpyKind_t kind, const void* const src, const void* const dst) const;
    rtError_t MemcpyAsyncCheckLocation(
    ...
    rtError_t MemcpyKindAutoCorrect(
    ...
    rtError_t KernelTaskGetAttribute(
...
struct hash<rtMemcpyKind> {
    std::size_t operator()(const rtMemcpyKind k) const noexcept { return static_cast<size_t>(k); }
};
} // namespace std
