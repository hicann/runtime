...
rtError_t OnlineProf::OnlineProfMalloc(Stream* const stm)
{
    ...
    if (!dev->IsSupportFeature(RtOptionalFeatureType::RT_FEATURE_PROFILING_ONLINE_DEVICE_MEM_CLEAR)) {
        ...
        ERROR_GOTO_MSG_INNER(
            ret, ERROR_FREE,
            "Failed to synchronize memset on device memory, "
            "size=%" PRIu64 ", retCode=%#x.",
            memSize, static_cast<uint32_t>(ret));
...
ERROR_FREE:
    ...
    ERROR_RETURN_MSG_INNER(
        ...
        "retCode=%#x.",
        static_cast<uint32_t>(freeRet1));
    ERROR_RETURN_MSG_INNER(
        freeRet2,
        "Failed to free online profiling host runtime memory, "
        "retCode=%#x.",
        static_cast<uint32_t>(freeRet2));
    ERROR_RETURN_MSG_INNER(
        freeRet3,
        "Failed to free online profiling host task scheduler memory, "
        "retCode=%#x.",
        static_cast<uint32_t>(freeRet3));
    return ret;
}
...
rtError_t OnlineProf::GetOnlineProfilingData(
    const Stream* const stm, rtProfDataInfo_t* const pProfData, const uint32_t profDataNum)
{
    ...
    uint64_t* const rtWriteAddr =
        RtValueToPtr<uint64_t*>(onlineProfAddr + (ONLINEPROF_HEAD_SIZE / 2U)); // 2:half head size
    rtError_t error = stm->Device_()->Driver_()->MemCopySync(
    ...
    uint64_t* const tsWriteAddr = RtValueToPtr<uint64_t*>(onlineProfAddr + (ONLINEPROF_HEAD_SIZE / 2U));

    if (*tsWriteAddr != *rtWriteAddr) {
        RT_LOG(
            RT_LOG_WARNING, "ts write index: %" PRIu64 " is not same as rt write index %" PRIu64, *tsWriteAddr,
            *rtWriteAddr);
    ...
    if (*tsReadAddr != *rtReadAddr) {
        RT_LOG(
            RT_LOG_WARNING, "ts read index: %" PRIu64 " is not same as rt read index %" PRIu64, *tsReadAddr,
            *rtReadAddr);
    ...
    for (uint32_t profDataIndex = 0U; profDataIndex < profDataNum; profDataIndex++) {
        ...
        if (readIndex == *rtWriteAddr) { /* should not happen */
            RT_LOG_INNER_MSG(
                RT_LOG_ERROR, "Failed to read data, current index=%u, need read num=%u.", profDataIndex, profDataNum);
            break;
        }
        ...
        if (stm != rtData.stream) { /* need or not? If yes, need return? */
            RT_LOG_INNER_MSG(
                RT_LOG_ERROR, "Failed to match stream, kernel stream is different from input stream, id=%d.",
                stm->Id_());
        }
        ...
        RT_LOG(
            RT_LOG_DEBUG, "[ts write index=%" PRIu64 "] totalcycle=%" PRIu64 ", ovcycle=%" PRIu64, readIndex,
    ...
    *tsReadAddr = *rtReadAddr;
    ...
    error = stm->Device_()->Driver_()->MemCopySync(
        deviceMem, ONLINEPROF_MEM_SIZE, hostTsMem, ONLINEPROF_MEM_SIZE, RT_MEMCPY_HOST_TO_DEVICE);
