...
void ProfCtrlCallbackManager::NotifyOneModule(
    const uint32_t moduleId, const uint32_t dataType, void* const data, const uint32_t dataLen)
{
...
rtError_t ProfCtrlCallbackManager::RegProfCtrlCallback(const uint32_t moduleId, const rtProfCtrlHandle callback)
{
    ...
    RT_LOG(
        RT_LOG_DEBUG, "%s profiling callback finish, moduleId:%u.", ((callback == nullptr) ? "unregister" : "register"),
        moduleId);
