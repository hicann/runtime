...
public:
    ...
    rtError_t LaunchKernel(
        ...
        const rtTaskCfgInfo_t* const cfgInfo) override;
    ...
    rtError_t DvppWaitGroupReport(
        DvppGrp* const grp, rtDvppGrpCallback const callBackFunc, const int32_t timeout) override;
...
private:
    ...
    rtError_t GetCaptureEvent(
        const Stream* const stm, Event* const evt, Event** const captureEvt, const bool isNewEvt = false) override;
    rtError_t CaptureWaitEvent(Context* const ctx, Stream* const stm, Event* const evt, const uint32_t timeout);
    rtError_t Starsv2CaptureExternalEventRecord(Event* const evt, Stream* const stm) const;
    rtError_t Starsv2CaptureExternalEventWait(Event* const evt, Stream* const stm) const;
    rtError_t LaunchKernelByArgsWithType(
