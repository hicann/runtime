...
void ProfileManager::InitProfileManager(const uint32_t deviceId)
{
    ...
    aicpuFeatureUseErrorLogThreshold_ =
        (bqs::GetRunContext() == bqs::RunContext::HOST) ? false : QSFeatureCtrl::UseErrorLogThreshold(deviceId);
    const float64_t logThresholdTickTemp = aicpuFeatureUseErrorLogThreshold_ ?
                                               (ERROR_LOG_THRESHOLD_MS * oneMsForTickTemp) :
                                               (EVENT_TIME_COST_THRESHOLD_MS * oneMsForTickTemp);
...
void ProfileManager::SetSrcQueueNum(const uint32_t srcQueueNum) { enqueEventTrack_.srcQueueNum = srcQueueNum; }

void ProfileManager::AddEnqueueNum() { enqueEventTrack_.enqueueNum++; }

void ProfileManager::AddDequeueNum() { enqueEventTrack_.dequeueNum++; }

void ProfileManager::AddCopyTotalCost(const uint64_t copyCost) { enqueEventTrack_.copyCost += copyCost; }

void ProfileManager::SetRelationCost(const uint64_t relationCost) { enqueEventTrack_.relationCost = relationCost; }

void ProfileManager::Setf2NFCost(const uint64_t f2NFCost) { enqueEventTrack_.f2NFCost = f2NFCost; }
...
void ProfileManager::AddHcclImprobeCost(const uint64_t cost)
{
    ...
    recvReqEventTrack_.maxHcclImprobeCost =
        (recvReqEventTrack_.maxHcclImprobeCost > cost) ? recvReqEventTrack_.maxHcclImprobeCost : cost;
}
...
void ProfileManager::AddHcclGetCountCost(const uint64_t cost)
{
    ...
    recvReqEventTrack_.maxHcclGetCountCost =
        (recvReqEventTrack_.maxHcclGetCountCost > cost) ? recvReqEventTrack_.maxHcclGetCountCost : cost;
}
...
void ProfileManager::AddHcclImrecvCost(const uint64_t cost)
{
    ...
    recvReqEventTrack_.maxHcclImrecvCost =
        (recvReqEventTrack_.maxHcclImrecvCost > cost) ? recvReqEventTrack_.maxHcclImrecvCost : cost;

    schedInfoTrack_.hcclImrecvNum++;
    schedInfoTrack_.totalHcclImrecvCost += cost;
    schedInfoTrack_.maxHcclImrecvCost =
        (schedInfoTrack_.maxHcclImrecvCost > cost) ? schedInfoTrack_.maxHcclImrecvCost : cost;
}
...
void ProfileManager::AddHcclTestSomeCost(const uint64_t cost, const bool isRecvCompEvent)
{
    if (isRecvCompEvent) {
        ...
        recvCompEventTrack_.maxHcclTestSomeCost =
            (recvCompEventTrack_.maxHcclTestSomeCost > cost) ? recvCompEventTrack_.maxHcclTestSomeCost : cost;
    } else {
        sendCompEventTrack_.hcclTestSomeNum++;
        sendCompEventTrack_.totalHcclTestSomeCost += cost;
        sendCompEventTrack_.maxHcclTestSomeCost =
            (sendCompEventTrack_.maxHcclTestSomeCost > cost) ? sendCompEventTrack_.maxHcclTestSomeCost : cost;
    }
}
...
const float64_t ProfileManager::AddReqProcCompCost(const uint64_t cost, const bool isRecvCompEvent)
{
    if (isRecvCompEvent) {
        ...
        recvCompEventTrack_.maxReqProcCompCost =
            (recvCompEventTrack_.maxReqProcCompCost > cost) ? recvCompEventTrack_.maxReqProcCompCost : cost;
    } else {
        sendCompEventTrack_.reqProcCompNum++;
        sendCompEventTrack_.totalReqProcCompCost += cost;
        sendCompEventTrack_.maxReqProcCompCost =
            (sendCompEventTrack_.maxReqProcCompCost > cost) ? sendCompEventTrack_.maxReqProcCompCost : cost;
    }
...
void ProfileManager::AddHcclIsendCost(const uint64_t cost)
{
    ...
    schedInfoTrack_.maxHcclIsendCost =
        (schedInfoTrack_.maxHcclIsendCost > cost) ? schedInfoTrack_.maxHcclIsendCost : cost;
}
...
void ProfileManager::AddMbufAllocCost(const uint64_t cost)
{
    ...
    recvReqEventTrack_.maxMbufAllocCost =
        (recvReqEventTrack_.maxMbufAllocCost > cost) ? recvReqEventTrack_.maxMbufAllocCost : cost;
}
...
void ProfileManager::AddHcclEnqueueCost(const uint64_t cost)
{
    ...
    recvCompEventTrack_.maxEnqueueCost =
        (recvCompEventTrack_.maxEnqueueCost > cost) ? recvCompEventTrack_.maxEnqueueCost : cost;
}
...
void ProfileManager::AddMbufFreeCost(const uint64_t cost)
{
    ...
    sendCompEventTrack_.maxMbufFreeCost =
        (sendCompEventTrack_.maxMbufFreeCost > cost) ? sendCompEventTrack_.maxMbufFreeCost : cost;
}
...
void ProfileManager::DoMarkerForRecvReqEvent(const uint64_t startTick)
{
    ...
    if (profMode_ == bqs::ProfilingMode::PROFILING_OPEN) {
        ...
        const float64_t avgHcclImprobeCost =
            (hcclImprobeNum == 0UL) ? 0.0 : (totalHcclImprobeCost / static_cast<float64_t>(hcclImprobeNum));
        ...
        const float64_t avgHcclGetCountCost =
            (hcclGetCountNum == 0UL) ? 0.0 : (totalHcclGetCountCost / static_cast<float64_t>(hcclGetCountNum));
        ...
        const float64_t avgHcclImrecvCost =
            (hcclImrecvNum == 0UL) ? 0.0 : (totalHcclImrecvCost / static_cast<float64_t>(hcclImrecvNum));
        ...
        const float64_t avgMbufAllocCost =
            (mbufAllocNum == 0UL) ? 0.0 : (totalMbufAllocCost / static_cast<float64_t>(mbufAllocNum));
        const float64_t maxMbufAllocCost = static_cast<float64_t>(recvReqEventTrack_.maxMbufAllocCost) / oneUsForTick_;

        BQS_LOG_RUN_INFO(
            "Hccl time cost info: {recv request event, count[%lu], "
            ...
            count, recvReqEventTrack_.schedTimes, recvReqEventTrack_.schedDelay, totalCost, startTick, hcclImprobeNum,
            totalHcclImprobeCost, avgHcclImprobeCost, maxHcclImprobeCost, hcclGetCountNum, totalHcclGetCountCost,
            avgHcclGetCountCost, maxHcclGetCountCost, hcclImrecvNum, totalHcclImrecvCost, avgHcclImrecvCost,
            maxHcclImrecvCost, mbufAllocNum, totalMbufAllocCost, avgMbufAllocCost, maxMbufAllocCost);
    }
}
...
void ProfileManager::DoMarkerForRecvCompEvent(const uint64_t startTick)
{
    ...
    if (profMode_ == bqs::ProfilingMode::PROFILING_OPEN) {
        ...
        const float64_t avgHcclTestSomeCost =
            (hcclTestSomeNum == 0UL) ? 0.0 : (totalHcclTestSomeCost / static_cast<float64_t>(hcclTestSomeNum));
        ...
        const float64_t avgEnqueueCost =
            (enqueueNum == 0UL) ? 0.0 : (totalEnqueueCost / static_cast<float64_t>(enqueueNum));
        ...
        const float64_t avgReqProcCompCost =
            (reqProcCompNum == 0UL) ? 0.0 : (totalReqProcCompCost / static_cast<float64_t>(reqProcCompNum));
        ...
        const float64_t avgSuccTestSomeOneReqCost =
            (reqProcCompNum == 0UL) ? 0.0 : (totalHcclTestSomeCost / static_cast<float64_t>(reqProcCompNum));

        BQS_LOG_RUN_INFO(
            "Hccl time cost info: {recv completion event, count[%lu], "
            "schedTimes[%lu], schedDelay[%lu]ticks, totalCost[%.2f]us, startStamp[%lu], "
            ...
            hcclTestSomeNum, totalHcclTestSomeCost, avgHcclTestSomeCost, maxHcclTestSomeCost, enqueueNum,
            totalEnqueueCost, avgEnqueueCost, maxEnqueueCost, reqProcCompNum, totalReqProcCompCost, avgReqProcCompCost,
            maxReqProcCompCost, reqProcCompNum, totalHcclTestSomeCost, avgSuccTestSomeOneReqCost);
    }
}
...
void ProfileManager::DoMarkerForSendCompEvent(const uint64_t startTick)
{
    ...
    if (profMode_ == bqs::ProfilingMode::PROFILING_OPEN) {
        ...
        const float64_t avgMbufFreeCost =
            (mbufFreeNum == 0UL) ? 0.0 : (totalMbufFreeCost / static_cast<float64_t>(mbufFreeNum));
        ...
        BQS_LOG_RUN_INFO(
            "Hccl time cost info: {send completion event, count[%lu], "
            "schedTimes[%lu], schedDelay[%lu]ticks, totalCost[%.2f]us, startStamp[%lu], "
            "hcclTestSome[count:%lu, total:%.2fus, average:%.2fus, max:%.2fus], "
            ...
            hcclTestSomeNum, totalHcclTestSomeCost, avgHcclTestSomeCost, maxHcclTestSomeCost, mbufFreeNum,
            totalMbufFreeCost, avgMbufFreeCost, maxMbufFreeCost, reqProcCompNum, totalReqProcCompCost,
            avgReqProcCompCost, maxReqProcCompCost, reqProcCompNum, totalHcclTestSomeCost, avgSuccTestSomeOneReqCost);
    }
}
...
void ProfileManager::DoMarker()
{
#ifndef USE_PROFILER
    BQS_LOG_INFO(
        "Time cost info: {event:%u, state:%u, schedTimes:%lu, schedDelay:%lu ticks, "
    ...
    if (profMode_ == bqs::ProfilingMode::PROFILING_OPEN) {
        ...
        const float64_t totalHcclIsendCost = static_cast<float64_t>(schedInfoTrack_.totalHcclIsendCost) / oneUsForTick_;
        const uint64_t hcclIsendNum = schedInfoTrack_.hcclIsendNum;
        const float64_t avgHcclIsendCost =
            (hcclIsendNum == 0U) ? 0.0 : totalHcclIsendCost / static_cast<float64_t>(hcclIsendNum);
        const float64_t maxHcclIsendCost = static_cast<float64_t>(schedInfoTrack_.maxHcclIsendCost) / oneUsForTick_;
        ...
        const float64_t avgHcclImrecvCost =
            (hcclImrecvNum == 0U) ? 0.0 : totalHcclImrecvCost / static_cast<float64_t>(hcclImrecvNum);
        const float64_t maxHcclImrecvCost = static_cast<float64_t>(schedInfoTrack_.maxHcclImrecvCost) / oneUsForTick_;
        ...
        const float64_t avgRecvReqEventDelay =
            (recvReqEventCount_ == 0U) ?
                0.0 :
        ...
        const float64_t avgRecvCompEventDelay =
            (recvCompEventCount_ == 0U) ?
                0.0 :
        ...
        BQS_LOG_RUN_INFO(
            ...
            count, hcclIsendNum, totalHcclIsendCost, avgHcclIsendCost, maxHcclIsendCost, hcclImrecvNum,
            totalHcclImrecvCost, avgHcclImrecvCost, maxHcclImrecvCost, maxRecvReqEventDelay, avgRecvReqEventDelay,
            minRecvReqEventDelay, maxRecvCompEventDelay, avgRecvCompEventDelay, minRecvCompEventDelay,
...
{
    BQS_LOG_RUN_INFO(
        "DoErrorLog:Time out info:{event:%u, state:%u, schedTimes:%lu, schedDelay:%lu ticks, "
        "startStamp:%lu, dequeueNum:%lu, enqueueNum:%lu, "
        "copyCost:%lu ticks, fullQueueNum:%u, srcQueueNum:%u, "
        "relationCost:%lu ticks, f2NFCost:%lu ticks, totalCost:%lu ticks}",
        enqueEventTrack_.event, enqueEventTrack_.state, enqueEventTrack_.schedTimes, enqueEventTrack_.schedDelay,
...
ProfilingMode ProfileManager::GetProfilingMode() const { return profMode_; }
