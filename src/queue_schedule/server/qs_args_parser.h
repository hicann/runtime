...
public:
    ArgsParser()
        : deviceId_(0U),
          ...
          vfId_(0U),
          logLevel_(ERROR_LOG),
          eventLevel_(EVENT_LOG),
          aicpulogLevel_(-1),
          deployMode_(QueueSchedulerRunMode::SINGLE_PROCESS),
          reschedInterval_(RESCHED_INTERVAL_DEFAULT),
          groupName_(""),
          schedPolicy_(0U),
          starter_(QsStartType::START_BY_TSD),
          profCfgData_(""),
          abnormalInterval_(ABNORMAL_INTERVAL_DEFAULT),
          profFlag_(false),
          withDeviceId_(false),
          withHostPid_(false),
          withPidSign_(false),
          withVfId_(false),
          withLogLevel_(false),
          withGroupName_(false),
          withStarter_(false),
          argsParseFuncMap_(
              {{PARAM_DEVICEID, &ArgsParser::ParseDeviceId},
    ...
    inline uint32_t GetHostPid() const { return hostPid_; }

    inline std::string GetPidSign() const { return pidSign_; }

    inline uint32_t GetVfId() const { return vfId_; }

    inline int32_t GetLogLevel() const { return logLevel_; }

    inline int32_t GetEventLevel() const { return eventLevel_; }

    inline int32_t GetAicpuLogLevel() const { return aicpulogLevel_; }

    inline QueueSchedulerRunMode GetDeployMode() const { return deployMode_; }

    inline int32_t GetReschedInterval() const { return reschedInterval_; }

    inline std::string GetGroupName() const { return groupName_; }

    inline uint64_t GetSchedPolicy() const { return schedPolicy_; }

    inline QsStartType GetStarter() const { return starter_; }

    inline std::string GetProfCfgData() const { return profCfgData_; }

    inline bool GetProfFlag() const { return profFlag_; }

    inline int32_t GetAbnormalInterval() const { return abnormalInterval_; }

    inline bool GetWithDeviceId() const { return withDeviceId_; }

    inline bool GetWithHostPid() const { return withHostPid_; }

    inline bool GetWithPidSign() const { return withPidSign_; }

    inline bool GetWithVfId() const { return withVfId_; }

    inline bool GetWithLogLevel() const { return withLogLevel_; }

    inline bool GetWithGroupName() const { return withGroupName_; }

    inline bool GetWithStarter() const { return withStarter_; }

    inline std::vector<uint32_t> GetResvec() const { return resVec_; }

    inline std::vector<uint32_t> GetDevIdVec() const { return devIdVec_; }
