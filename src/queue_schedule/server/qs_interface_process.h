...
public:
     ...
     * @return AICPU_SCHEDULE_OK: success, other: error code
     */
    const std::vector<uint32_t>& GetCtrlCpuIds() const { return ctrlCpuIds_; }

    const std::vector<uint32_t>& GetAiCpuIds() const { return aiCpuIds_; }
    ...
    uint32_t GetAiCpuNum() const { return aicpuNum_; }

    uint32_t GetExtraAiCpuNum() const { return aicpuNumExtra_; }
