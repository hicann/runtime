...
bool QsProcMemStatistic::GetXsMemInfoFromFile(uint64_t& xsMemValue)
{
    ...
    if (ret < XSMEM_VALUE_CNT) {
        BQS_LOG_WARN(
            "read summaryStr:%s failed, ret:%d, allocsize:%lu, real size:%lu", summaryStr.c_str(), ret, allocSize,
            realSize);
        return false;
    }
...
void QsProcMemStatistic::PrintOutProcMemInfo(const uint32_t hostPid)
{
    ...
    if (runDevice_) {
        ...
        BQS_LOG_RUN_INFO(
            "proc_metrics:pid=%u, rssavg=%lu B, rsshwm=%lu B, xsmemavg=%lu B, xsmemhwm=%lu B, "
            "svmmemavg=%lu B, svmmemhwm=%lu B",
            hostPid, rssavg * BYTE_TO_KBYTE, rssMem_.memHwm * BYTE_TO_KBYTE, xsmAvg, xsMem_.memHwm, svmAvg,
            svmMem_.memHwm);
    } else {
        BQS_LOG_RUN_INFO(
            "proc_metrics:pid=%u, rssavg=%lu B, rsshwm=%lu B, xsmemavg=%lu B, xsmemhwm=%lu B", hostPid,
