...
void DynamicSchedMgr::DeleteQueue(const uint32_t globalLogicId, const uint32_t rootModelId)
{
    ...
    for (auto iter = rootModelInfos_.begin(); iter != rootModelInfos_.end(); iter++) {
        if ((iter->first == rootModelId) && (iter->second.responseQue.globalLogicId == globalLogicId)) {
...
void DynamicSchedMgr::UpdateNodeId(const int32_t nodeId) { nodeId_ = nodeId; }

void DynamicSchedMgr::GenerateRequest(
...
FsmStatus DynamicSchedMgr::SendRequest(const uint32_t rootModelId, const std::vector<RequestInfo>& requests)
{
    // get root model info
    const auto iter = rootModelInfos_.find(rootModelId);
    if (iter == rootModelInfos_.end()) {
    ...
    const auto enqueueRet =
        EnqueueRequest(flowgwRequest, iter->second.requestQue.deviceId, iter->second.requestQue.queueId);
...
FsmStatus DynamicSchedMgr::EnqueueRequest(
...
void DynamicSchedMgr::PrintRequestLog(const dynamic::FlowgwRequest& flowgwRequest) const
{
    ...
    for (int32_t queue_infos_index = 0; queue_infos_index < queue_infos_size; queue_infos_index++) {
        ...
        DGW_LOG_INFO(
            "Dynamic sched send request, node_id=%d, input_index=%d, queue_id=%u, device_type=%d, "
            "device_id=%d, logic_id=%u, logic_group_id=%u, model_uuid=%u, trans_id=%lu, route_label=%u, "
            "root_model_id=%u, queue_infos_index=%d.",
            flowgwRequest.node_id(), flowgwRequest.input_index(), queue_info.queue_attrs().queue_id(),
            queue_info.queue_attrs().device_type(), queue_info.queue_attrs().device_id(),
            queue_info.queue_attrs().logic_id(), queue_info.logic_group_id(), queue_info.model_uuid(),
            queue_info.trans_id(), queue_info.route_label(), queue_info.root_model_id(), queue_infos_index);
    }
}
...
void DynamicSchedMgr::PrintResponseLog(const dynamic::FlowgwResponse& flowgwResponse) const
{
    ...
    for (int32_t queue_infos_index = 0; queue_infos_index < queue_infos_size; queue_infos_index++) {
        ...
        DGW_LOG_INFO(
            "Dynamic sched get response, queue_id=%u, device_type=%d, "
            "device_id=%d, logic_id=%u, logic_group_id=%u, model_uuid=%u, trans_id=%lu, route_label=%u, "
            ...
            queue_info.queue_attrs().device_id(), queue_info.queue_attrs().logic_id(), queue_info.logic_group_id(),
            queue_info.model_uuid(), queue_info.trans_id(), queue_info.route_label(), queue_info.choose_logic_id(),
            queue_info.root_model_id(), queue_infos_index, static_cast<int32_t>(queue_info.need_cache()));
    }
}

FsmStatus DynamicSchedMgr::GetResponse(const uint32_t rootModelId, std::vector<ResponseInfo>& responses)
{
    // get root model info
    const auto iter = rootModelInfos_.find(rootModelId);
    if (iter == rootModelInfos_.end()) {
    ...
    if (ret == DRV_ERROR_NONE) {
        ...
        auto mbufDeleter = [](Mbuf* buf) { (void)halMbufFree(buf); };
        std::unique_ptr<Mbuf, decltype(mbufDeleter)> mbufGuard(PtrToPtr<void, Mbuf>(mbuf), mbufDeleter);
        ...
        DGW_LOG_ERROR(
            "failed to dequeue, device_id = %u, queue_id = %u, ret = %d", iter->second.responseQue.deviceId,
            iter->second.responseQue.queueId, ret);
        return FsmStatus::FSM_FAILED;
    }
    GetResponseFromCacheResult(responses);
    return FsmStatus::FSM_SUCCESS;
...
void DynamicSchedMgr::SendRequestToCacheResult(
...
void DynamicSchedMgr::GetResponseFromCacheResult(std::vector<ResponseInfo>& responses)
{
    for (auto& cacheInfo : validCacheInfos_) {
        for (uint32_t index = 0U; index < cacheInfo.second.num; index++) {
            ...
            DGW_LOG_INFO(
                "get response from cache result, root_model_id=%u, src queue_id=%u, "
                "logic_id=%u, logic_group_id=%u, result_index=%u.",
                cacheInfo.first.srcQueueInfo.rootModelId, cacheInfo.first.srcQueueInfo.queueId,
                cacheInfo.first.srcQueueInfo.queueLogicId, cacheInfo.second.result.logicGroupId,
                cacheInfo.second.result.index);
        }
...
{
    BQS_LOG_RUN_INFO(
        "DynamicSched, flowgw data: Total(us)=%lu, Cnt=%lu, Per duration(ns)=%lu, Max duration(ns)=%lu,"
        " Greater 2ms cnt=%lu",
        durationTotal_ / kMicrosecondToNanosecond, cntTotal_, (durationTotal_ / (cntTotal_ != 0ULL ? cntTotal_ : 1UL)),
        durationMax_, durationSize_);
    durationTotal_ = 0ULL;
    cntTotal_ = 0ULL;
    durationMax_ = 0ULL;
    durationSize_ = 0ULL;
