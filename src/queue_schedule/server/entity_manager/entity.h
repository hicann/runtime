...
public:
    ...
    inline void AddScheduleCount() { scheduleCount_++; }
    inline void ResetScheduleCount() { scheduleCount_ = 0U; }
    inline uint32_t GetScheduleCount() const { return scheduleCount_; }
    inline EntityType GetType() const { return type_; }
    inline uint32_t GetId() const { return id_; }
    inline Mbuf* GetMbuf() const { return mbuf_; }
    inline void SetMbuf(Mbuf* const mbuf) { mbuf_ = mbuf; }
    inline uint32_t GetDeviceId() const { return deviceId_; }
    inline DataObjList& GetRecvDataObjs() { return recvDataObjs_; }
    inline const DataObjList& GetSendDataObjs() const { return sendDataObjs_; }
    inline uint32_t GetRefCount() const { return refCount_; }
    inline void DecreaseRefCount() { refCount_--; }
    inline void IncreaseRefCount() { refCount_++; }
    inline EntityDirection GetDirection() const { return direction_; }
    inline FsmState GetCurState() const { return curState_; }
    inline uint64_t GetTransId() const { return transId_; }
    inline void SetTransId(const uint64_t transId) { transId_ = transId; }
    inline uint32_t GetRouteLabel() const { return routeLabel_; }
    inline void SetRouteLabel(const uint32_t routeLabel) { routeLabel_ = routeLabel; }

    inline uint32_t GetQueueType() const { return queueType_; }

    inline int32_t GetHostGroupId() const { return hostGroupId_; }
    inline void SetNeedTransId(const bool needTransId) { needTransId_ = needTransId; }
    inline bool IsNeedTransId() const { return needTransId_; }
    inline void SetMessageType(const InnerMsgType msgType) { msgType_ = msgType; }
    inline InnerMsgType GetMessageType() const { return msgType_; }

    inline const std::string& ToString() const { return entityDesc_; }
    inline bqs::EntityStatisticInfo& GetStatisticInfo() { return statInfo_; }
    inline void SetWaitDecisionState(const bool waitState) { waitingDecision_ = waitState; }
    inline bool GetWaitDecisionState() const { return waitingDecision_; }
    inline uint32_t GetGlobalId() const { return globalId_; }

    inline uint32_t GetUuId() const { return uuId_; }

    inline uint32_t GetSchedCfgKey() const { return schedCfgKey_; }

    inline uint32_t GetResIndex() const { return resIndex_; }

    inline void SetDynamicReqTime(const uint64_t dynamicReqTime) { dynamicReqTime_ = dynamicReqTime; }

    inline uint64_t GetDynamicReqTime() const { return dynamicReqTime_; }
