...
EntityPtr EntityManager::DoGetEntity(
...
EntityPtr EntityManager::GetEntityById(
    const uint32_t queueType, const uint32_t deviceId, const EntityType eType, const uint32_t id,
    const EntityDirection direction)
{
    ...
    if (entity != nullptr) {
        DGW_LOG_DEBUG(
            "Direction:[%s] type:[%s] id:[%u] state:[%s] queue id:[%u] "
            "device id:[%u] owner deviceid:[%u] is found",
            GetDirectionDesc(entity->GetDirection()), entity->GetTypeDesc().c_str(), id,
            entity->GetStateDesc(entity->GetCurState()).c_str(), entity->GetQueueId(), deviceId, entity->GetDeviceId());
    } else {
        DGW_LOG_WARN(
            "Type:[%s] id:[%u], device id:[%u], queueType:[%u], direction[%d] does not exist",
            StateManager::Instance().GetTypeDesc(eType).c_str(), id, deviceId, queueType,
...
EntityPtr EntityManager::CreateEntity(const EntityMaterial& material)
{
    ...
    if (entity != nullptr) {
        DGW_LOG_INFO(
            "Type:[%s] id:[%u], direction:[%d] is existed", entity->GetTypeDesc().c_str(), material.id,
    ...
    if (entity == nullptr) {
        DGW_LOG_ERROR(
            "Create [%s] entity failed type:[%s] id:[%u].", GetDirectionDesc(material.direction),
    ...
    DGW_LOG_INFO(
        "Create entity[%s] success, state:[%s]", entity->ToString().c_str(),
...
FsmStatus EntityManager::DeleteEntity(
    const uint32_t queueType, const uint32_t deviceId, const EntityType eType, const uint32_t id,
    const EntityDirection direction)
{
    EntityPtr entity = DoGetEntity(queueType, deviceId, eType, id, direction);
    if (entity == nullptr) {
        DGW_LOG_WARN(
            "Failed to find entity for queueType[%u], deviceId[%u], type[%s], id:[%u], direction:[%d]", queueType,
            deviceId, StateManager::Instance().GetTypeDesc(eType).c_str(), id, static_cast<int32_t>(direction));
        return FsmStatus::FSM_SUCCESS;
    }
    ...
    DGW_LOG_INFO(
        "Success to delete entity[%s], direction:[%s].", entity->ToString().c_str(), GetDirectionDesc(direction));
    if (!dgw::ScheduleConfig::GetInstance().GetSchedKeys().empty() && (eType == dgw::EntityType::ENTITY_QUEUE)) {
    ...
    DGW_LOG_INFO(
        "Finish to delete entity[%s], direction:[%s].", entity->ToString().c_str(), GetDirectionDesc(direction));
    (void)entity->Uninit();
    return FsmStatus::FSM_SUCCESS;
}

void EntityManager::CleanEntityMap(
    const uint32_t queueType, const uint32_t deviceId, const EntityType eType, const uint32_t id)
{
...
FsmStatus EntityManager::SupplyRecvRequestEvent() { return SupplyEventForRecvRequest(EVENT_RECV_REQUEST_MSG); }

FsmStatus EntityManager::SupplyOneTrackEvent() { return SupplyEventForRecvRequest(EVENT_RECV_COMPLETION_MSG); }

FsmStatus EntityManager::SupplyEventForRecvRequest(uint32_t msgType)
{
    const auto checkFunc = [](const ChannelEntityPtr entity) -> bool { return entity->CheckRecvReqEventContinue(); };
...
FsmStatus EntityManager::EraseCommChannel(const EntityPtr& entity, const bool isSrc)
{
    ...
    DGW_LOG_INFO(
        "Success to erase comm channel entity, entity:[%s], isSrc:[%d] comm channels size:[%zu].",
...
FsmStatus EntityManager::InsertCommChannel(const ChannelEntityPtr& entity, const bool isSrc)
{
    ...
    DGW_LOG_INFO(
        "Success to save comm channel entity:[%s], isSrc:[%d] comm channels size:[%zu].", entity->ToString().c_str(),
        static_cast<int32_t>(isSrc), channels.entities.size());
    return FsmStatus::FSM_SUCCESS;
}
...
CommChannels& EntityManager::GetCommChannels(const bool isSrc) { return isSrc ? srcCommChannels_ : dstCommChannels_; }

FsmStatus EntityManager::SupplyEvent(const uint32_t eventId) const
{
    if (resIndex_ != 0U) {
        return SupplyEvent(
            eventId, bqs::QueueManager::GetInstance().GetExtraDeviceId(),
            static_cast<uint32_t>(bqs::EventGroupId::ENQUEUE_GROUP_ID_EXTRA));
    }
    return SupplyEvent(
        eventId, bqs::QueueManager::GetInstance().GetDeviceId(),
