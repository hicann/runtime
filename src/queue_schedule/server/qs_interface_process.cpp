...
int32_t QueueScheduleInterface::InitQueueScheduler(const InitQsParams& params)
{
    ...
    if (bqs::GetRunContext() != bqs::RunContext::HOST) {
        const auto initCpuInfoRet = BindCpuUtils::GetDevCpuInfo(
            params.deviceId, aiCpuIds_, ctrlCpuIds_, coreNumPerDev_, aicpuNum_, aicpuBaseId_);
    ...
    if (params.numaFlag) {
        const auto initCpuInfoRet = BindCpuUtils::GetDevCpuInfo(
            params.deviceIdExtra, aiCpuIdsExtra_, ctrlCpuIdsExtra_, coreNumPerDevExtra_, aicpuNumExtra_,
            aicpuBaseIdExtra_);
        if (initCpuInfoRet != BQS_STATUS_OK) {
    ...
    BQS_LOG_INFO(
        "Prepare bind process with host process, deviceId[%u], RunContext is [%u]", params.deviceId,
        static_cast<uint32_t>(bqs::GetRunContext()));
    // bind process with hostpid in process mode
    if ((bqs::GetRunContext() != bqs::RunContext::HOST) && (params.starter != bqs::QsStartType::START_BY_DEPLOYER) &&
