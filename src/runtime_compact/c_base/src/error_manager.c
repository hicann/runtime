...
static void InitErrorThreadKey(void) { pthread_key_create(&g_errorThreadKey, KeyDestructor); }
...
static char* FormatString(
    char* tmplArgList[MAX_ARG_NUMBER], char* tmplErrorMsg, char* args[], char* argValues[], int32_t argsNum)
...
static bool IsValidErrorCode(const char* errorCode) { return strlen(errorCode) == ERRCODE_LENGTH; }
...
static int32_t FormatTraceBefore(ErrorItem* firstItem)
{
    ...
    if (IsInnerErrorCode(firstItem->errorId)) {
        ...
        n = sprintf_s(
            g_errorThread->errorMsg, g_errorThread->errorMsgLen, "%s: %s\r\n", firstItem->errorId,
            firstItem->errorMessage);
        if (n < 0) {
            return -1;
        }
        if (firstItem->possibleCause != NULL) {
            nFirstCause = sprintf_s(
                g_errorThread->errorMsg + n, g_errorThread->errorMsgLen, "        PossibleCause: %s\r\n",
        ...
        if (firstItem->solution != NULL) {
            nFirstSolution = sprintf_s(
                g_errorThread->errorMsg + n, g_errorThread->errorMsgLen, "        Solution: %s\r\n",
                firstItem->solution);
            if (nFirstSolution < 0) {
                return -1;
            }
...
static int32_t FormatTraceAfter(ErrorItem* firstItem, size_t errorItemNum, int32_t offset)
{
    ...
    for (size_t i = 0; i < errorItemNum; i++) {
        ...
        if (!printTracebackOnce) {
            traceRet = sprintf_s(
                g_errorThread->errorMsg + n, g_errorThread->errorMsgLen, "%s\r\n",
