...
static inline uint32_t mmSetData(mmAtomicType* ptr, uint32_t value) { return __sync_lock_test_and_set(ptr, value); }

static inline uint64_t mmSetData64(mmAtomicType64* ptr, uint64_t value) { return __sync_lock_test_and_set(ptr, value); }

static inline uint32_t mmValueInc(mmAtomicType* ptr, uint32_t value) { return __sync_fetch_and_add(ptr, value); }
...
static inline void mmValueStore(mmAtomicType* ptr, uint32_t value) { __atomic_store(ptr, &value, __ATOMIC_SEQ_CST); }

static inline int32_t mmMutexInit(mmMutex_t* mutex) { return pthread_mutex_init(mutex, NULL); }

static inline int32_t mmMutexLock(mmMutex_t* mutex) { return pthread_mutex_lock(mutex); }

static inline int32_t mmMutexUnLock(mmMutex_t* mutex) { return pthread_mutex_unlock(mutex); }

static inline int32_t mmMutexDestroy(mmMutex_t* mutex) { return pthread_mutex_destroy(mutex); }

static inline void mmSchedYield(void) { (void)sched_yield(); }

static inline uint64_t mmGetTaskId(void) { return (uint64_t)(syscall(SYS_gettid)); }
...
static inline int32_t mmCloseFile(mmFileHandle* fd) { return fclose(fd); }
...
static inline int32_t mmAccess(const char* pathName, int32_t mode) { return access(pathName, mode); }
...
static inline long mmTellFile(mmFileHandle* fd) { return ftell(fd); }

static inline void* mmMalloc(unsigned long long size) { return malloc(size); }

static inline void mmFree(void* ptr) { free(ptr); }
