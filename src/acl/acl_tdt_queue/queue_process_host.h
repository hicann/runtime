...
public:
    ...
    aclError acltdtGetBufUserData(const acltdtBuf buf, void* dataPtr, const size_t size, const size_t offset) override;

    aclError acltdtSetBufUserData(acltdtBuf buf, const void* dataPtr, const size_t size, const size_t offset) override;

    aclError acltdtCopyBufRef(const acltdtBuf buf, acltdtBuf* const newBuf) override;

    aclError SendBindUnbindMsg(
        const int32_t deviceId, acltdtQueueRouteList* const qRouteList, const bool isBind,
        rtEschedEventSummary_t& eventSum, rtEschedEventReply_t& ack) const;

    aclError QueryQueueRoutes(
