...
enum PID_QUERY_TYPE : int32_t { CP_PID, QS_PID };
...
public:
    ...
    virtual aclError acltdtGrantQueue(
        const uint32_t qid, const int32_t pid, const uint32_t permission, const int32_t timeout);

    virtual aclError acltdtAttachQueue(const uint32_t qid, const int32_t timeout, uint32_t* const permission);
    ...
    virtual aclError acltdtQueryQueueRoutes(
        const acltdtQueueRouteQueryInfo* const queryInfo, acltdtQueueRouteList* const qRouteList) = 0;
    ...
    virtual aclError acltdtGetBufUserData(const acltdtBuf buf, void* dataPtr, const size_t size, const size_t offset);

    virtual aclError acltdtSetBufUserData(acltdtBuf buf, const void* dataPtr, const size_t size, const size_t offset);
    ...
    aclError SendBindUnbindMsgOnDevice(
    ...
    aclError GetDstInfo(
        const int32_t deviceId, const PID_QUERY_TYPE type, int32_t& dstPid, const bool isThreadMode = false) const;
    aclError GetQueuePermission(const int32_t deviceId, uint32_t qid, rtMemQueueShareAttr_t& permission) const;
    aclError GetQueueRouteNum(
    ...
    aclError QueryQueueRoutesOnDevice(
    ...
    virtual aclError acltdtEnqueueData(
    ...
    aclError acltdtCreateQueueWithAttr(
