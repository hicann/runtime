...
{
    ACL_LOG_INFO("Start to execute acltdtCreateQueue");
    ...
    ACL_LOG_INFO("Successfully executed acltdtCreateQueue, qid is %u", *qid);
    return ACL_SUCCESS;
}
...
aclError QueueProcessorHost::acltdtBindQueueRoutes(acltdtQueueRouteList* const qRouteList)
{
    ...
    ACL_LOG_INFO("Start to execute acltdtBindQueueRoutes, queue route is %zu", qRouteList->routeList.size());
    int32_t deviceId = 0;
    ACL_REQUIRES_RTS_OK_WARN_NOT_SUPPORT(rtGetDevice(&deviceId), rtGetDevice);
    ...
    ACL_LOG_INFO("Successfully executed acltdtBindQueueRoutes, queue route is %zu", qRouteList->routeList.size());
    return ACL_SUCCESS;
}
...
aclError QueueProcessorHost::acltdtQueryQueueRoutes(
    const acltdtQueueRouteQueryInfo* const queryInfo, acltdtQueueRouteList* const qRouteList)
{
    ACL_REQUIRES_NOT_NULL_WITH_INPUT_REPORT(queryInfo);
    ACL_REQUIRES_NOT_NULL_WITH_INPUT_REPORT(qRouteList);
    ACL_LOG_INFO("Start to execute acltdtQueryQueueRoutes");
    int32_t deviceId = 0;
    ACL_REQUIRES_RTS_OK_WARN_NOT_SUPPORT(rtGetDevice(&deviceId), rtGetDevice);
    // get dst id
    int32_t dstPid = 0;
...
aclError QueueProcessorHost::SendBindUnbindMsg(
    const int32_t deviceId, acltdtQueueRouteList* const qRouteList, const bool isBind, rtEschedEventSummary_t& eventSum,
    rtEschedEventReply_t& ack) const
{
    ...
    for (size_t i = 0UL; i < qRouteList->routeList.size(); ++i) {
        ...
        ret = rtMemcpy(
            (static_cast<uint8_t*>(devPtr) + offset), (routeSize - offset), &tmpRoute, sizeof(tmpRoute),
    ...
    for (size_t i = 0UL; i < qRouteList->routeList.size(); ++i) {
        ...
        ret = rtMemcpy(
            &retRoute, sizeof(retRoute), (static_cast<uint8_t*>(devPtr) + offset), sizeof(retRoute),
...
aclError QueueProcessorHost::QueryQueueRoutes(
    const int32_t deviceId, const acltdtQueueRouteQueryInfo* const queryInfo, const size_t routeNum,
    rtEschedEventSummary_t& eventSum, rtEschedEventReply_t& ack, acltdtQueueRouteList* const qRouteList) const
{
    ...
    ret = rtMemcpy(
        (static_cast<uint8_t*>(devPtr) + offset), (routeSize - offset), &routeQuery, sizeof(routeQuery),
        RT_MEMCPY_HOST_TO_DEVICE);
    if (ret != ACL_RT_SUCCESS) {
        (void)rtFree(devPtr);
        devPtr = nullptr;
...
aclError QueueProcessorHost::acltdtGetBufUserData(
...
aclError QueueProcessorHost::acltdtSetBufUserData(
...
aclError QueueProcessorHost::acltdtGetBufFromChain(const acltdtBuf headBuf, const uint32_t index, acltdtBuf* const buf)
{
    (void)(buf);
    (void)(headBuf);
    (void)(index);
