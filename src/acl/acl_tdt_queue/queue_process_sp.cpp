...
aclError QueueProcessorSp::acltdtDestroyQueue(const uint32_t qid) { return acltdtDestroyQueueOndevice(qid); }

aclError QueueProcessorSp::acltdtGrantQueue(
...
aclError QueueProcessorSp::acltdtBindQueueRoutes(acltdtQueueRouteList* const qRouteList)
{
    ...
    for (size_t i = 0UL; i < qRouteList->routeList.size(); ++i) {
        ...
        ACL_REQUIRES_RTS_OK_WARN_NOT_SUPPORT(
            rtMemQueueGrant(deviceId, qRouteList->routeList[i].srcId, dstPid, &attrSrc), rtMemQueueGrant);
        ACL_REQUIRES_RTS_OK_WARN_NOT_SUPPORT(
            rtMemQueueGrant(deviceId, qRouteList->routeList[i].dstId, dstPid, &attrDst), rtMemQueueGrant);
    }
...
aclError QueueProcessorSp::acltdtQueryQueueRoutes(
