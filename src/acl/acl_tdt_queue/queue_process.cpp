...
aclError QueueProcessor::acltdtGrantQueue(
...
aclError QueueProcessor::acltdtAttachQueue(const uint32_t qid, const int32_t timeout, uint32_t* const permission)
{
    (void)(qid);
    (void)(permission);
    (void)(timeout);
...
aclError QueueProcessor::GetQueuePermission(
    const int32_t deviceId, uint32_t qid, rtMemQueueShareAttr_t& permission) const
{
    uint32_t outLen = sizeof(permission);
    if (rtMemQueueQuery(deviceId, RT_MQ_QUERY_QUE_ATTR_OF_CUR_PROC, &qid, sizeof(qid), &permission, &outLen) !=
        RT_ERROR_NONE) {
        return ACL_ERROR_FAILURE;
    }
    return ACL_SUCCESS;
...
aclError QueueProcessor::GetDstInfo(
    const int32_t deviceId, const PID_QUERY_TYPE type, int32_t& dstPid, const bool isThreadMode) const
{
...
aclError QueueProcessor::SendBindUnbindMsgOnDevice(
    acltdtQueueRouteList* const qRouteList, const bool isBind, rtEschedEventSummary_t& eventSum,
    rtEschedEventReply_t& ack) const
{
    ...
    for (size_t i = 0UL; i < qRouteList->routeList.size(); ++i) {
        bqs::QueueRoute* const tmp = reinterpret_cast<bqs::QueueRoute*>(static_cast<uint8_t*>(devPtr) + offset);
    ...
    for (size_t i = 0UL; i < qRouteList->routeList.size(); ++i) {
        ...
        ACL_LOG_INFO(
            "route %zu, srcqid is %u, dst pid is %u, status is %d", i, qRouteList->routeList[i].srcId,
...
aclError QueueProcessor::GetQueueRouteNum(
    const acltdtQueueRouteQueryInfo* const queryInfo, const int32_t deviceId, rtEschedEventSummary_t& eventSum,
    rtEschedEventReply_t& ack, size_t& routeNum) const
{
...
aclError QueueProcessor::QueryQueueRoutesOnDevice(
    const acltdtQueueRouteQueryInfo* const queryInfo, const size_t routeNum, rtEschedEventSummary_t& eventSum,
    rtEschedEventReply_t& ack, acltdtQueueRouteList* const qRouteList) const
{
    ...
    const size_t routeSize =
        sizeof(bqs::QsRouteHead) + sizeof(bqs::QueueRouteQuery) + (routeNum * sizeof(bqs::QueueRoute));
    ACL_LOG_INFO("route size is %zu, queue route num is %zu", routeSize, qRouteList->routeList.size());
    void* devPtr = nullptr;
    void* mBuf = nullptr;
    ACL_REQUIRES_OK(AllocMBufOnDevice(&devPtr, &mBuf, routeSize));
...
aclError QueueProcessor::QueryAllocGroup()
{
    ...
    for (size_t num = 0U; num < grpNum; num++) {
        ...
        ACL_LOG_INFO("This proc [%d] has [%zu] group, alloc is %u, name is %s", pid, grpNum, alloc, grpName.c_str());
        if (alloc == 1U) {
...
aclError QueueProcessor::QueryGroupId(const std::string& grpName)
{
    ...
    const auto strcpyRet =
        strcpy_s(input.grpQueryGroupId.grpName, sizeof(input.grpQueryGroupId.grpName), grpName.c_str());
    if (strcpyRet != EOK) {
...
aclError QueueProcessor::acltdtFreeBuf(acltdtBuf buf) { return aclrtFreeBuf(buf); }
...
aclError QueueProcessor::acltdtGetBufUserData(
    const acltdtBuf buf, void* dataPtr, const size_t size, const size_t offset)
{
    return aclrtGetBufUserData(buf, dataPtr, size, offset);
}

aclError QueueProcessor::acltdtSetBufUserData(
    acltdtBuf buf, const void* dataPtr, const size_t size, const size_t offset)
{
...
uint64_t QueueProcessor::GetTimestamp() const
{
    ...
    const uint64_t totalUseTime = static_cast<size_t>(tv.tv_usec) + (static_cast<uint64_t>(tv.tv_sec) * 1000000UL);
    return totalUseTime;
}
...
aclError QueueProcessor::acltdtEnqueueData(
    const uint32_t qid, const void* const data, const size_t dataSize, const void* const userData,
    const size_t userDataSize, const int32_t timeout, const uint32_t rsv)
{
    ACL_LOG_INFO(
        "Start to enqueue data qid is %u, dataSize is %zu, userDataSize is %zu, "
        "timeout is %d, rsv is %u",
        qid, dataSize, userDataSize, timeout, rsv);
...
aclError QueueProcessor::acltdtDequeueData(
    const uint32_t qid, void* const data, const size_t dataSize, size_t* const retDataSize, void* const userData,
    const size_t userDataSize, const int32_t timeout)
{
    ACL_LOG_INFO(
        "Start to dequeue data qid is %u, dataSize is %zu, userDataSize is %zu, "
        "timeout is %d,",
        qid, dataSize, userDataSize, timeout);
    ACL_REQUIRES_NOT_NULL_WITH_INPUT_REPORT(data);
...
aclError QueueProcessor::acltdtCreateQueueWithAttr(
    const int32_t deviceId, const acltdtQueueAttr* const attr, uint32_t* const qid) const
{
    if (attr == nullptr) {
