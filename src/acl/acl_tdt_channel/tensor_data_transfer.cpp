...
const std::vector<size_t> GEAR_SIZE{
    1U * 1024U * 1024U, 10U * 1024U * 1024U, 100U * 1024U * 1024U, 500U * 1024U * 1024U};
...
void* GetHandler()
{
    ...
    if (handler == nullptr) {
        ACL_LOG_ERROR(
            "The corresponding dependent dynamic library cannot be found. "
            "Please confirm whether the environment supports it and if the extension package has been correctly "
            "installed. "
            "soName is %s.",
            soName.c_str());
    }
...
void* GetFunction(const std::string& func_name)
{
    ...
    if (func_ptr == nullptr) {
        ACL_LOG_ERROR(
            "The corresponding symbol cannot be found. Please confirm whether the installed extension package is "
            "correct, %s.",
            mmDlerror());
    }
    return func_ptr;
...
aclError TensorDatasetSerializesV2(const acltdtDataset* dataset, std::vector<acl::aclTdtDataItemInfo>& itemVec)
{
    ...
    for (size_t i = 0; i < dataset->blobs.size(); ++i) {
        ...
        itemVec.emplace_back(item);
        ACL_LOG_DEBUG(
            "TensorDatasetSerializesWithQueue, dataType %d, tensorType %d, dimNum %u, dataLen %lu",
    ...
    for (size_t i = 0; i < itemVec.size(); ++i) {
        ...
        if (aclType == ACL_TENSOR_DATA_TENSOR) {
            ...
            if (!GetTensorShape(itemVec[i].tensorShape_, dims)) {
                ACL_LOG_INNER_ERROR(
                    "[Check][TensorDataset]TensorDatasetDeserializes failed, "
                    "invalid tensor shape[%s]",
                    itemVec[i].tensorShape_.c_str());
                ret = ACL_ERROR_INTERNAL_ERROR;
                break;
            }

            std::map<std::string, aclDataType>::const_iterator iter = aclDataTypeStrMap.find(itemVec[i].tensorType_);
            if (iter == aclDataTypeStrMap.cend()) {
                ACL_LOG_INNER_ERROR(
                    "[Deserialize][TensorDataset]TensorDatasetDeserializes failed, "
                    "unknown data type[%s]",
            ...
            acltdtDataItem* item = new (std::nothrow) acltdtDataItem(
                aclType, nullptr, 0, itemVec[i].tensorShape_, ACL_DT_UNDEFINED, itemVec[i].tensorType_,
                itemVec[i].dataPtr_, itemVec[i].dataLen_);
            if (item == nullptr) {
...
aclError TensorDatasetDeserializesV2(const std::vector<acl::aclTdtDataItemInfo>& itemVec, acltdtDataset* dataset)
{
    ...
    for (size_t i = 0; i < itemVec.size(); ++i) {
        ...
        if (ret != ACL_SUCCESS) {
            ACL_LOG_INNER_ERROR(
                "[Check][Dataset]Failed to convert TdtDataType %d to acltdtTensorType.", itemVec[i].ctrlInfo.dataType);
            break;
        }
        if ((aclType == ACL_TENSOR_DATA_TENSOR) || (aclType == ACL_TENSOR_DATA_SLICE_TENSOR) ||
            ...
            if (itemVec[i].ctrlInfo.version == static_cast<int32_t>(VERSION_NAME)) {
                void* dataReal =
                    (itemVec[i].priorityDataPtr_ != nullptr) ? itemVec[i].priorityDataPtr_ : itemVec[i].dataPtr.get();
            ...
            acltdtDataItem* item = new (std::nothrow) acltdtDataItem(
                aclType, nullptr, 0, "", ACL_DT_UNDEFINED, "", itemVec[i].dataPtr, itemVec[i].ctrlInfo.dataLen);
            if (item == nullptr) {
                ACL_LOG_INNER_ERROR("[Check][Item]TensorDatasetDeserializes alloc failed");
                ret = ACL_ERROR_BAD_ALLOC;
                break;
...
aclError SaveCtrlSharedPtrToVec(
    const datasetMemType memType, rtMemQueueBuffInfo& qItem, const std::shared_ptr<uint8_t>& ctrlSharedPtr,
...
aclError UnpackageRecvDataInfo(uint8_t* outputHostAddr, size_t size, std::vector<acl::aclTdtDataItemInfo>& itemVec)
{
    ...
    for (uint32_t i = 0; i < cnt; ++i) {
        ...
        item.ctrlInfo = *tmp;
        ACL_LOG_INFO(
            "UnpackInfo version %d, dataType %d, curCnt %u, cnt %u, tensorType %d, dimNum %u, "
            "dynamicBitSize %u, sliceNum %u, sliceId %u, dataLen %lu",
            tmp->version, tmp->dataType, tmp->curCnt, tmp->cnt, tmp->tensorType, tmp->dimNum, tmp->dynamicBitSize,
            static_cast<uint32_t>(tmp->sliceNum), static_cast<uint32_t>(tmp->sliceId), tmp->dataLen);
...
aclError TensorDataitemSerialize(
    std::vector<acl::aclTdtDataItemInfo>& itemVec, const datasetMemType memType,
    std::vector<rtMemQueueBuffInfo>& qBufVec, std::vector<std::shared_ptr<uint8_t>>& ctrlSharedPtrVec)
{
    ...
    for (size_t i = 0; i < itemVec.size(); ++i) {
        ...
        void* ctrlPtr = ctrlSharedPtr.get();
        ACL_LOG_DEBUG(
            "TensorDataitemSerialize alignedSize is %zu, ctrlSize is %zu, dynamicBitSize is %u, i is %zu,"
            " lastDataSize is %zu, shape size is %zu",
            alignedSize, ctrlSize, itemVec[i].ctrlInfo.dynamicBitSize, i, lastDataSize, itemVec[i].dims.size());
        auto memcpyRet = memcpy_s(ctrlPtr, alignedSize, &itemVec[i].ctrlInfo, sizeof(ItemInfo));
        if (memcpyRet != EN_OK) {
            ACL_LOG_INNER_ERROR(
                "[Call][MemCpy]call memcpy failed, result=%d, srcLen=%zu, dstLen=%zu", memcpyRet, sizeof(ItemInfo),
                alignedSize);
        }
        size_t offset = sizeof(ItemInfo);
        for (size_t j = 0; j < itemVec[i].dims.size(); ++j) {
            memcpyRet = memcpy_s(
                reinterpret_cast<uint8_t*>(ctrlPtr) + offset, alignedSize - offset, &itemVec[i].dims[j],
                sizeof(int64_t));
            if (memcpyRet != EN_OK) {
                ACL_LOG_INNER_ERROR(
                    "[Call][MemCpy]call memcpy failed, result=%d, srcLen=%zu, dstLen=%zu", memcpyRet, sizeof(int64_t),
                    alignedSize - offset);
            }
            offset += sizeof(int64_t);
        }
...
aclError acltdtSendTensorV2(const acltdtChannelHandle* handle, const acltdtDataset* dataset, int32_t timeout)
{
    ...
    if (ret != ACL_SUCCESS) {
        ACL_LOG_INNER_ERROR(
            "[Serialize][Dataset]Failed to TensorDatasetSerializesV2, device is %u, name is %s.", handle->devId,
            handle->name.c_str());
        itemVec.clear();
        return ret;
    }
    ...
    if (ret != ACL_SUCCESS) {
        ACL_LOG_INNER_ERROR(
            "[Serialize][Dataset]Failed to TensorDataitemSerialize, device is %u, name is %s.", handle->devId,
            handle->name.c_str());
        return ret;
    }
    ...
    ACL_LOG_DEBUG("success to execute acltdtSendTensor, device is %u, name is %s", handle->devId, handle->name.c_str());
    return ACL_SUCCESS;
}
...
aclError EnsureCurrentThreadHasContext(const acltdtChannelHandle* handle)
{
    ...
    if (rtCtx == nullptr) {
        if (handle->ctx_ == nullptr) {
            ...
            ACL_REQUIRES_RTS_OK_WARN_NOT_SUPPORT(
                rtCtxCreateEx(&rtCtx, static_cast<uint32_t>(RT_CTX_NORMAL_MODE), static_cast<int32_t>(handle->devId)),
                rtCtxCreateEx);
            const_cast<acltdtChannelHandle*>(handle)->ctx_.reset(rtCtx, [](void* p) {
                if (p != nullptr) {
                    (void)rtCtxDestroyEx(p);
                }
            });
        }
...
aclError GetOrMallocHostMem(const acltdtChannelHandle* handle, acltdtDataset* dataset, size_t bufLen, void*& hostPtr)
{
    ...
    if (bufLen > dataset->sharedMemSize_) {
        ...
        dataset->sharedMem_.reset(outHostAddr, [](void* p) {
            if (p != nullptr) {
                (void)rtFreeHost(p);
            }
        });
...
aclError acltdtReceiveTensorV2(const acltdtChannelHandle* handle, acltdtDataset* dataset, int32_t timeout)
{
    ...
    if (ret != ACL_SUCCESS) {
        ACL_LOG_ERROR(
            "Failed to unpackage received data, device is %u, name is %s.", handle->devId, handle->name.c_str());
        return ret;
    }
    ret = acl::TensorDatasetDeserializesV2(itemVec, dataset);
    if (ret != ACL_SUCCESS) {
        ACL_LOG_INNER_ERROR(
            "[Deserialize][Dataset]Failed to deserialize tensor dataset, device is %u, name is %s.", handle->devId,
            handle->name.c_str());
        return ret;
    }
    ACL_LOG_INFO(
        "success to execute acltdtReceiveTensorV2, device is %u, name is %s.", handle->devId, handle->name.c_str());
    return ACL_SUCCESS;
}
} // namespace acl
...
aclError acltdtGetDimsFromItem(const acltdtDataItem* dataItem, int64_t* dims, size_t dimNum)
{
    ...
    if (dims == nullptr && dimNum != 0) {
        ...
        acl::AclErrorLogManager::ReportInputError(
            acl::INVALID_PARAM_REASON_MSG, std::vector<const char*>({"func", "value", "param", "reason"}),
            std::vector<const char*>(
                {__func__, value.c_str(), "dims/dimNum", "If dims is nullptr, dimNum should be 0"}));
        return ACL_ERROR_INVALID_PARAM;
    }
    ...
    if (dims != nullptr && dimNum == 0) {
        ...
        acl::AclErrorLogManager::ReportInputError(
            acl::INVALID_PARAM_REASON_MSG, std::vector<const char*>({"func", "value", "param", "reason"}),
            std::vector<const char*>(
                {__func__, value.c_str(), "dims/dimNum", "If dims is not nullptr, dimNum should be greater than 0"}));
        return ACL_ERROR_INVALID_PARAM;
    }

    if (dimNum < dataItem->dims.size()) {
        ACL_LOG_ERROR(
            "[Check][dimNum]output dimNum[%zu] cannot be less than dims number[%zu]", dimNum, dataItem->dims.size());
        const std::string dimNumVal = std::to_string(dimNum);
        std::string errMsg = acl::AclErrorLogManager::FormatStr(
            "dimNum %zu cannot be less than the size of dataItem's dims %zu", dimNum, dataItem->dims.size());
        acl::AclErrorLogManager::ReportInputError(
            acl::INVALID_PARAM_REASON_MSG, std::vector<const char*>({"func", "value", "param", "reason"}),
...
acltdtDataItem* acltdtCreateDataItem(
    acltdtTensorType tdtType, const int64_t* dims, size_t dimNum, aclDataType dataType, void* data, size_t size)
{
    ...
    if (dimNum > MAX_DIM_CNT) {
        ACL_LOG_ERROR(
            "[Check][Dimnum]acltdtCreateDataItem failed, dimNum[%zu] can't be larger than "
            "MAX_DIM_CNT[%zu]",
            dimNum, MAX_DIM_CNT);
...
acltdtDataset* acltdtCreateDataset() { return new (std::nothrow) acltdtDataset(); }
...
aclError acltdtAddDataItem(acltdtDataset* dataset, acltdtDataItem* dataItem)
{
    ...
    if (dataItem->dataPtr != nullptr) {
        ...
        ACL_REQUIRES_RTS_OK_WARN_NOT_SUPPORT(
            rtsPointerGetAttributes(dataItem->dataPtr.get(), &attr), rtsPointerGetAttributes);
    ...
    if ((dataset->memType != MEM_UNKNOWN) && (currentMemType != MEM_UNKNOWN)) {
        if (dataset->memType != currentMemType) {
            ...
            acl::AclErrorLogManager::ReportInputError(
                acl::INVALID_PARAM_REASON_MSG, std::vector<const char*>({"func", "value", "param", "reason"}),
                std::vector<const char*>(
                    {__func__, memTypeVal.c_str(), "dataset->memType",
                     "The memTypes in the dataset must be all host-side or device-side address"}));
            return ACL_ERROR_INVALID_PARAM;
        }
    }
...
acltdtDataItem* acltdtGetDataItem(const acltdtDataset* dataset, size_t index)
{
    ...
    if (index >= dataset->blobs.size()) {
        std::string errMsg = acl::AclErrorLogManager::FormatStr(
            "index %zu is greater than or equal to dataset size %zu", index, dataset->blobs.size());
...
acltdtChannelHandle* acltdtCreateChannel(uint32_t deviceId, const char* name)
{
    ...
    if (handle != nullptr) {
        if (!handle->recvName.empty()) {
            static TdtHostPreparePopDataFunc tdtHostPreparePopData =
                (TdtHostPreparePopDataFunc)GetFunction("TdtHostPreparePopData");
...
acltdtChannelHandle* acltdtCreateChannelWithCapacity(uint32_t deviceId, const char* name, size_t capacity)
{
    ...
    if (strnlen(name, RT_MQ_MAX_NAME_LEN) + 1 > RT_MQ_MAX_NAME_LEN) {
        ...
        std::string errMsg = acl::AclErrorLogManager::FormatStr(
            "name [%s] length %zu cannot be larger than %d", name, (strlen(name) + 1U), RT_MQ_MAX_NAME_LEN);
        acl::AclErrorLogManager::ReportInputError(
            acl::INVALID_PARAM_REASON_MSG, std::vector<const char*>({"func", "value", "param", "reason"}),
    ...
    if (ret != EN_OK) {
        ...
        std::stringstream ss;
        ss << std::hex << "name=0x" << reinterpret_cast<uintptr_t>(name) << ", dest=0x"
           << reinterpret_cast<uintptr_t>(attr.name) << std::dec << ", dest_max=" << RT_MQ_MAX_NAME_LEN
           << ", count=" << count << ".";
        ...
        acl::AclErrorLogManager::ReportInputError(
            ...
            std::vector<const char*>({__func__, "memcpy_s", retCode.c_str(), strerror(ret), extendInfo.c_str()}));
        ACL_LOG_ERROR(
            "[Call][MemCpy]call memcpy failed, result=%d, srcLen=%zu, dstLen=%d", ret, count, RT_MQ_MAX_NAME_LEN);
    ...
    ACL_LOG_INFO(
        "acltdtCreateChannelWithCapacity devId is %u, name is %s, real name is %s, qid is %u", deviceId,
        handle->name.c_str(), name, handle->qid);
    return handle;
}
...
{
    ACL_REQUIRES_NOT_NULL_WITH_INPUT_REPORT(handle);
    ACL_LOG_INFO("start to acltdtStopChannel, device is %u, name is %s", handle->devId, handle->name.c_str());
    ...
    if (!handle->recvName.empty()) {
        ...
        if (ret != 0) {
            ACL_LOG_INNER_ERROR(
                "[Init][Tdt]tdt host stop failed for channel %s, tdt result = %d", handle->name.c_str(), ret);
            return ACL_ERROR_FAILURE;
        }
    }
    ACL_LOG_INFO("acltdtStopChannel success, device is %u, name is %s", handle->devId, handle->name.c_str());
    return ACL_SUCCESS;
}
...
{
    ACL_REQUIRES_NOT_NULL_WITH_INPUT_REPORT(handle);
    ACL_LOG_INFO("start to acltdtDestroyChannel, device is %u, name is %s", handle->devId, handle->name.c_str());
    if (!handle->isTdtProcess) {
        ACL_REQUIRES_RTS_OK_WARN_NOT_SUPPORT(
            rtMemQueueDestroy(static_cast<int32_t>(handle->devId), handle->qid), rtMemQueueDestroy);
        ACL_LOG_INFO("acltdtDestroyChannel success, device is %u, name is %s", handle->devId, handle->name.c_str());
        ACL_DELETE_AND_SET_NULL(handle);
        return ACL_SUCCESS;
    }
    std::unique_lock<std::mutex> lk(aclChannleMutex);
...
{
    ACL_REQUIRES_NOT_NULL_WITH_INPUT_REPORT(handle);
    ACL_LOG_INFO("start to acltdtCleanChannel, device is %u, name is %s", handle->devId, handle->name.c_str());
    if (!handle->isTdtProcess) {
        ACL_REQUIRES_RTS_OK_WARN_NOT_SUPPORT(rtMemQueueReset(handle->devId, handle->qid), rtMemQueueReset);
        ACL_LOG_INFO("acltdtCleanChannel success, device is %u, name is %s", handle->devId, handle->name.c_str());
        return ACL_SUCCESS;
    }
    return ACL_ERROR_FEATURE_UNSUPPORTED;
}
...
aclError acltdtSendTensor(const acltdtChannelHandle* handle, const acltdtDataset* dataset, int32_t timeout)
{
    ACL_REQUIRES_NOT_NULL_WITH_INPUT_REPORT(handle);
    ACL_REQUIRES_NOT_NULL_WITH_INPUT_REPORT(dataset);
    ACL_LOG_DEBUG("start to execute acltdtSendTensor, device is %u, name is %s", handle->devId, handle->name.c_str());
    if (!handle->isTdtProcess) {
    ...
    ACL_CHECK_INVALID_PARAM_WITH_REASON(
        timeout != -1, timeout, "Only never timeout is supported, timeout can only be set to -1");

    std::vector<tdt::DataItem> itemVec;
    const auto ret = acl::TensorDatasetSerializes(dataset, itemVec);
    if (ret != ACL_SUCCESS) {
        ACL_LOG_INNER_ERROR(
            "[Serialize][Dataset]Failed to TensorDatasetSerializes, device is %u, name is %s.", handle->devId,
            handle->name.c_str());
        itemVec.clear();
        return ret;
    }
    ...
    if (sendRet != 0) {
        ACL_LOG_INNER_ERROR(
            "[Push][Data]Failed to push data, tdt result = %d, device is %u, name is %s.", sendRet, handle->devId,
            handle->name.c_str());
        return ACL_ERROR_FAILURE;
    }
...
aclError acltdtReceiveTensor(const acltdtChannelHandle* handle, acltdtDataset* dataset, int32_t timeout)
{
    ACL_REQUIRES_NOT_NULL_WITH_INPUT_REPORT(handle);
    ACL_REQUIRES_NOT_NULL_WITH_INPUT_REPORT(dataset);
    ACL_LOG_INFO("start to execute acltdtReceiveTensor, device is %u, name is %s", handle->devId, handle->name.c_str());
    if (!handle->isTdtProcess) {
    ...
    if (handle->recvName.empty()) {
        ACL_LOG_ERROR(
            "[Check][Recvname]it is not a receive channel, failed to receive, device is %u, name is %s", handle->devId,
            handle->name.c_str());
        return ACL_ERROR_INVALID_PARAM;
    }
    ...
    if (recvRet != 0) {
        ACL_LOG_INNER_ERROR(
            "[Pop][Data]Failed to receive, tdt result = %d, device is %u, name is %s.", recvRet, handle->devId,
            handle->name.c_str());
        return ACL_ERROR_FAILURE;
    }
    ...
    if (ret != ACL_SUCCESS) {
        ACL_LOG_INNER_ERROR(
            "[Deserialize][Dataset]Failed to TensorDatasetDeserializes, device is %u, name is %s.", handle->devId,
            handle->name.c_str());
        return ret;
    }

    ACL_LOG_INFO(
        "success to execute acltdtReceiveTensor, device is %u, name is %s", handle->devId, handle->name.c_str());
    return ACL_SUCCESS;
}
...
aclError acltdtQueryChannelSize(const acltdtChannelHandle* handle, size_t* size)
{
    ...
    ACL_REQUIRES_RTS_OK(rtMemQueueQueryInfo(static_cast<int32_t>(handle->devId), handle->qid, &info));
    *size = static_cast<size_t>(info.size);
    ACL_LOG_DEBUG(
        "success to execute acltdtQueryChannelSize, size is %zu, device is %u, qid is %u", *size, handle->devId,
        handle->qid);
    return ACL_SUCCESS;
}