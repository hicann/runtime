...
static aclError ProfInnerStart(const rtProfCommandHandle_t* const profilerConfig)
{
    ...
    if (!acl::AclProfilingManager::GetInstance().AclProfilingIsRun()) {
        ...
        if (ret != ACL_SUCCESS) {
            ACL_LOG_INNER_ERROR(
                "[Init][ProfilingManager]start acl profiling module failed,"
                " errorCode = %d",
                ret);
            return ret;
        }
...
static aclError ProfInnerStop(const rtProfCommandHandle_t* const profilerConfig)
{
    ...
    if (!acl::AclProfilingManager::GetInstance().IsDeviceListEmpty()) {
        (void)acl::AclProfilingManager::GetInstance().RemoveDeviceList(
            profilerConfig->devIdList, profilerConfig->devNums);
    }
...
namespace acl {
aclError AclProfiling::HandleProfilingCommand(
    const std::string& config, const bool configFileFlag, const bool noValidConfig)
{
    ...
    if (noValidConfig) {
        ...
        if (ret != MSPROF_ERROR_NONE) {
            ACL_LOG_CALL_ERROR("[Init][Profiling]init profiling with nullptr failed, profiling errorCode = %d", ret);
            return ACL_SUCCESS;
        }
    } else {
        if (configFileFlag) {
            ret = MsprofInit(
                MSPROF_CTRL_INIT_ACL_JSON, const_cast<char_t*>(config.c_str()), static_cast<uint32_t>(config.size()));
            if (ret != MSPROF_ERROR_NONE) {
                ACL_LOG_CALL_ERROR(
                    "[Init][Profiling]handle json config of profiling failed, profiling "
                    "result = %d",
                    ret);
                return ACL_ERROR_INVALID_PARAM;
            }
            ...
            ret = MsprofInit(
                MSPROF_CTRL_INIT_ACL_ENV, const_cast<char_t*>(config.c_str()), static_cast<uint32_t>(config.size()));
            if (ret != MSPROF_ERROR_NONE) {
                ACL_LOG_CALL_ERROR(
                    "[Init][Profiling]handle env config of profiling failed, profiling "
                    "result = %d",
                    ret);
                return ACL_ERROR_INVALID_PARAM;
            }
