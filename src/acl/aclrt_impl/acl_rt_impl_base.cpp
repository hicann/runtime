...
aclError UpdatePlatformInfoWithDevice(int32_t deviceId)
{
    ...
    if (fe::PlatformInfoManager::GeInstance().InitRuntimePlatformInfos(socVersion) != 0U) {
        ACL_LOG_WARN(
            "[Init][PlatformInfo]init runtime platform info unsuccessfully, SocVersion = %s", socVersion.c_str());
        return ACL_ERROR_INTERNAL_ERROR;
    }
    ...
    rtErr = rtGetDeviceInfo(static_cast<uint32_t>(deviceId), MODULE_TYPE_VECTOR_CORE, INFO_TYPE_CORE_NUM, &vecCoreCnt);
    if (rtErr != RT_ERROR_NONE) {
    ...
    rtErr = rtGetDeviceInfo(static_cast<uint32_t>(deviceId), MODULE_TYPE_AICORE, INFO_TYPE_CUBE_NUM, &cubeCoreCnt);
    if (rtErr != RT_ERROR_NONE) {
    ...
    platformRet = fe::PlatformInfoManager::GeInstance().UpdateRuntimePlatformInfosByDevice(deviceId, platformInfos);
    if (platformRet != 0U) {
...
aclError InitSocVersion()
{
    ...
    if (aclSocVersion.empty()) {
        ...
        if (rtErr != RT_ERROR_NONE) {
            ACL_LOG_INFO("Cannot get soc version, runtime retCode=%d", static_cast<int32_t>(rtErr));
            return ACL_GET_ERRCODE_RTS(rtErr);
        }
...
bool GetAclInitFlag() { return aclInitRefCount > 0UL; }

uint64_t& GetAclInitRefCount() { return aclInitRefCount; }

std::recursive_mutex& GetAclInitMutex() { return aclInitMutex; }

std::string& GetConfigPathStr() { return aclConfigStr; }

void SetConfigPathStr(std::string& configStr) { aclConfigStr = configStr; }
...
const char* aclrtGetSocNameImpl()
{
    ...
    if (ret != ACL_SUCCESS) {
        ACL_LOG_INFO("Cannot init soc version, retCode = %d", ret);
        return nullptr;
    }
