...
bool JsonParser::IsValidFileName(const char_t* const fileName)
{
    ...
    if (ret != EN_OK) {
        ...
        ACL_LOG_ERROR(
            "[Get][FileStatus]cannot get config file status, which path is %s, "
            "maybe does not exist, return %d, errcode %d",
    ...
    if ((pathStat.st_mode & S_IFMT) != S_IFREG) {
        ...
        ACL_LOG_ERROR(
            "[Config][ConfigFile]config file is not a regular file, which path is %s, "
            "mode is %u",
            trustedPath, pathStat.st_mode);
        return false;
    }
    if (pathStat.st_size > MAX_CONFIG_FILE_BYTE) {
        ...
        ACL_LOG_ERROR(
            "[Check][FileSize]config file %s size[%ld] is larger than "
            "max config file Bytes[%ld]",
            trustedPath, pathStat.st_size, MAX_CONFIG_FILE_BYTE);
        return false;
    }
    return true;
}

void JsonParser::GetMaxNestedLayers(
...
aclError JsonParser::GetConfigStrFromFile(const char_t* const fileName, std::string& configStr)
{
    ...
    ACL_CHECK_INVALID_FILE_MSG_RET(
        !fin.is_open(), fileName, "File cannot be opened for reading", ACL_ERROR_INVALID_FILE);
    ...
    if ((maxObjDepth > MAX_CONFIG_OBJ_DEPTH) || (maxArrayDepth > MAX_CONFIG_ARRAY_DEPTH)) {
        std::string reason = acl::AclErrorLogManager::FormatStr(
            "object depth %zu exceeds max %zu or array depth %zu exceeds max %zu", maxObjDepth, MAX_CONFIG_OBJ_DEPTH,
            maxArrayDepth, MAX_CONFIG_ARRAY_DEPTH);
        acl::AclErrorLogManager::ReportInputError(
            acl::INVALID_FILE_MSG, std::vector<const char*>({"path", "reason"}),
            std::vector<const char*>({fileName, reason.c_str()}));
        ACL_LOG_ERROR(
            "[Check][MaxArrayDepth]invalid json file, the object's depth[%zu] is larger than %zu, "
...
aclError JsonParser::GetJsonCtxByKey(
...
aclError JsonParser::GetAttrConfigFromFile(const char_t* const fileName, std::map<aclCannAttr, CannInfo>& cannInfoMap)
{
    nlohmann::json js;
    ...
    if (!std::regex_match(defaultDeviceIdStr, reg)) {
        acl::AclErrorLogManager::ReportInputError(
            ...
            std::vector<const char*>(
                {__func__, defaultDeviceIdStr.c_str(), "default_device", "value must be zero or a positive integer"}));
        ACL_LOG_ERROR(
    ...
    const char_t* const fileName, const std::string& typeName, size_t& fifoSize, bool& found)
{
    ...
    if (ret != ACL_SUCCESS) {
        ...
        ACL_LOG_ERROR(
            "can not parse config from file[%s], config[%s], errorCode = %d", fileName, typeName.c_str(), ret);
        return ret;
    }
    if (!found) {
        return ACL_SUCCESS;
