...
#define ACL_ADD_APPLY_TOTAL_COUNT(resourceType) acl::ResourceStatistics::GetInstance().AddApplyTotalCount(resourceType)
#define ACL_ADD_APPLY_SUCCESS_COUNT(resourceType) acl::ResourceStatistics::GetInstance().AddApplySuccCount(resourceType)
...
enum ResourceType : int32_t {
    ...
    // add enum before this comment, ACL_STATISTICS_RESOURCE_TYPE_SIZE is length of enum
    ACL_STATISTICS_RESOURCE_TYPE_SIZE,
};

enum ApplyReleaseType : int32_t { APPLY_TOTAL = 0, APPLY_SUCCESS, RELEASE_TOTAL, RELEASE_SUCCESS, APPLY_RELEASE_SIZE };

class ResourceStatisticsValue {
public:
    ResourceStatisticsValue() { Init(); }
...
private:
    ...
    ~ResourceStatistics() = default;
    ResourceStatisticsValue counter_[ACL_STATISTICS_RESOURCE_TYPE_SIZE];
};
} // namespace acl
