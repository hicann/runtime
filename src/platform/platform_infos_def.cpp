...
#ifndef _OPEN_SOURCE_LLT_
static void GetVecMapInfoFromProtoBuf(
    ProtoBufStringVecMap* protoMap, std::map<std::string, std::vector<std::string>>& dstMap)
{
...
static void GetMapMapInfoFromProtoBuf(
    ProtoBufStringMapMap* protoMap, std::map<std::string, std::map<std::string, std::string>>& dstMap)
{
    for (auto mapIter = protoMap->begin(); mapIter != protoMap->end(); ++mapIter) {
        const std::string curMapKey = mapIter->first;
        PF_LOGD("curMapKey:%s", curMapKey.c_str());
        ...
        for (auto secondIter = protoTmpMap.platform_map().begin(); secondIter != protoTmpMap.platform_map().end();
             ++secondIter) {
...
void PlatFormInfos::SetCoreNumByCoreType(const std::string& core_type)
{
    ...
    */
    ...
    PF_LOGD(
        "Set PlatFormInfos::core_num_ to %s: [%u], core_type: %s.", core_type_str.c_str(), core_num_,
        core_type.c_str());
}
