...
std::map<std::string, PlatformInfoType> g_platform_info_type_map{
...
void PlatformInfoManager::ParseVersion(
...
void PlatformInfoManager::ParseSocInfo(
    std::map<std::string, std::string>& soc_info_map, PlatformInfo& platform_info_temp)
{
    try {
        ...
        std::map<std::string, std::string>::const_iterator vector_core_cnt =
            soc_info_map.find(CONVERT(vector_core_cnt));
...
void PlatformInfoManager::ParseCubeOfAICoreSpec(
    std::map<std::string, std::string>& ai_core_spec_map, PlatformInfo& platform_info_temp)
{
    try {
        ...
        std::map<std::string, std::string>::const_iterator vec_calc_size =
            ai_core_spec_map.find(CONVERT(vec_calc_size));
...
void PlatformInfoManager::ParseBufferOfAICoreSpec(
    std::map<std::string, std::string>& ai_core_spec_map, PlatformInfo& platform_info_temp)
{
    try {
...
void PlatformInfoManager::ParseUBOfAICoreSpec(
    std::map<std::string, std::string>& ai_core_spec_map, PlatformInfo& platform_info_temp)
{
    try {
        ...
        std::map<std::string, std::string>::const_iterator ubburst_in_one_block =
            ai_core_spec_map.find(CONVERT(ubburst_in_one_block));
        if (ubburst_in_one_block != ai_core_spec_map.end()) {
            platform_info_temp.ai_core_spec.ubburst_in_one_block =
                static_cast<uint64_t>(stoll(ubburst_in_one_block->second));
        }

        std::map<std::string, std::string>::const_iterator ubbank_group_num =
            ai_core_spec_map.find(CONVERT(ubbank_group_num));
...
void PlatformInfoManager::ParseUnzipOfAICoreSpec(
    std::map<std::string, std::string>& ai_core_spec_map, PlatformInfo& platform_info_temp)
{
    try {
        std::map<std::string, std::string>::const_iterator unzip_engines =
            ai_core_spec_map.find(CONVERT(unzip_engines));
        ...
        std::map<std::string, std::string>::const_iterator unzip_max_ratios =
            ai_core_spec_map.find(CONVERT(unzip_max_ratios));
        ...
        std::map<std::string, std::string>::const_iterator unzip_channels =
            ai_core_spec_map.find(CONVERT(unzip_channels));
        ...
        std::map<std::string, std::string>::const_iterator unzip_is_tight =
            ai_core_spec_map.find(CONVERT(unzip_is_tight));
...
void PlatformInfoManager::ParseAICoreSpec(
    std::map<std::string, std::string>& ai_core_spec_map, PlatformInfo& platform_info_temp)
{
...
static double CalculateFraction(const std::string& fraction)
{
    ...
    try {
        ...
        if (trimmed_denominator == "0" || trimmed_denominator == "0.0" || trimmed_denominator == "0." ||
            trimmed_denominator == "-0" || trimmed_denominator == "-0.0" || trimmed_denominator == "-0.") {
...
void PlatformInfoManager::ParseBufferOfAICoreMemoryRates(
    std::map<std::string, std::string>& ai_core_memory_rates_map, PlatformInfo& platform_info_temp)
{
    try {
        ...
        std::map<std::string, std::string>::const_iterator ddr_read_rate =
            ai_core_memory_rates_map.find(CONVERT(ddr_read_rate));
        ...
        std::map<std::string, std::string>::const_iterator ddr_write_rate =
            ai_core_memory_rates_map.find(CONVERT(ddr_write_rate));
        ...
        std::map<std::string, std::string>::const_iterator l2_read_rate =
            ai_core_memory_rates_map.find(CONVERT(l2_read_rate));
        ...
        std::map<std::string, std::string>::const_iterator l2_write_rate =
            ai_core_memory_rates_map.find(CONVERT(l2_write_rate));
        ...
        std::map<std::string, std::string>::const_iterator l1_to_l0_a_rate =
            ai_core_memory_rates_map.find(CONVERT(l1_to_l0_a_rate));
        if (l1_to_l0_a_rate != ai_core_memory_rates_map.end()) {
            platform_info_temp.ai_core_memory_rates.l1_to_l0_a_rate =
                static_cast<double>(stod(l1_to_l0_a_rate->second));
        }

        std::map<std::string, std::string>::const_iterator l1_to_l0_b_rate =
            ai_core_memory_rates_map.find(CONVERT(l1_to_l0_b_rate));
        if (l1_to_l0_b_rate != ai_core_memory_rates_map.end()) {
            platform_info_temp.ai_core_memory_rates.l1_to_l0_b_rate =
                static_cast<double>(stod(l1_to_l0_b_rate->second));
        }

        std::map<std::string, std::string>::const_iterator l1_to_ub_rate =
            ai_core_memory_rates_map.find(CONVERT(l1_to_ub_rate));
        ...
        std::map<std::string, std::string>::const_iterator l0_c_to_ub_rate =
            ai_core_memory_rates_map.find(CONVERT(l0_c_to_ub_rate));
        if (l0_c_to_ub_rate != ai_core_memory_rates_map.end()) {
            platform_info_temp.ai_core_memory_rates.l0_c_to_ub_rate =
                static_cast<double>(stod(l0_c_to_ub_rate->second));
        }
    } catch (...) {
...
void PlatformInfoManager::ParseUBOfAICoreMemoryRates(
    std::map<std::string, std::string>& ai_core_memory_rates_map, PlatformInfo& platform_info_temp)
{
    std::map<std::string, std::string>::const_iterator ub_to_l2_rate =
        ai_core_memory_rates_map.find(CONVERT(ub_to_l2_rate));
    ...
    std::map<std::string, std::string>::const_iterator ub_to_ddr_rate =
        ai_core_memory_rates_map.find(CONVERT(ub_to_ddr_rate));
    ...
    std::map<std::string, std::string>::const_iterator ub_to_l1_rate =
        ai_core_memory_rates_map.find(CONVERT(ub_to_l1_rate));
...
void PlatformInfoManager::ParseAICoreMemoryRates(
    std::map<std::string, std::string>& ai_core_memory_rates_map, PlatformInfo& platform_info_temp)
{
...
void PlatformInfoManager::ParseAICoreintrinsicDtypeMap(
    std::map<std::string, std::string>& ai_coreintrinsic_dtype_map, PlatformInfo& platform_info_temp)
{
...
void PlatformInfoManager::ParseVectorCoreSpec(
    std::map<std::string, std::string>& vector_core_spec_map, PlatformInfo& platform_info_temp)
{
    try {
        ...
        std::map<std::string, std::string>::const_iterator vec_calc_size =
            vector_core_spec_map.find(CONVERT(vec_calc_size));
        ...
        std::map<std::string, std::string>::const_iterator smask_buffer =
            vector_core_spec_map.find(CONVERT(smask_buffer));
        ...
        std::map<std::string, std::string>::const_iterator ubblock_size =
            vector_core_spec_map.find(CONVERT(ubblock_size));
        ...
        std::map<std::string, std::string>::const_iterator ubbank_size =
            vector_core_spec_map.find(CONVERT(ubbank_size));
        ...
        std::map<std::string, std::string>::const_iterator ubburst_in_one_block =
            vector_core_spec_map.find(CONVERT(ubburst_in_one_block));
        ...
        std::map<std::string, std::string>::const_iterator ubbank_group_num =
            vector_core_spec_map.find(CONVERT(ubbank_group_num));
        if (ubbank_group_num != vector_core_spec_map.end()) {
            platform_info_temp.vector_core_spec.ubbank_group_num =
                static_cast<uint64_t>(stoll(ubbank_group_num->second));
        }
...
void PlatformInfoManager::ParseVectorCoreMemoryRates(
    std::map<std::string, std::string>& vector_core_memory_rates_map, PlatformInfo& platform_info_temp)
{
    try {
        std::map<std::string, std::string>::const_iterator ddr_rate =
            vector_core_memory_rates_map.find(CONVERT(ddr_rate));
        ...
        std::map<std::string, std::string>::const_iterator ddr_read_rate =
            vector_core_memory_rates_map.find(CONVERT(ddr_read_rate));
        if (ddr_read_rate != vector_core_memory_rates_map.end()) {
            platform_info_temp.vector_core_memory_rates.ddr_read_rate =
                static_cast<double>(stod(ddr_read_rate->second));
        }

        std::map<std::string, std::string>::const_iterator ddr_write_rate =
            vector_core_memory_rates_map.find(CONVERT(ddr_write_rate));
        if (ddr_write_rate != vector_core_memory_rates_map.end()) {
            platform_info_temp.vector_core_memory_rates.ddr_write_rate =
                static_cast<double>(stod(ddr_write_rate->second));
        }

        std::map<std::string, std::string>::const_iterator l2_rate =
            vector_core_memory_rates_map.find(CONVERT(l2_rate));
        ...
        std::map<std::string, std::string>::const_iterator l2_read_rate =
            vector_core_memory_rates_map.find(CONVERT(l2_read_rate));
        ...
        std::map<std::string, std::string>::const_iterator l2_write_rate =
            vector_core_memory_rates_map.find(CONVERT(l2_write_rate));
        if (l2_write_rate != vector_core_memory_rates_map.end()) {
            platform_info_temp.vector_core_memory_rates.l2_write_rate =
                static_cast<double>(stod(l2_write_rate->second));
        }

        std::map<std::string, std::string>::const_iterator ub_to_l2_rate =
            vector_core_memory_rates_map.find(CONVERT(ub_to_l2_rate));
        if (ub_to_l2_rate != vector_core_memory_rates_map.end()) {
            platform_info_temp.vector_core_memory_rates.ub_to_l2_rate =
                static_cast<double>(stod(ub_to_l2_rate->second));
        }

        std::map<std::string, std::string>::const_iterator ub_to_ddr_rate =
            vector_core_memory_rates_map.find(CONVERT(ub_to_ddr_rate));
        if (ub_to_ddr_rate != vector_core_memory_rates_map.end()) {
            platform_info_temp.vector_core_memory_rates.ub_to_ddr_rate =
                static_cast<double>(stod(ub_to_ddr_rate->second));
        }
    } catch (...) {
...
void PlatformInfoManager::ParseCPUCache(
    std::map<std::string, std::string>& CPUCacheMap, PlatformInfo& platform_info_temp)
{
    try {
...
void PlatformInfoManager::ParseVectorCoreintrinsicDtypeMap(
    std::map<std::string, std::string>& vector_coreintrinsic_dtype_map, PlatformInfo& platform_info_temp)
{
    std::map<std::string, std::string>::const_iterator iter;
...
void PlatformInfoManager::ParseSoftwareSpec(
    std::map<std::string, std::string>& software_spec_map, PlatformInfo& platform_info_temp)
{
    try {
        ...
        if (jit_compile_default_value != software_spec_map.end() && jit_compile_default_value->second == "1") {
...
uint32_t PlatformInfoManager::ParsePlatformInfoFromStrToStruct(
    std::map<std::string, std::map<std::string, std::string>>& content_info_map, std::string& soc_version,
...
uint32_t PlatformInfoManager::AssemblePlatformInfoVector(
    std::map<std::string, std::map<std::string, std::string>>& content_info_map)
{
...
void PlatformInfoManager::ParseAICoreintrinsicDtypeMap(
    std::map<std::string, std::string>& ai_coreintrinsic_dtype_map, PlatFormInfos& platform_info_temp)
{
    std::map<std::string, std::string>::const_iterator iter;
...
void PlatformInfoManager::ParseVectorCoreintrinsicDtypeMap(
    std::map<std::string, std::string>& vector_coreintrinsic_dtype_map, PlatFormInfos& platform_info_temp)
{
    std::map<std::string, std::string>::const_iterator iter;
    std::map<std::string, std::string> platform_res_map;
...
void PlatformInfoManager::ParsePlatformRes(
    const std::string& label, std::map<std::string, std::string>& platform_res_map, PlatFormInfos& platform_info_temp)
{
    platform_info_temp.SetPlatformRes(label, platform_res_map);
}

uint32_t PlatformInfoManager::ParsePlatformInfo(
    std::map<std::string, std::map<std::string, std::string>>& content_info_map, std::string& soc_version,
...
uint32_t PlatformInfoManager::UpdatePlatformInfos(fe::PlatFormInfos& platform_info)
{
    ...
    if (iter == platform_infos_map_.end()) {
        if (EnsureSocVersionLoaded(opti_compilation_infos_.GetSocVersion()) != PLATFORM_SUCCESS) {
            PF_LOGE(
                "Platform info for soc_version [%s] is not initialized.",
                opti_compilation_infos_.GetSocVersion().c_str());
            return PLATFORM_FAILED;
        }
