...
aclError aclplatformGetInstructionInfo(aclplatformCoreType type, const char* instruction, char* value, uint32_t maxLen)
{
    ...
    if (it == intrinsic_map.end() || it->second.empty()) {
        PF_LOGE("Instruction [%s] not found for core type %d.", instruction, static_cast<int>(type));
        return ACL_ERROR_INVALID_PARAM;
    }
    ...
    PF_LOGI(
        "Instruction info queried: [%s], core[%d], result = %s.", instruction, static_cast<int>(type), result.c_str());
    return CopyToBuffer(result, value, maxLen);
}
