...
| cmoType | 输入 | Cache内存操作类型。类型定义请参见[aclrtCmoType](aclrtCmoType.md)。 |
...
| stream | 输入 | 执行内存操作任务的Stream。类型定义请参见[aclrtStream](aclrtStream.md)。<br>此处只支持与模型绑定过的Stream，绑定模型与Stream需调用[aclmdlRIBindStream](aclmdlRIBindStream.md)接口。 |
