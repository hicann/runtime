...
| 参数名 | 输入/输出 | 说明 |
| --- | --- | --- |
| ptr | 输入 | Host内存地址。<br>Host内存地址需4K页对齐。<br>当os内核版本为5.10或更低时，使用非锁页内存会导致异常，因此必须调用aclrtMallocHost接口来申请Host内存。<br>当os内核版本为5.10以上时，支持使用非锁页的Host内存，因此也支持不调用aclrtMallocHost接口申请Host内存，可以使用malloc接口申请Host内存。 |
| size | 输入 | 内存大小，单位Byte。 |
| type | 输入 | 内存注册类型。 |
