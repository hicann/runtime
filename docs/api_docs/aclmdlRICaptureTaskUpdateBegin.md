...
-   针对Ascend 950PR/Ascend950DT，需将驱动包升级到26.0.RC1或更高版本。
...
| 参数名 | 输入/输出 | 说明 |
| --- | --- | --- |
| stream | 输入 | 指定Stream。类型定义请参见[aclrtStream](aclrtStream.md)。<br>此处的Stream必须是不在捕获状态的Stream。 |
| handle | 输入 | 标识任务组的句柄。类型定义请参见[aclrtTaskGrp](aclrtTaskGrp.md)。<br>提前调用[aclmdlRICaptureTaskGrpBegin](aclmdlRICaptureTaskGrpBegin.md)、[aclmdlRICaptureTaskGrpEnd](aclmdlRICaptureTaskGrpEnd.md)接口标记任务组，并通过aclmdlRICaptureTaskGrpEnd接口获取任务组句柄，再作为入参传入此处。 |
