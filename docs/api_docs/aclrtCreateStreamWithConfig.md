...
| priority | 输入 | 优先级。<br>该参数为预留参数，暂不使用，当前固定设置为0。 |
| flag | 输入 | Stream指针的flag。 |

## flag取值说明
flag既支持配置单个宏，也支持配置多个宏位或（例如ACL_STREAM_FAST_LAUNCH | ACL_STREAM_FAST_SYNC）。对于不支持位或的宏，本接口会返回报错。配置其他值创建出来的Stream等同于通过[aclrtCreateStream](aclrtCreateStream.md)接口创建出来的Stream。
