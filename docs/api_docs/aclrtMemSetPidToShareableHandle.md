...
在调用aclrtMemExportToShareableHandle接口的进程中，调用本接口设置进程白名单。本接口需与其它接口配合使用，以便实现内存共享的目的，请参见[aclrtMemExportToShareableHandle](aclrtMemExportToShareableHandle.md)接口处的说明。
