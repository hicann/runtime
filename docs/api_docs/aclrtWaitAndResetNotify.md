...
| 参数名 | 输入/输出 | 说明 |
| --- | --- | --- |
| notify | 输入 | 需等待的Notify。类型定义请参见[aclrtNotify](aclrtNotify.md)。 |
| stream | 输入 | 指定Stream。类型定义请参见[aclrtStream](aclrtStream.md)。<br>多Stream同步等待场景下，例如，Stream2等Stream1的场景，此处配置为Stream2。 |
