...
在Device上分配size大小的线性内存，并通过\*devPtr返回已分配内存的指针。本接口分配的内存会进行字节对齐，会对用户申请的size向上对齐成32字节整数倍。使用本接口申请的内存，需要通过[aclrtFree](aclrtFree.md)接口或[aclrtFreeWithDevSync](aclrtFreeWithDevSync.md)接口释放内存。
