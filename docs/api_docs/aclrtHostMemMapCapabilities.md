...
| hacType | 输入 | 硬件驱动加速器类型。类型定义请参见[aclrtHacType](aclrtHacType.md)。 |
| capabilities | 输出 | 是否支持访问通过aclrtHostRegister接口注册的内存。类型定义请参见[aclrtHostMemMapCapability](aclrtHostMemMapCapability.md)。 |
