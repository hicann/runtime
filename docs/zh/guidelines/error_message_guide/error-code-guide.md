...
| EE1023 | 资源错误 · Resource_Error | `func, reason` | `%s failed. Reason: %s.` | Cause: N/A<br>Solution: N/A | NPU硬件调度资源不足 | `stream_jetty_context.cc:62`<br>`RT_LOG_OUTER_MSG_IMPL(ErrorCode::EE1023, "Expanding the capacity", "There are too many asynchronous copy tasks in the ACL Graph. 1. If the value of numBatches for aclrtMemcpyBatchAsync in the ACL Graph is too large, reduce the value of numBatches. 2. If the value of height for aclrtMemcpy2dAsync in the ACL Graph is too large, reduce the value of height");` | `Expanding the capacity failed. Reason: There are too many asynchronous copy tasks in the ACL Graph. 1. If the value of numBatches for aclrtMemcpyBatchAsync in the ACL Graph is too large, reduce the value of numBatches. 2. If the value of height for aclrtMemcpy2dAsync in the ACL Graph is too large, reduce the value of height. ErrorCode=EE1023.` | / |
...
| EH0013 | 其他 · 标准函数失败 | `func1, func2, ret_code, reason, extend_info` | `%s failed. Reason: Standard function %s failed. [Errno %s] %s. %s` | Cause: N/A.<br>Solution: N/A. | ACL层标准库函数失败 | `queue.cpp:38`<br>`const auto ret = memcpy_s(...); if (ret != EOK) { ... acl::AclErrorLogManager::ReportInputError(acl::STANDARD_FUNC_FAILED_MSG, {"func1", "func2", "ret_code", "reason", "extend_info"}, {"Parameter copy", "memcpy_s", retVal.c_str(), strerror(ret), extendInfo.c_str()}); }` | `Parameter copy failed. Reason: Standard function memcpy_s failed. [Errno 22] Invalid argument. src=... ErrorCode=EH0013.` | 如参数是用户传入，extend_info打印参数名而非默认src、dst等；func1 参数优先使用语义化描述 |
