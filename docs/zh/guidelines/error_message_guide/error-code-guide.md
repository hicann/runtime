...
| EE1003 ⭐ | 输入参数非法 · Invalid_Argument | `func, value, param, expect` | `%s failed because value %s for parameter %s is invalid. Expected value: %s.` | Cause: N/A.<br>Solution: 1. Check the input parameter range of the function. 2. Check the function invocation relationship. | 参数值非法+参数名参数值可打印+能给出期望值 | `runtime.cc:4674`<br>`RT_LOG_OUTER_MSG_WITH_FUNC_DESC(ErrorCode::EE1003, "Obtaining the logical device ID based on the user device ID", userDevId, "userDevId", "[0, " + std::to_string(userDeviceCnt) + ")");` | `Obtaining the logical device ID based on the user device ID failed because value 8 for parameter userDevId is invalid. Expected value: [0, 4). ErrorCode=EE1003.` | ⭐ 首选通用码；优先使用 `_WITH_DESC` 变体传入语义化函数描述 |
...
| EE1005 | 不支持 · Not_Supported | `func` | `The current system or device does not support %s.` | 无 | 完整功能芯片不支持 | `api_impl.cc:4310`<br>`if (!IS_SUPPORT_CHIP_FEATURE(dev->GetChipType(), RtOptionalFeatureType::RT_FEATURE_MODEL_ABORT)) { RT_LOG_OUTER_MSG_WITH_FUNC_DESC(ErrorCode::EE1005, "Aborting the model running instance"); return RT_ERROR_FEATURE_NOT_SUPPORT; }` | `The current system or device does not support Aborting the model running instance. ErrorCode=EE1005.` | 仅芯片层面不支持某个完整功能；优先使用 `_WITH_FUNC_DESC` 变体传入语义化函数描述 |
| EE1006 | 不支持 · Not_Supported | `func, type, reason` | `%s failed. %s is not supported. Reason: %s.` | Cause: N/A.<br>Solution: N/A. | 非完整功能，某个配置参数不支持（非完整功能，即非一个完整的rt/rts接口） | `api_error.cc:1171`<br>`COND_RETURN_AND_MSG_OUTER((stm != nullptr) && ((stm->Flags() & RT_STREAM_AICPU) != 0U), RT_ERROR_STREAM_INVALID, ErrorCode::EE1006, "Synchronizing a stream", "Stream flags value " + std::to_string(stm->Flags()), "The current stream is used to carry AI CPU scheduling tasks and does not support stream synchronization");` | `Synchronizing a stream failed. Stream flags value 4 is not supported. Reason: The current stream is used to carry AI CPU scheduling tasks and does not support stream synchronization. ErrorCode=EE1006.` | ①如通过升级版本可解决，或因版本不兼容导致不支持，需要在Reason中写出支持的软件版本信息；<br>②非升级可解决，Reason可以写不支持的原则，可以写支持的操作，可以写不支持的详细信息。func 参数优先使用语义化描述 |
| EE1007 | 资源错误 · Bind_Stream | `id, reason` | `Failed to bind stream (stream_id=%s). Reason: %s.` | Cause: N/A.<br>Solution: Unbind the stream from the already bound model and then rebind it to the current model. | 流绑定到模型失败 | `model.cc:400`<br>`if (streamIn->IsModelStream()) { RT_LOG_OUTER_MSG_IMPL(ErrorCode::EE1007, streamId, RtFmtMsg("The current stream has been bound to a model (model_id=%u) and cannot be bound to the input model (model_id=%u)", streamIn->Model_()->Id_(), Id_())); return RT_ERROR_STREAM_MODEL; }` | `Failed to bind stream (stream_id=15). Reason: The current stream has been bound to a model (model_id=1) and cannot be bound to the input model (model_id=2). ErrorCode=EE1007.` | 专用于流绑定失败 |
...
| EE1015 | 不支持 · Incorrect_Driver_Version | `func, reason` | `%s failed. Reason: The driver version capability is insufficient. %s` | Cause: N/A.<br>Solution: Upgrade the driver software version. | 驱动版本不正确，升级驱动版本可解决 | `runtime.cc:2114`<br>`COND_RETURN_AND_MSG_OUTER(tsdOpenNetService_ == nullptr, RT_ERROR_DRV_TSD_ERR, ErrorCode::EE1015, "Starting the HCCP process", "Symbol TsdOpenNetService not found in libtsdclient.so.");` | `Starting the HCCP process failed. Reason: The driver version capability is insufficient. Symbol TsdOpenNetService not found in libtsdclient.so. ErrorCode=EE1015.` | 驱动符号不存在如需上报errmsg，可使用此错误码；func 参数优先使用语义化描述 |
| EE1016 | 不支持 · Not_Supported | `func, reason` | `%s failed. Reason: %s.` | Cause: N/A.<br>Solution: N/A. | ①固有不支持：某种状态或场景下不支持做某类操作，如非ACL Graph模型不支持ModelUpdate、遇错即停模式从stop修改为continue不支持；<br>②完整功能不支持（完整功能，即一个完整的rt/rts接口） | ①固有不支持示例一：`api_impl.cc:1904`<br>`COND_RETURN_AND_MSG_OUTER((curStm->GetFailureMode() == STOP_ON_FAILURE) && (failmode == CONTINUE_ON_FAILURE), RT_ERROR_FEATURE_NOT_SUPPORT, ErrorCode::EE1016, "Setting the stream error reporting mode", RtFmtMsg("Changing stream %u from stop mode to continue mode is not supported", curStm->Id_()));`<br>②固有不支持示例二：`COND_RETURN_AND_MSG_OUTER(mdl->GetModelType() != RT_MODEL_CAPTURE_MODEL, RT_ERROR_FEATURE_NOT_SUPPORT, ErrorCode::EE1016, "Updating the model running instance", "Non ACL Graph mode is not supported");`<br>③完整功能不支持示例（capture mode 检查专用宏）：`api_impl.cc:2674`<br>`CHECK_CAPTURE_MODE_SUPPORT_AND_RETURN_WITH_DESC(curCtx, "Synchronous memory copy");` | ①固有不支持示例一：`Setting the stream error reporting mode failed. Reason: Changing stream 15 from stop mode to continue mode is not supported. ErrorCode=EE1016.`<br>②固有不支持示例二：`Updating the model running instance failed. Reason: Non ACL Graph mode is not supported. ErrorCode=EE1016.`<br>③完整功能不支持示例：`Synchronous memory copy failed. Reason: the operation has been converted to a synchronous operation. Operation not permitted when a stream is capturing and the specified capture mode is not relaxed. ErrorCode=EE1016.` | reason描述要点：<br>①固有不支持→强调是什么状态或场景不支持什么操作；<br>②完整功能不支持→可以写不支持的原则，可以写支持的操作，可以写不支持的详细信息。func 参数优先使用语义化描述；capture mode 场景使用 `CHECK_CAPTURE_MODE_SUPPORT_AND_RETURN_WITH_DESC` 专用宏 |
| EE1017 | 输入参数非法 · Invalid_Argument | `func, param, reason` | `%s failed. Parameter %s is invalid. Reason: %s.` | Cause: N/A.<br>Solution: N/A. | 无法给出参数具体值 | `stream_c.cc:903`<br>`RT_LOG_OUTER_MSG_WITH_FUNC_DESC(ErrorCode::EE1017, "Writing data to the specified memory", "addr", "address is not aligned by awsize");` | `Writing data to the specified memory failed. Parameter addr is invalid. Reason: address is not aligned by awsize. ErrorCode=EE1017.` | 两个或多个用户参数关系不满足时也可以使用，如用户参数A和B不满足A<B，具体参数值和参数关系在Reason里呈现。func 参数优先使用语义化描述 |
| EE1018 | 输入参数非法 · API_Call_Sequence | `func, reason` | `%s failed. Reason: %s.` | Cause: N/A.<br>Solution: N/A. | API调用顺序错误 | `runtime.cc:3667`<br>`RT_LOG_OUTER_MSG_WITH_FUNC_DESC(ErrorCode::EE1018, "Starting profiling analysis", "profiler is already enabled, cannot set repeatedly");` | `Starting profiling analysis failed. Reason: profiler is already enabled, cannot set repeatedly. ErrorCode=EE1018.` | Reason应明确正确调用顺序；func 参数优先使用语义化描述 |
| EE1019 | 执行错误 · Execution_Error | `func, reason` | `%s failed. Reason: %s.` | Cause: N/A.<br>Solution: 1. Reduce the number of tasks delivered to the same stream. 2. Use multiple streams to submit tasks in parallel. | 流任务下发失败，包括流任务队列满且无回收机制的场景（如 persistent stream、bound stream） | `stream_xpu.cc:105`<br>`RT_LOG_OUTER_MSG_IMPL(ErrorCode::EE1019, "Adding task to stream", "Stream task public buffer is full");` | `Adding task to stream failed. Reason: Stream task public buffer is full. ErrorCode=EE1019.` | 流任务满且无回收机制（如 persistent stream、bound stream 无法 reclaim task）时，应使用 EE1019 报外部错误，不应仅打普通日志或使用 EE9999 |
| EE1020 | 其他 · 标准函数失败 | `func1, func2, ret_code, reason, extend_info` | `%s failed. Reason: Standard function %s failed. [Errno %s] %s. %s` | Cause: N/A.<br>Solution: N/A. | 标准库函数失败，如memcpy_s | ①`task_to_sqe.cc:99`<br>`const errno_t ret = memcpy_s(...); COND_RETURN_AND_MSG_OUTER(ret != EOK, RT_ERROR_SEC_HANDLE, ErrorCode::EE1020, __func__, "memcpy_s", std::to_string(ret), strerror(ret), "src=..., dest=..., dest_max=..., count=...");`<br>②用户入参示例：`api_c.cc:3999`<br>`const errno_t ret = memcpy_s(&data, sizeof(data), userdata, static_cast<size_t>(length)); if (ret != EOK) { std::stringstream ss; ss << std::hex << "dest=0x" << RtPtrToValue(&data) << ", userdata=0x" << RtPtrToValue(userdata) << std::dec << ", destMax=" << sizeof(data) << ", length=" << length << "."; RT_LOG_OUTER_MSG_IMPL(ErrorCode::EE1020, __func__, "memcpy_s", std::to_string(ret).c_str(), strerror(ret), ss.str().c_str()); }` | `TaskToSqe failed. Reason: Standard function memcpy_s failed. [Errno 22] Invalid argument. src=... ErrorCode=EE1020.`<br>②`rtsProfTrace failed. Reason: Standard function memcpy_s failed. [Errno 22] Invalid argument. dest=0x7f..., userdata=0x7f..., destMax=48, length=64. ErrorCode=EE1020.` | 如参数是用户传入，extend_info 应打印外部入参的参数名及对应的值或地址，而非仅打印默认的 src、dst 等；地址打印使用十六进制格式 |
...
| EE1023 | 资源错误 · Resource_Error | `func, reason` | `%s failed. Reason: %s.` | Cause: N/A<br>Solution: N/A | NPU硬件调度资源不足 | `stream_jetty_context.cc:62`<br>`RT_LOG_OUTER_MSG_IMPL(ErrorCode::EE1023, "Expanding the capacity", "There are too many asynchronous copy tasks. 1. If the value of numBatches for aclrtMemcpyBatchAsync is too large, reduce the value of numBatches. 2. If the value of height for aclrtMemcpy2dAsync is too large, reduce the value of height");` | `Expanding the capacity failed. Reason: There are too many asynchronous copy tasks. 1. If the value of numBatches for aclrtMemcpyBatchAsync is too large, reduce the value of numBatches. 2. If the value of height for aclrtMemcpy2dAsync is too large, reduce the value of height. ErrorCode=EE1023.` | / |
...
| EH0007 ⭐ | 输入参数非法 · Invalid_Argument | `func, value, param, expect` | `%s failed because value %s for parameter %s is invalid. Expected value: %s.` | Cause: N/A.<br>Solution: 1. Check the input parameter range of the function. 2. Check the function invocation relationship. | 能输出非法值+参数名可打印+期望值 | `memory.cpp:191`<br>`acl::AclErrorLogManager::ReportInputError(acl::INVALID_VALUE_MSG, {"func", "value", "param", "expect"}, {"Memory copy type conversion", acl::GetMemcpyKindDesc(kind), "kind", "ACL_MEMCPY_HOST_TO_DEVICE(1) or ..."});` | `Memory copy type conversion failed because value ACL_MEMCPY_HOST_TO_HOST for parameter kind is invalid. Expected value: ACL_MEMCPY_HOST_TO_DEVICE(1) or ... ErrorCode=EH0007.` | ⭐ ACL层首选码；非接口层代码 func 参数优先使用语义化描述 |
| EH0008 ⭐ | 输入参数非法 · Null_Pointer(含函数名) | `func, param` | `%s failed because %s cannot be a NULL pointer.` | Cause: N/A.<br>Solution: Try again with a correct pointer argument. | NULL指针含函数名 | `memory.cpp:217`<br>`ACL_REQUIRES_NOT_NULL_WITH_INPUT_REPORT_AND_FUNC_DESC(dst, "Checking the synchronous memory copy parameter validity");` | `Checking the synchronous memory copy parameter validity failed because dst cannot be a NULL pointer. ErrorCode=EH0008.` | ⭐ 空指针专用错误码；非接口层代码优先使用 `_AND_FUNC_DESC` 变体 |
| EH0009 | 输入参数非法 · Invalid_Argument | `func, value, param, reason` | `%s failed. Value %s for parameter %s is invalid. Reason: %s.` | Cause: N/A.<br>Solution: 1. Check the input parameter range of the function. 2. Check the function invocation relationship. | 无法给出期望值，能给出具体非法值+原因 | `memory.cpp:231`<br>`acl::AclErrorLogManager::ReportInputError(acl::INVALID_PARAM_REASON_MSG, {"func", "value", "param", "reason"}, {"Checking the synchronous memory copy parameter validity", widthVal.c_str(), "width", errMsg.c_str()});` | `Checking the synchronous memory copy parameter validity failed. Value 2048 for parameter width is invalid. Reason: must be less than spitch and dpitch, spitch=1024, dpitch=1024. ErrorCode=EH0009.` | 能给出具体非法值→EH0009；不能→EH0012；非接口层代码 func 参数优先使用语义化描述 |
...
| EH0013 | 其他 · 标准函数失败 | `func1, func2, ret_code, reason, extend_info` | `%s failed. Reason: Standard function %s failed. [Errno %s] %s. %s` | Cause: N/A.<br>Solution: N/A. | ACL层标准库函数失败 | ①`queue.cpp:38`<br>`const auto ret = memcpy_s(...); if (ret != EOK) { ... acl::AclErrorLogManager::ReportInputError(acl::STANDARD_FUNC_FAILED_MSG, {"func1", "func2", "ret_code", "reason", "extend_info"}, {"Parameter copy", "memcpy_s", retVal.c_str(), strerror(ret), extendInfo.c_str()}); }`<br>②用户入参示例：`memory.cpp:1581`<br>`const auto ret = memcpy_s(dataPtr, size, srcAddr, size); if (ret != EOK) { std::stringstream ss; ss << std::hex << "src=0x" << reinterpret_cast<uintptr_t>(srcAddr) << ", dataPtr=0x" << reinterpret_cast<uintptr_t>(dataPtr) << std::dec << ", size=" << size << ", count=" << size << "."; acl::AclErrorLogManager::ReportInputError(acl::STANDARD_FUNC_FAILED_MSG, {"func1", "func2", "ret_code", "reason", "extend_info"}, {funcName.c_str(), "memcpy_s", retVal.c_str(), strerror(ret), extendInfo.c_str()}); }` | `Parameter copy failed. Reason: Standard function memcpy_s failed. [Errno 22] Invalid argument. src=... ErrorCode=EH0013.`<br>②`aclrtGetBufUserData failed. Reason: Standard function memcpy_s failed. [Errno 22] Invalid argument. src=0x7f..., dataPtr=0x7f..., size=4096, count=4096. ErrorCode=EH0013.` | 如参数是用户传入，extend_info 应打印外部入参的参数名及对应的值或地址，而非仅打印默认的 src、dst 等；地址打印使用十六进制格式；func1 参数优先使用语义化描述 |
...
|--------|--------|---------------------------|---------|----------|----------|
| **1** | **EE1015** | `%s failed. Reason: The driver version capability is insufficient. %s` | func, reason | 驱动版本不正确，对应的驱动接口找不到，通过升级驱动包可解决 | 驱动版本不支持 halxxx 接口 |
...
RT_LOG_OUTER_MSG_WITH_FUNC_DESC(ErrorCode::EE1012, "Setting the current device for the thread", "3", "deviceId", "must be in [0,7]");
// 输出：Setting the current device for the thread failed. Value 3 for deviceId is invalid. — 缺少"parameter"不够正式

// ✓ 正确：参数名为 "deviceId"，加"parameter"通顺 → 用 EE1011
RT_LOG_OUTER_MSG_WITH_FUNC_DESC(ErrorCode::EE1011, "Setting the current device for the thread", "3", "deviceId", "must be in [0,7]");
// 输出：Setting the current device for the thread failed. Value 3 for parameter deviceId is invalid.

// ✓ 正确：参数名为 "queue name length"，加"parameter"冗余 → 用 EE1012
RT_LOG_OUTER_MSG_WITH_FUNC_DESC(ErrorCode::EE1012, "Setting the queue attribute", "256", "queue name length", "must be <= 128");
// 输出：Setting the queue attribute failed. Value 256 for queue name length is invalid. — 自然通顺
```
...
// ✓ 正确：用 EE1016‑版本相关不支持
EE1016: func="Setting the error reporting mode", reason="This feature requires CANN 3.x. CANN 2.x and 3.x packages are incompatible. Please upgrade the CANN package to 3.x."
// 输出：Setting the error reporting mode failed. Reason: This feature requires CANN 3.x. CANN 2.x and 3.x packages are incompatible. Please upgrade the CANN package to 3.x.
```
...
// 输出：Setting the stream error reporting mode failed. stop-to-continue mode switch is not supported. Reason: ...
// 问题：这不是配置参数的版本依赖问题，而是固有限制

// ✓ 正确：用 EE1016‑固有不支持
EE1016: func="Setting the stream error reporting mode", reason="Changing stop mode to continue mode is intrinsically not supported and cannot be resolved by upgrading."
// 输出：Setting the stream error reporting mode failed. Reason: Changing stop mode to continue mode is intrinsically not supported and cannot be resolved by upgrading.
```
...
COND_RETURN_AND_MSG_OUTER(stm->GetBindFlag(), RT_ERROR_STREAM_INVALID, ErrorCode::EE1006, "Clearing tasks in a stream",
    "Clearing model stream", "xxx");
// 输出：Clearing tasks in a stream failed. Clearing model stream is not supported. Reason: xxx. ErrorCode=EE1006.
...
// 输出：Clearing tasks in a stream failed. Reason: Clearing model stream is not supported. ErrorCode=EE1016.
```
