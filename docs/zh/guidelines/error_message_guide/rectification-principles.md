...
- 同一次 ErrMsg 输出中禁止出现完全相同的结构化消息。
- 错误码或措辞不同，但表达同一失败事实且没有有效信息增量的，属于实质相同，也禁止出现。
- 调用栈中尽量去除冗余 ErrMsg。中间层仅表达“调用失败”或重复下层根因时，删除中间层上报。
- 首错点信息不全，但 acl/rt 接口层能够补充调用场景或额外定位信息时，保留首错点和接口层上报，删除中间层冗余上报。
- 中间层仅在提供首错点和接口层均无法合理表达的独有信息时保留，且不得与其他层完全相同或实质相同。
- 删除或迁移上报前，确认每条实际可达且本应结构化上报的错误路径仍有一条合适的 ErrMsg，且不改变返回码、清理顺序和其他业务逻辑。
...
- 普通 `RT_LOG_*` 日志不属于结构化重复上报；不能依赖 ErrorManager 的容器去重代替上述检查。
