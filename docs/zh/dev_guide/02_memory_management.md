...
- **2M大页**：内存申请粒度为2MB，不足2MB的倍数会向上按2MB对齐。大页可以减少页表项数量，扩大TLB覆盖范围，适合较大、长期驻留、访问频繁的输入、输出、workspace等Device内存。
- **1G大页**：内存申请粒度为1GB，仅部分产品支持。与2MB大页相比，1GB大页能进一步减少页表项数量，例如1GB内存使用2MB大页需要512个页表项，而使用1GB大页只需要1个页表项。该策略适合超大块、长期驻留并且对访问性能敏感的内存，但会带来更明显的容量占用和对齐浪费。
