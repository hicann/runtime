...
**须知：`ACL_RT_ATOMIC_CAPABILITY_REDUCATION`枚举项在CANN 9.2.0版本标记为废弃，将在2027年9月30日之后的版本删除，替换为：`ACL_RT_ATOMIC_CAPABILITY_REDUCTION`枚举项。**
...
typedef enum aclrtAtomicOperationCapability {
    ...
    ACL_RT_ATOMIC_CAPABILITY_REDUCTION = 1U << 2,    // 归约操作
...
**须知：`ACL_RT_BINARY_LOAD_OPT_LAZY_MAGIC`枚举项在CANN 8.5.0版本标记为废弃，将在2026年12月30日之后的版本删除，替换为：`ACL_RT_BINARY_LOAD_OPT_MAGIC`枚举项。**
...
**须知：`ACL_DEV_ATTR_LOCAL_MEM_PER_VECTOR_CORE`枚举项在CANN 9.0.0版本标记为废弃，将在2027年3月30日之后的版本删除，替换为：`ACL_DEV_ATTR_UBUF_PER_VECTOR_CORE`枚举项。**

**须知：`ACL_DEV_ATTR_SUPER_POD_DEVIDE_ID`枚举项在CANN 9.0.0版本标记为废弃，将在2027年3月30日之后的版本删除，替换为：`ACL_DEV_ATTR_SUPER_POD_DEVICE_ID`枚举项。**
...
**须知：`ACL_RT_LAUNCH_KERNEL_ATTR_LOCAL_MEMORY_SIZE`枚举项在CANN 9.0.0版本标记为废弃，将在2027年3月30日之后的版本删除，替换为：`ACL_RT_LAUNCH_KERNEL_ATTR_DYN_UBUF_SIZE`枚举项。**
...
typedef enum aclrtMemLocationType {
    ...
    ACL_MEM_LOCATION_TYPE_MANAGED,       // UVM类型内存(Unified Virtual Memory, 统一虚拟内存)
...
| 枚举项 | 说明 |
| --- | --- |
| ACL_OPT_DETERMINISTIC | 是否开启确定性计算。<br><br>取值范围 [0, 4)：<br>  - 0：不开启确定性计算。默认不开启。<br>  - 1：开启确定性计算。<br>  - 2：开启强一致性计算。计算结果确定，且与数据的位置无关（等价于已不推荐使用的ACL_OPT_STRONG_CONSISTENCY）。<br>  - 3：开启Batch一致性计算。在机器学习或高性能计算中，将一个样本放入任意大小的Batch（批次）中、或者调整其在Batch中的排列顺序，该样本最终输出的计算结果都必须保持位级（Bit-wise）的完全相同。简单来说，就是："单样本的计算结果，与它和谁一组（Batch Size）、排在第几个（Batch Index）毫无关系。"<br><br>当开启确定性计算功能时，算子在相同的硬件和输入下，多次执行将产生相同的输出。但启用确定性计算往往导致算子执行变慢。<br>默认情况下，不开启确定性计算，算子在相同的硬件和输入下，多次执行的结果可能不同。这个差异的来源，一般是因为在算子实现中，存在异步的多线程执行，会导致浮点数累加的顺序变化。<br>通常建议不开启确定性计算，因为确定性计算往往会导致算子执行变慢，进而影响性能。当发现模型多次执行结果不同，或者是进行精度调优时，可开启确定性计算，辅助模型调试、调优。 |
