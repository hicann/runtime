...
用户自定义模块可使用`acl_log.h`中的`acllogRecord`、`acllogVaList`和`acllogCheckDebugLevel`接口记录日志。用户moduleId范围为`0xff00`~`0xffff`（65280~65535）。如需记录运行日志，可将用户moduleId与`RUN_LOG_MASK`按位或后传入，日志会落盘到run目录，运行日志不会记录debug级别日志，moduleId携带`RUN_LOG_MASK`时level不可为`DLOG_DEBUG`，此时该条日志不会落盘；未携带日志类型掩码时，默认按调测日志处理。该范围内的moduleId会在日志中按十进制ID记录，不转换为moduleName字符串。CANN内部模块moduleId范围为`0x0000`~`0x00ff`（0~255）。
...
| level | 输入 | 本条日志级别，宏定义请参见[数据类型定义](#数据类型定义)。<br> 说明： 运行日志不会记录debug级别日志，moduleId携带RUN_LOG_MASK时不可传入DLOG_DEBUG，此时该条日志不会落盘。 |
