...
| #define  ACL_ERROR_RT_STREAM_UNJOINED  107025 | 跨流捕获场景下，子流的最后一个任务不是event record任务，或者子流的最后一个event record任务没有对应的event wait任务，或者子流没有返回到主流。 | - 子流的最后一个任务不是event record任务。在报错的子流最后下发一个event record任务（调用aclrtRecordEvent接口）。<br>  - 子流的最后一个event record任务没有对应的event wait任务。在非报错的Stream上下发一个event wait任务（调用aclrtStreamWaitEvent接口）。<br>  - 子流没有返回到主流。通过合理下发event record和event wait任务，使子流返回到主流。 |
...
| #define  ACL_ERROR_RT_MODEL_UPDATE_FAILED  107040 | 模型未完全捕获、模型正在执行中，或参数更新失败导致模型更新失败。 | 确保模型已成功捕获、执行完成或参数更新成功。 |
...
| #define  ACL_ERROR_RT_SYMBOL_NOT_FOUND  107042 | 符号未找到。 | 请检查全局变量名称是否与算子二进制中的全局变量名称一致。 |
| #define  ACL_ERROR_RT_INVALID_SYMBOL  107043 | 无效的符号。 | 请检查变量名是否在Device代码中声明 |
| #define  ACL_ERROR_RT_INVALID_DEVICE_FUNCTION  107044 | 无效的核函数符号。 | 请检查核函数是否在Device代码中声明。 |
...
| #define  ACL_ERROR_RT_TIMEOUT  207020 | Driver执行超时。 | 您可以获取日志后单击[Link](https://www.hiascend.com/support)联系技术支持。 |
