...
| ACL_DEV_ATTR_HD_CONNECT_TYPE | Host和Device间的互联协议。<br>取值如下：<br><br>  - 0：通过PCIe互联传输。<br>  - 1：通过HCCS（Huawei Cache Coherence System，华为缓存一致性系统）互联传输。<br>  - 2：通过UB（Unified Bus，统一总线）互联传输。<br>Ascend 950PR/Ascend 950DT支持该选项，其他产品型号不支持。 |
...
| ACL_STREAM_ATTR_FLOAT_OVERFLOW_CHECK | 当与上层训练框架（例如PyTorch）对接时，针对指定Stream，可通过该属性打开或关闭溢出检测开关。关闭后，将无法通过溢出检测算子获取任务是否溢出。<br>打开或关闭溢出检测开关后，仅对后续新下的任务生效，已下发的任务仍维持原样。<br>通过该属性设置溢出检测开关，与[aclrtSetStreamOverflowSwitch](06_Stream管理.md#aclrtSetStreamOverflowSwitch)接口的功能一致。 |
...
| ACL_STREAM_ATTR_PRIORITY | 基于该属性值动态设置/查询stream优先级。当前支持的芯片类型: <br>Atlas A3 训练系列产品/Atlas A3 推理系列产品<br>Atlas A2 训练系列产品/Atlas A2 推理系列产品<br>Ascend 950PR/Ascend 950DT |
