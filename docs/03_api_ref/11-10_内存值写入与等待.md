...
| stream | 输入 | 执行写数据任务的stream。类型定义请参见[aclrtStream](24_数据类型及其操作接口.md#aclrtStream)。<br>此处支持传NULL，表示使用默认Stream。 |
...
| stream | 输入 | 执行等待任务的stream。类型定义请参见[aclrtStream](24_数据类型及其操作接口.md#aclrtStream)。<br>此处支持传NULL，表示使用默认Stream。 |
