...
| 参数名 | 输入/输出 | 说明 |
| --- | :---: | --- |
| handle | 输出 | 存放物理内存信息的handle。类型定义请参见[aclrtDrvMemHandle](24_数据类型及其操作接口.md#aclrtDrvMemHandle)。 |
...
| prop | 输入 | 物理内存属性信息。类型定义请参见[aclrtPhysicalMemProp](24_数据类型及其操作接口.md#aclrtPhysicalMemProp)。 |
...
| 参数名 | 输入/输出 | 说明 |
| --- | :---: | --- |
| handle | 输入 | 待释放的物理内存信息handle。类型定义请参见[aclrtDrvMemHandle](24_数据类型及其操作接口.md#aclrtDrvMemHandle)。 |
...
| 参数名 | 输入/输出 | 说明 |
| --- | :---: | --- |
| handle | 输入 | 存放物理内存信息的handle。类型定义请参见[aclrtDrvMemHandle](24_数据类型及其操作接口.md#aclrtDrvMemHandle)。<br>需先在本进程调用aclrtMallocPhysical接口申请物理内存，该接口调用成功，会返回一个handle。<br>handle与shareableHandle是一一对应的关系，在同一个进程中，不允许一对多、或多对一，否则报错，例如重复调用本接口导出时则会返回报错。 |
| handleType | 输入 | 预留参数，当前固定填ACL_MEM_HANDLE_TYPE_NONE。<br>类型定义请参见[aclrtMemHandleType](24_数据类型及其操作接口.md#aclrtMemHandleType)。 |
...
| handle | 输出 | 本进程的物理内存handle。类型定义请参见[aclrtDrvMemHandle](24_数据类型及其操作接口.md#aclrtDrvMemHandle)。 |
...
| shareType | 输入 | 导出的共享句柄类型。类型定义请参见[aclrtMemSharedHandleType](24_数据类型及其操作接口.md#aclrtMemSharedHandleType)。 |
...
| --- | :---: | --- |
| prop | 输入 | 物理内存属性信息。类型定义请参见[aclrtPhysicalMemProp](24_数据类型及其操作接口.md#aclrtPhysicalMemProp)。 |
| option | 输入 | 最小粒度或推荐粒度。类型定义请参见[aclrtMemGranularityOptions](24_数据类型及其操作接口.md#aclrtMemGranularityOptions)。 |
...
| desc | 输入 | 内存访问的配置信息，包含内存访问保护标志、内存所在位置等。类型定义请参见[aclrtMemAccessDesc](24_数据类型及其操作接口.md#aclrtMemAccessDesc)。 |
| count | 输入 | desc数组长度。 |
...
| --- | :---: | --- |
| virPtr | 输入 | 虚拟内存的起始地址。<br>必须与[aclrtMapMem](#aclrtMapMem)接口的virPtr地址相同。 |
| location | 输入 | 内存所在位置。类型定义请参见[aclrtMemLocation](24_数据类型及其操作接口.md#aclrtMemLocation)。<br>当前仅支持将aclrtMemLocation.type设置为ACL_MEM_LOCATION_TYPE_HOST或ACL_MEM_LOCATION_TYPE_DEVICE。当aclrtMemLocation.type为ACL_MEM_LOCATION_TYPE_HOST时，[aclrtMemLocation](24_数据类型及其操作接口.md#aclrtMemLocation).id无效，固定设置为0即可。 |
| flag | 输出 | 内存访问保护标志。 |
...
| 参数名 | 输入/输出 | 说明 |
| --- | :---: | --- |
| handle | 输入 | 存放物理内存信息的handle。类型定义请参见[aclrtDrvMemHandle](24_数据类型及其操作接口.md#aclrtDrvMemHandle)。<br>查询通过aclrtMallocPhysical接口申请的物理内存属性信息。 |
| prop | 输出 | 物理内存属性信息。类型定义请参见[aclrtPhysicalMemProp](24_数据类型及其操作接口.md#aclrtPhysicalMemProp)。 |
