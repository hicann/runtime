...
| size | 输入 | 内存大小，单位Byte。 |
| type | 输入 | 内存注册类型。类型定义请参见[aclrtHostRegisterType](24_数据类型及其操作接口.md#aclrtHostRegisterType)。 |
...
| hacType | 输入 | HAC（Hardware Accelerator Controller，硬件加速控制器）类型。类型定义请参见[aclrtHacType](24_数据类型及其操作接口.md#aclrtHacType)。 |
| capabilities | 输出 | 是否支持访问通过aclrtHostRegister接口注册的内存。类型定义请参见[aclrtHostMemMapCapability](24_数据类型及其操作接口.md#aclrtHostMemMapCapability)。 |
