...
| 参数名 | 输入/输出 | 说明 |
| --- | :---: | --- |
| attr | 输入 | 队列属性配置信息的指针。类型定义请参见[acltdtQueueAttr](24_数据类型及其操作接口.md#acltdtQueueAttr)。<br>需提前调用[acltdtCreateQueueAttr](24_数据类型及其操作接口.md#acltdtCreateQueueAttr)接口创建acltdtQueueAttr类型的数据，再调用[acltdtSetQueueAttr](24_数据类型及其操作接口.md#acltdtSetQueueAttr)接口设置队列属性值（例如，队列名）。<br>若该参数为nullptr时，则使用队列属性默认值，即队列名系统自动分配，队列深度默认为8。 |
| qid | 输出 | 队列ID的指针。 |
...
| --- | :---: | --- |
| qid | 输入 | 需要添加数据的队列。<br>队列需提前调用[acltdtCreateQueue](#acltdtCreateQueue)接口创建。 |
| buf | 输入 | 共享Buffer指针。类型定义请参见[acltdtBuf](24_数据类型及其操作接口.md#acltdtBuf)。<br>该内存必须提前调用[acltdtAllocBuf](17-03_共享Buffer管理.md#acltdtAllocBuf)接口申请。 |
...
| 参数名 | 输入/输出 | 说明 |
| --- | :---: | --- |
| qRouteList | 输入/输出 | 路由关系数组的指针，接口调用完成后返回路由绑定结果。类型定义请参见[acltdtQueueRouteList](24_数据类型及其操作接口.md#acltdtQueueRouteList)。<br>需提前调用[acltdtCreateQueueRouteList](24_数据类型及其操作接口.md#acltdtCreateQueueRouteList)接口创建acltdtQueueRouteList类型的数据，再调用[acltdtAddQueueRoute](24_数据类型及其操作接口.md#acltdtAddQueueRoute)接口添加路由关系。 |

### 返回值说明

返回0表示成功，返回其他值表示失败，请参见[aclError](24_数据类型及其操作接口.md#aclError)。

只有当所有队列关系绑定成功且路由状态正常时，本接口才会返回成功；任何一条绑定失败，本接口返回失败，如果您需要知道具体哪个路由关系绑定失败，您可以先调用[acltdtGetQueueRoute](24_数据类型及其操作接口.md#acltdtGetQueueRoute)接口从路由关系数组中获取每一个路由关系，再调用[acltdtGetQueueRouteParam](24_数据类型及其操作接口.md#acltdtGetQueueRouteParam)接口查询绑定关系状态。
...
| 参数名 | 输入/输出 | 说明 |
| --- | :---: | --- |
| qRouteList | 输入/输出 | 路由关系数组的指针，接口调用完成后返回路由去绑定结果。类型定义请参见[acltdtQueueRouteList](24_数据类型及其操作接口.md#acltdtQueueRouteList)。<br>可先通过[acltdtQueryQueueRoutes](#acltdtQueryQueueRoutes)获取路由关系数组。 |

### 返回值说明

返回0表示成功，返回其他值表示失败，请参见[aclError](24_数据类型及其操作接口.md#aclError)。

只有当所有队列关系解绑定成功，本接口才会返回成功；任何一条解绑定失败，本接口返回失败，如果您需要知道具体哪个路由关系解绑定失败，您可以先调用[acltdtGetQueueRoute](24_数据类型及其操作接口.md#acltdtGetQueueRoute)接口从路由关系数组中获取每一个路由关系，再调用[acltdtGetQueueRouteParam](24_数据类型及其操作接口.md#acltdtGetQueueRouteParam)接口查询绑定关系状态。
...
| 参数名 | 输入/输出 | 说明 |
| --- | :---: | --- |
| queryInfo | 输入 | 查询条件的指针。类型定义请参见[acltdtQueueRouteQueryInfo](24_数据类型及其操作接口.md#acltdtQueueRouteQueryInfo)。<br>需提前调用[acltdtCreateQueueRouteQueryInfo](24_数据类型及其操作接口.md#acltdtCreateQueueRouteQueryInfo)接口创建acltdtQueueRouteQueryInfo类型的数据。 |
| qRouteList | 输入&输出 | 路由关系数组的指针。类型定义请参见[acltdtQueueRouteList](24_数据类型及其操作接口.md#acltdtQueueRouteList)。<br>需提前调用[acltdtCreateQueueRouteList](24_数据类型及其操作接口.md#acltdtCreateQueueRouteList)接口创建acltdtQueueRouteList类型的数据。 |
