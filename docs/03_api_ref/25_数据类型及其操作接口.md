...
| 成员名 | 说明 |
| --- | --- |
| handle | 条件句柄。类型定义请参见[aclmdlRICondHandle](#aclmdlRICondHandle)。 |
| type | 条件类型。类型定义请参见[aclmdlRICondTaskType](#aclmdlRICondTaskType)。 |
| size | 分支数量。IF条件取1或2，WHILE条件取1，SWITCH条件取大于0的值。 |
| modelRIArray | 子图模型运行实例数组。类型定义请参见[aclmdlRI](#aclmdlRI)。 |
