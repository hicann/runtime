...
流等待事件接口的简化重载版本。内部调用 [aclrtStreamWaitEventWithTimeout](07_Event管理.md#aclrtStreamWaitEventWithTimeout)。
...
设置算子执行超时时间的简化重载版本。内部调用 [aclrtSetOpExecuteTimeOutV2](12_执行控制.md#aclrtSetOpExecuteTimeOutV2)。
...
| policy | 输入 | 内存分配策略。类型定义请参见[aclrtMemMallocPolicy](25-02_Enumerations.md#aclrtMemMallocPolicy)。 |
...
| kind | 输入 | 拷贝类型。类型定义请参见[aclrtMemcpyKind](25-02_Enumerations.md#aclrtMemcpyKind)。 |
