...
| cmoType | 输入 | Cache内存操作类型。类型定义请参见[aclrtCmoType](24_数据类型及其操作接口.md#aclrtCmoType)。<br>当前仅支持ACL_RT_CMO_TYPE_PREFETCH（内存预取）。 |
| stream | 输入 | 执行内存操作任务的Stream。类型定义请参见[aclrtStream](24_数据类型及其操作接口.md#aclrtStream)。 |
...
| src | 输入 | 待操作的Device内存地址。<br>只支持本Device上的Cache内存操作。 |
| size | 输入 | 待操作的Device内存大小，单位Byte。 |
| cmoType | 输入 | Cache内存操作类型。类型定义请参见[aclrtCmoType](24_数据类型及其操作接口.md#aclrtCmoType)。 |
...
| stream | 输入 | 执行内存操作任务的Stream。类型定义请参见[aclrtStream](24_数据类型及其操作接口.md#aclrtStream)。<br>此处只支持与模型绑定过的Stream，绑定模型与Stream需调用[aclmdlRIBindStream](15_模型运行实例管理.md#aclmdlRIBindStream)接口。 |
...
| 参数名 | 输入/输出 | 说明 |
| --- | :---: | --- |
| taskInfo | 输入 | Cache内存操作的任务信息。类型定义请参见[aclrtBarrierTaskInfo](24_数据类型及其操作接口.md#aclrtBarrierTaskInfo)。<br>任务信息中的cmoType当前仅支持ACL_RT_CMO_TYPE_INVALID。 |
| stream | 输入 | 执行等待任务的Stream。类型定义请参见[aclrtStream](24_数据类型及其操作接口.md#aclrtStream)。<br>此处只支持与模型绑定过的Stream，绑定模型与Stream需调用[aclmdlRIBindStream](15_模型运行实例管理.md#aclmdlRIBindStream)接口。 |
