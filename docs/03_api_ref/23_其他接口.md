...
- [`int32_t AlogCheckDebugLevel(uint32_t moduleId, int32_t level)`](#AlogCheckDebugLevel)
- [`void AlogRecord(uint32_t moduleId, uint32_t logType, int32_t level, const char *fmt, ...)`](#AlogRecord)
...
- [`float aclFloat16ToFloat(aclFloat16 value)`](#aclFloat16ToFloat)：将[aclFloat16](24_数据类型及其操作接口.md#aclFloat16)类型的数据转换为float（指float32）类型的数据。
- [`aclFloat16 aclFloatToFloat16(float value)`](#aclFloatToFloat16)：将float（指float32）类型的数据转换为[aclFloat16](24_数据类型及其操作接口.md#aclFloat16)类型的数据。
...
- [aclsysGetCANNVersion](#aclsysGetCANNVersion)
...
| 参数 | 输入/输出 | 说明 |
| --- | :---: | --- |
| moduleId | 输入 | 模块ID，枚举值请参见[数据类型定义](数据类型定义.md)。 |
| level | 输入 | 日志级别，宏定义请参见[数据类型定义](数据类型定义.md)。 |
...
| --- | :---: | --- |
| moduleId | 输入 | 模块ID，枚举值请参见[数据类型定义](数据类型定义.md)。 |
| logType | 输入 | 本条日志类型，枚举值请参见[数据类型定义](数据类型定义.md)。传入其他值视作调试日志。 |
| level | 输入 | 本条日志级别，宏定义请参见[数据类型定义](数据类型定义.md)。<br> 说明： 运行日志不会记录debug级别日志，DLOG_TYPE_RUN和DLOG_DEBUG不可组合使用。 |
...
| 参数名 | 输入/输出 | 说明 |
| --- | :---: | --- |
| dataType | 输入 | 指定要获取大小的aclDataType数据。类型定义请参见[aclDataType](24_数据类型及其操作接口.md#aclDataType)。 |
...
将[aclFloat16](24_数据类型及其操作接口.md#aclFloat16)类型的数据转换为float（指float32）类型的数据。
...
| 参数名 | 输入/输出 | 说明 |
| --- | :---: | --- |
| value | 输入 | 待转换的数据。类型定义请参见[aclFloat16](24_数据类型及其操作接口.md#aclFloat16)。 |
...
将float（指float32）类型的数据转换为[aclFloat16](24_数据类型及其操作接口.md#aclFloat16)类型的数据。
...
| 参数名 | 输入/输出 | 说明 |
| --- | :---: | --- |
| name | 输入 | 指定要查询的软件包。类型定义请参见[aclCANNPackageName](24_数据类型及其操作接口.md#aclCANNPackageName)。<br>若指定要查询的软件包没有安装，则本接口返回报错。 |
| version | 输出 | CANN软件包版本号。类型定义请参见[aclCANNPackageVersion](24_数据类型及其操作接口.md#aclCANNPackageVersion)。 |
...
| 参数名 | 输入/输出 | 说明 |
| --- | :---: | --- |
| cannAttrList | 输出 | 用于保存运行环境支持的特性枚举数组。类型定义请参见[aclCannAttr](24_数据类型及其操作接口.md#aclCannAttr)。<br>用户无需提前申请内存，应用进程退出时，内存自动释放。 |
...
| 参数名 | 输入/输出 | 说明 |
| --- | :---: | --- |
| cannAttr | 输入 | 特性列表枚举值，一次查询可指定其中一项。类型定义请参见[aclCannAttr](24_数据类型及其操作接口.md#aclCannAttr)。 |
...
| deviceInfo | 输入 | 指定Device上的硬件规格类型。类型定义请参见[aclDeviceInfo](24_数据类型及其操作接口.md#aclDeviceInfo)。 |
| value | 输出 | 硬件规格值。 |
...
8.  开启采集性能数据（参见[Profiling数据采集接口](Profiling数据采集接口.md)章节下的接口）后，调用[aclmdlRIExecuteAsync](15_模型运行实例管理.md#aclmdlRIExecuteAsync)接口执行推理。
...
| stream | 输入 | 指定执行打点任务的Stream。类型定义请参见[aclrtStream](24_数据类型及其操作接口.md#aclrtStream)。 |
