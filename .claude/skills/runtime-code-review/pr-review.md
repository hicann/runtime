...
8. 按共享规则加载规范文档：
    - `source_files`：`docs/zh/guidelines/coding-guidelines.md`
    ...
    - `doc_files`：必要时结合 `docs/zh/guidelines/design_document_template.md`
9. 按共享规则逐文件审查
10. 对发现的问题做验证并过滤误报
11. 使用共享规则中定义的格式输出结果
...
- **必须在终端输出完整审查结果**
- 如果未发现问题，明确输出"未发现问题"
- **不要在审查完成后自动执行任何 GitCode API POST 操作**

### 可选输出到 GitCode（需用户明确触发）

**关键约束**：只有在用户**明确要求**时才能发布评论。

#### 触发条件检查清单

在执行任何 POST 操作前，必须验证：

- [ ] 用户消息中包含明确关键词："发布"、"提交"、"post"、"发评论"
- [ ] 已向用户展示审查摘要
- [ ] 用户已回复确认（或无反对）
- [ ] **不是在审查完成后的首次响应**（不能因为"完成了"就自动发布）

**如果任何一项未满足，STOP 并询问用户：**

```
审查已完成。是否需要将审查结果发布到 GitCode？
```
