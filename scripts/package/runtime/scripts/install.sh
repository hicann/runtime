...
update_install_info_feature() {
    ...
    if [ "$featuremode" = "all" ] || [ "$operation" = "Upgrade" ]; then
        update_install_param "Runtime_Feature_Type" "$featuremode" "$install_info"
        return
    fi
    local current_featuremode=$(get_install_param "Runtime_Feature_Type" "$install_info")
...
update_install_info() {
    ...
    update_install_param "Runtime_Install_Type" "$installmode" "$install_info"
    update_install_param "Runtime_Chip_Type" "$chipmode" "$install_info"
    update_install_info_feature
    update_install_param "Runtime_UserName" "$username" "$install_info"
    update_install_param "Runtime_UserGroup" "$usergroup" "$install_info"
    update_install_param "Runtime_Install_Path_Param" "$install_path_param" "$install_info"
    update_install_param "Runtime_Arch_Linux_Path" "$arch_linux_path" "$install_info"
    update_install_param "Runtime_Hetero_Arch_Flag" "$hetero_arch" "$install_info"
}
...
prompt_set_env() {
    ...
    echo "Please make sure that
        - LD_LIBRARY_PATH includes ${install_path}/runtime/lib64"
}
...
install_run() {
    ...
    local runtime_install_path_param=""
    update_install_path
    update_install_info
    local runtime_input_install_path=$(get_install_param "Runtime_Install_Path_Param" "${install_info}")
    local runtime_install_type=$(get_install_param "Runtime_Install_Type" "${install_info}")
    if [ "$is_docker_install" = "y" ]; then
        runtime_install_path_param=$(concat_docker_install_path "${docker_root}" "${runtime_input_install_path}")
    else
        runtime_install_path_param="${runtime_input_install_path}"
    fi
    if [ "$1" = "install" ]; then
        unchattr_files
        chmod_start
        new_echo "INFO" "install ${runtime_install_path_param} ${runtime_install_type}"
        log "INFO" "install ${runtime_install_path_param} ${runtime_install_type}"
        sh "${curpath}/run_runtime_install.sh" "install" "${runtime_input_install_path}" "${runtime_install_type}" \
            "${is_quiet}" "${input_setenv}" "${docker_root}" "${in_install_for_all}" "$pkg_version_dir"
        if [ $? -eq 0 ]; then
            update_version_info_version
            log "INFO" "Runtime package installed successfully! The new version takes effect immediately."
            log_operation "${operation}" "succeeded"
            chmod_end
            prompt_set_env "${install_path_param}"
        else
            chmod_end
            log "ERROR" "Runtime package install failed, please retry after uninstall!"
...
upgrade_run() {
    ...
    if [ -f "$install_info" ]; then
        local runtime_input_install_path=$(get_install_param "Runtime_Install_Path_Param" "${install_info}")
        local runtime_install_type=$(get_install_param "Runtime_Install_Type" "${install_info}")
    elif [ -f "$install_info_old" ] && [ $(grep -c -i runtime_install_path_param "$install_info_old") -ne 0 ]; then
        local runtime_input_install_path="$(grep -iw runtime_install_path_param "$install_info_old" | cut -d"=" -f2-)"
        local runtime_install_type="$(grep -iw runtime_install_type "$install_info_old" | cut -d"=" -f2-)"
    else
    ...
    if [ "$1" = "upgrade" ]; then
        chmod_start
        new_echo "INFO" "upgrade ${runtime_install_path_param} ${runtime_install_type}"
        log "INFO" "upgrade ${runtime_install_path_param} ${runtime_install_type}"
        sh "${curpath}/run_runtime_upgrade.sh" "upgrade" "${runtime_input_install_path}" "${runtime_install_type}" \
            "${is_quiet}" "${input_setenv}" "${docker_root}" "${in_install_for_all}" "$pkg_version_dir"
        if [ $? -eq 0 ]; then
            update_version_info_version
            log "INFO" "Runtime package upgraded successfully! The new version takes effect immediately."
            log_operation "${operation}" "succeeded"
            chmod_end
            prompt_set_env "${install_path_param}"
        else
            chmod_end
            log "ERROR" "Runtime package upgrade failed, please retry after uninstall!"
            log_operation "${operation}" "failed!"
            exit_install_log 1
        fi
    fi
...
uninstall_run() {
    ...
    if [ -f "$upgrade_install_info" ]; then
        local runtime_input_install_path=$(get_install_param "Runtime_Install_Path_Param" "${upgrade_install_info}")
        local runtime_install_type=$(get_install_param "Runtime_Install_Type" "${upgrade_install_info}")
    elif [ -f "$install_info_old" ] && [ $(grep -c -i runtime_install_path_param "$install_info_old") -ne 0 ]; then
        local runtime_input_install_path="$(grep -iw runtime_install_path_param "$install_info_old" | cut -d"=" -f2-)"
        local runtime_install_type="$(grep -iw runtime_install_type "$install_info_old" | cut -d"=" -f2-)"
    else
    ...
    if [ ! -f "$upgrade_default_dir/script/run_runtime_uninstall.sh" ]; then
        log "WARNING" "run_runtime_uninstall.sh not found."
        return $?
    fi
    if [ "$1" = "uninstall" ]; then
        chmod_start "$upgrade_default_dir"
        new_echo "INFO" "uninstall ${runtime_install_path_param} ${runtime_install_type}"
        log "INFO" "uninstall ${runtime_install_path_param} ${runtime_install_type}"
        sh "$upgrade_default_dir/script/run_runtime_uninstall.sh" "uninstall" "${runtime_input_install_path}" "${runtime_install_type}" "${is_quiet}" \
        ...
        if [ $? -eq 0 ]; then
            ...
            if [ $(id -u) -eq 0 ]; then
                if [ "$uninstall" = "y" ] && [ -f "$install_info_old" ] && [ $(grep -c -i runtime_install_path_param "$install_info_old") -ne 0 ]; then
                    sed -i '/runtime_install_path_param=/Id' "$install_info_old" 2> /dev/null
                    sed -i '/runtime_install_type=/Id' "$install_info_old" 2> /dev/null
                fi
            fi
            new_echo "INFO" "Runtime package uninstalled successfully! Uninstallation takes effect immediately."
            log "INFO" "Runtime package uninstalled successfully! Uninstallation takes effect immediately."
            log_operation "${operation}" "succeeded"
        else
            log "ERROR" "Runtime package uninstall failed!"
            log_operation "${operation}" "failed!"
            exit_uninstall_log 1
        fi
    fi
...
judgmentpath() {
    ...
    if [ $? -ne 0 ]; then
        log "ERROR" "The Runtime install_path ${1} is invalid, only characters in [a-z,A-Z,0-9,-,_] are supported!"
        exit 1
    fi
}
...
if [ "$featuremode" != "all" ]; then
    ...
    if [ "$ret" = "false" ]; then
        log "WARNING" "Runtime package doesn't contain features $featuremode, skip installation."
        exit 0
    fi
fi
...
if [ "$pkg_is_multi_version" = "true" ] && [ "$hetero_arch" != "y" ]; then
    default_dir="${pkg_install_path}/$pkg_version_dir/share/info/runtime"
else
    default_dir="${pkg_install_path}/runtime"
fi
...
if [ "$input_pre_check" = "y" ]; then
    log "INFO" "Runtime do pre check started."
    pre_check
    if [ $? -ne 0 ]; then
        log "WARNING" "Runtime do pre check failed."
    else
        log "INFO" "Runtime do pre check finished."
    fi
...
if [ "$check" = "y" ]; then
    ...
    if [ -z "$pkg_version_dir" ]; then
        preinstall_check --install-path="$install_path_param" --script-dir="$curpath" --package="runtime" --logfile="$logfile" --docker-root="$docker_root"
        if [ $? -ne 0 ]; then
            exit_install_log 1
        else
    ...
    ver_check
    if [ -z "$pkg_version_dir" ]; then
        preinstall_process --install-path="$install_path_param" --script-dir="$curpath" --package="runtime" --logfile="$logfile" --docker-root="$docker_root"
        if [ $? -ne 0 ]; then
            exit_install_log 1
        else
            log "INFO" "version compatibility check successfully!"
...
uninstall_none_multi_version "$pkg_install_path/runtime"
check_install_for_all
create_default_install_dir_for_common_user
log_base_version
...
if [ "x$version_installed" != "x" -a "$version_installed" != "none" ] || [ -f "${install_info}" ]; then
    ...
    # 安装场景
        ...
        if [ "$is_quiet" = "n" ]; then
            log "INFO" "Runtime package has been installed on the path ${pkg_install_path}, the version is ${version_installed}, and the version of this package is ${version_of_package}, do you want to continue? [y/n]"
            while true
            do
                read yn
                if test "$yn" = n; then
