...
int main()
{
    ...
    CHECK_ERROR(aclrtMemcpyAsync(
        numDevice, sizeof(uint32_t), targetValDev_1, sizeof(uint32_t), ACL_MEMCPY_DEVICE_TO_DEVICE, stream1));
    ...
    CHECK_ERROR(aclrtMemcpyAsync(
        targetValDev_1, sizeof(uint32_t), &val2, sizeof(uint32_t), ACL_MEMCPY_HOST_TO_DEVICE, copyStream));
    CHECK_ERROR(aclrtSynchronizeStream(copyStream));
    // 将上一次的主机结果清零，以免混淆
