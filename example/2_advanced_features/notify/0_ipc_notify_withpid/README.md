...
## 描述
本样例展示了两个Device、两个进程间的Notify共享，但在共享Notify时启用进程白名单校验。
...
- Notify管理
    ...
    - 调用aclrtNotifySetImportPid接口设置共享Notify的进程白名单。
...
[INFO]  Process B: get Process B's pid successfully, pid = ...
[INFO]  Process A: get Process B's pid successfully, Process B's pid = ...
