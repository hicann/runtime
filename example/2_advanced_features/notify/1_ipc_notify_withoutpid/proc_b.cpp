...
int CheckAndEnablePeerAccess(int32_t deviceId, int32_t peerDeviceId, bool *isSupported)
{
    ...
    ret = aclrtDeviceEnablePeerAccess(peerDeviceId, 0);
    if (ret != ACL_SUCCESS) {
        return static_cast<int>(ret);
    }
    *isSupported = true;
...
int32_t main()
{
    ...
    CHECK_ERROR(memory::InitDeviceAndCreateStream(deviceId, &stream));

    const int32_t peerDeviceId = 2;
    bool isPeerAccessSupported = false;
    CHECK_ERROR(CheckAndEnablePeerAccess(deviceId, peerDeviceId, &isPeerAccessSupported));
