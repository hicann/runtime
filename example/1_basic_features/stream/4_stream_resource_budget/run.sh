...
if grep -q "\[SUCCESS\] Stream resource budget sample completed successfully" "${file_path}"; then
    echo "[SUCCESS] Stream resource budget sample executed successfully."
else
    echo "[FAILURE] Stream resource budget sample did not print the expected success marker."
    exit 1
fi
