...
## Description
This sample demonstrates synchronous memory copy from Host to Device using the `aclrtMemcpy` API for data transfer.
...
```bash
cd ${git_clone_path}/example/1_basic_features/memory/1_h2d_sync_memory_copy
```
...
- Data Transfer
    - Call `aclrtMemcpy` to implement Host-to-Device data transfer using memory copy.
