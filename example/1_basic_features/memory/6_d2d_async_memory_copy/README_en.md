...
## Description
This sample demonstrates asynchronous memory copy within a Device using the `aclrtMemcpyAsync` API for data transfer.
...
```bash
cd ${git_clone_path}/example/1_basic_features/memory/6_d2d_async_memory_copy
```
...
- Data Transfer
    - Call `aclrtMemcpyAsync` to implement Device-to-Device data transfer using memory copy.
