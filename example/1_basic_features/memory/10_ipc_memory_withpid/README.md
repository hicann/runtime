...
## 描述
本样例展示同一个 Device、两个进程间通过 IPC 共享 Device 内存，并在共享内存时启用进程白名单校验。进程 B 先提供自身进程 ID，进程 A 将进程 B 加入 IPC 共享内存白名单后导出共享内存 key，进程 B 再通过 key 导入共享内存。
...
本样例会由 `run.sh` 同时启动 `proc_a` 和 `proc_b` 两个进程，并通过 `file/` 目录下的临时文件交换进程 ID、共享内存 key 和完成标志。
...
- 内存管理
    ...
    - 调用 `aclrtIpcMemSetImportPid` 接口设置 IPC 共享内存的进程白名单。
