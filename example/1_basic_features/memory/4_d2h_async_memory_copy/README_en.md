...
## Description
This sample demonstrates asynchronous memory copy from Device to Host using the `aclrtMemcpyAsync` API for data transfer.
...
```bash
cd ${git_clone_path}/example/1_basic_features/memory/4_d2h_async_memory_copy
```
...
- Data Transfer
    - Call `aclrtMemcpyAsync` to implement Device-to-Host data transfer using memory copy.
