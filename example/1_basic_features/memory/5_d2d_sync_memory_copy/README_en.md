# 5_d2d_sync_memory_copy

## Description
This sample demonstrates synchronous memory copy within a Device using the `aclrtMemcpy` API for data transfer.
...
```bash
cd ${git_clone_path}/example/1_basic_features/memory/5_d2d_sync_memory_copy
```
...
- Data Transfer
    - Call `aclrtMemcpy` to implement Device-to-Device data transfer using memory copy.
