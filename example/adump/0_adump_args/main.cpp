...
int main()
{
    ...
    // The dump path for dump tensor args data
    const char* dumpPath = "/tmp";
    INFO_LOG("The dump data path is %s.", dumpPath);
    aclrtStream stream = nullptr;

    // 1. AscendCL Init
    Init(deviceId, &stream, dumpCfgPath);
    // Enable Dump Args Function
    CHECK_ERROR(aclopStartDumpArgs(ACL_OP_DUMP_OP_AICORE_ARGS, dumpPath));
    ...
    CHECK_ERROR(aclopStopDumpArgs(ACL_OP_DUMP_OP_AICORE_ARGS));
