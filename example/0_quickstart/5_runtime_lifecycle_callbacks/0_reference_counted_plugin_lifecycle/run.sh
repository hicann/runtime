...
env_script="${EXAMPLE_DIR}/set_sample_env.sh"
...
if [[ -z "${ASCEND_HOME_PATH:-}" || -z "${SOC_VERSION:-}" || -z "${ASCENDC_CMAKE_DIR:-}" ]]; then
...
if [[ -z "${ASCEND_HOME_PATH:-}" || -z "${SOC_VERSION:-}" || -z "${ASCENDC_CMAKE_DIR:-}" ]]; then
    echo "[ERROR]: ASCEND_HOME_PATH, SOC_VERSION and ASCENDC_CMAKE_DIR must be set."
    exit 1
fi
cd "${SCRIPT_DIR}"
