...
本文介绍Dump数据落盘后的文件目录、转换方法和常见字段含义。Dump配置接口的调用顺序和参数约束请参见[Dump配置](../../../docs/zh/api_ref/18_dump_configuration.md)。

## 文件路径

Dump文件目录由Dump类型、配置和环境变量共同决定：

- 模型Dump、单算子数据Dump和溢出算子Dump的文件目录由配置文件中的`dump_path`指定，环境变量不覆盖该路径。
- 算子信息统计文件由`aclopStartDumpArgs(uint32_t dumpType, const char *path)`的`path`参数指定，该接口输出算子信息文件，不输出tensor数据文件。
- 可调用`acldumpGetPath(acldumpType dumpType)`查询当前Dump类型的输出路径。
- 异常算子Dump的根路径为`<dumpPath>/extra-info/data-dump/<deviceId>/`，其路径优先级为`ASCEND_DUMP_PATH` > `ASCEND_WORK_PATH` > 配置文件`dump_path` > 当前目录`./`，也可调用`acldumpGetExceptionInfoPath(char *path, size_t maxLen)`查询。

常见Dump类型：

| 类型 | 说明 | 路径定位方式 |
| --- | --- | --- |
| 模型Dump | 导出模型中算子的输入/输出数据。 | 配置文件`dump_path`。 |
| 单算子数据Dump | 导出单算子的输入/输出tensor数据。 | 配置文件`dump_path`。 |
| 算子信息统计 | 导出算子信息文件，包含算子类型、属性、输入输出format、数据类型和shape等信息。 | `aclopStartDumpArgs`的`path`。 |
| 溢出算子Dump | 导出溢出算子的输入和输出数据。 | 配置文件`dump_path`。 |
| 异常算子Dump | 导出异常算子的输入输出、workspace、Tiling等信息。 | `ASCEND_DUMP_PATH`、`ASCEND_WORK_PATH`、配置文件`dump_path`或当前目录`./`下的`<dumpPath>/extra-info/data-dump/<deviceId>/`。 |

## 解析与转换

将tensor Dump文件转换为便于查看的格式，以下以numpy为例：
...
异常算子Dump或自定义tensor落盘场景中，`acldumpTensorInfo`的关键字段含义如下。
...
| `shapeNum` | `shape`的有效维度个数。 | 不得超过25，否则接口返回`ACL_ERROR_INVALID_PARAM`；未设置或为0时，落盘文件中当前shape为空。 |
| `originShapeNum` | `originShape`的有效维度个数。 | 不得超过25；动态shape或格式转换场景下需与`originShape`中的有效维度保持一致。 |
...
- 先根据路径优先级、`dump_path`或接口返回路径确认文件是否生成，再选择需要转换和查看的文件。
